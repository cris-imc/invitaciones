...
export function ModernoTemplate({ invitation, guest, isPersonalized = false }: ModernoTemplateProps) {
  ...
  return (
    <>
      ...
      {!isCoverOpen && (
        <div 
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#0F0E13', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden' }}
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#0F0E13] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-[#FAF8F5] border border-[#C9A876]/20 rounded-xl p-5 sm:p-7 mb-6 shadow-sm transition-all hover:shadow-md">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl font-bold text-[#FFFFFF] mb-4" style={{ fontFamily: 'var(--font-cormorant), serif' }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-[#FAF8F5] border border-[#C9A876]/20 rounded-xl p-5 sm:p-7 mb-10 shadow-sm transition-all hover:shadow-md">
                <span className="font-sans text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C9A876] block mb-3">
                  Fiesta / Salón
                </span>
                {lugarNombre && (
                ...
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex items-center py-5 border-b border-[#C9A876]/30 last:border-b-0">
                      {item.time && (
                        <span className="font-sans text-lg sm:text-xl text-[#C9A876]/90 font-light tracking-widest w-28 sm:w-32 flex-shrink-0">
                          {item.time}
                        </span>
                      )}
                      <span className="font-sans text-[1.1rem] sm:text-xl text-[#FFFFFF] font-normal">
                        {item.title}
                      </span>
                    </div>
                  ))}
