...
Esto es la base técnica de **todas** las plantillas de este pack. No es negociable porque así se garantiza que después se pueda instalar en el sistema real sin romper nada: (mas que nada tomar en cuenta Doodle Wedding standalone.html creado en este pack)
...
1. **Prompt #1 para ChatGPT Image en ingles** (para generar el moodboard) — basado en tu input original, con la paleta(explicitar colores en HEX, mood y motivos de ilustración ya traducidos a instrucciones visuales concretas. Es innegociable, que el resultado esperado NO DEBA TENER imagenes de personas, SIN ningun tipo de fotos, solo elementos visuales FLAT, iconografia, doodles que se usaran a en la invitacion web.
2. **Prompt #2 para Claude design en inglés**
...
- Entrego el prompt ya completo (sin corchetes) listo para pegar en Claude design, respetando punto por punto la arquitectura rígida de arriba. Este prompt debe Incluir una línea explícita aclarando que se le va a adjuntar un moodboard (el que generaste con el prompt #1) como referencia visual principal para paleta, tipografía y estilo de ilustración y tambien se adjuntara (opcionalmente) el link de una plantilla de invitaciones que solo servirá de referencia para animar iconos, agregar y ver diseño y complejidad de doodles y efectos en fotografias, solo de inspiracion.
