...
/**
 * BlackAndWhiteTemplateNegativo.tsx
 *
...
interface BlackAndWhiteTemplateNegativoProps {
...
export function BlackAndWhiteTemplateNegativo({ invitation, guest, isPersonalized = false }: BlackAndWhiteTemplateNegativoProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#0A0A0A";
      }
      if (stubRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#B0B0B0" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#B0B0B0" : "#B9B9BC";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#F4F1EA", tinta: "#0A0A0A", acento: "#B9B9BC", clase: `${bcwBodoni.variable} ${bcwMono.variable}`, fuente: "var(--bcw-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="bcw-scroller">
            ...
            <div className="bcw-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="10,10,10" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="10,10,10" />
                </div>
              )}
            </div>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="bcw-section" style={{ background: "#FFFFFF" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="bcw-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="bcw-pan-sticky">
            <div data-strip="1" className="bcw-strip">
              ...
              <div data-tone="dark" className="bcw-panel bcw-panel--center" style={{ background: "#FFFFFF", color: "#0A0A0A" }}>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="bcw-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="bcw-pan-sticky">
            <div data-strip="1" className="bcw-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="bcw-panel bcw-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="bcw-seguir bcw-seguir--split">
                    ...
                    <span className="bcw-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="bcw-section" style={{ background: "#FFFFFF" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="bcw-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="bcw-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#0A0A0A" : "#B9B9BC" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="bcw-section" style={{ background: "#FFFFFF" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="bcw-section bcw-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "#FFFFFF" }}>
      ...
      <div ref={coverRef} className="bcw-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
    ...
    <div className="bcw-cover-inner">
      ...
      <div className="bcw-cover-content">
        ...
        <div className="bcw-cover-bottom">
          <div ref={perfRef} className="bcw-perf-strip bcw-perf-strip--reveal bcw-perf-strip--cover" />
...
const GP_CSS = `
  ...
  .bcw-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #B0B0B0, #D4D4D4, #B9B9BC, #0A0A0A, #B0B0B0); filter: saturate(.75); }
  .bcw-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #F0EDE4; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .bcw-medallion-label { font-family: var(--bcw-bodoni), serif; font-size: 26px; line-height: 1; color: #0A0A0A; }
  .bcw-medallion-label-sm { font-family: var(--bcw-bodoni), serif; font-size: 16px; color: #0A0A0A; }
  ...
  .bcw-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #0A0A0A, transparent); animation: bcwScan 6s linear infinite; pointer-events: none; }

  .bcw-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .bcw-cd-box { border: 1px solid #C9C9C9; background: rgba(244,241,234,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .bcw-cd-num { font-family: var(--bcw-bodoni), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #0A0A0A; display: inline-block; }
  .bcw-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B9B9BC; }
  .bcw-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #F4F1EA 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .bcw-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .bcw-perf-strip--cover { opacity: .9; }
  ...
  .bcw-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .bcw-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  ...
  .bcw-stub { position: relative; overflow: hidden; border: 1px solid #C9C9C9; background: rgba(244,241,234,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .bcw-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #0A0A0A, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .bcw-rsvp-row > span:last-child { color: #0A0A0A; letter-spacing: 0.02em; text-align: right; }
  ...
  .bcw-rsvp-payment-total { color: #0A0A0A; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .bcw-rsvp-stepper span { font-family: var(--bcw-mono), monospace; color: #0A0A0A; min-width: 20px; text-align: center; }
  .bcw-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(185,185,188,0.3); color: #0A0A0A; font-family: var(--bcw-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .bcw-rsvp-input::placeholder { color: #6E6A5D; }
  .bcw-rsvp-input:focus { outline: none; border-bottom-color: #B9B9BC; }
  .bcw-rsvp-btn { width: 100%; padding: 16px; font-family: var(--bcw-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B9B9BC, #B0B0B0); border: 1px solid #B9B9BC; color: #0A0A0A; cursor: pointer; }
  ...
  .bcw-bank-row-value { font-size: 12px; color: #0A0A0A; word-break: break-all; }
  ...
  .bcw-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(185,185,188,0.3); color: #0A0A0A; font-family: var(--bcw-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  ...
  .bcw-song-submit:hover { color: #0A0A0A; }
  ...
  .bcw-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #0A0A0A; }
  ...
  .bcw-quiz-q-text { margin: 0; font-family: var(--bcw-bodoni), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #0A0A0A; }
  ...
  .bcw-quiz-result-score { margin: 0 0 6px; font-family: var(--bcw-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #0A0A0A; }
  ...
  .bcw-final-names { font-family: var(--bcw-bodoni), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #0A0A0A; }
  ...
  .bcw-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#0A0A0A, #B9B9BC); transition: height 260ms linear; display: block; }
  ...
  .bcw-cover-frame--outer { inset: 26px; border: 1px solid rgba(20,20,27,.35); }
  .bcw-cover-frame--inner { inset: 32px; border: 1px solid rgba(20,20,27,.12); }
  ...
  .bcw-cover-names { margin: 0; font-family: var(--bcw-bodoni), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #0A0A0A; }
  ...
  .bcw-cover-cta { border: 1px solid #B9B9BC; background: linear-gradient(100deg, rgba(185,185,188,0.08), rgba(255,255,255,0.2), rgba(185,185,188,0.08)); color: #0A0A0A; font-family: var(--bcw-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .bcw-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B9B9BC; background: rgba(0,0,0,0.4); color: #0A0A0A; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
