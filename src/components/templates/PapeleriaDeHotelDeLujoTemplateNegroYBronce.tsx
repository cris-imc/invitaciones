...
/**
 * PapeleriaDeHotelDeLujoTemplateNegroYBronce.tsx
 *
...
export function PapeleriaDeHotelDeLujoTemplateNegroYBronce({ invitation, guest, isPersonalized = false }: PapeleriaDeHotelDeLujoTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E8C9A0";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C1793E";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(193,121,62,.35), 0 18px 50px -30px #C1793E";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#7A4420" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(193,121,62,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(193,121,62,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#7A4420" : "#C1793E";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0B0B0A", tinta: "#F4F1EA", acento: "#C1793E", clase: `${phlPlayfair.variable} ${phlMono.variable}`, fuente: "var(--phl-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="phl-scroller">
            ...
            <div className="phl-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="11,11,10" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="11,11,10" />
                </div>
              )}
            </div>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="phl-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #211D19 0%, #0C0B11 52%, #0B0B0A 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="phl-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="phl-pan-sticky">
            <div data-strip="1" className="phl-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="phl-panel phl-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="phl-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#7A4420" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#7A4420" />
                  </svg>
              ...
              <div data-tone="dark" className="phl-panel phl-panel--center" style={{ background: "#141210", color: "#F4F1EA" }}>
            ...
            <div data-strip="1" className="phl-strip">
              ...
              <div data-tone="light" className="phl-panel phl-panel--gap" style={{ background: "#EDE8DE", color: "#14141B" }}>
                ...
                <div className="phl-album-embed">
                  {livePhotos.length > 0 ? (
                    <LiveAlbumStrip photos={livePhotos} tone="light" accentColor="#7A4420" />
                  ) : (
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="phl-section" style={{ background: "#141210" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="phl-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="phl-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E8C9A0" : "#C1793E" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="phl-section" style={{ background: "#141210" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C1793E" guest={guest} />
      )}
...
const PHL_CSS = `
  ...
  a { color: #C1793E; text-decoration: none; }
  ...
  .phl-date-month { font-family: var(--phl-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C1793E; padding-left: 12%; }

  .phl-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .phl-divider-line { width: 52px; height: 1px; background: #C1793E; display: inline-block; }
  ...
  .phl-cal-link:hover { color: #C1793E; }
  ...
  .phl-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2B2016, #3D2D1C, #C1793E, #E8C9A0, #2B2016); filter: saturate(.75); }
  ...
  .phl-medallion-label { font-family: var(--phl-playfair), serif; font-size: 26px; line-height: 1; color: #E8C9A0; }
  .phl-medallion-label-sm { font-family: var(--phl-playfair), serif; font-size: 16px; color: #E8C9A0; }
  .phl-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .phl-medallion-sub--accent { color: #C1793E; }
  ...
  .phl-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C1793E; opacity: 0.4; font-family: var(--phl-mono), monospace; }

  .phl-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(193,121,62,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .phl-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E8C9A0, transparent); animation: phlScan 6s linear infinite; pointer-events: none; }

  .phl-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .phl-cd-box { border: 1px solid #2E1B0E; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .phl-cd-num { font-family: var(--phl-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F2DCC0; display: inline-block; }
  .phl-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C1793E; }
  .phl-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0B0B0A 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .phl-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(43,32,22,0.5), rgba(61,45,28,0.3), rgba(193,121,62,0.4), rgba(43,32,22,0.5)); filter: blur(80px); opacity: .4; animation: phlFoil 30s linear infinite; }
  ...
  .phl-accent-italic { font-style: italic; color: #C1793E; }
  .phl-accent-serif { font-style: italic; color: #7A4420; font-family: var(--phl-playfair), serif; }
  .phl-accent-serif-2 { color: #7A4420; }
  ...
  .phl-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #7A4420; margin-top: auto; }
  ...
  .phl-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #7A4420; }
  ...
  .phl-crono-time { font-family: var(--phl-mono), monospace; color: #7A4420; min-width: 42px; }
  ...
  .phl-stub { position: relative; overflow: hidden; border: 1px solid #2E1B0E; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .phl-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E8C9A0, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .phl-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(193,121,62,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .phl-rsvp-payment-total { color: #E8C9A0; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .phl-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C1793E; background: transparent; color: #C1793E; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .phl-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(193,121,62,0.3); color: #F4F1EA; font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .phl-rsvp-input::placeholder { color: #6E6A5D; }
  .phl-rsvp-input:focus { outline: none; border-bottom-color: #C1793E; }
  .phl-rsvp-btn { width: 100%; padding: 16px; font-family: var(--phl-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C1793E, #8F5A2A); border: 1px solid #C1793E; color: #0B0B10; cursor: pointer; }
  .phl-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .phl-rsvp-btn--ghost { background: transparent; color: #C1793E; }
  .phl-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C1793E; margin: 0; }
  ...
  .phl-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(193,121,62,0.18); }
  ...
  .phl-bank-copy { flex-shrink: 0; font-family: var(--phl-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C1793E; background: transparent; color: #C1793E; cursor: pointer; }
  .phl-bank-copy:hover { background: rgba(193,121,62,0.12); }
  ...
  .phl-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(193,121,62,0.3); padding-bottom: 12px; }
  ...
  .phl-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(193,121,62,0.3); color: #F4F1EA; font-family: var(--phl-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .phl-song-input::placeholder { color: #6E6A5D; }
  .phl-song-input:focus { outline: none; border-bottom-color: #C1793E; }
  .phl-song-sep { color: #8A8577; flex-shrink: 0; }
  .phl-song-submit { flex-shrink: 0; background: none; border: none; color: #C1793E; font-family: var(--phl-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .phl-song-submit:hover { color: #E8C9A0; }
  .phl-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .phl-song-error { font-size: 10px; color: #C1793E; margin-top: 6px; }
  ...
  .phl-quiz-opt { font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(193,121,62,0.4); background: transparent; color: #C1793E; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .phl-quiz-opt:disabled { cursor: default; }
  .phl-quiz-opt--picked { background: #C1793E; border-color: #C1793E; color: #0B0B10; }
  .phl-quiz-opt--correct { background: #C1793E; border-color: #C1793E; color: #0B0B10; }
  .phl-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .phl-quiz-result { padding-top: 18px; border-top: 1px solid rgba(193,121,62,0.2); }
  .phl-quiz-result-score { margin: 0 0 6px; font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E8C9A0; }
  .phl-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .phl-final-card { border: 1px solid #C1793E; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .phl-final-names { font-family: var(--phl-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E8C9A0; }
  ...
  .phl-replay { cursor: pointer; color: #C1793E; }
  .phl-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .phl-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(193,121,62,0.14); }
  .phl-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .phl-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(193,121,62,0.16); position: relative; }
  .phl-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E8C9A0, #C1793E); transition: height 260ms linear; display: block; }
  .phl-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C1793E; transition: color 500ms ease; }
  ...
  .phl-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #181410 0%, #141210 46%, #0B0B0A 100%); }
  .phl-cover-half--bottom .phl-cover-inner { top: auto; bottom: 0; }
  .phl-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(43,32,22,0.7), rgba(61,45,28,0.4), rgba(193,121,62,0.6), rgba(43,32,22,0.7)); filter: blur(64px); opacity: .62; animation: phlFoil 26s linear infinite; }
  .phl-cover-sunburst { position: absolute; inset: 0; background-image: repeating-conic-gradient(from 0deg at 50% 50%, rgba(193,121,62,.05) 0deg 1deg, transparent 1deg 3deg); }
  ...
  .phl-cover-names { margin: 0; font-family: var(--phl-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E8C9A0; }
  .phl-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C1793E, transparent); display: block; }
  ...
  .phl-cover-cta { border: 1px solid #C1793E; background: linear-gradient(100deg, rgba(193,121,62,0.08), rgba(232,201,160,0.2), rgba(193,121,62,0.08)); color: #F2DCC0; font-family: var(--phl-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .phl-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .phl-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,201,160,0.34), rgba(246,239,221,0.5), rgba(232,201,160,0.34)); color: #0B0B0F; }
  }
  ...
  .phl-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(10,9,8,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .phl-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C1793E; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
