...
/**
 * GraduacionTemplatePlata.tsx
 *
 * Portado 1:1 desde el mockup "Graduacion Nueva Etapa" (mockup/Eventos),
...
const grdDisplay = Playfair_Display({
  ...
  style: ["normal", "italic"],
  weight: ["400", "500"],
  variable: "--grd-display",
  display: "swap",
});

const grdMono = IBM_Plex_Mono({
  subsets: ["latin"],
  preload: false,
  weight: ["300", "400", "500"],
  variable: "--grd-mono",
...
interface GraduacionTemplatePlataProps {
...
export function GraduacionTemplatePlata({ invitation, guest, isPersonalized = false }: GraduacionTemplatePlataProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#EFF3F5";
      }
      if (stubRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#5A6D78" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#5A6D78" : "#B6C4CF";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A0D18", tinta: "#F4F1EA", acento: "#B6C4CF", clase: `${grdDisplay.variable} ${grdMono.variable}`, fuente: "var(--grd-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${grdDisplay.variable} ${grdMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#0A0D18",
        fontFamily: "var(--grd-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{GP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="grd-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="grd-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #0D111E 55%, #0A0D18 100%)" }}>
          ...
          <div className="grd-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="grd-date-num grd-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="grd-divider">
            <span className="grd-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="grd-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="grd-cal-link"
          />
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="grd-section grd-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #1A2438 0%, #0D0B10 55%, #0A0D18 100%)" }}>
          <div className="grd-scan-grid" />
          ...
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="grd-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #0A0D18 100%)" }}>
            ...
            <h2 ref={phraseRef} className="grd-phrase" style={{ fontSize: fraseFontSize }}>
              {fraseWords.map((w, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= fraseAccentStart ? "grd-accent-italic" : undefined}>
                    {w}
                  </span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="120" data-dist="90" className="grd-divider" style={{ position: "relative" }}>
              <span className="grd-divider-line grd-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
            </div>
          </section>
        )}
          ...
          <div className="grd-pan-sticky">
            <div data-strip="1" className="grd-strip">
              <div id="details" data-tone="light" className="grd-panel grd-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="grd-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="grd-crono-row">
                        <span className="grd-crono-time">{item.time || ""}</span>
                        <span className="grd-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="grd-panel grd-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="grd-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#5A6D78" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#5A6D78" />
                  </svg>
            ...
            <GrdDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="grd-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #0D111E 60%, #0A0D18 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="grd-rsvp">
              <GrdRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="grd-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="grd-pan-sticky">
            <div data-strip="1" className="grd-strip">
                ...
                <div key={pageIndex} data-tone="light" className="grd-panel grd-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="grd-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`grd-mosaic-cell${i === 0 ? " grd-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="grd-seguir grd-seguir--split">
                    ...
                    <span className="grd-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <GrdDots count={photoPages.length} />
          </div>
        </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="grd-section" style={{ background: "#0D111E" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="grd-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="grd-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#EFF3F5" : "#B6C4CF" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="grd-song-wrap">
              <GrdSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="grd-section grd-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #0D111E 55%, #0A0D18 100%)" }}>
          ...
          <div className="grd-footer-credit">
      ...
      <div ref={railRef} className="grd-rail">
        ...
        <span ref={railLabelRef} className="grd-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="grd-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="grd-cover-half grd-cover-half--top">
          <GrdCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="grd-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="grd-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
...
function GrdCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="grd-cd-box">
      <span ref={refEl} className="grd-cd-num">—</span>
      <span className="grd-cd-label">{label}</span>
    </div>
  );
}

function GrdDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="grd-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="grd-dot" />
      ))}
    </div>
  );
}

function GrdMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="grd-medallion-arc" style={{ animation: spin === "reverse" ? "grdRingRev 32s linear infinite" : "grdRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="grd-medallion-arc-text">
...
function GrdCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="grd-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="grd-bank-row-label">{label}</span>
    ...
    <>
      <div className="grd-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="grd-rsvp-row grd-rsvp-row--payment">
      ...
      <div ref={stubRef} className="grd-stub">
        ...
        <div ref={sealRef} className="grd-seal">
          <GrdMedallionCmp label="GR" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="grd-beam" />
        <div className="grd-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="grd-rsvp-error">{error}</p>}
...
function GrdSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="grd-song">
      ...
      {error && <p className="grd-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="grd-song-list">
...
function GrdQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: GrdQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="grd-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="grd-quiz-q">
            <span className="grd-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="grd-quiz-q-text">{q.pregunta}</p>
            <div className="grd-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " grd-quiz-opt--correct";
                  else if (chosen) stateClass = " grd-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " grd-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`grd-quiz-opt${stateClass}`}
...
function GrdInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="grd-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="grd-bank-row-label">{label}</span>
    ...
    <div className="grd-cover-inner">
      ...
      <div className="grd-cover-content">
        ...
        <div className="grd-cover-center">
          ...
          <h1 ref={namesRef} className="grd-cover-names">{namesTitle}</h1>
          <span className="grd-cover-rule" />
          <span className="grd-cover-date">{fechaCorta}</span>
        </div>
        <div className="grd-cover-bottom">
          <div ref={perfRef} className="grd-perf-strip grd-perf-strip--reveal grd-perf-strip--cover" />
          <div className="grd-cover-facts">
...
// ---------------------------------------------------------------------
const GP_CSS = `
  .grd-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .grd-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #B6C4CF; text-decoration: none; }
  a:hover { color: #EFF3F5; }

  @keyframes grdFoil { to { transform: rotate(360deg); } }
  @keyframes grdRing { to { transform: rotate(360deg); } }
  @keyframes grdRingRev { to { transform: rotate(-360deg); } }
  @keyframes grdScan { 0% { transform: translateY(0); opacity: 0; } 12% { opacity: .85; } 88% { opacity: .85; } 100% { transform: translateY(320px); opacity: 0; } }
  @keyframes grdEq { 0%,100% { transform: scaleY(.25); } 50% { transform: scaleY(1); } }
  @keyframes grdHint { 0%,100% { transform: translateY(0); } 50% { transform: translateY(7px); } }
  @keyframes grdSide { 0%,100% { transform: translateX(0); } 50% { transform: translateX(7px); } }
  @media (prefers-reduced-motion: reduce) { .grd-scroller * { animation: none !important; } }

  .grd-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .grd-section--between { justify-content: space-between; }
  ...
  .grd-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0A0D18; }
  .grd-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  .grd-hero-photo-kicker { position: absolute; left: 0; right: 0; bottom: 0; z-index: 2; padding: 0 max(24px, calc((100% - 560px) / 2)) 48px; }
  ...
  @media (max-width: 767px) {
    .grd-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .grd-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(200,164,92,.3); }
    .grd-hero-photo-kicker { bottom: 40px; }
    .grd-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .grd-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .grd-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .grd-date-num { font-family: var(--grd-display), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .grd-date-num--right { text-align: right; line-height: 0.86; }
  .grd-date-month { font-family: var(--grd-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B6C4CF; padding-left: 12%; }

  .grd-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .grd-divider-line { width: 52px; height: 1px; background: #B6C4CF; display: inline-block; }
  .grd-divider-line--long { width: 64px; }

  .grd-lead { margin: 0; font-family: var(--grd-display), serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .grd-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--grd-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .grd-cal-link:hover { color: #B6C4CF; }

  .grd-medallion { position: relative; }
  .grd-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .grd-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .grd-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .grd-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #B6C4CF, #EFF3F5, #6E5AA8); filter: saturate(.75); }
  .grd-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .grd-medallion-label { font-family: var(--grd-display), serif; font-size: 26px; line-height: 1; color: #EFF3F5; }
  .grd-medallion-label-sm { font-family: var(--grd-display), serif; font-size: 16px; color: #EFF3F5; }
  .grd-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .grd-medallion-sub--accent { color: #B6C4CF; }
  .grd-medallion-arc { position: absolute; inset: -14%; }
  ...
  .grd-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B6C4CF; opacity: 0.4; font-family: var(--grd-mono), monospace; }

  .grd-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .grd-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #EFF3F5, transparent); animation: grdScan 6s linear infinite; pointer-events: none; }

  .grd-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .grd-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .grd-cd-num { font-family: var(--grd-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #EFF3F5; display: inline-block; }
  .grd-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B6C4CF; }
  .grd-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A0D18 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .grd-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .grd-perf-strip--cover { opacity: .9; }

  .grd-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(200,164,92,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: grdFoil 30s linear infinite; }
  .grd-phrase { margin: 0; position: relative; font-family: var(--grd-display), serif; font-weight: 400; font-size: clamp(50px, 15vw, 96px); line-height: 0.92; letter-spacing: -0.03em; }
  .grd-accent-italic { font-style: italic; color: #B6C4CF; }
  .grd-accent-serif { font-style: italic; color: #5A6D78; font-family: var(--grd-display), serif; }
  .grd-accent-serif-2 { color: #5A6D78; }
  .grd-h2 { margin: 0; font-family: var(--grd-display), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .grd-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .grd-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .grd-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .grd-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .grd-panel--between { justify-content: space-between; }
  ...
  .grd-pan[data-scroll="vertical"] { height: auto; }
  .grd-pan[data-scroll="vertical"] .grd-pan-sticky { position: static; height: auto; overflow: visible; }
  .grd-pan[data-scroll="vertical"] .grd-strip { position: static; display: block; width: 100%; transform: none !important; }
  .grd-pan[data-scroll="vertical"] .grd-panel { height: calc(var(--vh, 1vh) * 100); }
  .grd-panel--end { justify-content: flex-end; }
  .grd-panel--center { align-items: center; justify-content: center; text-align: center; }
  .grd-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .grd-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .grd-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .grd-panel-title { margin: 0; position: relative; font-family: var(--grd-display), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .grd-panel-title-md { margin: 0; position: relative; font-family: var(--grd-display), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .grd-panel-title-sm { margin: 0; font-family: var(--grd-display), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .grd-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .grd-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .grd-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  .grd-facts { position: relative; display: flex; flex-direction: column; gap: 12px; font-size: 11px; letter-spacing: 0.14em; color: #4A473F; }
  .grd-facts-row { display: flex; justify-content: space-between; border-bottom: 1px solid #D6D1C4; padding-bottom: 10px; }
  .grd-facts-row--last { border-bottom: none; padding-bottom: 0; }
  .grd-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #5A6D78; margin-top: auto; }
  .grd-seguir--split { justify-content: space-between; }
  .grd-side-hint { display: inline-block; animation: grdSide 2.2s ease-in-out infinite; }
  .grd-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .grd-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #5A6D78; }

  .grd-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .grd-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .grd-crono-time { font-family: var(--grd-mono), monospace; color: #5A6D78; min-width: 42px; }
  .grd-crono-title { flex: 1; }

  .grd-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .grd-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .grd-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .grd-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .grd-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .grd-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .grd-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #EFF3F5, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .grd-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--grd-mono), monospace; }
  .grd-rsvp-rows { display: flex; flex-direction: column; }
  .grd-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .grd-rsvp-row > span:first-child { flex-shrink: 0; }
  .grd-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .grd-rsvp-row--payment { align-items: flex-start; }
  .grd-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .grd-rsvp-payment-total { color: #EFF3F5; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .grd-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .grd-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .grd-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B6C4CF; background: transparent; color: #B6C4CF; font-size: 14px; line-height: 1; cursor: pointer; }
  .grd-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .grd-rsvp-stepper span { font-family: var(--grd-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .grd-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--grd-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .grd-rsvp-input::placeholder { color: #6E6A5D; }
  .grd-rsvp-input:focus { outline: none; border-bottom-color: #B6C4CF; }
  .grd-rsvp-btn { width: 100%; padding: 16px; font-family: var(--grd-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B6C4CF, #5A6D78); border: 1px solid #B6C4CF; color: #0D111E; cursor: pointer; }
  .grd-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .grd-rsvp-btn--ghost { background: transparent; color: #B6C4CF; }
  .grd-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B6C4CF; margin: 0; }
  .grd-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .grd-rsvp-declined-text { margin: 0; font-family: var(--grd-display), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .grd-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .grd-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .grd-photo-cell--offset { margin-top: 18px; }
  .grd-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .grd-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .grd-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .grd-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .grd-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .grd-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .grd-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .grd-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .grd-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  ...
  .grd-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .grd-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  .grd-bank-row:last-child { border-bottom: none; }
  .grd-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .grd-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .grd-bank-copy { flex-shrink: 0; font-family: var(--grd-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B6C4CF; background: transparent; color: #B6C4CF; cursor: pointer; }
  .grd-bank-copy:hover { background: rgba(200,164,92,0.12); }

  .grd-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .grd-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: grdEq 1.1s ease-in-out infinite; display: inline-block; }
  .grd-song-wrap { font-family: var(--grd-mono), monospace; }

  .grd-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .grd-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  .grd-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .grd-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--grd-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .grd-song-input::placeholder { color: #6E6A5D; }
  .grd-song-input:focus { outline: none; border-bottom-color: #B6C4CF; }
  .grd-song-sep { color: #8A8577; flex-shrink: 0; }
  .grd-song-submit { flex-shrink: 0; background: none; border: none; color: #B6C4CF; font-family: var(--grd-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .grd-song-submit:hover { color: #EFF3F5; }
  .grd-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .grd-song-error { font-size: 10px; color: #B6C4CF; margin-top: 6px; }
  .grd-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .grd-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--grd-mono), monospace; }
  .grd-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .grd-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .grd-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .grd-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .grd-quiz-q-num { font-family: var(--grd-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .grd-quiz-q-text { margin: 0; font-family: var(--grd-display), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .grd-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .grd-quiz-opt { font-family: var(--grd-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #B6C4CF; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .grd-quiz-opt:disabled { cursor: default; }
  .grd-quiz-opt--picked { background: #B6C4CF; border-color: #B6C4CF; color: #0D111E; }
  .grd-quiz-opt--correct { background: #B6C4CF; border-color: #B6C4CF; color: #0D111E; }
  .grd-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .grd-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .grd-quiz-result-score { margin: 0 0 6px; font-family: var(--grd-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #EFF3F5; }
  .grd-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .grd-final-card { border: 1px solid #B6C4CF; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .grd-final-names { font-family: var(--grd-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #EFF3F5; }
  .grd-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .grd-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .grd-replay { cursor: pointer; color: #B6C4CF; }
  .grd-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .grd-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .grd-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .grd-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .grd-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#EFF3F5, #B6C4CF); transition: height 260ms linear; display: block; }
  .grd-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B6C4CF; transition: color 500ms ease; }

  .grd-cover { position: absolute; inset: 0; z-index: 5; }
  .grd-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .grd-cover-half--top { top: 0; }
  .grd-cover-half--bottom { bottom: 0; }
  .grd-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #141A28 0%, #0D111E 46%, #0A0D18 100%); }
  .grd-cover-half--bottom .grd-cover-inner { top: auto; bottom: 0; }
  .grd-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(28,43,74,0.55), rgba(182,196,207,0.32), rgba(182,196,207,0.55), rgba(28,43,74,0.55)); filter: blur(64px); opacity: .62; animation: grdFoil 26s linear infinite; }
  .grd-cover-texture { position: absolute; inset: 0; background-image: repeating-linear-gradient(90deg, rgba(239,243,245,.06) 0 1px, transparent 1px 40px); }
  .grd-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .grd-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .grd-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .grd-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .grd-cover-names { margin: 0; font-family: var(--grd-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EFF3F5; }
  .grd-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B6C4CF, transparent); display: block; }
  .grd-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .grd-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .grd-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  ...
  .grd-cover-facts:has(> span:only-child) { justify-content: center; }
  .grd-cover-cta { border: 1px solid #B6C4CF; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--grd-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .grd-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .grd-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0D111E; }
  }
  .grd-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .grd-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: grdHint 2.4s ease-in-out infinite; }

  .grd-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .grd-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B6C4CF; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .grd-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
