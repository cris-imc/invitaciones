...
/**
 * AtelierDePapelTemplateBorgonaVino.tsx
 *
 * Portado 1:1 desde el diseño "Atelier de Papel" aprobado en Claude Design.
 * Variante de color "Borgoña Vino" (colección Storytelling para boda: mismo
 * fondo casi negro, ahora con velo vino borgoña, acento vino borgoña
 * #9B3B4B en vez del bronce original, Cormorant Garamond + IBM Plex Mono, ticket
 * perforado tipo "lámina" numerada, medallón circular con iniciales de la
 * pareja y texto en arco, scroll horizontal "pineado" para Cuándo y dónde /
 * Álbum, riel lateral de progreso y portada que se abre en dos mitades con
 * una hoja de papel rotada como motivo decorativo -- NUNCA el sunburst de
 * Guest Pass VIP). Todo el motion es CSS + SVG + un loop de scroll propio --
 * cero PNGs, así que escala sin pixelarse en cualquier viewport.
 *
 * Arquitectura idéntica a GuestPassVipTemplate.tsx (mismo motor de scroll/
 * reveal, mismo gesto lateral en paneles pineados, mismo álbum con foto
 * destacada + lightbox + paginación balanceada, mismo LiveAlbumStrip para la
 * sección EN VIVO, misma lista de canciones acotada) -- solo cambian
 * paleta, tipografía, copy y el motivo decorativo. Habla SIEMPRE con los
 * mismos endpoints/datos reales del backend (mismo /api/guests/[token]/confirm
 * que usa RSVPWizardV2, mismo /api/quiz, /api/songs, etc). El destello de
 * sello se dispara al confirmar asistencia, vía el mismo callback
 * onConfirmed que usaban los componentes compartidos.
 *
 * Terminología propia de esta plantilla: el "pase" de Guest Pass VIP se
 * llama "lámina" acá (Atelier de Papel = atelier de láminas/planchas de
 * papel), reflejando el copy real del mockup ("LÁMINA Nº 07", "GUARDÁ TU
 * LÁMINA", "LA PRÓXIMA LÁMINA LLEGA EN").
 */
...
interface GuestRecord {
  ...
  // backend no reserva mesas/sectores, así que la lámina usa este número de
  // orden en vez de inventar una ubicación que no existe.
  orderNumber?: number;
}

interface AtelierDePapelTemplateProps {
  invitation: Record<string, unknown>;
  guest?: GuestRecord | null;
  isPersonalized?: boolean;
}

// El backend no reserva mesas/sectores -- la lámina muestra el orden real
...
// real que mostrar.
function laminaNumberFrom(orderNumber: number | undefined): string {
  if (!orderNumber) return "---";
  return String(orderNumber).padStart(3, "0");
}

// Iniciales de la pareja para el centro del medallón (mockup: "LM" para
// Lucía & Mateo) -- nunca un texto fijo tipo "VIP", siempre derivado de los
// nombres reales cargados en la invitación.
function coupleInitialsFrom(novia: string, novio: string): string {
  const a = novia.trim().charAt(0).toUpperCase();
  const b = novio.trim().charAt(0).toUpperCase();
  const initials = `${a}${b}`.trim();
  return initials || "AP";
}

export function AtelierDePapelTemplateBorgonaVino({ invitation, guest, isPersonalized = false }: AtelierDePapelTemplateProps) {
  const tx = useTextos();
  ...
  // invitado/familia en vez de los novios -- "una lámina nueva para la boda
  // DE Familia Juarez" suena mal, así que el kicker pierde el "de" y el
  // nombre va debajo, como un dato propio (no como si Familia Juarez fuera
  // quien se casa).
  ...
  // de la frase en color plano, segunda mitad en beige itálico. Antes esto
  ...
  const laminaNumber = laminaNumberFrom(guest?.orderNumber);
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F4E6E8";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#9B3B4B";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(155,59,75,.35), 0 18px 50px -30px #9B3B4B";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#2B2620" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(155,59,75,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(155,59,75,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#2B2620" : "#9B3B4B";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#150F10", tinta: "#F4F1EA", acento: "#9B3B4B", clase: `${adpSerif.variable} ${adpMono.variable}`, fuente: "var(--adp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${adpSerif.variable} ${adpMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#150F10",
        fontFamily: "var(--adp-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{ADP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="adp-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="adp-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #231523 0%, #150F10 55%, #150F10 100%)" }}>
          ...
          <div className="adp-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="adp-date-num adp-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="adp-divider">
            ...
            <div className="adp-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="21,15,16" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="21,15,16" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="adp-section adp-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #251D1E 0%, #150F10 55%, #150F10 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="adp-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #231523 0%, #120C12 52%, #150F10 100%)" }}>
          ...
          <h2 ref={phraseRef} className="adp-phrase" style={{ fontSize: fraseFontSize }}>
            {fraseWords.map((w, i) => (
              ...
              <span key={i}>
                <span data-w="1" className={i >= fraseAccentStart ? "adp-phrase-accent" : undefined}>
                  {w}
                </span>{" "}
              </span>
            ))}
          </h2>
          <div data-xin="1" data-delay="120" data-dist="90" className="adp-divider" style={{ position: "relative" }}>
            <span className="adp-divider-line adp-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
          </div>
        </section>
        )}
          ...
          <div className="adp-pan-sticky">
            <div data-strip="1" className="adp-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="adp-panel adp-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <h2 className="adp-panel-title">
                    ...
                    {ceremoniaDireccion && <><br /><span className="adp-accent-serif-loc">{ceremoniaDireccion}</span></>}
                  </h2>
              ...
              <div id="details" data-tone="light" className="adp-panel adp-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="adp-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="adp-crono-row">
                        <span className="adp-crono-time">{item.time || ""}</span>
                        <span className="adp-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="adp-panel adp-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="adp-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#2B2620" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#2B2620" />
                  </svg>
              ...
              <div data-tone="dark" className="adp-panel adp-panel--center" style={{ background: "#150F10", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="adp-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #231523 0%, #150F10 60%, #150F10 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="adp-rsvp">
              <AdpRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="adp-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="adp-pan-sticky">
            <div data-strip="1" className="adp-strip">
                ...
                <div key={pageIndex} data-tone="light" className="adp-panel adp-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="adp-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`adp-mosaic-cell${i === 0 ? " adp-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="adp-seguir adp-seguir--split">
                    ...
                    <span className="adp-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="adp-section" style={{ background: "#150F10" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="adp-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="adp-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F4E6E8" : "#9B3B4B" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="adp-song-wrap">
              <AdpSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="adp-section" style={{ background: "#150F10" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="adp-section" style={{ background: "#150F10" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.sabor.tuLamina")} className="adp-section adp-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #231523 0%, #150F10 55%, #150F10 100%)" }}>
          <span data-xin="1" data-dist="-60" className="adp-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection, quizEnabled].filter(Boolean).length + 6)} — {tx("invitacion.kicker.guarda")} TU LÁMINA</span>
          <div data-xin="1" data-delay="100" data-dist="130" className="adp-final-card">
            ...
            <span className="adp-mini-label" style={{ color: "#A8A292" }}>{fechaCorta} — {hora} H</span>
          ...
          <div className="adp-footer-credit">
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#9B3B4B" guest={guest} />
      )}
...
function CdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="adp-cd-box">
      <span ref={refEl} className="adp-cd-num">—</span>
      <span className="adp-cd-label">{label}</span>
    </div>
  );
}
...
function Dots({ count = 3 }: { count?: number }) {
  ...
  return (
    <div data-dots="1" className="adp-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="adp-dot" />
      ))}
    </div>
  );
}
    ...
    <>
      ...
      <div className="adp-medallion-core">
        ...
        {/* El segundo renglón solo se destaca con el acento bronce cuando hay
            ...
            04") -- en los demás medallones (tapa, lámina final) ese renglón
            es un dato secundario apagado ("ACCESO", "CONFIRMADO"). */}
        {sub && <span className={`adp-medallion-sub${title ? " adp-medallion-sub--accent" : ""}`}>{sub}</span>}
      </div>
      {arcId && (
        <svg viewBox="0 0 100 100" className="adp-medallion-arc" style={{ animation: spin === "reverse" ? "adpRingRev 32s linear infinite" : "adpRingRev 34s linear infinite" }}>
          <defs>
...
function AdpCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="adp-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="adp-bank-row-label">{label}</span>
...
// Reimplementación propia del RSVP (ver mockup: filas de datos + el
// ticket/sello ya existente en la plantilla), en vez del look genérico de
    ...
    <>
      <div className="adp-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="adp-rsvp-row adp-rsvp-row--payment">
      ...
      <div ref={stubRef} className="adp-stub">
        ...
        <div ref={beamRef} className="adp-beam" />
        <div className="adp-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="adp-rsvp-error">{error}</p>}
...
// Reimplementación minimalista del look original del mockup -- misma API
...
function AdpSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="adp-song">
      ...
      {error && <p className="adp-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="adp-song-list">
...
function AdpQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: AdpQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="adp-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="adp-quiz-q">
            <span className="adp-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="adp-quiz-q-text">{q.pregunta}</p>
            <div className="adp-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " adp-quiz-opt--correct";
                  else if (chosen) stateClass = " adp-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " adp-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`adp-quiz-opt${stateClass}`}
...
function AdpInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="adp-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="adp-bank-row-label">{label}</span>
...
function CoverHalf({
  ...
  return (
    <div className="adp-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="244,230,232" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="244,230,232" />
        </div>
      )}
      ...
      {/* Motivo de tapa propio de Atelier de Papel: una hoja/lámina de papel
          ...
          mockup ("Atelier de Papel - Panoramica.dc.html"), donde la tapa
      ...
      <div className="adp-cover-content">
        ...
        <div className="adp-cover-center">
          ...
          <h1 ref={namesRef} className="adp-cover-names">{namesTitle}</h1>
          <span className="adp-cover-rule" />
...
// ---------------------------------------------------------------------
const ADP_CSS = `
  .adp-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .adp-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #9B3B4B; text-decoration: none; }
  a:hover { color: #F4E6E8; }

  @keyframes adpFoil { to { transform: rotate(360deg); } }
  @keyframes adpRing { to { transform: rotate(360deg); } }
  @keyframes adpRingRev { to { transform: rotate(-360deg); } }
  @keyframes adpScan { 0% { transform: translateY(0); opacity: 0; } 12% { opacity: .85; } 88% { opacity: .85; } 100% { transform: translateY(320px); opacity: 0; } }
  @keyframes adpEq { 0%,100% { transform: scaleY(.25); } 50% { transform: scaleY(1); } }
  @keyframes adpHint { 0%,100% { transform: translateY(0); } 50% { transform: translateY(7px); } }
  @keyframes adpSide { 0%,100% { transform: translateX(0); } 50% { transform: translateX(7px); } }
  @media (prefers-reduced-motion: reduce) { .adp-scroller * { animation: none !important; } }
  ...
  .adp-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; }
  ...
  @media (max-width: 767px) {
    .adp-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .adp-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(155,59,75,.3); }
    .adp-hero-photo-kicker { bottom: 40px; }
    .adp-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .adp-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .adp-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .adp-date-num { font-family: var(--adp-serif), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .adp-date-num--right { text-align: right; line-height: 0.86; }
  .adp-date-month { font-family: var(--adp-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #9B3B4B; padding-left: 12%; }

  .adp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .adp-divider-line { width: 52px; height: 1px; background: #9B3B4B; display: inline-block; }
  ...
  .adp-cal-link:hover { color: #9B3B4B; }
  ...
  .adp-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .adp-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .adp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2B2620, #9B3B4B, #9B3B4B, #F4E6E8, #2B2620); filter: saturate(.75); }
  .adp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #120C12; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .adp-medallion-label { font-family: var(--adp-serif), serif; font-size: 26px; line-height: 1; color: #F4E6E8; }
  .adp-medallion-label-sm { font-family: var(--adp-serif), serif; font-size: 16px; color: #F4E6E8; }
  .adp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .adp-medallion-sub--accent { color: #9B3B4B; }
  .adp-medallion-arc { position: absolute; inset: -14%; }
  ...
  .adp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #9B3B4B; opacity: 0.4; font-family: var(--adp-mono), monospace; }

  .adp-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(155,59,75,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .adp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F4E6E8, transparent); animation: adpScan 6s linear infinite; pointer-events: none; }

  .adp-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .adp-cd-box { border: 1px solid #2C171A; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .adp-cd-num { font-family: var(--adp-serif), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F4E6E8; display: inline-block; }
  .adp-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #9B3B4B; }
  .adp-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #150F10 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .adp-perf-strip--light { background: radial-gradient(circle at 6px 50%, #F4E6E8 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; }

  .adp-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(155,59,75,0.3), rgba(221,176,184,0.2), rgba(155,59,75,0.32), rgba(155,59,75,0.3)); filter: blur(80px); opacity: .4; animation: adpFoil 30s linear infinite; }
  ...
  .adp-accent-italic { font-style: italic; color: #9B3B4B; }
  .adp-phrase-accent { font-style: italic; color: #DDB0B8; }
  .adp-accent-serif { font-style: italic; color: #2B2620; font-family: var(--adp-serif), serif; }
  .adp-accent-serif-loc { font-style: italic; color: #9B3B4B; font-family: var(--adp-serif), serif; }
  .adp-accent-serif-2 { color: #2B2620; }
  .adp-h2 { margin: 0; font-family: var(--adp-serif), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .adp-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .adp-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .adp-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .adp-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .adp-panel--between { justify-content: space-between; }
  ...
  .adp-pan[data-scroll="vertical"] { height: auto; }
  .adp-pan[data-scroll="vertical"] .adp-pan-sticky { position: static; height: auto; overflow: visible; }
  .adp-pan[data-scroll="vertical"] .adp-strip { position: static; display: block; width: 100%; transform: none !important; }
  .adp-pan[data-scroll="vertical"] .adp-panel { height: calc(var(--vh, 1vh) * 100); }
  .adp-panel--end { justify-content: flex-end; }
  .adp-panel--center { align-items: center; justify-content: center; text-align: center; }
  .adp-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .adp-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .adp-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .adp-panel-title { margin: 0; position: relative; font-family: var(--adp-serif), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .adp-panel-title-md { margin: 0; position: relative; font-family: var(--adp-serif), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  ...
  .adp-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #2B2620; margin-top: auto; }
  .adp-seguir--split { justify-content: space-between; }
  .adp-seguir--split > span:first-child { color: #7C7768; }
  .adp-side-hint { display: inline-block; animation: adpSide 2.2s ease-in-out infinite; }
  .adp-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .adp-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #2B2620; }

  .adp-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .adp-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .adp-crono-time { font-family: var(--adp-mono), monospace; color: #2B2620; min-width: 42px; }
  .adp-crono-title { flex: 1; }

  .adp-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .adp-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .adp-stub { position: relative; overflow: hidden; border: 1px solid #2C171A; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .adp-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .adp-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .adp-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .adp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F4E6E8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .adp-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--adp-mono), monospace; }
  .adp-rsvp-rows { display: flex; flex-direction: column; }
  .adp-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(155,59,75,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .adp-rsvp-row > span:first-child { flex-shrink: 0; }
  .adp-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .adp-rsvp-row--payment { align-items: flex-start; }
  .adp-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .adp-rsvp-payment-total { color: #F4E6E8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .adp-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .adp-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .adp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #9B3B4B; background: transparent; color: #9B3B4B; font-size: 14px; line-height: 1; cursor: pointer; }
  .adp-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .adp-rsvp-stepper span { font-family: var(--adp-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .adp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(155,59,75,0.3); color: #F4F1EA; font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .adp-rsvp-input::placeholder { color: #6E6A5D; }
  .adp-rsvp-input:focus { outline: none; border-bottom-color: #9B3B4B; }
  .adp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--adp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #9B3B4B, #762D39); border: 1px solid #9B3B4B; color: #150F10; cursor: pointer; }
  .adp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .adp-rsvp-btn--ghost { background: transparent; color: #9B3B4B; }
  .adp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #9B3B4B; margin: 0; }
  .adp-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .adp-rsvp-declined-text { margin: 0; font-family: var(--adp-serif), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .adp-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .adp-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .adp-photo-cell--offset { margin-top: 18px; }
  .adp-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .adp-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .adp-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .adp-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .adp-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }

  /* Mosaico fijo del álbum: grilla pareja de 3 columnas, foto destacada 2x2
     en la primera celda, hasta 5 por hoja. */
  ...
  .adp-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .adp-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .adp-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .adp-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }

  .adp-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .adp-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(155,59,75,0.18); }
  .adp-bank-row:last-child { border-bottom: none; }
  .adp-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .adp-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .adp-bank-copy { flex-shrink: 0; font-family: var(--adp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #9B3B4B; background: transparent; color: #9B3B4B; cursor: pointer; }
  .adp-bank-copy:hover { background: rgba(155,59,75,0.12); }

  .adp-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .adp-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: adpEq 1.1s ease-in-out infinite; display: inline-block; }
  .adp-song-wrap { font-family: var(--adp-mono), monospace; }

  .adp-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .adp-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(155,59,75,0.3); padding-bottom: 12px; }
  .adp-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .adp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(155,59,75,0.3); color: #F4F1EA; font-family: var(--adp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .adp-song-input::placeholder { color: #6E6A5D; }
  .adp-song-input:focus { outline: none; border-bottom-color: #9B3B4B; }
  .adp-song-sep { color: #8A8577; flex-shrink: 0; }
  .adp-song-submit { flex-shrink: 0; background: none; border: none; color: #9B3B4B; font-family: var(--adp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .adp-song-submit:hover { color: #F4E6E8; }
  .adp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .adp-song-error { font-size: 10px; color: #9B3B4B; margin-top: 6px; }
  .adp-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .adp-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--adp-mono), monospace; }
  .adp-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .adp-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .adp-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .adp-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .adp-quiz-q-num { font-family: var(--adp-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .adp-quiz-q-text { margin: 0; font-family: var(--adp-serif), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .adp-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .adp-quiz-opt { font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(155,59,75,0.4); background: transparent; color: #9B3B4B; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .adp-quiz-opt:disabled { cursor: default; }
  .adp-quiz-opt--picked { background: #9B3B4B; border-color: #9B3B4B; color: #150F10; }
  .adp-quiz-opt--correct { background: #9B3B4B; border-color: #9B3B4B; color: #150F10; }
  .adp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .adp-quiz-result { padding-top: 18px; border-top: 1px solid rgba(155,59,75,0.2); }
  .adp-quiz-result-score { margin: 0 0 6px; font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F4E6E8; }
  .adp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .adp-final-card { border: 1px solid #9B3B4B; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .adp-final-names { font-family: var(--adp-serif), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F4E6E8; }
  .adp-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .adp-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .adp-replay { cursor: pointer; color: #9B3B4B; }
  .adp-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .adp-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(155,59,75,0.14); }
  .adp-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .adp-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(155,59,75,0.16); position: relative; }
  .adp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F4E6E8, #9B3B4B); transition: height 260ms linear; display: block; }
  .adp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #9B3B4B; transition: color 500ms ease; }

  .adp-cover { position: absolute; inset: 0; z-index: 5; }
  ...
  .adp-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: #F4E6E8; }
  .adp-cover-half--bottom .adp-cover-inner { top: auto; bottom: 0; }
  .adp-cover-glow { position: absolute; inset: 0; background-image: repeating-linear-gradient(0deg, rgba(155,59,75,.16) 0 1px, transparent 1px 5px), repeating-linear-gradient(90deg, rgba(155,59,75,.12) 0 1.5px, transparent 1.5px 46px); }
  ...
  .adp-cover-paper { position: absolute; width: 66%; height: 50%; left: 17%; top: 22%; background: #ECD3D8; border-radius: 8px; box-shadow: 0 20px 40px rgba(43,38,32,.18); transform: rotate(-3deg); }
  .adp-cover-paper--2 { width: 62%; height: 46%; left: 19%; top: 24%; background: #DDB0B8; box-shadow: 0 14px 28px rgba(43,38,32,.14); transform: rotate(2deg); }
  ...
  .adp-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #9B3B4B; }
  .adp-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .adp-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #9B3B4B; }
  ...
  .adp-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#9B3B4B, transparent); display: block; }
  ...
  .adp-cover-cta { border: 1px solid #2B2620; background: #2B2620; color: #F4E6E8; font-family: var(--adp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .adp-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .adp-cover-cta--btn:hover { background: #9B3B4B; border-color: #9B3B4B; }
  }
  ...
  .adp-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(20,18,14,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .adp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #9B3B4B; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .adp-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
