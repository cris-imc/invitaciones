...
export function FashionLookbookTemplateMilitar({ invitation, guest, isPersonalized = false }: FashionLookbookTemplateProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#6B8F3B";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(107,143,59,.35), 0 18px 50px -30px #6B8F3B";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#0B0B0B" : "#6B8F3B";
        }
...
const FLB_CSS = `
  ...
  a { color: #6B8F3B; text-decoration: none; }
  ...
  .flb-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; background: radial-gradient(120% 80% at 50% 0%, #1B2314 0%, #0D0F0A 55%, #080906 100%); }
  .flb-section--between { justify-content: space-between; }
  .flb-section:nth-of-type(2) { background: radial-gradient(100% 60% at 50% 100%, #1A1D10 0%, #10120A 55%, #080906 100%); }
  .flb-section:nth-of-type(3) { background: radial-gradient(130% 90% at 86% 16%, #1E2416 0%, #12150C 52%, #080906 100%); }
  ...
  .flb-date-month { font-family: var(--flb-archivo), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #6B8F3B; padding-left: 12%; }

  .flb-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .flb-divider-line { width: 52px; height: 1px; background: #6B8F3B; display: inline-block; }
  ...
  .flb-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6B8F3B, #0D0F0A, #6B8F3B, #F5F2ED, #6B8F3B); filter: saturate(.75); }
  .flb-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #12150C; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .flb-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #6B8F3B; font-family: var(--flb-mono), monospace; }
  ...
  .flb-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #6B8F3B; }
  .flb-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #080906 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .flb-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }

  .flb-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(107,143,59,0.24), rgba(13,15,10,0.3), rgba(107,143,59,0.36), rgba(107,143,59,0.24)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .flb-accent-italic { font-style: italic; color: #6B8F3B; }
  .flb-accent-mono { font-style: italic; color: #0B0B0B; font-family: var(--flb-archivo), sans-serif; }
  .flb-accent-red-serif { font-style: italic; color: #6B8F3B; font-family: var(--flb-archivo), sans-serif; }
  .flb-accent-plain { color: #0B0B0B; }
  .flb-accent-red { color: #6B8F3B; }
  ...
  .flb-crono-time { font-family: var(--flb-mono), monospace; color: #6B8F3B; min-width: 42px; }
  ...
  .flb-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(107,143,59,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .flb-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #6B8F3B; background: transparent; color: #6B8F3B; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .flb-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(107,143,59,0.3); color: #F4F1EA; font-family: var(--flb-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .flb-rsvp-input::placeholder { color: #6E6A5D; }
  .flb-rsvp-input:focus { outline: none; border-bottom-color: #6B8F3B; }
  .flb-rsvp-btn { width: 100%; padding: 17px 0; font-family: var(--flb-mono), monospace; font-size: 11.5px; font-weight: 600; letter-spacing: 0.26em; text-transform: uppercase; border: 1px solid #6B8F3B; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 200ms ease, color 200ms ease; }
  ...
  .flb-rsvp-btn--ghost { background: transparent; color: #6B8F3B; }
  .flb-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(107,143,59,0.1); color: #6B8F3B; }
  .flb-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #6B8F3B; margin: 0; }
  ...
  .flb-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(107,143,59,0.18); }
  ...
  .flb-bank-copy { flex-shrink: 0; font-family: var(--flb-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #6B8F3B; background: transparent; color: #6B8F3B; cursor: pointer; }
  .flb-bank-copy:hover { background: rgba(107,143,59,0.12); }
  ...
  .flb-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(107,143,59,0.3); padding-bottom: 12px; }
  ...
  .flb-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(107,143,59,0.3); color: #F4F1EA; font-family: var(--flb-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .flb-song-input::placeholder { color: #6E6A5D; }
  .flb-song-input:focus { outline: none; border-bottom-color: #6B8F3B; }
  .flb-song-sep { color: #8A8577; flex-shrink: 0; }
  .flb-song-submit { flex-shrink: 0; background: none; border: none; color: #6B8F3B; font-family: var(--flb-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .flb-song-submit:hover { color: #F5F2ED; }
  .flb-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .flb-song-error { font-size: 10px; color: #6B8F3B; margin-top: 6px; }
  ...
  .flb-quiz-opt { font-family: var(--flb-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(107,143,59,0.4); background: transparent; color: #6B8F3B; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .flb-quiz-opt:disabled { cursor: default; }
  .flb-quiz-opt--picked { background: #6B8F3B; border-color: #6B8F3B; color: #0B0B10; }
  .flb-quiz-opt--correct { background: #6B8F3B; border-color: #6B8F3B; color: #0B0B10; }
  .flb-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .flb-quiz-result { padding-top: 18px; border-top: 1px solid rgba(107,143,59,0.2); }
  ...
  .flb-final-card { border: 1px solid #6B8F3B; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .flb-replay { cursor: pointer; color: #6B8F3B; }
  ...
  .flb-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F5F2ED, #6B8F3B); transition: height 260ms linear; display: block; }
  .flb-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #6B8F3B; transition: color 500ms ease; }
  ...
  .flb-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1A1D10 0%, #0D0F0A 46%, #080906 100%); }
  .flb-cover-half--bottom .flb-cover-inner { top: auto; bottom: 0; }
  .flb-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(107,143,59,0.35), rgba(13,15,10,0.5), rgba(107,143,59,0.5), rgba(107,143,59,0.35)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .flb-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#6B8F3B, transparent); display: block; }
  ...
  .flb-cover-cta { border: 1px solid #6B8F3B; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--flb-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .flb-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #6B8F3B; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
