 ...
 * en Claude Design (fondo casi negro #0D0A0C, acento terracota/óxido #A83A5A,
 ...
 *
 * Particularidad de paleta: en el mockup real, el acento terracota (#A83A5A)
...
export function EncajeContemporaneoTemplateBorgona({ invitation, guest, isPersonalized = false }: EncajeContemporaneoTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#A83A5A";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(214,150,168,.35), 0 18px 50px -30px #A83A5A";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(168,58,90,0.18)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(168,58,90,0.2)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#1C1A18" : "#A83A5A";
        }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="enc-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="enc-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #21141C 0%, #100D10 55%, #0D0A0C 100%)" }}>
            ...
            <div className="enc-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="13,10,12" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="13,10,12" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="enc-section enc-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #271722 0%, #120E12 55%, #0D0A0C 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="enc-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #24151F 0%, #0E0B0D 52%, #0D0A0C 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="enc-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="enc-pan-sticky">
            <div data-strip="1" className="enc-strip">
              ...
              <div data-tone="dark" className="enc-panel enc-panel--center" style={{ background: "#100D10", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="enc-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #21141C 0%, #100D10 60%, #0D0A0C 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="enc-section" style={{ background: "#100D10" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="enc-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="enc-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F7F3ED" : "#A83A5A" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="enc-section" style={{ background: "#100D10" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="enc-section" style={{ background: "#100D10" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="enc-section enc-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #21141C 0%, #100D10 55%, #0D0A0C 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#A83A5A" guest={guest} />
      )}
...
const ENC_CSS = `
  ...
  a { color: #A83A5A; text-decoration: none; }
  ...
  .enc-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0D0A0C; }
  ...
  @media (min-width: 768px) {
    .enc-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(168,58,90,.3); }
  ...
  .enc-date-month { font-family: var(--enc-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #A83A5A; padding-left: 12%; }

  .enc-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .enc-divider-line { width: 52px; height: 1px; background: #A83A5A; display: inline-block; }
  ...
  .enc-cal-link:hover { color: #A83A5A; }
  ...
  .enc-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #1C1A18, #3A362E, #A83A5A, #F7F3ED, #1C1A18); filter: saturate(.75); }
  .enc-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0F0C0F; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .enc-medallion-sub--accent { color: #A83A5A; }
  ...
  .enc-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #A83A5A; opacity: 0.4; font-family: var(--enc-mono), monospace; }

  .enc-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(214,150,168,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  ...
  .enc-cd-box { border: 1px solid #3D1F2A; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  ...
  .enc-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #A83A5A; }
  .enc-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0D0A0C 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .enc-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(28,26,24,0.34), rgba(140,133,119,0.2), rgba(168,58,90,0.36), rgba(28,26,24,0.34)); filter: blur(80px); opacity: .4; animation: encFoil 30s linear infinite; }
  ...
  .enc-accent-italic { font-style: italic; color: #A83A5A; }
  .enc-amp { font-style: italic; }
  .enc-accent-rust { color: #A83A5A; }
  /* Sobre paneles claros (Cuándo y dónde / Álbum), el énfasis de la marca
     ...
     el mockup real. */
  .enc-accent-serif { font-style: italic; color: #A83A5A; font-family: var(--enc-playfair), serif; }
  ...
  .enc-stub { position: relative; overflow: hidden; border: 1px solid #3D1F2A; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .enc-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(168,58,90,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .enc-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #A83A5A; background: transparent; color: #A83A5A; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .enc-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(168,58,90,0.3); color: #F4F1EA; font-family: var(--enc-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .enc-rsvp-input::placeholder { color: #6E6A5D; }
  .enc-rsvp-input:focus { outline: none; border-bottom-color: #A83A5A; }
  .enc-rsvp-btn { width: 100%; padding: 16px; font-family: var(--enc-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; border: 1px solid #A83A5A; background: linear-gradient(100deg, rgba(214,150,168,0.08), rgba(235,205,214,0.2), rgba(214,150,168,0.08)); color: #F6EFDD; cursor: pointer; transition: background 200ms ease, color 200ms ease; }
  .enc-rsvp-btn:hover:not(:disabled) { background: linear-gradient(100deg, rgba(235,205,214,0.34), rgba(246,239,221,0.5), rgba(235,205,214,0.34)); color: #0B0B0F; }
  .enc-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .enc-rsvp-btn--ghost { background: transparent; color: #A83A5A; }
  .enc-rsvp-btn--ghost:hover:not(:disabled) { background: transparent; color: #F7F3ED; }
  .enc-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #A83A5A; margin: 0; }
  ...
  .enc-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(168,58,90,0.18); }
  ...
  .enc-bank-copy { flex-shrink: 0; font-family: var(--enc-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #A83A5A; background: transparent; color: #A83A5A; cursor: pointer; }
  .enc-bank-copy:hover { background: rgba(168,58,90,0.12); }
  ...
  .enc-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(168,58,90,0.3); padding-bottom: 12px; }
  ...
  .enc-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(168,58,90,0.3); color: #F4F1EA; font-family: var(--enc-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .enc-song-input::placeholder { color: #6E6A5D; }
  .enc-song-input:focus { outline: none; border-bottom-color: #A83A5A; }
  .enc-song-sep { color: #8A8577; flex-shrink: 0; }
  .enc-song-submit { flex-shrink: 0; background: none; border: none; color: #A83A5A; font-family: var(--enc-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .enc-song-submit:hover { color: #F7F3ED; }
  .enc-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .enc-song-error { font-size: 10px; color: #A83A5A; margin-top: 6px; }
  ...
  .enc-quiz-opt { font-family: var(--enc-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(168,58,90,0.4); background: transparent; color: #A83A5A; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .enc-quiz-opt:disabled { cursor: default; }
  .enc-quiz-opt--picked { background: #A83A5A; border-color: #A83A5A; color: #F7F3ED; }
  .enc-quiz-opt--correct { background: #A83A5A; border-color: #A83A5A; color: #F7F3ED; }
  .enc-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .enc-quiz-result { padding-top: 18px; border-top: 1px solid rgba(168,58,90,0.2); }
  ...
  .enc-final-card { border: 1px solid #A83A5A; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .enc-replay { cursor: pointer; color: #A83A5A; }
  .enc-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .enc-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(168,58,90,0.14); }
  .enc-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .enc-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(168,58,90,0.16); position: relative; }
  .enc-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F7F3ED, #A83A5A); transition: height 260ms linear; display: block; }
  .enc-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #A83A5A; transition: color 500ms ease; }
  ...
  @media (hover: hover) {
    .enc-cover-cta--btn:hover { background: #A83A5A; border-color: #A83A5A; }
  }
  ...
  .enc-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #A83A5A; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
