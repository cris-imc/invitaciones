  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="enc-scroller">
        ...
        <section id="quote" data-tone="dark" data-screen-label="Frase" className="enc-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #24151F 0%, #0E0B0D 52%, #0D0A0C 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label="Música" className="enc-section" style={{ background: "#100D10" }}>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label="Regalos" className="enc-section" style={{ background: "#100D10" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pase" className="enc-section enc-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #21141C 0%, #100D10 55%, #0D0A0C 100%)" }}>
...
const ENC_CSS = `
  ...
  .enc-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1F131A 0%, #100D10 46%, #0D0A0C 100%); }
  .enc-cover-half--bottom .enc-cover-inner { top: auto; bottom: 0; }
  .enc-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(28,26,24,0.5), rgba(140,133,119,0.3), rgba(168,58,90,0.55), rgba(28,26,24,0.5)); filter: blur(64px); opacity: .62; animation: encFoil 26s linear infinite; }
  ...
  .enc-cover-cta { border: 1px solid #A83A5A; background: linear-gradient(100deg, rgba(214,150,168,0.08), rgba(235,205,214,0.2), rgba(214,150,168,0.08)); color: #F6EFDD; font-family: var(--enc-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .enc-cover-cta--btn { cursor: pointer; border-radius: 0; }
  .enc-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(235,205,214,0.34), rgba(246,239,221,0.5), rgba(235,205,214,0.34)); color: #0B0B0F; }
