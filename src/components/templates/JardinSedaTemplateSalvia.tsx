 ...
 * fijo (#6F9A76) + un verde salvia secundario (#7C9473, variable por
...
interface JardinSedaTemplateSalviaProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#6F9A76]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#6F9A76] uppercase tracking-wider mb-0.5">{label}</span>
          ...
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#6F9A76] shrink-0" />
...
export function JardinSedaTemplateSalvia({ invitation, guest, isPersonalized = false }: JardinSedaTemplateSalviaProps) {
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(111, 154, 118, 0.2) !important;
        }

        /* RSVP Custom Aesthetics for JardinSedaTemplateSalvia */
        #rsvp.section.dark {
          background-color: #FCEFF1 !important;
          color: #3A2E33 !important;
        ...
        #rsvp.section.dark input {
          ...
          color: #3A2E33 !important;
          border-radius: 999px !important;
          border: 1px solid rgba(111, 154, 118, 0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #6F9A76 !important;
          color: #3A2E33 !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #6F9A76 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
      ...
      {!isCoverOpen && (
        ...
        >
          <div style={{
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(111,154,118,0.13), transparent), radial-gradient(45% 40% at 85% 80%, rgba(124,148,115,0.2), transparent)',
            backgroundSize: '160% 160%',
            animation: 'jardinseda-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(111,154,118,0.13), transparent 70%)',
          ...
          <IconHeartDoodle className="jardinseda-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '15%', right: '12%', color: 'rgba(111,154,118,0.5)' }} />
          <IconRibbon className="jardinseda-doodle opacity-0 absolute" style={{ width: 26, height: 19, bottom: '18%', left: '14%', color: 'rgba(111,154,118,0.45)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="jardinseda-seal opacity-0" style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #6F9A76',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#6F9A76',
            }}>
              {monogram}
            </div>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #6F9A76', color: '#6F9A76',
                background: 'rgba(111,154,118,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#6F9A76'; e.currentTarget.style.color = '#FCEFF1'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(111,154,118,0.08)'; e.currentTarget.style.color = '#6F9A76'; }}
            >
              ABRIR INVITACIÓN
            </button>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#6F9A76]/40 shadow-md ${isTicketMaximized ? 'bg-[#FCEFF1]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#3A2E33]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#6F9A76] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#3A2E33] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#6F9A76]/20 pl-3">
            ...
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#6F9A76]" />
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#6F9A76', margin: '6px 0 14px', animation: 'jardinseda-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#FCEFF1]">
          ...
          <div className="px-8 pt-16 pb-12 text-left bg-[#FCEFF1] z-10 relative">
            ...
            <IconRibbon className="jardinseda-scroll-doodle opacity-0 absolute" style={{ width: 26, height: 15, top: 64, right: 52, color: 'rgba(111,154,118,0.4)' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#6F9A76] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#3A2E33] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#6F9A76' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#6F9A76', margin: '0 0 20px', animation: 'jardinseda-lineExpand 1.2s ease-out' }} />
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#6F9A76] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden rounded-b-[2.5rem]">
            ...
            <div className="absolute inset-3 pointer-events-none z-20 rounded-[2rem]" aria-hidden="true" style={{ border: "1px solid rgba(111,154,118,0.6)" }}>
              <IconRings style={{ position: "absolute", top: -10, left: -8, width: 30, height: 20, color: "#7C9473" }} />
              <IconHeartDoodle style={{ position: "absolute", bottom: -8, right: -6, width: 18, height: 18, color: "#6F9A76" }} />
            </div>
            ...
            <div ref={heroFlareRef} className="pointer-events-none z-20" style={{ position: "absolute", width: 140, height: 140, top: "10%", left: "8%", borderRadius: "50%", background: "radial-gradient(circle, rgba(111,154,118,0.55) 0%, rgba(124,148,115,0.3) 55%, transparent 75%)", opacity: 0, transform: "translate(-50%,-50%)", filter: "blur(2px)" }} />
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#FCEFF1]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #6F9A76, transparent)' }} />
          <IconRings className="jardinseda-scroll-doodle opacity-0" style={{ width: 26, height: 17, color: '#7C9473' }} />
          <IconHeartDoodle className="jardinseda-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#6F9A76' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #6F9A76, transparent)' }} />
        </div>
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#FCEFF1] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#6F9A76]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#6F9A76] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#6F9A76] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#6F9A76] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9C8A9B] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#6F9A76] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#6F9A76]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <IconRibbon className="jardinseda-scroll-doodle opacity-0" style={{ width: 20, height: 12 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#6F9A76]/10 last:border-b-0">
                      {item.time && (
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#F3E6EF] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#6F9A76]">
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#FCEFF1' }} aria-hidden="true">
            <IconMusic className="jardinseda-scroll-doodle opacity-0" style={{ width: 26, height: 26, color: '#6F9A76' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#FCEFF1' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #6F9A76, transparent)' }} />
      ...
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#6F9A76" surfaceColor="#FFFFFF" />}
      </div>
  );
}
