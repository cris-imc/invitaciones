 ...
 * esta familia está en scripts/familias/tipografica/pop.json.
 *
 * El cómic: Bangers en mayúsculas sobre negro, Nunito para el texto y tres
 * colores que se pisan entre sí. Los títulos llevan contorno, como un
 * globo de diálogo.
 *
...
export function PopTemplate({ invitation, guest, isPersonalized = false }: PopTemplateProps) {
  const tx = useTextos();
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="pop-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="pop-section pop-std">
          <div className="pop-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="pop-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="pop-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="pop-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="pop-std-fila">
              <span className="pop-cinta">{mesLargo}</span>
              <span className="pop-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="pop-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="pop-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="pop-btn-papel pop-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="pop-foto-seccion">
            <div className="pop-foto-marco">
              <div ref={ventanaRef} className="pop-ventana">
                {fotoMobile && (
              ...
              <div className="pop-foto-firma">{titulo}</div>
            </div>
          </section>
        )}

        {/* ── 02 Falta poco ──────────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="pop-section pop-countdown">
          <div className="pop-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="pop-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="pop-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="pop-section pop-frase-seccion">
            <div className="pop-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pop-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            <h2 ref={fraseRef} className="pop-frase">
              {palabras.map((p, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= desdeAcento ? "pop-acento" : undefined}>{p}</span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="200" className="pop-firma">
              <span className="pop-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="pop-pan-fijo">
            <div data-strip="1" className="pop-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="pop-panel pop-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="pop-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="pop-tarjeta pop-tarjeta--izq">
                  <span className="pop-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="pop-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="pop-filas">
                    {direccion && (
                      <div className="pop-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="pop-fila-valor">{direccion}</span></div>
                    )}
                    <div className="pop-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="pop-fila pop-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="pop-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="pop-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="pop-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="pop-panel pop-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="pop-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="pop-tarjeta pop-tarjeta--der">
                    <h2 className="pop-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="pop-filas">
                      {ceremoniaDireccion && (
                        <div className="pop-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="pop-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="pop-fila pop-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="pop-panel pop-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="pop-escena" aria-hidden="true">
                  ...
                  <h2 className="pop-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="pop-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="pop-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="pop-panel pop-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="pop-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="pop-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="pop-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="pop-crono-fila">
                        <span className="pop-crono-hora">{item.time || ""}</span>
                        <span className="pop-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* ── 05 Check-in ────────────────────────────────────────────── */}
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="pop-section pop-checkin">
            <div className="pop-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pop-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="pop-h2">
            ...
            <div className="pop-pan-fijo">
              <div data-strip="1" className="pop-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="pop-panel pop-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="pop-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="pop-h2 pop-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="pop-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="pop-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="pop-polaroid-img" />
                          {i % 2 === 0 && <span className="pop-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="pop-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="pop-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="pop-section pop-musica">
            <div className="pop-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pop-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="pop-h2">
              {tituloEnDosLineas(tx("invitacion.sabor.preguntaCancionFaltar"), "pop-acento")}
            </h2>
            <CancionesPop
              invitationId={String(invitation.id ?? "")}
              guestToken={guest?.uniqueToken}
        ...
        {/* ── 08 Regalos ─────────────────────────────────────────────── */}
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="pop-section pop-regalos">
            <span data-xin="1" data-dist="-20" className="pop-kicker pop-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="pop-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="pop-parrafo pop-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {/* ── 09 Trivia ──────────────────────────────────────────────── */}
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="pop-section pop-quiz">
            <div className="pop-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pop-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="pop-h2 pop-h2--claro">{triviaTitulo}</h2>
            <TriviaPop
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="pop-section pop-pase">
          <div className="pop-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="pop-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="pop-ticket">
            <div className="pop-ticket-izq">
              <span className="pop-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="pop-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="pop-ticket-dato">
                {[
              ...
              <span className="pop-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="pop-ticket-muesca pop-ticket-muesca--arriba" aria-hidden="true" />
              <span className="pop-ticket-muesca pop-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="pop-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="pop-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="pop-pase-pie">
            <span className="pop-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="pop-pase-creditos">
      ...
      <div ref={rielRef} className="pop-riel">
        <span ref={rielTopRef} className="pop-riel-top">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
      ...
      <div ref={portadaRef} data-tone={TONO} className="pop-portada">
        ...
        <div ref={escenaPortadaRef} className="pop-portada-escena">
          <div data-cl="1" className="pop-trama" aria-hidden="true" />
        </div>
        <div className="pop-portada-contenido">
          <div className="pop-portada-arriba">
            <div ref={cartelRef} className="pop-cartel">
              <h1 className="pop-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="pop-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="pop-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="pop-portada-abajo">
            <div className="pop-portada-datos">
              <span className="pop-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="pop-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="pop-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="pop-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="pop-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="pop-portada-pase">
            ...
            <button type="button" onClick={abrir} className="pop-portada-btn">
...
function CuentaPop({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="pop-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`pop-cuenta-caja pop-cuenta-caja--${i + 1}`}>
          <span className={`pop-cuenta-num${i === 3 ? " pop-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_POP = `
  ...
  .pop-raiz a { color: var(--pp-acc); text-decoration: none; }
  .pop-raiz button { font: inherit; }
  .pop-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --pop-y y gira en --pop-giro: son
  ...
  .pop-scroller [data-xin] { transform: translate3d(0, var(--pop-y, 0px), 0) rotate(var(--pop-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .pop-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .pop-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .pop-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .pop-kicker--tinta { color: var(--pp-ink); }
  .pop-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--pop-serif), 'Bangers', cursive;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .pop-h2 em, .pop-acento { font-style: italic; color: var(--pp-acc); }
  .pop-h2--claro { color: var(--pp-night-ink); }
  .pop-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .pop-acento-plano { color: var(--pp-acc); }
  .pop-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .pop-parrafo--tinta { color: var(--pp-ink); }
  .pop-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .pop-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .pop-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .pop-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .pop-std-dia { font-family: var(--pop-serif), 'Bangers', cursive; font-weight: 500;
  ...
  .pop-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .pop-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--pop-serif), 'Bangers', cursive; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .pop-std-anio { font-family: var(--pop-serif), 'Bangers', cursive; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .pop-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .pop-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .pop-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .pop-foto-capa { position: absolute; inset: 0; }
  .pop-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--pop-serif), 'Bangers', cursive; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .pop-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .pop-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .pop-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .pop-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .pop-cuenta-caja--1 { --pop-giro: -1.6deg; transform: rotate(var(--pop-giro)); }
  .pop-cuenta-caja--2 { --pop-giro: 1.2deg; transform: rotate(var(--pop-giro)); margin-top: 10px; }
  .pop-cuenta-caja--3 { --pop-giro: 1.8deg; transform: rotate(var(--pop-giro)); }
  .pop-cuenta-caja--4 { --pop-giro: -1.1deg; transform: rotate(var(--pop-giro)); margin-top: 10px; }
  .pop-cuenta-num { font-family: var(--pop-serif), 'Bangers', cursive; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .pop-cuenta-num--acc { color: var(--pp-acc); }
  .pop-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .pop-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .pop-frase { position: relative; z-index: 1; margin: 0; font-family: var(--pop-serif), 'Bangers', cursive;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .pop-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .pop-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .pop-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .pop-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .pop-panel--abajo { justify-content: flex-end; }
  .pop-panel--centro { justify-content: center; }
  .pop-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .pop-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .pop-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .pop-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .pop-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .pop-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .pop-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .pop-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .pop-tarjeta--izq { --pop-giro: -1deg; transform: rotate(var(--pop-giro)); }
  .pop-tarjeta--der { --pop-giro: 1.1deg; transform: rotate(var(--pop-giro)); }
  .pop-tarjeta--talon { --pop-giro: -.8deg; transform: rotate(var(--pop-giro)); overflow: visible; }
  .pop-tarjeta--hoy { --pop-giro: -1deg; transform: rotate(var(--pop-giro)); align-items: flex-start; }
  .pop-tarjeta--banco { margin-top: 4px; }
  .pop-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .pop-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .pop-tarjeta-titulo { margin: 0; font-family: var(--pop-serif), 'Bangers', cursive; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .pop-tarjeta-pregunta { font-family: var(--pop-serif), 'Bangers', cursive; font-size: 26px; line-height: 1.15; }
  .pop-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .pop-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .pop-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .pop-fila > span:first-child { color: var(--pp-ink2); }
  .pop-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .pop-fila-valor { text-align: right; }
  .pop-fila--copiable { align-items: center; }
  .pop-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .pop-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .pop-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .pop-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .pop-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .pop-checkin { background: var(--pp-bg); }
  .pop-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .pop-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .pop-campo { display: flex; flex-direction: column; gap: 6px; }
  .pop-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .pop-contador { display: flex; align-items: center; gap: 12px; }
  .pop-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .pop-contador button:disabled { opacity: .35; cursor: default; }
  .pop-contador > span { font-family: var(--pop-serif), 'Bangers', cursive; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .pop-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--pop-sans), 'Nunito', sans-serif; font-size: 16px; padding: 6px 2px; }
  .pop-input--serif { font-family: var(--pop-serif), 'Bangers', cursive; font-size: 20px; }
  .pop-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .pop-input::placeholder { color: #9A9388; }
  .pop-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .pop-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .pop-precio-total { font-family: var(--pop-serif), 'Bangers', cursive; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .pop-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .pop-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .pop-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .pop-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .pop-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--pop-sans), 'Nunito', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .pop-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .pop-btn-solido:disabled { opacity: .6; cursor: default; }
  .pop-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .pop-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --pop-giro: -1deg; transform: rotate(var(--pop-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .pop-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .pop-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .pop-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .pop-crono-hora { font-family: var(--pop-serif), 'Bangers', cursive; font-size: 26px; min-width: 64px; line-height: 1; }
  .pop-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .pop-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .pop-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .pop-polaroids[data-cantidad="1"] .pop-polaroid { grid-column: span 6; }
  .pop-polaroids[data-cantidad="2"] .pop-polaroid { grid-column: span 3; }
  .pop-polaroids[data-cantidad="4"] .pop-polaroid { grid-column: span 3; }
  .pop-polaroids[data-cantidad="5"] .pop-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .pop-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .pop-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .pop-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .pop-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .pop-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .pop-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .pop-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .pop-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .pop-lista-tema { font-size: 15px; }
  .pop-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .pop-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .pop-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .pop-opciones { display: flex; flex-direction: column; gap: 8px; }
  .pop-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--pop-sans), 'Nunito', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .pop-opcion:disabled { cursor: default; }
  .pop-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .pop-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .pop-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .pop-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --pop-giro: -.6deg; transform: rotate(var(--pop-giro));
    border-radius: 12px; overflow: hidden; }
  .pop-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .pop-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .pop-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .pop-ticket-nombre { font-family: var(--pop-serif), 'Bangers', cursive; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .pop-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .pop-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .pop-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .pop-ticket-muesca--arriba { top: -12px; }
  .pop-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .pop-ticket-der .qr-ingreso, .pop-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .pop-info-extra { position: relative; z-index: 1; }
  .pop-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .pop-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .pop-raiz .ia-icon-box { display: none !important; }
  .pop-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .pop-pase-firma { font-family: var(--pop-serif), 'Bangers', cursive; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .pop-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .pop-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }

  /* ── Riel ──────────────────────────────────────────────────────────── */
  .pop-riel { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column;
  ...
  .pop-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .pop-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  .pop-riel-barra { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: var(--pp-acc); transition: height 260ms linear; display: block; }
  .pop-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .pop-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .pop-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .pop-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .pop-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .pop-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .pop-cartel-nombres { margin: 0; font-family: var(--pop-serif), 'Bangers', cursive; font-weight: 500;
  ...
  .pop-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .pop-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .pop-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .pop-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .pop-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .pop-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .pop-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .pop-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--pop-sans), 'Nunito', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .pop-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }

  .pop-pista { position: absolute; left: 0; right: 34px; bottom: calc(18px + env(safe-area-inset-bottom)); z-index: 6; text-align: center;
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .pop-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .pop-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .pop-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .pop-section, .pop-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .pop-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .pop-raiz { font-family: var(--pop-sans), 'Nunito', sans-serif; }
  ...
  .pop-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .pop-kicker, .pop-panel-top, .pop-panel-pie, .pop-meta, .pop-riel-top, .pop-riel-etiqueta,
  .pop-ticket-kicker, .pop-cuenta-etq, .pop-tarjeta-kicker, .pop-fila-etq, .pop-portada-pase {
    font-family: var(--pop-sans), 'Nunito', sans-serif; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .pop-h2, .pop-frase, .pop-tarjeta-titulo, .pop-std-dia, .pop-cartel-nombres, .pop-ticket-nombre, .pop-cifra, .pop-cuenta-num {
    font-family: var(--pop-serif), 'Bangers', cursive; letter-spacing: -.03em; }
  .pop-h2 em, .pop-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .pop-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .pop-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .pop-std-fila { justify-content: space-between; }
  .pop-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--pop-serif), 'Bangers', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .pop-std-anio { font-family: var(--pop-serif), 'Bangers', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .pop-std .pop-kicker, .pop-std .pop-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .pop-foto-marco { border: 3px solid var(--pp-ink); }
  .pop-ventana { clip-path: none !important; box-shadow: none; }
  .pop-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--pop-serif), 'Bangers', cursive; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .pop-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .pop-tarjeta--izq, .pop-tarjeta--der, .pop-tarjeta--talon, .pop-tarjeta--hoy { transform: none; }
  .pop-doblez::before { display: none; }
  .pop-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .pop-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .pop-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .pop-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .pop-cuenta-caja:first-child { border-left: none; }
  .pop-cuenta-caja::before { display: none; }
  .pop-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .pop-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .pop-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .pop-panel[data-tone="dark"] .pop-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .pop-panel-top, .pop-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .pop-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .pop-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .pop-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .pop-btn-solido, .pop-btn-papel, .pop-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .pop-btn-papel { background: transparent; border: 1px solid currentColor; }
  .pop-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .pop-portada { background: var(--pp-bg); }
  .pop-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .pop-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .pop-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .pop-cartel-chinche { display: none; }
  .pop-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .pop-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .pop-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Pop: el titular tiene contorno y sombra dura, como una viñeta. */
  .pop-cartel-nombres, .pop-h2 { letter-spacing: .02em; -webkit-text-stroke: 2px var(--pp-ink); text-shadow: 4px 4px 0 var(--pp-acc); }
  .pop-trama { opacity: .18; background-size: 8px 8px; }

  @media (prefers-reduced-motion: reduce) {
    .pop-raiz * { animation: none !important; }
    .pop-scroller [data-xin] { opacity: 1 !important; --pop-y: 0px; }
  }
`;
