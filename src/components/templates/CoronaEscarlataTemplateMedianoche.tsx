...
/**
 * CoronaEscarlataTemplateMedianoche.tsx
 *
...
export function CoronaEscarlataTemplateMedianoche({ invitation, guest, isPersonalized = false }: CoronaEscarlataTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E6F5EE";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#3FAE7D";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(63,174,125,.35), 0 18px 50px -30px #3FAE7D";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#1F6E4C" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(63,174,125,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(63,174,125,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#1F6E4C" : "#3FAE7D";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A0A0C", tinta: "#F4F1EA", acento: "#3FAE7D", clase: `${cnePlayfair.variable} ${cneMono.variable}`, fuente: "var(--cne-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="cne-scroller">
            ...
            <div className="cne-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="10,10,12" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="10,10,12" />
                </div>
              )}
            </div>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cne-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #151517 0%, #0C0C0D 52%, #0A0A0C 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="cne-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="cne-pan-sticky">
            <div data-strip="1" className="cne-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cne-panel cne-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cne-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#1F6E4C" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#1F6E4C" />
                  </svg>
              ...
              <div data-tone="dark" className="cne-panel cne-panel--center" style={{ background: "#18181B", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cne-section" style={{ background: "#18181B" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="cne-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cne-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E6F5EE" : "#3FAE7D" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="cne-section" style={{ background: "#18181B" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="cne-section" style={{ background: "#18181B" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#3FAE7D" guest={guest} />
      )}
    ...
    <>
      ...
      <div className="cne-medallion-core">
        <svg viewBox="0 0 60 34" className={compact ? "cne-tiara-svg-sm" : "cne-tiara-svg"}>
          <path d="M4 30 L4 16 L16 24 L30 6 L44 24 L56 16 L56 30 Z" fill="none" stroke="#E6F5EE" strokeWidth={2.2} />
        </svg>
...
function CneCoverHalf({
  ...
  return (
    <div className="cne-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#3FAE7D" tintColor2="#0A0A0C" effect="enfoque" scrimColorRgb="10,10,12" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#3FAE7D" tintColor2="#0A0A0C" effect="enfoque" scrimColorRgb="10,10,12" />
        </div>
      )}
...
const CNE_CSS = `
  ...
  a { color: #3FAE7D; text-decoration: none; }
  a:hover { color: #E6F5EE; }
  ...
  .cne-date-month { font-family: var(--cne-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #3FAE7D; padding-left: 12%; }

  .cne-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cne-divider-line { width: 52px; height: 1px; background: #3FAE7D; display: inline-block; }
  ...
  .cne-cal-link:hover { color: #3FAE7D; }
  ...
  .cne-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #1F6E4C, #1A4A34, #3FAE7D, #E6F5EE, #1F6E4C); filter: saturate(.75); }
  ...
  .cne-medallion-sub--accent { color: #3FAE7D; }
  .cne-medallion-arc { position: absolute; inset: -14%; }
  .cne-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #3FAE7D; opacity: 0.4; font-family: var(--cne-mono), monospace; }

  .cne-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(63,174,125,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .cne-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E6F5EE, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .cne-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .cne-cd-box { border: 1px solid #232320; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .cne-cd-num { font-family: var(--cne-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #E6F5EE; display: inline-block; }
  .cne-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #3FAE7D; }
  .cne-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A0A0C 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .cne-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(63,174,125,0.3), rgba(31,110,76,0.24), rgba(63,174,125,0.34), rgba(63,174,125,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .cne-accent-italic { font-style: italic; color: #3FAE7D; }
  .cne-accent-serif { font-style: italic; color: #1F6E4C; font-family: var(--cne-playfair), serif; }
  .cne-accent-serif-2 { color: #1F6E4C; }
  ...
  .cne-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #1F6E4C; margin-top: auto; }
  ...
  .cne-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #1F6E4C; }
  ...
  .cne-crono-time { font-family: var(--cne-mono), monospace; color: #1F6E4C; min-width: 42px; }
  ...
  .cne-stub { position: relative; overflow: hidden; border: 1px solid #232320; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cne-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E6F5EE, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .cne-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(63,174,125,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cne-rsvp-payment-total { color: #E6F5EE; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .cne-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #3FAE7D; background: transparent; color: #3FAE7D; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cne-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(63,174,125,0.3); color: #F4F1EA; font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cne-rsvp-input::placeholder { color: #6E6A5D; }
  .cne-rsvp-input:focus { outline: none; border-bottom-color: #3FAE7D; }
  .cne-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cne-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #3FAE7D, #1F6E4C); border: 1px solid #3FAE7D; color: #0B0B10; cursor: pointer; }
  .cne-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cne-rsvp-btn--ghost { background: transparent; color: #3FAE7D; }
  .cne-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #3FAE7D; margin: 0; }
  ...
  .cne-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(63,174,125,0.18); }
  ...
  .cne-bank-copy { flex-shrink: 0; font-family: var(--cne-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #3FAE7D; background: transparent; color: #3FAE7D; cursor: pointer; }
  .cne-bank-copy:hover { background: rgba(63,174,125,0.12); }
  ...
  .cne-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(63,174,125,0.3); padding-bottom: 12px; }
  ...
  .cne-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(63,174,125,0.3); color: #F4F1EA; font-family: var(--cne-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cne-song-input::placeholder { color: #6E6A5D; }
  .cne-song-input:focus { outline: none; border-bottom-color: #3FAE7D; }
  .cne-song-sep { color: #8A8577; flex-shrink: 0; }
  .cne-song-submit { flex-shrink: 0; background: none; border: none; color: #3FAE7D; font-family: var(--cne-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cne-song-submit:hover { color: #E6F5EE; }
  .cne-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cne-song-error { font-size: 10px; color: #3FAE7D; margin-top: 6px; }
  ...
  .cne-quiz-opt { font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(63,174,125,0.4); background: transparent; color: #3FAE7D; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cne-quiz-opt:disabled { cursor: default; }
  .cne-quiz-opt--picked { background: #3FAE7D; border-color: #3FAE7D; color: #0B0B10; }
  .cne-quiz-opt--correct { background: #3FAE7D; border-color: #3FAE7D; color: #0B0B10; }
  .cne-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cne-quiz-result { padding-top: 18px; border-top: 1px solid rgba(63,174,125,0.2); }
  .cne-quiz-result-score { margin: 0 0 6px; font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E6F5EE; }
  .cne-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .cne-final-card { border: 1px solid #3FAE7D; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .cne-final-names { font-family: var(--cne-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E6F5EE; }
  ...
  .cne-replay { cursor: pointer; color: #3FAE7D; }
  .cne-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cne-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(63,174,125,0.14); }
  .cne-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cne-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(63,174,125,0.16); position: relative; }
  .cne-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E6F5EE, #3FAE7D); transition: height 260ms linear; display: block; }
  .cne-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #3FAE7D; transition: color 500ms ease; }
  ...
  .cne-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #202023 0%, #18181B 46%, #0A0A0C 100%); }
  .cne-cover-half--bottom .cne-cover-inner { top: auto; bottom: 0; }
  .cne-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(31,110,76,0.6), rgba(74,14,20,0.5), rgba(63,174,125,0.55), rgba(31,110,76,0.6)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .cne-cover-lattice { position: absolute; inset: 0; background-image: repeating-linear-gradient(45deg, rgba(63,174,125,.14) 0 1px, transparent 1px 24px), repeating-linear-gradient(-45deg, rgba(63,174,125,.14) 0 1px, transparent 1px 24px); }
  ...
  .cne-cover-names { margin: 0; font-family: var(--cne-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E6F5EE; }
  .cne-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#3FAE7D, transparent); display: block; }
  ...
  .cne-cover-cta { border: 1px solid #3FAE7D; background: linear-gradient(100deg, rgba(63,174,125,0.08), rgba(230,245,238,0.2), rgba(63,174,125,0.08)); color: #E6F5EE; font-family: var(--cne-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cne-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cne-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(31,110,76,0.34), rgba(230,245,238,0.5), rgba(31,110,76,0.34)); color: #0B0B0F; }
  }
  ...
  .cne-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #3FAE7D; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
