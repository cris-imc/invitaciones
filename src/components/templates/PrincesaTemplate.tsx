  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#22111F", tinta: "#F4F1EA", acento: "#B48CC9", clase: `${prcCormorant.variable} ${prcMono.variable}`, fuente: "var(--prc-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="prc-scroller">
            ...
            <div className="prc-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="34,17,31" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="34,17,31" />
                </div>
              )}
            </div>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="prc-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #22111F 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="prc-section" style={{ background: "#2A1628" }}>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="prc-section" style={{ background: "#2A1628" }}>
  ...
  return (
    <div className="prc-cover-inner">
      {/* Con foto cargada, la foto reemplaza el degradé de fondo -- el
          resto de la ornamentación (glow, destellos, medallón, nombres,
          CTA) se mantiene arriba sin cambios. Misma lógica que la
          Colección Flat: SOLO en mobile (el efecto está pensado para el
          recorte vertical de una foto de celular); en desktop el degradé
          original se ve igual que siempre, con o sin foto cargada. */}
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto
...
const PRC_CSS = `
  ...
  /* Foto principal: mobile borde a borde, desktop enmarcada con el acento
     propio de la familia en vez de estirarse. */
  .prc-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #22111F; }
  ...
  @media (min-width: 768px) {
    .prc-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(180,140,201,.3); }
  ...
  .prc-cal-link:hover { color: #B48CC9; }
  ...
  .prc-cover-names { margin: 0; font-family: var(--prc-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F7E9F0; }
