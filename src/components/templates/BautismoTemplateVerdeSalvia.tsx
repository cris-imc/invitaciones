...
/**
 * BautismoTemplateVerdeSalvia.tsx
 *
 * Portado 1:1 desde el mockup "Bautismo - Luz y Lino" (mockup/Eventos),
...
interface BautismoTemplateVerdeSalviaProps {
...
export function BautismoTemplateVerdeSalvia({ invitation, guest, isPersonalized = false }: BautismoTemplateVerdeSalviaProps) {
  const tx = useTextos();
  ...
  const namesTitle = festejado || "Bautismo";
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F2F7EF";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#A9C4A0";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #A9C4A0";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#5E7A56" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#5E7A56" : "#A9C4A0";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#11120E", tinta: "#F4F1EA", acento: "#A9C4A0", clase: `${btsDisplay.variable} ${btsMono.variable}`, fuente: "var(--bts-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${btsDisplay.variable} ${btsMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#11120E",
        fontFamily: "var(--bts-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{GP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="bts-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="bts-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #161712 55%, #11120E 100%)" }}>
          ...
          <div className="bts-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="bts-date-num bts-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="bts-divider">
            <span className="bts-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="bts-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="bts-cal-link"
          />
        ...
        {(photoMobile || photoDesktop) && (
          ...
          >
            ...
            <span data-xin="1" data-dist="-60" className="bts-kicker bts-hero-photo-kicker">02 — LA PRIMERA LUZ</span>
          </section>
        )}

        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="bts-section bts-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A2A1C 0%, #0D0B10 55%, #11120E 100%)" }}>
          <div className="bts-scan-grid" />
          ...
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="bts-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #11120E 100%)" }}>
            ...
            <h2 ref={phraseRef} className="bts-phrase" style={{ fontSize: fraseFontSize }}>
              {fraseWords.map((w, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= fraseAccentStart ? "bts-accent-italic" : undefined}>
                    {w}
                  </span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="120" data-dist="90" className="bts-divider" style={{ position: "relative" }}>
              <span className="bts-divider-line bts-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
            </div>
          </section>
        )}
          ...
          <div className="bts-pan-sticky">
            <div data-strip="1" className="bts-strip">
              <div id="details" data-tone="light" className="bts-panel bts-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="bts-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="bts-crono-row">
                        <span className="bts-crono-time">{item.time || ""}</span>
                        <span className="bts-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="bts-panel bts-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="bts-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#5E7A56" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#5E7A56" />
                  </svg>
            ...
            <BtsDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="bts-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #161712 60%, #11120E 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="bts-rsvp">
              <BtsRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="bts-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="bts-pan-sticky">
            <div data-strip="1" className="bts-strip">
                ...
                <div key={pageIndex} data-tone="light" className="bts-panel bts-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="bts-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`bts-mosaic-cell${i === 0 ? " bts-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="bts-seguir bts-seguir--split">
                    ...
                    <span className="bts-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <BtsDots count={photoPages.length} />
          </div>
        </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="bts-section" style={{ background: "#161712" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="bts-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="bts-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F2F7EF" : "#A9C4A0" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="bts-song-wrap">
              <BtsSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="bts-section bts-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #161712 55%, #11120E 100%)" }}>
          ...
          <div className="bts-footer-credit">
      ...
      <div ref={railRef} className="bts-rail">
        ...
        <span ref={railLabelRef} className="bts-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="bts-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="bts-cover-half bts-cover-half--top">
          <BtsCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="bts-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="bts-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#A9C4A0" guest={guest} />
      )}
...
function BtsCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="bts-cd-box">
      <span ref={refEl} className="bts-cd-num">—</span>
      <span className="bts-cd-label">{label}</span>
    </div>
  );
}

function BtsDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="bts-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="bts-dot" />
      ))}
    </div>
  );
}

function BtsMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="bts-medallion-arc" style={{ animation: spin === "reverse" ? "btsRingRev 32s linear infinite" : "btsRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="bts-medallion-arc-text">
...
function BtsCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="bts-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="bts-bank-row-label">{label}</span>
    ...
    <>
      <div className="bts-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="bts-rsvp-row bts-rsvp-row--payment">
      ...
      <div ref={stubRef} className="bts-stub">
        ...
        <div ref={sealRef} className="bts-seal">
          <BtsMedallionCmp label="BB" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="bts-beam" />
        <div className="bts-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="bts-rsvp-error">{error}</p>}
...
function BtsSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="bts-song">
      ...
      {error && <p className="bts-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="bts-song-list">
...
function BtsQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: BtsQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="bts-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="bts-quiz-q">
            <span className="bts-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="bts-quiz-q-text">{q.pregunta}</p>
            <div className="bts-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " bts-quiz-opt--correct";
                  else if (chosen) stateClass = " bts-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " bts-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`bts-quiz-opt${stateClass}`}
...
function BtsInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="bts-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="bts-bank-row-label">{label}</span>
    ...
    <div className="bts-cover-inner">
      ...
      <div className="bts-cover-content">
        ...
        <div className="bts-cover-center">
          ...
          <h1 ref={namesRef} className="bts-cover-names">{namesTitle}</h1>
          <span className="bts-cover-rule" />
          <span className="bts-cover-date">{fechaCorta}</span>
        </div>
        <div className="bts-cover-bottom">
          <div ref={perfRef} className="bts-perf-strip bts-perf-strip--reveal bts-perf-strip--cover" />
          <div className="bts-cover-facts">
...
// ---------------------------------------------------------------------
const GP_CSS = `
  .bts-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .bts-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #A9C4A0; text-decoration: none; }
  a:hover { color: #F2F7EF; }

  @keyframes bbsFoil { to { transform: rotate(360deg); } }
  @keyframes btsRing { to { transform: rotate(360deg); } }
  @keyframes btsRingRev { to { transform: rotate(-360deg); } }
  ...
  @media (prefers-reduced-motion: reduce) { .bts-scroller * { animation: none !important; } }

  .bts-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .bts-section--between { justify-content: space-between; }
  ...
  .bts-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #11120E; }
  .bts-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  .bts-hero-photo-kicker { position: absolute; left: 0; right: 0; bottom: 0; z-index: 2; padding: 0 max(24px, calc((100% - 560px) / 2)) 48px; }
  ...
  @media (max-width: 767px) {
    .bts-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .bts-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(200,164,92,.3); }
    .bts-hero-photo-kicker { bottom: 40px; }
    .bts-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .bts-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .bts-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .bts-date-num { font-family: var(--bts-display), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .bts-date-num--right { text-align: right; line-height: 0.86; }
  .bts-date-month { font-family: var(--bts-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #A9C4A0; padding-left: 12%; }

  .bts-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .bts-divider-line { width: 52px; height: 1px; background: #A9C4A0; display: inline-block; }
  .bts-divider-line--long { width: 64px; }

  .bts-lead { margin: 0; font-family: var(--bts-display), serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .bts-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--bts-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .bts-cal-link:hover { color: #A9C4A0; }

  .bts-medallion { position: relative; }
  .bts-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .bts-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .bts-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .bts-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #A9C4A0, #F2F7EF, #6E5AA8); filter: saturate(.75); }
  .bts-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .bts-medallion-label { font-family: var(--bts-display), serif; font-size: 26px; line-height: 1; color: #F2F7EF; }
  .bts-medallion-label-sm { font-family: var(--bts-display), serif; font-size: 16px; color: #F2F7EF; }
  .bts-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .bts-medallion-sub--accent { color: #A9C4A0; }
  .bts-medallion-arc { position: absolute; inset: -14%; }
  ...
  .bts-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #A9C4A0; opacity: 0.4; font-family: var(--bts-mono), monospace; }

  .bts-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .bts-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F2F7EF, transparent); animation: bbsScan 6s linear infinite; pointer-events: none; }

  .bts-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .bts-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .bts-cd-num { font-family: var(--bts-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F2F7EF; display: inline-block; }
  .bts-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #A9C4A0; }
  .bts-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #11120E 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .bts-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .bts-perf-strip--cover { opacity: .9; }

  .bts-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(200,164,92,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: bbsFoil 30s linear infinite; }
  .bts-phrase { margin: 0; position: relative; font-family: var(--bts-display), serif; font-weight: 400; font-size: clamp(50px, 15vw, 96px); line-height: 0.92; letter-spacing: -0.03em; }
  .bts-accent-italic { font-style: italic; color: #A9C4A0; }
  .bts-accent-serif { font-style: italic; color: #5E7A56; font-family: var(--bts-display), serif; }
  .bts-accent-serif-2 { color: #5E7A56; }
  .bts-h2 { margin: 0; font-family: var(--bts-display), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .bts-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .bts-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .bts-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .bts-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .bts-panel--between { justify-content: space-between; }
  ...
  .bts-pan[data-scroll="vertical"] { height: auto; }
  .bts-pan[data-scroll="vertical"] .bts-pan-sticky { position: static; height: auto; overflow: visible; }
  .bts-pan[data-scroll="vertical"] .bts-strip { position: static; display: block; width: 100%; transform: none !important; }
  .bts-pan[data-scroll="vertical"] .bts-panel { height: calc(var(--vh, 1vh) * 100); }
  .bts-panel--end { justify-content: flex-end; }
  .bts-panel--center { align-items: center; justify-content: center; text-align: center; }
  .bts-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .bts-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .bts-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .bts-panel-title { margin: 0; position: relative; font-family: var(--bts-display), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .bts-panel-title-md { margin: 0; position: relative; font-family: var(--bts-display), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .bts-panel-title-sm { margin: 0; font-family: var(--bts-display), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .bts-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .bts-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .bts-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  .bts-facts { position: relative; display: flex; flex-direction: column; gap: 12px; font-size: 11px; letter-spacing: 0.14em; color: #4A473F; }
  .bts-facts-row { display: flex; justify-content: space-between; border-bottom: 1px solid #D6D1C4; padding-bottom: 10px; }
  .bts-facts-row--last { border-bottom: none; padding-bottom: 0; }
  .bts-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #5E7A56; margin-top: auto; }
  .bts-seguir--split { justify-content: space-between; }
  .bts-side-hint { display: inline-block; animation: bbsSide 2.2s ease-in-out infinite; }
  .bts-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .bts-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #5E7A56; }

  .bts-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .bts-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .bts-crono-time { font-family: var(--bts-mono), monospace; color: #5E7A56; min-width: 42px; }
  .bts-crono-title { flex: 1; }

  .bts-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .bts-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .bts-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .bts-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .bts-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .bts-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .bts-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F2F7EF, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .bts-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--bts-mono), monospace; }
  .bts-rsvp-rows { display: flex; flex-direction: column; }
  .bts-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .bts-rsvp-row > span:first-child { flex-shrink: 0; }
  .bts-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .bts-rsvp-row--payment { align-items: flex-start; }
  .bts-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .bts-rsvp-payment-total { color: #F2F7EF; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .bts-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .bts-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .bts-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #A9C4A0; background: transparent; color: #A9C4A0; font-size: 14px; line-height: 1; cursor: pointer; }
  .bts-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .bts-rsvp-stepper span { font-family: var(--bts-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .bts-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--bts-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .bts-rsvp-input::placeholder { color: #6E6A5D; }
  .bts-rsvp-input:focus { outline: none; border-bottom-color: #A9C4A0; }
  .bts-rsvp-btn { width: 100%; padding: 16px; font-family: var(--bts-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #A9C4A0, #5E7A56); border: 1px solid #A9C4A0; color: #161712; cursor: pointer; }
  .bts-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .bts-rsvp-btn--ghost { background: transparent; color: #A9C4A0; }
  .bts-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #A9C4A0; margin: 0; }
  .bts-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .bts-rsvp-declined-text { margin: 0; font-family: var(--bts-display), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .bts-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .bts-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .bts-photo-cell--offset { margin-top: 18px; }
  .bts-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .bts-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .bts-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .bts-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .bts-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .bts-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .bts-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .bts-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .bts-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  ...
  .bts-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .bts-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  .bts-bank-row:last-child { border-bottom: none; }
  .bts-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .bts-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .bts-bank-copy { flex-shrink: 0; font-family: var(--bts-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #A9C4A0; background: transparent; color: #A9C4A0; cursor: pointer; }
  .bts-bank-copy:hover { background: rgba(200,164,92,0.12); }

  .bts-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .bts-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: bbsEq 1.1s ease-in-out infinite; display: inline-block; }
  .bts-song-wrap { font-family: var(--bts-mono), monospace; }

  .bts-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .bts-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  .bts-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .bts-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--bts-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .bts-song-input::placeholder { color: #6E6A5D; }
  .bts-song-input:focus { outline: none; border-bottom-color: #A9C4A0; }
  .bts-song-sep { color: #8A8577; flex-shrink: 0; }
  .bts-song-submit { flex-shrink: 0; background: none; border: none; color: #A9C4A0; font-family: var(--bts-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .bts-song-submit:hover { color: #F2F7EF; }
  .bts-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .bts-song-error { font-size: 10px; color: #A9C4A0; margin-top: 6px; }
  .bts-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .bts-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--bts-mono), monospace; }
  .bts-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .bts-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .bts-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .bts-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .bts-quiz-q-num { font-family: var(--bts-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .bts-quiz-q-text { margin: 0; font-family: var(--bts-display), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .bts-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .bts-quiz-opt { font-family: var(--bts-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #A9C4A0; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .bts-quiz-opt:disabled { cursor: default; }
  .bts-quiz-opt--picked { background: #A9C4A0; border-color: #A9C4A0; color: #161712; }
  .bts-quiz-opt--correct { background: #A9C4A0; border-color: #A9C4A0; color: #161712; }
  .bts-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .bts-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .bts-quiz-result-score { margin: 0 0 6px; font-family: var(--bts-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F2F7EF; }
  .bts-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .bts-final-card { border: 1px solid #A9C4A0; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .bts-final-names { font-family: var(--bts-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F2F7EF; }
  .bts-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .bts-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .bts-replay { cursor: pointer; color: #A9C4A0; }
  .bts-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .bts-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .bts-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .bts-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .bts-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F2F7EF, #A9C4A0); transition: height 260ms linear; display: block; }
  .bts-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #A9C4A0; transition: color 500ms ease; }

  .bts-cover { position: absolute; inset: 0; z-index: 5; }
  .bts-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .bts-cover-half--top { top: 0; }
  .bts-cover-half--bottom { bottom: 0; }
  .bts-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #20221C 0%, #161712 46%, #11120E 100%); }
  .bts-cover-half--bottom .bts-cover-inner { top: auto; bottom: 0; }
  .bts-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(124,142,122,0.4), rgba(169,196,160,0.3), rgba(169,196,160,0.5), rgba(124,142,122,0.4)); filter: blur(64px); opacity: .62; animation: bbsFoil 26s linear infinite; }
  .bts-cover-texture { position: absolute; inset: 0; background-image: repeating-linear-gradient(45deg, rgba(242,247,239,.06) 0 1px, transparent 1px 16px), repeating-linear-gradient(-45deg, rgba(242,247,239,.05) 0 1px, transparent 1px 16px); }
  .bts-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .bts-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .bts-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .bts-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .bts-cover-names { margin: 0; font-family: var(--bts-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F2F7EF; }
  .bts-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#A9C4A0, transparent); display: block; }
  .bts-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .bts-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .bts-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  ...
  .bts-cover-facts:has(> span:only-child) { justify-content: center; }
  .bts-cover-cta { border: 1px solid #A9C4A0; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--bts-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .bts-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .bts-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #161712; }
  }
  .bts-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .bts-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: bbsHint 2.4s ease-in-out infinite; }

  .bts-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .bts-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #A9C4A0; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .bts-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
