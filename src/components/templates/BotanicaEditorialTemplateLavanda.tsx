...
/**
 * BotanicaEditorialTemplateLavanda.tsx
 *
 * Portado 1:1 desde el mockup "Botanica Editorial - Panoramica" (Colección
 * Storytelling, evento Casamiento): fondo verde-negro casi opaco (#0D0A12),
 * acentos verde oliva/salvia (#6E5A8A / #B7A8CC) con un toque de foil dorado
...
interface BotanicaEditorialTemplateLavandaProps {
...
export function BotanicaEditorialTemplateLavanda({ invitation, guest, isPersonalized = false }: BotanicaEditorialTemplateLavandaProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#B7A8CC";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#6E5A8A";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #6E5A8A";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#4A3A66" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#4A3A66" : "#6E5A8A";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0D0A12", tinta: "#F4F1EA", acento: "#6E5A8A", clase: `${bteSerif.variable} ${bteMono.variable}`, fuente: "var(--bte-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="bte-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="bte-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #1C1730 0%, #130F1C 55%, #0D0A12 100%)" }}>
          ...
          <svg data-leaf="1" viewBox="0 0 200 400" className="bte-leaf-deco" aria-hidden="true">
            <path d="M100 398 C92 340 108 300 92 250 C78 205 100 175 84 130 C70 90 92 70 66 30" fill="none" stroke="#9A87B8" strokeWidth={1.4} />
            <g fill="none" stroke="#9A87B8" strokeWidth={1.1}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="bte-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #241B3C 0%, #130E20 52%, #0D0A12 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="bte-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="bte-pan-sticky">
            <div data-strip="1" className="bte-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="bte-panel bte-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="bte-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#4A3A66" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#4A3A66" />
                  </svg>
              ...
              <div data-tone="dark" className="bte-panel bte-panel--center" style={{ background: "#130F1C", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="bte-section" style={{ background: "#130F1C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="bte-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="bte-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#B7A8CC" : "#6E5A8A" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="bte-section" style={{ background: "#130F1C" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.sabor.tuLamina")} className="bte-section bte-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #1C1730 0%, #130F1C 55%, #0D0A12 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#6E5A8A" guest={guest} />
      )}
...
function BteCoverHalf({
  ...
  return (
    <div className="bte-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#6E5A8A" tintColor2="#0D0A12" effect="enfoque" scrimColorRgb="13,10,18" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#6E5A8A" tintColor2="#0D0A12" effect="enfoque" scrimColorRgb="13,10,18" />
        </div>
      )}
...
const BTE_CSS = `
  ...
  a { color: #6E5A8A; text-decoration: none; }
  a:hover { color: #B7A8CC; }
  ...
  .bte-date-month { font-family: var(--bte-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #6E5A8A; padding-left: 12%; }

  .bte-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .bte-divider-line { width: 52px; height: 1px; background: #6E5A8A; display: inline-block; }
  ...
  .bte-cal-link:hover { color: #6E5A8A; }
  ...
  .bte-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #4A3A66, #8C6F35, #6E5A8A, #B7A8CC, #4A3A66); filter: saturate(.75); }
  ...
  .bte-medallion-label { font-family: var(--bte-serif), serif; font-size: 26px; line-height: 1; color: #B7A8CC; }
  .bte-medallion-label-sm { font-family: var(--bte-serif), serif; font-size: 16px; color: #B7A8CC; }
  .bte-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .bte-medallion-sub--accent { color: #6E5A8A; }
  ...
  .bte-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #6E5A8A; opacity: 0.4; font-family: var(--bte-mono), monospace; }
  ...
  .bte-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #B7A8CC, transparent); animation: bteScan 6s linear infinite; pointer-events: none; }
  ...
  .bte-cd-num--accent { color: #B7A8CC; }
  .bte-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #6E5A8A; }
  .bte-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0D0A12 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .bte-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,138,0.36), rgba(192,138,91,0.24), rgba(154,135,184,0.4), rgba(110,90,138,0.36)); filter: blur(80px); opacity: .4; animation: bteFoil 30s linear infinite; }
  ...
  .bte-accent-italic { font-style: italic; color: #6E5A8A; }
  .bte-accent-phrase { font-style: italic; color: #9A87B8; }
  .bte-accent-serif { font-style: italic; color: #4A3A66; font-family: var(--bte-serif), serif; }
  .bte-accent-serif-2 { color: #4A3A66; }
  .bte-accent-dark-2 { color: #6E5A8A; }
  ...
  .bte-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #4A3A66; margin-top: auto; }
  ...
  .bte-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #4A3A66; }
  ...
  .bte-crono-time { font-family: var(--bte-mono), monospace; color: #4A3A66; min-width: 42px; }
  ...
  .bte-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #B7A8CC, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .bte-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(110,90,138,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .bte-rsvp-payment-total { color: #B7A8CC; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .bte-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #6E5A8A; background: transparent; color: #6E5A8A; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .bte-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(110,90,138,0.3); color: #F4F1EA; font-family: var(--bte-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .bte-rsvp-input::placeholder { color: #6E6A5D; }
  .bte-rsvp-input:focus { outline: none; border-bottom-color: #6E5A8A; }
  .bte-rsvp-btn { width: 100%; padding: 16px; font-family: var(--bte-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; border: 1px solid #6E5A8A; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 200ms ease, color 200ms ease; }
  ...
  .bte-rsvp-btn--ghost { background: transparent; color: #6E5A8A; }
  .bte-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(110,90,138,0.12); color: #6E5A8A; }
  .bte-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #6E5A8A; margin: 0; }
  ...
  .bte-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(110,90,138,0.18); }
  ...
  .bte-bank-copy { flex-shrink: 0; font-family: var(--bte-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #6E5A8A; background: transparent; color: #6E5A8A; cursor: pointer; }
  .bte-bank-copy:hover { background: rgba(110,90,138,0.12); }
  ...
  .bte-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(110,90,138,0.3); padding-bottom: 12px; }
  ...
  .bte-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(110,90,138,0.3); color: #F4F1EA; font-family: var(--bte-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .bte-song-input::placeholder { color: #6E6A5D; }
  .bte-song-input:focus { outline: none; border-bottom-color: #6E5A8A; }
  .bte-song-sep { color: #8A8577; flex-shrink: 0; }
  .bte-song-submit { flex-shrink: 0; background: none; border: none; color: #6E5A8A; font-family: var(--bte-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .bte-song-submit:hover { color: #B7A8CC; }
  .bte-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .bte-song-error { font-size: 10px; color: #6E5A8A; margin-top: 6px; }
  ...
  .bte-quiz-opt { font-family: var(--bte-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(110,90,138,0.4); background: transparent; color: #6E5A8A; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .bte-quiz-opt:disabled { cursor: default; }
  .bte-quiz-opt--picked { background: #6E5A8A; border-color: #6E5A8A; color: #0B0B10; }
  .bte-quiz-opt--correct { background: #6E5A8A; border-color: #6E5A8A; color: #0B0B10; }
  .bte-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .bte-quiz-result { padding-top: 18px; border-top: 1px solid rgba(110,90,138,0.2); }
  .bte-quiz-result-score { margin: 0 0 6px; font-family: var(--bte-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #B7A8CC; }
  .bte-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .bte-final-card { border: 1px solid #6E5A8A; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .bte-replay { cursor: pointer; color: #6E5A8A; }
  ...
  .bte-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#B7A8CC, #6E5A8A); transition: height 260ms linear; display: block; }
  .bte-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #6E5A8A; transition: color 500ms ease; }
  ...
  .bte-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1E1832 0%, #130F1C 46%, #0D0A12 100%); }
  .bte-cover-half--bottom .bte-cover-inner { top: auto; bottom: 0; }
  .bte-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,138,0.5), rgba(192,138,91,0.35), rgba(154,135,184,0.6), rgba(110,90,138,0.5)); filter: blur(64px); opacity: .62; animation: bteFoil 26s linear infinite; }
  ...
  .bte-cover-linen { position: absolute; inset: 0; background-image: repeating-linear-gradient(78deg, rgba(183,168,204,.16) 0 1px, transparent 1px 40px), repeating-linear-gradient(-32deg, rgba(183,168,204,.1) 0 1px, transparent 1px 64px); }
  ...
  .bte-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#6E5A8A, transparent); display: block; }
  ...
  .bte-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(13,10,18,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .bte-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #6E5A8A; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
