...
/**
 * CoronaEscarlataTemplateZafiro.tsx
 *
...
export function CoronaEscarlataTemplateZafiro({ invitation, guest, isPersonalized = false }: CoronaEscarlataTemplateProps) {
  const tx = useTextos();
  ...
  const coverKickerText = showGuestNameInCover ? tx("invitacion.sabor.invitacionRealPara").toUpperCase() : tx("invitacion.sabor.invitacionRealQuinceDe").toUpperCase();
  ...
  const portadaMensaje = String(
    invitation.portadaMensaje || tx("invitacion.sabor.mensajeCoronaPorUnaVez")
  );
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#EAF1F8";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#B8C4D6";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(184,196,214,.35), 0 18px 50px -30px #B8C4D6";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#2E6FB8" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(184,196,214,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(184,196,214,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#2E6FB8" : "#B8C4D6";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#050B16", tinta: "#F4F1EA", acento: "#B8C4D6", clase: `${cnePlayfair.variable} ${cneMono.variable}`, fuente: "var(--cne-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="cne-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="cne-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #0C1524 0%, #0A1C33 55%, #050B16 100%)" }}>
          <span data-xin="1" data-dist="-60" className="cne-kicker">{"01 — " + tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          ...
          <div data-drift="-70" className="cne-medallion cne-medallion--corner">
            <CoronaMedallion sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="cneArc1" arcText={tx("invitacion.sabor.arcoRealezaPorUnaNoche").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        {(photoMobile || photoDesktop) && (
          <section
            ...
            className={`cne-hero-photo-section${!photoMobile ? " cne-hero-photo-section--no-mobile" : ""}${!photoDesktop ? " cne-hero-photo-section--no-desktop" : ""}`}
          >
            <div className="cne-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="5,11,22" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="5,11,22" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="cne-section cne-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #123A63 0%, #13233A 55%, #050B16 100%)" }}>
          <div className="cne-scan-grid" />
          <div className="cne-scanline" />
          <span data-xin="1" data-dist="-60" className="cne-kicker" style={{ position: "relative" }}>{knPre(2)} — LA CORONACIÓN ES EN</span>
          <div className="cne-cd-grid">
            <CneCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <CneCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <CneCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <CneCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
          <div className="cne-perf-strip" />
        </section>

        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cne-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #101A30 0%, #090C14 52%, #050B16 100%)" }}>
          <div data-drift="-130" className="cne-glow-blob" />
          <span data-xin="1" data-dist="-60" className="cne-kicker" style={{ position: "relative" }}>{knPre(3)} {"— " + tx("invitacion.frase.unMensajeParaVos").toUpperCase()}</span>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="cne-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
            ...
            <div data-strip="1" className="cne-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="cne-panel cne-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <div className="cne-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="cne-side-hint">{scrollVertical ? "↓" : "→"}</span></div>
                </div>
              )}
              ...
              <div id="details" data-tone="light" className="cne-panel cne-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                <div className="cne-facts">
                  ...
                  {dressCode && (
                    <div className="cne-facts-row cne-facts-row--last">
                      <span>{tx("invitacion.pase.codigo").toUpperCase()}</span><span className="cne-accent-serif-2">{dressCode.toUpperCase()}</span>
                    </div>
                  )}
                </div>
                {cronograma.length > 0 && (
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cne-panel cne-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cne-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#2E6FB8" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#2E6FB8" />
                  </svg>
                  <div className="cne-panel-block">
                    <span className="cne-mini-label">{ceremoniaHabilitada ? "03" : "02"} / {LUGAR_PANEL_COUNT}</span>
                    <span className="cne-panel-title-sm">{tx("invitacion.ubicacion.comoLlegar")}</span>
              ...
              <div data-tone="dark" className="cne-panel cne-panel--center" style={{ background: "#0A1C33", color: "#F4F1EA" }}>
                <div className="cne-medallion cne-medallion--lg">
                  <CoronaMedallion sub={tx("invitacion.pase.numeroCorona", { n: passNumber }).toUpperCase()} arcId="cneArc2" arcText={tx("invitacion.pase.arcoAccesoVipCorona", { n: passNumber }).toUpperCase()} spin="reverse" />
                </div>
                <span className="cne-mini-label">{LUGAR_PANEL_COUNT} / {LUGAR_PANEL_COUNT} {"— " + tx("invitacion.ubicacion.tuUbicacion").toUpperCase()}</span>
                {/* mesa-en-ubicacion */}
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="cne-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #0C1524 0%, #0A1C33 60%, #050B16 100%)" }}>
          <span data-xin="1" data-dist="-60" className="cne-kicker">{kn(4)} — CHECK-IN</span>
          <h2 data-xin="1" data-delay="80" data-dist="130" className="cne-h2">
            {tx("invitacion.rsvp.confirmaLinea1")}<br /><span className="cne-accent-italic">tu asistencia</span>
          </h2>
          ...
          {rsvpEnabled ? (
            ...
            <p className="cne-lead">{tx("invitacion.rsvp.cerrada")}</p>
          )}
        </section>

        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="cne-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="cne-pan-sticky">
            <div data-strip="1" className="cne-strip">
                ...
                <div key={pageIndex} data-tone="light" className="cne-panel cne-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="cne-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      ...
                      <span className="cne-photo-placeholder">{tx("invitacion.album.sinFotos")}</span>
                    )}
                  </div>
                  <div className="cne-seguir cne-seguir--split">
                    <span>{tx("invitacion.album.fotosSubidas", { n: allPhotos.length }).toUpperCase()}</span>
                    <span className="cne-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cne-section" style={{ background: "#0A1C33" }}>
            <span data-xin="1" data-dist="-60" className="cne-kicker">{kn(6)} {"— " + tx("invitacion.musica.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="cne-h2">{tituloEnDosLineas(tx("invitacion.sabor.preguntaValsAbreNoche"), "cne-accent-italic")}</h2>
            <div data-xin="1" data-delay="160" data-dist="-80" className="cne-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cne-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#EAF1F8" : "#B8C4D6" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="cne-section" style={{ background: "#0A1C33" }}>
            <span data-xin="1" data-dist="-60" className="cne-kicker">{sugerenciaMusicaHabilitada ? kn(7) : kn(6)} {"— " + tx("invitacion.regalos.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="cne-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><span className="cne-accent-italic">{tx("invitacion.regalos.siQueresLinea2")}</span>
            </h2>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="cne-section" style={{ background: "#0A1C33" }}>
            <span data-xin="1" data-dist="-60" className="cne-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection].filter(Boolean).length + 6)} {"— " + tx("invitacion.quiz.kicker").toUpperCase()}</span>
        ...
        <section data-tone="dark" data-screen-label="Tu corona" className="cne-section cne-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #0C1524 0%, #0A1C33 55%, #050B16 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="cne-final-card">
            ...
            <span className="cne-mini-label cne-accent-serif-2">{tx("invitacion.pase.numeroCoronaAdmite", { n: passNumber, cantidad: guestAdults + guestTeens + guestChildren || 1 }).toUpperCase()}</span>
          ...
          <div className="cne-final-footer">
            ...
            <span className="cne-replay" onClick={reset}>{tx("invitacion.portada.verAperturaOtraVez").toUpperCase() + " ↺"}</span>
          </div>
      ...
      <div ref={railRef} className="cne-rail">
        <span ref={railTopRef} className="cne-rail-top">{tx("invitacion.pase.numeroCorona", { n: passNumber }).toUpperCase()}</span>
      ...
      <div ref={coverRef} className="cne-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="cne-cover-half cne-cover-half--top">
          ...
          >
            <div className="cne-cover-cta">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</div>
          </CneCoverHalf>
        </div>
        <div ref={bottomRef} className="cne-cover-half cne-cover-half--bottom">
          <CneCoverHalf
            ...
            photoDesktop={photoDesktop}
          >
            <button onClick={open} className="cne-cover-cta cne-cover-cta--btn">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</button>
          </CneCoverHalf>
        </div>
      </div>

      <div ref={hintRef} className="cne-hint">{tx("invitacion.portada.desliza").toUpperCase() + " ↓"}</div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#B8C4D6" guest={guest} />
      )}
    ...
    <>
      ...
      <div className="cne-medallion-core">
        <svg viewBox="0 0 60 34" className={compact ? "cne-tiara-svg-sm" : "cne-tiara-svg"}>
          <path d="M4 30 L4 16 L16 24 L30 6 L44 24 L56 16 L56 30 Z" fill="none" stroke="#EAF1F8" strokeWidth={2.2} />
        </svg>
  ...
  if (status === "DECLINED") {
    return (
      <div className="cne-rsvp-declined">
        <p className="cne-rsvp-declined-text">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
  ...
  return (
    <>
      <div className="cne-rsvp-rows">
        ...
        {status === "CONFIRMED" && (
          <>
            {totalGuests > 1 && adultCount > 0 && <div className="cne-rsvp-row"><span>{tx("invitacion.rsvp.adultos").toUpperCase()}</span><span>{String(adultCount).padStart(2, "0")}</span></div>}
            {teenCount > 0 && <div className="cne-rsvp-row"><span>{tx("invitacion.rsvp.adolescentes").toUpperCase()}</span><span>{String(teenCount).padStart(2, "0")}</span></div>}
            {childCount > 0 && <div className="cne-rsvp-row"><span>{tx("invitacion.rsvp.ninos").toUpperCase()}</span><span>{String(childCount).padStart(2, "0")}</span></div>}
          </>
        )}
          ...
          <div className="cne-rsvp-row cne-rsvp-row--payment">
            ...
            <div className="cne-rsvp-payment-value">
              ...
              {paymentStatus === "PARTIAL" && (
                <div className="cne-rsvp-payment-detail"><span>{tx("invitacion.pago.pagoParcialRegistrado")}</span></div>
              )}
      ...
      <div ref={stubRef} className="cne-stub">
        <div className="cne-stub-top">
          <span>{tx("invitacion.pase.numeroCorona", { n: passNumber }).toUpperCase()}</span>
    ...
    <div className="cne-song">
      <form onSubmit={handleSubmit} className="cne-song-row">
        <div className="cne-song-inputs">
          <input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder={tx("invitacion.musica.artista").toUpperCase()} maxLength={80} className="cne-song-input" />
          <span className="cne-song-sep">—</span>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tx("invitacion.musica.tema").toUpperCase()} maxLength={100} className="cne-song-input" />
        </div>
        ...
        <div className="cne-song-list">
          {songs.slice(0, 12).map((s, i) => (
            <div key={s.id} className="cne-song-item">
              ...
              <span className="cne-song-item-by">{tx("invitacion.musica.sumadoPor")} {s.guestName || tx("invitacion.evento.invitado")}</span>
            </div>
          ))}
        </div>
      )}
...
function CneCoverHalf({
  ...
  return (
    <div className="cne-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#B8C4D6" tintColor2="#050B16" effect="enfoque" scrimColorRgb="5,11,22" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#B8C4D6" tintColor2="#050B16" effect="enfoque" scrimColorRgb="5,11,22" />
        </div>
      )}
      ...
      <div className="cne-cover-content">
        <div className="cne-cover-top-row">
          <span>{tx("invitacion.pase.numeroCorona", { n: passNumber }).toUpperCase()}</span><span className="cne-accent-serif-2">ALL ACCESS</span>
        </div>
        ...
        <div className="cne-cover-bottom">
          ...
          <div className="cne-barcode-wrap">
            ...
            <span className="cne-mini-label cne-mini-label--cover" style={{ color: "#56534A" }}>{tx("invitacion.pase.noTransferible").toUpperCase()}</span>
          </div>
        </div>
      </div>
    </div>
...
const CNE_CSS = `
  ...
  a { color: #B8C4D6; text-decoration: none; }
  a:hover { color: #EAF1F8; }
  ...
  .cne-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #050B16; }
  ...
  @media (max-width: 767px) {
    .cne-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .cne-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(184,196,214,.3); }
    .cne-hero-photo-kicker { bottom: 40px; }
    .cne-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }
  ...
  .cne-date-month { font-family: var(--cne-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B8C4D6; padding-left: 12%; }

  .cne-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cne-divider-line { width: 52px; height: 1px; background: #B8C4D6; display: inline-block; }
  ...
  .cne-cal-link:hover { color: #B8C4D6; }
  ...
  .cne-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2E6FB8, #2C3F6E, #B8C4D6, #EAF1F8, #2E6FB8); filter: saturate(.75); }
  ...
  .cne-medallion-sub--accent { color: #B8C4D6; }
  .cne-medallion-arc { position: absolute; inset: -14%; }
  .cne-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B8C4D6; opacity: 0.4; font-family: var(--cne-mono), monospace; }

  .cne-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(184,196,214,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .cne-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #EAF1F8, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .cne-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .cne-cd-box { border: 1px solid #1B2636; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .cne-cd-num { font-family: var(--cne-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #EAF1F8; display: inline-block; }
  .cne-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B8C4D6; }
  .cne-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #050B16 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .cne-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(184,196,214,0.3), rgba(46,111,184,0.24), rgba(184,196,214,0.34), rgba(184,196,214,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .cne-accent-italic { font-style: italic; color: #B8C4D6; }
  .cne-accent-serif { font-style: italic; color: #2E6FB8; font-family: var(--cne-playfair), serif; }
  .cne-accent-serif-2 { color: #2E6FB8; }
  .cne-h2 { margin: 0; font-family: var(--cne-playfair), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .cne-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .cne-pan[data-scroll="vertical"] { height: auto; }
  .cne-pan[data-scroll="vertical"] .cne-pan-sticky { position: static; height: auto; overflow: visible; }
  .cne-pan[data-scroll="vertical"] .cne-strip { position: static; display: block; width: 100%; transform: none !important; }
  .cne-pan[data-scroll="vertical"] .cne-panel { height: calc(var(--vh, 1vh) * 100); }
  ...
  .cne-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #2E6FB8; margin-top: auto; }
  ...
  .cne-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #2E6FB8; }
  ...
  .cne-crono-time { font-family: var(--cne-mono), monospace; color: #2E6FB8; min-width: 42px; }
  ...
  .cne-stub { position: relative; overflow: hidden; border: 1px solid #1B2636; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cne-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #EAF1F8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .cne-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(184,196,214,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cne-rsvp-payment-total { color: #EAF1F8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .cne-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B8C4D6; background: transparent; color: #B8C4D6; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cne-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(184,196,214,0.3); color: #F4F1EA; font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cne-rsvp-input::placeholder { color: #6E6A5D; }
  .cne-rsvp-input:focus { outline: none; border-bottom-color: #B8C4D6; }
  .cne-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cne-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B8C4D6, #2E6FB8); border: 1px solid #B8C4D6; color: #0B0B10; cursor: pointer; }
  .cne-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cne-rsvp-btn--ghost { background: transparent; color: #B8C4D6; }
  .cne-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B8C4D6; margin: 0; }
  ...
  .cne-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(184,196,214,0.18); }
  ...
  .cne-bank-copy { flex-shrink: 0; font-family: var(--cne-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B8C4D6; background: transparent; color: #B8C4D6; cursor: pointer; }
  .cne-bank-copy:hover { background: rgba(184,196,214,0.12); }
  ...
  .cne-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(184,196,214,0.3); padding-bottom: 12px; }
  ...
  .cne-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(184,196,214,0.3); color: #F4F1EA; font-family: var(--cne-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cne-song-input::placeholder { color: #6E6A5D; }
  .cne-song-input:focus { outline: none; border-bottom-color: #B8C4D6; }
  .cne-song-sep { color: #8A8577; flex-shrink: 0; }
  .cne-song-submit { flex-shrink: 0; background: none; border: none; color: #B8C4D6; font-family: var(--cne-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cne-song-submit:hover { color: #EAF1F8; }
  .cne-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cne-song-error { font-size: 10px; color: #B8C4D6; margin-top: 6px; }
  ...
  .cne-quiz-opt { font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(184,196,214,0.4); background: transparent; color: #B8C4D6; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cne-quiz-opt:disabled { cursor: default; }
  .cne-quiz-opt--picked { background: #B8C4D6; border-color: #B8C4D6; color: #0B0B10; }
  .cne-quiz-opt--correct { background: #B8C4D6; border-color: #B8C4D6; color: #0B0B10; }
  .cne-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cne-quiz-result { padding-top: 18px; border-top: 1px solid rgba(184,196,214,0.2); }
  .cne-quiz-result-score { margin: 0 0 6px; font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #EAF1F8; }
  .cne-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .cne-final-card { border: 1px solid #B8C4D6; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .cne-final-names { font-family: var(--cne-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #EAF1F8; }
  ...
  .cne-replay { cursor: pointer; color: #B8C4D6; }
  .cne-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cne-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(184,196,214,0.14); }
  .cne-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cne-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(184,196,214,0.16); position: relative; }
  .cne-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#EAF1F8, #B8C4D6); transition: height 260ms linear; display: block; }
  .cne-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B8C4D6; transition: color 500ms ease; }
  ...
  .cne-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #164272 0%, #0A1C33 46%, #050B16 100%); }
  .cne-cover-half--bottom .cne-cover-inner { top: auto; bottom: 0; }
  .cne-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(46,111,184,0.6), rgba(74,14,20,0.5), rgba(184,196,214,0.55), rgba(46,111,184,0.6)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .cne-cover-lattice { position: absolute; inset: 0; background-image: repeating-linear-gradient(45deg, rgba(184,196,214,.14) 0 1px, transparent 1px 24px), repeating-linear-gradient(-45deg, rgba(184,196,214,.14) 0 1px, transparent 1px 24px); }
  ...
  .cne-cover-names { margin: 0; font-family: var(--cne-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EAF1F8; }
  .cne-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B8C4D6, transparent); display: block; }
  ...
  .cne-cover-cta { border: 1px solid #B8C4D6; background: linear-gradient(100deg, rgba(184,196,214,0.08), rgba(234,241,248,0.2), rgba(184,196,214,0.08)); color: #EAF1F8; font-family: var(--cne-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cne-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cne-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(46,111,184,0.34), rgba(234,241,248,0.5), rgba(46,111,184,0.34)); color: #0B0B0F; }
  }
  ...
  .cne-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B8C4D6; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
