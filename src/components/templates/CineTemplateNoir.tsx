 ...
 * Elena & Tomás): fondo casi negro cálido (#17130F) + acento sepia/latón
 * (#8FA3B0, variable por color), tipografía serif clásica itálica (EB
...
interface CineTemplateNoirProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#8FA3B0]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#8FA3B0] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#F0E9DC] break-all">{value}</span>
      </div>
      <button 
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#8FA3B0]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#8FA3B0] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#F0E9DC] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#F0E9DC" }}>
          ¡Juego Completado!
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#E4DAC8" }}>
          RESPONDISTE {score} DE {preguntas.length} CORRECTAMENTE ({percent}%)
        </p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#9C8F7A" }}>Guardando tus resultados...</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#8FA3B0] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#E4DAC8" }}>
                  El promedio global de aciertos del resto de los invitados ({stats.count}) es del <strong style={{ color: "#F0E9DC" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#E4DAC8] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function CineTemplateNoir({ invitation, guest, isPersonalized = false }: CineTemplateNoirProps) {
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(143,163,176, 0.2) !important;
        }
        ...
        #rsvp.section.dark input {
          background-color: #1D1811 !important;
          color: #F0E9DC !important;
          border-radius: 6px !important;
          border: 1px solid rgba(143,163,176, 0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #8FA3B0 !important;
          color: #17130F !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #8FA3B0 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #17130F !important; /* Matches light sections */
          color: #F0E9DC !important;
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div style={{
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(143,163,176,0.14), transparent), radial-gradient(45% 40% at 85% 80%, rgba(107,70,48,0.22), transparent)',
            backgroundSize: '160% 160%',
            animation: 'cine-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(143,163,176,0.14), transparent 70%)',
          ...
          <IconClapper className="cine-doodle opacity-0 absolute" style={{ width: 38, height: 30, top: '9%', left: '10%', color: 'rgba(143,163,176,0.5)' }} />
          <IconStarDoodle className="cine-doodle opacity-0 absolute" style={{ width: 18, height: 18, top: '15%', right: '14%', color: 'rgba(143,163,176,0.5)' }} />
          <IconFilmReel className="cine-doodle opacity-0 absolute" style={{ width: 30, height: 18, bottom: '20%', left: '12%', color: 'rgba(143,163,176,0.4)' }} />
          <IconStarDoodle className="cine-doodle opacity-0 absolute" style={{ width: 12, height: 12, bottom: '26%', right: '18%', color: 'rgba(143,163,176,0.35)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="cine-seal opacity-0" style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #8FA3B0',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#8FA3B0',
            }}>
              {monogram}
            </div>

            {/* Guest Name */}
            <h2 className="text-4xl sm:text-5xl font-light tracking-wide text-[#E4DAC8] leading-relaxed" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className=" text-sm font-medium text-[#9C8F7A] tracking-wide uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8 }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #8FA3B0', color: '#8FA3B0',
                background: 'rgba(143,163,176,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#8FA3B0'; e.currentTarget.style.color = '#17130F'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(143,163,176,0.08)'; e.currentTarget.style.color = '#8FA3B0'; }}
            >
              ABRIR INVITACIÓN
            </button>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#8FA3B0]/40 shadow-md ${isTicketMaximized ? 'bg-[#17130F]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#F0E9DC]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#8FA3B0] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#F0E9DC] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#8FA3B0]/20 pl-3">
                <span className="text-[#F0E9DC] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#9C8F7A] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? 'Lugar' : 'Lugares'}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#8FA3B0]" />
              <span className="text-[#221C15]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <aside className="d-left hide-mobile" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
          <div className="absolute inset-0" style={{ border: "1px solid rgba(143,163,176,0.55)" }} />
          <IconClapper style={{ position: "absolute", bottom: -10, right: -8, width: 26, height: 20, color: "#8FA3B0" }} />
        </div>
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#8FA3B0', margin: '6px 0 14px', animation: 'cine-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#17130F]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#17130F] z-10 relative">
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#8FA3B0] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#E4DAC8] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#8FA3B0' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#8FA3B0', margin: '0 0 20px', animation: 'cine-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#9C8F7A] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#8FA3B0] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #17130F 100%)' }} />
            <div
            ...
            {/* Fuga de luz cálida tipo proyector, ligada al progreso de
                scroll. */}
            <div ref={heroFlareRef} className="pointer-events-none z-20" style={{ position: "absolute", inset: 0, opacity: 0, background: "linear-gradient(75deg, transparent 30%, rgba(255,224,168,0.5) 48%, rgba(143,163,176,0.25) 55%, transparent 72%)" }} />
            {/* Marco completo + perforaciones tipo filmina + claqueta
                "saliendo" de la esquina, calcado del mockup. */}
            <div className="cine-sprockets absolute left-0 right-0 top-0 h-[7px] pointer-events-none z-20" aria-hidden="true" />
            <div className="cine-sprockets absolute left-0 right-0 bottom-0 h-[7px] pointer-events-none z-20" aria-hidden="true" />
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              <div className="absolute inset-0" style={{ border: "1px solid rgba(143,163,176,0.6)" }} />
              <IconClapper style={{ position: "absolute", top: -10, left: -8, width: 28, height: 22, color: "#8FA3B0" }} />
              <IconStarDoodle style={{ position: "absolute", bottom: -8, right: -6, width: 16, height: 16, color: "#8FA3B0" }} />
            </div>
          </div>
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #6B463014, transparent 70%), #221C15" }}>
            <div className="max-w-2xl mx-auto text-center">
              <TypewriterText 
                text={`"${String(invitation.frasePersonalizadaTexto)}"`}
                className="text-[#E4DAC8] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#17130F]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #8FA3B0, transparent)' }} />
          <IconFilmReel className="cine-scroll-doodle opacity-0" style={{ width: 26, height: 16, color: '#8FA3B0' }} />
          <IconStarDoodle className="cine-scroll-doodle opacity-0" style={{ width: 13, height: 13, color: '#8FA3B0' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #8FA3B0, transparent)' }} />
        </div>

        <SectionWrapper id="details" delay={150} className="w-full bg-[#17130F] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#8FA3B0]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              <IconMap className="cine-scroll-doodle opacity-0" style={{ width: 14, height: 14 }} />
              CUÁNDO Y DÓNDE
            </p>
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#8FA3B0] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9C8F7A] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#F0E9DC] mb-3" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#9C8F7A]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#9C8F7A]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#8FA3B0] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#8FA3B0] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9C8F7A] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#8FA3B0] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#8FA3B0]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <IconFilmReel className="cine-scroll-doodle opacity-0" style={{ width: 18, height: 11 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#8FA3B0]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#9C8F7A] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--cine-mono), monospace" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#F0E9DC] font-light" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#221C15] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#1A150F] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#8FA3B0]">
                  <IconFilmReel className="cine-scroll-doodle opacity-0" style={{ width: 20, height: 12 }} />
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #6B463018, transparent 70%), #1A150F" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#1A150F' }} aria-hidden="true">
            <IconMusic className="cine-scroll-doodle opacity-0" style={{ width: 26, height: 26, color: '#8FA3B0' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#17130F' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #8FA3B0, transparent)' }} />
          <IconFilmReel className="cine-scroll-doodle opacity-0" style={{ width: 20, height: 12, color: '#8FA3B0' }} />
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #8FA3B0, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#17130F" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#8FA3B0" surfaceColor="#1D1811" />}
      </div>
  );
}
