...
/**
 * CorporativoAniversarioTemplateEsmeralda.tsx
 *
 * Portado 1:1 desde el mockup "Corporativo Aniversario" (mockup/Eventos),
...
interface CorporativoAniversarioTemplateEsmeraldaProps {
...
export function CorporativoAniversarioTemplateEsmeralda({ invitation, guest, isPersonalized = false }: CorporativoAniversarioTemplateEsmeraldaProps) {
  const tx = useTextos();
  ...
  const namesTitle = festejado || "Aniversario";
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E6F2EC";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#6FA98A";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #6FA98A";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#3D6B54" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#3D6B54" : "#6FA98A";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0B0F13", tinta: "#F4F1EA", acento: "#6FA98A", clase: `${cpaDisplay.variable} ${cpaMono.variable}`, fuente: "var(--cpa-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${cpaDisplay.variable} ${cpaMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#0B0F13",
        fontFamily: "var(--cpa-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{GP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="cpa-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="cpa-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #0E1319 55%, #0B0F13 100%)" }}>
          ...
          <div className="cpa-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="cpa-date-num cpa-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="cpa-divider">
            <span className="cpa-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="cpa-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="cpa-cal-link"
          />
        ...
        {(photoMobile || photoDesktop) && (
          ...
          >
            ...
            <span data-xin="1" data-dist="-60" className="cpa-kicker cpa-hero-photo-kicker">02 — UN VISTAZO AL LEGADO</span>
          </section>
        )}

        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="cpa-section cpa-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A2012 0%, #0D0B10 55%, #0B0F13 100%)" }}>
          <div className="cpa-scan-grid" />
          ...
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cpa-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #0B0F13 100%)" }}>
            ...
            <h2 ref={phraseRef} className="cpa-phrase" style={{ fontSize: fraseFontSize }}>
              {fraseWords.map((w, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= fraseAccentStart ? "cpa-accent-italic" : undefined}>
                    {w}
                  </span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="120" data-dist="90" className="cpa-divider" style={{ position: "relative" }}>
              <span className="cpa-divider-line cpa-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
            </div>
          </section>
        )}
          ...
          <div className="cpa-pan-sticky">
            <div data-strip="1" className="cpa-strip">
              <div id="details" data-tone="light" className="cpa-panel cpa-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="cpa-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="cpa-crono-row">
                        <span className="cpa-crono-time">{item.time || ""}</span>
                        <span className="cpa-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cpa-panel cpa-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cpa-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#3D6B54" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#3D6B54" />
                  </svg>
            ...
            <CpaDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="cpa-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #0E1319 60%, #0B0F13 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="cpa-rsvp">
              <CpaRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="cpa-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="cpa-pan-sticky">
            <div data-strip="1" className="cpa-strip">
                ...
                <div key={pageIndex} data-tone="light" className="cpa-panel cpa-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="cpa-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`cpa-mosaic-cell${i === 0 ? " cpa-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="cpa-seguir cpa-seguir--split">
                    ...
                    <span className="cpa-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <CpaDots count={photoPages.length} />
          </div>
        </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cpa-section" style={{ background: "#0E1319" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="cpa-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cpa-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E6F2EC" : "#6FA98A" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="cpa-song-wrap">
              <CpaSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
      ...
      <div ref={railRef} className="cpa-rail">
        ...
        <span ref={railLabelRef} className="cpa-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="cpa-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="cpa-cover-half cpa-cover-half--top">
          <CpaCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="cpa-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="cpa-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#6FA98A" guest={guest} />
      )}
...
function CpaCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="cpa-cd-box">
      <span ref={refEl} className="cpa-cd-num">—</span>
      <span className="cpa-cd-label">{label}</span>
    </div>
  );
}

function CpaDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="cpa-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="cpa-dot" />
      ))}
    </div>
  );
}

function CpaMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="cpa-medallion-arc" style={{ animation: spin === "reverse" ? "cpaRingRev 32s linear infinite" : "cpaRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="cpa-medallion-arc-text">
...
function CpaCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="cpa-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="cpa-bank-row-label">{label}</span>
    ...
    <>
      <div className="cpa-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="cpa-rsvp-row cpa-rsvp-row--payment">
      ...
      <div ref={stubRef} className="cpa-stub">
        ...
        <div ref={sealRef} className="cpa-seal">
          <CpaMedallionCmp label="BB" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="cpa-beam" />
        <div className="cpa-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="cpa-rsvp-error">{error}</p>}
...
function CpaSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="cpa-song">
      ...
      {error && <p className="cpa-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="cpa-song-list">
  ...
  return (
    <div className="cpa-quiz">
      {preguntas.map((q, qi) => {
        ...
        return (
          <div key={qi} className="cpa-quiz-q">
            <span className="cpa-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="cpa-quiz-q-text">{q.pregunta}</p>
            <div className="cpa-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " cpa-quiz-opt--correct";
                  else if (chosen) stateClass = " cpa-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " cpa-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`cpa-quiz-opt${stateClass}`}
...
function CpaInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="cpa-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="cpa-bank-row-label">{label}</span>
    ...
    <div className="cpa-cover-inner">
      ...
      <div className="cpa-cover-content">
        ...
        <div className="cpa-cover-center">
          ...
          <h1 ref={namesRef} className="cpa-cover-names">{namesTitle}</h1>
          <span className="cpa-cover-rule" />
          <span className="cpa-cover-date">{fechaCorta}</span>
        </div>
        <div className="cpa-cover-bottom">
          <div ref={perfRef} className="cpa-perf-strip cpa-perf-strip--reveal cpa-perf-strip--cover" />
          <div className="cpa-cover-facts">
...
// ---------------------------------------------------------------------
const GP_CSS = `
  .cpa-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .cpa-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #6FA98A; text-decoration: none; }
  a:hover { color: #E6F2EC; }

  @keyframes bbsFoil { to { transform: rotate(360deg); } }
  @keyframes cpaRing { to { transform: rotate(360deg); } }
  @keyframes cpaRingRev { to { transform: rotate(-360deg); } }
  ...
  @media (prefers-reduced-motion: reduce) { .cpa-scroller * { animation: none !important; } }

  .cpa-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .cpa-section--between { justify-content: space-between; }
  ...
  .cpa-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0B0F13; }
  .cpa-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  .cpa-hero-photo-kicker { position: absolute; left: 0; right: 0; bottom: 0; z-index: 2; padding: 0 max(24px, calc((100% - 560px) / 2)) 48px; }
  ...
  @media (max-width: 767px) {
    .cpa-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .cpa-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(200,164,92,.3); }
    .cpa-hero-photo-kicker { bottom: 40px; }
    .cpa-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .cpa-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .cpa-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .cpa-date-num { font-family: var(--cpa-display), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .cpa-date-num--right { text-align: right; line-height: 0.86; }
  .cpa-date-month { font-family: var(--cpa-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #6FA98A; padding-left: 12%; }

  .cpa-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cpa-divider-line { width: 52px; height: 1px; background: #6FA98A; display: inline-block; }
  .cpa-divider-line--long { width: 64px; }

  .cpa-lead { margin: 0; font-family: var(--cpa-display), serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .cpa-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--cpa-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .cpa-cal-link:hover { color: #6FA98A; }

  .cpa-medallion { position: relative; }
  .cpa-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .cpa-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .cpa-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .cpa-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #6FA98A, #E6F2EC, #6E5AA8); filter: saturate(.75); }
  .cpa-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .cpa-medallion-label { font-family: var(--cpa-display), serif; font-size: 26px; line-height: 1; color: #E6F2EC; }
  .cpa-medallion-label-sm { font-family: var(--cpa-display), serif; font-size: 16px; color: #E6F2EC; }
  .cpa-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .cpa-medallion-sub--accent { color: #6FA98A; }
  .cpa-medallion-arc { position: absolute; inset: -14%; }
  ...
  .cpa-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #6FA98A; opacity: 0.4; font-family: var(--cpa-mono), monospace; }

  .cpa-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .cpa-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E6F2EC, transparent); animation: bbsScan 6s linear infinite; pointer-events: none; }

  .cpa-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .cpa-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .cpa-cd-num { font-family: var(--cpa-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #E6F2EC; display: inline-block; }
  .cpa-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #6FA98A; }
  .cpa-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0B0F13 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .cpa-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .cpa-perf-strip--cover { opacity: .9; }

  .cpa-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(200,164,92,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: bbsFoil 30s linear infinite; }
  .cpa-phrase { margin: 0; position: relative; font-family: var(--cpa-display), serif; font-weight: 400; font-size: clamp(50px, 15vw, 96px); line-height: 0.92; letter-spacing: -0.03em; }
  .cpa-accent-italic { font-style: italic; color: #6FA98A; }
  .cpa-accent-serif { font-style: italic; color: #3D6B54; font-family: var(--cpa-display), serif; }
  .cpa-accent-serif-2 { color: #3D6B54; }
  .cpa-h2 { margin: 0; font-family: var(--cpa-display), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .cpa-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .cpa-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .cpa-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .cpa-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .cpa-panel--between { justify-content: space-between; }
  ...
  .cpa-pan[data-scroll="vertical"] { height: auto; }
  .cpa-pan[data-scroll="vertical"] .cpa-pan-sticky { position: static; height: auto; overflow: visible; }
  .cpa-pan[data-scroll="vertical"] .cpa-strip { position: static; display: block; width: 100%; transform: none !important; }
  .cpa-pan[data-scroll="vertical"] .cpa-panel { height: calc(var(--vh, 1vh) * 100); }
  .cpa-panel--end { justify-content: flex-end; }
  .cpa-panel--center { align-items: center; justify-content: center; text-align: center; }
  .cpa-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .cpa-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .cpa-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .cpa-panel-title { margin: 0; position: relative; font-family: var(--cpa-display), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .cpa-panel-title-md { margin: 0; position: relative; font-family: var(--cpa-display), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .cpa-panel-title-sm { margin: 0; font-family: var(--cpa-display), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .cpa-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .cpa-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .cpa-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  .cpa-facts { position: relative; display: flex; flex-direction: column; gap: 12px; font-size: 11px; letter-spacing: 0.14em; color: #4A473F; }
  .cpa-facts-row { display: flex; justify-content: space-between; border-bottom: 1px solid #D6D1C4; padding-bottom: 10px; }
  .cpa-facts-row--last { border-bottom: none; padding-bottom: 0; }
  .cpa-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #3D6B54; margin-top: auto; }
  .cpa-seguir--split { justify-content: space-between; }
  .cpa-side-hint { display: inline-block; animation: bbsSide 2.2s ease-in-out infinite; }
  .cpa-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .cpa-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #3D6B54; }

  .cpa-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .cpa-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .cpa-crono-time { font-family: var(--cpa-mono), monospace; color: #3D6B54; min-width: 42px; }
  .cpa-crono-title { flex: 1; }

  .cpa-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .cpa-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .cpa-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .cpa-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .cpa-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .cpa-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .cpa-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E6F2EC, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .cpa-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--cpa-mono), monospace; }
  .cpa-rsvp-rows { display: flex; flex-direction: column; }
  .cpa-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .cpa-rsvp-row > span:first-child { flex-shrink: 0; }
  .cpa-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .cpa-rsvp-row--payment { align-items: flex-start; }
  .cpa-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .cpa-rsvp-payment-total { color: #E6F2EC; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .cpa-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .cpa-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .cpa-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #6FA98A; background: transparent; color: #6FA98A; font-size: 14px; line-height: 1; cursor: pointer; }
  .cpa-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .cpa-rsvp-stepper span { font-family: var(--cpa-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .cpa-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--cpa-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cpa-rsvp-input::placeholder { color: #6E6A5D; }
  .cpa-rsvp-input:focus { outline: none; border-bottom-color: #6FA98A; }
  .cpa-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cpa-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #6FA98A, #3D6B54); border: 1px solid #6FA98A; color: #0E1319; cursor: pointer; }
  .cpa-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cpa-rsvp-btn--ghost { background: transparent; color: #6FA98A; }
  .cpa-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #6FA98A; margin: 0; }
  .cpa-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .cpa-rsvp-declined-text { margin: 0; font-family: var(--cpa-display), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .cpa-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .cpa-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .cpa-photo-cell--offset { margin-top: 18px; }
  .cpa-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .cpa-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .cpa-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .cpa-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .cpa-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .cpa-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .cpa-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .cpa-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .cpa-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  ...
  .cpa-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .cpa-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  .cpa-bank-row:last-child { border-bottom: none; }
  .cpa-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .cpa-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .cpa-bank-copy { flex-shrink: 0; font-family: var(--cpa-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #6FA98A; background: transparent; color: #6FA98A; cursor: pointer; }
  .cpa-bank-copy:hover { background: rgba(200,164,92,0.12); }

  .cpa-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .cpa-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: bbsEq 1.1s ease-in-out infinite; display: inline-block; }
  .cpa-song-wrap { font-family: var(--cpa-mono), monospace; }

  .cpa-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .cpa-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  .cpa-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .cpa-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--cpa-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cpa-song-input::placeholder { color: #6E6A5D; }
  .cpa-song-input:focus { outline: none; border-bottom-color: #6FA98A; }
  .cpa-song-sep { color: #8A8577; flex-shrink: 0; }
  .cpa-song-submit { flex-shrink: 0; background: none; border: none; color: #6FA98A; font-family: var(--cpa-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cpa-song-submit:hover { color: #E6F2EC; }
  .cpa-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cpa-song-error { font-size: 10px; color: #6FA98A; margin-top: 6px; }
  .cpa-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .cpa-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--cpa-mono), monospace; }
  .cpa-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .cpa-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .cpa-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .cpa-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .cpa-quiz-q-num { font-family: var(--cpa-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .cpa-quiz-q-text { margin: 0; font-family: var(--cpa-display), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .cpa-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .cpa-quiz-opt { font-family: var(--cpa-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #6FA98A; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cpa-quiz-opt:disabled { cursor: default; }
  .cpa-quiz-opt--picked { background: #6FA98A; border-color: #6FA98A; color: #0E1319; }
  .cpa-quiz-opt--correct { background: #6FA98A; border-color: #6FA98A; color: #0E1319; }
  .cpa-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cpa-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .cpa-quiz-result-score { margin: 0 0 6px; font-family: var(--cpa-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E6F2EC; }
  .cpa-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .cpa-final-card { border: 1px solid #6FA98A; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .cpa-final-names { font-family: var(--cpa-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E6F2EC; }
  .cpa-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .cpa-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .cpa-replay { cursor: pointer; color: #6FA98A; }
  .cpa-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cpa-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .cpa-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cpa-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .cpa-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E6F2EC, #6FA98A); transition: height 260ms linear; display: block; }
  .cpa-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #6FA98A; transition: color 500ms ease; }

  .cpa-cover { position: absolute; inset: 0; z-index: 5; }
  .cpa-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .cpa-cover-half--top { top: 0; }
  .cpa-cover-half--bottom { bottom: 0; }
  .cpa-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #161C22 0%, #0E1319 46%, #0B0F13 100%); }
  .cpa-cover-half--bottom .cpa-cover-inner { top: auto; bottom: 0; }
  .cpa-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(30,42,56,0.55), rgba(111,169,138,0.32), rgba(111,169,138,0.55), rgba(30,42,56,0.55)); filter: blur(64px); opacity: .62; animation: bbsFoil 26s linear infinite; }
  .cpa-cover-texture { position: absolute; inset: 0; background-image: radial-gradient(rgba(230,242,236,.08) 1px, transparent 1.2px); background-size: 22px 22px; }
  .cpa-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .cpa-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .cpa-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .cpa-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .cpa-cover-names { margin: 0; font-family: var(--cpa-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E6F2EC; }
  .cpa-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#6FA98A, transparent); display: block; }
  .cpa-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .cpa-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .cpa-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  ...
  .cpa-cover-facts:has(> span:only-child) { justify-content: center; }
  .cpa-cover-cta { border: 1px solid #6FA98A; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--cpa-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cpa-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cpa-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0E1319; }
  }
  .cpa-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .cpa-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: bbsHint 2.4s ease-in-out infinite; }

  .cpa-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .cpa-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #6FA98A; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .cpa-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
