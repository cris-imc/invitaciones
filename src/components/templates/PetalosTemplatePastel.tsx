 ...
 *   --t-bg #FFF8F9 · --t-surface #FDEFF2 · --petalos-ink #4A2E35
 *   --t-muted #9a6f79 · --t-acc #F0B8C6 (rojo vibrante) · --t-acc2 #D98BA0 (rojo profundo)
 *
...
interface PetalosTemplatePastelProps {
...
export function PetalosTemplatePastel({ invitation, guest, isPersonalized = false }: PetalosTemplatePastelProps) {
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #FFFFFF 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #4A2E35)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <style jsx>{`
            ...
            .petalos-cover-text { color: #FFF8F9; }
            .petalos-cover-text-muted { color: #FFF8F9; }
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #4A2E35)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#F0B8C6" surfaceColor="#FDEFF2" inactiveColor="#4A2E35" solid />}
      </div>
  );
}
