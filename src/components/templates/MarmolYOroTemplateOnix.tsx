 ...
 * mármol y dorado #C9CDD1, Italiana + Cormorant Garamond + IBM Plex Mono,
...
interface MarmolYOroTemplateOnixProps {
...
export function MarmolYOroTemplateOnix({ invitation, guest, isPersonalized = false }: MarmolYOroTemplateOnixProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C9CDD1";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(201,205,209,.35), 0 18px 50px -30px #C9CDD1";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#6E7478" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(201,205,209,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(201,205,209,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#6E7478" : "#C9CDD1";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#060607", tinta: "#F4F1EA", acento: "#C9CDD1", clase: `${myoItaliana.variable} ${myoCormorant.variable} ${myoMono.variable}`, fuente: "var(--myo-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="myo-scroller">
            ...
            <div className="myo-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="6,6,7" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="6,6,7" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="myo-section myo-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #26282A 0%, #111213 55%, #060607 100%)" }}>
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="myo-panel myo-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="myo-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#6E7478" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#6E7478" />
                  </svg>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C9CDD1" guest={guest} />
      )}
...
// mármol blanco/gris + dorado #C9CDD1, nunca el dorado #C8A45C ni el
...
const MYO_CSS = `
  ...
  a { color: #C9CDD1; text-decoration: none; }
  a:hover { color: #F0F2F4; }
  ...
  .myo-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #060607; }
  ...
  @media (min-width: 768px) {
    .myo-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(201,205,209,.3); }
  ...
  .myo-date-month { font-family: var(--myo-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C9CDD1; padding-left: 12%; }

  .myo-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .myo-divider-line { width: 52px; height: 1px; background: #C9CDD1; display: inline-block; }
  ...
  .myo-cal-link:hover { color: #C9CDD1; }
  ...
  .myo-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #7A7E82, #1A1A1C, #C9CDD1, #F0F2F4, #7A7E82); filter: saturate(.75); }
  .myo-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #090A0A; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .myo-medallion-label { font-family: var(--myo-italiana), serif; font-size: 26px; line-height: 1; color: #F0F2F4; }
  .myo-medallion-label-sm { font-family: var(--myo-italiana), serif; font-size: 16px; color: #F0F2F4; }
  .myo-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .myo-medallion-sub--accent { color: #C9CDD1; }
  ...
  .myo-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C9CDD1; opacity: 0.4; font-family: var(--myo-mono), monospace; }

  .myo-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(201,205,209,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .myo-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F0F2F4, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .myo-cd-num { font-family: var(--myo-italiana), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #ECEEEF; display: inline-block; }
  .myo-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C9CDD1; }
  .myo-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #060607 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .myo-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(130,132,134,0.3), rgba(26,26,28,0.26), rgba(201,205,209,0.4), rgba(130,132,134,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .myo-accent-italic { font-family: var(--myo-cormorant), serif; font-style: italic; color: #C9CDD1; }
  .myo-accent-serif { font-style: italic; color: #6E7478; font-family: var(--myo-cormorant), serif; }
  .myo-accent-serif-2 { color: #6E7478; }
  ...
  .myo-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #6E7478; margin-top: auto; }
  ...
  .myo-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #6E7478; }
  ...
  .myo-crono-time { font-family: var(--myo-mono), monospace; color: #6E7478; min-width: 42px; }
  ...
  .myo-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F0F2F4, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .myo-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(201,205,209,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .myo-rsvp-payment-total { color: #F0F2F4; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .myo-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C9CDD1; background: transparent; color: #C9CDD1; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .myo-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(201,205,209,0.3); color: #F4F1EA; font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .myo-rsvp-input::placeholder { color: #6E6A5D; }
  .myo-rsvp-input:focus { outline: none; border-bottom-color: #C9CDD1; }
  .myo-rsvp-btn { width: 100%; padding: 16px; font-family: var(--myo-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C9CDD1, #6E7478); border: 1px solid #C9CDD1; color: #0B0B10; cursor: pointer; }
  .myo-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .myo-rsvp-btn--ghost { background: transparent; color: #C9CDD1; }
  .myo-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C9CDD1; margin: 0; }
  ...
  .myo-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(201,205,209,0.18); }
  ...
  .myo-bank-copy { flex-shrink: 0; font-family: var(--myo-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C9CDD1; background: transparent; color: #C9CDD1; cursor: pointer; }
  .myo-bank-copy:hover { background: rgba(201,205,209,0.12); }
  ...
  .myo-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(201,205,209,0.3); padding-bottom: 12px; }
  ...
  .myo-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(201,205,209,0.3); color: #F4F1EA; font-family: var(--myo-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .myo-song-input::placeholder { color: #6E6A5D; }
  .myo-song-input:focus { outline: none; border-bottom-color: #C9CDD1; }
  .myo-song-sep { color: #8A8577; flex-shrink: 0; }
  .myo-song-submit { flex-shrink: 0; background: none; border: none; color: #C9CDD1; font-family: var(--myo-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .myo-song-submit:hover { color: #F0F2F4; }
  .myo-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .myo-song-error { font-size: 10px; color: #C9CDD1; margin-top: 6px; }
  ...
  .myo-quiz-opt { font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(201,205,209,0.4); background: transparent; color: #C9CDD1; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .myo-quiz-opt:disabled { cursor: default; }
  .myo-quiz-opt--picked { background: #C9CDD1; border-color: #C9CDD1; color: #0B0B10; }
  .myo-quiz-opt--correct { background: #C9CDD1; border-color: #C9CDD1; color: #0B0B10; }
  .myo-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .myo-quiz-result { padding-top: 18px; border-top: 1px solid rgba(201,205,209,0.2); }
  .myo-quiz-result-score { margin: 0 0 6px; font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F0F2F4; }
  .myo-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .myo-final-card { border: 1px solid #C9CDD1; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .myo-final-names { font-family: var(--myo-italiana), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F0F2F4; }
  ...
  .myo-replay { cursor: pointer; color: #C9CDD1; }
  .myo-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .myo-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(201,205,209,0.14); }
  .myo-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .myo-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(201,205,209,0.16); position: relative; }
  .myo-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F0F2F4, #C9CDD1); transition: height 260ms linear; display: block; }
  .myo-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C9CDD1; transition: color 500ms ease; }
  ...
  .myo-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #161718 0%, #121213 46%, #060607 100%); }
  .myo-cover-half--bottom .myo-cover-inner { top: auto; bottom: 0; }
  .myo-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(130,132,134,0.35), rgba(26,26,28,0.3), rgba(201,205,209,0.55), rgba(130,132,134,0.35)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .myo-cover-marble { position: absolute; inset: 0; background-image: repeating-linear-gradient(12deg, rgba(201,205,209,.18) 0 1px, transparent 1px 60px), repeating-linear-gradient(-68deg, rgba(232,185,140,.1) 0 1px, transparent 1px 90px), repeating-linear-gradient(40deg, rgba(201,205,209,.14) 0 1px, transparent 1px 130px); animation: myoMarbleDrift 22s ease-in-out infinite alternate; }
  ...
  .myo-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C9CDD1, transparent); display: block; }
  ...
  .myo-cover-cta { border: 1px solid #C9CDD1; background: linear-gradient(100deg, rgba(201,205,209,0.08), rgba(232,185,140,0.2), rgba(201,205,209,0.08)); color: #ECEEEF; font-family: var(--myo-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .myo-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C9CDD1; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
