...
/**
 * InfantilJurasicoTemplateAmarillo.tsx
 *
 * Portado 1:1 desde el mockup "Infantil Mundo Jurasico" (mockup/Eventos),
...
const ifjDisplay = Baloo_2({
  ...
  preload: false,
  weight: ["500", "700", "800"],
  variable: "--ifj-display",
  display: "swap",
});

const ifjMono = IBM_Plex_Mono({
  subsets: ["latin"],
  preload: false,
  weight: ["300", "400", "500"],
  variable: "--ifj-mono",
...
interface InfantilJurasicoTemplateAmarilloProps {
...
export function InfantilJurasicoTemplateAmarillo({ invitation, guest, isPersonalized = false }: InfantilJurasicoTemplateAmarilloProps) {
  const tx = useTextos();
  ...
  // la frase del kicker de arriba ("UNA EXPEDICIÓN PREHISTÓRICA PARA
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#FAF3D8";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#D4B23C";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #D4B23C";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#4A3E12" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#4A3E12" : "#D4B23C";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#101408", tinta: "#F4F1EA", acento: "#D4B23C", clase: `${ifjDisplay.variable} ${ifjMono.variable}`, fuente: "var(--ifj-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${ifjDisplay.variable} ${ifjMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#101408",
        fontFamily: "var(--ifj-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{GP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="ifj-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="ifj-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #141A0C 55%, #101408 100%)" }}>
          ...
          <div className="ifj-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="ifj-date-num ifj-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="ifj-divider">
            <span className="ifj-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="ifj-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="ifj-cal-link"
          />
        ...
        {(photoMobile || photoDesktop) && (
          ...
          >
            ...
            <span data-xin="1" data-dist="-60" className="ifj-kicker ifj-hero-photo-kicker">02 — EL PRIMER VISTAZO</span>
          </section>
        )}

        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="ifj-section ifj-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A3418 0%, #0D0B10 55%, #101408 100%)" }}>
          <div className="ifj-scan-grid" />
          ...
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="ifj-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #101408 100%)" }}>
            ...
            <h2 ref={phraseRef} className="ifj-phrase" style={{ fontSize: fraseFontSize }}>
              {fraseWords.map((w, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= fraseAccentStart ? "ifj-accent-italic" : undefined}>
                    {w}
                  </span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="120" data-dist="90" className="ifj-divider" style={{ position: "relative" }}>
              <span className="ifj-divider-line ifj-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
            </div>
          </section>
        )}
          ...
          <div className="ifj-pan-sticky">
            <div data-strip="1" className="ifj-strip">
              <div id="details" data-tone="light" className="ifj-panel ifj-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="ifj-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="ifj-crono-row">
                        <span className="ifj-crono-time">{item.time || ""}</span>
                        <span className="ifj-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="ifj-panel ifj-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="ifj-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#4A3E12" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#4A3E12" />
                  </svg>
            ...
            <IfjDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="ifj-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #141A0C 60%, #101408 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="ifj-rsvp">
              <IfjRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="ifj-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="ifj-pan-sticky">
            <div data-strip="1" className="ifj-strip">
                ...
                <div key={pageIndex} data-tone="light" className="ifj-panel ifj-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="ifj-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`ifj-mosaic-cell${i === 0 ? " ifj-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="ifj-seguir ifj-seguir--split">
                    ...
                    <span className="ifj-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <IfjDots count={photoPages.length} />
          </div>
        </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="ifj-section" style={{ background: "#141A0C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="ifj-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="ifj-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FAF3D8" : "#D4B23C" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="ifj-song-wrap">
              <IfjSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="ifj-section ifj-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #141A0C 55%, #101408 100%)" }}>
          ...
          <div className="ifj-footer-credit">
      ...
      <div ref={railRef} className="ifj-rail">
        ...
        <span ref={railLabelRef} className="ifj-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="ifj-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="ifj-cover-half ifj-cover-half--top">
          <IfjCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="ifj-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="ifj-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#D4B23C" guest={guest} />
      )}
...
function IfjCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="ifj-cd-box">
      <span ref={refEl} className="ifj-cd-num">—</span>
      <span className="ifj-cd-label">{label}</span>
    </div>
  );
}

function IfjDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="ifj-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="ifj-dot" />
      ))}
    </div>
  );
}

function IfjMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="ifj-medallion-arc" style={{ animation: spin === "reverse" ? "bbsRingRev 32s linear infinite" : "bbsRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="ifj-medallion-arc-text">
...
function IfjCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="ifj-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="ifj-bank-row-label">{label}</span>
    ...
    <>
      <div className="ifj-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="ifj-rsvp-row ifj-rsvp-row--payment">
      ...
      <div ref={stubRef} className="ifj-stub">
        ...
        <div ref={sealRef} className="ifj-seal">
          <IfjMedallionCmp label="BB" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="ifj-beam" />
        <div className="ifj-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="ifj-rsvp-error">{error}</p>}
...
function IfjSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="ifj-song">
      ...
      {error && <p className="ifj-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="ifj-song-list">
...
function IfjQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: IfjQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="ifj-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="ifj-quiz-q">
            <span className="ifj-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="ifj-quiz-q-text">{q.pregunta}</p>
            <div className="ifj-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " ifj-quiz-opt--correct";
                  else if (chosen) stateClass = " ifj-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " ifj-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`ifj-quiz-opt${stateClass}`}
...
function IfjInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="ifj-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="ifj-bank-row-label">{label}</span>
    ...
    <div className="ifj-cover-inner">
      ...
      <div className="ifj-cover-content">
        ...
        <div className="ifj-cover-center">
          ...
          <h1 ref={namesRef} className="ifj-cover-names">{namesTitle}</h1>
          <span className="ifj-cover-rule" />
          <span className="ifj-cover-date">{fechaCorta}</span>
        </div>
        <div className="ifj-cover-bottom">
          <div ref={perfRef} className="ifj-perf-strip ifj-perf-strip--reveal ifj-perf-strip--cover" />
          <div className="ifj-cover-facts">
...
// ---------------------------------------------------------------------
const GP_CSS = `
  .ifj-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .ifj-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #D4B23C; text-decoration: none; }
  a:hover { color: #FAF3D8; }
  ...
  @media (prefers-reduced-motion: reduce) { .ifj-scroller * { animation: none !important; } }

  .ifj-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .ifj-section--between { justify-content: space-between; }
  ...
  .ifj-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #101408; }
  .ifj-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  .ifj-hero-photo-kicker { position: absolute; left: 0; right: 0; bottom: 0; z-index: 2; padding: 0 max(24px, calc((100% - 560px) / 2)) 48px; }
  ...
  @media (max-width: 767px) {
    .ifj-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .ifj-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(200,164,92,.3); }
    .ifj-hero-photo-kicker { bottom: 40px; }
    .ifj-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .ifj-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .ifj-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .ifj-date-num { font-family: var(--ifj-display), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .ifj-date-num--right { text-align: right; line-height: 0.86; }
  .ifj-date-month { font-family: var(--ifj-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #D4B23C; padding-left: 12%; }

  .ifj-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .ifj-divider-line { width: 52px; height: 1px; background: #D4B23C; display: inline-block; }
  .ifj-divider-line--long { width: 64px; }

  .ifj-lead { margin: 0; font-family: var(--ifj-display), serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .ifj-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--ifj-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .ifj-cal-link:hover { color: #D4B23C; }

  .ifj-medallion { position: relative; }
  .ifj-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .ifj-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .ifj-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .ifj-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #D4B23C, #FAF3D8, #6E5AA8); filter: saturate(.75); }
  .ifj-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .ifj-medallion-label { font-family: var(--ifj-display), serif; font-size: 26px; line-height: 1; color: #FAF3D8; }
  .ifj-medallion-label-sm { font-family: var(--ifj-display), serif; font-size: 16px; color: #FAF3D8; }
  .ifj-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .ifj-medallion-sub--accent { color: #D4B23C; }
  .ifj-medallion-arc { position: absolute; inset: -14%; }
  ...
  .ifj-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #D4B23C; opacity: 0.4; font-family: var(--ifj-mono), monospace; }

  .ifj-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .ifj-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #FAF3D8, transparent); animation: bbsScan 6s linear infinite; pointer-events: none; }

  .ifj-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .ifj-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .ifj-cd-num { font-family: var(--ifj-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #FAF3D8; display: inline-block; }
  .ifj-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #D4B23C; }
  .ifj-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #101408 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .ifj-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .ifj-perf-strip--cover { opacity: .9; }

  .ifj-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(200,164,92,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: bbsFoil 30s linear infinite; }
  .ifj-phrase { margin: 0; position: relative; font-family: var(--ifj-display), serif; font-weight: 400; font-size: clamp(50px, 15vw, 96px); line-height: 0.92; letter-spacing: -0.03em; }
  .ifj-accent-italic { font-style: italic; color: #D4B23C; }
  .ifj-accent-serif { font-style: italic; color: #4A3E12; font-family: var(--ifj-display), serif; }
  .ifj-accent-serif-2 { color: #4A3E12; }
  .ifj-h2 { margin: 0; font-family: var(--ifj-display), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .ifj-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .ifj-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .ifj-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .ifj-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .ifj-panel--between { justify-content: space-between; }
  ...
  .ifj-pan[data-scroll="vertical"] { height: auto; }
  .ifj-pan[data-scroll="vertical"] .ifj-pan-sticky { position: static; height: auto; overflow: visible; }
  .ifj-pan[data-scroll="vertical"] .ifj-strip { position: static; display: block; width: 100%; transform: none !important; }
  .ifj-pan[data-scroll="vertical"] .ifj-panel { height: calc(var(--vh, 1vh) * 100); }
  .ifj-panel--end { justify-content: flex-end; }
  .ifj-panel--center { align-items: center; justify-content: center; text-align: center; }
  .ifj-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .ifj-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .ifj-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .ifj-panel-title { margin: 0; position: relative; font-family: var(--ifj-display), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .ifj-panel-title-md { margin: 0; position: relative; font-family: var(--ifj-display), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .ifj-panel-title-sm { margin: 0; font-family: var(--ifj-display), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .ifj-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .ifj-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .ifj-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  .ifj-facts { position: relative; display: flex; flex-direction: column; gap: 12px; font-size: 11px; letter-spacing: 0.14em; color: #4A473F; }
  .ifj-facts-row { display: flex; justify-content: space-between; border-bottom: 1px solid #D6D1C4; padding-bottom: 10px; }
  .ifj-facts-row--last { border-bottom: none; padding-bottom: 0; }
  .ifj-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #4A3E12; margin-top: auto; }
  .ifj-seguir--split { justify-content: space-between; }
  .ifj-side-hint { display: inline-block; animation: bbsSide 2.2s ease-in-out infinite; }
  .ifj-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .ifj-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #4A3E12; }

  .ifj-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .ifj-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .ifj-crono-time { font-family: var(--ifj-mono), monospace; color: #4A3E12; min-width: 42px; }
  .ifj-crono-title { flex: 1; }

  .ifj-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .ifj-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .ifj-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .ifj-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .ifj-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .ifj-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .ifj-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #FAF3D8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .ifj-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--ifj-mono), monospace; }
  .ifj-rsvp-rows { display: flex; flex-direction: column; }
  .ifj-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .ifj-rsvp-row > span:first-child { flex-shrink: 0; }
  .ifj-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .ifj-rsvp-row--payment { align-items: flex-start; }
  .ifj-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .ifj-rsvp-payment-total { color: #FAF3D8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .ifj-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .ifj-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .ifj-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #D4B23C; background: transparent; color: #D4B23C; font-size: 14px; line-height: 1; cursor: pointer; }
  .ifj-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .ifj-rsvp-stepper span { font-family: var(--ifj-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .ifj-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--ifj-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .ifj-rsvp-input::placeholder { color: #6E6A5D; }
  .ifj-rsvp-input:focus { outline: none; border-bottom-color: #D4B23C; }
  .ifj-rsvp-btn { width: 100%; padding: 16px; font-family: var(--ifj-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #D4B23C, #4A3E12); border: 1px solid #D4B23C; color: #141A0C; cursor: pointer; }
  .ifj-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .ifj-rsvp-btn--ghost { background: transparent; color: #D4B23C; }
  .ifj-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #D4B23C; margin: 0; }
  .ifj-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .ifj-rsvp-declined-text { margin: 0; font-family: var(--ifj-display), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .ifj-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .ifj-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .ifj-photo-cell--offset { margin-top: 18px; }
  .ifj-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .ifj-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .ifj-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .ifj-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .ifj-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .ifj-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .ifj-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .ifj-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .ifj-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  ...
  .ifj-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .ifj-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  .ifj-bank-row:last-child { border-bottom: none; }
  .ifj-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .ifj-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .ifj-bank-copy { flex-shrink: 0; font-family: var(--ifj-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #D4B23C; background: transparent; color: #D4B23C; cursor: pointer; }
  .ifj-bank-copy:hover { background: rgba(200,164,92,0.12); }

  .ifj-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .ifj-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: bbsEq 1.1s ease-in-out infinite; display: inline-block; }
  .ifj-song-wrap { font-family: var(--ifj-mono), monospace; }

  .ifj-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .ifj-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  .ifj-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .ifj-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--ifj-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .ifj-song-input::placeholder { color: #6E6A5D; }
  .ifj-song-input:focus { outline: none; border-bottom-color: #D4B23C; }
  .ifj-song-sep { color: #8A8577; flex-shrink: 0; }
  .ifj-song-submit { flex-shrink: 0; background: none; border: none; color: #D4B23C; font-family: var(--ifj-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .ifj-song-submit:hover { color: #FAF3D8; }
  .ifj-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .ifj-song-error { font-size: 10px; color: #D4B23C; margin-top: 6px; }
  .ifj-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .ifj-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--ifj-mono), monospace; }
  .ifj-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .ifj-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .ifj-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .ifj-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .ifj-quiz-q-num { font-family: var(--ifj-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .ifj-quiz-q-text { margin: 0; font-family: var(--ifj-display), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .ifj-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .ifj-quiz-opt { font-family: var(--ifj-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #D4B23C; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .ifj-quiz-opt:disabled { cursor: default; }
  .ifj-quiz-opt--picked { background: #D4B23C; border-color: #D4B23C; color: #141A0C; }
  .ifj-quiz-opt--correct { background: #D4B23C; border-color: #D4B23C; color: #141A0C; }
  .ifj-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .ifj-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .ifj-quiz-result-score { margin: 0 0 6px; font-family: var(--ifj-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #FAF3D8; }
  .ifj-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .ifj-final-card { border: 1px solid #D4B23C; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .ifj-final-names { font-family: var(--ifj-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #FAF3D8; }
  .ifj-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .ifj-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .ifj-replay { cursor: pointer; color: #D4B23C; }
  .ifj-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .ifj-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .ifj-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .ifj-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .ifj-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FAF3D8, #D4B23C); transition: height 260ms linear; display: block; }
  .ifj-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #D4B23C; transition: color 500ms ease; }

  .ifj-cover { position: absolute; inset: 0; z-index: 5; }
  .ifj-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .ifj-cover-half--top { top: 0; }
  .ifj-cover-half--bottom { bottom: 0; }
  .ifj-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1C2412 0%, #141A0C 46%, #101408 100%); }
  .ifj-cover-half--bottom .ifj-cover-inner { top: auto; bottom: 0; }
  .ifj-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(90,58,34,0.45), rgba(212,178,60,0.32), rgba(212,178,60,0.55), rgba(90,58,34,0.45)); filter: blur(64px); opacity: .62; animation: bbsFoil 26s linear infinite; }
  .ifj-cover-texture { position: absolute; inset: 0; background-image: repeating-linear-gradient(60deg, rgba(250,243,216,.06) 0 3px, transparent 3px 34px), repeating-linear-gradient(-60deg, rgba(250,243,216,.05) 0 3px, transparent 3px 34px); }
  .ifj-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .ifj-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .ifj-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .ifj-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .ifj-cover-names { margin: 0; font-family: var(--ifj-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FAF3D8; }
  .ifj-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#D4B23C, transparent); display: block; }
  .ifj-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .ifj-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .ifj-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  ...
  .ifj-cover-facts:has(> span:only-child) { justify-content: center; }
  .ifj-cover-cta { border: 1px solid #D4B23C; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--ifj-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .ifj-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .ifj-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #141A0C; }
  }
  .ifj-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .ifj-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: bbsHint 2.4s ease-in-out infinite; }

  .ifj-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .ifj-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #D4B23C; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .ifj-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
