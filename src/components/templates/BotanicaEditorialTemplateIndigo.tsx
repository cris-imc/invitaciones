...
/**
 * BotanicaEditorialTemplateIndigo.tsx
 *
 * Portado 1:1 desde el mockup "Botanica Editorial - Panoramica" (Colección
 * Storytelling, evento Casamiento): fondo verde-negro casi opaco (#080D12),
 * acentos verde oliva/salvia (#3E5A82 / #8FB8CE) con un toque de foil dorado
 ...
 * autor) y scroll horizontal "pineado" para Cuándo y dónde / Álbum.
 *
 * Arquitectura clonada de GuestPassVipTemplate.tsx (motor de scroll/reveal,
 * riel de progreso, gesto lateral en paneles pineados, álbum con foto
 * destacada + lightbox + paginación balanceada, LiveAlbumStrip para EN VIVO):
 * mismas props, mismos endpoints/datos reales -- solo cambia paleta,
 * tipografía, copy y el motivo decorativo de la tapa.
 */
...
// Tonos claros que rotan entre hojas del álbum -- idénticos a los 3 fondos de
// hoja del mockup real (F6F3EC / F1EDE3 / EDE8DE).
...
interface BotanicaEditorialTemplateIndigoProps {
  invitation: Record<string, unknown>;
  guest?: GuestRecord | null;
  isPersonalized?: boolean;
}

// El backend no reserva mesas/sectores -- la lámina muestra el orden real del
...
export function BotanicaEditorialTemplateIndigo({ invitation, guest, isPersonalized = false }: BotanicaEditorialTemplateIndigoProps) {
  const tx = useTextos();
  ...
  const initialsRaw = `${novia.charAt(0)}${novio.charAt(0)}`.trim();
  const coupleInitials = initialsRaw ? initialsRaw.toUpperCase() : "B";
  ...
  // invitado/familia en vez de los novios -- "un herbario para la boda DE
  // Familia Juarez" suena mal, así que el kicker pierde el "de" y el nombre
  // va debajo, como un dato propio (no como si Familia Juarez fuera quien se
  // casa).
  ...
  // tal cual lo cargó el cliente en el wizard, en la misma hoja que el lugar.
  ...
  // jardín se" plano + "abre una sola vez." acentuado, 4 de 7 palabras).
  // Math.floor (no ceil) reproduce ese reparto para cantidades impares de
  // palabras y sigue funcionando igual que antes para las pares.
  ...
  // ---------------------------------------------------------------------
  // refs para el motor de motion (idéntico al portado desde el mockup, pero
  // corriendo sobre requestAnimationFrame en vez de setInterval(16ms))
  // ---------------------------------------------------------------------
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#8FB8CE";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#3E5A82";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #3E5A82";
      }
      if (sealRef.current) {
  ...
  // -- montaje: intro, countdown, reveals, hoja del herbario, loop de scroll --
  useEffect(() => {
    ...
    // Hoja/vena decorativa de la portada (motivo herbario): se "dibuja" con
    // stroke-dasharray/dashoffset una única vez, medio segundo después del
    // montaje -- igual que en el mockup real.
    const leaf = root.querySelector<SVGElement>("[data-leaf]");
    const leafTimer = leaf ? window.setTimeout(() => leaf.classList.add("in-view"), 500) : undefined;
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#26405E" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#26405E" : "#3E5A82";
        }
    ...
    return () => {
      ...
      if (leafTimer) window.clearTimeout(leafTimer);
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#080D12", tinta: "#F4F1EA", acento: "#3E5A82", clase: `${bteSerif.variable} ${bteMono.variable}`, fuente: "var(--bte-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${bteSerif.variable} ${bteMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="bte-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="bte-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #141F34 0%, #0C121E 55%, #080D12 100%)" }}>
          ...
          <div className="bte-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="bte-date-num bte-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="bte-divider">
          ...
          <svg data-leaf="1" viewBox="0 0 200 400" className="bte-leaf-deco" aria-hidden="true">
            <path d="M100 398 C92 340 108 300 92 250 C78 205 100 175 84 130 C70 90 92 70 66 30" fill="none" stroke="#7CA8C2" strokeWidth={1.4} />
            <g fill="none" stroke="#7CA8C2" strokeWidth={1.1}>
              ...
              <path d="M0 0 Q10 -7 24 0 Q10 7 0 0 Z" transform="translate(96,285) rotate(-15) scale(0.95)" />
              <path d="M0 0 Q10 -7 24 0 Q10 7 0 0 Z" transform="translate(80,250) rotate(205) scale(1)" />
              <path d="M0 0 Q10 -7 24 0 Q10 7 0 0 Z" transform="translate(94,215) rotate(-10) scale(1)" />
              <path d="M0 0 Q10 -7 24 0 Q10 7 0 0 Z" transform="translate(76,180) rotate(200) scale(0.9)" />
              <path d="M0 0 Q10 -7 24 0 Q10 7 0 0 Z" transform="translate(88,145) rotate(-15) scale(0.85)" />
              <path d="M0 0 Q10 -7 24 0 Q10 7 0 0 Z" transform="translate(66,55) rotate(-80) scale(0.7)" />
            </g>
          </svg>
            ...
            <div className="bte-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="8,13,18" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="8,13,18" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="bte-section bte-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #16223C 0%, #0D1422 55%, #080D12 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="bte-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #182038 0%, #0B1119 52%, #080D12 100%)" }}>
          ...
          <h2 ref={phraseRef} className="bte-phrase" style={{ fontSize: fraseFontSize }}>
            {fraseWords.map((w, i) => (
              ...
              <span key={i}>
                <span data-w="1" className={i >= fraseAccentStart ? "bte-accent-phrase" : undefined}>
                  {w}
                </span>{" "}
              </span>
            ))}
          </h2>
          <div data-xin="1" data-delay="120" data-dist="90" className="bte-divider" style={{ position: "relative" }}>
          ...
          <div className="bte-pan-sticky">
            <div data-strip="1" className="bte-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="bte-panel bte-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <h2 className="bte-panel-title">
                    ...
                    {ceremoniaDireccion && <><br /><span className="bte-accent-serif">{ceremoniaDireccion}</span></>}
                  </h2>
              ...
              <div id="details" data-tone="light" className="bte-panel bte-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="bte-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="bte-crono-row">
                        <span className="bte-crono-time">{item.time || ""}</span>
                        <span className="bte-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="bte-panel bte-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="bte-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#26405E" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#26405E" />
                  </svg>
              ...
              <div data-tone="dark" className="bte-panel bte-panel--center" style={{ background: "#0C121E", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="bte-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #141F34 0%, #0C121E 60%, #080D12 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="bte-rsvp">
              <BteRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="bte-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="bte-pan-sticky">
            <div data-strip="1" className="bte-strip">
                ...
                <div key={pageIndex} data-tone="light" className="bte-panel bte-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="bte-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`bte-mosaic-cell${i === 0 ? " bte-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="bte-seguir bte-seguir--split">
                    ...
                    <span className="bte-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <BteDots count={photoPages.length} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="bte-section" style={{ background: "#0C121E" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="bte-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="bte-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#8FB8CE" : "#3E5A82" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="bte-section" style={{ background: "#0C121E" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="bte-section" style={{ background: "#0C121E" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.sabor.tuLamina")} className="bte-section bte-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #141F34 0%, #0C121E 55%, #080D12 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="bte-final-card">
            ...
            <span className="bte-mini-label" style={{ color: "#A8A292" }}>{fechaCorta} — {hora} H</span>
          ...
          <div className="bte-footer-credit">
      ...
      <div ref={railRef} className="bte-rail">
        ...
        <span ref={railLabelRef} className="bte-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="bte-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="bte-cover-half bte-cover-half--top">
          <BteCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="bte-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="bte-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#3E5A82" guest={guest} />
      )}
...
function BteCdBox({ refEl, delay, dist, label, accent }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string; accent?: boolean }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="bte-cd-box">
      <span ref={refEl} className={`bte-cd-num${accent ? " bte-cd-num--accent" : ""}`}>—</span>
      <span className="bte-cd-label">{label}</span>
    </div>
  );
}

function BteDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="bte-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="bte-dot" />
      ))}
    </div>
  );
}

function BteMedallion({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="bte-medallion-arc" style={{ animation: spin === "reverse" ? "bteRingRev 32s linear infinite" : "bteRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="bte-medallion-arc-text">
...
function BteCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="bte-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="bte-bank-row-label">{label}</span>
...
// Reimplementación propia del RSVP (ticket/lámina + sello ya existente en la
// plantilla), en vez del look genérico de <RSVPWizardV2>. Habla con el mismo
// endpoint/payload que ese componente (/api/guests/[token]/confirm o
// /api/rsvp), así que no cambia ningún dato que pida el backend, solo cómo
// se ve.
function BteRsvpCard({
  invitationId,
  guestToken,
  guestName,
  maxAdults,
    ...
    <>
      ...
      <div ref={stubRef} className="bte-stub">
        ...
        <div ref={sealRef} className="bte-seal">
          <BteMedallion label="B" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="bte-beam" />
        <div className="bte-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="bte-rsvp-error">{error}</p>}
...
// Reimplementación minimalista del look editorial (misma API que
...
function BteSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="bte-song">
      ...
      {error && <p className="bte-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="bte-song-list">
...
function BteQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: BteQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="bte-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="bte-quiz-q">
            <span className="bte-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="bte-quiz-q-text">{q.pregunta}</p>
            <div className="bte-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " bte-quiz-opt--correct";
                  else if (chosen) stateClass = " bte-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " bte-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`bte-quiz-opt${stateClass}`}
...
function BteInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="bte-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="bte-bank-row-label">{label}</span>
...
function BteCoverHalf({
  ...
  return (
    <div className="bte-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#3E5A82" tintColor2="#080D12" effect="enfoque" scrimColorRgb="8,13,18" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#3E5A82" tintColor2="#080D12" effect="enfoque" scrimColorRgb="8,13,18" />
        </div>
      )}
      <div className="bte-cover-glow" />
      ...
      <div className="bte-cover-content">
        ...
        <div className="bte-cover-center">
          ...
          <h1 ref={namesRef} className="bte-cover-names">{namesTitle}</h1>
          <span className="bte-cover-rule" />
          <span className="bte-cover-date">{fechaCorta}</span>
        </div>
        <div className="bte-cover-bottom">
          <div ref={perfRef} className="bte-perf-strip bte-perf-strip--reveal bte-perf-strip--cover" />
          <div className="bte-cover-facts">
...
const BTE_CSS = `
  ...
  a { color: #3E5A82; text-decoration: none; }
  a:hover { color: #8FB8CE; }

  @keyframes bteFoil { to { transform: rotate(360deg); } }
  @keyframes bteRing { to { transform: rotate(360deg); } }
  @keyframes bteRingRev { to { transform: rotate(-360deg); } }
  @keyframes bteScan { 0% { transform: translateY(0); opacity: 0; } 12% { opacity: .85; } 88% { opacity: .85; } 100% { transform: translateY(320px); opacity: 0; } }
  @keyframes bteEq { 0%,100% { transform: scaleY(.25); } 50% { transform: scaleY(1); } }
  @keyframes bteHint { 0%,100% { transform: translateY(0); } 50% { transform: translateY(7px); } }
  @keyframes bteSide { 0%,100% { transform: translateX(0); } 50% { transform: translateX(7px); } }
  @keyframes bteLeafDraw { to { stroke-dashoffset: 0; } }
  [data-leaf] path { stroke-dasharray: 500; stroke-dashoffset: 500; }
  [data-leaf].in-view path { animation: bteLeafDraw 1.4s cubic-bezier(.4,0,.2,1) forwards; }
  @media (prefers-reduced-motion: reduce) { .bte-scroller * { animation: none !important; } }

  .bte-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .bte-section--between { justify-content: space-between; }
  ...
  .bte-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #080D12; }
  ...
  @media (min-width: 768px) {
    .bte-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(62,90,130,.3); }
  ...
  .bte-date-month { font-family: var(--bte-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #3E5A82; padding-left: 12%; }

  .bte-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .bte-divider-line { width: 52px; height: 1px; background: #3E5A82; display: inline-block; }
  ...
  .bte-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #26405E, #8C6F35, #3E5A82, #8FB8CE, #26405E); filter: saturate(.75); }
  ...
  .bte-medallion-label { font-family: var(--bte-serif), serif; font-size: 26px; line-height: 1; color: #8FB8CE; }
  .bte-medallion-label-sm { font-family: var(--bte-serif), serif; font-size: 16px; color: #8FB8CE; }
  .bte-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .bte-medallion-sub--accent { color: #3E5A82; }
  ...
  .bte-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #3E5A82; opacity: 0.4; font-family: var(--bte-mono), monospace; }
  ...
  .bte-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #8FB8CE, transparent); animation: bteScan 6s linear infinite; pointer-events: none; }
  ...
  .bte-cd-num--accent { color: #8FB8CE; }
  .bte-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #3E5A82; }
  .bte-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #080D12 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .bte-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .bte-perf-strip--cover { opacity: .9; }

  .bte-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(62,90,130,0.36), rgba(192,138,91,0.24), rgba(124,168,194,0.4), rgba(62,90,130,0.36)); filter: blur(80px); opacity: .4; animation: bteFoil 30s linear infinite; }
  ...
  .bte-accent-italic { font-style: italic; color: #3E5A82; }
  .bte-accent-phrase { font-style: italic; color: #7CA8C2; }
  .bte-accent-serif { font-style: italic; color: #26405E; font-family: var(--bte-serif), serif; }
  .bte-accent-serif-2 { color: #26405E; }
  .bte-accent-dark-2 { color: #3E5A82; }
  .bte-h2 { margin: 0; font-family: var(--bte-serif), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .bte-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .bte-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .bte-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .bte-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .bte-panel--between { justify-content: space-between; }
  ...
  .bte-pan[data-scroll="vertical"] { height: auto; }
  .bte-pan[data-scroll="vertical"] .bte-pan-sticky { position: static; height: auto; overflow: visible; }
  .bte-pan[data-scroll="vertical"] .bte-strip { position: static; display: block; width: 100%; transform: none !important; }
  .bte-pan[data-scroll="vertical"] .bte-panel { height: calc(var(--vh, 1vh) * 100); }
  .bte-panel--end { justify-content: flex-end; }
  .bte-panel--center { align-items: center; justify-content: center; text-align: center; }
  .bte-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .bte-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .bte-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .bte-panel-title { margin: 0; position: relative; font-family: var(--bte-serif), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .bte-panel-title-md { margin: 0; position: relative; font-family: var(--bte-serif), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .bte-panel-title-sm { margin: 0; font-family: var(--bte-serif), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .bte-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .bte-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .bte-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  ...
  .bte-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #26405E; margin-top: auto; }
  ...
  .bte-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #26405E; }
  ...
  .bte-crono-time { font-family: var(--bte-mono), monospace; color: #26405E; min-width: 42px; }
  ...
  .bte-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .bte-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .bte-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #8FB8CE, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .bte-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(62,90,130,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .bte-rsvp-payment-total { color: #8FB8CE; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .bte-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #3E5A82; background: transparent; color: #3E5A82; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .bte-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(62,90,130,0.3); color: #F4F1EA; font-family: var(--bte-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .bte-rsvp-input::placeholder { color: #6E6A5D; }
  .bte-rsvp-input:focus { outline: none; border-bottom-color: #3E5A82; }
  .bte-rsvp-btn { width: 100%; padding: 16px; font-family: var(--bte-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; border: 1px solid #3E5A82; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 200ms ease, color 200ms ease; }
  ...
  .bte-rsvp-btn--ghost { background: transparent; color: #3E5A82; }
  .bte-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(62,90,130,0.12); color: #3E5A82; }
  .bte-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #3E5A82; margin: 0; }
  ...
  .bte-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .bte-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .bte-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  .bte-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }

  /* Mosaico fijo del álbum: grilla pareja de 3 columnas, con la primera foto
     de cada hoja destacada en 2x2, hasta 5 por hoja. grid-auto-rows:auto +
     align-content:start (no stretch) -- si dejáramos 1fr, las pocas filas de
     una página con menos fotos se repartían TODO el alto disponible del
     panel y las celdas se estiraban en vez de quedar cuadradas. Con auto,
     cada fila mide justo lo que necesita (cuadrado, según el ancho de
     columna) y el resto del panel queda en blanco en vez de deformar las
     fotos. */
  .bte-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .bte-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .bte-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  ...
  .bte-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(62,90,130,0.18); }
  ...
  .bte-bank-copy { flex-shrink: 0; font-family: var(--bte-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #3E5A82; background: transparent; color: #3E5A82; cursor: pointer; }
  .bte-bank-copy:hover { background: rgba(62,90,130,0.12); }
  ...
  .bte-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(62,90,130,0.3); padding-bottom: 12px; }
  ...
  .bte-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(62,90,130,0.3); color: #F4F1EA; font-family: var(--bte-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .bte-song-input::placeholder { color: #6E6A5D; }
  .bte-song-input:focus { outline: none; border-bottom-color: #3E5A82; }
  .bte-song-sep { color: #8A8577; flex-shrink: 0; }
  .bte-song-submit { flex-shrink: 0; background: none; border: none; color: #3E5A82; font-family: var(--bte-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .bte-song-submit:hover { color: #8FB8CE; }
  .bte-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .bte-song-error { font-size: 10px; color: #3E5A82; margin-top: 6px; }
  ...
  .bte-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .bte-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .bte-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  ...
  .bte-quiz-opt { font-family: var(--bte-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(62,90,130,0.4); background: transparent; color: #3E5A82; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .bte-quiz-opt:disabled { cursor: default; }
  .bte-quiz-opt--picked { background: #3E5A82; border-color: #3E5A82; color: #0B0B10; }
  .bte-quiz-opt--correct { background: #3E5A82; border-color: #3E5A82; color: #0B0B10; }
  .bte-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .bte-quiz-result { padding-top: 18px; border-top: 1px solid rgba(62,90,130,0.2); }
  .bte-quiz-result-score { margin: 0 0 6px; font-family: var(--bte-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #8FB8CE; }
  .bte-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .bte-final-card { border: 1px solid #3E5A82; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .bte-replay { cursor: pointer; color: #3E5A82; }
  ...
  .bte-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#8FB8CE, #3E5A82); transition: height 260ms linear; display: block; }
  .bte-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #3E5A82; transition: color 500ms ease; }
  ...
  .bte-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #131F30 0%, #0C121E 46%, #080D12 100%); }
  .bte-cover-half--bottom .bte-cover-inner { top: auto; bottom: 0; }
  .bte-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(62,90,130,0.5), rgba(192,138,91,0.35), rgba(124,168,194,0.6), rgba(62,90,130,0.5)); filter: blur(64px); opacity: .62; animation: bteFoil 26s linear infinite; }
  ...
  .bte-cover-linen { position: absolute; inset: 0; background-image: repeating-linear-gradient(78deg, rgba(143,184,206,.16) 0 1px, transparent 1px 40px), repeating-linear-gradient(-32deg, rgba(143,184,206,.1) 0 1px, transparent 1px 64px); }
  ...
  .bte-cover-names { margin: 0; font-family: var(--bte-serif), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F4F1EA; }
  .bte-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#3E5A82, transparent); display: block; }
  ...
  .bte-cover-cta { border: 1px solid #3E5A82; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--bte-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .bte-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,13,18,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .bte-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #3E5A82; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
