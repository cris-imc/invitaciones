...
/**
 * PapeleriaDeHotelDeLujoTemplateAzulMarinoPlata.tsx
 *
...
export function PapeleriaDeHotelDeLujoTemplateAzulMarinoPlata({ invitation, guest, isPersonalized = false }: PapeleriaDeHotelDeLujoTemplateProps) {
  const tx = useTextos();
  ...
  const coverKickerText = showGuestNameInCover ? tx("invitacion.sabor.reservaConfirmadaBoda").toUpperCase() : tx("invitacion.sabor.reservaConfirmadaBodaDe").toUpperCase();
  ...
  const portadaMensaje = String(
    invitation.portadaMensaje || tx("invitacion.sabor.mensajeConserjeria")
  );
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E8EEF7";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#B8C9DE";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(184,201,222,.35), 0 18px 50px -30px #B8C9DE";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#3E5A78" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(184,201,222,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(184,201,222,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#3E5A78" : "#B8C9DE";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A1220", tinta: "#F4F1EA", acento: "#B8C9DE", clase: `${phlPlayfair.variable} ${phlMono.variable}`, fuente: "var(--phl-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="phl-scroller">
        <section data-tone="dark" data-screen-label="Save the Date" className="phl-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #141F33 0%, #0D1B2E 55%, #0A1220 100%)" }}>
          <span data-xin="1" data-dist="-60" className="phl-kicker">{"01 — " + tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          ...
          <div data-drift="-70" className="phl-medallion phl-medallion--corner">
            <Medallion label="LM" sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="phlArc1" arcText={tx("invitacion.sabor.arcoGrandHotel").toUpperCase()} spin="normal" />
          </div>
        </section>
            ...
            <div className="phl-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="10,18,32" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="10,18,32" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="phl-section phl-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #0F2038 0%, #0B1A2C 55%, #0A1220 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="phl-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1B2A44 0%, #0C0B11 52%, #0A1220 100%)" }}>
          <div data-drift="-130" className="phl-glow-blob" />
          <span data-xin="1" data-dist="-60" className="phl-kicker" style={{ position: "relative" }}>{knPre(3)} {"— " + tx("invitacion.cuentaRegresiva.cuandoLlegueACero").toUpperCase()}</span>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="phl-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
            ...
            <div data-strip="1" className="phl-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="phl-panel phl-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <div className="phl-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="phl-side-hint">{scrollVertical ? "↓" : "→"}</span></div>
                </div>
              )}
              ...
              <div id="details" data-tone="light" className="phl-panel phl-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                <div className="phl-facts">
                  ...
                  {dressCode && (
                    <div className="phl-facts-row phl-facts-row--last">
                      <span>{tx("invitacion.pase.codigo").toUpperCase()}</span><span className="phl-accent-serif-2">{dressCode.toUpperCase()}</span>
                    </div>
                  )}
                </div>
                {cronograma.length > 0 && (
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="phl-panel phl-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="phl-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#3E5A78" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#3E5A78" />
                  </svg>
                  <div className="phl-panel-block">
                    <span className="phl-mini-label">{ceremoniaHabilitada ? "03" : "02"} / {LUGAR_PANEL_COUNT}</span>
                    <span className="phl-panel-title-sm">{tx("invitacion.ubicacion.comoLlegar")}</span>
                    <span className="phl-mini-label">{tx("invitacion.sabor.valetConserjeria").toUpperCase()}</span>
              ...
              <div data-tone="dark" className="phl-panel phl-panel--center" style={{ background: "#0D1B2E", color: "#F4F1EA" }}>
                <div className="phl-medallion phl-medallion--lg">
                  <Medallion label={dressCode ? dressCode.toUpperCase() : tx("invitacion.pase.acceso").toUpperCase()} sub={tx("invitacion.pase.numeroSuite", { n: passNumber }).toUpperCase()} arcId="phlArc2" arcText={tx("invitacion.pase.arcoAccesoVipSuite", { n: passNumber }).toUpperCase()} spin="reverse" title={tx("invitacion.pase.reservado")} />
                </div>
                <span className="phl-mini-label">{LUGAR_PANEL_COUNT} / {LUGAR_PANEL_COUNT} {"— " + tx("invitacion.ubicacion.tuUbicacion").toUpperCase()}</span>
                {/* mesa-en-ubicacion */}
                {guest?.mesas && guest.mesas.length > 0 && (
                  <span
                    className="phl-mini-label"
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="phl-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #141F33 0%, #0D1B2E 60%, #0A1220 100%)" }}>
          ...
          <h2 data-xin="1" data-delay="80" data-dist="130" className="phl-h2">
            {tx("invitacion.rsvp.confirmaLinea1")}<br /><span className="phl-accent-italic">{tx("invitacion.rsvp.confirmaLinea2")}</span>
          </h2>
          ...
          {rsvpEnabled ? (
            ...
            <p className="phl-lead">{tx("invitacion.rsvp.cerrada")}</p>
          )}
        </section>

        <div id="album" data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="phl-pan">
          <div className="phl-pan-sticky">
            <div data-strip="1" className="phl-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="phl-panel phl-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  {pageIndex === 0 && <h2 className="phl-panel-title-md">{tx("invitacion.album.titulo")} <span className="phl-accent-serif">{tx("invitacion.album.deFotos")}</span></h2>}
                  <div className="phl-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      ...
                      <span className="phl-photo-placeholder">{tx("invitacion.album.sinFotos")}</span>
                    )}
                  </div>
                  <div className="phl-seguir phl-seguir--split">
                    <span>{tx("invitacion.album.fotosSubidas", { n: allPhotos.length }).toUpperCase()}</span>
                    <span className="phl-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + " →"}</span>
                  </div>
                </div>
              ))}

              <div data-tone="light" className="phl-panel phl-panel--gap" style={{ background: "#EDE8DE", color: "#14141B" }}>
                <span className="phl-panel-top" style={{ display: "block" }}>{tx("invitacion.album.hoja", { n: String(photoPages.length + 1).padStart(2, "0"), etiqueta: "— " + tx("invitacion.enVivo.titulo").toUpperCase() }).toUpperCase()}</span>
                <h2 className="phl-panel-title">{tx("invitacion.album.tituloLinea1")}<br /><span className="phl-accent-serif">{tx("invitacion.album.tituloLinea2")}</span></h2>
                <div className="phl-album-embed">
                  {livePhotos.length > 0 ? (
                    <LiveAlbumStrip photos={livePhotos} tone="light" accentColor="#3E5A78" />
                  ) : (
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="phl-section" style={{ background: "#0D1B2E" }}>
            <span data-xin="1" data-dist="-60" className="phl-kicker">{kn(6)} {"— " + tx("invitacion.musica.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="phl-h2">{tituloEnDosLineas(tx("invitacion.sabor.preguntaPedidoFaltar"), "phl-accent-italic")}</h2>
            <div data-xin="1" data-delay="160" data-dist="-80" className="phl-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="phl-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E8EEF7" : "#B8C9DE" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="phl-section" style={{ background: "#0D1B2E" }}>
            <span data-xin="1" data-dist="-60" className="phl-kicker">{sugerenciaMusicaHabilitada ? kn(7) : kn(6)} {"— " + tx("invitacion.regalos.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="phl-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><span className="phl-accent-italic">{tx("invitacion.regalos.siQueresLinea2")}</span>
            </h2>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="phl-section" style={{ background: "#0D1B2E" }}>
            <span data-xin="1" data-dist="-60" className="phl-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection].filter(Boolean).length + 6)} {"— " + tx("invitacion.quiz.kicker").toUpperCase()}</span>
        ...
        <section data-tone="dark" data-screen-label="Tu reserva" className="phl-section phl-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #141F33 0%, #0D1B2E 55%, #0A1220 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="phl-final-card">
            ...
            <span className="phl-mini-label phl-accent-serif-2">{tx("invitacion.pase.numeroSuiteAdmite", { n: passNumber, cantidad: guestAdults + guestTeens + guestChildren || 1 }).toUpperCase()}</span>
          ...
          <div className="phl-final-footer">
            ...
            <span className="phl-replay" onClick={reset}>{tx("invitacion.portada.verAperturaOtraVez").toUpperCase() + " ↺"}</span>
          </div>
      ...
      <div ref={railRef} className="phl-rail">
        <span ref={railTopRef} className="phl-rail-top">{tx("invitacion.pase.numeroSuite", { n: passNumber }).toUpperCase()}</span>
      ...
      <div ref={coverRef} className="phl-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="phl-cover-half phl-cover-half--top">
          ...
          >
            <div className="phl-cover-cta">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</div>
          </CoverHalf>
        </div>
        <div ref={bottomRef} className="phl-cover-half phl-cover-half--bottom">
          <CoverHalf
            ...
            photoDesktop={photoDesktop}
          >
            <button onClick={open} className="phl-cover-cta phl-cover-cta--btn">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</button>
          </CoverHalf>
        </div>
      </div>

      <div ref={hintRef} className="phl-hint">{tx("invitacion.portada.desliza").toUpperCase() + " ↓"}</div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#B8C9DE" guest={guest} />
      )}
  ...
  return (
    <>
      ...
      <div className="phl-medallion-core">
        {title && <span className="phl-medallion-sub">{tx("invitacion.pase.sector").toUpperCase()}</span>}
  ...
  if (status === "DECLINED") {
    return (
      <div className="phl-rsvp-declined">
        <p className="phl-rsvp-declined-text">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
  ...
  return (
    <>
      <div className="phl-rsvp-rows">
        ...
        {status === "CONFIRMED" && (
          <>
            {totalGuests > 1 && adultCount > 0 && <div className="phl-rsvp-row"><span>{tx("invitacion.rsvp.adultos").toUpperCase()}</span><span>{String(adultCount).padStart(2, "0")}</span></div>}
            {teenCount > 0 && <div className="phl-rsvp-row"><span>{tx("invitacion.rsvp.adolescentes").toUpperCase()}</span><span>{String(teenCount).padStart(2, "0")}</span></div>}
            {childCount > 0 && <div className="phl-rsvp-row"><span>{tx("invitacion.rsvp.ninos").toUpperCase()}</span><span>{String(childCount).padStart(2, "0")}</span></div>}
          </>
        )}
          ...
          <div className="phl-rsvp-row phl-rsvp-row--payment">
            ...
            <div className="phl-rsvp-payment-value">
              ...
              {paymentStatus === "PARTIAL" && (
                <div className="phl-rsvp-payment-detail"><span>{tx("invitacion.pago.pagoParcialRegistrado")}</span></div>
              )}
      ...
      <div ref={stubRef} className="phl-stub">
        <div className="phl-stub-top">
          <span>{tx("invitacion.pase.numeroSuite", { n: passNumber }).toUpperCase()}</span>
    ...
    <div className="phl-song">
      <form onSubmit={handleSubmit} className="phl-song-row">
        <div className="phl-song-inputs">
          <input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder={tx("invitacion.musica.artista").toUpperCase()} maxLength={80} className="phl-song-input" />
          <span className="phl-song-sep">—</span>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tx("invitacion.musica.tema").toUpperCase()} maxLength={100} className="phl-song-input" />
        </div>
        ...
        <div className="phl-song-list">
          {songs.slice(0, 12).map((s, i) => (
            <div key={s.id} className="phl-song-item">
              ...
              <span className="phl-song-item-by">{tx("invitacion.musica.sumadoPor")} {s.guestName || tx("invitacion.evento.invitado")}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div className="phl-cover-inner">
      ...
      <div className="phl-cover-content">
        <div className="phl-cover-top-row">
          <span>{tx("invitacion.pase.numeroSuite", { n: passNumber }).toUpperCase()}</span><span className="phl-accent-serif-2">ALL ACCESS</span>
        </div>
        ...
        <div className="phl-cover-bottom">
          <div ref={perfRef} className="phl-perf-strip phl-perf-strip--reveal phl-perf-strip--cover" />
          ...
          <div className="phl-barcode-wrap">
            <div className="phl-barcode" style={{ width: "62%", height: "clamp(15px, 3vh, 26px)", opacity: 0.6 }} />
            <span className="phl-mini-label phl-mini-label--cover" style={{ color: "#56534A" }}>{tx("invitacion.pase.noTransferible").toUpperCase()}</span>
          </div>
        </div>
      </div>
    </div>
...
const PHL_CSS = `
  ...
  a { color: #B8C9DE; text-decoration: none; }
  a:hover { color: #E8EEF7; }
  ...
  .phl-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0A1220; }
  ...
  @media (min-width: 768px) {
    .phl-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(184,201,222,.3); }
  ...
  .phl-date-month { font-family: var(--phl-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B8C9DE; padding-left: 12%; }

  .phl-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .phl-divider-line { width: 52px; height: 1px; background: #B8C9DE; display: inline-block; }
  ...
  .phl-cal-link:hover { color: #B8C9DE; }
  ...
  .phl-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #122A42, #1E3F5E, #B8C9DE, #E8EEF7, #122A42); filter: saturate(.75); }
  ...
  .phl-medallion-label { font-family: var(--phl-playfair), serif; font-size: 26px; line-height: 1; color: #E8EEF7; }
  .phl-medallion-label-sm { font-family: var(--phl-playfair), serif; font-size: 16px; color: #E8EEF7; }
  .phl-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .phl-medallion-sub--accent { color: #B8C9DE; }
  ...
  .phl-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B8C9DE; opacity: 0.4; font-family: var(--phl-mono), monospace; }

  .phl-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(184,201,222,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .phl-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E8EEF7, transparent); animation: phlScan 6s linear infinite; pointer-events: none; }

  .phl-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .phl-cd-box { border: 1px solid #1A2C40; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .phl-cd-num { font-family: var(--phl-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F2F6FC; display: inline-block; }
  .phl-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B8C9DE; }
  .phl-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A1220 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .phl-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .phl-perf-strip--cover { opacity: .9; }

  .phl-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(18,42,66,0.5), rgba(30,63,94,0.3), rgba(184,201,222,0.4), rgba(18,42,66,0.5)); filter: blur(80px); opacity: .4; animation: phlFoil 30s linear infinite; }
  ...
  .phl-accent-italic { font-style: italic; color: #B8C9DE; }
  .phl-accent-serif { font-style: italic; color: #3E5A78; font-family: var(--phl-playfair), serif; }
  .phl-accent-serif-2 { color: #3E5A78; }
  ...
  .phl-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  ...
  .phl-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #3E5A78; margin-top: auto; }
  ...
  .phl-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #3E5A78; }
  ...
  .phl-crono-time { font-family: var(--phl-mono), monospace; color: #3E5A78; min-width: 42px; }
  ...
  .phl-stub { position: relative; overflow: hidden; border: 1px solid #1A2C40; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .phl-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E8EEF7, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .phl-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(184,201,222,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .phl-rsvp-payment-total { color: #E8EEF7; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .phl-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B8C9DE; background: transparent; color: #B8C9DE; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .phl-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(184,201,222,0.3); color: #F4F1EA; font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .phl-rsvp-input::placeholder { color: #6E6A5D; }
  .phl-rsvp-input:focus { outline: none; border-bottom-color: #B8C9DE; }
  .phl-rsvp-btn { width: 100%; padding: 16px; font-family: var(--phl-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B8C9DE, #8FA3BC); border: 1px solid #B8C9DE; color: #0B0B10; cursor: pointer; }
  .phl-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .phl-rsvp-btn--ghost { background: transparent; color: #B8C9DE; }
  .phl-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B8C9DE; margin: 0; }
  ...
  .phl-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(184,201,222,0.18); }
  ...
  .phl-bank-copy { flex-shrink: 0; font-family: var(--phl-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B8C9DE; background: transparent; color: #B8C9DE; cursor: pointer; }
  .phl-bank-copy:hover { background: rgba(184,201,222,0.12); }
  ...
  .phl-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(184,201,222,0.3); padding-bottom: 12px; }
  ...
  .phl-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(184,201,222,0.3); color: #F4F1EA; font-family: var(--phl-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .phl-song-input::placeholder { color: #6E6A5D; }
  .phl-song-input:focus { outline: none; border-bottom-color: #B8C9DE; }
  .phl-song-sep { color: #8A8577; flex-shrink: 0; }
  .phl-song-submit { flex-shrink: 0; background: none; border: none; color: #B8C9DE; font-family: var(--phl-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .phl-song-submit:hover { color: #E8EEF7; }
  .phl-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .phl-song-error { font-size: 10px; color: #B8C9DE; margin-top: 6px; }
  ...
  .phl-quiz-opt { font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(184,201,222,0.4); background: transparent; color: #B8C9DE; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .phl-quiz-opt:disabled { cursor: default; }
  .phl-quiz-opt--picked { background: #B8C9DE; border-color: #B8C9DE; color: #0B0B10; }
  .phl-quiz-opt--correct { background: #B8C9DE; border-color: #B8C9DE; color: #0B0B10; }
  .phl-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .phl-quiz-result { padding-top: 18px; border-top: 1px solid rgba(184,201,222,0.2); }
  .phl-quiz-result-score { margin: 0 0 6px; font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E8EEF7; }
  .phl-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .phl-final-card { border: 1px solid #B8C9DE; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .phl-final-names { font-family: var(--phl-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E8EEF7; }
  ...
  .phl-replay { cursor: pointer; color: #B8C9DE; }
  .phl-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .phl-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(184,201,222,0.14); }
  .phl-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .phl-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(184,201,222,0.16); position: relative; }
  .phl-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E8EEF7, #B8C9DE); transition: height 260ms linear; display: block; }
  .phl-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B8C9DE; transition: color 500ms ease; }
  ...
  .phl-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #0E2238 0%, #0D1B2E 46%, #0A1220 100%); }
  .phl-cover-half--bottom .phl-cover-inner { top: auto; bottom: 0; }
  .phl-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(18,42,66,0.7), rgba(30,63,94,0.4), rgba(184,201,222,0.6), rgba(18,42,66,0.7)); filter: blur(64px); opacity: .62; animation: phlFoil 26s linear infinite; }
  .phl-cover-sunburst { position: absolute; inset: 0; background-image: repeating-conic-gradient(from 0deg at 50% 50%, rgba(184,201,222,.05) 0deg 1deg, transparent 1deg 3deg); }
  ...
  .phl-cover-names { margin: 0; font-family: var(--phl-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E8EEF7; }
  .phl-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B8C9DE, transparent); display: block; }
  ...
  .phl-cover-cta { border: 1px solid #B8C9DE; background: linear-gradient(100deg, rgba(184,201,222,0.08), rgba(232,238,247,0.2), rgba(184,201,222,0.08)); color: #F2F6FC; font-family: var(--phl-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .phl-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .phl-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,238,247,0.34), rgba(246,239,221,0.5), rgba(232,238,247,0.34)); color: #0B0B0F; }
  }
  ...
  .phl-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(10,18,32,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .phl-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B8C9DE; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
