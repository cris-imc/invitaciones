...
export function ShojoTemplate({ invitation, guest, isPersonalized = false }: ShojoTemplateProps) {
  const tx = useTextos();
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="sho-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="sho-section sho-std">
          <div className="sho-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="sho-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="sho-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="sho-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="sho-std-fila">
              <span className="sho-cinta">{mesLargo}</span>
              <span className="sho-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="sho-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="sho-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="sho-btn-papel sho-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="sho-foto-seccion">
            <div className="sho-foto-marco">
              <div ref={ventanaRef} className="sho-ventana">
                {fotoMobile && (
              ...
              <div className="sho-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="sho-section sho-countdown">
          <div className="sho-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="sho-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="sho-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="sho-section sho-frase-seccion">
            <div className="sho-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="sho-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="sho-firma">
              <span className="sho-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="sho-pan-fijo">
            <div data-strip="1" className="sho-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="sho-panel sho-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="sho-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="sho-tarjeta sho-tarjeta--izq">
                  <span className="sho-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="sho-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="sho-filas">
                    {direccion && (
                      <div className="sho-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="sho-fila-valor">{direccion}</span></div>
                    )}
                    <div className="sho-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="sho-fila sho-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="sho-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="sho-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="sho-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="sho-panel sho-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="sho-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="sho-tarjeta sho-tarjeta--der">
                    <h2 className="sho-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="sho-filas">
                      {ceremoniaDireccion && (
                        <div className="sho-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="sho-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="sho-fila sho-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="sho-panel sho-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="sho-escena" aria-hidden="true">
                  ...
                  <h2 className="sho-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="sho-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="sho-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="sho-panel sho-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="sho-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="sho-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="sho-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="sho-crono-fila">
                        <span className="sho-crono-hora">{item.time || ""}</span>
                        <span className="sho-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="sho-section sho-checkin">
            <div className="sho-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="sho-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="sho-h2">
            ...
            <div className="sho-pan-fijo">
              <div data-strip="1" className="sho-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="sho-panel sho-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="sho-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="sho-h2 sho-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="sho-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="sho-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="sho-polaroid-img" />
                          {i % 2 === 0 && <span className="sho-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="sho-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="sho-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="sho-section sho-musica">
            <div className="sho-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="sho-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="sho-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="sho-section sho-regalos">
            <span data-xin="1" data-dist="-20" className="sho-kicker sho-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="sho-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="sho-parrafo sho-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="sho-section sho-quiz">
            <div className="sho-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="sho-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="sho-h2 sho-h2--claro">{triviaTitulo}</h2>
            <TriviaShojo
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="sho-section sho-pase">
          <div className="sho-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="sho-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="sho-ticket">
            <div className="sho-ticket-izq">
              <span className="sho-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="sho-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="sho-ticket-dato">
                {[
              ...
              <span className="sho-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="sho-ticket-muesca sho-ticket-muesca--arriba" aria-hidden="true" />
              <span className="sho-ticket-muesca sho-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="sho-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="sho-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="sho-pase-pie">
            <span className="sho-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="sho-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="sho-portada">
        ...
        <div ref={escenaPortadaRef} className="sho-portada-escena">
          <div data-cl="1" className="sho-trama" aria-hidden="true" />
        </div>
        <div className="sho-portada-contenido">
          <div className="sho-portada-arriba">
            <div ref={cartelRef} className="sho-cartel">
              <h1 className="sho-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="sho-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="sho-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="sho-portada-abajo">
            <div className="sho-portada-datos">
              <span className="sho-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="sho-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="sho-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="sho-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="sho-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="sho-portada-pase">
            ...
            <button type="button" onClick={abrir} className="sho-portada-btn">
...
function CuentaShojo({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="sho-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`sho-cuenta-caja sho-cuenta-caja--${i + 1}`}>
          <span className={`sho-cuenta-num${i === 3 ? " sho-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_SHO = `
  ...
  .sho-raiz a { color: var(--pp-acc); text-decoration: none; }
  .sho-raiz button { font: inherit; }
  .sho-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --sho-y y gira en --sho-giro: son
  ...
  .sho-scroller [data-xin] { transform: translate3d(0, var(--sho-y, 0px), 0) rotate(var(--sho-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .sho-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .sho-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .sho-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .sho-kicker--tinta { color: var(--pp-ink); }
  .sho-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--sho-serif), 'Cherry Bomb One', cursive;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .sho-h2 em, .sho-acento { font-style: italic; color: var(--pp-acc); }
  .sho-h2--claro { color: var(--pp-night-ink); }
  .sho-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .sho-acento-plano { color: var(--pp-acc); }
  .sho-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .sho-parrafo--tinta { color: var(--pp-ink); }
  .sho-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .sho-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .sho-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .sho-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .sho-std-dia { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-weight: 500;
  ...
  .sho-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .sho-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .sho-std-anio { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .sho-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .sho-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .sho-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .sho-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .sho-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .sho-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .sho-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .sho-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .sho-cuenta-caja--1 { --sho-giro: -1.6deg; transform: rotate(var(--sho-giro)); }
  .sho-cuenta-caja--2 { --sho-giro: 1.2deg; transform: rotate(var(--sho-giro)); margin-top: 10px; }
  .sho-cuenta-caja--3 { --sho-giro: 1.8deg; transform: rotate(var(--sho-giro)); }
  .sho-cuenta-caja--4 { --sho-giro: -1.1deg; transform: rotate(var(--sho-giro)); margin-top: 10px; }
  .sho-cuenta-num { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .sho-cuenta-num--acc { color: var(--pp-acc); }
  .sho-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .sho-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .sho-frase { position: relative; z-index: 1; margin: 0; font-family: var(--sho-serif), 'Cherry Bomb One', cursive;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .sho-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .sho-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .sho-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .sho-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .sho-panel--abajo { justify-content: flex-end; }
  .sho-panel--centro { justify-content: center; }
  .sho-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .sho-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .sho-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .sho-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .sho-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .sho-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .sho-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .sho-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .sho-tarjeta--izq { --sho-giro: -1deg; transform: rotate(var(--sho-giro)); }
  .sho-tarjeta--der { --sho-giro: 1.1deg; transform: rotate(var(--sho-giro)); }
  .sho-tarjeta--talon { --sho-giro: -.8deg; transform: rotate(var(--sho-giro)); overflow: visible; }
  .sho-tarjeta--hoy { --sho-giro: -1deg; transform: rotate(var(--sho-giro)); align-items: flex-start; }
  .sho-tarjeta--banco { margin-top: 4px; }
  .sho-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .sho-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .sho-tarjeta-titulo { margin: 0; font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .sho-tarjeta-pregunta { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: 26px; line-height: 1.15; }
  .sho-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .sho-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .sho-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .sho-fila > span:first-child { color: var(--pp-ink2); }
  .sho-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .sho-fila-valor { text-align: right; }
  .sho-fila--copiable { align-items: center; }
  .sho-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .sho-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .sho-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .sho-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .sho-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .sho-checkin { background: var(--pp-bg); }
  .sho-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .sho-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .sho-campo { display: flex; flex-direction: column; gap: 6px; }
  .sho-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .sho-contador { display: flex; align-items: center; gap: 12px; }
  .sho-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .sho-contador button:disabled { opacity: .35; cursor: default; }
  .sho-contador > span { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .sho-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--sho-sans), 'M PLUS Rounded 1c', sans-serif; font-size: 16px; padding: 6px 2px; }
  .sho-input--serif { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: 20px; }
  .sho-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .sho-input::placeholder { color: #9A9388; }
  .sho-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .sho-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .sho-precio-total { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .sho-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .sho-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .sho-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .sho-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .sho-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--sho-sans), 'M PLUS Rounded 1c', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .sho-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .sho-btn-solido:disabled { opacity: .6; cursor: default; }
  .sho-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .sho-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --sho-giro: -1deg; transform: rotate(var(--sho-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .sho-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .sho-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .sho-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .sho-crono-hora { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: 26px; min-width: 64px; line-height: 1; }
  .sho-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .sho-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .sho-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .sho-polaroids[data-cantidad="1"] .sho-polaroid { grid-column: span 6; }
  .sho-polaroids[data-cantidad="2"] .sho-polaroid { grid-column: span 3; }
  .sho-polaroids[data-cantidad="4"] .sho-polaroid { grid-column: span 3; }
  .sho-polaroids[data-cantidad="5"] .sho-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .sho-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .sho-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .sho-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .sho-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .sho-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .sho-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .sho-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .sho-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .sho-lista-tema { font-size: 15px; }
  .sho-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .sho-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .sho-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .sho-opciones { display: flex; flex-direction: column; gap: 8px; }
  .sho-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--sho-sans), 'M PLUS Rounded 1c', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .sho-opcion:disabled { cursor: default; }
  .sho-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .sho-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .sho-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .sho-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --sho-giro: -.6deg; transform: rotate(var(--sho-giro));
    border-radius: 12px; overflow: hidden; }
  .sho-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .sho-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .sho-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .sho-ticket-nombre { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .sho-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .sho-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .sho-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .sho-ticket-muesca--arriba { top: -12px; }
  .sho-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .sho-ticket-der .qr-ingreso, .sho-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .sho-info-extra { position: relative; z-index: 1; }
  .sho-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .sho-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .sho-raiz .ia-icon-box { display: none !important; }
  .sho-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .sho-pase-firma { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .sho-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .sho-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }
  ...
  .sho-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .sho-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  ...
  .sho-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .sho-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .sho-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .sho-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .sho-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .sho-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .sho-cartel-nombres { margin: 0; font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-weight: 500;
  ...
  .sho-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .sho-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .sho-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .sho-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .sho-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .sho-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .sho-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .sho-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--sho-sans), 'M PLUS Rounded 1c', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .sho-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .sho-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .sho-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .sho-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .sho-section, .sho-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .sho-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .sho-raiz { font-family: var(--sho-sans), 'M PLUS Rounded 1c', sans-serif; }
  ...
  .sho-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .sho-kicker, .sho-panel-top, .sho-panel-pie, .sho-meta, .sho-riel-top, .sho-riel-etiqueta,
  .sho-ticket-kicker, .sho-cuenta-etq, .sho-tarjeta-kicker, .sho-fila-etq, .sho-portada-pase {
    font-family: var(--sho-sans), 'M PLUS Rounded 1c', sans-serif; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .sho-h2, .sho-frase, .sho-tarjeta-titulo, .sho-std-dia, .sho-cartel-nombres, .sho-ticket-nombre, .sho-cifra, .sho-cuenta-num {
    font-family: var(--sho-serif), 'Cherry Bomb One', cursive; letter-spacing: -.03em; }
  .sho-h2 em, .sho-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .sho-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .sho-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .sho-std-fila { justify-content: space-between; }
  .sho-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .sho-std-anio { font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .sho-std .sho-kicker, .sho-std .sho-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .sho-foto-marco { border: 3px solid var(--pp-ink); }
  .sho-ventana { clip-path: none !important; box-shadow: none; }
  .sho-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--sho-serif), 'Cherry Bomb One', cursive; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .sho-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .sho-tarjeta--izq, .sho-tarjeta--der, .sho-tarjeta--talon, .sho-tarjeta--hoy { transform: none; }
  .sho-doblez::before { display: none; }
  .sho-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .sho-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .sho-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .sho-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .sho-cuenta-caja:first-child { border-left: none; }
  .sho-cuenta-caja::before { display: none; }
  .sho-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .sho-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .sho-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .sho-panel[data-tone="dark"] .sho-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .sho-panel-top, .sho-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .sho-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .sho-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .sho-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .sho-btn-solido, .sho-btn-papel, .sho-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .sho-btn-papel { background: transparent; border: 1px solid currentColor; }
  .sho-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .sho-portada { background: var(--pp-bg); }
  .sho-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .sho-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .sho-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .sho-cartel-chinche { display: none; }
  .sho-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .sho-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .sho-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Shōjo: nada tiene esquinas y todo tiene un brillo. */
  .sho-tarjeta, .sho-ticket, .sho-btn-solido, .sho-portada-btn, .sho-opcion { border-radius: 18px; }
  .sho-cartel-nombres, .sho-h2 { letter-spacing: 0; text-shadow: 0 3px 0 color-mix(in srgb, var(--pp-acc) 45%, transparent); }
  .sho-trama { opacity: .25; background-image: radial-gradient(currentColor 1.2px, transparent 1.4px); background-size: 20px 20px; }

  @media (prefers-reduced-motion: reduce) {
    .sho-raiz * { animation: none !important; }
    .sho-scroller [data-xin] { opacity: 1 !important; --sho-y: 0px; }
  }
`;
