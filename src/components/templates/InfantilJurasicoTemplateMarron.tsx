...
/**
 * InfantilJurasicoTemplateMarron.tsx
 *
...
interface InfantilJurasicoTemplateMarronProps {
...
export function InfantilJurasicoTemplateMarron({ invitation, guest, isPersonalized = false }: InfantilJurasicoTemplateMarronProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F3E9DA";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#A87D4A";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #A87D4A";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#3A2C18" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#3A2C18" : "#A87D4A";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#101408", tinta: "#F4F1EA", acento: "#A87D4A", clase: `${ifjDisplay.variable} ${ifjMono.variable}`, fuente: "var(--ifj-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="ifj-panel ifj-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="ifj-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#3A2C18" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#3A2C18" />
                  </svg>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="ifj-section" style={{ background: "#141A0C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="ifj-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="ifj-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F3E9DA" : "#A87D4A" }} />
              ))}
            </div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#A87D4A" guest={guest} />
      )}
...
const GP_CSS = `
  ...
  a { color: #A87D4A; text-decoration: none; }
  a:hover { color: #F3E9DA; }
  ...
  .ifj-date-month { font-family: var(--ifj-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #A87D4A; padding-left: 12%; }

  .ifj-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .ifj-divider-line { width: 52px; height: 1px; background: #A87D4A; display: inline-block; }
  ...
  .ifj-cal-link:hover { color: #A87D4A; }
  ...
  .ifj-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #A87D4A, #F3E9DA, #6E5AA8); filter: saturate(.75); }
  ...
  .ifj-medallion-label { font-family: var(--ifj-display), serif; font-size: 26px; line-height: 1; color: #F3E9DA; }
  .ifj-medallion-label-sm { font-family: var(--ifj-display), serif; font-size: 16px; color: #F3E9DA; }
  .ifj-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .ifj-medallion-sub--accent { color: #A87D4A; }
  ...
  .ifj-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #A87D4A; opacity: 0.4; font-family: var(--ifj-mono), monospace; }
  ...
  .ifj-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F3E9DA, transparent); animation: bbsScan 6s linear infinite; pointer-events: none; }
  ...
  .ifj-cd-num { font-family: var(--ifj-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F3E9DA; display: inline-block; }
  .ifj-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #A87D4A; }
  ...
  .ifj-accent-italic { font-style: italic; color: #A87D4A; }
  .ifj-accent-serif { font-style: italic; color: #3A2C18; font-family: var(--ifj-display), serif; }
  .ifj-accent-serif-2 { color: #3A2C18; }
  ...
  .ifj-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #3A2C18; margin-top: auto; }
  ...
  .ifj-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #3A2C18; }
  ...
  .ifj-crono-time { font-family: var(--ifj-mono), monospace; color: #3A2C18; min-width: 42px; }
  ...
  .ifj-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F3E9DA, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .ifj-rsvp-payment-total { color: #F3E9DA; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .ifj-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #A87D4A; background: transparent; color: #A87D4A; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .ifj-rsvp-input:focus { outline: none; border-bottom-color: #A87D4A; }
  .ifj-rsvp-btn { width: 100%; padding: 16px; font-family: var(--ifj-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #A87D4A, #3A2C18); border: 1px solid #A87D4A; color: #141A0C; cursor: pointer; }
  .ifj-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .ifj-rsvp-btn--ghost { background: transparent; color: #A87D4A; }
  .ifj-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #A87D4A; margin: 0; }
  ...
  .ifj-bank-copy { flex-shrink: 0; font-family: var(--ifj-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #A87D4A; background: transparent; color: #A87D4A; cursor: pointer; }
  ...
  .ifj-song-input:focus { outline: none; border-bottom-color: #A87D4A; }
  .ifj-song-sep { color: #8A8577; flex-shrink: 0; }
  .ifj-song-submit { flex-shrink: 0; background: none; border: none; color: #A87D4A; font-family: var(--ifj-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .ifj-song-submit:hover { color: #F3E9DA; }
  .ifj-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .ifj-song-error { font-size: 10px; color: #A87D4A; margin-top: 6px; }
  ...
  .ifj-quiz-opt { font-family: var(--ifj-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #A87D4A; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .ifj-quiz-opt:disabled { cursor: default; }
  .ifj-quiz-opt--picked { background: #A87D4A; border-color: #A87D4A; color: #141A0C; }
  .ifj-quiz-opt--correct { background: #A87D4A; border-color: #A87D4A; color: #141A0C; }
  ...
  .ifj-quiz-result-score { margin: 0 0 6px; font-family: var(--ifj-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F3E9DA; }
  .ifj-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .ifj-final-card { border: 1px solid #A87D4A; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .ifj-final-names { font-family: var(--ifj-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F3E9DA; }
  ...
  .ifj-replay { cursor: pointer; color: #A87D4A; }
  ...
  .ifj-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F3E9DA, #A87D4A); transition: height 260ms linear; display: block; }
  .ifj-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #A87D4A; transition: color 500ms ease; }
  ...
  .ifj-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(90,58,34,0.45), rgba(168,125,74,0.32), rgba(168,125,74,0.55), rgba(90,58,34,0.45)); filter: blur(64px); opacity: .62; animation: bbsFoil 26s linear infinite; }
  .ifj-cover-texture { position: absolute; inset: 0; background-image: repeating-linear-gradient(60deg, rgba(243,233,218,.06) 0 3px, transparent 3px 34px), repeating-linear-gradient(-60deg, rgba(243,233,218,.05) 0 3px, transparent 3px 34px); }
  ...
  .ifj-cover-names { margin: 0; font-family: var(--ifj-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F3E9DA; }
  .ifj-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#A87D4A, transparent); display: block; }
  ...
  .ifj-cover-cta { border: 1px solid #A87D4A; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--ifj-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .ifj-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #A87D4A; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
