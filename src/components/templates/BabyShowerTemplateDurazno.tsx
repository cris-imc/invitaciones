...
interface BabyShowerTemplateDuraznoProps {
...
export function BabyShowerTemplateDurazno({ invitation, guest, isPersonalized = false }: BabyShowerTemplateDuraznoProps) {
  const tx = useTextos();
  ...
  const namesTitle = festejado || "Baby Shower";
  ...
  const coverKickerText = showGuestNameInCover ? tx("invitacion.portada.unaInvitacionEspecialPara").toUpperCase() : tx("invitacion.sabor.esperamosLlegadaDe").toUpperCase();
  ...
  const portadaMensaje = String(
    invitation.portadaMensaje || tx("invitacion.sabor.mensajeEsperaJuntos")
  );
  ...
  const triviaPreguntas: BbsQuizQuestion[] = safeJson<BbsQuizQuestion[]>(String(invitation.triviaPreguntas ?? ""), []);
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#E8A87C";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #E8A87C";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#A8623A" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#A8623A" : "#E8A87C";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#14101A", tinta: "#F4F1EA", acento: "#E8A87C", clase: `${bbsDisplay.variable} ${bbsMono.variable}`, fuente: "var(--bbs-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: "hidden",
        background: "#14101A",
        fontFamily: "var(--bbs-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{GP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="bbs-scroller">
        <section data-tone="dark" data-screen-label="Save the Date" className="bbs-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #191420 55%, #14101A 100%)" }}>
          <span data-xin="1" data-dist="-60" className="bbs-kicker">{"01 — " + tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="bbs-date-stack">
            <span data-xin="1" data-delay="60" data-dist="-110" className="bbs-date-num">{dayNum}</span>
            <span data-xin="1" data-delay="170" data-dist="140" className="bbs-date-month">{monthAbbr}</span>
            <span data-xin="1" data-delay="280" data-dist="220" className="bbs-date-num bbs-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="bbs-divider">
            <span className="bbs-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="bbs-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="bbs-cal-link"
          />

          <div data-drift="-70" className="bbs-medallion bbs-medallion--corner">
            <BbsMedallionCmp label="BB" sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="bbsArc1" arcText={tx("invitacion.sabor.arcoBabyShower").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        {(photoMobile || photoDesktop) && (
          <section
            ...
            className={`bbs-hero-photo-section${!photoMobile ? " bbs-hero-photo-section--no-mobile" : ""}${!photoDesktop ? " bbs-hero-photo-section--no-desktop" : ""}`}
          >
            <div className="bbs-hero-photo-frame">
              {photoMobile && (
            ...
            <span data-xin="1" data-dist="-60" className="bbs-kicker bbs-hero-photo-kicker">02 — EL PRIMER VISTAZO</span>
          </section>
        )}

        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="bbs-section bbs-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A2032 0%, #0D0B10 55%, #14101A 100%)" }}>
          <div className="bbs-scan-grid" />
          <div className="bbs-scanline" />
          <span data-xin="1" data-dist="-60" className="bbs-kicker" style={{ position: "relative" }}>{knPre(2)} — LA CELEBRACIÓN ES EN</span>
          <div className="bbs-cd-grid">
            <BbsCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <BbsCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <BbsCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <BbsCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
          <div className="bbs-perf-strip" />
        </section>

        {hasFrase && (
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="bbs-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #14101A 100%)" }}>
            <div data-drift="-130" className="bbs-glow-blob" />
            <span data-xin="1" data-dist="-60" className="bbs-kicker" style={{ position: "relative" }}>{knPre(3)} {"— " + tx("invitacion.frase.unMensajeParaVos").toUpperCase()}</span>
            <h2 ref={phraseRef} className="bbs-phrase" style={{ fontSize: fraseFontSize }}>
              {fraseWords.map((w, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= fraseAccentStart ? "bbs-accent-italic" : undefined}>
                    {w}
                  </span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="120" data-dist="90" className="bbs-divider" style={{ position: "relative" }}>
              <span className="bbs-divider-line bbs-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
            </div>
          </section>
        )}

        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="bbs-pan">
          <div className="bbs-pan-sticky">
            <div data-strip="1" className="bbs-strip">
              <div id="details" data-tone="light" className="bbs-panel bbs-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                <div className="bbs-hair-bg" />
                <div className="bbs-panel-top">
                  <span>{kn(3)} {"— " + tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>01 / {LUGAR_PANEL_COUNT}</span>
                </div>
                <h2 className="bbs-panel-title">
                  {lugarNombre || tx("invitacion.ubicacion.elLugar")}
                  {direccion && <><br /><span className="bbs-accent-serif">{direccion}</span></>}
                </h2>
                <div className="bbs-facts">
                  <div className="bbs-facts-row">
                    <span>{tx("invitacion.ubicacion.horario").toUpperCase()}</span><span>{hora} H</span>
                  </div>
                  {dressCode && (
                    <div className="bbs-facts-row bbs-facts-row--last">
                      <span>{tx("invitacion.pase.codigo").toUpperCase()}</span><span className="bbs-accent-serif-2">{dressCode.toUpperCase()}</span>
                    </div>
                  )}
                </div>
                {cronograma.length > 0 && (
                  <div className="bbs-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="bbs-crono-row">
                        <span className="bbs-crono-time">{item.time || ""}</span>
                        <span className="bbs-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
                <div className="bbs-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="bbs-side-hint">{scrollVertical ? "↓" : "→"}</span></div>
              </div>

              {hayComoLlegar && (
                <div data-tone="light" className="bbs-panel bbs-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="bbs-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#A8623A" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#A8623A" />
                  </svg>
                  <div className="bbs-panel-block">
                    <span className="bbs-mini-label">02 / {LUGAR_PANEL_COUNT}</span>
                    <span className="bbs-panel-title-sm">{tx("invitacion.ubicacion.comoLlegar")}</span>
                    {direccion && <span className="bbs-mini-label">{direccion}</span>}
                    {mapUrl && (
                      <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="bbs-link-cta">
              ...
              <div data-tone="dark" className="bbs-panel bbs-panel--center" style={{ background: "#191420", color: "#F4F1EA" }}>
                <div className="bbs-medallion bbs-medallion--lg">
                  <BbsMedallionCmp label={dressCode ? dressCode.toUpperCase() : tx("invitacion.pase.acceso").toUpperCase()} sub={tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()} arcId="bbsArc2" arcText={tx("invitacion.pase.arcoAccesoVipPase", { n: passNumber }).toUpperCase()} spin="reverse" title={tx("invitacion.pase.reservado")} />
                </div>
                <span className="bbs-mini-label">{LUGAR_PANEL_COUNT} / {LUGAR_PANEL_COUNT} {"— " + tx("invitacion.ubicacion.tuUbicacion").toUpperCase()}</span>
                {/* mesa-en-ubicacion */}
                {guest?.mesas && guest.mesas.length > 0 && (
                  <span
                    className="bbs-mini-label"
            ...
            <BbsDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>

        <section data-tone="dark" data-screen-label="Check-in" className="bbs-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #191420 60%, #14101A 100%)" }}>
          <span data-xin="1" data-dist="-60" className="bbs-kicker">{kn(4)} — CHECK-IN</span>
          <h2 data-xin="1" data-delay="80" data-dist="130" className="bbs-h2">
            {tx("invitacion.rsvp.confirmaLinea1")}<br /><span className="bbs-accent-italic">{tx("invitacion.rsvp.confirmaLinea2")}</span>
          </h2>

          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="bbs-rsvp">
              <BbsRsvpCard
            ...
            <p className="bbs-lead">{tx("invitacion.rsvp.cerrada")}</p>
          )}
        </section>

        <div id="album" data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="bbs-pan">
          <div className="bbs-pan-sticky">
            <div data-strip="1" className="bbs-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="bbs-panel bbs-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  <div className="bbs-hair-bg" />
                  <div className="bbs-panel-top">
                  ...
                  {pageIndex === 0 && <h2 className="bbs-panel-title-md">{tx("invitacion.album.titulo")} <span className="bbs-accent-serif">{tx("invitacion.album.deFotos")}</span></h2>}
                  <div className="bbs-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`bbs-mosaic-cell${i === 0 ? " bbs-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                      ...
                      >
                        ...
                        <img src={url} alt="" className="bbs-mosaic-img" />
                      </div>
                    )) : (
                      <span className="bbs-photo-placeholder">{tx("invitacion.album.sinFotos")}</span>
                    )}
                  </div>
                  <div className="bbs-seguir bbs-seguir--split">
                    <span>{tx("invitacion.album.fotosSubidas", { n: allPhotos.length }).toUpperCase()}</span>
                    <span className="bbs-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + " →"}</span>
                  </div>
                </div>
              ))}

              <div data-tone="light" className="bbs-panel bbs-panel--gap" style={{ background: "#EDE8DE", color: "#14141B" }}>
                <span className="bbs-panel-top" style={{ display: "block" }}>{tx("invitacion.album.hoja", { n: String(photoPages.length + 1).padStart(2, "0"), etiqueta: "— " + tx("invitacion.enVivo.titulo").toUpperCase() }).toUpperCase()}</span>
                <h2 className="bbs-panel-title">{tx("invitacion.album.tituloLinea1")}<br /><span className="bbs-accent-serif">{tx("invitacion.album.tituloLinea2")}</span></h2>
                <div className="bbs-album-embed">
                  {livePhotos.length > 0 ? (
                    <LiveAlbumStrip photos={livePhotos} tone="light" accentColor="#A8623A" />
                  ) : (
                    <div className="bbs-live-placeholder">
                      <span className="bbs-mini-label">
                        {eventHasStarted
            ...
            <BbsDots count={photoPages.length + 1} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="bbs-section" style={{ background: "#191420" }}>
            <span data-xin="1" data-dist="-60" className="bbs-kicker">{kn(6)} {"— " + tx("invitacion.musica.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="bbs-h2">{tituloEnDosLineas(tx("invitacion.sabor.preguntaNombreTeGusta"), "bbs-accent-italic")}</h2>
            <div data-xin="1" data-delay="160" data-dist="-80" className="bbs-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="bbs-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FDF1E7" : "#E8A87C" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="bbs-song-wrap">
              <BbsSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="bbs-section" style={{ background: "#191420" }}>
            <span data-xin="1" data-dist="-60" className="bbs-kicker">{sugerenciaMusicaHabilitada ? kn(7) : kn(6)} {"— " + tx("invitacion.regalos.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="bbs-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><span className="bbs-accent-italic">{tx("invitacion.regalos.siQueresLinea2")}</span>
            </h2>
            <div data-xin="1" data-delay="160" data-dist="-80" className="bbs-bank-wrap">
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="bbs-section" style={{ background: "#191420" }}>
            <span data-xin="1" data-dist="-60" className="bbs-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection].filter(Boolean).length + 6)} {"— " + tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="bbs-h2" style={{ fontSize: "clamp(28px, 6vw, 44px)" }}>
              {triviaTitulo}
            </h2>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="bbs-section bbs-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #191420 55%, #14101A 100%)" }}>
          <span data-xin="1" data-dist="-60" className="bbs-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection, quizEnabled].filter(Boolean).length + 6)} {"— " + tx("invitacion.pase.guardaTuPase").toUpperCase()}</span>
          <div data-xin="1" data-delay="100" data-dist="130" className="bbs-final-card">
            <div className="bbs-medallion bbs-medallion--final">
              <BbsMedallionCmp label="BB" sub={confirmed ? "CONFIRMADO" : tx("invitacion.pase.pendiente").toUpperCase()} arcId="bbsArc3" arcText={textoArco(namesTitle, fechaCorta)} spin="reverse" />
            </div>
            <span className="bbs-mini-label bbs-accent-serif-2">{tx("invitacion.pase.numeroPaseAdmite", { n: passNumber, cantidad: guestAdults + guestTeens + guestChildren || 1 }).toUpperCase()}</span>
            <span className="bbs-final-names">{namesTitle}</span>
            <span className="bbs-mini-label" style={{ color: "#A8A292" }}>{fechaCorta} — {hora} H</span>
            <div className="bbs-barcode" style={{ width: "60%", height: 26, opacity: 0.6 }} />
          </div>
          <div className="bbs-final-footer">
            <span>{tx("invitacion.pase.noTransferible").toUpperCase()}</span>
            <span className="bbs-replay" onClick={reset}>{tx("invitacion.portada.verAperturaOtraVez").toUpperCase() + " ↺"}</span>
          </div>
          {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
          <InfoAdicionalSection invitation={invitation as any} />
          <QrDeIngreso guest={guest as never} />
          <div className="bbs-footer-credit">
            <LogoFooterCredit bgColor="transparent" />
          </div>
        </section>
      </div>

      <div ref={railRef} className="bbs-rail">
        <span ref={railTopRef} className="bbs-rail-top">{tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()}</span>
        <div ref={railLineRef} className="bbs-rail-line">
          <span ref={railBarRef} className="bbs-rail-bar" />
        </div>
      ...
      <div ref={coverRef} className="bbs-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="bbs-cover-half bbs-cover-half--top">
          ...
          >
            <div className="bbs-cover-cta">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</div>
          </BbsCoverHalf>
        </div>
        <div ref={bottomRef} className="bbs-cover-half bbs-cover-half--bottom">
          <BbsCoverHalf
            kickerText={coverKickerText}
            namesTitle={coverNamesTitle}
            fechaCorta={fechaCorta}
            ...
            photoDesktop={photoDesktop}
          >
            <button onClick={open} className="bbs-cover-cta bbs-cover-cta--btn">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</button>
          </BbsCoverHalf>
        </div>
      </div>

      <div ref={hintRef} className="bbs-hint">{tx("invitacion.portada.desliza").toUpperCase() + " ↓"}</div>

      {expandedPhoto && (
        <div
          className="bbs-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="bbs-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#E8A87C" guest={guest} />
      )}
...
function BbsCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="bbs-cd-box">
      <span ref={refEl} className="bbs-cd-num">—</span>
      <span className="bbs-cd-label">{label}</span>
    </div>
  );
}

function BbsDots({ count = 3 }: { count?: number }) {
  return (
    <div data-dots="1" className="bbs-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="bbs-dot" />
      ))}
    </div>
  );
}

function BbsMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      <div className="bbs-medallion-ring" style={{ animation: spin === "none" ? "none" : `bbsRing ${ringDuration}s linear infinite` }} />
      <div className="bbs-medallion-core">
        {title && <span className="bbs-medallion-sub">{tx("invitacion.pase.sector").toUpperCase()}</span>}
        <span className={compact ? "bbs-medallion-label-sm" : "bbs-medallion-label"}>{title || label}</span>
        {sub && <span className="bbs-medallion-sub bbs-medallion-sub--accent">{sub}</span>}
      </div>
      {arcId && (
        <svg viewBox="0 0 100 100" className="bbs-medallion-arc" style={{ animation: spin === "reverse" ? "bbsRingRev 32s linear infinite" : "bbsRingRev 34s linear infinite" }}>
          <defs>
...
function BbsCopyField({ label, value }: { label: string; value: string }) {
  const tx = useTextos();
  ...
  return (
    <div className="bbs-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="bbs-bank-row-label">{label}</span>
        <span className="bbs-bank-row-value">{value}</span>
      </div>
      <button type="button" className="bbs-bank-copy" onClick={handle}>
...
function BbsRsvpCard({
  ...
  if (status === "DECLINED") {
    return (
      <div className="bbs-rsvp-declined">
        <p className="bbs-rsvp-declined-text">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
        <button type="button" className="bbs-rsvp-btn bbs-rsvp-btn--ghost" onClick={() => setStatus("PENDING")}>
  ...
  return (
    <>
      <div className="bbs-rsvp-rows">
        <div className="bbs-rsvp-row">
        ...
        {totalGuests > 1 && status !== "CONFIRMED" && (
          <div className="bbs-rsvp-row">
            ...
            <div className="bbs-rsvp-stepper">
        ...
        {status === "CONFIRMED" && (
          <>
            {totalGuests > 1 && adultCount > 0 && <div className="bbs-rsvp-row"><span>{tx("invitacion.rsvp.adultos").toUpperCase()}</span><span>{String(adultCount).padStart(2, "0")}</span></div>}
            {teenCount > 0 && <div className="bbs-rsvp-row"><span>{tx("invitacion.rsvp.adolescentes").toUpperCase()}</span><span>{String(teenCount).padStart(2, "0")}</span></div>}
            {childCount > 0 && <div className="bbs-rsvp-row"><span>{tx("invitacion.rsvp.ninos").toUpperCase()}</span><span>{String(childCount).padStart(2, "0")}</span></div>}
          </>
        )}
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="bbs-rsvp-row bbs-rsvp-row--payment">
            <span>{paymentStatus === "PAID" ? "ABONADO" : "VALOR"}</span>
            <div className="bbs-rsvp-payment-value">
              <span className="bbs-rsvp-payment-total">{formatARS(totalPayment)}</span>
              {paymentStatus === "PARTIAL" && (
                <div className="bbs-rsvp-payment-detail"><span>{tx("invitacion.pago.pagoParcialRegistrado")}</span></div>
              )}
              {(adultCount > 0 || teenCount > 0 || childCount > 0) && (
                <div className="bbs-rsvp-payment-detail">
                  {useServerTotal
      ...
      <div ref={stubRef} className="bbs-stub">
        <div className="bbs-stub-top">
          <span>{tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()}</span>
          <span ref={statusRef} className="bbs-stub-status">
            {confirmed ? tx("invitacion.pase.accesoConfirmado").toUpperCase() : tx("invitacion.pase.pendiente").toUpperCase()}
          </span>
        </div>
        <div ref={sealRef} className="bbs-seal">
          <BbsMedallionCmp label="BB" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="bbs-beam" />
        <div className="bbs-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="bbs-rsvp-error">{error}</p>}

      {status !== "CONFIRMED" ? (
        <>
          <button type="button" className="bbs-rsvp-btn" disabled={isSubmitting} onClick={() => submit("CONFIRMA")}>
            {isSubmitting ? tx("invitacion.rsvp.guardando").toUpperCase() : tx("invitacion.rsvp.confirmarAsistencia").toUpperCase()}
          </button>
          <button type="button" className="bbs-rsvp-btn bbs-rsvp-btn--ghost" disabled={isSubmitting} onClick={() => submit("NO_ASISTE")}>
...
function BbsSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
  ...
  return (
    <div className="bbs-song">
      <form onSubmit={handleSubmit} className="bbs-song-row">
        <div className="bbs-song-inputs">
          <input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder={tx("invitacion.musica.artista").toUpperCase()} maxLength={80} className="bbs-song-input" />
          <span className="bbs-song-sep">—</span>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tx("invitacion.musica.tema").toUpperCase()} maxLength={100} className="bbs-song-input" />
        </div>
        <button type="submit" disabled={isSubmitting} className="bbs-song-submit">+ {isSubmitting ? "..." : "SUMAR"}</button>
      </form>
      {error && <p className="bbs-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="bbs-song-list">
          {songs.slice(0, 12).map((s, i) => (
            <div key={s.id} className="bbs-song-item">
              <span className="bbs-song-item-title">{String(i + 1).padStart(2, "0")} · {s.artist} — {s.title}</span>
              <span className="bbs-song-item-by">{tx("invitacion.musica.sumadoPor")} {s.guestName || tx("invitacion.evento.invitado")}</span>
            </div>
          ))}
        </div>
      )}
...
function BbsQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: BbsQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  const tx = useTextos();
  ...
  return (
    <div className="bbs-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="bbs-quiz-q">
            <span className="bbs-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="bbs-quiz-q-text">{q.pregunta}</p>
            <div className="bbs-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " bbs-quiz-opt--correct";
                  else if (chosen) stateClass = " bbs-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " bbs-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`bbs-quiz-opt${stateClass}`}
      ...
      {finished && (
        <div className="bbs-quiz-result">
          <p className="bbs-quiz-result-score">
          ...
          {!isSaving && stats && stats.count > 0 && (
            <p className="bbs-quiz-result-stat">
...
function BbsInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="bbs-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="bbs-bank-row-label">{label}</span>
    ...
    <div className="bbs-cover-inner">
      ...
      <div className="bbs-cover-content">
        <div className="bbs-cover-top-row">
          <span>{tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()}</span><span className="bbs-accent-serif-2">ALL ACCESS</span>
        </div>
        <div className="bbs-cover-center">
          <span ref={kickerRef} className="bbs-cover-kicker">{kickerText}</span>
          <h1 ref={namesRef} className="bbs-cover-names">{namesTitle}</h1>
          <span className="bbs-cover-rule" />
        ...
        <div className="bbs-cover-bottom">
          ...
          <div className="bbs-barcode-wrap">
            ...
            <span className="bbs-mini-label bbs-mini-label--cover" style={{ color: "#56534A" }}>{tx("invitacion.pase.noTransferible").toUpperCase()}</span>
          </div>
        </div>
      </div>
    </div>
...
// ---------------------------------------------------------------------
const GP_CSS = `
  .bbs-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .bbs-scroller::-webkit-scrollbar { width: 0; height: 0; }
  ...
  @media (prefers-reduced-motion: reduce) { .bbs-scroller * { animation: none !important; } }

  .bbs-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .bbs-section--between { justify-content: space-between; }
  ...
  .bbs-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #14101A; }
  .bbs-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  .bbs-hero-photo-kicker { position: absolute; left: 0; right: 0; bottom: 0; z-index: 2; padding: 0 max(24px, calc((100% - 560px) / 2)) 48px; }
  ...
  @media (max-width: 767px) {
    .bbs-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .bbs-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(200,164,92,.3); }
    .bbs-hero-photo-kicker { bottom: 40px; }
    .bbs-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .bbs-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .bbs-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .bbs-date-num { font-family: var(--bbs-display), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .bbs-date-num--right { text-align: right; line-height: 0.86; }
  .bbs-date-month { font-family: var(--bbs-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #E8A87C; padding-left: 12%; }

  .bbs-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .bbs-divider-line { width: 52px; height: 1px; background: #E8A87C; display: inline-block; }
  .bbs-divider-line--long { width: 64px; }

  .bbs-lead { margin: 0; font-family: var(--bbs-display), serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .bbs-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--bbs-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .bbs-cal-link:hover { color: #E8A87C; }

  .bbs-medallion { position: relative; }
  .bbs-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .bbs-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .bbs-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .bbs-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #E8A87C, #FDF1E7, #6E5AA8); filter: saturate(.75); }
  .bbs-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .bbs-medallion-label { font-family: var(--bbs-display), serif; font-size: 26px; line-height: 1; color: #FDF1E7; }
  .bbs-medallion-label-sm { font-family: var(--bbs-display), serif; font-size: 16px; color: #FDF1E7; }
  .bbs-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .bbs-medallion-sub--accent { color: #E8A87C; }
  .bbs-medallion-arc { position: absolute; inset: -14%; }
  ...
  .bbs-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #E8A87C; opacity: 0.4; font-family: var(--bbs-mono), monospace; }

  .bbs-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .bbs-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #FDF1E7, transparent); animation: bbsScan 6s linear infinite; pointer-events: none; }

  .bbs-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .bbs-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .bbs-cd-num { font-family: var(--bbs-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #FDF1E7; display: inline-block; }
  .bbs-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #E8A87C; }
  ...
  .bbs-accent-italic { font-style: italic; color: #E8A87C; }
  .bbs-accent-serif { font-style: italic; color: #A8623A; font-family: var(--bbs-display), serif; }
  .bbs-accent-serif-2 { color: #A8623A; }
  .bbs-h2 { margin: 0; font-family: var(--bbs-display), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .bbs-pan { height: 260vh; position: relative; }
  .bbs-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .bbs-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .bbs-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .bbs-panel--between { justify-content: space-between; }
  ...
  .bbs-pan[data-scroll="vertical"] { height: auto; }
  .bbs-pan[data-scroll="vertical"] .bbs-pan-sticky { position: static; height: auto; overflow: visible; }
  .bbs-pan[data-scroll="vertical"] .bbs-strip { position: static; display: block; width: 100%; transform: none !important; }
  .bbs-pan[data-scroll="vertical"] .bbs-panel { height: calc(var(--vh, 1vh) * 100); }
  .bbs-panel--end { justify-content: flex-end; }
  .bbs-panel--center { align-items: center; justify-content: center; text-align: center; }
  .bbs-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .bbs-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .bbs-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .bbs-panel-title { margin: 0; position: relative; font-family: var(--bbs-display), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .bbs-panel-title-md { margin: 0; position: relative; font-family: var(--bbs-display), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  ...
  .bbs-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #A8623A; margin-top: auto; }
  .bbs-seguir--split { justify-content: space-between; }
  .bbs-side-hint { display: inline-block; animation: bbsSide 2.2s ease-in-out infinite; }
  .bbs-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .bbs-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #A8623A; }

  .bbs-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .bbs-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .bbs-crono-time { font-family: var(--bbs-mono), monospace; color: #A8623A; min-width: 42px; }
  .bbs-crono-title { flex: 1; }

  .bbs-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .bbs-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .bbs-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .bbs-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .bbs-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .bbs-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .bbs-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #FDF1E7, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .bbs-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--bbs-mono), monospace; }
  .bbs-rsvp-rows { display: flex; flex-direction: column; }
  .bbs-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .bbs-rsvp-row > span:first-child { flex-shrink: 0; }
  .bbs-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .bbs-rsvp-row--payment { align-items: flex-start; }
  .bbs-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .bbs-rsvp-payment-total { color: #FDF1E7; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .bbs-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .bbs-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .bbs-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #E8A87C; background: transparent; color: #E8A87C; font-size: 14px; line-height: 1; cursor: pointer; }
  .bbs-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .bbs-rsvp-stepper span { font-family: var(--bbs-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .bbs-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--bbs-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .bbs-rsvp-input::placeholder { color: #6E6A5D; }
  .bbs-rsvp-input:focus { outline: none; border-bottom-color: #E8A87C; }
  .bbs-rsvp-btn { width: 100%; padding: 16px; font-family: var(--bbs-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #E8A87C, #A8623A); border: 1px solid #E8A87C; color: #191420; cursor: pointer; }
  .bbs-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .bbs-rsvp-btn--ghost { background: transparent; color: #E8A87C; }
  .bbs-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #E8A87C; margin: 0; }
  .bbs-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .bbs-rsvp-declined-text { margin: 0; font-family: var(--bbs-display), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .bbs-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .bbs-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .bbs-photo-cell--offset { margin-top: 18px; }
  .bbs-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .bbs-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .bbs-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .bbs-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .bbs-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .bbs-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .bbs-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .bbs-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .bbs-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  ...
  .bbs-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .bbs-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  .bbs-bank-row:last-child { border-bottom: none; }
  .bbs-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .bbs-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .bbs-bank-copy { flex-shrink: 0; font-family: var(--bbs-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #E8A87C; background: transparent; color: #E8A87C; cursor: pointer; }
  .bbs-bank-copy:hover { background: rgba(200,164,92,0.12); }

  .bbs-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .bbs-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: bbsEq 1.1s ease-in-out infinite; display: inline-block; }
  .bbs-song-wrap { font-family: var(--bbs-mono), monospace; }

  .bbs-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .bbs-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  .bbs-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .bbs-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--bbs-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .bbs-song-input::placeholder { color: #6E6A5D; }
  .bbs-song-input:focus { outline: none; border-bottom-color: #E8A87C; }
  .bbs-song-sep { color: #8A8577; flex-shrink: 0; }
  .bbs-song-submit { flex-shrink: 0; background: none; border: none; color: #E8A87C; font-family: var(--bbs-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .bbs-song-submit:hover { color: #FDF1E7; }
  .bbs-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .bbs-song-error { font-size: 10px; color: #E8A87C; margin-top: 6px; }
  .bbs-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .bbs-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--bbs-mono), monospace; }
  .bbs-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .bbs-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .bbs-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .bbs-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .bbs-quiz-q-num { font-family: var(--bbs-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .bbs-quiz-q-text { margin: 0; font-family: var(--bbs-display), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .bbs-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .bbs-quiz-opt { font-family: var(--bbs-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #E8A87C; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .bbs-quiz-opt:disabled { cursor: default; }
  .bbs-quiz-opt--picked { background: #E8A87C; border-color: #E8A87C; color: #191420; }
  .bbs-quiz-opt--correct { background: #E8A87C; border-color: #E8A87C; color: #191420; }
  .bbs-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .bbs-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .bbs-quiz-result-score { margin: 0 0 6px; font-family: var(--bbs-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #FDF1E7; }
  .bbs-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .bbs-final-card { border: 1px solid #E8A87C; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .bbs-final-names { font-family: var(--bbs-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #FDF1E7; }
  .bbs-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .bbs-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .bbs-replay { cursor: pointer; color: #E8A87C; }
  .bbs-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .bbs-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .bbs-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .bbs-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .bbs-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FDF1E7, #E8A87C); transition: height 260ms linear; display: block; }
  .bbs-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #E8A87C; transition: color 500ms ease; }

  .bbs-cover { position: absolute; inset: 0; z-index: 5; }
  ...
  .bbs-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(138,174,208,0.4), rgba(232,168,124,0.32), rgba(232,168,124,0.5), rgba(138,174,208,0.4)); filter: blur(64px); opacity: .62; animation: bbsFoil 26s linear infinite; }
  .bbs-cover-texture { position: absolute; inset: 0; background-image: radial-gradient(rgba(253,241,231,.5) 1.6px, transparent 1.8px); background-size: 34px 34px; }
  ...
  .bbs-cover-names { margin: 0; font-family: var(--bbs-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FDF1E7; }
  .bbs-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#E8A87C, transparent); display: block; }
  ...
  .bbs-cover-cta { border: 1px solid #E8A87C; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--bbs-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .bbs-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .bbs-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #191420; }
  }
  .bbs-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .bbs-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: bbsHint 2.4s ease-in-out infinite; }

  .bbs-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .bbs-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #E8A87C; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .bbs-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
