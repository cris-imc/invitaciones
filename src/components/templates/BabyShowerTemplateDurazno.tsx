  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="bbs-scroller">
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="bbs-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="bbs-pan-sticky">
            <div data-strip="1" className="bbs-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="bbs-panel bbs-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="bbs-seguir bbs-seguir--split">
                    ...
                    <span className="bbs-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <BbsDots count={photoPages.length} />
          </div>
        </div>
...
const GP_CSS = `
  ...
  .bbs-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
