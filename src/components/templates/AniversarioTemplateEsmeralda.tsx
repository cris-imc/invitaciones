        ...
        <section data-tone="dark" data-screen-label="Tu pase" className="anv-section anv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="anv-final-card">
            <div className="anv-medallion anv-medallion--final">
              <AniversarioTemplateEsmeraldaMedallionCmp label="20" sub={confirmed ? "CONFIRMADO" : "PENDIENTE"} arcId="anvArc3" arcText={`${namesTitle.toUpperCase()} · ${fechaCorta} · `} spin="reverse" />
            </div>
...
function AniversarioTemplateEsmeraldaCoverHalf({
  ...
  return (
    <div className="anv-cover-inner">
      ...
      <div className="anv-cover-sunburst" />
      <div className="anv-cover-content">
        ...
        <div className="anv-cover-bottom">
          <div ref={perfRef} className="anv-perf-strip anv-perf-strip--reveal" />
          ...
          <div className="anv-barcode-wrap">
            <div className="anv-barcode" style={{ width: "62%" }} />
            <span className="anv-mini-label" style={{ color: "#56534A" }}>NO TRANSFERIBLE</span>
          </div>
        </div>
      </div>
    </div>
...
const GP_CSS = `
  ...
  .anv-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1A1726 0%, #180F0C 46%, #130C09 100%); }
  .anv-cover-half--bottom .anv-cover-inner { top: auto; bottom: 0; }
  .anv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.7), rgba(78,127,134,0.45), rgba(200,164,92,0.65), rgba(110,90,168,0.7)); filter: blur(64px); opacity: .62; animation: anvFoil 26s linear infinite; }
  .anv-cover-sunburst { position: absolute; inset: 0; background: repeating-conic-gradient(from 0deg at 50% 44%, rgba(232,214,168,0.75) 0deg 0.4deg, transparent 0.4deg 3.2deg); opacity: .3; -webkit-mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); }
  ...
  .anv-cover-names { margin: 0; font-family: var(--anv-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), 12.5vh); line-height: 0.86; letter-spacing: -0.02em; color: #DCF0E4; }
  ...
  .anv-cover-cta { border: 1px solid #4E8C6E; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #DCF0E4; font-family: var(--anv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
