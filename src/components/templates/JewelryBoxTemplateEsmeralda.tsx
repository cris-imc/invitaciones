...
/**
 * JewelryBoxTemplateEsmeralda.tsx (variante de color "Esmeralda")
 *
 * Portado 1:1 desde el diseño "Jewelry Box" (Colección Storytelling, mis
 * quince años, variante "Esmeralda": fondo verde esmeralda oscuro, acento verde + dorado,
...
export function JewelryBoxTemplateEsmeralda({ invitation, guest, isPersonalized = false }: JewelryBoxTemplateProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#5FC38A";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,168,88,.35), 0 18px 50px -30px #5FC38A";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#E3C171" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(200,168,88,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(200,168,88,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#E3C171" : "#5FC38A";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0D2318", tinta: "#F4F1EA", acento: "#5FC38A", clase: `${jwbCormorant.variable} ${jwbMono.variable}`, fuente: "var(--jwb-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
            ...
            <div className="jwb-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="13,35,24" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="13,35,24" />
                </div>
              )}
            </div>
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="jwb-panel jwb-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="jwb-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#E3C171" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#E3C171" />
                  </svg>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#5FC38A" guest={guest} />
      )}
...
function JwbCoverHalf({
  ...
  return (
    <div className="jwb-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#5FC38A" tintColor2="#0D2318" effect="enfoque" scrimColorRgb="13,35,24" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#5FC38A" tintColor2="#0D2318" effect="enfoque" scrimColorRgb="13,35,24" />
        </div>
      )}
...
const JWB_CSS = `
  ...
  a { color: #5FC38A; text-decoration: none; }
  a:hover { color: #E9F7EE; }
  ...
  .jwb-date-month { font-family: var(--jwb-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #5FC38A; padding-left: 12%; }

  .jwb-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .jwb-divider-line { width: 52px; height: 1px; background: #5FC38A; display: inline-block; }
  ...
  .jwb-cal-link:hover { color: #5FC38A; }
  ...
  .jwb-gem { position: absolute; inset: -6px; border-radius: 50%; box-shadow: 0 0 24px 4px rgba(95,195,138,.7); animation: gemFlash 4s ease-in-out infinite; pointer-events: none; }
  .jwb-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2E7A4C, #1A4A2F, #5FC38A, #E9F7EE, #2E7A4C); filter: saturate(.75); }
  ...
  .jwb-medallion-label { font-family: var(--jwb-cormorant), serif; font-size: 26px; line-height: 1; color: #E9F7EE; }
  .jwb-medallion-label-sm { font-family: var(--jwb-cormorant), serif; font-size: 16px; color: #E9F7EE; }
  .jwb-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .jwb-medallion-sub--accent { color: #5FC38A; }
  .jwb-medallion-arc { position: absolute; inset: -14%; }
  .jwb-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #5FC38A; opacity: 0.4; font-family: var(--jwb-mono), monospace; }

  .jwb-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,168,88,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .jwb-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E9F7EE, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .jwb-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .jwb-cd-box { border: 1px solid #1A3322; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .jwb-cd-num { font-family: var(--jwb-cormorant), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F0E6BE; display: inline-block; }
  .jwb-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #5FC38A; }
  .jwb-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0D2318 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .jwb-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(46,122,76,0.4), rgba(227,193,113,0.24), rgba(95,195,138,0.36), rgba(46,122,76,0.4)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .jwb-accent-italic { font-style: italic; color: #5FC38A; }
  .jwb-accent-serif { font-style: italic; color: #E3C171; font-family: var(--jwb-cormorant), serif; }
  .jwb-accent-serif-2 { color: #E3C171; }
  ...
  .jwb-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #E3C171; margin-top: auto; }
  ...
  .jwb-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #E3C171; }
  ...
  .jwb-crono-time { font-family: var(--jwb-mono), monospace; color: #E3C171; min-width: 42px; }
  ...
  .jwb-stub { position: relative; overflow: hidden; border: 1px solid #1A3322; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .jwb-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E9F7EE, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .jwb-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(95,195,138,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .jwb-rsvp-payment-total { color: #E9F7EE; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .jwb-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #5FC38A; background: transparent; color: #5FC38A; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .jwb-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(95,195,138,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .jwb-rsvp-input::placeholder { color: #6E6A5D; }
  .jwb-rsvp-input:focus { outline: none; border-bottom-color: #5FC38A; }
  .jwb-rsvp-btn { width: 100%; padding: 16px; font-family: var(--jwb-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #5FC38A, #E3C171); border: 1px solid #5FC38A; color: #0B0B10; cursor: pointer; }
  .jwb-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .jwb-rsvp-btn--ghost { background: transparent; color: #5FC38A; }
  .jwb-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #5FC38A; margin: 0; }
  ...
  .jwb-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(95,195,138,0.18); }
  ...
  .jwb-bank-copy { flex-shrink: 0; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #5FC38A; background: transparent; color: #5FC38A; cursor: pointer; }
  .jwb-bank-copy:hover { background: rgba(95,195,138,0.12); }
  ...
  .jwb-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(95,195,138,0.3); padding-bottom: 12px; }
  ...
  .jwb-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(95,195,138,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .jwb-song-input::placeholder { color: #6E6A5D; }
  .jwb-song-input:focus { outline: none; border-bottom-color: #5FC38A; }
  .jwb-song-sep { color: #8A8577; flex-shrink: 0; }
  .jwb-song-submit { flex-shrink: 0; background: none; border: none; color: #5FC38A; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .jwb-song-submit:hover { color: #E9F7EE; }
  .jwb-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .jwb-song-error { font-size: 10px; color: #5FC38A; margin-top: 6px; }
  ...
  .jwb-quiz-opt { font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(95,195,138,0.4); background: transparent; color: #5FC38A; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .jwb-quiz-opt:disabled { cursor: default; }
  .jwb-quiz-opt--picked { background: #5FC38A; border-color: #5FC38A; color: #0B0B10; }
  .jwb-quiz-opt--correct { background: #5FC38A; border-color: #5FC38A; color: #0B0B10; }
  .jwb-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .jwb-quiz-result { padding-top: 18px; border-top: 1px solid rgba(95,195,138,0.2); }
  .jwb-quiz-result-score { margin: 0 0 6px; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E9F7EE; }
  .jwb-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .jwb-final-card { border: 1px solid #5FC38A; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .jwb-final-names { font-family: var(--jwb-cormorant), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E9F7EE; }
  ...
  .jwb-replay { cursor: pointer; color: #5FC38A; }
  .jwb-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .jwb-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,168,88,0.14); }
  .jwb-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .jwb-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,168,88,0.16); position: relative; }
  .jwb-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E9F7EE, #5FC38A); transition: height 260ms linear; display: block; }
  .jwb-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #5FC38A; transition: color 500ms ease; }
  ...
  .jwb-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1A4A2F 0%, #143D28 46%, #0D2318 100%); }
  .jwb-cover-half--bottom .jwb-cover-inner { top: auto; bottom: 0; }
  .jwb-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(46,122,76,0.6), rgba(227,193,113,0.35), rgba(95,195,138,0.55), rgba(46,122,76,0.6)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .jwb-cover-names { margin: 0; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E9F7EE; }
  .jwb-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#5FC38A, transparent); display: block; }
  ...
  .jwb-cover-cta { border: 1px solid #5FC38A; background: linear-gradient(100deg, rgba(200,168,88,0.08), rgba(230,214,150,0.2), rgba(200,168,88,0.08)); color: #F0E6BE; font-family: var(--jwb-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .jwb-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .jwb-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(230,214,150,0.34), rgba(240,230,190,0.5), rgba(230,214,150,0.34)); color: #0B0B0F; }
  }
  ...
  .jwb-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #5FC38A; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
