...
/**
 * LiquidGlassTemplateAmatista.tsx
 *
 * Variante de color "Amatista" de LiquidGlassTemplate.tsx (Colección
 * Storytelling, Casamiento). Copia 1:1 de la plantilla base -- único cambio:
 * los hex del fondo oscuro (degradés) y del acento vidrio protagonista por la
 * paleta propia de esta variante. Layout, copy, tipografía, gestos, panel de
 * vidrio esmerilado de la tapa, medallón y estructura idénticos.
 *
 * Portado 1:1 desde el diseño "Liquid Glass — Panorámica" aprobado en Claude
 * Design (panel de acceso de vidrio para boda: fondo casi negro con tinte violeta amatista,
 * acento vidrio violeta amatista #A86FC9, Cormorant Garamond + IBM Plex Mono,
 * medallón circular con texto en arco, tapa con panel de vidrio esmerilado
 * que se aclara al abrir -- `backdrop-filter` animado UNA sola vez sobre un
 * panel decorativo sin descendientes fixed --, scroll horizontal "pineado"
 * para Cuándo y dónde / Álbum, riel lateral de progreso y portada que se
 * abre en dos mitades). Comparte la arquitectura ya probada de
 * `GuestPassVipTemplate` (mismo motor de scroll/reveal, mismos fixes de
 * `--vh`, álbum balanceado + lightbox, `LiveAlbumStrip`, gesto lateral en
 * paneles pineados y lista de canciones acotada) -- todo el motion es CSS +
 * SVG + un loop de scroll propio, cero PNGs.
 *
 * Secciones fijas del producto (Save the Date, Countdown, Frase, Cuándo y
 * dónde, Check-in/RSVP, Álbum, Música, Regalos, Quiz, Footer) reinterpretadas
 * visualmente con el look "vidrio líquido", conectadas a datos reales de
 * `Invitation`/`Guest`.
 *
 ...
 *
 * OJO -- ancestros de `position: fixed` sin filter/backdrop-filter animado:
 * el root wrapper de esta plantilla es `position: fixed; inset: 0` y el
 * lightbox del álbum también es `position: fixed`. El único
 * `backdrop-filter` animado de esta plantilla (`.lqg-cover-glasspane`, el
 * panel de vidrio esmerilado de la tapa) es un div HOJA sin ningún
 * descendiente -- nunca es ancestro de un elemento fixed -- así que animar
 * su blur al montar es seguro (ver comentario en
 * `src/components/layout/PageTransition.tsx` sobre por qué animar
 * filter/backdrop-filter en un ANCESTRO de un fixed rompe su posicionamiento).
 */
...
interface GuestRecord {
  ...
  // backend no reserva mesas/sectores, así que el panel usa este número de
...
// El backend no reserva mesas/sectores -- el panel muestra el orden real del
...
// real que mostrar.
function panelNumberFrom(orderNumber: number | undefined): string {
  if (!orderNumber) return "---";
  return String(orderNumber).padStart(3, "0");
}

export function LiquidGlassTemplateAmatista({ invitation, guest, isPersonalized = false }: LiquidGlassTemplateProps) {
  const tx = useTextos();
  ...
  const coupleInitials =
    `${initialOf(novia)}${initialOf(novio)}` || initialOf(String(invitation.nombreEvento ?? "")) || "—";
  ...
  // Combinación de colores del mockup (ver mockup/Liquid Glass - Panoramica.dc.html):
  // primera mitad de la frase en color plano, segunda mitad en celeste
  // itálico. Antes esto se perdía apenas se cargaba una frase personalizada
  // (quedaba toda pareja); ahora se aplica siempre, partiendo por la mitad
  // de la cantidad real de palabras en vez de un índice fijo pensado solo
  // para la frase default.
  ...
  const panelNumber = panelNumberFrom(guest?.orderNumber);
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#A86FC9";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(168,111,201,.35), 0 18px 50px -30px #A86FC9";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#251C30" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#251C30" : "#A86FC9";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A080D", tinta: "#F4F1EA", acento: "#A86FC9", clase: `${lqgCormorant.variable} ${lqgMono.variable}`, fuente: "var(--lqg-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${lqgCormorant.variable} ${lqgMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#0A080D",
        fontFamily: "var(--lqg-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{LQG_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="lqg-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="lqg-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #14171F 0%, #0D0A11 55%, #0A080D 100%)" }}>
          ...
          <div className="lqg-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="lqg-date-num lqg-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="lqg-divider">
            ...
            <div className="lqg-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="10,8,13" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="10,8,13" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="lqg-section lqg-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #1B1422 0%, #0E0B12 55%, #0A080D 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="lqg-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #171C27 0%, #0B0C11 52%, #0A080D 100%)" }}>
          ...
          <h2 ref={phraseRef} className="lqg-phrase" style={{ fontSize: fraseFontSize }}>
            {fraseWords.map((w, i) => (
              ...
              <span key={i}>
                <span data-w="1" className={i >= fraseAccentStart ? "lqg-accent-italic" : undefined}>
                  {w}
                </span>{" "}
              </span>
            ))}
          </h2>
          <div data-xin="1" data-delay="120" data-dist="90" className="lqg-divider" style={{ position: "relative" }}>
            <span className="lqg-divider-line lqg-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
          </div>
        </section>
        )}
          ...
          <div className="lqg-pan-sticky">
            <div data-strip="1" className="lqg-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="lqg-panel lqg-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <h2 className="lqg-panel-title">
                    ...
                    {ceremoniaDireccion && <><br /><span className="lqg-accent-serif">{ceremoniaDireccion}</span></>}
                  </h2>
              ...
              <div id="details" data-tone="light" className="lqg-panel lqg-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="lqg-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="lqg-crono-row">
                        <span className="lqg-crono-time">{item.time || ""}</span>
                        <span className="lqg-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="lqg-panel lqg-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="lqg-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#251C30" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#251C30" />
                  </svg>
              ...
              <div data-tone="dark" className="lqg-panel lqg-panel--center" style={{ background: "#0D0A11", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="lqg-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #14171F 0%, #0D0A11 60%, #0A080D 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="lqg-rsvp">
              <LqgRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="lqg-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="lqg-pan-sticky">
            <div data-strip="1" className="lqg-strip">
                ...
                <div key={pageIndex} data-tone="light" className="lqg-panel lqg-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="lqg-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`lqg-mosaic-cell${i === 0 ? " lqg-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="lqg-seguir lqg-seguir--split">
                    ...
                    <span className="lqg-accent-navy-solid">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="lqg-section" style={{ background: "#0D0A11" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="lqg-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="lqg-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F5F8F9" : "#A86FC9" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="lqg-song-wrap">
              <LqgSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="lqg-section" style={{ background: "#0D0A11" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="lqg-section" style={{ background: "#0D0A11" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu panel" className="lqg-section lqg-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #14171F 0%, #0D0A11 55%, #0A080D 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="lqg-final-card">
            ...
            <span className="lqg-mini-label" style={{ color: "#A8A292" }}>{fechaCorta} — {hora} H</span>
          ...
          <div className="lqg-footer-credit">
      ...
      <div ref={railRef} className="lqg-rail">
        ...
        <span ref={railLabelRef} className="lqg-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="lqg-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="lqg-cover-half lqg-cover-half--top">
          <CoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="lqg-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="lqg-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#A86FC9" guest={guest} />
      )}
...
function CdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="lqg-cd-box">
      <span ref={refEl} className="lqg-cd-num">—</span>
      <span className="lqg-cd-label">{label}</span>
    </div>
  );
}
...
function Dots({ count = 3 }: { count?: number }) {
  ...
  return (
    <div data-dots="1" className="lqg-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="lqg-dot" />
      ))}
    </div>
  );
}
...
function Medallion({
  ...
  const labelFontSize = compact ? 16 : size ?? 26;
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="lqg-medallion-arc" style={{ animation: spin === "reverse" ? "lqgRingRev 32s linear infinite" : "lqgRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="lqg-medallion-arc-text">
...
function LqgCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="lqg-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="lqg-bank-row-label">{label}</span>
...
// Reimplementación propia del RSVP (mismo patrón que GuestPassVipTemplate):
// filas de datos + el ticket/sello ya existente en la plantilla, en vez del
    ...
    <>
      ...
      <div ref={stubRef} className="lqg-stub">
        ...
        <div ref={sealRef} className="lqg-seal">
          <Medallion label={coupleInitials} sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="lqg-beam" />
        <div className="lqg-barcode lqg-barcode--light" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="lqg-rsvp-error">{error}</p>}
...
function LqgSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="lqg-song">
      ...
      {error && <p className="lqg-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="lqg-song-list">
...
function LqgQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: LqgQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="lqg-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="lqg-quiz-q">
            <span className="lqg-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="lqg-quiz-q-text">{q.pregunta}</p>
            <div className="lqg-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " lqg-quiz-opt--correct";
                  else if (chosen) stateClass = " lqg-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " lqg-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`lqg-quiz-opt${stateClass}`}
...
function LqgInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="lqg-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="lqg-bank-row-label">{label}</span>
    ...
    <div className="lqg-cover-inner">
      ...
      <div className="lqg-cover-content">
        <div className="lqg-cover-top-row">
          <span>{tx("invitacion.pase.numeroPanel", { n: panelNumber }).toUpperCase()}</span><span className="lqg-accent-navy-solid" style={{ color: "#A86FC9" }}>ALL ACCESS</span>
        </div>
        <div className="lqg-cover-center">
          <span ref={kickerRef} className="lqg-cover-kicker">{kickerText}</span>
          <h1 ref={namesRef} className="lqg-cover-names">{namesTitle}</h1>
          <span className="lqg-cover-rule" />
          <span className="lqg-cover-date">{fechaCorta}</span>
        </div>
        <div className="lqg-cover-bottom">
          <div ref={perfRef} className="lqg-perf-strip lqg-perf-strip--reveal lqg-perf-strip--cover" />
          <div className="lqg-cover-facts">
...
// ver mockup/Liquid Glass - Panoramica.dc.html)
// ---------------------------------------------------------------------
const LQG_CSS = `
  .lqg-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .lqg-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #A86FC9; text-decoration: none; }
  a:hover { color: #F5F8F9; }

  @keyframes lqgFoil { to { transform: rotate(360deg); } }
  @keyframes lqgRing { to { transform: rotate(360deg); } }
  @keyframes lqgRingRev { to { transform: rotate(-360deg); } }
  @keyframes lqgScan { 0% { transform: translateY(0); opacity: 0; } 12% { opacity: .85; } 88% { opacity: .85; } 100% { transform: translateY(320px); opacity: 0; } }
  @keyframes lqgEq { 0%,100% { transform: scaleY(.25); } 50% { transform: scaleY(1); } }
  @keyframes lqgHint { 0%,100% { transform: translateY(0); } 50% { transform: translateY(7px); } }
  @keyframes lqgSide { 0%,100% { transform: translateX(0); } 50% { transform: translateX(7px); } }
  @keyframes lqgGlassClear { 0% { backdrop-filter: blur(14px); opacity: .4; } 100% { backdrop-filter: blur(0); opacity: 1; } }
  @media (prefers-reduced-motion: reduce) { .lqg-scroller * { animation: none !important; } }
  ...
  .lqg-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0A080D; }
  ...
  @media (max-width: 767px) {
    .lqg-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .lqg-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(168,111,201,.3); }
    .lqg-hero-photo-kicker { bottom: 40px; }
    .lqg-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .lqg-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .lqg-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .lqg-date-num { font-family: var(--lqg-cormorant), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .lqg-date-num--right { text-align: right; line-height: 0.86; }
  .lqg-date-month { font-family: var(--lqg-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #A86FC9; padding-left: 12%; }

  .lqg-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .lqg-divider-line { width: 52px; height: 1px; background: #A86FC9; display: inline-block; }
  ...
  .lqg-cal-link:hover { color: #A86FC9; }
  ...
  .lqg-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .lqg-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .lqg-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #251C30, #110E16, #A86FC9, #F5F8F9, #251C30); filter: saturate(.75); }
  .lqg-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .lqg-medallion-label { font-family: var(--lqg-cormorant), serif; line-height: 1; color: #F5F8F9; }
  .lqg-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .lqg-medallion-sub--accent { color: #A86FC9; }
  .lqg-medallion-arc { position: absolute; inset: -14%; }
  /* Opacity baja a propósito: el anillo gira sin parar, así que en algún
     momento de su vuelta una letra queda justo al lado de la etiqueta
     central y, al ser el mismo color/fuente, se leen como una sola palabra
     pegada. Atenuado no compite con el texto del centro y se lee como
     textura decorativa del anillo. */
  .lqg-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #A86FC9; opacity: 0.4; font-family: var(--lqg-mono), monospace; }

  .lqg-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .lqg-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F5F8F9, transparent); animation: lqgScan 6s linear infinite; pointer-events: none; }

  .lqg-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .lqg-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .lqg-cd-num { font-family: var(--lqg-cormorant), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6EFDD; display: inline-block; }
  .lqg-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #A86FC9; }
  .lqg-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A080D 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .lqg-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .lqg-perf-strip--cover { opacity: .9; }

  .lqg-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(168,111,201,0.3), rgba(240,232,242,0.2), rgba(168,111,201,0.34), rgba(168,111,201,0.3)); filter: blur(80px); opacity: .4; animation: lqgFoil 30s linear infinite; }
  .lqg-phrase { margin: 0; position: relative; font-family: var(--lqg-cormorant), serif; font-weight: 400; font-size: clamp(50px, 15vw, 96px); line-height: 0.92; letter-spacing: -0.03em; }
  .lqg-accent-italic { font-style: italic; color: #A86FC9; }
  .lqg-accent-serif { font-style: italic; color: #A86FC9; font-family: var(--lqg-cormorant), serif; }
  .lqg-accent-navy { font-style: italic; color: #251C30; font-family: var(--lqg-cormorant), serif; }
  .lqg-accent-navy-solid { color: #251C30; }
  .lqg-h2 { margin: 0; font-family: var(--lqg-cormorant), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .lqg-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .lqg-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .lqg-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .lqg-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .lqg-panel-title { margin: 0; position: relative; font-family: var(--lqg-cormorant), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .lqg-panel-title-md { margin: 0; position: relative; font-family: var(--lqg-cormorant), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .lqg-panel-title-sm { margin: 0; font-family: var(--lqg-cormorant), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .lqg-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .lqg-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .lqg-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  .lqg-facts { position: relative; display: flex; flex-direction: column; gap: 12px; font-size: 11px; letter-spacing: 0.14em; color: #4A473F; }
  .lqg-facts-row { display: flex; justify-content: space-between; border-bottom: 1px solid #D6D1C4; padding-bottom: 10px; }
  .lqg-facts-row--last { border-bottom: none; padding-bottom: 0; }
  .lqg-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #251C30; margin-top: auto; }
  .lqg-seguir--split { justify-content: space-between; }
  .lqg-side-hint { display: inline-block; animation: lqgSide 2.2s ease-in-out infinite; }
  .lqg-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .lqg-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #251C30; }

  .lqg-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .lqg-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .lqg-crono-time { font-family: var(--lqg-mono), monospace; color: #251C30; min-width: 42px; }
  .lqg-crono-title { flex: 1; }

  .lqg-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .lqg-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .lqg-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .lqg-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .lqg-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .lqg-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .lqg-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F5F8F9, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .lqg-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--lqg-mono), monospace; }
  .lqg-rsvp-rows { display: flex; flex-direction: column; }
  .lqg-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(168,111,201,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .lqg-rsvp-row > span:first-child { flex-shrink: 0; }
  .lqg-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .lqg-rsvp-row--payment { align-items: flex-start; }
  .lqg-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .lqg-rsvp-payment-total { color: #F5F8F9; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .lqg-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .lqg-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .lqg-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #A86FC9; background: transparent; color: #A86FC9; font-size: 14px; line-height: 1; cursor: pointer; }
  .lqg-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .lqg-rsvp-stepper span { font-family: var(--lqg-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .lqg-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(168,111,201,0.3); color: #F4F1EA; font-family: var(--lqg-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .lqg-rsvp-input::placeholder { color: #6E6A5D; }
  .lqg-rsvp-input:focus { outline: none; border-bottom-color: #A86FC9; }
  .lqg-rsvp-btn { width: 100%; padding: 16px; font-family: var(--lqg-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #A86FC9, #7A4A96); border: 1px solid #A86FC9; color: #0B0B10; cursor: pointer; }
  .lqg-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .lqg-rsvp-btn--ghost { background: transparent; color: #A86FC9; }
  .lqg-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #A86FC9; margin: 0; }
  .lqg-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .lqg-rsvp-declined-text { margin: 0; font-family: var(--lqg-cormorant), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .lqg-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .lqg-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .lqg-photo-cell--offset { margin-top: 18px; }
  .lqg-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .lqg-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .lqg-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .lqg-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .lqg-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .lqg-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .lqg-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .lqg-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .lqg-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }

  .lqg-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .lqg-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(168,111,201,0.18); }
  .lqg-bank-row:last-child { border-bottom: none; }
  .lqg-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .lqg-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .lqg-bank-copy { flex-shrink: 0; font-family: var(--lqg-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #A86FC9; background: transparent; color: #A86FC9; cursor: pointer; }
  .lqg-bank-copy:hover { background: rgba(168,111,201,0.12); }

  .lqg-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .lqg-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: lqgEq 1.1s ease-in-out infinite; display: inline-block; }
  .lqg-song-wrap { font-family: var(--lqg-mono), monospace; }

  .lqg-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .lqg-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(168,111,201,0.3); padding-bottom: 12px; }
  .lqg-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .lqg-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(168,111,201,0.3); color: #F4F1EA; font-family: var(--lqg-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .lqg-song-input::placeholder { color: #6E6A5D; }
  .lqg-song-input:focus { outline: none; border-bottom-color: #A86FC9; }
  .lqg-song-sep { color: #8A8577; flex-shrink: 0; }
  .lqg-song-submit { flex-shrink: 0; background: none; border: none; color: #A86FC9; font-family: var(--lqg-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .lqg-song-submit:hover { color: #F5F8F9; }
  .lqg-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .lqg-song-error { font-size: 10px; color: #A86FC9; margin-top: 6px; }
  .lqg-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .lqg-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--lqg-mono), monospace; }
  .lqg-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .lqg-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .lqg-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .lqg-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .lqg-quiz-q-num { font-family: var(--lqg-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .lqg-quiz-q-text { margin: 0; font-family: var(--lqg-cormorant), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .lqg-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .lqg-quiz-opt { font-family: var(--lqg-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(168,111,201,0.4); background: transparent; color: #A86FC9; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .lqg-quiz-opt:disabled { cursor: default; }
  .lqg-quiz-opt--picked { background: #A86FC9; border-color: #A86FC9; color: #0B0B10; }
  .lqg-quiz-opt--correct { background: #A86FC9; border-color: #A86FC9; color: #0B0B10; }
  .lqg-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .lqg-quiz-result { padding-top: 18px; border-top: 1px solid rgba(168,111,201,0.2); }
  .lqg-quiz-result-score { margin: 0 0 6px; font-family: var(--lqg-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F5F8F9; }
  .lqg-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .lqg-final-card { border: 1px solid #A86FC9; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .lqg-final-names { font-family: var(--lqg-cormorant), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F5F8F9; }
  .lqg-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .lqg-barcode--light { background: repeating-linear-gradient(90deg, #A8A292 0 2px, transparent 2px 4px, #A8A292 4px 5px, transparent 5px 9px, #A8A292 9px 11px, transparent 11px 14px); height: 28px; }
  .lqg-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .lqg-replay { cursor: pointer; color: #A86FC9; }
  .lqg-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .lqg-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .lqg-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .lqg-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .lqg-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F5F8F9, #A86FC9); transition: height 260ms linear; display: block; }
  .lqg-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #A86FC9; transition: color 500ms ease; }

  .lqg-cover { position: absolute; inset: 0; z-index: 5; }
  .lqg-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .lqg-cover-half--top { top: 0; }
  .lqg-cover-half--bottom { bottom: 0; }
  .lqg-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #110E16 0%, #0D0A11 46%, #0A080D 100%); }
  .lqg-cover-half--bottom .lqg-cover-inner { top: auto; bottom: 0; }
  .lqg-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(168,111,201,0.4), rgba(240,232,242,0.3), rgba(168,111,201,0.5), rgba(168,111,201,0.4)); filter: blur(64px); opacity: .62; animation: lqgFoil 26s linear infinite; }
  /* Panel de vidrio esmerilado -- el motivo decorativo propio de la tapa de
     esta plantilla (reemplaza el sunburst dorado de otras familias). Es una
     hoja SIN descendientes: animar su backdrop-filter una sola vez al
     montar (glassClear) es seguro porque nunca es ancestro de un
     position:fixed (ver comentario arriba del componente). */
  .lqg-cover-glasspane { position: absolute; inset: 20px; border-radius: 20px; background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.24); backdrop-filter: blur(3px); animation: lqgGlassClear 1s ease both; }
  .lqg-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .lqg-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .lqg-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .lqg-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .lqg-cover-names { margin: 0; font-family: var(--lqg-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F5F8F9; }
  .lqg-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#A86FC9, transparent); display: block; }
  .lqg-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .lqg-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .lqg-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  .lqg-cover-cta { border: 1px solid #A86FC9; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--lqg-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .lqg-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .lqg-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0B0B0F; }
  }
  .lqg-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .lqg-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: lqgHint 2.4s ease-in-out infinite; }

  .lqg-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,10,13,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .lqg-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #A86FC9; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .lqg-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
