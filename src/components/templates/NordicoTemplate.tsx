 ...
 * grafito cálido (#5B5850, variable por color), tipografía grotesca
...
interface NordicoTemplateProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#5B5850]/25 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#5B5850] uppercase tracking-wider mb-0.5">{label}</span>
          ...
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#5B5850] shrink-0" />
...
export function NordicoTemplate({ invitation, guest, isPersonalized = false }: NordicoTemplateProps) {
  const tx = useTextos();
  ...
  // #111111, sin acento de color fuerte) -> effect="geometric" sin tinte
    ...
    >
      <style>{`
        ...
        #banco .copy-btn.copied {
          background-color: #5B5850 !important;
          color: #FFFFFF !important;
        }
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#5B5850]/40 shadow-md ${isTicketMaximized ? 'bg-[#FFFFFF]/95 backdrop-blur-md w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#111111]/95 backdrop-blur-md px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#5B5850] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#5B5850] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#5B5850]/20 pl-3">
      ...
      <aside className="d-left hide-mobile" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#5B5850', margin: '6px 0 14px', animation: 'nordico-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#FFFFFF]">
          ...
          <div className="px-8 pt-16 pb-12 text-left bg-[#FFFFFF] z-10 relative">
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#5B5850] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#111111] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#5B5850' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#5B5850', margin: '0 0 20px', animation: 'nordico-lineExpand 1.2s ease-out' }} />
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#5B5850] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#FFFFFF]" aria-hidden="true">
          ...
          <IconLineDoodle className="nordico-scroll-doodle opacity-0" style={{ width: 28, height: 6, color: '#5B5850' }} />
          <div style={{ width: 40, height: 1, background: 'rgba(17,17,17,0.18)' }} />
        </div>
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#FFFFFF] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="border border-[#111111]/12 border-l-2 border-l-[#5B5850] p-6 sm:p-8 mb-6">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#5B5850] hover:text-[#111111] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="border border-[#111111]/12 border-l-2 border-l-[#5B5850] p-6 sm:p-8 mb-10">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#6E6E6E] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#5B5850] hover:text-[#111111] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#FFFFFF' }} aria-hidden="true">
          ...
          <IconRegisterMark className="nordico-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#5B5850' }} />
          <div style={{ width: 28, height: 1, background: 'rgba(17,17,17,0.15)' }} />
        </div>
