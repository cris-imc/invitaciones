  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#10130A", tinta: "#F4F1EA", acento: "#9CAA5E", clase: `${vtePlayfair.variable} ${vteMono.variable}`, fuente: "var(--vte-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="vte-scroller">
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="vte-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1D2716 0%, #0B0F09 52%, #10130A 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="vte-section" style={{ background: "#1A2010" }}>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="vte-section" style={{ background: "#1A2010" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.sabor.tuEdicion")} className="vte-section vte-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #141A0F 0%, #1A2010 55%, #10130A 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#9CAA5E" guest={guest} />
      )}
...
const VTE_CSS = `
  ...
  .vte-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #10130A; }
  ...
  @media (min-width: 768px) {
    .vte-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(156,170,94,.3); }
  ...
  .vte-cal-link:hover { color: #9CAA5E; }
  ...
  .vte-cover-names { margin: 0; font-family: var(--vte-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E8E9C6; }
