...
/**
 * AcrylicPopTemplateSunset.tsx
 *
 * Portado 1:1 desde el diseño "Acrylic Pop" (Colección Storytelling, mis
 * quince años: fondo tinta casi negra, acento neón naranja/violeta/verde lima estilo
...
// Ciclo de acento naranja/violeta/verde lima (variante "Atardecer Ácido") del mockup para las palabras destacadas de
// la frase (mismo orden en que aparecen: prende=naranja, una=violeta, sola=verde lima,
// vez.=fucsia otra vez).
const PHRASE_ACCENTS = ["#FF7A29", "#B84DFF", "#D6FF3D"];
...
interface AcrylicPopTemplateSunsetProps {
...
export function AcrylicPopTemplateSunset({ invitation, guest, isPersonalized = false }: AcrylicPopTemplateSunsetProps) {
  const tx = useTextos();
  ...
  // plano, segunda mitad itálica ciclando naranja/violeta/verde lima (mismo orden que
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#D6FF3D";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#FF7A29";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #FF7A29";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#B84DFF" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#B84DFF" : "#FF7A29";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#140C08", tinta: "#F4F1EA", acento: "#FF7A29", clase: `${acpPoppins.variable} ${acpMono.variable}`, fuente: "var(--acp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="acp-scroller">
            ...
            <div className="acp-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="20,12,8" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="20,12,8" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="acp-section acp-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #291708 0%, #1A100A 55%, #140C08 100%)" }}>
          ...
          <div className="acp-cd-grid">
            ...
            <AcpCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} numColor="#D6FF3D" />
          </div>
          <div className="acp-perf-strip" />
        </section>

        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="acp-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #2A1810 0%, #170D09 52%, #140C08 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="acp-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="acp-pan-sticky">
            <div data-strip="1" className="acp-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="acp-panel acp-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="acp-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#B84DFF" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#B84DFF" />
                  </svg>
              ...
              <div data-tone="dark" className="acp-panel acp-panel--center" style={{ background: "#1C1109", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="acp-section" style={{ background: "#1C1109" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="acp-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="acp-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#D6FF3D" : "#FF7A29" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="acp-section" style={{ background: "#1C1109" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="acp-section acp-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #241407 0%, #1C1109 55%, #140C08 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#FF7A29" guest={guest} />
      )}
...
const ACP_CSS = `
  ...
  .acp-date-month { font-family: var(--acp-poppins), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #FF7A29; padding-left: 12%; }

  .acp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .acp-divider-line { width: 52px; height: 1px; background: #FF7A29; display: inline-block; }
  ...
  .acp-cal-link:hover { color: #FF7A29; }
  ...
  .acp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #FF7A29, #B84DFF, #FF7A29, #D6FF3D, #FF7A29); filter: saturate(.75); }
  .acp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #140B08; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .acp-medallion-num { font-family: var(--acp-poppins), sans-serif; font-size: 26px; line-height: 1; color: #D6FF3D; }
  .acp-medallion-num-sm { font-family: var(--acp-poppins), sans-serif; font-size: 16px; line-height: 1; color: #D6FF3D; }
  .acp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .acp-medallion-arc { position: absolute; inset: -14%; }
  .acp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #FF7A29; font-family: var(--acp-mono), monospace; }
  ...
  .acp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #D6FF3D, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .acp-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #FF7A29; }
  .acp-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #140C08 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .acp-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(255,122,41,0.32), rgba(184,77,255,0.26), rgba(214,255,61,0.34), rgba(255,122,41,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .acp-accent-serif { font-style: italic; color: #B84DFF; font-family: var(--acp-poppins), sans-serif; }
  .acp-accent-serif-2 { color: #B84DFF; }
  ...
  .acp-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #B84DFF; margin-top: auto; }
  ...
  .acp-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #B84DFF; }
  ...
  .acp-crono-time { font-family: var(--acp-mono), monospace; color: #B84DFF; min-width: 42px; }
  ...
  .acp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #D6FF3D, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .acp-rsvp-payment-total { color: #D6FF3D; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .acp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #FF7A29; background: transparent; color: #FF7A29; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .acp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(255,122,41,0.3); color: #F4F1EA; font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .acp-rsvp-input::placeholder { color: #6E6A5D; }
  .acp-rsvp-input:focus { outline: none; border-bottom-color: #FF7A29; }
  .acp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--acp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); border: 1px solid #FF7A29; color: #F6EFDD; cursor: pointer; }
  .acp-rsvp-btn:hover:not(:disabled) { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #130B07; }
  .acp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .acp-rsvp-btn--ghost { background: transparent; color: #FF7A29; }
  .acp-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(255,122,41,0.1); color: #FF7A29; }
  .acp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #FF7A29; margin: 0; }
  ...
  .acp-bank-copy { flex-shrink: 0; font-family: var(--acp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #FF7A29; background: transparent; color: #FF7A29; cursor: pointer; }
  .acp-bank-copy:hover { background: rgba(255,122,41,0.12); }
  ...
  .acp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(255,122,41,0.3); color: #F4F1EA; font-family: var(--acp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .acp-song-input::placeholder { color: #6E6A5D; }
  .acp-song-input:focus { outline: none; border-bottom-color: #FF7A29; }
  .acp-song-sep { color: #8A8577; flex-shrink: 0; }
  .acp-song-submit { flex-shrink: 0; background: none; border: none; color: #FF7A29; font-family: var(--acp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .acp-song-submit:hover { color: #D6FF3D; }
  .acp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .acp-song-error { font-size: 10px; color: #FF7A29; margin-top: 6px; }
  ...
  .acp-quiz-opt { font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(255,122,41,0.4); background: transparent; color: #FF7A29; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .acp-quiz-opt:disabled { cursor: default; }
  .acp-quiz-opt--picked { background: #FF7A29; border-color: #FF7A29; color: #130B07; }
  .acp-quiz-opt--correct { background: #D6FF3D; border-color: #D6FF3D; color: #130B07; }
  .acp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .acp-quiz-result { padding-top: 18px; border-top: 1px solid #2A2A33; }
  .acp-quiz-result-score { margin: 0 0 6px; font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #D6FF3D; }
  .acp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .acp-final-card { border: 1px solid #FF7A29; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .acp-replay { cursor: pointer; color: #FF7A29; }
  ...
  .acp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#D6FF3D, #FF7A29); transition: height 260ms linear; display: block; }
  .acp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #FF7A29; transition: color 500ms ease; }
  ...
  .acp-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #22130C 0%, #1C1109 46%, #140C08 100%); }
  .acp-cover-half--bottom .acp-cover-inner { top: auto; bottom: 0; }
  .acp-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(255,122,41,0.5), rgba(184,77,255,0.4), rgba(214,255,61,0.5), rgba(255,122,41,0.5)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  /* Manchas de luz suaves (ver mockup original) -- propias de Acrylic Pop:
     ...
     plantillas de la colección. */
  .acp-cover-spots { position: absolute; inset: 0; background-image: radial-gradient(circle at 20% 20%, rgba(255,122,41,.25), transparent 40%), radial-gradient(circle at 80% 70%, rgba(184,77,255,.22), transparent 40%), radial-gradient(circle at 50% 90%, rgba(214,255,61,.18), transparent 35%); }
  ...
  .acp-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#FF7A29, transparent); display: block; }
  ...
  .acp-cover-cta { border: 1px solid #FF7A29; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--acp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .acp-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .acp-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #130B07; }
  }
  ...
  .acp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #FF7A29; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
