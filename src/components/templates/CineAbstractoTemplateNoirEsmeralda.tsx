...
/**
 * CineAbstractoTemplateNoirEsmeralda.tsx
 *
 * Variante de color "Noir Verde Esmeralda" de la familia Cine Abstracto (Casamiento).
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        ...
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(63,156,116,.35), 0 18px 50px -30px #3F9C74";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#1F4A3B" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(63,156,116,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(63,156,116,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#1F4A3B" : "#3F9C74";
        }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="cab-scroller">
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="cab-section cab-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #13221A 0%, #0D0B10 55%, #08080B 100%)" }}>
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cab-panel cab-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cab-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#1F4A3B" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#1F4A3B" />
                  </svg>
...
const CAB_CSS = `
  ...
  @media (min-width: 768px) {
    .cab-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(63,156,116,.3); }
  ...
  .cab-date-month { font-family: var(--cab-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #3F9C74; padding-left: 12%; }

  .cab-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cab-divider-line { width: 52px; height: 1px; background: #3F9C74; display: inline-block; }
  ...
  .cab-cal-link:hover { color: #3F9C74; }
  ...
  .cab-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: repeating-conic-gradient(#0C0C11 0deg 7deg, #3F9C74 7deg 10deg, #0C0C11 10deg 16deg, #CFEEDD 16deg 19deg); }
  ...
  .cab-medallion-label { font-family: var(--cab-serif), serif; font-size: 26px; line-height: 1; color: #CFEEDD; }
  .cab-medallion-label-sm { font-family: var(--cab-serif), serif; font-size: 16px; color: #CFEEDD; }
  .cab-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .cab-medallion-sub--accent { color: #3F9C74; }
  ...
  .cab-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #3F9C74; opacity: 0.4; font-family: var(--cab-mono), monospace; }

  .cab-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(63,156,116,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .cab-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #CFEEDD, transparent); animation: cabScan 6s linear infinite; pointer-events: none; }

  .cab-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .cab-cd-box { border: 1px solid #1E3A2E; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  ...
  .cab-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #3F9C74; }
  ...
  .cab-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(56,138,102,0.5), rgba(18,38,30,0.36), rgba(63,156,116,0.46), rgba(56,138,102,0.5)); filter: blur(80px); opacity: .4; animation: cabFoil 30s linear infinite; }
  ...
  .cab-accent-italic { font-style: italic; color: #3F9C74; }
  .cab-accent-serif { font-style: italic; color: #1F4A3B; font-family: var(--cab-serif), serif; }
  .cab-accent-serif-2 { color: #1F4A3B; }
  ...
  .cab-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #1F4A3B; margin-top: auto; }
  ...
  .cab-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #1F4A3B; }
  ...
  .cab-crono-time { font-family: var(--cab-mono), monospace; color: #1F4A3B; min-width: 42px; }
  ...
  .cab-stub { position: relative; overflow: hidden; border: 1px solid #1E3A2E; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cab-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #CFEEDD, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .cab-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(63,156,116,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cab-rsvp-payment-total { color: #CFEEDD; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .cab-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #3F9C74; background: transparent; color: #3F9C74; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cab-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(63,156,116,0.3); color: #F4F1EA; font-family: var(--cab-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cab-rsvp-input::placeholder { color: #6E6A5D; }
  .cab-rsvp-input:focus { outline: none; border-bottom-color: #3F9C74; }
  .cab-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cab-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #3F9C74, #1F4A3B); border: 1px solid #3F9C74; color: #0B0B10; cursor: pointer; }
  .cab-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cab-rsvp-btn--ghost { background: transparent; color: #3F9C74; }
  .cab-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #3F9C74; margin: 0; }
  ...
  .cab-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(63,156,116,0.18); }
  ...
  .cab-bank-copy { flex-shrink: 0; font-family: var(--cab-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #3F9C74; background: transparent; color: #3F9C74; cursor: pointer; }
  .cab-bank-copy:hover { background: rgba(63,156,116,0.12); }
  ...
  .cab-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(63,156,116,0.3); padding-bottom: 12px; }
  ...
  .cab-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(63,156,116,0.3); color: #F4F1EA; font-family: var(--cab-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cab-song-input::placeholder { color: #6E6A5D; }
  .cab-song-input:focus { outline: none; border-bottom-color: #3F9C74; }
  .cab-song-sep { color: #8A8577; flex-shrink: 0; }
  .cab-song-submit { flex-shrink: 0; background: none; border: none; color: #3F9C74; font-family: var(--cab-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cab-song-submit:hover { color: #CFEEDD; }
  .cab-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cab-song-error { font-size: 10px; color: #3F9C74; margin-top: 6px; }
  ...
  .cab-quiz-opt { font-family: var(--cab-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(63,156,116,0.4); background: transparent; color: #3F9C74; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cab-quiz-opt:disabled { cursor: default; }
  .cab-quiz-opt--picked { background: #3F9C74; border-color: #3F9C74; color: #0B0B10; }
  .cab-quiz-opt--correct { background: #3F9C74; border-color: #3F9C74; color: #0B0B10; }
  .cab-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cab-quiz-result { padding-top: 18px; border-top: 1px solid rgba(63,156,116,0.2); }
  .cab-quiz-result-score { margin: 0 0 6px; font-family: var(--cab-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #CFEEDD; }
  .cab-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .cab-final-card { border: 1px solid #3F9C74; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .cab-replay { cursor: pointer; color: #3F9C74; }
  .cab-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cab-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(63,156,116,0.14); }
  .cab-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cab-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(63,156,116,0.16); position: relative; }
  .cab-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#CFEEDD, #3F9C74); transition: height 260ms linear; display: block; }
  .cab-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #3F9C74; transition: color 500ms ease; }
  ...
  .cab-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #0F1D17 0%, #0B0B10 46%, #08080B 100%); }
  .cab-cover-half--bottom .cab-cover-inner { top: auto; bottom: 0; }
  .cab-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(56,138,102,0.7), rgba(18,38,30,0.5), rgba(63,156,116,0.65), rgba(56,138,102,0.7)); filter: blur(64px); opacity: .62; animation: cabFoil 26s linear infinite; }
  ...
  .cab-cover-grain { position: absolute; inset: 0; background-image: radial-gradient(rgba(207,238,221,.09) .6px, transparent .6px); background-size: 3px 3px; mix-blend-mode: screen; animation: cabGrainFlicker 2.4s steps(6) infinite; }
  ...
  .cab-cover-sprockets { position: absolute; left: 0; right: 0; height: 7px; background-image: repeating-linear-gradient(90deg, rgba(207,238,221,.65) 0 7px, transparent 7px 18px); }
  ...
  .cab-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#3F9C74, transparent); display: block; }
  ...
  .cab-cover-cta { border: 1px solid #3F9C74; background: linear-gradient(100deg, rgba(63,156,116,0.08), rgba(207,238,221,0.2), rgba(63,156,116,0.08)); color: #F4EEE6; font-family: var(--cab-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cab-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cab-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(207,238,221,0.34), rgba(207,238,221,0.55), rgba(207,238,221,0.34)); color: #0B0B0F; }
  }
  ...
  .cab-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #3F9C74; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
