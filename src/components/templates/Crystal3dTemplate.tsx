...
export function Crystal3dTemplate({ invitation, guest, isPersonalized = false }: Crystal3dTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F0F4F8";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#7FD1E0";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(127,209,224,.35), 0 18px 50px -30px #7FD1E0";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#C9A9F5" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(127,209,224,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(127,209,224,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#C9A9F5" : "#7FD1E0";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#080B10", tinta: "#F4F1EA", acento: "#C9A9F5", clase: `${c3dPoppins.variable} ${c3dMono.variable}`, fuente: "var(--c3d-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="c3d-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="c3d-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #0A0E14 55%, #080B10 100%)" }}>
            ...
            <div className="c3d-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="8,11,16" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="8,11,16" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="c3d-section c3d-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #151E2A 0%, #0B0F16 55%, #080B10 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="c3d-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #080B10 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="c3d-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="c3d-pan-sticky">
            <div data-strip="1" className="c3d-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="c3d-panel c3d-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="c3d-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#C9A9F5" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#C9A9F5" />
                  </svg>
              ...
              <div data-tone="dark" className="c3d-panel c3d-panel--center" style={{ background: "#0A0E14", color: "#F4F1EA" }}>
                <div className="c3d-medallion c3d-medallion--lg">
                  <C3dMedallion
                    ringGradient="conic-gradient(from 90deg, #7FD1E0, #F0F4F8, #C9A9F5, #7FD1E0, #7FD1E0)"
                    topLabel="ACCESO VIP"
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="c3d-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #0A0E14 60%, #080B10 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="c3d-section" style={{ background: "#0A0E14" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="c3d-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="c3d-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F0F4F8" : "#7FD1E0" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="c3d-section" style={{ background: "#0A0E14" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="c3d-section" style={{ background: "#0A0E14" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="c3d-section c3d-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #0A0E14 55%, #080B10 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="c3d-final-card">
            <div className="c3d-medallion c3d-medallion--final">
              <C3dMedallion
                ringGradient="conic-gradient(from 140deg, #C9A9F5, #7FD1E0, #7FD1E0, #F0F4F8, #C9A9F5)"
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#7FD1E0" guest={guest} />
      )}
    ...
    <>
      ...
      <div ref={stubRef} className="c3d-stub">
        ...
        <div ref={sealRef} className="c3d-seal">
          <C3dMedallion ringGradient="conic-gradient(from 0deg, #C9A9F5, #7FD1E0, #F0F4F8, #C9A9F5)" sub="" arcId="" arcText="" compact />
        </div>
...
function C3dCoverHalf({
  ...
  return (
    <div className="c3d-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#7FD1E0" tintColor2="#080B10" effect="enfoque" scrimColorRgb="8,11,16" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#7FD1E0" tintColor2="#080B10" effect="enfoque" scrimColorRgb="8,11,16" />
        </div>
      )}
...
const C3D_CSS = `
  ...
  a { color: #C9A9F5; text-decoration: none; }
  a:hover { color: #F0F4F8; }
  ...
  @media (min-width: 768px) { .c3d-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(127,209,224,.3); } }
  ...
  .c3d-date-month { font-family: var(--c3d-poppins), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #7FD1E0; padding-left: 12%; }

  .c3d-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .c3d-divider-line { width: 52px; height: 1px; background: #7FD1E0; display: inline-block; }
  ...
  .c3d-medallion-num { font-family: var(--c3d-poppins), sans-serif; font-size: 30px; line-height: 1; color: #F0F4F8; }
  .c3d-medallion-num-sm { font-family: var(--c3d-poppins), sans-serif; font-size: 16px; line-height: 1; color: #F0F4F8; }
  .c3d-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .c3d-medallion-sub--cyan { color: #7FD1E0; }
  .c3d-medallion-sub--lavender { color: #C9A9F5; }
  .c3d-medallion-arc { position: absolute; inset: -14%; }
  .c3d-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #7FD1E0; font-family: var(--c3d-mono), monospace; }

  .c3d-facet-gem { position: absolute; inset: 0; clip-path: polygon(50% 0%, 95% 30%, 80% 90%, 20% 90%, 5% 30%); background: linear-gradient(135deg, #7FD1E0, #C9A9F5 55%, #F0F4F8); filter: saturate(.9); animation: facetSpin 6s ease-in-out infinite; }
  ...
  .c3d-facet-num { font-family: var(--c3d-poppins), sans-serif; font-size: 26px; line-height: 1; color: #F0F4F8; }
  ...
  .c3d-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F0F4F8, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .c3d-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #7FD1E0; }
  .c3d-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #080B10 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .c3d-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(201,169,245,0.32), rgba(127,209,224,0.26), rgba(127,209,224,0.36), rgba(201,169,245,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .c3d-accent-italic-cyan { font-style: italic; color: #7FD1E0; }
  .c3d-accent-italic-lavender { font-style: italic; color: #C9A9F5; }
  .c3d-accent-cyan { color: #7FD1E0; }
  .c3d-accent-lavender { color: #C9A9F5; }
  ...
  .c3d-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #C9A9F5; margin-top: auto; }
  ...
  .c3d-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #C9A9F5; }
  ...
  .c3d-crono-time { font-family: var(--c3d-mono), monospace; color: #7A6FA8; min-width: 42px; }
  ...
  .c3d-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F0F4F8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .c3d-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(127,209,224,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .c3d-rsvp-payment-total { color: #F0F4F8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .c3d-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #7FD1E0; background: transparent; color: #7FD1E0; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .c3d-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(127,209,224,0.3); color: #F4F1EA; font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .c3d-rsvp-input::placeholder { color: #6E6A5D; }
  .c3d-rsvp-input:focus { outline: none; border-bottom-color: #7FD1E0; }
  .c3d-rsvp-btn { width: 100%; padding: 16px; font-family: var(--c3d-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #7FD1E0, #3E8FA0); border: 1px solid #7FD1E0; color: #0B0B10; cursor: pointer; }
  .c3d-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .c3d-rsvp-btn--ghost { background: transparent; color: #7FD1E0; }
  .c3d-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #7FD1E0; margin: 0; }
  ...
  .c3d-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(127,209,224,0.18); }
  ...
  .c3d-bank-copy { flex-shrink: 0; font-family: var(--c3d-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #7FD1E0; background: transparent; color: #7FD1E0; cursor: pointer; }
  .c3d-bank-copy:hover { background: rgba(127,209,224,0.12); }
  ...
  .c3d-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(127,209,224,0.3); padding-bottom: 12px; }
  ...
  .c3d-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(127,209,224,0.3); color: #F4F1EA; font-family: var(--c3d-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .c3d-song-input::placeholder { color: #6E6A5D; }
  .c3d-song-input:focus { outline: none; border-bottom-color: #7FD1E0; }
  .c3d-song-sep { color: #8A8577; flex-shrink: 0; }
  .c3d-song-submit { flex-shrink: 0; background: none; border: none; color: #7FD1E0; font-family: var(--c3d-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .c3d-song-submit:hover { color: #F0F4F8; }
  .c3d-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .c3d-song-error { font-size: 10px; color: #7FD1E0; margin-top: 6px; }
  ...
  .c3d-quiz-opt { font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(127,209,224,0.4); background: transparent; color: #7FD1E0; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .c3d-quiz-opt:disabled { cursor: default; }
  .c3d-quiz-opt--picked { background: #7FD1E0; border-color: #7FD1E0; color: #0B0B10; }
  .c3d-quiz-opt--correct { background: #7FD1E0; border-color: #7FD1E0; color: #0B0B10; }
  .c3d-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .c3d-quiz-result { padding-top: 18px; border-top: 1px solid rgba(127,209,224,0.2); }
  .c3d-quiz-result-score { margin: 0 0 6px; font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F0F4F8; }
  .c3d-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .c3d-final-card { border: 1px solid #7FD1E0; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .c3d-final-names { font-family: var(--c3d-poppins), sans-serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F0F4F8; }
  ...
  .c3d-replay { cursor: pointer; color: #7FD1E0; }
  .c3d-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .c3d-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(127,209,224,0.14); }
  .c3d-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .c3d-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(127,209,224,0.16); position: relative; }
  .c3d-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F0F4F8, #7FD1E0); transition: height 260ms linear; display: block; }
  .c3d-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #7FD1E0; transition: color 500ms ease; }
  ...
  .c3d-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #111A26 0%, #0A0E14 46%, #080B10 100%); }
  ...
  .c3d-cover-facets { position: absolute; inset: 0; background-image: repeating-linear-gradient(64deg, rgba(240,244,248,.08) 0 1px, transparent 1px 34px), repeating-linear-gradient(-64deg, rgba(201,169,245,.08) 0 1px, transparent 1px 34px); }
  ...
  .c3d-cover-names { margin: 0; font-family: var(--c3d-poppins), sans-serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F0F4F8; }
  .c3d-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#7FD1E0, transparent); display: block; }
  ...
  .c3d-cover-cta { border: 1px solid #7FD1E0; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--c3d-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .c3d-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #7FD1E0; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
