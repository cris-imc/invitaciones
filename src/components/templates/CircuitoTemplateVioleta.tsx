/**
 * CircuitoTemplateVioleta.tsx
 * Derivado de ModernoTemplate.tsx (misma arquitectura que NeonTemplate.tsx /
 * ChicTemplate.tsx): misma estructura, props, secciones y componentes
 * reutilizados (Countdown, AlbumCarousel, RSVPWizardV2, SongSuggestion,
 * etc). Cambia la capa visual a la estética "Circuito" del pack Tech 15
 * años (mockup/nuevo/Plantillas 15 Años Tech.dc.html, tarjeta Zoe/Mora):
 * negro absoluto (#08080A) + acentos verde terminal (#B14EFF) y magenta
 * (#39FFD0), UI cyber/dark 100% monoespaciada (Orbitron para títulos +
 * JetBrains Mono para cuerpo/cifras), doodles de trazo fino propios (hex de
 * circuito, nodo/via de PCB), scanlines animadas, glitch ambiente en el
 * hero y una cuenta regresiva con estética de terminal.
 * Gating: pensada para QUINCE_ANOS/CUMPLEANOS (pack "Tech 15 años") — el
 * gating real vive en TemplatePreviewModal.tsx, este archivo no valida nada
 * por su cuenta.
 */
"use client";
...
// decorativo (placa, hexágonos, nodos/vías soldadas).
...
// Vinilo/disco -- el mismo ícono base que usa el resto de las plantillas
// para "Sugerí una canción" (no es un doodle propio del tema, es utilitario).
...
// Hexágono de circuito doodle -- contorno hexagonal (chip/IC) con una traza
// interna en zigzag, como una pista de PCB. Decorativa, usada en portada/hero.
const IconHex = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 40 40" fill="none" stroke="currentColor" strokeWidth={1.1} className={className} style={style} aria-hidden="true">
    <path d="M20 4 34.6 12v16L20 36 5.4 28V12Z" />
    <path d="M13 16h5l2 4h5" strokeWidth={0.9} opacity={0.8} />
    <circle cx="13" cy="16" r="1.3" />
    <circle cx="25" cy="20" r="1.3" />
  </svg>
);
// Nodo/vía de PCB doodle -- un punto de soldadura con cuatro pistas
// saliendo en cruz, como un "via" de placa. Acompaña títulos y CTAs.
const IconNode = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className={className} style={style} aria-hidden="true">
    <path d="M12 3v4M12 17v4M3 12h4M17 12h4" />
    <rect x="9" y="9" width="6" height="6" />
  </svg>
);

// Marco completo alrededor de la foto de portada/hero: borde fino verde
// terminal en todo el perímetro -- no solo esquinas -- con marcas de
// medición tipo HUD en las esquinas y un doodle de nodo magenta.
const PhotoCornerFrame = () => (
  <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
    <div className="absolute inset-0" style={{ border: "1px solid rgba(177,78,255,0.55)" }} />
    <div className="absolute" style={{ top: -1, left: -1, width: 14, height: 14, borderTop: "2px solid #B14EFF", borderLeft: "2px solid #B14EFF" }} />
    <div className="absolute" style={{ top: -1, right: -1, width: 14, height: 14, borderTop: "2px solid #B14EFF", borderRight: "2px solid #B14EFF" }} />
    <div className="absolute" style={{ bottom: -1, left: -1, width: 14, height: 14, borderBottom: "2px solid #B14EFF", borderLeft: "2px solid #B14EFF" }} />
    <div className="absolute" style={{ bottom: -1, right: -1, width: 14, height: 14, borderBottom: "2px solid #B14EFF", borderRight: "2px solid #B14EFF" }} />
    <IconNode style={{ position: "absolute", bottom: -9, right: -9, width: 16, height: 16, color: "#39FFD0" }} />
  </div>
);

// Tipografía exacta del mockup "Circuito" (Orbitron para títulos/kickers
// display + JetBrains Mono para todo el cuerpo de texto y cifras, estética
// 100% terminal), escopeada solo a este componente vía CSS var override en
...
interface CircuitoTemplateVioletaProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#B14EFF]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#B14EFF] uppercase tracking-wider mb-0.5">{label}</span>
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        ...
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#E9FFF6" }}>
          RESPONDISTE {score} DE {preguntas.length} CORRECTAMENTE ({percent}%)
        </p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#7C8C88" }}>Guardando tus resultados...</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#B14EFF] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#E9FFF6" }}>
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#E9FFF6] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function CircuitoTemplateVioleta({ invitation, guest, isPersonalized = false }: CircuitoTemplateVioletaProps) {
  ...
  // Entrada animada de los doodles de la portada (hexágonos de circuito,
  // nodos de PCB) y pulso del sello con anime.js. Corre una sola vez, cuando
  // la portada aparece (isCoverOpen pasa a false al montar, así que esto
  // dispara en el primer render real).
  const coverRootRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (isCoverOpen || !coverRootRef.current) return;
    const root = coverRootRef.current;
    animate(root.querySelectorAll(".circ-doodle"), {
      scale: [0, 1],
      rotate: [-25, 0],
      opacity: [0, 1],
      duration: 900,
    ...
    animate(root.querySelectorAll(".circ-seal"), {
      scale: [0.6, 1],
      opacity: [0, 1],
      duration: 700,
      delay: 150,
  ...
  useEffect(() => {
    ...
    const els = document.querySelectorAll(".circ-scroll-doodle");
  ...
  // Barrido de "escaneo de datos" (línea horizontal verde) sobre la foto de
  // portada, ligado al progreso de scroll (no dispara una vez y listo -- va
  // "scrubando" con el scroll real, hacia adelante y atrás) usando
  // ScrollObserver de anime.js.
  ...
  useEffect(() => {
    ...
    const observer = onScroll({
      ...
      onUpdate: (self) => {
        ...
        shine.style.opacity = String(Math.sin(p * Math.PI) * 0.85);
        shine.style.top = `${p * 100}%`;
      },
    });
  ...
  // Portada animada -- mismo criterio que la base de Circuito: tinte +
  // effect="flash" (familia alocada/extravagante), acento propio de esta
  // variante + magenta compartido, scrim = rgb(#08080A), igual en todas
  // las variantes (bg no cambia por variante).
  const portadaImagenFondoDesktopRaw = String(invitation.portadaImagenFondoDesktop ?? "") || undefined;
  const portadaFondoAnimado = Boolean(portadaImagenFondoDesktopRaw);
  const portadaTintColor1 = "#B14EFF";
  ...
  return (
    <div
      className={`${circDisplay.variable} ${circBody.variable} ${circMono.variable}`}
      style={{
    ...
    >
      <style>{`
        .desktop-stage .tpl .font-mono, .tpl .font-mono {
          font-family: var(--circ-mono), monospace !important;
        }
        /* Overlay de scanlines CRT -- rayas horizontales estáticas muy sutiles
           + una barra de escaneo que recorre la pantalla en loop, ambas
           puramente decorativas (pointer-events:none, no interfieren). */
        .circ-scanlines {
          position: fixed;
          inset: 0;
          z-index: 40;
          ...
          background: repeating-linear-gradient(
            to bottom,
            rgba(177,78,255,0.025) 0px,
            rgba(177,78,255,0.025) 1px,
            transparent 1px,
            transparent 3px
          );
          mix-blend-mode: screen;
        }
        .circ-scanlines::after {
          content: "";
          position: absolute;
          left: 0; right: 0; height: 120px;
          background: linear-gradient(to bottom, transparent, rgba(177,78,255,0.06), transparent);
          animation: circ-scanSweep 7s linear infinite;
        }
        @keyframes circ-scanSweep {
          0% { top: -120px; }
          100% { top: 100%; }
        }
        /* Glitch ambiente en el título del hero -- RGB split muy breve cada
           pocos segundos, sensación de señal de video inestable. */
        .circ-glitch-text {
          position: relative;
          animation: circ-glitchShake 5.5s infinite;
        }
        .circ-glitch-text::before,
        .circ-glitch-text::after {
        ...
        .circ-glitch-text::before {
          color: #39FFD0;
          animation: circ-glitchTop 5.5s infinite;
        }
        .circ-glitch-text::after {
          color: #B14EFF;
          animation: circ-glitchBottom 5.5s infinite;
        }
        @keyframes circ-glitchShake {
          0%, 96%, 100% { transform: translate(0, 0); }
          97% { transform: translate(-1px, 0.5px); }
          98% { transform: translate(1px, -0.5px); }
        }
        @keyframes circ-glitchTop {
          0%, 96%, 100% { opacity: 0; transform: translate(0, 0); clip-path: inset(0 0 0 0); }
          96.5% { opacity: 0.8; transform: translate(2px, 0); clip-path: inset(10% 0 60% 0); }
          97.5% { opacity: 0.8; transform: translate(-2px, 0); clip-path: inset(50% 0 10% 0); }
          98.5% { opacity: 0; }
        }
        @keyframes circ-glitchBottom {
          0%, 96%, 100% { opacity: 0; transform: translate(0, 0); clip-path: inset(0 0 0 0); }
          96.5% { opacity: 0.8; transform: translate(-2px, 0); clip-path: inset(60% 0 10% 0); }
          97.5% { opacity: 0.8; transform: translate(2px, 0); clip-path: inset(10% 0 50% 0); }
          98.5% { opacity: 0; }
        }
        /* Cuenta regresiva con estética de terminal -- ventana con "chrome"
           de tres puntos + rótulo de proceso, cifras con brackets. */
        #countdown.dark {
          position: relative;
        }
        #countdown.dark::before {
          content: "● ● ●  cuenta_regresiva.sh";
          display: block;
          font-family: var(--circ-mono), monospace;
          font-size: 10px;
          letter-spacing: 0.05em;
          color: rgba(177,78,255,0.45);
          padding: 8px 20px 0;
        }
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          background-color: rgba(177,78,255,0.05) !important;
          border-color: rgba(177,78,255,0.35) !important;
          box-shadow: inset 0 0 12px rgba(177,78,255,0.08) !important;
        }
        #countdown[data-style="clasico"].dark > div > div > div > *:first-child {
          text-shadow: 0 0 8px rgba(177,78,255,0.55);
        }

        /* RSVP Custom Aesthetics for CircuitoTemplateVioleta */
        #rsvp.section.dark {
          background-color: #08080A !important;
          color: #FFFFFF !important;
        ...
        #rsvp.section.dark input {
          ...
          border: 1px solid rgba(177,78,255,0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #B14EFF !important;
          color: #08080A !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #B14EFF !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #08080A !important; /* Matches light sections */
          color: #FFFFFF !important;
      ...
      {/* Overlay de scanlines CRT, siempre presente (portada y cuerpo) --
          puramente decorativo, no captura eventos. */}
      <div className="circ-scanlines" aria-hidden="true" />

      {/* PORTADA / WELCOME OVERLAY (mesh negro con acentos verde/magenta, glow pulsante) */}
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
        ...
        >
          ...
          <div style={{
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(177,78,255,0.13), transparent), radial-gradient(45% 40% at 85% 80%, rgba(57,255,208,0.2), transparent)',
            backgroundSize: '160% 160%',
            animation: 'circ-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(177,78,255,0.13), transparent 70%)',
          ...
          <IconHex className="circ-doodle opacity-0 absolute" style={{ width: 34, height: 34, top: '10%', left: '12%', color: 'rgba(177,78,255,0.5)' }} />
          <IconHex className="circ-doodle opacity-0 absolute" style={{ width: 26, height: 26, top: '14%', right: '14%', color: 'rgba(57,255,208,0.4)' }} />
          <IconHex className="circ-doodle opacity-0 absolute" style={{ width: 30, height: 30, bottom: '16%', left: '18%', color: 'rgba(177,78,255,0.35)' }} />
          <IconNode className="circ-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '30%', right: '20%', color: '#39FFD0' }} />
          <IconNode className="circ-doodle opacity-0 absolute" style={{ width: 16, height: 16, bottom: '22%', right: '28%', color: '#B14EFF' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="circ-seal opacity-0" style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #B14EFF',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#B14EFF',
            }}>
              {monogram}
            </div>

            {/* Guest Name */}
            <h2 className="text-4xl sm:text-5xl font-light tracking-wide text-[#E9FFF6] leading-relaxed" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className=" text-sm font-medium text-[#7C8C88] tracking-wide uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8 }}>
                Dress code: {activeDressCode}
              </p>
            )}

            {/* Botón terminal -- borde verde recto, sin redondeos, texto con
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--circ-mono), monospace', border: '1px solid #B14EFF', color: '#B14EFF',
                background: 'rgba(177,78,255,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#B14EFF'; e.currentTarget.style.color = '#08080A'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(177,78,255,0.08)'; e.currentTarget.style.color = '#B14EFF'; }}
            >
              [ ABRIR_INVITACIÓN ]
            </button>
          ...
          <style jsx>{`
            @keyframes circ-meshDrift { 0%, 100% { background-position: 0% 0%, 100% 100%; } 50% { background-position: 30% 20%, 70% 80%; } }
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#B14EFF]/40 shadow-md ${isTicketMaximized ? 'bg-[#08080A]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#FFFFFF]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#B14EFF] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#FFFFFF] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#B14EFF]/20 pl-3">
                <span className="text-[#FFFFFF] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#7C8C88] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? 'Lugar' : 'Lugares'}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#B14EFF]" />
              <span className="text-[#0C0C10]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <div className="desktop-stage" data-theme={theme} style={{
        ...
        // El wrapper de escritorio necesita las mismas 6 CSS vars que el
        // mobile (ver sección 3.2 de la guía técnica: si no se definen acá
        // también, Countdown/RSVP/BottomNavPill en escritorio caen al color
        // de fallback en vez del verde/magenta de Circuito).
        "--t-acc": "#B14EFF",
        "--t-acc2": "#39FFD0",
        "--c-accent": "#B14EFF",
        "--t-bg": "#08080A",
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#B14EFF', margin: '6px 0 14px', animation: 'circ-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#08080A]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#08080A] z-10 relative">
            <IconHex className="circ-scroll-doodle opacity-0 absolute" style={{ width: 24, height: 24, top: 10, right: 24, color: '#39FFD0' }} />
            <IconNode className="circ-scroll-doodle opacity-0 absolute" style={{ width: 16, height: 16, top: 60, right: 56, color: '#B14EFF' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#B14EFF] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#E9FFF6] leading-[1.0] mb-3 circ-glitch-text" data-text={title} style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#B14EFF' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#B14EFF', margin: '0 0 20px', animation: 'circ-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#7C8C88] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#B14EFF] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          
          {/* Image Container -- marco HUD con esquinas reforzadas + barrido
              de "escaneo" de datos verde ligado al scroll. */}
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #08080A 100%)' }} />
            <div
            ...
            />
            <PhotoCornerFrame />
            {/* Barrido de "escaneo de datos" (línea horizontal verde con
                estela) que recorre la foto según el progreso de scroll
            ...
            <div
              ...
              style={{
                height: '18%',
                background: 'linear-gradient(to bottom, transparent, rgba(177,78,255,0.35) 45%, rgba(177,78,255,0.55) 50%, rgba(177,78,255,0.35) 55%, transparent)',
                opacity: 0,
              }}
            />
          </div>
        ...
        {/* Divisor doodle entre hero y cuenta regresiva (dos hexágonos de
            circuito flanqueando una línea con un nodo de PCB al centro). */}
        <div className="w-full flex items-center justify-center gap-4 py-8 bg-[#08080A]" aria-hidden="true">
          <IconHex className="circ-scroll-doodle opacity-0" style={{ width: 30, height: 30, color: 'rgba(177,78,255,0.4)' }} />
          <div style={{ width: 48, height: 1, background: 'linear-gradient(90deg, transparent, #39FFD0, transparent)' }} />
          <IconNode className="circ-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#39FFD0' }} />
          <div style={{ width: 48, height: 1, background: 'linear-gradient(90deg, transparent, #39FFD0, transparent)' }} />
          <IconHex className="circ-scroll-doodle opacity-0" style={{ width: 30, height: 30, color: 'rgba(177,78,255,0.4)' }} />
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #39FFD014, transparent 70%), #0C0C10" }}>
            <div className="max-w-2xl mx-auto text-center">
              <TypewriterText 
                text={`"${String(invitation.frasePersonalizadaTexto)}"`}
                className="text-[#E9FFF6] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#08080A] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#B14EFF]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              <IconNode className="circ-scroll-doodle opacity-0" style={{ width: 14, height: 14 }} />
              CUÁNDO Y DÓNDE
            </p>
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#B14EFF] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#7C8C88] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  ...
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#7C8C88]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#7C8C88]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#B14EFF] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#B14EFF] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#7C8C88] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#B14EFF] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#B14EFF]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <IconHex className="circ-scroll-doodle opacity-0" style={{ width: 16, height: 16 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#B14EFF]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#7C8C88] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#0C0C10] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#0A0A0D] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#B14EFF]">
                  <IconHex className="circ-scroll-doodle opacity-0" style={{ width: 16, height: 16 }} />
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #39FFD018, transparent 70%), #0A0A0D" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#08080A' }} aria-hidden="true">
            <IconMusic className="circ-scroll-doodle opacity-0" style={{ width: 28, height: 28, color: '#B14EFF' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        {/* Separador doodle de footer -- traza de PCB horizontal con dos
            vías soldadas, eco del hexágono/nodo del resto de la plantilla. */}
        <div className="w-full flex justify-center py-6" style={{ background: '#08080A' }} aria-hidden="true">
          <svg className="circ-scroll-doodle opacity-0" viewBox="0 0 90 12" width="90" height="12" fill="none" stroke="rgba(177,78,255,0.6)" strokeWidth="1.2">
            <path d="M2 6h24M64 6h24M32 6h8M46 6h8" />
            <circle cx="26" cy="6" r="2" fill="rgba(177,78,255,0.6)" stroke="none" />
            <circle cx="64" cy="6" r="2" fill="rgba(177,78,255,0.6)" stroke="none" />
            <circle cx="45" cy="6" r="1.6" fill="#39FFD0" stroke="none" />
          </svg>
        </div>

        <LogoFooterCredit bgColor="#08080A" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#B14EFF" surfaceColor="#101014" />}
      </div>
  );
}
