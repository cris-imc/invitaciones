...
/**
 * FashionTagTemplateGoldenMustard.tsx
 *
...
interface FashionTagTemplateGoldenMustardProps {
...
export function FashionTagTemplateGoldenMustard({ invitation, guest, isPersonalized = false }: FashionTagTemplateGoldenMustardProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C2903A";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(194,144,58,.35), 0 18px 50px -30px #C2903A";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#1C1A18" : "#C2903A";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#100D08", tinta: "#F4F1EA", acento: "#C2903A", clase: `${ftgPlayfair.variable} ${ftgMono.variable}`, fuente: "var(--ftg-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="ftg-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="ftg-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #241C0F 0%, #1A140C 55%, #100D08 100%)" }}>
            ...
            <div className="ftg-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="16,13,8" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="16,13,8" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="ftg-section ftg-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #332715 0%, #1C150C 55%, #100D08 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="ftg-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #2B2012 0%, #150F09 52%, #100D08 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="ftg-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="ftg-pan-sticky">
            <div data-strip="1" className="ftg-strip">
              ...
              <div data-tone="dark" className="ftg-panel ftg-panel--center" style={{ background: "#1A140C", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="ftg-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #241C0F 0%, #1A140C 60%, #100D08 100%)" }}>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="ftg-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="ftg-pan-sticky">
            ...
            <FtgDots count={photoPages.length} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="ftg-section" style={{ background: "#1A140C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="ftg-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="ftg-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F7F3EA" : "#C2903A" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="ftg-section" style={{ background: "#1A140C" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="ftg-section" style={{ background: "#1A140C" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu etiqueta" className="ftg-section ftg-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #241C0F 0%, #1A140C 55%, #100D08 100%)" }}>
          <span data-xin="1" data-dist="-60" className="ftg-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection, quizEnabled].filter(Boolean).length + 6)} — {tx("invitacion.kicker.guarda")} TU ETIQUETA</span>
      ...
      <div ref={coverRef} className="ftg-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C2903A" guest={guest} />
      )}
...
function FtgCoverHalf({
  ...
  return (
    <div className="ftg-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#C2903A" tintColor2="#100D08" effect="enfoque" scrimColorRgb="16,13,8" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#C2903A" tintColor2="#100D08" effect="enfoque" scrimColorRgb="16,13,8" />
        </div>
      )}
      ...
      <div className="ftg-cover-content">
        ...
        <div className="ftg-cover-bottom">
          <div ref={perfRef} className="ftg-perf-strip ftg-perf-strip--reveal ftg-perf-strip--cover" />
...
const FTG_CSS = `
  ...
  a { color: #C2903A; text-decoration: none; }
  ...
  .ftg-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #100D08; }
  ...
  @media (min-width: 768px) {
    .ftg-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(194,144,58,.3); }
  ...
  .ftg-date-month { font-family: var(--ftg-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C2903A; padding-left: 12%; }

  .ftg-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .ftg-divider-line { width: 52px; height: 1px; background: #C2903A; display: inline-block; }
  ...
  .ftg-cal-link:hover { color: #C2903A; }
  ...
  .ftg-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #1C1A18, #5C4520, #C2903A, #F7F3EA, #1C1A18); filter: saturate(.75); }
  .ftg-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #130F08; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .ftg-medallion-sub--accent { color: #C2903A; }
  .ftg-medallion-arc { position: absolute; inset: -14%; }
  .ftg-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C2903A; opacity: 0.4; font-family: var(--ftg-mono), monospace; }
  ...
  .ftg-cd-box { border: 1px solid #3A2C14; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  ...
  .ftg-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C2903A; }
  .ftg-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #100D08 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .ftg-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .ftg-perf-strip--cover { opacity: .9; }

  .ftg-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(28,26,24,0.32), rgba(199,184,154,0.2), rgba(194,144,58,0.36), rgba(28,26,24,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .ftg-accent-italic { font-style: italic; color: #C2903A; }
  .ftg-accent-serif { font-style: italic; color: #C2903A; font-family: var(--ftg-playfair), serif; }
  ...
  .ftg-pan[data-scroll="vertical"] { height: auto; }
  .ftg-pan[data-scroll="vertical"] .ftg-pan-sticky { position: static; height: auto; overflow: visible; }
  .ftg-pan[data-scroll="vertical"] .ftg-strip { position: static; display: block; width: 100%; transform: none !important; }
  .ftg-pan[data-scroll="vertical"] .ftg-panel { height: calc(var(--vh, 1vh) * 100); }
  ...
  .ftg-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  ...
  .ftg-stub { position: relative; overflow: hidden; border: 1px solid #3A2C14; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .ftg-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(194,144,58,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .ftg-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C2903A; background: transparent; color: #C2903A; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .ftg-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(194,144,58,0.3); color: #F4F1EA; font-family: var(--ftg-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .ftg-rsvp-input::placeholder { color: #6E6A5D; }
  .ftg-rsvp-input:focus { outline: none; border-bottom-color: #C2903A; }
  .ftg-rsvp-btn { width: 100%; padding: 17px 0; font-family: var(--ftg-mono), monospace; font-size: 11.5px; font-weight: 400; letter-spacing: 0.26em; text-transform: uppercase; border: 1px solid #C2903A; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 300ms ease, color 300ms ease; }
  ...
  .ftg-rsvp-btn--ghost { background: transparent; color: #C2903A; }
  .ftg-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(194,144,58,0.1); color: #C2903A; }
  .ftg-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C2903A; margin: 0; }
  ...
  .ftg-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(194,144,58,0.18); }
  ...
  .ftg-bank-copy { flex-shrink: 0; font-family: var(--ftg-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C2903A; background: transparent; color: #C2903A; cursor: pointer; }
  .ftg-bank-copy:hover { background: rgba(194,144,58,0.12); }
  ...
  .ftg-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(194,144,58,0.3); padding-bottom: 12px; }
  ...
  .ftg-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(194,144,58,0.3); color: #F4F1EA; font-family: var(--ftg-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .ftg-song-input::placeholder { color: #6E6A5D; }
  .ftg-song-input:focus { outline: none; border-bottom-color: #C2903A; }
  .ftg-song-sep { color: #8A8577; flex-shrink: 0; }
  .ftg-song-submit { flex-shrink: 0; background: none; border: none; color: #C2903A; font-family: var(--ftg-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .ftg-song-submit:hover { color: #F7F3EA; }
  .ftg-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .ftg-song-error { font-size: 10px; color: #C2903A; margin-top: 6px; }
  ...
  .ftg-quiz-opt { font-family: var(--ftg-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(194,144,58,0.4); background: transparent; color: #C2903A; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .ftg-quiz-opt:disabled { cursor: default; }
  .ftg-quiz-opt--picked { background: #C2903A; border-color: #C2903A; color: #0B0B10; }
  .ftg-quiz-opt--correct { background: #C2903A; border-color: #C2903A; color: #0B0B10; }
  .ftg-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .ftg-quiz-result { padding-top: 18px; border-top: 1px solid rgba(194,144,58,0.2); }
  ...
  .ftg-final-card { border: 1px solid #C2903A; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .ftg-replay { cursor: pointer; color: #C2903A; }
  ...
  .ftg-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F7F3EA, #C2903A); transition: height 260ms linear; display: block; }
  .ftg-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C2903A; transition: color 500ms ease; }
  ...
  .ftg-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #281F10 0%, #1A140C 46%, #100D08 100%); }
  .ftg-cover-half--bottom .ftg-cover-inner { top: auto; bottom: 0; }
  .ftg-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(28,26,24,0.5), rgba(199,184,154,0.3), rgba(194,144,58,0.55), rgba(28,26,24,0.5)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .ftg-cover-names { margin: 0; font-family: var(--ftg-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F7F3EA; }
  .ftg-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C2903A, transparent); display: block; }
  ...
  .ftg-cover-cta { border: 1px solid #C2903A; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--ftg-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .ftg-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C2903A; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
