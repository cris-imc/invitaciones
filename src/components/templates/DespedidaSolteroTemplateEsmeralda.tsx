...
/**
 * DespedidaSolteroTemplateEsmeralda.tsx
 *
 * Portado 1:1 desde el mockup "Despedida de Soltero" (mockup/Eventos),
...
const dpoDisplay = Bodoni_Moda({
  ...
  style: ["normal", "italic"],
  weight: ["400", "500"],
  variable: "--dpo-display",
  display: "swap",
});

const dpoMono = IBM_Plex_Mono({
  subsets: ["latin"],
  preload: false,
  weight: ["300", "400", "500"],
  variable: "--dpo-mono",
...
interface DespedidaSolteroTemplateEsmeraldaProps {
...
export function DespedidaSolteroTemplateEsmeralda({ invitation, guest, isPersonalized = false }: DespedidaSolteroTemplateEsmeraldaProps) {
  const tx = useTextos();
  ...
  const namesTitle = festejado || "Despedida de Soltero";
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E7F5EE";
      }
      if (stubRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#286148" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#286148" : "#4FA983";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#08090B", tinta: "#F4F1EA", acento: "#4FA983", clase: `${dpoDisplay.variable} ${dpoMono.variable}`, fuente: "var(--dpo-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${dpoDisplay.variable} ${dpoMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#08090B",
        fontFamily: "var(--dpo-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{GP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="dpo-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="dpo-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #0B0D10 55%, #08090B 100%)" }}>
          ...
          <div className="dpo-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="dpo-date-num dpo-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="dpo-divider">
            <span className="dpo-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="dpo-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="dpo-cal-link"
          />
        ...
        {(photoMobile || photoDesktop) && (
          ...
          >
            ...
            <span data-xin="1" data-dist="-60" className="dpo-kicker dpo-hero-photo-kicker">02 — ASÍ EMPEZÓ TODO</span>
          </section>
        )}

        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="dpo-section dpo-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #12202B 0%, #0D0B10 55%, #08090B 100%)" }}>
          <div className="dpo-scan-grid" />
          ...
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="dpo-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #08090B 100%)" }}>
            ...
            <h2 ref={phraseRef} className="dpo-phrase" style={{ fontSize: fraseFontSize }}>
              {fraseWords.map((w, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= fraseAccentStart ? "dpo-accent-italic" : undefined}>
                    {w}
                  </span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="120" data-dist="90" className="dpo-divider" style={{ position: "relative" }}>
              <span className="dpo-divider-line dpo-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
            </div>
          </section>
        )}
          ...
          <div className="dpo-pan-sticky">
            <div data-strip="1" className="dpo-strip">
              <div id="details" data-tone="light" className="dpo-panel dpo-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="dpo-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="dpo-crono-row">
                        <span className="dpo-crono-time">{item.time || ""}</span>
                        <span className="dpo-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="dpo-panel dpo-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="dpo-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#286148" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#286148" />
                  </svg>
            ...
            <DpoDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="dpo-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #0B0D10 60%, #08090B 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="dpo-rsvp">
              <DpoRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="dpo-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="dpo-pan-sticky">
            <div data-strip="1" className="dpo-strip">
                ...
                <div key={pageIndex} data-tone="light" className="dpo-panel dpo-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="dpo-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`dpo-mosaic-cell${i === 0 ? " dpo-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="dpo-seguir dpo-seguir--split">
                    ...
                    <span className="dpo-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="dpo-section" style={{ background: "#0B0D10" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="dpo-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="dpo-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E7F5EE" : "#4FA983" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="dpo-song-wrap">
              <DpoSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="dpo-section dpo-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #0B0D10 55%, #08090B 100%)" }}>
          ...
          <div className="dpo-footer-credit">
      ...
      <div ref={railRef} className="dpo-rail">
        ...
        <span ref={railLabelRef} className="dpo-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="dpo-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="dpo-cover-half dpo-cover-half--top">
          <DpoCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="dpo-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="dpo-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
...
function DpoCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="dpo-cd-box">
      <span ref={refEl} className="dpo-cd-num">—</span>
      <span className="dpo-cd-label">{label}</span>
    </div>
  );
}

function DpoDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="dpo-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="dpo-dot" />
      ))}
    </div>
  );
}

function DpoMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="dpo-medallion-arc" style={{ animation: spin === "reverse" ? "dpoRingRev 32s linear infinite" : "dpoRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="dpo-medallion-arc-text">
...
function DpoCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="dpo-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="dpo-bank-row-label">{label}</span>
    ...
    <>
      <div className="dpo-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="dpo-rsvp-row dpo-rsvp-row--payment">
      ...
      <div ref={stubRef} className="dpo-stub">
        ...
        <div ref={sealRef} className="dpo-seal">
          <DpoMedallionCmp label="DS" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="dpo-beam" />
        <div className="dpo-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="dpo-rsvp-error">{error}</p>}
...
function DpoSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="dpo-song">
      ...
      {error && <p className="dpo-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="dpo-song-list">
...
function DpoQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: DpoQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="dpo-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="dpo-quiz-q">
            <span className="dpo-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="dpo-quiz-q-text">{q.pregunta}</p>
            <div className="dpo-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " dpo-quiz-opt--correct";
                  else if (chosen) stateClass = " dpo-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " dpo-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`dpo-quiz-opt${stateClass}`}
...
function DpoInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="dpo-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="dpo-bank-row-label">{label}</span>
    ...
    <div className="dpo-cover-inner">
      ...
      <div className="dpo-cover-content">
        ...
        <div className="dpo-cover-center">
          ...
          <h1 ref={namesRef} className="dpo-cover-names">{namesTitle}</h1>
          <span className="dpo-cover-rule" />
          <span className="dpo-cover-date">{fechaCorta}</span>
        </div>
        <div className="dpo-cover-bottom">
          <div ref={perfRef} className="dpo-perf-strip dpo-perf-strip--reveal dpo-perf-strip--cover" />
          <div className="dpo-cover-facts">
...
// ---------------------------------------------------------------------
const GP_CSS = `
  .dpo-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .dpo-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #4FA983; text-decoration: none; }
  a:hover { color: #E7F5EE; }

  @keyframes dpoFoil { to { transform: rotate(360deg); } }
  @keyframes dpoRing { to { transform: rotate(360deg); } }
  @keyframes dpoRingRev { to { transform: rotate(-360deg); } }
  @keyframes dpoScan { 0% { transform: translateY(0); opacity: 0; } 12% { opacity: .85; } 88% { opacity: .85; } 100% { transform: translateY(320px); opacity: 0; } }
  @keyframes dpoEq { 0%,100% { transform: scaleY(.25); } 50% { transform: scaleY(1); } }
  @keyframes dpoHint { 0%,100% { transform: translateY(0); } 50% { transform: translateY(7px); } }
  @keyframes dpoSide { 0%,100% { transform: translateX(0); } 50% { transform: translateX(7px); } }
  @media (prefers-reduced-motion: reduce) { .dpo-scroller * { animation: none !important; } }

  .dpo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .dpo-section--between { justify-content: space-between; }
  ...
  .dpo-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #08090B; }
  .dpo-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  .dpo-hero-photo-kicker { position: absolute; left: 0; right: 0; bottom: 0; z-index: 2; padding: 0 max(24px, calc((100% - 560px) / 2)) 48px; }
  ...
  @media (max-width: 767px) {
    .dpo-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .dpo-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(200,164,92,.3); }
    .dpo-hero-photo-kicker { bottom: 40px; }
    .dpo-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .dpo-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .dpo-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .dpo-date-num { font-family: var(--dpo-display), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .dpo-date-num--right { text-align: right; line-height: 0.86; }
  .dpo-date-month { font-family: var(--dpo-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #4FA983; padding-left: 12%; }

  .dpo-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .dpo-divider-line { width: 52px; height: 1px; background: #4FA983; display: inline-block; }
  .dpo-divider-line--long { width: 64px; }

  .dpo-lead { margin: 0; font-family: var(--dpo-display), serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .dpo-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--dpo-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .dpo-cal-link:hover { color: #4FA983; }

  .dpo-medallion { position: relative; }
  .dpo-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .dpo-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .dpo-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .dpo-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #4FA983, #E7F5EE, #6E5AA8); filter: saturate(.75); }
  .dpo-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .dpo-medallion-label { font-family: var(--dpo-display), serif; font-size: 26px; line-height: 1; color: #E7F5EE; }
  .dpo-medallion-label-sm { font-family: var(--dpo-display), serif; font-size: 16px; color: #E7F5EE; }
  .dpo-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .dpo-medallion-sub--accent { color: #4FA983; }
  .dpo-medallion-arc { position: absolute; inset: -14%; }
  ...
  .dpo-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #4FA983; opacity: 0.4; font-family: var(--dpo-mono), monospace; }

  .dpo-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .dpo-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E7F5EE, transparent); animation: dpoScan 6s linear infinite; pointer-events: none; }

  .dpo-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .dpo-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .dpo-cd-num { font-family: var(--dpo-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #E7F5EE; display: inline-block; }
  .dpo-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #4FA983; }
  .dpo-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #08090B 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .dpo-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .dpo-perf-strip--cover { opacity: .9; }

  .dpo-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(200,164,92,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: dpoFoil 30s linear infinite; }
  .dpo-phrase { margin: 0; position: relative; font-family: var(--dpo-display), serif; font-weight: 400; font-size: clamp(50px, 15vw, 96px); line-height: 0.92; letter-spacing: -0.03em; }
  .dpo-accent-italic { font-style: italic; color: #4FA983; }
  .dpo-accent-serif { font-style: italic; color: #286148; font-family: var(--dpo-display), serif; }
  .dpo-accent-serif-2 { color: #286148; }
  .dpo-h2 { margin: 0; font-family: var(--dpo-display), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .dpo-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .dpo-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .dpo-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .dpo-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .dpo-panel--between { justify-content: space-between; }
  ...
  .dpo-pan[data-scroll="vertical"] { height: auto; }
  .dpo-pan[data-scroll="vertical"] .dpo-pan-sticky { position: static; height: auto; overflow: visible; }
  .dpo-pan[data-scroll="vertical"] .dpo-strip { position: static; display: block; width: 100%; transform: none !important; }
  .dpo-pan[data-scroll="vertical"] .dpo-panel { height: calc(var(--vh, 1vh) * 100); }
  .dpo-panel--end { justify-content: flex-end; }
  .dpo-panel--center { align-items: center; justify-content: center; text-align: center; }
  .dpo-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .dpo-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .dpo-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .dpo-panel-title { margin: 0; position: relative; font-family: var(--dpo-display), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .dpo-panel-title-md { margin: 0; position: relative; font-family: var(--dpo-display), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .dpo-panel-title-sm { margin: 0; font-family: var(--dpo-display), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .dpo-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .dpo-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .dpo-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  .dpo-facts { position: relative; display: flex; flex-direction: column; gap: 12px; font-size: 11px; letter-spacing: 0.14em; color: #4A473F; }
  .dpo-facts-row { display: flex; justify-content: space-between; border-bottom: 1px solid #D6D1C4; padding-bottom: 10px; }
  .dpo-facts-row--last { border-bottom: none; padding-bottom: 0; }
  .dpo-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #286148; margin-top: auto; }
  .dpo-seguir--split { justify-content: space-between; }
  .dpo-side-hint { display: inline-block; animation: dpoSide 2.2s ease-in-out infinite; }
  .dpo-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .dpo-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #286148; }

  .dpo-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .dpo-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .dpo-crono-time { font-family: var(--dpo-mono), monospace; color: #286148; min-width: 42px; }
  .dpo-crono-title { flex: 1; }

  .dpo-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .dpo-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .dpo-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .dpo-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .dpo-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .dpo-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .dpo-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E7F5EE, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .dpo-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--dpo-mono), monospace; }
  .dpo-rsvp-rows { display: flex; flex-direction: column; }
  .dpo-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .dpo-rsvp-row > span:first-child { flex-shrink: 0; }
  .dpo-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .dpo-rsvp-row--payment { align-items: flex-start; }
  .dpo-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .dpo-rsvp-payment-total { color: #E7F5EE; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .dpo-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .dpo-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .dpo-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #4FA983; background: transparent; color: #4FA983; font-size: 14px; line-height: 1; cursor: pointer; }
  .dpo-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .dpo-rsvp-stepper span { font-family: var(--dpo-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .dpo-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--dpo-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .dpo-rsvp-input::placeholder { color: #6E6A5D; }
  .dpo-rsvp-input:focus { outline: none; border-bottom-color: #4FA983; }
  .dpo-rsvp-btn { width: 100%; padding: 16px; font-family: var(--dpo-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #4FA983, #286148); border: 1px solid #4FA983; color: #0B0D10; cursor: pointer; }
  .dpo-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .dpo-rsvp-btn--ghost { background: transparent; color: #4FA983; }
  .dpo-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #4FA983; margin: 0; }
  .dpo-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .dpo-rsvp-declined-text { margin: 0; font-family: var(--dpo-display), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .dpo-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .dpo-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .dpo-photo-cell--offset { margin-top: 18px; }
  .dpo-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .dpo-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .dpo-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .dpo-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .dpo-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .dpo-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .dpo-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .dpo-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .dpo-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  ...
  .dpo-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .dpo-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  .dpo-bank-row:last-child { border-bottom: none; }
  .dpo-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .dpo-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .dpo-bank-copy { flex-shrink: 0; font-family: var(--dpo-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #4FA983; background: transparent; color: #4FA983; cursor: pointer; }
  .dpo-bank-copy:hover { background: rgba(200,164,92,0.12); }

  .dpo-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .dpo-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: dpoEq 1.1s ease-in-out infinite; display: inline-block; }
  .dpo-song-wrap { font-family: var(--dpo-mono), monospace; }

  .dpo-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .dpo-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  .dpo-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .dpo-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--dpo-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .dpo-song-input::placeholder { color: #6E6A5D; }
  .dpo-song-input:focus { outline: none; border-bottom-color: #4FA983; }
  .dpo-song-sep { color: #8A8577; flex-shrink: 0; }
  .dpo-song-submit { flex-shrink: 0; background: none; border: none; color: #4FA983; font-family: var(--dpo-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .dpo-song-submit:hover { color: #E7F5EE; }
  .dpo-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .dpo-song-error { font-size: 10px; color: #4FA983; margin-top: 6px; }
  .dpo-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .dpo-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--dpo-mono), monospace; }
  .dpo-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .dpo-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .dpo-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .dpo-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .dpo-quiz-q-num { font-family: var(--dpo-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .dpo-quiz-q-text { margin: 0; font-family: var(--dpo-display), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .dpo-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .dpo-quiz-opt { font-family: var(--dpo-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #4FA983; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .dpo-quiz-opt:disabled { cursor: default; }
  .dpo-quiz-opt--picked { background: #4FA983; border-color: #4FA983; color: #0B0D10; }
  .dpo-quiz-opt--correct { background: #4FA983; border-color: #4FA983; color: #0B0D10; }
  .dpo-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .dpo-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .dpo-quiz-result-score { margin: 0 0 6px; font-family: var(--dpo-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E7F5EE; }
  .dpo-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .dpo-final-card { border: 1px solid #4FA983; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .dpo-final-names { font-family: var(--dpo-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E7F5EE; }
  .dpo-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .dpo-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .dpo-replay { cursor: pointer; color: #4FA983; }
  .dpo-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .dpo-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .dpo-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .dpo-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .dpo-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E7F5EE, #4FA983); transition: height 260ms linear; display: block; }
  .dpo-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #4FA983; transition: color 500ms ease; }

  .dpo-cover { position: absolute; inset: 0; z-index: 5; }
  .dpo-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .dpo-cover-half--top { top: 0; }
  .dpo-cover-half--bottom { bottom: 0; }
  .dpo-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #121418 0%, #0B0D10 46%, #08090B 100%); }
  .dpo-cover-half--bottom .dpo-cover-inner { top: auto; bottom: 0; }
  .dpo-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(36,36,36,0.5), rgba(79,169,131,0.32), rgba(79,169,131,0.55), rgba(36,36,36,0.5)); filter: blur(64px); opacity: .62; animation: dpoFoil 26s linear infinite; }
  .dpo-cover-texture { position: absolute; inset: 0; background-image: repeating-linear-gradient(0deg, rgba(231,245,238,.05) 0 1px, transparent 1px 22px), repeating-linear-gradient(90deg, rgba(231,245,238,.05) 0 1px, transparent 1px 22px); }
  .dpo-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .dpo-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .dpo-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .dpo-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .dpo-cover-names { margin: 0; font-family: var(--dpo-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E7F5EE; }
  .dpo-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#4FA983, transparent); display: block; }
  .dpo-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .dpo-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .dpo-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  ...
  .dpo-cover-facts:has(> span:only-child) { justify-content: center; }
  .dpo-cover-cta { border: 1px solid #4FA983; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--dpo-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .dpo-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .dpo-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0B0D10; }
  }
  .dpo-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .dpo-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: dpoHint 2.4s ease-in-out infinite; }

  .dpo-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .dpo-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #4FA983; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .dpo-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
