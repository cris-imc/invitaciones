 ...
 * mármol y dorado #C9A45C, Italiana + Cormorant Garamond + IBM Plex Mono,
...
interface MarmolYOroTemplateProps {
...
export function MarmolYOroTemplate({ invitation, guest, isPersonalized = false }: MarmolYOroTemplateProps) {
  const tx = useTextos();
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#8C6F35" : "#C9A45C";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0C0C0E", tinta: "#F4F1EA", acento: "#C9A45C", clase: `${myoItaliana.variable} ${myoCormorant.variable} ${myoMono.variable}`, fuente: "var(--myo-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="myo-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="myo-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #111113 55%, #0C0C0E 100%)" }}>
            ...
            <div className="myo-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="12,12,14" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="12,12,14" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="myo-section myo-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #241D0F 0%, #0D0B10 55%, #0C0C0E 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="myo-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #0C0C0E 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="myo-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="myo-pan-sticky">
            <div data-strip="1" className="myo-strip">
              ...
              <div data-tone="dark" className="myo-panel myo-panel--center" style={{ background: "#111113", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="myo-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #111113 60%, #0C0C0E 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="myo-section" style={{ background: "#111113" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="myo-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="myo-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E8D6A8" : "#C9A45C" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="myo-section" style={{ background: "#111113" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="myo-section" style={{ background: "#111113" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pieza" className="myo-section myo-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #111113 55%, #0C0C0E 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C9A45C" guest={guest} />
      )}
...
function CoverHalf({
  ...
  return (
    <div className="myo-cover-inner">
      {/* Con foto cargada, la foto reemplaza el degradé/mármol de fondo --
          el resto de la ornamentación (glow, mármol, medallón, nombres,
          CTA) se mantiene arriba sin cambios. Misma lógica que la
          Colección Flat: SOLO en mobile (en desktop el degradé original
          de la plantilla se ve igual que siempre, con o sin foto). */}
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto
...
// mármol blanco/gris + dorado #C9A45C, nunca el dorado #C8A45C ni el
...
const MYO_CSS = `
  ...
  /* Foto principal (ver rama experimento-foto-storytelling). Mobile: la
     ...
     borde dorado propio en vez de estirarse. */
  .myo-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0C0C0E; }
  ...
  @media (min-width: 768px) {
    .myo-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(201,164,92,.3); }
  ...
  .myo-date-month { font-family: var(--myo-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C9A45C; padding-left: 12%; }

  .myo-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .myo-divider-line { width: 52px; height: 1px; background: #C9A45C; display: inline-block; }
  ...
  .myo-cal-link:hover { color: #C9A45C; }
  ...
  .myo-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #A09C94, #1A1A1C, #C9A45C, #E8D6A8, #A09C94); filter: saturate(.75); }
  .myo-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .myo-medallion-label { font-family: var(--myo-italiana), serif; font-size: 26px; line-height: 1; color: #E8D6A8; }
  .myo-medallion-label-sm { font-family: var(--myo-italiana), serif; font-size: 16px; color: #E8D6A8; }
  .myo-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .myo-medallion-sub--accent { color: #C9A45C; }
  ...
  .myo-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C9A45C; opacity: 0.4; font-family: var(--myo-mono), monospace; }
  ...
  .myo-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E8D6A8, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .myo-cd-num { font-family: var(--myo-italiana), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6EFDD; display: inline-block; }
  .myo-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C9A45C; }
  .myo-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0C0C0E 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .myo-accent-italic { font-family: var(--myo-cormorant), serif; font-style: italic; color: #C9A45C; }
  .myo-accent-serif { font-style: italic; color: #8C6F35; font-family: var(--myo-cormorant), serif; }
  .myo-accent-serif-2 { color: #8C6F35; }
  ...
  .myo-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #8C6F35; margin-top: auto; }
  ...
  .myo-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #8C6F35; }
  ...
  .myo-crono-time { font-family: var(--myo-mono), monospace; color: #8C6F35; min-width: 42px; }
  ...
  .myo-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E8D6A8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .myo-rsvp-payment-total { color: #E8D6A8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .myo-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C9A45C; background: transparent; color: #C9A45C; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .myo-rsvp-input:focus { outline: none; border-bottom-color: #C9A45C; }
  .myo-rsvp-btn { width: 100%; padding: 16px; font-family: var(--myo-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C9A45C, #8C6F35); border: 1px solid #C9A45C; color: #0B0B10; cursor: pointer; }
  .myo-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .myo-rsvp-btn--ghost { background: transparent; color: #C9A45C; }
  .myo-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C9A45C; margin: 0; }
  ...
  .myo-bank-copy { flex-shrink: 0; font-family: var(--myo-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C9A45C; background: transparent; color: #C9A45C; cursor: pointer; }
  ...
  .myo-song-input:focus { outline: none; border-bottom-color: #C9A45C; }
  .myo-song-sep { color: #8A8577; flex-shrink: 0; }
  .myo-song-submit { flex-shrink: 0; background: none; border: none; color: #C9A45C; font-family: var(--myo-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .myo-song-submit:hover { color: #E8D6A8; }
  .myo-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .myo-song-error { font-size: 10px; color: #C9A45C; margin-top: 6px; }
  ...
  .myo-quiz-opt { font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #C9A45C; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .myo-quiz-opt:disabled { cursor: default; }
  .myo-quiz-opt--picked { background: #C9A45C; border-color: #C9A45C; color: #0B0B10; }
  .myo-quiz-opt--correct { background: #C9A45C; border-color: #C9A45C; color: #0B0B10; }
  ...
  .myo-quiz-result-score { margin: 0 0 6px; font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E8D6A8; }
  .myo-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .myo-final-card { border: 1px solid #C9A45C; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .myo-final-names { font-family: var(--myo-italiana), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E8D6A8; }
  ...
  .myo-replay { cursor: pointer; color: #C9A45C; }
  ...
  .myo-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E8D6A8, #C9A45C); transition: height 260ms linear; display: block; }
  .myo-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C9A45C; transition: color 500ms ease; }
  ...
  .myo-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #151417 0%, #111113 46%, #0C0C0E 100%); }
  .myo-cover-half--bottom .myo-cover-inner { top: auto; bottom: 0; }
  .myo-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(160,156,148,0.35), rgba(26,26,28,0.3), rgba(201,164,92,0.55), rgba(160,156,148,0.35)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .myo-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C9A45C, transparent); display: block; }
  ...
  .myo-cover-cta { border: 1px solid #C9A45C; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--myo-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .myo-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C9A45C; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
