/**
 * JardinSedaTemplateRosaAntiguo.tsx
 ...
 * (base #FCEFF1) + tinta ciruela oscura (#3A2E33) + acento malva/lavanda
 * fijo (#D68FA0) + un verde salvia secundario (#7C9473, variable por
...
interface JardinSedaTemplateRosaAntiguoProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#D68FA0]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#D68FA0] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#3A2E33] break-all">{value}</span>
      </div>
      <button 
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#D68FA0]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#D68FA0] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#3A2E33] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#3A2E33" }}>
          ¡Juego Completado!
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#3A2E33" }}>
          RESPONDISTE {score} DE {preguntas.length} CORRECTAMENTE ({percent}%)
        </p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#9C8A9B" }}>Guardando tus resultados...</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#D68FA0] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#3A2E33" }}>
                  El promedio global de aciertos del resto de los invitados ({stats.count}) es del <strong style={{ color: "#3A2E33" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#3A2E33] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function JardinSedaTemplateRosaAntiguo({ invitation, guest, isPersonalized = false }: JardinSedaTemplateRosaAntiguoProps) {
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#FCEFF1] via-[#F3E6EF] to-[#FFFFFF] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center border-t border-white/10 font-sans">
          <LogoFooterCredit bgColor="transparent" textColor="var(--jardinseda-ink, #3A2E33)" />
        </footer>
      </div>
    );
  }
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(214, 143, 160, 0.2) !important;
        }

        /* RSVP Custom Aesthetics for JardinSedaTemplateRosaAntiguo */
        #rsvp.section.dark {
          background-color: #FCEFF1 !important;
          color: #3A2E33 !important;
        ...
        #rsvp.section.dark input {
          ...
          color: #3A2E33 !important;
          border-radius: 999px !important;
          border: 1px solid rgba(214, 143, 160, 0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #D68FA0 !important;
          color: #3A2E33 !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #D68FA0 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #FCEFF1 !important; /* Matches light sections */
          color: #3A2E33 !important;
        ...
        #banco .copy-btn.copied {
          background-color: #3A2E33 !important;
          color: #FCEFF1 !important;
        }
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div className={portadaFondoAnimado ? "acp-desktop-only" : undefined}>
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(214,143,160,0.13), transparent), radial-gradient(45% 40% at 85% 80%, rgba(124,148,115,0.2), transparent)',
            backgroundSize: '160% 160%',
            animation: 'jardinseda-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(214,143,160,0.13), transparent 70%)',
          ...
          <IconHeartDoodle className="jardinseda-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '15%', right: '12%', color: 'rgba(214,143,160,0.5)' }} />
          <IconRibbon className="jardinseda-doodle opacity-0 absolute" style={{ width: 26, height: 19, bottom: '18%', left: '14%', color: 'rgba(214,143,160,0.45)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="jardinseda-seal opacity-0" style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #D68FA0',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#D68FA0',
            }}>
              {monogram}
            </div>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #D68FA0', color: '#D68FA0',
                background: 'rgba(214,143,160,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#D68FA0'; e.currentTarget.style.color = '#FCEFF1'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(214,143,160,0.08)'; e.currentTarget.style.color = '#D68FA0'; }}
            >
              ABRIR INVITACIÓN
            </button>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#D68FA0]/40 shadow-md ${isTicketMaximized ? 'bg-[#FCEFF1]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#3A2E33]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#D68FA0] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#3A2E33] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#D68FA0]/20 pl-3">
                <span className="text-[#3A2E33] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#9C8A9B] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? 'Lugar' : 'Lugares'}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#D68FA0]" />
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#D68FA0', margin: '6px 0 14px', animation: 'jardinseda-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#FCEFF1]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#FCEFF1] z-10 relative">
            ...
            <IconRibbon className="jardinseda-scroll-doodle opacity-0 absolute" style={{ width: 26, height: 15, top: 64, right: 52, color: 'rgba(214,143,160,0.4)' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#D68FA0] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#3A2E33] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#D68FA0' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#D68FA0', margin: '0 0 20px', animation: 'jardinseda-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#9C8A9B] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#D68FA0] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden rounded-b-[2.5rem]">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #FCEFF1 100%)' }} />
            <div
            ...
            <div className="absolute inset-3 pointer-events-none z-20 rounded-[2rem]" aria-hidden="true" style={{ border: "1px solid rgba(214,143,160,0.6)" }}>
              <IconRings style={{ position: "absolute", top: -10, left: -8, width: 30, height: 20, color: "#7C9473" }} />
              <IconHeartDoodle style={{ position: "absolute", bottom: -8, right: -6, width: 18, height: 18, color: "#D68FA0" }} />
            </div>
            ...
            <div ref={heroFlareRef} className="pointer-events-none z-20" style={{ position: "absolute", width: 140, height: 140, top: "10%", left: "8%", borderRadius: "50%", background: "radial-gradient(circle, rgba(214,143,160,0.55) 0%, rgba(124,148,115,0.3) 55%, transparent 75%)", opacity: 0, transform: "translate(-50%,-50%)", filter: "blur(2px)" }} />
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#FCEFF1]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #D68FA0, transparent)' }} />
          <IconRings className="jardinseda-scroll-doodle opacity-0" style={{ width: 26, height: 17, color: '#7C9473' }} />
          <IconHeartDoodle className="jardinseda-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#D68FA0' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #D68FA0, transparent)' }} />
        </div>
          ...
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #7C947314, transparent 70%), var(--t-surface)" }}>
            <div className="max-w-2xl mx-auto text-center">
              <TypewriterText 
                ...
                className="text-[#3A2E33] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#FCEFF1] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#D68FA0]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              <IconChurch className="jardinseda-scroll-doodle opacity-0" style={{ width: 15, height: 15 }} />
              CUÁNDO Y DÓNDE
            </p>
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#D68FA0] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9C8A9B] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#3A2E33] mb-3" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#9C8A9B]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#9C8A9B]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#D68FA0] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#D68FA0] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9C8A9B] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#D68FA0] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#D68FA0]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <IconRibbon className="jardinseda-scroll-doodle opacity-0" style={{ width: 20, height: 12 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#D68FA0]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#9C8A9B] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#3A2E33] font-light" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
          ...
          <SectionWrapper id="album" delay={200} className="w-full py-20 overflow-hidden" style={{ background: "var(--t-surface)" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-10 flex items-center gap-2">
                <IconHeartDoodle className="jardinseda-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#7C9473' }} />
                ÁLBUM
              </p>
            </div>
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#F3E6EF] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#D68FA0]">
                  <IconRings className="jardinseda-scroll-doodle opacity-0" style={{ width: 22, height: 15 }} />
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #7C947318, transparent 70%), #F3E6EF" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#FCEFF1' }} aria-hidden="true">
            <IconMusic className="jardinseda-scroll-doodle opacity-0" style={{ width: 26, height: 26, color: '#D68FA0' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#FCEFF1' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #D68FA0, transparent)' }} />
          <svg className="jardinseda-scroll-doodle opacity-0" viewBox="0 0 16 12" width="16" height="12" fill="none" stroke="#7C9473" strokeWidth="1.1" aria-hidden="true">
            <path d="M8 11V2M8 5 4 3M8 5l4-2M8 8 4.5 6.3M8 8l3.5-1.7" />
          </svg>
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #D68FA0, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#FCEFF1" textColor="var(--jardinseda-ink, #3A2E33)" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#D68FA0" surfaceColor="#FFFFFF" />}
      </div>
  );
}
