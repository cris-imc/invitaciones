...
// ObservatorioTemplate.tsx — no editar a mano: los cambios se pierden al regenerar.
...
export function ObservatorioTemplateAmanecer({ invitation, guest, isPersonalized = false }: ObservatorioTemplateProps) {
  const tx = useTextos();
    ...
    return (
      <PostEventoStorytelling
        ...
        paleta={{
          ...
          tinta: PALETA.ink,
          acento: PALETA.acc,
          clase: `${obsSerif.variable} ${obsSans.variable} ${obsMono.variable}`,
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="obs-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="obs-section obs-std">
          <div className="obs-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="obs-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="obs-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="obs-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="obs-std-fila">
              <span className="obs-cinta">{mesLargo}</span>
              <span className="obs-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="obs-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="obs-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="obs-btn-papel obs-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="obs-foto-seccion">
            <div className="obs-foto-marco">
              <div ref={ventanaRef} className="obs-ventana">
                {fotoMobile && (
              ...
              <div className="obs-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="obs-section obs-countdown">
          <div className="obs-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="obs-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="obs-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="obs-section obs-frase-seccion">
            <div className="obs-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="obs-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="obs-firma">
              <span className="obs-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="obs-pan-fijo">
            <div data-strip="1" className="obs-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="obs-panel obs-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="obs-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="obs-tarjeta obs-tarjeta--izq">
                  <span className="obs-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="obs-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="obs-filas">
                    {direccion && (
                      <div className="obs-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="obs-fila-valor">{direccion}</span></div>
                    )}
                    <div className="obs-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="obs-fila obs-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="obs-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="obs-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="obs-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="obs-panel obs-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="obs-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="obs-tarjeta obs-tarjeta--der">
                    <h2 className="obs-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="obs-filas">
                      {ceremoniaDireccion && (
                        <div className="obs-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="obs-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="obs-fila obs-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="obs-panel obs-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="obs-escena" aria-hidden="true">
                  ...
                  <h2 className="obs-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="obs-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="obs-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="obs-panel obs-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="obs-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="obs-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="obs-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="obs-crono-fila">
                        <span className="obs-crono-hora">{item.time || ""}</span>
                        <span className="obs-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="obs-section obs-checkin">
            <div className="obs-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="obs-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="obs-h2">
            ...
            <div className="obs-pan-fijo">
              <div data-strip="1" className="obs-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="obs-panel obs-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="obs-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="obs-h2 obs-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="obs-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="obs-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="obs-polaroid-img" />
                          {i % 2 === 0 && <span className="obs-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="obs-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="obs-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="obs-section obs-musica">
            <div className="obs-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="obs-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="obs-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="obs-section obs-regalos">
            <span data-xin="1" data-dist="-20" className="obs-kicker obs-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="obs-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="obs-parrafo obs-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="obs-section obs-quiz">
            <div className="obs-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="obs-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="obs-h2 obs-h2--claro">{triviaTitulo}</h2>
            <TriviaObservatorio
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="obs-section obs-pase">
          <div className="obs-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="obs-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="obs-ticket">
            <div className="obs-ticket-izq">
              <span className="obs-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="obs-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="obs-ticket-dato">
                {[
              ...
              <span className="obs-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="obs-ticket-muesca obs-ticket-muesca--arriba" aria-hidden="true" />
              <span className="obs-ticket-muesca obs-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="obs-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="obs-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="obs-pase-pie">
            <span className="obs-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="obs-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="obs-portada">
        ...
        <div ref={escenaPortadaRef} className="obs-portada-escena">
          <div data-cl="1" className="obs-trama" aria-hidden="true" />
        </div>
        <div className="obs-portada-contenido">
          <div className="obs-portada-arriba">
            <div ref={cartelRef} className="obs-cartel">
              <h1 className="obs-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="obs-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="obs-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="obs-portada-abajo">
            <div className="obs-portada-datos">
              <span className="obs-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="obs-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="obs-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="obs-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="obs-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="obs-portada-pase">
            ...
            <button type="button" onClick={abrir} className="obs-portada-btn">
...
function CuentaObservatorio({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="obs-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`obs-cuenta-caja obs-cuenta-caja--${i + 1}`}>
          <span className={`obs-cuenta-num${i === 3 ? " obs-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_OBS = `
  ...
  .obs-raiz a { color: var(--pp-acc); text-decoration: none; }
  .obs-raiz button { font: inherit; }
  .obs-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --obs-y y gira en --obs-giro: son
  ...
  .obs-scroller [data-xin] { transform: translate3d(0, var(--obs-y, 0px), 0) rotate(var(--obs-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .obs-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .obs-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .obs-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .obs-kicker--tinta { color: var(--pp-ink); }
  .obs-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--obs-serif), 'Cormorant Garamond', serif;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .obs-h2 em, .obs-acento { font-style: italic; color: var(--pp-acc); }
  .obs-h2--claro { color: var(--pp-night-ink); }
  .obs-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .obs-acento-plano { color: var(--pp-acc); }
  .obs-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .obs-parrafo--tinta { color: var(--pp-ink); }
  .obs-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .obs-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .obs-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .obs-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .obs-std-dia { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-weight: 500;
  ...
  .obs-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .obs-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .obs-std-anio { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .obs-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .obs-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .obs-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .obs-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .obs-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .obs-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .obs-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .obs-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .obs-cuenta-caja--1 { --obs-giro: -1.6deg; transform: rotate(var(--obs-giro)); }
  .obs-cuenta-caja--2 { --obs-giro: 1.2deg; transform: rotate(var(--obs-giro)); margin-top: 10px; }
  .obs-cuenta-caja--3 { --obs-giro: 1.8deg; transform: rotate(var(--obs-giro)); }
  .obs-cuenta-caja--4 { --obs-giro: -1.1deg; transform: rotate(var(--obs-giro)); margin-top: 10px; }
  .obs-cuenta-num { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .obs-cuenta-num--acc { color: var(--pp-acc); }
  .obs-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .obs-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .obs-frase { position: relative; z-index: 1; margin: 0; font-family: var(--obs-serif), 'Cormorant Garamond', serif;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .obs-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .obs-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .obs-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .obs-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .obs-panel--abajo { justify-content: flex-end; }
  .obs-panel--centro { justify-content: center; }
  .obs-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .obs-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .obs-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .obs-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .obs-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .obs-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .obs-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .obs-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .obs-tarjeta--izq { --obs-giro: -1deg; transform: rotate(var(--obs-giro)); }
  .obs-tarjeta--der { --obs-giro: 1.1deg; transform: rotate(var(--obs-giro)); }
  .obs-tarjeta--talon { --obs-giro: -.8deg; transform: rotate(var(--obs-giro)); overflow: visible; }
  .obs-tarjeta--hoy { --obs-giro: -1deg; transform: rotate(var(--obs-giro)); align-items: flex-start; }
  .obs-tarjeta--banco { margin-top: 4px; }
  .obs-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .obs-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .obs-tarjeta-titulo { margin: 0; font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .obs-tarjeta-pregunta { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: 26px; line-height: 1.15; }
  .obs-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .obs-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .obs-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .obs-fila > span:first-child { color: var(--pp-ink2); }
  .obs-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .obs-fila-valor { text-align: right; }
  .obs-fila--copiable { align-items: center; }
  .obs-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .obs-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .obs-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .obs-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .obs-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .obs-checkin { background: var(--pp-bg); }
  .obs-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .obs-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .obs-campo { display: flex; flex-direction: column; gap: 6px; }
  .obs-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .obs-contador { display: flex; align-items: center; gap: 12px; }
  .obs-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .obs-contador button:disabled { opacity: .35; cursor: default; }
  .obs-contador > span { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .obs-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--obs-sans), 'Jost', sans-serif; font-size: 16px; padding: 6px 2px; }
  .obs-input--serif { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: 20px; }
  .obs-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .obs-input::placeholder { color: #9A9388; }
  .obs-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .obs-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .obs-precio-total { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .obs-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .obs-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .obs-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .obs-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .obs-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--obs-sans), 'Jost', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .obs-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .obs-btn-solido:disabled { opacity: .6; cursor: default; }
  .obs-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .obs-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --obs-giro: -1deg; transform: rotate(var(--obs-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .obs-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .obs-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .obs-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .obs-crono-hora { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: 26px; min-width: 64px; line-height: 1; }
  .obs-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .obs-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .obs-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .obs-polaroids[data-cantidad="1"] .obs-polaroid { grid-column: span 6; }
  .obs-polaroids[data-cantidad="2"] .obs-polaroid { grid-column: span 3; }
  .obs-polaroids[data-cantidad="4"] .obs-polaroid { grid-column: span 3; }
  .obs-polaroids[data-cantidad="5"] .obs-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .obs-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .obs-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .obs-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .obs-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .obs-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .obs-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .obs-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .obs-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .obs-lista-tema { font-size: 15px; }
  .obs-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .obs-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .obs-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .obs-opciones { display: flex; flex-direction: column; gap: 8px; }
  .obs-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--obs-sans), 'Jost', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .obs-opcion:disabled { cursor: default; }
  .obs-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .obs-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .obs-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .obs-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --obs-giro: -.6deg; transform: rotate(var(--obs-giro));
    border-radius: 12px; overflow: hidden; }
  .obs-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .obs-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .obs-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .obs-ticket-nombre { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .obs-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .obs-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .obs-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .obs-ticket-muesca--arriba { top: -12px; }
  .obs-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .obs-ticket-der .qr-ingreso, .obs-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .obs-info-extra { position: relative; z-index: 1; }
  .obs-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .obs-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .obs-raiz .ia-icon-box { display: none !important; }
  .obs-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .obs-pase-firma { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .obs-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .obs-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }
  ...
  .obs-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .obs-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  ...
  .obs-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .obs-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .obs-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .obs-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .obs-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .obs-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .obs-cartel-nombres { margin: 0; font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-weight: 500;
  ...
  .obs-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .obs-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .obs-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .obs-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .obs-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .obs-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .obs-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .obs-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--obs-sans), 'Jost', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .obs-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .obs-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .obs-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .obs-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .obs-section, .obs-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .obs-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .obs-raiz { font-family: var(--obs-sans), 'Jost', sans-serif; }
  ...
  .obs-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .obs-kicker, .obs-panel-top, .obs-panel-pie, .obs-meta, .obs-riel-top, .obs-riel-etiqueta,
  .obs-ticket-kicker, .obs-cuenta-etq, .obs-tarjeta-kicker, .obs-fila-etq, .obs-portada-pase {
    font-family: var(--obs-mono), 'IBM Plex Mono', monospace; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .obs-h2, .obs-frase, .obs-tarjeta-titulo, .obs-std-dia, .obs-cartel-nombres, .obs-ticket-nombre, .obs-cifra, .obs-cuenta-num {
    font-family: var(--obs-serif), 'Cormorant Garamond', serif; letter-spacing: -.03em; }
  .obs-h2 em, .obs-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .obs-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .obs-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .obs-std-fila { justify-content: space-between; }
  .obs-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .obs-std-anio { font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .obs-std .obs-kicker, .obs-std .obs-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .obs-foto-marco { border: 3px solid var(--pp-ink); }
  .obs-ventana { clip-path: none !important; box-shadow: none; }
  .obs-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--obs-serif), 'Cormorant Garamond', serif; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .obs-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .obs-tarjeta--izq, .obs-tarjeta--der, .obs-tarjeta--talon, .obs-tarjeta--hoy { transform: none; }
  .obs-doblez::before { display: none; }
  .obs-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .obs-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .obs-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .obs-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .obs-cuenta-caja:first-child { border-left: none; }
  .obs-cuenta-caja::before { display: none; }
  .obs-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .obs-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .obs-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .obs-panel[data-tone="dark"] .obs-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .obs-panel-top, .obs-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .obs-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .obs-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .obs-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .obs-btn-solido, .obs-btn-papel, .obs-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .obs-btn-papel { background: transparent; border: 1px solid currentColor; }
  .obs-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .obs-portada { background: var(--pp-bg); }
  .obs-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .obs-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .obs-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .obs-cartel-chinche { display: none; }
  .obs-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .obs-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .obs-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Observatorio: la trama son estrellas, no puntos de imprenta. */
  .obs-trama { opacity: .35; background-image: radial-gradient(currentColor .6px, transparent .7px), radial-gradient(currentColor .4px, transparent .5px); background-size: 46px 46px, 23px 23px; background-position: 0 0, 12px 8px; }

  @media (prefers-reduced-motion: reduce) {
    .obs-raiz * { animation: none !important; }
    .obs-scroller [data-xin] { opacity: 1 !important; --obs-y: 0px; }
  }
`;
