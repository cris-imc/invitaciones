 ...
 * (mockup/nuevo, botón "Ónix"): fondo negro mármol (#140B14) + tinta clara
 * rosa-hueso (#F1E4E6) + acento oro-rosado fijo (#B48CD9), tipografía
 ...
 * foto de portada + un spotlight cálido que la recorre en diagonal al
 * hacer scroll (como una luz de gala/alfombra roja).
...
const IconMusic = ({ className, style }: { className?: string; style?: React.CSSProperties } = {}) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.3} className={className} style={style} aria-hidden="true"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>;
...
// Destello de cuatro puntas ("sparkle") de trazo fino -- decorativa, usada
// en portada/hero y como acento de gala junto a los títulos.
const IconRings = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1} className={className} style={style} aria-hidden="true">
    ...
    <path d="M19 15c.3 1.6.9 2.2 2.5 2.5-1.6.3-2.2.9-2.5 2.5-.3-1.6-.9-2.2-2.5-2.5 1.6-.3 2.2-.9 2.5-2.5Z" strokeWidth={0.8} />
  </svg>
);
// Luna creciente doodle -- coherente con "se viene la noche", divisor de
// secciones y acento de cover.
const IconHeartDoodle = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.1} className={className} style={style} aria-hidden="true">
    <path d="M15.5 3.5a8.5 8.5 0 1 0 0 17c-3.6-1.6-6-5-6-8.5s2.4-6.9 6-8.5Z" />
  </svg>
);
// Ramita de laurel dorada doodle -- separador de secciones, tallo con hojas
// alternadas de trazo fino.
const IconRibbon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 30 16" fill="none" stroke="currentColor" strokeWidth={1} className={className} style={style} aria-hidden="true">
    <path d="M1 8h28" />
    <path d="M7 8c0-2.4 1.6-3.7 3.6-4M7 8c0 2.4 1.6 3.7 3.6 4" />
    <path d="M14 8c0-2.4 1.6-3.7 3.6-4M14 8c0 2.4 1.6 3.7 3.6 4" />
    <path d="M21 8c0-2.4 1.6-3.7 3.6-4M21 8c0 2.4 1.6 3.7 3.6 4" />
  </svg>
);
// Insignia circular ("badge") con gema facetada al centro -- para la tarjeta
// de Ceremonia/Fiesta y el sello de portada, motivo "Noir & Oro".
const IconChurch = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1} className={className} style={style} aria-hidden="true">
    <circle cx="12" cy="12" r="10" />
    <circle cx="12" cy="12" r="7.2" strokeWidth={0.7} />
    <path d="M9 10 12 7l3 3-3 8-3-8Z" strokeWidth={0.9} />
  </svg>
);

// Tipografía del sistema "Noir & Oro" (Italiana display + Cormorant
// Garamond texto/citas), escopeada solo a este componente vía CSS var
// override en el wrapper raíz — no toca layout.tsx ni las demás plantillas
...
interface OnixTemplateAmatistaProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#B48CD9]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#B48CD9] uppercase tracking-wider mb-0.5">{label}</span>
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        ...
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#F1E4E6" }}>
          RESPONDISTE {score} DE {preguntas.length} CORRECTAMENTE ({percent}%)
        </p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#B9A6B4" }}>Guardando tus resultados...</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#B48CD9] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#F1E4E6" }}>
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#F1E4E6] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function OnixTemplateAmatista({ invitation, guest, isPersonalized = false }: OnixTemplateAmatistaProps) {
  ...
  // Entrada animada de los doodles de la portada (sparkle, luna, laurel) con
  ...
  useEffect(() => {
    ...
    animate(root.querySelectorAll(".onix-doodle"), {
      scale: [0, 1],
      rotate: [-15, 0],
      opacity: [0, 1],
      duration: 900,
    ...
    animate(root.querySelectorAll(".onix-seal"), {
      scale: [0.6, 1],
      opacity: [0, 1],
      duration: 700,
      delay: 150,
  ...
  useEffect(() => {
    ...
    const els = document.querySelectorAll(".onix-scroll-doodle");
  ...
  // Spotlight de gala: un haz de luz cálido que recorre la foto de portada
  // en diagonal al hacer scroll -- como una luz de alfombra roja pasando,
  // ligado al progreso de scroll (onScroll de anime.js).
  ...
  useEffect(() => {
    ...
    const observer = onScroll({
      ...
      onUpdate: (self) => {
        ...
        const x = p * 150 - 25; // % a lo largo de la diagonal, de punta a punta
        sheen.style.opacity = String(intensity * 0.6);
        sheen.style.transform = `translateX(${x}%) rotate(14deg)`;
      },
    });
  ...
  // Portada animada -- mismo criterio que la base de Onix: tinte +
  // effect="enfoque", acento propio de esta variante + ciruela compartido,
  // scrimColorRgb = rgb(#140B14), igual en todas (bg no cambia por variante).
  const portadaImagenFondoDesktopRaw = String(invitation.portadaImagenFondoDesktop ?? "") || undefined;
  const portadaFondoAnimado = Boolean(portadaImagenFondoDesktopRaw);
  const portadaTintColor1 = "#B48CD9";
  ...
  return (
    <div
      className={`${onixItaliana.variable} ${onixCormorant.variable}`}
      style={{
        "--font-cormorant": "var(--onix-italiana)",
        "--font-inter": "var(--onix-cormorant)",
        "--font-sans": "var(--onix-cormorant)",
        "--t-acc": "#B48CD9",
        "--t-acc2": "#B48CD9",
        "--c-accent": "#B48CD9",
        "--t-bg": "#140B14",
    ...
    >
      <style>{`
        ...
        #countdown.dark {
          background-color: #140B14 !important;
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(180, 140, 217, 0.2) !important;
        }

        /* RSVP Custom Aesthetics for OnixTemplateAmatista */
        #rsvp.section.dark {
          background-color: #140B14 !important;
          color: #FFFFFF !important;
        ...
        #rsvp.section.dark label {
          ...
          color: #B9A6B4 !important;
          font-weight: 600 !important;
        }
        #rsvp.section.dark input {
          background-color: #1C141C !important;
          color: #FFFFFF !important;
          border-radius: 6px !important;
          border: 1px solid rgba(180, 140, 217, 0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #B48CD9 !important;
          color: #140B14 !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #B48CD9 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #140B14 !important; /* Matches light sections */
          color: #FFFFFF !important;
      ...
      {/* PORTADA / WELCOME OVERLAY (mesh negro-mármol + amatista animado, glow pulsante) */}
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
        ...
        >
          ...
          <div style={{
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(180,140,217,0.13), transparent), radial-gradient(45% 40% at 85% 80%, rgba(120,70,110,0.2), transparent)',
            backgroundSize: '160% 160%',
            animation: 'onix-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(180,140,217,0.13), transparent 70%)',
          ...
          <IconRings className="onix-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '10%', left: '10%', color: 'rgba(180,140,217,0.55)' }} />
          <IconHeartDoodle className="onix-doodle opacity-0 absolute" style={{ width: 24, height: 24, top: '15%', right: '12%', color: 'rgba(180,140,217,0.4)' }} />
          <IconRibbon className="onix-doodle opacity-0 absolute" style={{ width: 34, height: 18, bottom: '18%', left: '12%', color: 'rgba(180,140,217,0.45)' }} />
          <IconRings className="onix-doodle opacity-0 absolute" style={{ width: 14, height: 14, bottom: '24%', right: '18%', color: 'rgba(180,140,217,0.4)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="onix-seal opacity-0" style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #B48CD9',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#B48CD9',
            }}>
              {monogram}
            </div>

            {/* Guest Name */}
            <h2 className="text-4xl sm:text-5xl font-light tracking-wide text-[#F1E4E6] leading-relaxed" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className=" text-sm font-medium text-[#B9A6B4] tracking-wide uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8 }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #B48CD9', color: '#B48CD9',
                background: 'rgba(180,140,217,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#B48CD9'; e.currentTarget.style.color = '#140B14'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(180,140,217,0.08)'; e.currentTarget.style.color = '#B48CD9'; }}
            >
              ABRIR INVITACIÓN
            </button>
          ...
          <style jsx>{`
            @keyframes onix-meshDrift { 0%, 100% { background-position: 0% 0%, 100% 100%; } 50% { background-position: 30% 20%, 70% 80%; } }
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#B48CD9]/40 shadow-md ${isTicketMaximized ? 'bg-[#140B14]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#FFFFFF]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#B48CD9] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#FFFFFF] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#B48CD9]/20 pl-3">
                <span className="text-[#FFFFFF] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#B9A6B4] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? 'Lugar' : 'Lugares'}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#B48CD9]" />
              <span className="text-[#231A22]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <div className="desktop-stage" data-theme={theme} style={{
        ...
        // el de mobile) necesita las mismas CSS vars del tema -- ver
        // sección 3.2 de la guía técnica (trampa ya conocida).
        "--t-acc": "#B48CD9",
        "--t-acc2": "#B48CD9",
        "--c-accent": "#B48CD9",
        "--t-bg": "#140B14",
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#B48CD9', margin: '6px 0 14px', animation: 'onix-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#140B14]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#140B14] z-10 relative">
            <IconHeartDoodle className="onix-scroll-doodle opacity-0 absolute" style={{ width: 16, height: 16, top: 12, right: 28, color: 'rgba(180,140,217,0.4)' }} />
            <IconRings className="onix-scroll-doodle opacity-0 absolute" style={{ width: 16, height: 16, top: 64, right: 52, color: 'rgba(180,140,217,0.45)' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#B48CD9] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#F1E4E6] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#B48CD9' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#B48CD9', margin: '0 0 20px', animation: 'onix-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#B9A6B4] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#B48CD9] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          
          {/* Image Container -- marco dorado completo + spotlight de gala
              ligado al scroll (ver useEffect de heroPhotoRef). */}
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #140B14 100%)' }} />
            <div
              ...
              style={heroBgMobile ? {
                ...
                backgroundRepeat: "no-repeat"
              } : { backgroundColor: '#231A22' }}
            />
            {/* Marco dorado completo + insignia circular "saliendo" de una
                esquina, motivo Noir & Oro. */}
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              <div className="absolute inset-0" style={{ border: "1px solid rgba(180,140,217,0.6)" }} />
              <IconChurch style={{ position: "absolute", top: -14, right: -14, width: 28, height: 28, color: "#B48CD9" }} />
            </div>
            {/* Spotlight de gala: un haz de luz cálido en diagonal, único
                (sin reflejos fantasma), ligado al progreso de scroll. */}
            <div
              ref={heroSheenRef}
              className="pointer-events-none z-20"
              style={{
                position: "absolute", top: "-30%", left: "-15%", width: "40%", height: "160%",
                background: "linear-gradient(90deg, transparent, rgba(180,140,217,0.5) 45%, rgba(255,238,225,0.6) 50%, rgba(180,140,217,0.5) 55%, transparent)",
                opacity: 0, transform: "translateX(-30%) rotate(14deg)",
              }}
            />
          </div>
        </div>
        ...
        {/* Divisor doodle entre hero y cuenta regresiva: ramita de laurel
            flanqueada por sparkles, motivo Noir & Oro. */}
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#140B14]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #B48CD9, transparent)' }} />
          <IconRibbon className="onix-scroll-doodle opacity-0" style={{ width: 26, height: 14, color: '#B48CD9' }} />
          <IconRings className="onix-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#B48CD9' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #B48CD9, transparent)' }} />
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #78468E14, transparent 70%), #231A22" }}>
            <div className="max-w-2xl mx-auto text-center">
              <TypewriterText 
                text={`"${String(invitation.frasePersonalizadaTexto)}"`}
                className="text-[#F1E4E6] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#140B14] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#B48CD9]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              <IconChurch className="onix-scroll-doodle opacity-0" style={{ width: 15, height: 15 }} />
              CUÁNDO Y DÓNDE
            </p>
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#B48CD9] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#B9A6B4] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  ...
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#B9A6B4]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#B9A6B4]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#B48CD9] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#B48CD9] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#B9A6B4] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#B48CD9] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#B48CD9]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <IconRibbon className="onix-scroll-doodle opacity-0" style={{ width: 20, height: 11 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#B48CD9]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#B9A6B4] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#231A22] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#1C141C] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#B48CD9]">
                  <IconChurch className="onix-scroll-doodle opacity-0" style={{ width: 16, height: 16 }} />
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #78468E18, transparent 70%), #1C141C" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#1C141C' }} aria-hidden="true">
            <IconMusic className="onix-scroll-doodle opacity-0" style={{ width: 24, height: 24, color: '#B48CD9' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        {/* Separador doodle de footer: laurel + sparkle, motivo Noir & Oro. */}
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#140B14' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #B48CD9, transparent)' }} />
          <IconRings className="onix-scroll-doodle opacity-0" style={{ width: 12, height: 12, color: '#B48CD9' }} />
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #B48CD9, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#140B14" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#B48CD9" surfaceColor="#1C141C" />}
      </div>
  );
}
