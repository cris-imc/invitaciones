 ...
 *   --t-bg #F3EEF2 · --t-surface #EDE4EC · --seda-ink #2A2029
 *   --t-muted #82727E · --t-acc #6B4A6B (rosa polvo) · --t-acc2 #B98D57 (oro champagne)
 *
...
interface SedaTemplateCiruelaProps {
...
export function SedaTemplateCiruela({ invitation, guest, isPersonalized = false }: SedaTemplateCiruelaProps) {
  ...
  // Portada animada -- Seda es CSS-vars, tema claro (--t-bg #F3EEF2, ink
  // #2A2029) -- mismo caso que Perlada/Petalos: texto no puede forzarse a
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #FFFFFF 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #2A2029)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #2A2029)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#6B4A6B" surfaceColor="#EDE4EC" inactiveColor="#2A2029" solid />}
      </div>
  );
}
