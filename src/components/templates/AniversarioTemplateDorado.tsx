  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#130C09", tinta: "#F4F1EA", acento: "#C9A45C", clase: `${anvDisplay.variable} ${anvMono.variable}`, fuente: "var(--anv-mono), monospace" }}
      />
    );
  }
      ...
      <div ref={scrollerRef} data-scroller="1" className="anv-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="anv-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-drift="-70" className="anv-medallion anv-medallion--corner">
            <AniversarioTemplateDoradoMedallionCmp label="20" sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="anvArc1" arcText={tx("invitacion.sabor.arcoAniversario").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="anv-section anv-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2E1E14 0%, #0D0B10 55%, #130C09 100%)" }}>
          ...
          <div className="anv-cd-grid">
            <AniversarioTemplateDoradoCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <AniversarioTemplateDoradoCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <AniversarioTemplateDoradoCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <AniversarioTemplateDoradoCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="anv-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="anv-pan-sticky">
            ...
            <AniversarioTemplateDoradoDots count={photoPages.length} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="anv-section anv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="anv-final-card">
            <div className="anv-medallion anv-medallion--final">
              <AniversarioTemplateDoradoMedallionCmp label="20" sub={confirmed ? "CONFIRMADO" : tx("invitacion.pase.pendiente").toUpperCase()} arcId="anvArc3" arcText={textoArco(namesTitle, fechaCorta)} spin="reverse" />
            </div>
...
const GP_CSS = `
  ...
  .anv-cover-names { margin: 0; font-family: var(--anv-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F6ECD4; }
