 ...
 * (#0D0D10) + acentos neón cian (#B6FF3C) y magenta (#FF2E9B), tipografía
...
const PhotoCornerFrame = () => (
  <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
    <div className="absolute inset-0" style={{ border: "1px solid rgba(182,255,60,0.55)" }} />
...
interface NeonTemplateVerdeProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#B6FF3C]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#B6FF3C] uppercase tracking-wider mb-0.5">{label}</span>
          ...
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#B6FF3C] shrink-0" />
...
export function NeonTemplateVerde({ invitation, guest, isPersonalized = false }: NeonTemplateVerdeProps) {
  const tx = useTextos();
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(182,255,60,0.2) !important;
        }

        /* RSVP Custom Aesthetics for NeonTemplateVerde */
        #rsvp.section.dark {
          background-color: #0D0D10 !important;
          color: #FFFFFF !important;
        ...
        #rsvp.section.dark input {
          ...
          border: 1px solid rgba(182,255,60,0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #B6FF3C !important;
          color: #0D0D10 !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #B6FF3C !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div style={{
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(182,255,60,0.13), transparent), radial-gradient(45% 40% at 85% 80%, rgba(255,46,155,0.2), transparent)',
            backgroundSize: '160% 160%',
            animation: 'neon-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(182,255,60,0.13), transparent 70%)',
          ...
          <IconDiscoBall className="neon-doodle opacity-0 absolute" style={{ width: 34, height: 34, top: '10%', left: '12%', color: 'rgba(182,255,60,0.5)' }} />
          ...
          <IconDiscoBall className="neon-doodle opacity-0 absolute" style={{ width: 30, height: 30, bottom: '16%', left: '18%', color: 'rgba(182,255,60,0.35)' }} />
          <IconSpark className="neon-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '30%', right: '20%', color: '#FF2E9B' }} />
          <IconSpark className="neon-doodle opacity-0 absolute" style={{ width: 16, height: 16, bottom: '22%', right: '28%', color: '#B6FF3C' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #B6FF3C', color: '#B6FF3C',
                background: 'rgba(182,255,60,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#B6FF3C'; e.currentTarget.style.color = '#0D0D10'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(182,255,60,0.08)'; e.currentTarget.style.color = '#B6FF3C'; }}
            >
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#B6FF3C]/40 shadow-md ${isTicketMaximized ? 'bg-[#0D0D10]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#FFFFFF]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#B6FF3C] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#B6FF3C] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#B6FF3C]/20 pl-3">
            ...
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#B6FF3C]" />
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#B6FF3C', margin: '6px 0 14px', animation: 'neon-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#0D0D10]">
          ...
          <div className="px-8 pt-16 pb-12 text-left bg-[#0D0D10] z-10 relative">
            ...
            <IconSpark className="neon-scroll-doodle opacity-0 absolute" style={{ width: 16, height: 16, top: 60, right: 56, color: '#B6FF3C' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#B6FF3C] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#F2F2F5] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#B6FF3C' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#B6FF3C', margin: '0 0 20px', animation: 'neon-lineExpand 1.2s ease-out' }} />
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#B6FF3C] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#0D0D10] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#B6FF3C]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#B6FF3C] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#B6FF3C] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#B6FF3C] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8F8F98] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#B6FF3C] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#B6FF3C]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#B6FF3C]/10 last:border-b-0">
                      {item.time && (
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#111114] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                ...
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#B6FF3C]">
        ...
        <div className="w-full flex justify-center py-6" style={{ background: '#0D0D10' }} aria-hidden="true">
          <svg className="neon-scroll-doodle opacity-0" viewBox="0 0 90 12" width="90" height="12" stroke="rgba(182,255,60,0.6)" strokeWidth="1.4">
      ...
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#B6FF3C" surfaceColor="#141418" />}
      </div>
  );
}
