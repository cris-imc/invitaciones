 ...
 * (#FFF7F2) + tinta violeta oscuro (#2A2140) + acento coral (#4FB8E8,
 * variable por color) + acento lavanda (#FF5C8A) + acento menta (#3FBF9F),
...
interface InfantilTemplateCelesteProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#4FB8E8]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#4FB8E8] uppercase tracking-wider mb-0.5">{label}</span>
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        ...
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#2A2140" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>

        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#8478A0" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,92,138,0.08)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,92,138,0.18)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#4FB8E8] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#2A2140" }}>
                  {tx("invitacion.quiz.promedioGlobal", { n: stats.count })} <strong style={{ color: "#2A2140" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
...
export function InfantilTemplateCeleste({ invitation, guest, isPersonalized = false }: InfantilTemplateCelesteProps) {
  const tx = useTextos();
          ...
          <div className="rounded-[2rem] bg-black/40 border border-white/10 shadow-2xl backdrop-blur-3xl text-center max-w-4xl mx-auto relative overflow-hidden flex flex-col">
            ...
            <div className="p-10 md:p-16 space-y-8">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif font-light text-white tracking-wide drop-shadow-md">
                Un festejo <AnimatedSynonyms words={[tx("invitacion.frase.inolvidable"), tx("invitacion.frase.unico"), "divertido", tx("invitacion.frase.magico")]} className="italic text-pink-200/90 font-serif" />
              </h1>
    ...
    >
      <style>{`
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #4FB8E8 !important;
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div className={portadaFondoAnimado ? "acp-desktop-only" : undefined}>
            <div style={{ position: 'absolute', width: 200, height: 200, borderRadius: '50%', background: '#4FB8E8', opacity: 0.16, top: '6%', left: '-8%', filter: 'blur(14px)', animation: 'infantil-blobFloat 7s ease-in-out infinite', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', width: 170, height: 170, borderRadius: '50%', background: '#FF5C8A', opacity: 0.18, bottom: '8%', right: '-6%', filter: 'blur(14px)', animation: 'infantil-blobFloat2 8s ease-in-out infinite', pointerEvents: 'none' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <button
              ...
              style={{
                ...
                background: '#4FB8E8', boxShadow: '0 10px 24px rgba(255,92,138,0.35)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#E8447A'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = '#4FB8E8'; }}
            >
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#4FB8E8]/40 shadow-md rounded-full ${isTicketMaximized ? 'bg-[#FFF7F2]/95 backdrop-blur-md w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#241B38]/95 backdrop-blur-md px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-bold uppercase tracking-[0.15em] text-[#4FB8E8] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-bold uppercase tracking-[0.15em] text-[#4FB8E8] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#4FB8E8]/20 pl-3">
                <span className="text-[#2A2140] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#8478A0] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#4FB8E8]" />
              <span className="text-[#FFFFFF]  text-[10px] font-bold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
          ...
          <h1 className="text-5xl font-extrabold text-white leading-tight mb-2 drop-shadow-md" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), sans-serif' }}>
            {em ? (
              <>
                ...
                <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), sans-serif', fontStyle: 'normal', color: '#4FB8E8' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
              </>
            ) : (
              <span className="block">{title}</span>
            )}
          </h1>
          <div style={{ width: 40, height: 4, background: '#4FB8E8', borderRadius: 4, margin: '6px 0 14px' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#FFF7F2]">
          <div className="px-8 pt-16 pb-12 text-left bg-[#FFF7F2] z-10 relative">
            ...
            <p className=" text-xs font-bold uppercase tracking-[0.15em] text-[#4FB8E8] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            ...
            <div style={{ width: 40, height: 4, background: '#4FB8E8', borderRadius: 4, margin: '0 0 20px' }} />
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-bold text-[#4FB8E8] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              ...
              <IconStar style={{ position: "absolute", top: -12, left: -8, width: 24, height: 24, color: "#4FB8E8" }} />
              <IconBalloon style={{ position: "absolute", bottom: -14, right: -6, width: 20, height: 26, color: "#FF5C8A" }} />
            </div>
        ...
        {(invitation.contadorHabilitado ?? true) ? (
          <Countdown
            ...
            title={tipo === "CASAMIENTO" ? tx("invitacion.cuentaRegresiva.faltanPoquitosDias") : tx("invitacion.sabor.laCuentaYaEmpezoExclam")}
            dark
          />
        ) : null}

        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center relative" style={{ background: "#FF5C8A1A" }}>
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#FFF7F2] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-bold tracking-[0.15em] uppercase text-[#4FB8E8]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              ...
              <div className="bg-white border-l-[4px] border-l-[#3FBF9F] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-bold tracking-wider text-[#4FB8E8] hover:text-[#2A2140] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-white border-l-[4px] border-l-[#3FBF9F] p-6 sm:p-8 mb-10 shadow-sm">
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-bold tracking-wider text-[#4FB8E8] hover:text-[#2A2140] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-bold tracking-[0.15em] uppercase text-[#4FB8E8]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b-2 border-dashed border-[#FF5C8A]/25 last:border-b-0">
                      {item.time && (
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#FFF0E4] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                ...
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#4FB8E8]">
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#FFF7F2' }} aria-hidden="true">
          <IconConfetti className="infantil-scroll-doodle opacity-0" style={{ width: 24, height: 12, color: '#FF5C8A' }} />
        </div>
      ...
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#4FB8E8" surfaceColor="#FFFFFF" />}
      </div>
  );
}
