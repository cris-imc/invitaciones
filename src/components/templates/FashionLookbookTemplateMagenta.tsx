...
export function FashionLookbookTemplateMagenta({ invitation, guest, isPersonalized = false }: FashionLookbookTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#D93A93";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(217,58,147,.35), 0 18px 50px -30px #D93A93";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#0B0B0B" : "#D93A93";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A0709", tinta: "#F4F1EA", acento: "#D93A93", clase: `${flbArchivo.variable} ${flbMono.variable}`, fuente: "var(--flb-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="flb-scroller">
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="flb-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="flb-pan-sticky">
            <div data-strip="1" className="flb-strip">
              ...
              <div data-tone="dark" className="flb-panel flb-panel--center" style={{ background: "#110B12", color: "#F4F1EA" }}>
                <div className="flb-medallion flb-medallion--lg">
                  <FlbMedallion kicker={tx("invitacion.pase.sector").toUpperCase()} main={tx("invitacion.pase.pasarela")} sub="LOOK 04" subColor="#D93A93" arcId="flbArc2" arcText={tx("invitacion.pase.arcoAccesoVipLook", { n: passNumber }).toUpperCase()} spin="reverse" />
                </div>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="flb-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="flb-pan-sticky">
            <div data-strip="1" className="flb-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="flb-panel flb-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="flb-seguir flb-seguir--split">
                    ...
                    <span className="flb-accent-plain">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <FlbDots count={photoPages.length} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="flb-section" style={{ background: "#110B12" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="flb-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="flb-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F5F2ED" : "#D93A93" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="flb-section" style={{ background: "#110B12" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="flb-section" style={{ background: "#110B12" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu look" className="flb-section flb-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))" }}>
          <span data-xin="1" data-dist="-60" className="flb-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection, quizEnabled].filter(Boolean).length + 6)} — {tx("invitacion.kicker.guarda")} TU LOOK</span>
      ...
      <div ref={coverRef} className="flb-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#D93A93" guest={guest} />
      )}
...
const FLB_CSS = `
  ...
  a { color: #D93A93; text-decoration: none; }
  ...
  .flb-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; background: radial-gradient(120% 80% at 50% 0%, #241528 0%, #110B12 55%, #0A0709 100%); }
  ...
  @media (min-width: 768px) {
    .flb-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(217,58,147,.3); }
    .flb-hero-photo-kicker { bottom: 40px; }
    .flb-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }
  .flb-section:nth-of-type(2) { background: radial-gradient(100% 60% at 50% 100%, #221019 0%, #130A11 55%, #0A0709 100%); }
  .flb-section:nth-of-type(3) { background: radial-gradient(130% 90% at 86% 16%, #271730 0%, #160D17 52%, #0A0709 100%); }
  ...
  .flb-date-month { font-family: var(--flb-archivo), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #D93A93; padding-left: 12%; }

  .flb-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .flb-divider-line { width: 52px; height: 1px; background: #D93A93; display: inline-block; }
  ...
  .flb-cal-link:hover { color: #D93A93; }
  ...
  .flb-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #D93A93, #110B12, #D93A93, #F5F2ED, #D93A93); filter: saturate(.75); }
  .flb-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #160D17; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .flb-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #D93A93; font-family: var(--flb-mono), monospace; }
  ...
  .flb-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #D93A93; }
  .flb-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A0709 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .flb-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(217,58,147,0.24), rgba(17,11,18,0.3), rgba(217,58,147,0.36), rgba(217,58,147,0.24)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .flb-pan[data-scroll="vertical"] { height: auto; }
  .flb-pan[data-scroll="vertical"] .flb-pan-sticky { position: static; height: auto; overflow: visible; }
  .flb-pan[data-scroll="vertical"] .flb-strip { position: static; display: block; width: 100%; transform: none !important; }
  .flb-pan[data-scroll="vertical"] .flb-panel { height: calc(var(--vh, 1vh) * 100); }
  ...
  .flb-crono-time { font-family: var(--flb-mono), monospace; color: #D93A93; min-width: 42px; }
  ...
  .flb-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(217,58,147,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .flb-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #D93A93; background: transparent; color: #D93A93; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .flb-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(217,58,147,0.3); color: #F4F1EA; font-family: var(--flb-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .flb-rsvp-input::placeholder { color: #6E6A5D; }
  .flb-rsvp-input:focus { outline: none; border-bottom-color: #D93A93; }
  .flb-rsvp-btn { width: 100%; padding: 17px 0; font-family: var(--flb-mono), monospace; font-size: 11.5px; font-weight: 600; letter-spacing: 0.26em; text-transform: uppercase; border: 1px solid #D93A93; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 200ms ease, color 200ms ease; }
  ...
  .flb-rsvp-btn--ghost { background: transparent; color: #D93A93; }
  .flb-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(217,58,147,0.1); color: #D93A93; }
  .flb-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #D93A93; margin: 0; }
  ...
  .flb-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(217,58,147,0.18); }
  ...
  .flb-bank-copy { flex-shrink: 0; font-family: var(--flb-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #D93A93; background: transparent; color: #D93A93; cursor: pointer; }
  .flb-bank-copy:hover { background: rgba(217,58,147,0.12); }
  ...
  .flb-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(217,58,147,0.3); padding-bottom: 12px; }
  ...
  .flb-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(217,58,147,0.3); color: #F4F1EA; font-family: var(--flb-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .flb-song-input::placeholder { color: #6E6A5D; }
  .flb-song-input:focus { outline: none; border-bottom-color: #D93A93; }
  .flb-song-sep { color: #8A8577; flex-shrink: 0; }
  .flb-song-submit { flex-shrink: 0; background: none; border: none; color: #D93A93; font-family: var(--flb-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .flb-song-submit:hover { color: #F5F2ED; }
  .flb-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .flb-song-error { font-size: 10px; color: #D93A93; margin-top: 6px; }
  ...
  .flb-quiz-opt { font-family: var(--flb-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(217,58,147,0.4); background: transparent; color: #D93A93; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .flb-quiz-opt:disabled { cursor: default; }
  .flb-quiz-opt--picked { background: #D93A93; border-color: #D93A93; color: #0B0B10; }
  .flb-quiz-opt--correct { background: #D93A93; border-color: #D93A93; color: #0B0B10; }
  .flb-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .flb-quiz-result { padding-top: 18px; border-top: 1px solid rgba(217,58,147,0.2); }
  ...
  .flb-final-card { border: 1px solid #D93A93; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .flb-replay { cursor: pointer; color: #D93A93; }
  ...
  .flb-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F5F2ED, #D93A93); transition: height 260ms linear; display: block; }
  .flb-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #D93A93; transition: color 500ms ease; }
  ...
  .flb-cover-names { margin: 0; font-family: var(--flb-archivo), sans-serif; font-weight: 400; font-size: min(clamp(40px, 13vw, 78px), calc(10.5vh * var(--esc, 1))); line-height: 1.05; letter-spacing: -0.03em; color: #0B0B0B; }
  ...
  @media (hover: hover) {
    .flb-cover-cta--btn:hover { background: #D93A93; border-color: #D93A93; }
  }
  ...
  .flb-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #D93A93; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
