...
interface LuzLunaTemplatePerlaSuaveProps {
...
export function LuzLunaTemplatePerlaSuave({ invitation, guest, isPersonalized = false }: LuzLunaTemplatePerlaSuaveProps) {
  ...
  // Portada animada -- PerlaSuave es de paleta clara (--t-bg #F4F1F9, ink
  // #3A3450), mismo caso que Perlada: texto no puede forzarse a claro por
  // JS. Se resuelve con clases CSS + media query
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #0A0816 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #3A3450)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #3A3450)" />
        </div>
      </div>
