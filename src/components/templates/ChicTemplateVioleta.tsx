 ...
 * (#F6F1F8) + tinta oscura (#241E12) + dorado fijo (#8779A0) + acento oliva
 * (#8779A0, variable por color), tipografía itálica elegante (Playfair
...
interface ChicTemplateVioletaProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#8779A0]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#8779A0] uppercase tracking-wider mb-0.5">{label}</span>
          ...
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#8779A0] shrink-0" />
...
export function ChicTemplateVioleta({ invitation, guest, isPersonalized = false }: ChicTemplateVioletaProps) {
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#F6F1F8] via-[#ECE3EF] to-[#FDFCFE] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
  ...
  return (
    <div
      ...
      style={{
        ...
        "--t-surface": "#FDFCFE",
        "--t-muted": "#8A7A63",
        // Usado por Countdown.tsx/RSVPWizardV2.tsx (dark ? var(--chic-ink, #FDFCFE) : ...)
        ...
        // fallback (#FDFCFE/#EDE9F4) los deja exactamente como estaban.
        "--chic-ink": "#241E12",
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(135,121,160,0.2) !important;
        }

        /* RSVP Custom Aesthetics for ChicTemplateVioleta */
        #rsvp.section.dark {
          background-color: #F6F1F8 !important;
          color: #241E12 !important;
        ...
        #rsvp.section.dark input {
          ...
          border: 1px solid rgba(135,121,160,0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #8779A0 !important;
          color: #241E12 !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #8779A0 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #F6F1F8 !important; /* Matches light sections */
          color: #241E12 !important;
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div style={{
            ...
            background: 'radial-gradient(circle, rgba(135,121,160,0.13), transparent 70%)',
          ...
          <IconRings className="chic-doodle opacity-0 absolute" style={{ width: 40, height: 26, top: '9%', left: '10%', color: 'rgba(135,121,160,0.55)' }} />
          <IconHeartDoodle className="chic-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '15%', right: '12%', color: 'rgba(135,121,160,0.45)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <button
              ...
              style={{
                ...
                background: 'rgba(135,121,160,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#8779A0'; e.currentTarget.style.color = '#F6F1F8'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(135,121,160,0.08)'; e.currentTarget.style.color = '#8779A0'; }}
            >
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#8779A0]/40 shadow-md ${isTicketMaximized ? 'bg-[#F6F1F8]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#241E12]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#8779A0] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#8779A0] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#8779A0]/20 pl-3">
            ...
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#8779A0]" />
              <span className="text-[#FDFCFE]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#8779A0', margin: '6px 0 14px', animation: 'chic-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#F6F1F8]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#F6F1F8] z-10 relative">
            <IconHeartDoodle className="chic-scroll-doodle opacity-0 absolute" style={{ width: 18, height: 18, top: 12, right: 28, color: 'rgba(135,121,160,0.4)' }} />
            <IconRibbon className="chic-scroll-doodle opacity-0 absolute" style={{ width: 26, height: 15, top: 64, right: 52, color: 'rgba(135,121,160,0.4)' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#8779A0] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#241E12] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#8779A0' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#8779A0', margin: '0 0 20px', animation: 'chic-lineExpand 1.2s ease-out' }} />
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#8779A0] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #F6F1F8 100%)' }} />
            <div
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              <div className="absolute inset-0" style={{ border: "1px solid rgba(135,121,160,0.65)" }} />
              <IconRings style={{ position: "absolute", top: -10, left: -8, width: 30, height: 20, color: "#8779A0" }} />
              <IconHeartDoodle style={{ position: "absolute", bottom: -8, right: -6, width: 18, height: 18, color: "#8779A0" }} />
            </div>
            {/* Destello cálido tipo "sol en el lente" + 2 reflejos fantasma
                más chicos, todos ligados al progreso de scroll. */}
            <div ref={heroFlareRef} className="pointer-events-none z-20" style={{ position: "absolute", width: 90, height: 90, borderRadius: "50%", background: "radial-gradient(circle, rgba(255,247,224,0.9) 0%, rgba(135,121,160,0.35) 45%, transparent 72%)", opacity: 0, transform: "translate(-50%,-50%)" }} />
            <div ref={heroGhost1Ref} className="pointer-events-none z-20" style={{ position: "absolute", width: 30, height: 30, borderRadius: "50%", background: "radial-gradient(circle, rgba(135,121,160,0.5), transparent 70%)", opacity: 0, transform: "translate(-50%,-50%)" }} />
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #8779A014, transparent 70%), #FDFCFE" }}>
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#F6F1F8] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#8779A0]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#8779A0] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#8779A0] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#8779A0] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8A7A63] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#8779A0] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#8779A0]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#8779A0]/10 last:border-b-0">
                      {item.time && (
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#FDFCFE] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#ECE3EF] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <div className="flex justify-center mb-4">
                  <DrawLucideIcon icon={Landmark} size={46} color="var(--t-acc)" strokeWidth={1.5} />
                </div>
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#8779A0]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #8779A018, transparent 70%), #ECE3EF" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#F6F1F8' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #8779A0, transparent)' }} />
          <svg className="chic-scroll-doodle opacity-0" viewBox="0 0 16 12" width="16" height="12" fill="none" stroke="#8779A0" strokeWidth="1.1" aria-hidden="true">
            <path d="M8 11V2M8 5 4 3M8 5l4-2M8 8 4.5 6.3M8 8l3.5-1.7" />
          </svg>
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #8779A0, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#F6F1F8" textColor="var(--chic-ink, #241E12)" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#8779A0" surfaceColor="#FDFCFE" />}
      </div>
  );
}
