...
// NoirTemplate.tsx — no editar a mano: los cambios se pierden al regenerar.
...
export function NoirTemplateEsmeralda({ invitation, guest, isPersonalized = false }: NoirTemplateProps) {
  const tx = useTextos();
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="noi-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="noi-section noi-std">
          <div className="noi-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="noi-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="noi-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="noi-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="noi-std-fila">
              <span className="noi-cinta">{mesLargo}</span>
              <span className="noi-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="noi-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="noi-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="noi-btn-papel noi-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="noi-foto-seccion">
            <div className="noi-foto-marco">
              <div ref={ventanaRef} className="noi-ventana">
                {fotoMobile && (
              ...
              <div className="noi-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="noi-section noi-countdown">
          <div className="noi-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="noi-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="noi-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="noi-section noi-frase-seccion">
            <div className="noi-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="noi-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="noi-firma">
              <span className="noi-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="noi-pan-fijo">
            <div data-strip="1" className="noi-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="noi-panel noi-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="noi-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="noi-tarjeta noi-tarjeta--izq">
                  <span className="noi-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="noi-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="noi-filas">
                    {direccion && (
                      <div className="noi-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="noi-fila-valor">{direccion}</span></div>
                    )}
                    <div className="noi-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="noi-fila noi-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="noi-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="noi-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="noi-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="noi-panel noi-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="noi-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="noi-tarjeta noi-tarjeta--der">
                    <h2 className="noi-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="noi-filas">
                      {ceremoniaDireccion && (
                        <div className="noi-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="noi-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="noi-fila noi-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="noi-panel noi-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="noi-escena" aria-hidden="true">
                  ...
                  <h2 className="noi-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="noi-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="noi-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="noi-panel noi-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="noi-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="noi-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="noi-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="noi-crono-fila">
                        <span className="noi-crono-hora">{item.time || ""}</span>
                        <span className="noi-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="noi-section noi-checkin">
            <div className="noi-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="noi-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="noi-h2">
            ...
            <div className="noi-pan-fijo">
              <div data-strip="1" className="noi-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="noi-panel noi-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="noi-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="noi-h2 noi-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="noi-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="noi-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="noi-polaroid-img" />
                          {i % 2 === 0 && <span className="noi-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="noi-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="noi-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="noi-section noi-musica">
            <div className="noi-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="noi-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="noi-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="noi-section noi-regalos">
            <span data-xin="1" data-dist="-20" className="noi-kicker noi-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="noi-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="noi-parrafo noi-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="noi-section noi-quiz">
            <div className="noi-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="noi-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="noi-h2 noi-h2--claro">{triviaTitulo}</h2>
            <TriviaNoir
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="noi-section noi-pase">
          <div className="noi-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="noi-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="noi-ticket">
            <div className="noi-ticket-izq">
              <span className="noi-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="noi-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="noi-ticket-dato">
                {[
              ...
              <span className="noi-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="noi-ticket-muesca noi-ticket-muesca--arriba" aria-hidden="true" />
              <span className="noi-ticket-muesca noi-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="noi-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="noi-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="noi-pase-pie">
            <span className="noi-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="noi-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="noi-portada">
        ...
        <div ref={escenaPortadaRef} className="noi-portada-escena">
          <div data-cl="1" className="noi-trama" aria-hidden="true" />
        </div>
        <div className="noi-portada-contenido">
          <div className="noi-portada-arriba">
            <div ref={cartelRef} className="noi-cartel">
              <h1 className="noi-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="noi-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="noi-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="noi-portada-abajo">
            <div className="noi-portada-datos">
              <span className="noi-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="noi-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="noi-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="noi-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="noi-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="noi-portada-pase">
            ...
            <button type="button" onClick={abrir} className="noi-portada-btn">
...
function CuentaNoir({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="noi-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`noi-cuenta-caja noi-cuenta-caja--${i + 1}`}>
          <span className={`noi-cuenta-num${i === 3 ? " noi-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_NOI = `
  ...
  .noi-raiz a { color: var(--pp-acc); text-decoration: none; }
  .noi-raiz button { font: inherit; }
  .noi-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --noi-y y gira en --noi-giro: son
  ...
  .noi-scroller [data-xin] { transform: translate3d(0, var(--noi-y, 0px), 0) rotate(var(--noi-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .noi-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .noi-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .noi-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .noi-kicker--tinta { color: var(--pp-ink); }
  .noi-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--noi-serif), 'Playfair Display', serif;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .noi-h2 em, .noi-acento { font-style: italic; color: var(--pp-acc); }
  .noi-h2--claro { color: var(--pp-night-ink); }
  .noi-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .noi-acento-plano { color: var(--pp-acc); }
  .noi-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .noi-parrafo--tinta { color: var(--pp-ink); }
  .noi-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .noi-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .noi-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .noi-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .noi-std-dia { font-family: var(--noi-serif), 'Playfair Display', serif; font-weight: 500;
  ...
  .noi-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .noi-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--noi-serif), 'Playfair Display', serif; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .noi-std-anio { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .noi-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .noi-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .noi-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .noi-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--noi-serif), 'Playfair Display', serif; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .noi-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .noi-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .noi-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .noi-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .noi-cuenta-caja--1 { --noi-giro: -1.6deg; transform: rotate(var(--noi-giro)); }
  .noi-cuenta-caja--2 { --noi-giro: 1.2deg; transform: rotate(var(--noi-giro)); margin-top: 10px; }
  .noi-cuenta-caja--3 { --noi-giro: 1.8deg; transform: rotate(var(--noi-giro)); }
  .noi-cuenta-caja--4 { --noi-giro: -1.1deg; transform: rotate(var(--noi-giro)); margin-top: 10px; }
  .noi-cuenta-num { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .noi-cuenta-num--acc { color: var(--pp-acc); }
  .noi-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .noi-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .noi-frase { position: relative; z-index: 1; margin: 0; font-family: var(--noi-serif), 'Playfair Display', serif;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .noi-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .noi-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .noi-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .noi-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .noi-panel--abajo { justify-content: flex-end; }
  .noi-panel--centro { justify-content: center; }
  .noi-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .noi-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .noi-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .noi-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .noi-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .noi-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .noi-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .noi-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .noi-tarjeta--izq { --noi-giro: -1deg; transform: rotate(var(--noi-giro)); }
  .noi-tarjeta--der { --noi-giro: 1.1deg; transform: rotate(var(--noi-giro)); }
  .noi-tarjeta--talon { --noi-giro: -.8deg; transform: rotate(var(--noi-giro)); overflow: visible; }
  .noi-tarjeta--hoy { --noi-giro: -1deg; transform: rotate(var(--noi-giro)); align-items: flex-start; }
  .noi-tarjeta--banco { margin-top: 4px; }
  .noi-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .noi-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .noi-tarjeta-titulo { margin: 0; font-family: var(--noi-serif), 'Playfair Display', serif; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .noi-tarjeta-pregunta { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: 26px; line-height: 1.15; }
  .noi-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .noi-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .noi-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .noi-fila > span:first-child { color: var(--pp-ink2); }
  .noi-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .noi-fila-valor { text-align: right; }
  .noi-fila--copiable { align-items: center; }
  .noi-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .noi-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .noi-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .noi-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .noi-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .noi-checkin { background: var(--pp-bg); }
  .noi-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .noi-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .noi-campo { display: flex; flex-direction: column; gap: 6px; }
  .noi-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .noi-contador { display: flex; align-items: center; gap: 12px; }
  .noi-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .noi-contador button:disabled { opacity: .35; cursor: default; }
  .noi-contador > span { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .noi-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--noi-sans), 'Courier Prime', monospace; font-size: 16px; padding: 6px 2px; }
  .noi-input--serif { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: 20px; }
  .noi-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .noi-input::placeholder { color: #9A9388; }
  .noi-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .noi-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .noi-precio-total { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .noi-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .noi-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .noi-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .noi-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .noi-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--noi-sans), 'Courier Prime', monospace; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .noi-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .noi-btn-solido:disabled { opacity: .6; cursor: default; }
  .noi-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .noi-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --noi-giro: -1deg; transform: rotate(var(--noi-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .noi-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .noi-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .noi-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .noi-crono-hora { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: 26px; min-width: 64px; line-height: 1; }
  .noi-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .noi-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .noi-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .noi-polaroids[data-cantidad="1"] .noi-polaroid { grid-column: span 6; }
  .noi-polaroids[data-cantidad="2"] .noi-polaroid { grid-column: span 3; }
  .noi-polaroids[data-cantidad="4"] .noi-polaroid { grid-column: span 3; }
  .noi-polaroids[data-cantidad="5"] .noi-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .noi-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .noi-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .noi-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .noi-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .noi-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .noi-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .noi-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .noi-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .noi-lista-tema { font-size: 15px; }
  .noi-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .noi-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .noi-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .noi-opciones { display: flex; flex-direction: column; gap: 8px; }
  .noi-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--noi-sans), 'Courier Prime', monospace; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .noi-opcion:disabled { cursor: default; }
  .noi-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .noi-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .noi-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .noi-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --noi-giro: -.6deg; transform: rotate(var(--noi-giro));
    border-radius: 12px; overflow: hidden; }
  .noi-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .noi-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .noi-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .noi-ticket-nombre { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .noi-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .noi-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .noi-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .noi-ticket-muesca--arriba { top: -12px; }
  .noi-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .noi-ticket-der .qr-ingreso, .noi-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .noi-info-extra { position: relative; z-index: 1; }
  .noi-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .noi-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .noi-raiz .ia-icon-box { display: none !important; }
  .noi-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .noi-pase-firma { font-family: var(--noi-serif), 'Playfair Display', serif; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .noi-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .noi-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }
  ...
  .noi-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .noi-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  ...
  .noi-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .noi-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .noi-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .noi-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .noi-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .noi-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .noi-cartel-nombres { margin: 0; font-family: var(--noi-serif), 'Playfair Display', serif; font-weight: 500;
  ...
  .noi-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .noi-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .noi-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .noi-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .noi-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .noi-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .noi-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .noi-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--noi-sans), 'Courier Prime', monospace; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .noi-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .noi-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .noi-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .noi-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .noi-section, .noi-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .noi-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .noi-raiz { font-family: var(--noi-sans), 'Courier Prime', monospace; }
  ...
  .noi-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .noi-kicker, .noi-panel-top, .noi-panel-pie, .noi-meta, .noi-riel-top, .noi-riel-etiqueta,
  .noi-ticket-kicker, .noi-cuenta-etq, .noi-tarjeta-kicker, .noi-fila-etq, .noi-portada-pase {
    font-family: var(--noi-sans), 'Courier Prime', monospace; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .noi-h2, .noi-frase, .noi-tarjeta-titulo, .noi-std-dia, .noi-cartel-nombres, .noi-ticket-nombre, .noi-cifra, .noi-cuenta-num {
    font-family: var(--noi-serif), 'Playfair Display', serif; letter-spacing: -.03em; }
  .noi-h2 em, .noi-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .noi-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .noi-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .noi-std-fila { justify-content: space-between; }
  .noi-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--noi-serif), 'Playfair Display', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .noi-std-anio { font-family: var(--noi-serif), 'Playfair Display', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .noi-std .noi-kicker, .noi-std .noi-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .noi-foto-marco { border: 3px solid var(--pp-ink); }
  .noi-ventana { clip-path: none !important; box-shadow: none; }
  .noi-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--noi-serif), 'Playfair Display', serif; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .noi-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .noi-tarjeta--izq, .noi-tarjeta--der, .noi-tarjeta--talon, .noi-tarjeta--hoy { transform: none; }
  .noi-doblez::before { display: none; }
  .noi-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .noi-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .noi-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .noi-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .noi-cuenta-caja:first-child { border-left: none; }
  .noi-cuenta-caja::before { display: none; }
  .noi-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .noi-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .noi-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .noi-panel[data-tone="dark"] .noi-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .noi-panel-top, .noi-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .noi-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .noi-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .noi-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .noi-btn-solido, .noi-btn-papel, .noi-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .noi-btn-papel { background: transparent; border: 1px solid currentColor; }
  .noi-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .noi-portada { background: var(--pp-bg); }
  .noi-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .noi-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .noi-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .noi-cartel-chinche { display: none; }
  .noi-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .noi-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .noi-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Noir: la luz entra de costado, como una persiana. */
  .noi-trama { opacity: .10; background-image: repeating-linear-gradient(105deg, currentColor 0 2px, transparent 2px 22px); background-size: auto; }

  @media (prefers-reduced-motion: reduce) {
    .noi-raiz * { animation: none !important; }
    .noi-scroller [data-xin] { opacity: 1 !important; --noi-y: 0px; }
  }
`;
