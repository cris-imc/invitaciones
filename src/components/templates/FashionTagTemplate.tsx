...
interface FashionTagTemplateProps {
...
export function FashionTagTemplate({ invitation, guest, isPersonalized = false }: FashionTagTemplateProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#B0562E";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(176,86,46,.35), 0 18px 50px -30px #B0562E";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#1C1A18" : "#B0562E";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#100C0A", tinta: "#F4F1EA", acento: "#B0562E", clase: `${ftgPlayfair.variable} ${ftgMono.variable}`, fuente: "var(--ftg-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
            ...
            <div className="ftg-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="16,12,10" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="16,12,10" />
                </div>
              )}
            </div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#B0562E" guest={guest} />
      )}
...
function FtgCoverHalf({
  ...
  return (
    <div className="ftg-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#B0562E" tintColor2="#100C0A" effect="enfoque" scrimColorRgb="16,12,10" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#B0562E" tintColor2="#100C0A" effect="enfoque" scrimColorRgb="16,12,10" />
        </div>
      )}
...
const FTG_CSS = `
  ...
  a { color: #B0562E; text-decoration: none; }
  ...
  .ftg-date-month { font-family: var(--ftg-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B0562E; padding-left: 12%; }

  .ftg-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .ftg-divider-line { width: 52px; height: 1px; background: #B0562E; display: inline-block; }
  ...
  .ftg-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #1C1A18, #3A2A20, #B0562E, #F7F3EA, #1C1A18); filter: saturate(.75); }
  .ftg-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .ftg-medallion-sub--accent { color: #B0562E; }
  .ftg-medallion-arc { position: absolute; inset: -14%; }
  .ftg-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B0562E; opacity: 0.4; font-family: var(--ftg-mono), monospace; }
  ...
  .ftg-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  ...
  .ftg-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B0562E; }
  .ftg-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #100C0A 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .ftg-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(28,26,24,0.32), rgba(199,184,154,0.2), rgba(176,86,46,0.36), rgba(28,26,24,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .ftg-accent-italic { font-style: italic; color: #B0562E; }
  .ftg-accent-serif { font-style: italic; color: #B0562E; font-family: var(--ftg-playfair), serif; }
  ...
  .ftg-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .ftg-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(176,86,46,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .ftg-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B0562E; background: transparent; color: #B0562E; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .ftg-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(176,86,46,0.3); color: #F4F1EA; font-family: var(--ftg-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .ftg-rsvp-input::placeholder { color: #6E6A5D; }
  .ftg-rsvp-input:focus { outline: none; border-bottom-color: #B0562E; }
  .ftg-rsvp-btn { width: 100%; padding: 17px 0; font-family: var(--ftg-mono), monospace; font-size: 11.5px; font-weight: 400; letter-spacing: 0.26em; text-transform: uppercase; border: 1px solid #B0562E; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 300ms ease, color 300ms ease; }
  ...
  .ftg-rsvp-btn--ghost { background: transparent; color: #B0562E; }
  .ftg-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(176,86,46,0.1); color: #B0562E; }
  .ftg-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B0562E; margin: 0; }
  ...
  .ftg-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(176,86,46,0.18); }
  ...
  .ftg-bank-copy { flex-shrink: 0; font-family: var(--ftg-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B0562E; background: transparent; color: #B0562E; cursor: pointer; }
  .ftg-bank-copy:hover { background: rgba(176,86,46,0.12); }
  ...
  .ftg-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(176,86,46,0.3); padding-bottom: 12px; }
  ...
  .ftg-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(176,86,46,0.3); color: #F4F1EA; font-family: var(--ftg-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .ftg-song-input::placeholder { color: #6E6A5D; }
  .ftg-song-input:focus { outline: none; border-bottom-color: #B0562E; }
  .ftg-song-sep { color: #8A8577; flex-shrink: 0; }
  .ftg-song-submit { flex-shrink: 0; background: none; border: none; color: #B0562E; font-family: var(--ftg-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .ftg-song-submit:hover { color: #F7F3EA; }
  .ftg-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .ftg-song-error { font-size: 10px; color: #B0562E; margin-top: 6px; }
  ...
  .ftg-quiz-opt { font-family: var(--ftg-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(176,86,46,0.4); background: transparent; color: #B0562E; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .ftg-quiz-opt:disabled { cursor: default; }
  .ftg-quiz-opt--picked { background: #B0562E; border-color: #B0562E; color: #0B0B10; }
  .ftg-quiz-opt--correct { background: #B0562E; border-color: #B0562E; color: #0B0B10; }
  .ftg-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .ftg-quiz-result { padding-top: 18px; border-top: 1px solid rgba(176,86,46,0.2); }
  ...
  .ftg-final-card { border: 1px solid #B0562E; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .ftg-replay { cursor: pointer; color: #B0562E; }
  ...
  .ftg-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F7F3EA, #B0562E); transition: height 260ms linear; display: block; }
  .ftg-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B0562E; transition: color 500ms ease; }
  ...
  .ftg-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1C1613 0%, #14100E 46%, #100C0A 100%); }
  .ftg-cover-half--bottom .ftg-cover-inner { top: auto; bottom: 0; }
  .ftg-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(28,26,24,0.5), rgba(199,184,154,0.3), rgba(176,86,46,0.55), rgba(28,26,24,0.5)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .ftg-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B0562E, transparent); display: block; }
  ...
  .ftg-cover-cta { border: 1px solid #B0562E; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--ftg-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .ftg-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B0562E; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
