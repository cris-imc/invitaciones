...
// EditorialBlancNoirTemplate.tsx — no editar a mano: los cambios se pierden al regenerar.

/**
 * EDITORIAL BLANC & NOIR · Colección Iconic — sub-colección "Tipográfica Editorial"
 * Variante: Ciruela. Es la familia BASE de la sub-colección: las otras
...
interface EditorialBlancNoirTemplateProps {
...
export function EditorialBlancNoirTemplateCiruela({ invitation, guest, isPersonalized = false }: EditorialBlancNoirTemplateProps) {
  const tx = useTextos();
  ...
  useEffect(() => {
    ...
    reveals.forEach((el) => {
      ...
      el.style.opacity = "0";
      el.style.setProperty("--ebn-y", `${dist}px`);
      ...
      if (sc) {
        ...
        for (let i = pendientes.length - 1; i >= 0; i--) {
          ...
          if (r.top < vh * 0.92 && r.bottom > 0 && r.left < window.innerWidth && r.right > 0) {
            el.style.opacity = "1";
            el.style.setProperty("--ebn-y", "0px");
    ...
    return (
      <PostEventoStorytelling
        ...
        paleta={{
          ...
          tinta: PALETA.ink,
          acento: PALETA.acc,
          clase: `${ebnSerif.variable} ${ebnSans.variable} ${ebnMono.variable}`,
          fuente: "var(--ebn-sans), 'Archivo', sans-serif",
        }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="ebn-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="ebn-section ebn-std">
          <div className="ebn-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="ebn-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="ebn-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="ebn-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="ebn-std-fila">
              <span className="ebn-cinta">{mesLargo}</span>
              <span className="ebn-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="ebn-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="ebn-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="ebn-btn-papel ebn-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="ebn-foto-seccion">
            <div className="ebn-foto-marco">
              <div ref={ventanaRef} className="ebn-ventana">
                {fotoMobile && (
              ...
              <div className="ebn-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="ebn-section ebn-countdown">
          <div className="ebn-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="ebn-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="ebn-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="ebn-section ebn-frase-seccion">
            <div className="ebn-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="ebn-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="ebn-firma">
              <span className="ebn-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="ebn-pan-fijo">
            <div data-strip="1" className="ebn-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="ebn-panel ebn-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="ebn-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="ebn-tarjeta ebn-tarjeta--izq">
                  <span className="ebn-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="ebn-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="ebn-filas">
                    {direccion && (
                      <div className="ebn-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="ebn-fila-valor">{direccion}</span></div>
                    )}
                    <div className="ebn-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="ebn-fila ebn-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="ebn-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="ebn-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="ebn-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="ebn-panel ebn-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="ebn-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="ebn-tarjeta ebn-tarjeta--der">
                    <h2 className="ebn-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="ebn-filas">
                      {ceremoniaDireccion && (
                        <div className="ebn-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="ebn-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="ebn-fila ebn-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="ebn-panel ebn-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="ebn-escena" aria-hidden="true">
                  ...
                  <h2 className="ebn-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="ebn-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="ebn-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="ebn-panel ebn-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="ebn-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="ebn-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="ebn-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="ebn-crono-fila">
                        <span className="ebn-crono-hora">{item.time || ""}</span>
                        <span className="ebn-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="ebn-section ebn-checkin">
            <div className="ebn-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="ebn-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="ebn-h2">
            ...
            <div className="ebn-pan-fijo">
              <div data-strip="1" className="ebn-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="ebn-panel ebn-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="ebn-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="ebn-h2 ebn-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="ebn-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="ebn-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="ebn-polaroid-img" />
                          {i % 2 === 0 && <span className="ebn-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="ebn-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="ebn-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="ebn-section ebn-musica">
            <div className="ebn-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="ebn-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="ebn-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="ebn-section ebn-regalos">
            <span data-xin="1" data-dist="-20" className="ebn-kicker ebn-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="ebn-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="ebn-parrafo ebn-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="ebn-section ebn-quiz">
            <div className="ebn-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="ebn-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="ebn-h2 ebn-h2--claro">{triviaTitulo}</h2>
            <TriviaEditorial
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="ebn-section ebn-pase">
          <div className="ebn-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="ebn-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="ebn-ticket">
            <div className="ebn-ticket-izq">
              <span className="ebn-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="ebn-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="ebn-ticket-dato">
                {[
              ...
              <span className="ebn-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="ebn-ticket-muesca ebn-ticket-muesca--arriba" aria-hidden="true" />
              <span className="ebn-ticket-muesca ebn-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="ebn-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="ebn-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="ebn-pase-pie">
            <span className="ebn-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="ebn-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="ebn-portada">
        ...
        <div ref={escenaPortadaRef} className="ebn-portada-escena">
          <div data-cl="1" className="ebn-trama" aria-hidden="true" />
        </div>
        <div className="ebn-portada-contenido">
          <div className="ebn-portada-arriba">
            <div ref={cartelRef} className="ebn-cartel">
              <h1 className="ebn-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="ebn-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="ebn-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="ebn-portada-abajo">
            <div className="ebn-portada-datos">
              <span className="ebn-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="ebn-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="ebn-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="ebn-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="ebn-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="ebn-portada-pase">
            ...
            <button type="button" onClick={abrir} className="ebn-portada-btn">
...
function Puntos({ cantidad }: { cantidad: number }) {
  ...
  return (
    <div className="ebn-puntos" aria-hidden="true">
      {Array.from({ length: cantidad }).map((_, i) => (
        <span key={i} data-dot={i} className="ebn-punto" style={{ background: i === 0 ? PALETA.acc : "rgba(43,42,51,.18)" }} />
      ))}
    </div>
  );
}
 ...
 */
function CuentaEditorial({ targetDate }: { targetDate: Date }) {
  const tx = useTextos();
  const { time, isEventDay, isPast, hasEnded } = useCountdown(targetDate);

  if (isEventDay || hasEnded || isPast) {
    return (
      <div className="ebn-tarjeta ebn-tarjeta--hoy" data-xin="1" data-delay="80" data-dist="30">
        <span className="ebn-tarjeta-kicker">{tx("invitacion.cuentaRegresiva.kicker").toUpperCase()}</span>
        <span className="ebn-tarjeta-titulo">
...
function FilaCopiable({ etiqueta, valor }: { etiqueta: string; valor: string }) {
  ...
  return (
    <div className="ebn-fila ebn-fila--copiable">
      <div className="ebn-fila-texto">
        <span className="ebn-fila-etq">{etiqueta.toUpperCase()}</span>
        <span className="ebn-fila-dato">{valor}</span>
      </div>
      <button type="button" onClick={copiar} className={`ebn-btn-copiar${copiado ? " ebn-btn-copiar--hecho" : ""}`}>
...
function TarjetaBancaria({
  ...
  return (
    <div
      ...
      data-delay={retraso}
      data-dist="30"
      className={`ebn-tarjeta ebn-tarjeta--banco${dobleZ ? " ebn-doblez" : ""}${inclinada ? " ebn-tarjeta--der" : " ebn-tarjeta--izq"}`}
    >
      <span className="ebn-tarjeta-kicker">{titulo.toUpperCase()}</span>
      {mensaje && <p className="ebn-tarjeta-mensaje">{mensaje}</p>}
      <div className="ebn-filas">
        <FilaCopiable etiqueta="Alias" valor={alias} />
        <FilaCopiable etiqueta="CBU" valor={cbu} />
        {(banco || titular) && (
          <div className="ebn-fila ebn-fila--ultima">
            <span>{banco || tx("invitacion.regalos.banco")}</span>
            <span className="ebn-fila-valor">{titular}</span>
          </div>
        )}
      </div>
    </div>
...
function CheckinEditorial({
  ...
  if (estado === "DECLINED") {
    return (
      <div data-xin="1" data-delay="140" data-dist="30" className="ebn-tarjeta ebn-tarjeta--izq">
        <p className="ebn-tarjeta-mensaje">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
        <button type="button" className="ebn-btn-fantasma" onClick={() => setEstado("PENDING")}>
  ...
  return (
    <>
      <div ref={tarjetaRef} data-xin="1" data-delay="140" data-dist="30" className="ebn-tarjeta ebn-tarjeta--talon">
        <div className="ebn-talon-top">
          <span>{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}{guestName ? ` · ${guestName.toUpperCase()}` : ""}</span>
          <span ref={estadoRef} className="ebn-talon-estado">
        ...
        {!confirmadoYa ? (
          <>
            {lugares > 1 && (
              <div className="ebn-campo">
                <label className="ebn-etiqueta">{tx("invitacion.rsvp.adultos").toUpperCase()}</label>
                <div className="ebn-contador">
            ...
            {maxAdolescentes > 0 && (
              <div className="ebn-campo">
                <label className="ebn-etiqueta">{tx("invitacion.rsvp.adolescentes").toUpperCase()}</label>
                <div className="ebn-contador">
            ...
            {maxNinos > 0 && (
              <div className="ebn-campo">
                <label className="ebn-etiqueta">{tx("invitacion.rsvp.ninos").toUpperCase()}</label>
                <div className="ebn-contador">
            ...
            <div className="ebn-campo">
              <label className="ebn-etiqueta">{tx("invitacion.rsvp.restriccionAlimentaria").toUpperCase()}</label>
              <input
                value={dieta}
          ...
          <div className="ebn-filas">
            {lugares > 1 && adultos > 0 && <div className="ebn-fila"><span>{tx("invitacion.rsvp.adultos")}</span><span>{adultos}</span></div>}
            {adolescentes > 0 && <div className="ebn-fila"><span>{tx("invitacion.rsvp.adolescentes")}</span><span>{adolescentes}</span></div>}
            {ninos > 0 && <div className="ebn-fila"><span>{tx("invitacion.rsvp.ninos")}</span><span>{ninos}</span></div>}
            <div className="ebn-fila ebn-fila--ultima">
              <span>{tx("invitacion.rsvp.restriccionAlimentaria")}</span>
              <span className="ebn-fila-valor">{restricciones || dieta || "—"}</span>
            </div>
          </div>
        )}

        {hayPago && monto != null && !exento && (
          <div className="ebn-precio">
            <span>{estadoDePago === "PAID" ? tx("invitacion.pago.abonado") : tx("invitacion.pago.valor")}</span>
            <div className="ebn-precio-valor">
              <span className="ebn-precio-total">{formatearMoneda(totalAPagar)}</span>
              {usarTotalDelServidor && vistaDePago
                ? vistaDePago.lines.map((l, i) => <span key={i} className="ebn-precio-detalle">{l}</span>)
                : (
                  <>
                    {adultos > 0 && <span className="ebn-precio-detalle">{adultos} × {formatearMoneda(precioAdulto)}</span>}
                    {adolescentes > 0 && <span className="ebn-precio-detalle">{adolescentes} × {formatearMoneda(precioAdo)}</span>}
                    {ninos > 0 && <span className="ebn-precio-detalle">{ninos} × {formatearMoneda(precioNin)}</span>}
                  </>
                )}
            </div>
          </div>
        ...
        <div ref={selloRef} className="ebn-sello" aria-hidden="true">{tx("invitacion.pase.confirmado").toUpperCase()}</div>
        <div ref={petalosRef} className="ebn-petalos" aria-hidden="true" />
      </div>

      {error && <p className="ebn-error">{error}</p>}

      {!confirmadoYa ? (
        <>
          <button type="button" data-xin="1" data-delay="220" className="ebn-btn-solido" disabled={enviando} onClick={() => enviar("CONFIRMA")}>
            {enviando ? tx("invitacion.rsvp.guardando").toUpperCase() : tx("invitacion.rsvp.confirmarAsistencia").toUpperCase()}
          </button>
          <button type="button" data-xin="1" data-delay="280" className="ebn-btn-fantasma" disabled={enviando} onClick={() => enviar("NO_ASISTE")}>
...
function CancionesEditorial({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
  ...
  return (
    <>
      <form onSubmit={enviar} data-xin="1" data-delay="140" data-dist="30" className="ebn-tarjeta ebn-tarjeta--der">
        <div className="ebn-campo">
          <label className="ebn-etiqueta">{tx("invitacion.musica.tema").toUpperCase()}</label>
          <input value={tema} onChange={(e) => setTema(e.target.value)} placeholder={tx("invitacion.musica.nombreDeLaCancion")} maxLength={100} className="ebn-input ebn-input--serif" />
        </div>
        <div className="ebn-campo">
          <label className="ebn-etiqueta">{tx("invitacion.musica.artista").toUpperCase()}</label>
          <input value={artista} onChange={(e) => setArtista(e.target.value)} placeholder={tx("invitacion.musica.artista")} maxLength={80} className="ebn-input ebn-input--serif" />
        </div>
        {error && <p className="ebn-error">{error}</p>}
        <button type="submit" disabled={enviando} className="ebn-btn-solido ebn-btn-solido--tinta">
          {enviando ? tx("invitacion.musica.enviando").toUpperCase() : tx("invitacion.musica.sugeriUnaCancion").toUpperCase()}
        </button>
      </form>

      {canciones.length > 0 && (
        <div data-xin="1" data-delay="220" className="ebn-lista">
          {canciones.slice(0, 8).map((c) => (
            <div key={c.id} className="ebn-lista-fila">
              <div className="ebn-lista-texto">
                <span className="ebn-lista-tema">{c.title}</span>
                <span className="ebn-lista-quien">{c.artist} · {tx("invitacion.musica.sumadoPor")} {c.guestName || tx("invitacion.evento.invitado")}</span>
              </div>
            </div>
          ))}
        </div>
 ...
 */
function TriviaEditorial({ preguntas, invitationId, guestToken, guestName }: { preguntas: QuizPregunta[]; invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
  ...
  if (terminado) {
    return (
      <div data-xin="1" data-delay="140" data-dist="30" className="ebn-tarjeta ebn-tarjeta--izq">
        <span className="ebn-tarjeta-kicker">{tx("invitacion.quiz.kicker").toUpperCase()}</span>
        <span className="ebn-tarjeta-titulo">
        ...
        {!guardando && stats && stats.count > 0 && (
          <p className="ebn-tarjeta-mensaje">{tx("invitacion.quiz.promedio", { n: stats.count, avg: stats.avg })}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div data-xin="1" data-delay="140" data-dist="30" className="ebn-tarjeta ebn-tarjeta--izq">
      <span className="ebn-tarjeta-kicker">{tx("invitacion.quiz.preguntaDeTotal", { n: indice + 1, total: preguntas.length }).toUpperCase()}</span>
      <span className="ebn-tarjeta-pregunta">{q.pregunta}</span>
      <div className="ebn-opciones">
        {q.opciones.map((op, oi) => {
          let clase = "";
          if (yaEligio && correcta !== undefined) {
            if (oi === correcta) clase = " ebn-opcion--bien";
            else if (elegidas[indice] === oi) clase = " ebn-opcion--mal";
          }
          return (
            <button key={oi} type="button" disabled={yaEligio} onClick={() => elegir(oi)} className={`ebn-opcion${clase}`}>
              {op}
            </button>
          );
        })}
...
// leen la Bienvenida y el Post-evento compartidos (esperan `ebn-section` y
// `ebn-kicker`).
const CSS_EBN = `
  .ebn-raiz { position: fixed; inset: 0; width: 100%; height: calc(var(--vh, 1vh) * 100); overflow: hidden;
    background: var(--pp-bg); color: var(--pp-ink); font-family: var(--ebn-sans), 'Archivo', sans-serif; }
  .ebn-raiz a { color: var(--pp-acc); text-decoration: none; }
  .ebn-raiz button { font: inherit; }
  .ebn-defs { position: absolute; width: 0; height: 0; overflow: hidden; }

  .ebn-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0;
    transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .ebn-scroller::-webkit-scrollbar { width: 0; height: 0; }
  ...
  /* Lo que entra al scrollear se mueve en --ebn-y y gira en --ebn-giro: son
  ...
  .ebn-scroller [data-xin] { transform: translate3d(0, var(--ebn-y, 0px), 0) rotate(var(--ebn-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .ebn-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .ebn-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .ebn-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .ebn-kicker--tinta { color: var(--pp-ink); }
  .ebn-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--ebn-serif), 'Instrument Serif', serif;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .ebn-h2 em, .ebn-acento { font-style: italic; color: var(--pp-acc); }
  .ebn-h2--claro { color: var(--pp-night-ink); }
  .ebn-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .ebn-acento-plano { color: var(--pp-acc); }
  .ebn-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .ebn-parrafo--tinta { color: var(--pp-ink); }
  .ebn-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .ebn-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .ebn-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .ebn-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .ebn-std-dia { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-weight: 500;
  ...
  .ebn-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .ebn-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--ebn-serif), 'Instrument Serif', serif; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .ebn-std-anio { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .ebn-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .ebn-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .ebn-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .ebn-foto-capa { position: absolute; inset: 0; }
  .ebn-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--ebn-serif), 'Instrument Serif', serif; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .ebn-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .ebn-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .ebn-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .ebn-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .ebn-cuenta-caja--1 { --ebn-giro: -1.6deg; transform: rotate(var(--ebn-giro)); }
  .ebn-cuenta-caja--2 { --ebn-giro: 1.2deg; transform: rotate(var(--ebn-giro)); margin-top: 10px; }
  .ebn-cuenta-caja--3 { --ebn-giro: 1.8deg; transform: rotate(var(--ebn-giro)); }
  .ebn-cuenta-caja--4 { --ebn-giro: -1.1deg; transform: rotate(var(--ebn-giro)); margin-top: 10px; }
  .ebn-cuenta-num { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .ebn-cuenta-num--acc { color: var(--pp-acc); }
  .ebn-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .ebn-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .ebn-frase { position: relative; z-index: 1; margin: 0; font-family: var(--ebn-serif), 'Instrument Serif', serif;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .ebn-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .ebn-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .ebn-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .ebn-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .ebn-tira { position: absolute; top: 0; left: 0; height: 100%; display: flex; will-change: transform; }
  .ebn-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden;
    display: flex; flex-direction: column; gap: 18px; padding: 80px max(24px, calc((100vw - 560px) / 2)) 96px; }
  .ebn-panel--abajo { justify-content: flex-end; }
  .ebn-panel--centro { justify-content: center; }
  .ebn-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  /* Apilado: los mismos paneles, uno abajo del otro, recorridos bajando. */
  .ebn-pan[data-scroll="vertical"] { height: auto; }
  .ebn-pan[data-scroll="vertical"] .ebn-pan-fijo { position: static; height: auto; overflow: visible; }
  .ebn-pan[data-scroll="vertical"] .ebn-tira { position: static; display: block; width: 100%; transform: none !important; }
  .ebn-pan[data-scroll="vertical"] .ebn-panel { height: auto; min-height: calc(var(--vh, 1vh) * 100); }
  .ebn-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .ebn-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .ebn-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .ebn-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .ebn-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .ebn-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .ebn-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .ebn-tarjeta--izq { --ebn-giro: -1deg; transform: rotate(var(--ebn-giro)); }
  .ebn-tarjeta--der { --ebn-giro: 1.1deg; transform: rotate(var(--ebn-giro)); }
  .ebn-tarjeta--talon { --ebn-giro: -.8deg; transform: rotate(var(--ebn-giro)); overflow: visible; }
  .ebn-tarjeta--hoy { --ebn-giro: -1deg; transform: rotate(var(--ebn-giro)); align-items: flex-start; }
  .ebn-tarjeta--banco { margin-top: 4px; }
  .ebn-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .ebn-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .ebn-tarjeta-titulo { margin: 0; font-family: var(--ebn-serif), 'Instrument Serif', serif; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .ebn-tarjeta-pregunta { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: 26px; line-height: 1.15; }
  .ebn-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .ebn-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .ebn-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .ebn-fila > span:first-child { color: var(--pp-ink2); }
  .ebn-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .ebn-fila-valor { text-align: right; }
  .ebn-fila--copiable { align-items: center; }
  .ebn-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .ebn-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .ebn-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .ebn-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .ebn-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .ebn-checkin { background: var(--pp-bg); }
  .ebn-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .ebn-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .ebn-campo { display: flex; flex-direction: column; gap: 6px; }
  .ebn-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .ebn-contador { display: flex; align-items: center; gap: 12px; }
  .ebn-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .ebn-contador button:disabled { opacity: .35; cursor: default; }
  .ebn-contador > span { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .ebn-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--ebn-sans), 'Archivo', sans-serif; font-size: 16px; padding: 6px 2px; }
  .ebn-input--serif { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: 20px; }
  .ebn-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .ebn-input::placeholder { color: #9A9388; }
  .ebn-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .ebn-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .ebn-precio-total { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .ebn-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .ebn-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .ebn-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .ebn-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .ebn-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--ebn-sans), 'Archivo', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .ebn-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .ebn-btn-solido:disabled { opacity: .6; cursor: default; }
  .ebn-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .ebn-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --ebn-giro: -1deg; transform: rotate(var(--ebn-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .ebn-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .ebn-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .ebn-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .ebn-crono-hora { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: 26px; min-width: 64px; line-height: 1; }
  .ebn-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .ebn-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .ebn-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .ebn-polaroids[data-cantidad="1"] .ebn-polaroid { grid-column: span 6; }
  .ebn-polaroids[data-cantidad="2"] .ebn-polaroid { grid-column: span 3; }
  .ebn-polaroids[data-cantidad="4"] .ebn-polaroid { grid-column: span 3; }
  .ebn-polaroids[data-cantidad="5"] .ebn-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .ebn-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .ebn-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .ebn-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .ebn-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .ebn-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .ebn-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .ebn-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .ebn-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .ebn-lista-tema { font-size: 15px; }
  .ebn-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .ebn-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .ebn-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .ebn-opciones { display: flex; flex-direction: column; gap: 8px; }
  .ebn-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--ebn-sans), 'Archivo', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .ebn-opcion:disabled { cursor: default; }
  .ebn-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .ebn-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .ebn-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .ebn-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --ebn-giro: -.6deg; transform: rotate(var(--ebn-giro));
    border-radius: 12px; overflow: hidden; }
  .ebn-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .ebn-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .ebn-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .ebn-ticket-nombre { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .ebn-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .ebn-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .ebn-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .ebn-ticket-muesca--arriba { top: -12px; }
  .ebn-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .ebn-ticket-der .qr-ingreso, .ebn-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .ebn-info-extra { position: relative; z-index: 1; }
  .ebn-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .ebn-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .ebn-raiz .ia-icon-box { display: none !important; }
  .ebn-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .ebn-pase-firma { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .ebn-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .ebn-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }

  /* ── Riel ──────────────────────────────────────────────────────────── */
  .ebn-riel { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column;
  ...
  .ebn-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .ebn-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  .ebn-riel-barra { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: var(--pp-acc); transition: height 260ms linear; display: block; }
  .ebn-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .ebn-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .ebn-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .ebn-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .ebn-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .ebn-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .ebn-cartel-nombres { margin: 0; font-family: var(--ebn-serif), 'Instrument Serif', serif; font-weight: 500;
  ...
  .ebn-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .ebn-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .ebn-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .ebn-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .ebn-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .ebn-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .ebn-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .ebn-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--ebn-sans), 'Archivo', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .ebn-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }

  .ebn-pista { position: absolute; left: 0; right: 34px; bottom: calc(18px + env(safe-area-inset-bottom)); z-index: 6; text-align: center;
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .ebn-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .ebn-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .ebn-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .ebn-section, .ebn-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .ebn-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .ebn-raiz { font-family: var(--ebn-sans), 'Archivo', sans-serif; }
  ...
  .ebn-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .ebn-kicker, .ebn-panel-top, .ebn-panel-pie, .ebn-meta, .ebn-riel-top, .ebn-riel-etiqueta,
  .ebn-ticket-kicker, .ebn-cuenta-etq, .ebn-tarjeta-kicker, .ebn-fila-etq, .ebn-portada-pase {
    font-family: var(--ebn-mono), 'JetBrains Mono', monospace; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .ebn-h2, .ebn-frase, .ebn-tarjeta-titulo, .ebn-std-dia, .ebn-cartel-nombres, .ebn-ticket-nombre, .ebn-cifra, .ebn-cuenta-num {
    font-family: var(--ebn-serif), 'Instrument Serif', serif; letter-spacing: -.03em; }
  .ebn-h2 em, .ebn-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .ebn-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .ebn-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .ebn-std-fila { justify-content: space-between; }
  .ebn-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .ebn-std-anio { font-family: var(--ebn-serif), 'Instrument Serif', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .ebn-std .ebn-kicker, .ebn-std .ebn-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .ebn-foto-marco { border: 3px solid var(--pp-ink); }
  .ebn-ventana { clip-path: none !important; box-shadow: none; }
  .ebn-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--ebn-serif), 'Instrument Serif', serif; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .ebn-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .ebn-tarjeta--izq, .ebn-tarjeta--der, .ebn-tarjeta--talon, .ebn-tarjeta--hoy { transform: none; }
  .ebn-doblez::before { display: none; }
  .ebn-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .ebn-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .ebn-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .ebn-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .ebn-cuenta-caja:first-child { border-left: none; }
  .ebn-cuenta-caja::before { display: none; }
  .ebn-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .ebn-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .ebn-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .ebn-panel[data-tone="dark"] .ebn-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .ebn-panel-top, .ebn-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .ebn-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .ebn-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .ebn-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .ebn-btn-solido, .ebn-btn-papel, .ebn-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .ebn-btn-papel { background: transparent; border: 1px solid currentColor; }
  .ebn-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .ebn-portada { background: var(--pp-bg); }
  .ebn-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .ebn-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .ebn-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .ebn-cartel-chinche { display: none; }
  .ebn-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .ebn-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .ebn-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  @media (prefers-reduced-motion: reduce) {
    .ebn-raiz * { animation: none !important; }
    .ebn-scroller [data-xin] { opacity: 1 !important; --ebn-y: 0px; }
  }
`;
