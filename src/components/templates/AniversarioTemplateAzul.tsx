...
interface AniversarioTemplateAzulQuizQuestion {
  pregunta: string;
  opciones: string[];
...
interface AniversarioTemplateAzulProps {
...
export function AniversarioTemplateAzul({ invitation, guest, isPersonalized = false }: AniversarioTemplateAzulProps) {
  const tx = useTextos();
  ...
  const triviaPreguntas: AniversarioTemplateAzulQuizQuestion[] = safeJson<AniversarioTemplateAzulQuizQuestion[]>(String(invitation.triviaPreguntas ?? ""), []);
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#DCE6F0";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#5C7EA8";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #5C7EA8";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#32465E" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#32465E" : "#5C7EA8";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#130C09", tinta: "#F4F1EA", acento: "#5C7EA8", clase: `${anvDisplay.variable} ${anvMono.variable}`, fuente: "var(--anv-mono), monospace" }}
      />
    );
  }
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="anv-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="anv-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-drift="-70" className="anv-medallion anv-medallion--corner">
            <AniversarioTemplateAzulMedallionCmp label="20" sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="anvArc1" arcText={tx("invitacion.sabor.arcoAniversario").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="anv-section anv-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2E1E14 0%, #0D0B10 55%, #130C09 100%)" }}>
          ...
          <div className="anv-cd-grid">
            <AniversarioTemplateAzulCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <AniversarioTemplateAzulCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <AniversarioTemplateAzulCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <AniversarioTemplateAzulCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="anv-pan">
          <div className="anv-pan-sticky">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="anv-panel anv-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="anv-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#32465E" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#32465E" />
                  </svg>
            ...
            <AniversarioTemplateAzulDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="anv-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #180F0C 60%, #130C09 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="anv-rsvp">
              <AniversarioTemplateAzulRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="anv-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="anv-pan-sticky">
            ...
            <AniversarioTemplateAzulDots count={photoPages.length} />
          </div>
        </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="anv-section" style={{ background: "#180F0C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="anv-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="anv-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#DCE6F0" : "#5C7EA8" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="anv-song-wrap">
              <AniversarioTemplateAzulSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="anv-bank-wrap">
              {pagoTarjetaHabilitado && (
                <BankDetailsCard
                  ...
                  InfoRow={AniversarioTemplateAzulInfoRow}
                  CopyField={AniversarioTemplateAzulCopyField}
                />
              )}
              {regaloHabilitado && (
                <BankDetailsCard
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="anv-section anv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="anv-final-card">
            <div className="anv-medallion anv-medallion--final">
              <AniversarioTemplateAzulMedallionCmp label="20" sub={confirmed ? "CONFIRMADO" : tx("invitacion.pase.pendiente").toUpperCase()} arcId="anvArc3" arcText={textoArco(namesTitle, fechaCorta)} spin="reverse" />
            </div>
      ...
      <div ref={coverRef} className="anv-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="anv-cover-half anv-cover-half--top">
          <AniversarioTemplateAzulCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#5C7EA8" guest={guest} />
      )}
...
function AniversarioTemplateAzulCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
...
function AniversarioTemplateAzulDots({ count = 3 }: { count?: number }) {
...
function AniversarioTemplateAzulMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
...
function AniversarioTemplateAzulCopyField({ label, value }: { label: string; value: string }) {
  const tx = useTextos();
...
function AniversarioTemplateAzulRsvpCard({
  invitationId,
  guestToken,
  guestName,
  maxAdults,
    ...
    <>
      ...
      <div ref={stubRef} className="anv-stub">
        ...
        <div ref={sealRef} className="anv-seal">
          <AniversarioTemplateAzulMedallionCmp label="20" sub="" arcId="" arcText="" spin="none" compact />
        </div>
...
function AniversarioTemplateAzulSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
...
function AniversarioTemplateAzulQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: AniversarioTemplateAzulQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  const tx = useTextos();
...
function AniversarioTemplateAzulInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="anv-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="anv-bank-row-label">{label}</span>
...
function AniversarioTemplateAzulCoverHalf({
  namesRef,
  kickerRef,
  perfRef,
  kickerText,
...
const GP_CSS = `
  ...
  a { color: #5C7EA8; text-decoration: none; }
  a:hover { color: #DCE6F0; }
  ...
  .anv-date-month { font-family: var(--anv-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #5C7EA8; padding-left: 12%; }

  .anv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .anv-divider-line { width: 52px; height: 1px; background: #5C7EA8; display: inline-block; }
  ...
  .anv-cal-link:hover { color: #5C7EA8; }
  ...
  .anv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #5C7EA8, #DCE6F0, #6E5AA8); filter: saturate(.75); }
  ...
  .anv-medallion-label { font-family: var(--anv-display), serif; font-size: 26px; line-height: 1; color: #DCE6F0; }
  .anv-medallion-label-sm { font-family: var(--anv-display), serif; font-size: 16px; color: #DCE6F0; }
  .anv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .anv-medallion-sub--accent { color: #5C7EA8; }
  ...
  .anv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #5C7EA8; opacity: 0.4; font-family: var(--anv-mono), monospace; }
  ...
  .anv-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #DCE6F0, transparent); animation: anvScan 6s linear infinite; pointer-events: none; }
  ...
  .anv-cd-num { font-family: var(--anv-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #DCE6F0; display: inline-block; }
  .anv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #5C7EA8; }
  ...
  .anv-accent-italic { font-style: italic; color: #5C7EA8; }
  .anv-accent-serif { font-style: italic; color: #32465E; font-family: var(--anv-display), serif; }
  .anv-accent-serif-2 { color: #32465E; }
  ...
  .anv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #32465E; margin-top: auto; }
  ...
  .anv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #32465E; }
  ...
  .anv-crono-time { font-family: var(--anv-mono), monospace; color: #32465E; min-width: 42px; }
  ...
  .anv-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #DCE6F0, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .anv-rsvp-payment-total { color: #DCE6F0; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .anv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #5C7EA8; background: transparent; color: #5C7EA8; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .anv-rsvp-input:focus { outline: none; border-bottom-color: #5C7EA8; }
  .anv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--anv-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #5C7EA8, #32465E); border: 1px solid #5C7EA8; color: #180F0C; cursor: pointer; }
  .anv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .anv-rsvp-btn--ghost { background: transparent; color: #5C7EA8; }
  .anv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #5C7EA8; margin: 0; }
  ...
  .anv-bank-copy { flex-shrink: 0; font-family: var(--anv-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #5C7EA8; background: transparent; color: #5C7EA8; cursor: pointer; }
  ...
  .anv-song-input:focus { outline: none; border-bottom-color: #5C7EA8; }
  .anv-song-sep { color: #8A8577; flex-shrink: 0; }
  .anv-song-submit { flex-shrink: 0; background: none; border: none; color: #5C7EA8; font-family: var(--anv-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .anv-song-submit:hover { color: #DCE6F0; }
  .anv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .anv-song-error { font-size: 10px; color: #5C7EA8; margin-top: 6px; }
  ...
  .anv-quiz-opt { font-family: var(--anv-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #5C7EA8; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .anv-quiz-opt:disabled { cursor: default; }
  .anv-quiz-opt--picked { background: #5C7EA8; border-color: #5C7EA8; color: #180F0C; }
  .anv-quiz-opt--correct { background: #5C7EA8; border-color: #5C7EA8; color: #180F0C; }
  ...
  .anv-quiz-result-score { margin: 0 0 6px; font-family: var(--anv-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #DCE6F0; }
  .anv-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .anv-final-card { border: 1px solid #5C7EA8; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .anv-final-names { font-family: var(--anv-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #DCE6F0; }
  ...
  .anv-replay { cursor: pointer; color: #5C7EA8; }
  ...
  .anv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#DCE6F0, #5C7EA8); transition: height 260ms linear; display: block; }
  .anv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #5C7EA8; transition: color 500ms ease; }
  ...
  .anv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(90,58,68,0.4), rgba(92,126,168,0.32), rgba(92,126,168,0.55), rgba(90,58,68,0.4)); filter: blur(64px); opacity: .62; animation: anvFoil 26s linear infinite; }
  .anv-cover-texture { position: absolute; inset: 0; background-image: radial-gradient(circle at 70% 30%, rgba(220,230,240,.14), transparent 45%); }
  ...
  .anv-cover-names { margin: 0; font-family: var(--anv-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #DCE6F0; }
  .anv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#5C7EA8, transparent); display: block; }
  ...
  .anv-cover-cta { border: 1px solid #5C7EA8; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--anv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .anv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #5C7EA8; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
