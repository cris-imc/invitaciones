 ...
 * mármol y dorado #B87333, Italiana + Cormorant Garamond + IBM Plex Mono,
 ...
 * una veta de mármol + resplandor dorado como motivo decorativo -- NUNCA el
 * sunburst de Guest Pass VIP). Todo el motion es CSS + SVG + un loop de
 * scroll propio -- cero PNGs, así que escala sin pixelarse en cualquier
 * viewport.
 *
 ...
 * sección EN VIVO, misma lista de canciones acotada) -- solo cambian paleta,
 * tipografía, copy y el motivo decorativo. Habla SIEMPRE con los mismos
 * endpoints/datos reales del backend (mismo /api/guests/[token]/confirm que
 * usa RSVPWizardV2, mismo /api/quiz, /api/songs, etc). El destello de sello
 * se dispara al confirmar asistencia, vía el mismo callback onConfirmed que
 * usaban los componentes compartidos.
 *
...
import { Italiana, Cormorant_Garamond, IBM_Plex_Mono } from "next/font/google";
...
interface MarmolYOroTemplateBronceProps {
...
// real que mostrar.
function pieceNumberFrom(orderNumber: number | undefined): string {
...
export function MarmolYOroTemplateBronce({ invitation, guest, isPersonalized = false }: MarmolYOroTemplateBronceProps) {
  const tx = useTextos();
  ...
  // Combinación de colores del diseño original (ver mockup "Marmol y Oro -
  // Panoramica"): primera mitad de la frase en color plano, segunda mitad en
  // dorado itálico. Antes esto se perdía apenas se cargaba una frase
  // personalizada (quedaba toda pareja); ahora se aplica siempre, partiendo
  // por la mitad de la cantidad real de palabras en vez de un índice fijo
  // pensado solo para la frase default.
  ...
  const pieceNumber = pieceNumberFrom(guest?.orderNumber);
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E8B98C";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#B87333";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(184,115,51,.35), 0 18px 50px -30px #B87333";
      }
      if (sealRef.current) {
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(184,115,51,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(184,115,51,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#8A5A2E" : "#B87333";
        }
    ...
    // (había que arrastrar muchas veces para avanzar un poco). En cambio, se
    // escala para que un arrastre de TODO el ancho de pantalla mueva
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#08070A", tinta: "#F4F1EA", acento: "#B87333", clase: `${myoItaliana.variable} ${myoCormorant.variable} ${myoMono.variable}`, fuente: "var(--myo-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${myoItaliana.variable} ${myoCormorant.variable} ${myoMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="myo-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="myo-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #1C1710 0%, #121010 55%, #08070A 100%)" }}>
          ...
          <div className="myo-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="myo-date-num myo-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="myo-divider">
            <span className="myo-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="myo-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="myo-cal-link"
          />
            ...
            <div className="myo-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="8,7,10" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="8,7,10" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="myo-section myo-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A1B0D 0%, #120E0A 55%, #08070A 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="myo-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1D160E 0%, #100C09 52%, #08070A 100%)" }}>
          ...
          <h2 ref={phraseRef} className="myo-phrase" style={{ fontSize: fraseFontSize }}>
            {fraseWords.map((w, i) => (
              ...
              <span key={i}>
                <span data-w="1" className={i >= fraseAccentStart ? "myo-accent-italic" : undefined}>
                  {w}
                </span>{" "}
              </span>
            ))}
          </h2>
          <div data-xin="1" data-delay="120" data-dist="90" className="myo-divider" style={{ position: "relative" }}>
            <span className="myo-divider-line myo-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
          </div>
        </section>
        )}
          ...
          <div className="myo-pan-sticky">
            <div data-strip="1" className="myo-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="myo-panel myo-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <h2 className="myo-panel-title">
                    ...
                    {ceremoniaDireccion && <><br /><span className="myo-accent-serif">{ceremoniaDireccion}</span></>}
                  </h2>
              ...
              <div id="details" data-tone="light" className="myo-panel myo-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="myo-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="myo-crono-row">
                        <span className="myo-crono-time">{item.time || ""}</span>
                        <span className="myo-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div data-tone="dark" className="myo-panel myo-panel--center" style={{ background: "#121010", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="myo-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #1C1710 0%, #121010 60%, #08070A 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="myo-rsvp">
              <MyoRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="myo-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="myo-pan-sticky">
            <div data-strip="1" className="myo-strip">
                ...
                <div key={pageIndex} data-tone="light" className="myo-panel myo-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="myo-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`myo-mosaic-cell${i === 0 ? " myo-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="myo-seguir myo-seguir--split">
                    ...
                    <span className="myo-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="myo-section" style={{ background: "#121010" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="myo-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="myo-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E8B98C" : "#B87333" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="myo-section" style={{ background: "#121010" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="myo-section" style={{ background: "#121010" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pieza" className="myo-section myo-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #1C1710 0%, #121010 55%, #08070A 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="myo-final-card">
            ...
            <span className="myo-mini-label" style={{ color: "#A8A292" }}>{fechaCorta} — {hora} H</span>
          ...
          <div className="myo-footer-credit">
      ...
      <div ref={railRef} className="myo-rail">
        ...
        <span ref={railLabelRef} className="myo-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="myo-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="myo-cover-half myo-cover-half--top">
          <CoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="myo-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="myo-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#B87333" guest={guest} />
      )}
...
function CdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="myo-cd-box">
      <span ref={refEl} className="myo-cd-num">—</span>
      <span className="myo-cd-label">{label}</span>
    </div>
  );
}
...
function Dots({ count = 3 }: { count?: number }) {
  ...
  return (
    <div data-dots="1" className="myo-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="myo-dot" />
      ))}
    </div>
  );
}
...
function Medallion({
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="myo-medallion-arc" style={{ animation: spin === "reverse" ? "gpRingRev 32s linear infinite" : "gpRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="myo-medallion-arc-text">
...
function MyoCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="myo-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="myo-bank-row-label">{label}</span>
...
// Habla con el mismo endpoint/payload que ese componente (/api/guests/[token]/confirm
// o /api/rsvp), así que no cambia ningún dato que pida el backend, solo cómo
// se ve.
function MyoRsvpCard({
  invitationId,
  guestToken,
  guestName,
  maxAdults,
    ...
    <>
      <div className="myo-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="myo-rsvp-row myo-rsvp-row--payment">
      ...
      <div ref={stubRef} className="myo-stub">
        ...
        <div ref={beamRef} className="myo-beam" />
        <div className="myo-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="myo-rsvp-error">{error}</p>}
...
function MyoSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="myo-song">
      ...
      {error && <p className="myo-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="myo-song-list">
...
function MyoQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: MyoQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="myo-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="myo-quiz-q">
            <span className="myo-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="myo-quiz-q-text">{q.pregunta}</p>
            <div className="myo-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " myo-quiz-opt--correct";
                  else if (chosen) stateClass = " myo-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " myo-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`myo-quiz-opt${stateClass}`}
...
function MyoInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="myo-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="myo-bank-row-label">{label}</span>
    ...
    <div className="myo-cover-inner">
      ...
      <div className="myo-cover-content">
        ...
        <div className="myo-cover-center">
          ...
          <h1 ref={namesRef} className="myo-cover-names">{namesTitle}</h1>
          <span className="myo-cover-rule" />
          <span className="myo-cover-date">{fechaCorta}</span>
        </div>
        <div className="myo-cover-bottom">
          <div ref={perfRef} className="myo-perf-strip myo-perf-strip--reveal myo-perf-strip--cover" />
          <div className="myo-cover-facts">
...
// mármol blanco/gris + dorado #B87333, nunca el dorado #C8A45C ni el
...
const MYO_CSS = `
  ...
  /* Veta de mármol de la tapa (propia de este mockup -- NUNCA el sunburst de
     Guest Pass VIP): deriva lentamente en vez de girar, como si la luz se
     moviera sobre la piedra. */
  @keyframes myoMarbleDrift { 0% { transform: scale(1.04) translate(0,0); } 100% { transform: scale(1.12) translate(-1.5%, -1%); } }
  @media (prefers-reduced-motion: reduce) { .myo-scroller * { animation: none !important; } }
  ...
  .myo-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #08070A; }
  ...
  @media (max-width: 767px) {
    .myo-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .myo-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(184,115,51,.3); }
    .myo-hero-photo-kicker { bottom: 40px; }
    .myo-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }
  ...
  .myo-date-month { font-family: var(--myo-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B87333; padding-left: 12%; }

  .myo-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .myo-divider-line { width: 52px; height: 1px; background: #B87333; display: inline-block; }
  ...
  .myo-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--myo-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .myo-cal-link:hover { color: #B87333; }

  .myo-medallion { position: relative; }
  ...
  .myo-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E645A, #1A1A1C, #B87333, #E8B98C, #6E645A); filter: saturate(.75); }
  .myo-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0A0808; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .myo-medallion-label { font-family: var(--myo-italiana), serif; font-size: 26px; line-height: 1; color: #E8B98C; }
  .myo-medallion-label-sm { font-family: var(--myo-italiana), serif; font-size: 16px; color: #E8B98C; }
  .myo-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .myo-medallion-sub--accent { color: #B87333; }
  ...
  .myo-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B87333; opacity: 0.4; font-family: var(--myo-mono), monospace; }

  .myo-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(184,115,51,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .myo-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E8B98C, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .myo-cd-num { font-family: var(--myo-italiana), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F0DFC8; display: inline-block; }
  .myo-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B87333; }
  .myo-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #08070A 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .myo-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .myo-perf-strip--cover { opacity: .9; }

  .myo-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,100,90,0.3), rgba(26,26,28,0.26), rgba(184,115,51,0.4), rgba(110,100,90,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .myo-accent-italic { font-family: var(--myo-cormorant), serif; font-style: italic; color: #B87333; }
  .myo-accent-serif { font-style: italic; color: #8A5A2E; font-family: var(--myo-cormorant), serif; }
  .myo-accent-serif-2 { color: #8A5A2E; }
  .myo-h2 { margin: 0; font-family: var(--myo-italiana), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .myo-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .myo-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .myo-strip { position: absolute; top: 0; left: 0; height: 100%; display: flex; width: 300vw; will-change: transform; }
  .myo-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 84px max(24px, calc((100vw - 560px) / 2)) 100px; gap: 22px; }
  .myo-panel--between { justify-content: space-between; }
  ...
  .myo-pan[data-scroll="vertical"] { height: auto; }
  .myo-pan[data-scroll="vertical"] .myo-pan-sticky { position: static; height: auto; overflow: visible; }
  .myo-pan[data-scroll="vertical"] .myo-strip { position: static; display: block; width: 100%; transform: none !important; }
  .myo-pan[data-scroll="vertical"] .myo-panel { height: calc(var(--vh, 1vh) * 100); }
  .myo-panel--end { justify-content: flex-end; }
  .myo-panel--center { align-items: center; justify-content: center; text-align: center; }
  .myo-panel--gap { gap: clamp(14px, 2.4vh, 22px); padding: clamp(52px, 9vh, 84px) max(24px, calc((100vw - 600px) / 2)) clamp(62px, 11vh, 100px); }
  .myo-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .myo-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .myo-panel-title { margin: 0; position: relative; font-family: var(--myo-italiana), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .myo-panel-title-md { margin: 0; position: relative; font-family: var(--myo-italiana), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .myo-panel-title-sm { margin: 0; font-family: var(--myo-italiana), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .myo-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .myo-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .myo-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  ...
  .myo-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #8A5A2E; margin-top: auto; }
  ...
  .myo-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #8A5A2E; }
  ...
  .myo-crono-time { font-family: var(--myo-mono), monospace; color: #8A5A2E; min-width: 42px; }
  ...
  .myo-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .myo-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .myo-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E8B98C, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .myo-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(184,115,51,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .myo-rsvp-payment-total { color: #E8B98C; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .myo-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B87333; background: transparent; color: #B87333; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .myo-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(184,115,51,0.3); color: #F4F1EA; font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .myo-rsvp-input::placeholder { color: #6E6A5D; }
  .myo-rsvp-input:focus { outline: none; border-bottom-color: #B87333; }
  .myo-rsvp-btn { width: 100%; padding: 16px; font-family: var(--myo-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B87333, #8A5A2E); border: 1px solid #B87333; color: #0B0B10; cursor: pointer; }
  .myo-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .myo-rsvp-btn--ghost { background: transparent; color: #B87333; }
  .myo-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B87333; margin: 0; }
  ...
  .myo-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .myo-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .myo-photo-cell--offset { margin-top: 18px; }
  .myo-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .myo-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .myo-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .myo-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .myo-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .myo-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .myo-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .myo-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  ...
  .myo-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(184,115,51,0.18); }
  ...
  .myo-bank-copy { flex-shrink: 0; font-family: var(--myo-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B87333; background: transparent; color: #B87333; cursor: pointer; }
  .myo-bank-copy:hover { background: rgba(184,115,51,0.12); }
  ...
  .myo-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(184,115,51,0.3); padding-bottom: 12px; }
  ...
  .myo-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(184,115,51,0.3); color: #F4F1EA; font-family: var(--myo-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .myo-song-input::placeholder { color: #6E6A5D; }
  .myo-song-input:focus { outline: none; border-bottom-color: #B87333; }
  .myo-song-sep { color: #8A8577; flex-shrink: 0; }
  .myo-song-submit { flex-shrink: 0; background: none; border: none; color: #B87333; font-family: var(--myo-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .myo-song-submit:hover { color: #E8B98C; }
  .myo-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .myo-song-error { font-size: 10px; color: #B87333; margin-top: 6px; }
  ...
  .myo-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .myo-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .myo-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  ...
  .myo-quiz-opt { font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(184,115,51,0.4); background: transparent; color: #B87333; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .myo-quiz-opt:disabled { cursor: default; }
  .myo-quiz-opt--picked { background: #B87333; border-color: #B87333; color: #0B0B10; }
  .myo-quiz-opt--correct { background: #B87333; border-color: #B87333; color: #0B0B10; }
  .myo-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .myo-quiz-result { padding-top: 18px; border-top: 1px solid rgba(184,115,51,0.2); }
  .myo-quiz-result-score { margin: 0 0 6px; font-family: var(--myo-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E8B98C; }
  .myo-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .myo-final-card { border: 1px solid #B87333; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .myo-final-names { font-family: var(--myo-italiana), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E8B98C; }
  ...
  .myo-replay { cursor: pointer; color: #B87333; }
  .myo-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .myo-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(184,115,51,0.14); }
  .myo-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .myo-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(184,115,51,0.16); position: relative; }
  .myo-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E8B98C, #B87333); transition: height 260ms linear; display: block; }
  .myo-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B87333; transition: color 500ms ease; }
  ...
  .myo-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #171310 0%, #121010 46%, #08070A 100%); }
  .myo-cover-half--bottom .myo-cover-inner { top: auto; bottom: 0; }
  .myo-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,100,90,0.35), rgba(26,26,28,0.3), rgba(184,115,51,0.55), rgba(110,100,90,0.35)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .myo-cover-marble { position: absolute; inset: 0; background-image: repeating-linear-gradient(12deg, rgba(184,115,51,.18) 0 1px, transparent 1px 60px), repeating-linear-gradient(-68deg, rgba(232,185,140,.1) 0 1px, transparent 1px 90px), repeating-linear-gradient(40deg, rgba(184,115,51,.14) 0 1px, transparent 1px 130px); animation: myoMarbleDrift 22s ease-in-out infinite alternate; }
  ...
  .myo-cover-names { margin: 0; font-family: var(--myo-italiana), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F7F5F1; }
  .myo-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B87333, transparent); display: block; }
  ...
  .myo-cover-cta { border: 1px solid #B87333; background: linear-gradient(100deg, rgba(184,115,51,0.08), rgba(232,185,140,0.2), rgba(184,115,51,0.08)); color: #F0DFC8; font-family: var(--myo-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .myo-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B87333; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
