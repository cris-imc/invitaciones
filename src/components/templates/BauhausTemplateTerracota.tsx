...
// BauhausTemplate.tsx — no editar a mano: los cambios se pierden al regenerar.
...
export function BauhausTemplateTerracota({ invitation, guest, isPersonalized = false }: BauhausTemplateProps) {
  const tx = useTextos();
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="bau-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="bau-section bau-std">
          <div className="bau-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="bau-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="bau-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="bau-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="bau-std-fila">
              <span className="bau-cinta">{mesLargo}</span>
              <span className="bau-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="bau-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="bau-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="bau-btn-papel bau-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="bau-foto-seccion">
            <div className="bau-foto-marco">
              <div ref={ventanaRef} className="bau-ventana">
                {fotoMobile && (
              ...
              <div className="bau-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="bau-section bau-countdown">
          <div className="bau-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="bau-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="bau-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="bau-section bau-frase-seccion">
            <div className="bau-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="bau-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="bau-firma">
              <span className="bau-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="bau-pan-fijo">
            <div data-strip="1" className="bau-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="bau-panel bau-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="bau-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="bau-tarjeta bau-tarjeta--izq">
                  <span className="bau-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="bau-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="bau-filas">
                    {direccion && (
                      <div className="bau-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="bau-fila-valor">{direccion}</span></div>
                    )}
                    <div className="bau-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="bau-fila bau-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="bau-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="bau-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="bau-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="bau-panel bau-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="bau-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="bau-tarjeta bau-tarjeta--der">
                    <h2 className="bau-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="bau-filas">
                      {ceremoniaDireccion && (
                        <div className="bau-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="bau-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="bau-fila bau-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="bau-panel bau-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="bau-escena" aria-hidden="true">
                  ...
                  <h2 className="bau-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="bau-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="bau-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="bau-panel bau-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="bau-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="bau-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="bau-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="bau-crono-fila">
                        <span className="bau-crono-hora">{item.time || ""}</span>
                        <span className="bau-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="bau-section bau-checkin">
            <div className="bau-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="bau-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="bau-h2">
            ...
            <div className="bau-pan-fijo">
              <div data-strip="1" className="bau-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="bau-panel bau-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="bau-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="bau-h2 bau-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="bau-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="bau-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="bau-polaroid-img" />
                          {i % 2 === 0 && <span className="bau-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="bau-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="bau-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="bau-section bau-musica">
            <div className="bau-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="bau-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="bau-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="bau-section bau-regalos">
            <span data-xin="1" data-dist="-20" className="bau-kicker bau-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="bau-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="bau-parrafo bau-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="bau-section bau-quiz">
            <div className="bau-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="bau-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="bau-h2 bau-h2--claro">{triviaTitulo}</h2>
            <TriviaBauhaus
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="bau-section bau-pase">
          <div className="bau-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="bau-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="bau-ticket">
            <div className="bau-ticket-izq">
              <span className="bau-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="bau-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="bau-ticket-dato">
                {[
              ...
              <span className="bau-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="bau-ticket-muesca bau-ticket-muesca--arriba" aria-hidden="true" />
              <span className="bau-ticket-muesca bau-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="bau-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="bau-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="bau-pase-pie">
            <span className="bau-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="bau-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="bau-portada">
        ...
        <div ref={escenaPortadaRef} className="bau-portada-escena">
          <div data-cl="1" className="bau-trama" aria-hidden="true" />
        </div>
        <div className="bau-portada-contenido">
          <div className="bau-portada-arriba">
            <div ref={cartelRef} className="bau-cartel">
              <h1 className="bau-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="bau-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="bau-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="bau-portada-abajo">
            <div className="bau-portada-datos">
              <span className="bau-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="bau-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="bau-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="bau-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="bau-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="bau-portada-pase">
            ...
            <button type="button" onClick={abrir} className="bau-portada-btn">
...
function CuentaBauhaus({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="bau-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`bau-cuenta-caja bau-cuenta-caja--${i + 1}`}>
          <span className={`bau-cuenta-num${i === 3 ? " bau-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_BAU = `
  ...
  .bau-raiz a { color: var(--pp-acc); text-decoration: none; }
  .bau-raiz button { font: inherit; }
  .bau-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --bau-y y gira en --bau-giro: son
  ...
  .bau-scroller [data-xin] { transform: translate3d(0, var(--bau-y, 0px), 0) rotate(var(--bau-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .bau-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .bau-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .bau-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .bau-kicker--tinta { color: var(--pp-ink); }
  .bau-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--bau-serif), 'Jost', sans-serif;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .bau-h2 em, .bau-acento { font-style: italic; color: var(--pp-acc); }
  .bau-h2--claro { color: var(--pp-night-ink); }
  .bau-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .bau-acento-plano { color: var(--pp-acc); }
  .bau-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .bau-parrafo--tinta { color: var(--pp-ink); }
  .bau-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .bau-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .bau-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .bau-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .bau-std-dia { font-family: var(--bau-serif), 'Jost', sans-serif; font-weight: 500;
  ...
  .bau-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .bau-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--bau-serif), 'Jost', sans-serif; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .bau-std-anio { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .bau-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .bau-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .bau-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .bau-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--bau-serif), 'Jost', sans-serif; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .bau-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .bau-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .bau-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .bau-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .bau-cuenta-caja--1 { --bau-giro: -1.6deg; transform: rotate(var(--bau-giro)); }
  .bau-cuenta-caja--2 { --bau-giro: 1.2deg; transform: rotate(var(--bau-giro)); margin-top: 10px; }
  .bau-cuenta-caja--3 { --bau-giro: 1.8deg; transform: rotate(var(--bau-giro)); }
  .bau-cuenta-caja--4 { --bau-giro: -1.1deg; transform: rotate(var(--bau-giro)); margin-top: 10px; }
  .bau-cuenta-num { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .bau-cuenta-num--acc { color: var(--pp-acc); }
  .bau-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .bau-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .bau-frase { position: relative; z-index: 1; margin: 0; font-family: var(--bau-serif), 'Jost', sans-serif;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .bau-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .bau-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .bau-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .bau-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .bau-panel--abajo { justify-content: flex-end; }
  .bau-panel--centro { justify-content: center; }
  .bau-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .bau-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .bau-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .bau-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .bau-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .bau-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .bau-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .bau-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .bau-tarjeta--izq { --bau-giro: -1deg; transform: rotate(var(--bau-giro)); }
  .bau-tarjeta--der { --bau-giro: 1.1deg; transform: rotate(var(--bau-giro)); }
  .bau-tarjeta--talon { --bau-giro: -.8deg; transform: rotate(var(--bau-giro)); overflow: visible; }
  .bau-tarjeta--hoy { --bau-giro: -1deg; transform: rotate(var(--bau-giro)); align-items: flex-start; }
  .bau-tarjeta--banco { margin-top: 4px; }
  .bau-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .bau-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .bau-tarjeta-titulo { margin: 0; font-family: var(--bau-serif), 'Jost', sans-serif; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .bau-tarjeta-pregunta { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: 26px; line-height: 1.15; }
  .bau-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .bau-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .bau-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .bau-fila > span:first-child { color: var(--pp-ink2); }
  .bau-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .bau-fila-valor { text-align: right; }
  .bau-fila--copiable { align-items: center; }
  .bau-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .bau-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .bau-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .bau-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .bau-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .bau-checkin { background: var(--pp-bg); }
  .bau-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .bau-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .bau-campo { display: flex; flex-direction: column; gap: 6px; }
  .bau-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .bau-contador { display: flex; align-items: center; gap: 12px; }
  .bau-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .bau-contador button:disabled { opacity: .35; cursor: default; }
  .bau-contador > span { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .bau-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--bau-sans), 'Work Sans', sans-serif; font-size: 16px; padding: 6px 2px; }
  .bau-input--serif { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: 20px; }
  .bau-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .bau-input::placeholder { color: #9A9388; }
  .bau-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .bau-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .bau-precio-total { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .bau-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .bau-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .bau-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .bau-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .bau-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--bau-sans), 'Work Sans', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .bau-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .bau-btn-solido:disabled { opacity: .6; cursor: default; }
  .bau-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .bau-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --bau-giro: -1deg; transform: rotate(var(--bau-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .bau-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .bau-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .bau-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .bau-crono-hora { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: 26px; min-width: 64px; line-height: 1; }
  .bau-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .bau-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .bau-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .bau-polaroids[data-cantidad="1"] .bau-polaroid { grid-column: span 6; }
  .bau-polaroids[data-cantidad="2"] .bau-polaroid { grid-column: span 3; }
  .bau-polaroids[data-cantidad="4"] .bau-polaroid { grid-column: span 3; }
  .bau-polaroids[data-cantidad="5"] .bau-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .bau-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .bau-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .bau-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .bau-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .bau-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .bau-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .bau-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .bau-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .bau-lista-tema { font-size: 15px; }
  .bau-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .bau-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .bau-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .bau-opciones { display: flex; flex-direction: column; gap: 8px; }
  .bau-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--bau-sans), 'Work Sans', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .bau-opcion:disabled { cursor: default; }
  .bau-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .bau-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .bau-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .bau-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --bau-giro: -.6deg; transform: rotate(var(--bau-giro));
    border-radius: 12px; overflow: hidden; }
  .bau-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .bau-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .bau-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .bau-ticket-nombre { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .bau-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .bau-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .bau-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .bau-ticket-muesca--arriba { top: -12px; }
  .bau-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .bau-ticket-der .qr-ingreso, .bau-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .bau-info-extra { position: relative; z-index: 1; }
  .bau-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .bau-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .bau-raiz .ia-icon-box { display: none !important; }
  .bau-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .bau-pase-firma { font-family: var(--bau-serif), 'Jost', sans-serif; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .bau-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .bau-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }
  ...
  .bau-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .bau-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  ...
  .bau-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .bau-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .bau-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .bau-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .bau-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .bau-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .bau-cartel-nombres { margin: 0; font-family: var(--bau-serif), 'Jost', sans-serif; font-weight: 500;
  ...
  .bau-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .bau-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .bau-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .bau-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .bau-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .bau-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .bau-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .bau-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--bau-sans), 'Work Sans', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .bau-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .bau-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .bau-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .bau-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .bau-section, .bau-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .bau-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .bau-raiz { font-family: var(--bau-sans), 'Work Sans', sans-serif; }
  ...
  .bau-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .bau-kicker, .bau-panel-top, .bau-panel-pie, .bau-meta, .bau-riel-top, .bau-riel-etiqueta,
  .bau-ticket-kicker, .bau-cuenta-etq, .bau-tarjeta-kicker, .bau-fila-etq, .bau-portada-pase {
    font-family: var(--bau-sans), 'Work Sans', sans-serif; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .bau-h2, .bau-frase, .bau-tarjeta-titulo, .bau-std-dia, .bau-cartel-nombres, .bau-ticket-nombre, .bau-cifra, .bau-cuenta-num {
    font-family: var(--bau-serif), 'Jost', sans-serif; letter-spacing: -.03em; }
  .bau-h2 em, .bau-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .bau-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .bau-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .bau-std-fila { justify-content: space-between; }
  .bau-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--bau-serif), 'Jost', sans-serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .bau-std-anio { font-family: var(--bau-serif), 'Jost', sans-serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .bau-std .bau-kicker, .bau-std .bau-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .bau-foto-marco { border: 3px solid var(--pp-ink); }
  .bau-ventana { clip-path: none !important; box-shadow: none; }
  .bau-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--bau-serif), 'Jost', sans-serif; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .bau-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .bau-tarjeta--izq, .bau-tarjeta--der, .bau-tarjeta--talon, .bau-tarjeta--hoy { transform: none; }
  .bau-doblez::before { display: none; }
  .bau-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .bau-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .bau-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .bau-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .bau-cuenta-caja:first-child { border-left: none; }
  .bau-cuenta-caja::before { display: none; }
  .bau-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .bau-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .bau-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .bau-panel[data-tone="dark"] .bau-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .bau-panel-top, .bau-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .bau-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .bau-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .bau-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .bau-btn-solido, .bau-btn-papel, .bau-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .bau-btn-papel { background: transparent; border: 1px solid currentColor; }
  .bau-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .bau-portada { background: var(--pp-bg); }
  .bau-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .bau-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .bau-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .bau-cartel-chinche { display: none; }
  .bau-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .bau-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .bau-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Bauhaus: sin itálicas y sin curvas. El acento es un bloque. */
  .bau-h2 em, .bau-acento { font-style: normal; background: var(--pp-acc); color: var(--pp-bg); padding: 0 .12em; }
  .bau-h2, .bau-cartel-nombres { letter-spacing: -.02em; text-transform: uppercase; }
  .bau-trama { background-image: none; }

  @media (prefers-reduced-motion: reduce) {
    .bau-raiz * { animation: none !important; }
    .bau-scroller [data-xin] { opacity: 1 !important; --bau-y: 0px; }
  }
`;
