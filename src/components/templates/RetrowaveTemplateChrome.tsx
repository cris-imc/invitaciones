...
export function RetrowaveTemplateChrome({ invitation, guest, isPersonalized = false }: RetrowaveTemplateProps) {
  const tx = useTextos();
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="rtw-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="rtw-section rtw-std">
          <div className="rtw-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="rtw-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="rtw-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="rtw-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="rtw-std-fila">
              <span className="rtw-cinta">{mesLargo}</span>
              <span className="rtw-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="rtw-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="rtw-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="rtw-btn-papel rtw-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="rtw-foto-seccion">
            <div className="rtw-foto-marco">
              <div ref={ventanaRef} className="rtw-ventana">
                {fotoMobile && (
              ...
              <div className="rtw-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="rtw-section rtw-countdown">
          <div className="rtw-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="rtw-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="rtw-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="rtw-section rtw-frase-seccion">
            <div className="rtw-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="rtw-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="rtw-firma">
              <span className="rtw-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="rtw-pan-fijo">
            <div data-strip="1" className="rtw-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="rtw-panel rtw-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="rtw-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="rtw-tarjeta rtw-tarjeta--izq">
                  <span className="rtw-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="rtw-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="rtw-filas">
                    {direccion && (
                      <div className="rtw-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="rtw-fila-valor">{direccion}</span></div>
                    )}
                    <div className="rtw-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="rtw-fila rtw-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="rtw-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="rtw-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="rtw-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="rtw-panel rtw-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="rtw-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="rtw-tarjeta rtw-tarjeta--der">
                    <h2 className="rtw-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="rtw-filas">
                      {ceremoniaDireccion && (
                        <div className="rtw-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="rtw-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="rtw-fila rtw-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="rtw-panel rtw-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="rtw-escena" aria-hidden="true">
                  ...
                  <h2 className="rtw-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="rtw-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="rtw-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="rtw-panel rtw-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="rtw-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="rtw-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="rtw-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="rtw-crono-fila">
                        <span className="rtw-crono-hora">{item.time || ""}</span>
                        <span className="rtw-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="rtw-section rtw-checkin">
            <div className="rtw-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="rtw-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="rtw-h2">
            ...
            <div className="rtw-pan-fijo">
              <div data-strip="1" className="rtw-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="rtw-panel rtw-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="rtw-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="rtw-h2 rtw-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="rtw-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        ...
                        >
                          ...
                          {i % 2 === 0 && <span className="rtw-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="rtw-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="rtw-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="rtw-section rtw-musica">
            <div className="rtw-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="rtw-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="rtw-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="rtw-section rtw-regalos">
            <span data-xin="1" data-dist="-20" className="rtw-kicker rtw-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="rtw-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="rtw-parrafo rtw-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="rtw-section rtw-quiz">
            <div className="rtw-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="rtw-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="rtw-h2 rtw-h2--claro">{triviaTitulo}</h2>
            <TriviaRetrowave
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="rtw-section rtw-pase">
          <div className="rtw-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="rtw-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="rtw-ticket">
            <div className="rtw-ticket-izq">
              <span className="rtw-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="rtw-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="rtw-ticket-dato">
                {[
              ...
              <span className="rtw-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="rtw-ticket-muesca rtw-ticket-muesca--arriba" aria-hidden="true" />
              <span className="rtw-ticket-muesca rtw-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="rtw-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="rtw-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="rtw-pase-pie">
            <span className="rtw-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="rtw-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="rtw-portada">
        ...
        <div ref={escenaPortadaRef} className="rtw-portada-escena">
          <div data-cl="1" className="rtw-trama" aria-hidden="true" />
        </div>
        <div className="rtw-portada-contenido">
          <div className="rtw-portada-arriba">
            <div ref={cartelRef} className="rtw-cartel">
              <h1 className="rtw-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="rtw-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="rtw-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="rtw-portada-abajo">
            <div className="rtw-portada-datos">
              <span className="rtw-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="rtw-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="rtw-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="rtw-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="rtw-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="rtw-portada-pase">
            ...
            <button type="button" onClick={abrir} className="rtw-portada-btn">
...
function CuentaRetrowave({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="rtw-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`rtw-cuenta-caja rtw-cuenta-caja--${i + 1}`}>
          <span className={`rtw-cuenta-num${i === 3 ? " rtw-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_RTW = `
  ...
  .rtw-raiz a { color: var(--pp-acc); text-decoration: none; }
  .rtw-raiz button { font: inherit; }
  .rtw-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --rtw-y y gira en --rtw-giro: son
  ...
  .rtw-scroller [data-xin] { transform: translate3d(0, var(--rtw-y, 0px), 0) rotate(var(--rtw-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .rtw-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .rtw-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .rtw-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .rtw-kicker--tinta { color: var(--pp-ink); }
  .rtw-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--rtw-serif), 'Righteous', cursive;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .rtw-h2 em, .rtw-acento { font-style: italic; color: var(--pp-acc); }
  .rtw-h2--claro { color: var(--pp-night-ink); }
  .rtw-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .rtw-acento-plano { color: var(--pp-acc); }
  .rtw-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .rtw-parrafo--tinta { color: var(--pp-ink); }
  .rtw-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .rtw-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .rtw-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .rtw-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .rtw-std-dia { font-family: var(--rtw-serif), 'Righteous', cursive; font-weight: 500;
  ...
  .rtw-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .rtw-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--rtw-serif), 'Righteous', cursive; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .rtw-std-anio { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .rtw-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .rtw-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .rtw-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .rtw-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--rtw-serif), 'Righteous', cursive; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .rtw-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .rtw-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .rtw-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .rtw-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .rtw-cuenta-caja--1 { --rtw-giro: -1.6deg; transform: rotate(var(--rtw-giro)); }
  .rtw-cuenta-caja--2 { --rtw-giro: 1.2deg; transform: rotate(var(--rtw-giro)); margin-top: 10px; }
  .rtw-cuenta-caja--3 { --rtw-giro: 1.8deg; transform: rotate(var(--rtw-giro)); }
  .rtw-cuenta-caja--4 { --rtw-giro: -1.1deg; transform: rotate(var(--rtw-giro)); margin-top: 10px; }
  .rtw-cuenta-num { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .rtw-cuenta-num--acc { color: var(--pp-acc); }
  .rtw-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .rtw-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .rtw-frase { position: relative; z-index: 1; margin: 0; font-family: var(--rtw-serif), 'Righteous', cursive;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .rtw-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .rtw-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .rtw-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .rtw-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .rtw-panel--abajo { justify-content: flex-end; }
  .rtw-panel--centro { justify-content: center; }
  .rtw-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .rtw-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .rtw-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .rtw-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .rtw-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .rtw-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .rtw-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .rtw-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .rtw-tarjeta--izq { --rtw-giro: -1deg; transform: rotate(var(--rtw-giro)); }
  .rtw-tarjeta--der { --rtw-giro: 1.1deg; transform: rotate(var(--rtw-giro)); }
  .rtw-tarjeta--talon { --rtw-giro: -.8deg; transform: rotate(var(--rtw-giro)); overflow: visible; }
  .rtw-tarjeta--hoy { --rtw-giro: -1deg; transform: rotate(var(--rtw-giro)); align-items: flex-start; }
  .rtw-tarjeta--banco { margin-top: 4px; }
  .rtw-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .rtw-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .rtw-tarjeta-titulo { margin: 0; font-family: var(--rtw-serif), 'Righteous', cursive; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .rtw-tarjeta-pregunta { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: 26px; line-height: 1.15; }
  .rtw-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .rtw-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .rtw-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .rtw-fila > span:first-child { color: var(--pp-ink2); }
  .rtw-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .rtw-fila-valor { text-align: right; }
  .rtw-fila--copiable { align-items: center; }
  .rtw-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .rtw-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .rtw-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .rtw-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .rtw-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .rtw-checkin { background: var(--pp-bg); }
  .rtw-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .rtw-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .rtw-campo { display: flex; flex-direction: column; gap: 6px; }
  .rtw-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .rtw-contador { display: flex; align-items: center; gap: 12px; }
  .rtw-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .rtw-contador button:disabled { opacity: .35; cursor: default; }
  .rtw-contador > span { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .rtw-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--rtw-sans), 'Manrope', sans-serif; font-size: 16px; padding: 6px 2px; }
  .rtw-input--serif { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: 20px; }
  .rtw-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .rtw-input::placeholder { color: #9A9388; }
  .rtw-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .rtw-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .rtw-precio-total { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .rtw-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .rtw-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .rtw-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .rtw-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .rtw-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--rtw-sans), 'Manrope', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .rtw-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .rtw-btn-solido:disabled { opacity: .6; cursor: default; }
  .rtw-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .rtw-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --rtw-giro: -1deg; transform: rotate(var(--rtw-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .rtw-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .rtw-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .rtw-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .rtw-crono-hora { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: 26px; min-width: 64px; line-height: 1; }
  .rtw-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .rtw-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .rtw-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .rtw-polaroids[data-cantidad="1"] .rtw-polaroid { grid-column: span 6; }
  .rtw-polaroids[data-cantidad="2"] .rtw-polaroid { grid-column: span 3; }
  .rtw-polaroids[data-cantidad="4"] .rtw-polaroid { grid-column: span 3; }
  .rtw-polaroids[data-cantidad="5"] .rtw-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .rtw-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .rtw-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .rtw-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .rtw-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .rtw-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .rtw-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .rtw-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .rtw-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .rtw-lista-tema { font-size: 15px; }
  .rtw-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .rtw-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .rtw-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .rtw-opciones { display: flex; flex-direction: column; gap: 8px; }
  .rtw-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--rtw-sans), 'Manrope', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .rtw-opcion:disabled { cursor: default; }
  .rtw-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .rtw-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .rtw-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .rtw-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --rtw-giro: -.6deg; transform: rotate(var(--rtw-giro));
    border-radius: 12px; overflow: hidden; }
  .rtw-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .rtw-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .rtw-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .rtw-ticket-nombre { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .rtw-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .rtw-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .rtw-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .rtw-ticket-muesca--arriba { top: -12px; }
  .rtw-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .rtw-ticket-der .qr-ingreso, .rtw-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .rtw-info-extra { position: relative; z-index: 1; }
  .rtw-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .rtw-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .rtw-raiz .ia-icon-box { display: none !important; }
  .rtw-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .rtw-pase-firma { font-family: var(--rtw-serif), 'Righteous', cursive; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .rtw-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .rtw-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }
  ...
  .rtw-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .rtw-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  ...
  .rtw-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .rtw-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .rtw-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .rtw-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .rtw-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .rtw-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .rtw-cartel-nombres { margin: 0; font-family: var(--rtw-serif), 'Righteous', cursive; font-weight: 500;
  ...
  .rtw-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .rtw-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .rtw-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .rtw-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .rtw-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .rtw-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .rtw-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .rtw-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--rtw-sans), 'Manrope', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .rtw-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .rtw-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .rtw-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .rtw-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .rtw-section, .rtw-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .rtw-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .rtw-raiz { font-family: var(--rtw-sans), 'Manrope', sans-serif; }
  ...
  .rtw-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .rtw-kicker, .rtw-panel-top, .rtw-panel-pie, .rtw-meta, .rtw-riel-top, .rtw-riel-etiqueta,
  .rtw-ticket-kicker, .rtw-cuenta-etq, .rtw-tarjeta-kicker, .rtw-fila-etq, .rtw-portada-pase {
    font-family: var(--rtw-sans), 'Manrope', sans-serif; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .rtw-h2, .rtw-frase, .rtw-tarjeta-titulo, .rtw-std-dia, .rtw-cartel-nombres, .rtw-ticket-nombre, .rtw-cifra, .rtw-cuenta-num {
    font-family: var(--rtw-serif), 'Righteous', cursive; letter-spacing: -.03em; }
  .rtw-h2 em, .rtw-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .rtw-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .rtw-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .rtw-std-fila { justify-content: space-between; }
  .rtw-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--rtw-serif), 'Righteous', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .rtw-std-anio { font-family: var(--rtw-serif), 'Righteous', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .rtw-std .rtw-kicker, .rtw-std .rtw-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .rtw-foto-marco { border: 3px solid var(--pp-ink); }
  .rtw-ventana { clip-path: none !important; box-shadow: none; }
  .rtw-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--rtw-serif), 'Righteous', cursive; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .rtw-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .rtw-tarjeta--izq, .rtw-tarjeta--der, .rtw-tarjeta--talon, .rtw-tarjeta--hoy { transform: none; }
  .rtw-doblez::before { display: none; }
  .rtw-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .rtw-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .rtw-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .rtw-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .rtw-cuenta-caja:first-child { border-left: none; }
  .rtw-cuenta-caja::before { display: none; }
  .rtw-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .rtw-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .rtw-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .rtw-panel[data-tone="dark"] .rtw-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .rtw-panel-top, .rtw-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .rtw-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .rtw-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .rtw-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .rtw-btn-solido, .rtw-btn-papel, .rtw-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .rtw-btn-papel { background: transparent; border: 1px solid currentColor; }
  .rtw-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .rtw-portada { background: var(--pp-bg); }
  .rtw-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .rtw-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .rtw-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .rtw-cartel-chinche { display: none; }
  .rtw-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .rtw-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .rtw-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Retrowave: la grilla en fuga sobre el horizonte, y el titular con el
     resplandor de un tubo de neón. */
  .rtw-cartel-nombres, .rtw-h2 { text-shadow: 0 0 22px color-mix(in srgb, var(--pp-acc) 60%, transparent); }
  .rtw-trama { opacity: .22; background-image: linear-gradient(color-mix(in srgb, var(--pp-acc) 40%, transparent) 1px, transparent 1px), linear-gradient(90deg, color-mix(in srgb, var(--pp-acc) 40%, transparent) 1px, transparent 1px); background-size: 40px 40px; }

  @media (prefers-reduced-motion: reduce) {
    .rtw-raiz * { animation: none !important; }
    .rtw-scroller [data-xin] { opacity: 1 !important; --rtw-y: 0px; }
  }
`;
