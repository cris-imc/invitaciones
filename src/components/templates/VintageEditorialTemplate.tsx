...
export function VintageEditorialTemplate({ invitation, guest, isPersonalized = false }: VintageEditorialTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#EFE3C8";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C9A66B";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(201,166,107,.35), 0 18px 50px -30px #C9A66B";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(201,166,107,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(201,166,107,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#8B4B3B" : "#C9A66B";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#150F0A", tinta: "#F4F1EA", acento: "#C9A66B", clase: `${vtePlayfair.variable} ${vteMono.variable}`, fuente: "var(--vte-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="vte-scroller">
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="vte-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #150F0A 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="vte-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="vte-pan-sticky">
            <div data-strip="1" className="vte-strip">
              ...
              <div data-tone="dark" className="vte-panel vte-panel--center" style={{ background: "#1B140D", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="vte-section" style={{ background: "#1B140D" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="vte-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="vte-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#EFE3C8" : "#C9A66B" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="vte-section" style={{ background: "#1B140D" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.sabor.tuEdicion")} className="vte-section vte-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #1B140D 55%, #150F0A 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C9A66B" guest={guest} />
      )}
...
const VTE_CSS = `
  ...
  a { color: #C9A66B; text-decoration: none; }
  a:hover { color: #EFE3C8; }
  ...
  .vte-date-month { font-family: var(--vte-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C9A66B; padding-left: 12%; }

  .vte-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .vte-divider-line { width: 52px; height: 1px; background: #C9A66B; display: inline-block; }
  ...
  .vte-cal-link:hover { color: #C9A66B; }
  ...
  .vte-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #8B4B3B, #3B2A1E, #C9A66B, #EFE3C8, #8B4B3B); filter: saturate(.75); }
  .vte-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .vte-medallion-label { font-family: var(--vte-playfair), serif; font-size: 26px; line-height: 1; color: #EFE3C8; }
  .vte-medallion-label-sm { font-family: var(--vte-playfair), serif; font-size: 16px; color: #EFE3C8; }
  .vte-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .vte-medallion-sub--accent { color: #C9A66B; }
  ...
  .vte-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C9A66B; opacity: 0.4; font-family: var(--vte-mono), monospace; }

  .vte-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(201,166,107,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .vte-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #EFE3C8, transparent); animation: vteScan 6s linear infinite; pointer-events: none; }

  .vte-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .vte-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .vte-cd-num { font-family: var(--vte-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6EFDD; display: inline-block; }
  .vte-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C9A66B; }
  .vte-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #150F0A 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .vte-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(139,75,59,0.28), rgba(59,42,30,0.3), rgba(201,166,107,0.32), rgba(139,75,59,0.28)); filter: blur(80px); opacity: .4; animation: vteFoil 30s linear infinite; }
  ...
  .vte-accent-italic { font-style: italic; color: #C9A66B; }
  .vte-accent-serif { font-style: italic; color: #8B4B3B; font-family: var(--vte-playfair), serif; }
  .vte-accent-serif-2 { color: #8B4B3B; }
  ...
  .vte-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #8B4B3B; margin-top: auto; }
  ...
  .vte-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #8B4B3B; }
  ...
  .vte-crono-time { font-family: var(--vte-mono), monospace; color: #8B4B3B; min-width: 42px; }
  ...
  .vte-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .vte-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #EFE3C8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .vte-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(201,166,107,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .vte-rsvp-payment-total { color: #EFE3C8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .vte-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C9A66B; background: transparent; color: #C9A66B; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .vte-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(201,166,107,0.3); color: #F4F1EA; font-family: var(--vte-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .vte-rsvp-input::placeholder { color: #6E6A5D; }
  .vte-rsvp-input:focus { outline: none; border-bottom-color: #C9A66B; }
  .vte-rsvp-btn { width: 100%; padding: 16px; font-family: var(--vte-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C9A66B, #A6813F); border: 1px solid #C9A66B; color: #0B0B10; cursor: pointer; }
  .vte-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .vte-rsvp-btn--ghost { background: transparent; color: #C9A66B; }
  .vte-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C9A66B; margin: 0; }
  ...
  .vte-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(201,166,107,0.18); }
  ...
  .vte-bank-copy { flex-shrink: 0; font-family: var(--vte-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C9A66B; background: transparent; color: #C9A66B; cursor: pointer; }
  .vte-bank-copy:hover { background: rgba(201,166,107,0.12); }
  ...
  .vte-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(201,166,107,0.3); padding-bottom: 12px; }
  ...
  .vte-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(201,166,107,0.3); color: #F4F1EA; font-family: var(--vte-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .vte-song-input::placeholder { color: #6E6A5D; }
  .vte-song-input:focus { outline: none; border-bottom-color: #C9A66B; }
  .vte-song-sep { color: #8A8577; flex-shrink: 0; }
  .vte-song-submit { flex-shrink: 0; background: none; border: none; color: #C9A66B; font-family: var(--vte-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .vte-song-submit:hover { color: #EFE3C8; }
  .vte-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .vte-song-error { font-size: 10px; color: #C9A66B; margin-top: 6px; }
  ...
  .vte-quiz-opt { font-family: var(--vte-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(201,166,107,0.4); background: transparent; color: #C9A66B; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .vte-quiz-opt:disabled { cursor: default; }
  .vte-quiz-opt--picked { background: #C9A66B; border-color: #C9A66B; color: #0B0B10; }
  .vte-quiz-opt--correct { background: #C9A66B; border-color: #C9A66B; color: #0B0B10; }
  .vte-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .vte-quiz-result { padding-top: 18px; border-top: 1px solid rgba(201,166,107,0.2); }
  .vte-quiz-result-score { margin: 0 0 6px; font-family: var(--vte-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #EFE3C8; }
  .vte-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .vte-final-card { border: 1px solid #C9A66B; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .vte-final-names { font-family: var(--vte-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #EFE3C8; }
  ...
  .vte-replay { cursor: pointer; color: #C9A66B; }
  .vte-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .vte-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(201,166,107,0.14); }
  .vte-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .vte-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(201,166,107,0.16); position: relative; }
  .vte-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#EFE3C8, #C9A66B); transition: height 260ms linear; display: block; }
  .vte-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C9A66B; transition: color 500ms ease; }
  ...
  .vte-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #241A12 0%, #1B140D 46%, #150F0A 100%); }
  .vte-cover-half--bottom .vte-cover-inner { top: auto; bottom: 0; }
  .vte-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(139,75,59,0.4), rgba(59,42,30,0.5), rgba(201,166,107,0.5), rgba(139,75,59,0.4)); filter: blur(64px); opacity: .62; animation: vteFoil 26s linear infinite; }
  .vte-cover-sunburst { position: absolute; inset: 0; background-image: radial-gradient(rgba(239,227,200,.07) .8px, transparent .8px); background-size: 3px 3px; }
  ...
  .vte-cover-names { margin: 0; font-family: var(--vte-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EFE3C8; }
  .vte-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C9A66B, transparent); display: block; }
  ...
  .vte-cover-cta { border: 1px solid #C9A66B; background: linear-gradient(100deg, rgba(201,166,107,0.08), rgba(239,227,200,0.2), rgba(201,166,107,0.08)); color: #F6EFDD; font-family: var(--vte-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .vte-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .vte-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(239,227,200,0.34), rgba(246,239,221,0.5), rgba(239,227,200,0.34)); color: #0B0B0F; }
  }
  ...
  .vte-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C9A66B; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
