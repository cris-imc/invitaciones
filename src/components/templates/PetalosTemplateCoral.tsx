 ...
 *   --t-bg #FFF5EE · --t-surface #FCE4D6 · --petalos-ink #3D241A
 *   --t-muted #9C6B54 · --t-acc #F2946B (rojo vibrante) · --t-acc2 #D9673A (rojo profundo)
 *
...
interface PetalosTemplateCoralProps {
...
export function PetalosTemplateCoral({ invitation, guest, isPersonalized = false }: PetalosTemplateCoralProps) {
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #FFFFFF 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #3D241A)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <style jsx>{`
            ...
            .petalos-cover-text { color: #FFF5EE; }
            .petalos-cover-text-muted { color: #FFF5EE; }
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #3D241A)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#F2946B" surfaceColor="#FCE4D6" inactiveColor="#3D241A" solid />}
      </div>
  );
}
