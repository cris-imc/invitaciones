...
/**
 * AcrylicPopTemplateUltraViolet.tsx
 *
 * Portado 1:1 desde el diseño "Acrylic Pop" (Colección Storytelling, mis
 * quince años: fondo tinta casi negra, acento neón violeta/turquesa/amarillo estilo
...
// Ciclo de acento violeta/turquesa/amarillo (variante "Ultravioleta Pop") del mockup para las palabras destacadas de
// la frase (mismo orden en que aparecen: prende=violeta, una=turquesa, sola=amarillo,
// vez.=fucsia otra vez).
const PHRASE_ACCENTS = ["#C13DFF", "#3DFFE0", "#FFD23D"];
...
interface AcrylicPopTemplateUltraVioletProps {
...
export function AcrylicPopTemplateUltraViolet({ invitation, guest, isPersonalized = false }: AcrylicPopTemplateUltraVioletProps) {
  const tx = useTextos();
  ...
  // plano, segunda mitad itálica ciclando violeta/turquesa/amarillo (mismo orden que
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#FFD23D";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C13DFF";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #C13DFF";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#3DFFE0" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#3DFFE0" : "#C13DFF";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#120A18", tinta: "#F4F1EA", acento: "#C13DFF", clase: `${acpPoppins.variable} ${acpMono.variable}`, fuente: "var(--acp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="acp-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="acp-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #1E1029 0%, #170D20 55%, #120A18 100%)" }}>
            ...
            <div className="acp-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="18,10,24" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="18,10,24" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="acp-section acp-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #221030 0%, #180D22 55%, #120A18 100%)" }}>
          ...
          <div className="acp-cd-grid">
            ...
            <AcpCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} numColor="#FFD23D" />
          </div>
          <div className="acp-perf-strip" />
        </section>

        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="acp-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #241232 0%, #140A1C 52%, #120A18 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="acp-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="acp-pan-sticky">
            <div data-strip="1" className="acp-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="acp-panel acp-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="acp-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#3DFFE0" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#3DFFE0" />
                  </svg>
              ...
              <div data-tone="dark" className="acp-panel acp-panel--center" style={{ background: "#170D20", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="acp-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #1E1029 0%, #170D20 60%, #120A18 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="acp-section" style={{ background: "#170D20" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="acp-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="acp-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FFD23D" : "#C13DFF" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="acp-section" style={{ background: "#170D20" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="acp-section" style={{ background: "#170D20" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="acp-section acp-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #1E1029 0%, #170D20 55%, #120A18 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C13DFF" guest={guest} />
      )}
...
const ACP_CSS = `
  ...
  a { color: #C13DFF; text-decoration: none; }
  a:hover { color: #FFD23D; }
  ...
  .acp-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #120A18; }
  ...
  @media (min-width: 768px) {
    .acp-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(193,61,255,.3); }
  ...
  .acp-date-month { font-family: var(--acp-poppins), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C13DFF; padding-left: 12%; }

  .acp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .acp-divider-line { width: 52px; height: 1px; background: #C13DFF; display: inline-block; }
  ...
  .acp-cal-link:hover { color: #C13DFF; }
  ...
  .acp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #C13DFF, #3DFFE0, #C13DFF, #FFD23D, #C13DFF); filter: saturate(.75); }
  .acp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #120A16; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .acp-medallion-num { font-family: var(--acp-poppins), sans-serif; font-size: 26px; line-height: 1; color: #FFD23D; }
  .acp-medallion-num-sm { font-family: var(--acp-poppins), sans-serif; font-size: 16px; line-height: 1; color: #FFD23D; }
  .acp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .acp-medallion-arc { position: absolute; inset: -14%; }
  .acp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C13DFF; font-family: var(--acp-mono), monospace; }
  ...
  .acp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #FFD23D, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .acp-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C13DFF; }
  .acp-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #120A18 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .acp-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(193,61,255,0.32), rgba(61,255,224,0.26), rgba(255,210,61,0.34), rgba(193,61,255,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .acp-accent-serif { font-style: italic; color: #3DFFE0; font-family: var(--acp-poppins), sans-serif; }
  .acp-accent-serif-2 { color: #3DFFE0; }
  ...
  .acp-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #3DFFE0; margin-top: auto; }
  ...
  .acp-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #3DFFE0; }
  ...
  .acp-crono-time { font-family: var(--acp-mono), monospace; color: #3DFFE0; min-width: 42px; }
  ...
  .acp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #FFD23D, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .acp-rsvp-payment-total { color: #FFD23D; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .acp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C13DFF; background: transparent; color: #C13DFF; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .acp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(193,61,255,0.3); color: #F4F1EA; font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .acp-rsvp-input::placeholder { color: #6E6A5D; }
  .acp-rsvp-input:focus { outline: none; border-bottom-color: #C13DFF; }
  .acp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--acp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); border: 1px solid #C13DFF; color: #F6EFDD; cursor: pointer; }
  .acp-rsvp-btn:hover:not(:disabled) { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #110915; }
  .acp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .acp-rsvp-btn--ghost { background: transparent; color: #C13DFF; }
  .acp-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(193,61,255,0.1); color: #C13DFF; }
  .acp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C13DFF; margin: 0; }
  ...
  .acp-bank-copy { flex-shrink: 0; font-family: var(--acp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C13DFF; background: transparent; color: #C13DFF; cursor: pointer; }
  .acp-bank-copy:hover { background: rgba(193,61,255,0.12); }
  ...
  .acp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(193,61,255,0.3); color: #F4F1EA; font-family: var(--acp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .acp-song-input::placeholder { color: #6E6A5D; }
  .acp-song-input:focus { outline: none; border-bottom-color: #C13DFF; }
  .acp-song-sep { color: #8A8577; flex-shrink: 0; }
  .acp-song-submit { flex-shrink: 0; background: none; border: none; color: #C13DFF; font-family: var(--acp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .acp-song-submit:hover { color: #FFD23D; }
  .acp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .acp-song-error { font-size: 10px; color: #C13DFF; margin-top: 6px; }
  ...
  .acp-quiz-opt { font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(193,61,255,0.4); background: transparent; color: #C13DFF; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .acp-quiz-opt:disabled { cursor: default; }
  .acp-quiz-opt--picked { background: #C13DFF; border-color: #C13DFF; color: #110915; }
  .acp-quiz-opt--correct { background: #FFD23D; border-color: #FFD23D; color: #110915; }
  .acp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .acp-quiz-result { padding-top: 18px; border-top: 1px solid #2A2A33; }
  .acp-quiz-result-score { margin: 0 0 6px; font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #FFD23D; }
  .acp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .acp-final-card { border: 1px solid #C13DFF; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .acp-replay { cursor: pointer; color: #C13DFF; }
  ...
  .acp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FFD23D, #C13DFF); transition: height 260ms linear; display: block; }
  .acp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C13DFF; transition: color 500ms ease; }
  ...
  .acp-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1C0F28 0%, #170D20 46%, #120A18 100%); }
  .acp-cover-half--bottom .acp-cover-inner { top: auto; bottom: 0; }
  .acp-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(193,61,255,0.5), rgba(61,255,224,0.4), rgba(255,210,61,0.5), rgba(193,61,255,0.5)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  /* Manchas de luz suaves (ver mockup original) -- propias de Acrylic Pop:
     ...
     plantillas de la colección. */
  .acp-cover-spots { position: absolute; inset: 0; background-image: radial-gradient(circle at 20% 20%, rgba(193,61,255,.25), transparent 40%), radial-gradient(circle at 80% 70%, rgba(61,255,224,.22), transparent 40%), radial-gradient(circle at 50% 90%, rgba(255,210,61,.18), transparent 35%); }
  ...
  .acp-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C13DFF, transparent); display: block; }
  ...
  .acp-cover-cta { border: 1px solid #C13DFF; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--acp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .acp-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .acp-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #110915; }
  }
  ...
  .acp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C13DFF; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
