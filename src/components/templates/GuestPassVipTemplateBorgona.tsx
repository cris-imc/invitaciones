...
export function GuestPassVipTemplateBorgona({ invitation, guest, isPersonalized = false }: GuestPassVipTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E9BFC7";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#BE6774";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(190,103,116,.35), 0 18px 50px -30px #BE6774";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#7A3341" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(190,103,116,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(190,103,116,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#7A3341" : "#BE6774";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A0708", tinta: "#F4F1EA", acento: "#BE6774", clase: `${gpBodoni.variable} ${gpMono.variable}`, fuente: "var(--gp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="gpv-scroller">
        <section data-tone="dark" data-screen-label="Save the Date" className="gpv-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #211116 0%, #0F0A0B 55%, #0A0708 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="gpv-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #251119 0%, #100B0D 52%, #0A0708 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="gpv-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="gpv-pan-sticky">
            <div data-strip="1" className="gpv-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="gpv-panel gpv-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="gpv-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#7A3341" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#7A3341" />
                  </svg>
              ...
              <div data-tone="dark" className="gpv-panel gpv-panel--center" style={{ background: "#0F0A0B", color: "#F4F1EA" }}>
            ...
            <div data-strip="1" className="gpv-strip">
              ...
              <div data-tone="light" className="gpv-panel gpv-panel--gap" style={{ background: "#EDE8DE", color: "#14141B" }}>
                ...
                <div className="gpv-album-embed">
                  {livePhotos.length > 0 ? (
                    <LiveAlbumStrip photos={livePhotos} tone="light" accentColor="#7A3341" />
                  ) : (
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="gpv-section" style={{ background: "#0F0A0B" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="gpv-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="gpv-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E9BFC7" : "#BE6774" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="gpv-section" style={{ background: "#0F0A0B" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="gpv-section gpv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #211116 0%, #0F0A0B 55%, #0A0708 100%)" }}>
...
const GP_CSS = `
  ...
  .gpv-date-month { font-family: var(--gp-bodoni), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #BE6774; padding-left: 12%; }

  .gpv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .gpv-divider-line { width: 52px; height: 1px; background: #BE6774; display: inline-block; }
  ...
  .gpv-cal-link:hover { color: #BE6774; }
  ...
  .gpv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #BE6774, #E9BFC7, #6E5AA8); filter: saturate(.75); }
  .gpv-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0F0A0B; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .gpv-medallion-label { font-family: var(--gp-bodoni), serif; font-size: 26px; line-height: 1; color: #E9BFC7; }
  .gpv-medallion-label-sm { font-family: var(--gp-bodoni), serif; font-size: 16px; color: #E9BFC7; }
  .gpv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .gpv-medallion-sub--accent { color: #BE6774; }
  ...
  .gpv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #BE6774; opacity: 0.4; font-family: var(--gp-mono), monospace; }

  .gpv-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(190,103,116,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .gpv-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E9BFC7, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .gpv-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .gpv-cd-box { border: 1px solid #241014; background: rgba(20,13,15,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .gpv-cd-num { font-family: var(--gp-bodoni), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F8E9EB; display: inline-block; }
  .gpv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #BE6774; }
  .gpv-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A0708 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .gpv-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }

  .gpv-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(190,103,116,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .gpv-accent-italic { font-style: italic; color: #BE6774; }
  .gpv-accent-serif { font-style: italic; color: #7A3341; font-family: var(--gp-bodoni), serif; }
  .gpv-accent-serif-2 { color: #7A3341; }
  ...
  .gpv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #7A3341; margin-top: auto; }
  ...
  .gpv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #7A3341; }
  ...
  .gpv-crono-time { font-family: var(--gp-mono), monospace; color: #7A3341; min-width: 42px; }
  ...
  .gpv-stub { position: relative; overflow: hidden; border: 1px solid #241014; background: rgba(20,13,15,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .gpv-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E9BFC7, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .gpv-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(190,103,116,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .gpv-rsvp-payment-total { color: #E9BFC7; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .gpv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #BE6774; background: transparent; color: #BE6774; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .gpv-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(190,103,116,0.3); color: #F4F1EA; font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .gpv-rsvp-input::placeholder { color: #6E6A5D; }
  .gpv-rsvp-input:focus { outline: none; border-bottom-color: #BE6774; }
  .gpv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--gp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #BE6774, #8B4552); border: 1px solid #BE6774; color: #0F0A0B; cursor: pointer; }
  .gpv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .gpv-rsvp-btn--ghost { background: transparent; color: #BE6774; }
  .gpv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #BE6774; margin: 0; }
  ...
  .gpv-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(190,103,116,0.18); }
  ...
  .gpv-bank-copy { flex-shrink: 0; font-family: var(--gp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #BE6774; background: transparent; color: #BE6774; cursor: pointer; }
  .gpv-bank-copy:hover { background: rgba(190,103,116,0.12); }
  ...
  .gpv-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(190,103,116,0.3); padding-bottom: 12px; }
  ...
  .gpv-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(190,103,116,0.3); color: #F4F1EA; font-family: var(--gp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .gpv-song-input::placeholder { color: #6E6A5D; }
  .gpv-song-input:focus { outline: none; border-bottom-color: #BE6774; }
  .gpv-song-sep { color: #8A8577; flex-shrink: 0; }
  .gpv-song-submit { flex-shrink: 0; background: none; border: none; color: #BE6774; font-family: var(--gp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .gpv-song-submit:hover { color: #E9BFC7; }
  .gpv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .gpv-song-error { font-size: 10px; color: #BE6774; margin-top: 6px; }
  ...
  .gpv-quiz-opt { font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(190,103,116,0.4); background: transparent; color: #BE6774; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .gpv-quiz-opt:disabled { cursor: default; }
  .gpv-quiz-opt--picked { background: #BE6774; border-color: #BE6774; color: #0F0A0B; }
  .gpv-quiz-opt--correct { background: #BE6774; border-color: #BE6774; color: #0F0A0B; }
  .gpv-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .gpv-quiz-result { padding-top: 18px; border-top: 1px solid rgba(190,103,116,0.2); }
  .gpv-quiz-result-score { margin: 0 0 6px; font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E9BFC7; }
  .gpv-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .gpv-final-card { border: 1px solid #BE6774; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .gpv-final-names { font-family: var(--gp-bodoni), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E9BFC7; }
  ...
  .gpv-replay { cursor: pointer; color: #BE6774; }
  .gpv-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .gpv-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(190,103,116,0.14); }
  .gpv-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .gpv-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(190,103,116,0.16); position: relative; }
  .gpv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E9BFC7, #BE6774); transition: height 260ms linear; display: block; }
  .gpv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #BE6774; transition: color 500ms ease; }
  ...
  .gpv-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1F1015 0%, #0F0A0B 46%, #0A0708 100%); }
  .gpv-cover-half--bottom .gpv-cover-inner { top: auto; bottom: 0; }
  .gpv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.7), rgba(78,127,134,0.45), rgba(190,103,116,0.65), rgba(110,90,168,0.7)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  .gpv-cover-sunburst { position: absolute; inset: 0; background: repeating-conic-gradient(from 0deg at 50% 44%, rgba(233,191,199,0.75) 0deg 0.4deg, transparent 0.4deg 3.2deg); opacity: .3; -webkit-mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); }
  ...
  .gpv-cover-names { margin: 0; font-family: var(--gp-bodoni), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E9BFC7; }
  .gpv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#BE6774, transparent); display: block; }
  ...
  .gpv-cover-cta { border: 1px solid #BE6774; background: linear-gradient(100deg, rgba(190,103,116,0.08), rgba(233,191,199,0.2), rgba(190,103,116,0.08)); color: #F8E9EB; font-family: var(--gp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .gpv-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .gpv-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(233,191,199,0.34), rgba(248,233,235,0.5), rgba(233,191,199,0.34)); color: #0F0A0B; }
  }
  ...
  .gpv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #BE6774; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
