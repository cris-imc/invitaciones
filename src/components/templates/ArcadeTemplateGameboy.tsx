 ...
 * está en scripts/familias/tipografica/arc.json.
 *
      ...
      if (sc) {
        ...
        if (!menosMovimiento) {
          ...
          if (ven) {
            ...
            ven.style.setProperty("--arc-punto", (7.2 * (1 - t)).toFixed(2));
          }
        }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="arc-scroller">
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="arc-section arc-std">
          <div className="arc-trama arc-trama--media" aria-hidden="true" />
          <div className="arc-spread">
            <div className="arc-pagina">
              <div className="arc-folio">
              ...
              <div className="arc-fecha">
                <span data-xin="1" data-dist="-160" className="arc-fecha-linea">{diaNum}</span>
                <span data-xin="1" data-dist="160" data-delay="120" className="arc-fecha-linea arc-fecha-linea--acc">{mesLargo.slice(0, 3)}</span>
                <span data-xin="1" data-dist="-160" data-delay="240" className="arc-fecha-linea">{anio}</span>
              </div>
              <div data-xin="1" data-delay="360" className="arc-fecha-pie">
            ...
            {hayFoto && (
              <div ref={ventanaRef} data-xin="1" data-delay="200" data-dist="0" className="arc-foto">
                {fotoMobile && (
                ...
                {/* La trama que tapa la foto y se disuelve al subir: el radio
                    del punto lo mueve el motor en --arc-punto. */}
                <span className="arc-foto-revelado" aria-hidden="true" />
                <span className="arc-foto-anio">{anio}</span>
                <span className="arc-foto-pie">{tx("invitacion.album.nuestraFoto").toUpperCase()}</span>
              </div>
            )}
          </div>
        </section>
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="arc-section arc-countdown">
          ...
          <div className="arc-marquesina" aria-hidden="true">
            <div className="arc-marquesina-tira">
              {[0, 1].map((i) => (
                <span key={i}>
          ...
          <div className="arc-marquesina arc-marquesina--contraria" aria-hidden="true">
            <div className="arc-marquesina-tira">
              {[0, 1].map((i) => (
                <span key={i}>
        ...
        {hayFrase && (
          <section data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="arc-section arc-frase-seccion">
            <div className="arc-folio">
            ...
            <div className="arc-spread">
              ...
              <div data-xin="1" data-delay="900" data-dist="60" className="arc-sello">
                <span>{tx("invitacion.frase.conAmor")}</span>
              </div>
            </div>
            <div className="arc-folio arc-folio--pie">
        ...
        >
          <div className="arc-pan-fijo">
            <div data-strip="1" className="arc-tira">
              <div data-tone={TONO} className="arc-panel">
                <div className="arc-folio">
                  <span>{nCuando} — {tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="arc-spread">
                  <h2 className="arc-panel-titulo">
                    {(lugarNombre || tx("invitacion.ubicacion.elLugar")).split(" ")[0]}
                    <br /><span className="arc-acento">{(lugarNombre || "").split(" ").slice(1).join(" ") || ciudad}</span>
                  </h2>
                  <div className="arc-lineas">
                    <div className="arc-linea"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {direccion && <div className="arc-linea"><span>{tx("invitacion.ubicacion.direccion")}</span><span>{direccion}</span></div>}
                    {dressCode && <div className="arc-linea"><span>{tx("invitacion.ubicacion.dressCode")}</span><span>{dressCode}</span></div>}
                    {mapUrl && (
                      <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="arc-cta">
                        {tx("invitacion.ubicacion.comoLlegar")}<span className="arc-cta-flecha">↗</span>
                      </a>
                    )}
                  </div>
                </div>
              ...
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="dark" className="arc-panel">
                  <div className="arc-folio">
                  ...
                  <div className="arc-spread">
                    <h2 className="arc-panel-titulo">
                      ...
                      <br /><span className="arc-acento">{(ceremoniaNombre || "").split(" ").slice(1).join(" ") || ceremoniaTitulo}</span>
                    </h2>
                    <div className="arc-lineas">
                      {ceremoniaHora && <div className="arc-linea"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>}
                      {ceremoniaDireccion && <div className="arc-linea"><span>{tx("invitacion.ubicacion.direccion")}</span><span>{ceremoniaDireccion}</span></div>}
                    </div>
                  </div>
                  <div className="arc-folio arc-folio--pie">
              ...
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="arc-panel">
                  <div className="arc-folio">
                  ...
                  <div className="arc-spread">
                    ...
                    <div className="arc-lineas">
                      ...
                      <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="arc-cta">
                        {tx("invitacion.ubicacion.abrirEnMapas")}<span className="arc-cta-flecha">↗</span>
                      </a>
                    </div>
                  </div>
                  <div className="arc-folio arc-folio--pie">
              ...
              {cronograma.length > 0 && (
                <div id="schedule" data-tone="dark" className="arc-panel arc-panel--acento">
                  <div className="arc-folio">
                  ...
                  <div className="arc-spread">
                    <h2 className="arc-panel-titulo">
                      ...
                      <br /><span className="arc-acento arc-acento--tinta">{tx("invitacion.ubicacion.laNochePasoAPaso").split(",").slice(1).join(",").trim()}</span>
                    </h2>
                    <div className="arc-lineas">
                      {cronograma.map((item, i) => (
                        <div key={i} className="arc-linea"><span>{item.time || ""}</span><span>{item.title}</span></div>
                      ))}
                    </div>
                  </div>
                  <div className="arc-folio arc-folio--pie">
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="arc-section arc-checkin">
            ...
            <div className="arc-spread">
              <div className="arc-pagina">
                <h2 data-xin="1" data-dist="-80" className="arc-h2">
                  {tx("invitacion.rsvp.confirmaLinea1")}<br /><span className="arc-acento">{tx("invitacion.rsvp.confirmaLinea2")}</span>
                </h2>
              </div>
              <div className="arc-cupon">
                <span className="arc-cupon-corte" aria-hidden="true" />
                <CheckinArcade
            ...
            <div className="arc-pan-fijo">
              <div data-strip="1" className="arc-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="arc-panel arc-panel--album">
                    <div className="arc-folio">
                    ...
                    {iHoja === 0 && (
                      <h2 className="arc-h2 arc-h2--album">
                        {tx("invitacion.album.titulo")} <span className="arc-acento">{tx("invitacion.album.deFotos")}</span>
                      </h2>
                    )}
                    <div className="arc-contactos" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="arc-contacto"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="arc-contacto-img" />
                          <span className="arc-contacto-tinta" aria-hidden="true" />
                          <span className="arc-contacto-n">{String(i + 1).padStart(2, "0")}</span>
                        </div>
                      ))}
                    </div>
                    <div className="arc-folio arc-folio--pie">
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="arc-section arc-musica">
            <div className="arc-folio">
            ...
            <div className="arc-spread">
              <div className="arc-pagina">
                ...
                <div data-xin="1" data-delay="120" className="arc-eq" aria-hidden="true">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="arc-section arc-regalos">
            ...
            <div className="arc-spread">
              <div className="arc-pagina">
                <h2 data-xin="1" data-dist="-80" className="arc-h2">
                  {tx("invitacion.regalos.siQueresLinea1")}<br /><span className="arc-acento">{tx("invitacion.regalos.siQueresLinea2")}</span>
                </h2>
                {Boolean(invitation.regaloMensaje) && (
                  <p data-xin="1" data-delay="120" className="arc-parrafo">{String(invitation.regaloMensaje)}</p>
                )}
              </div>
              <div className="arc-pagina">
                {regaloHabilitado && (
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="arc-section arc-quiz">
            ...
            <div className="arc-spread">
              <div className="arc-pagina">
                <h2 data-xin="1" data-dist="-80" className="arc-h2">{triviaTitulo}</h2>
              </div>
              <div className="arc-pagina">
                <TriviaArcade
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="arc-section arc-pase">
          ...
          <div className="arc-spread">
            <div data-xin="1" data-dist="-60" className="arc-pagina arc-pagina--qr">
              <QrDeIngreso guest={guest as never} />
            </div>
            <div className="arc-pagina">
              <div data-xin="1" data-delay="100" className="arc-pase-cabeza">
                <div className="arc-pase-numero">
                  <span className="arc-folio-etq">{tx("invitacion.pase.pase").toUpperCase()} Nº</span>
                  <span>{pase}</span>
                </div>
                <Sello texto={`${titulo} · ${fechaPuntos} · `} />
              </div>
              <div className="arc-lineas">
                <div className="arc-linea"><span>{saludaAlInvitado ? tx("invitacion.pase.reservadoPara").toUpperCase() : tx("invitacion.evento.invitado").toUpperCase()}</span><span>{nombreInvitado || titulo}</span></div>
                {lugaresDelPase > 0 && (
                  <div className="arc-linea"><span>{tx("invitacion.pase.lugares").toUpperCase()}</span><span>{lugaresDelPase}</span></div>
                )}
                {guest?.mesas && guest.mesas.length > 0 && (
                  <div className="arc-linea"><span>{tx("invitacion.pase.tuMesa").toUpperCase()}</span><span>{guest.mesas.join(" · ")}</span></div>
                )}
                <div className="arc-linea"><span>{tx("invitacion.ubicacion.horario").toUpperCase()}</span><span>{fechaPuntos} · {hora} H</span></div>
              </div>
              <div className="arc-info-extra">
      ...
      <div ref={portadaRef} data-tone={TONO} className="arc-portada">
        <div ref={escenaPortadaRef} className="arc-portada-hoja">
          <div className="arc-trama arc-trama--tapa" aria-hidden="true" />

          <div data-cl="1" className="arc-folio">
          ...
          <div data-cl="2" className="arc-tapa-centro">
            <div className="arc-tapa-fila">
              <span className="arc-tapa-fecha">{diaSemana} {diaNum} · {mesLargo.toUpperCase()} · {anio}</span>
              <Sello texto={`${tx(invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.nosCasamos")} · ${fechaPuntos} · `} amp />
            </div>
            <h1 ref={cartelRef} className="arc-tapa-nombres">
              {saludaAlInvitado ? (
                <span className="arc-tapa-linea"><span data-pieza="1">{nombreInvitado}</span></span>
              ) : (
                <>
                  <span className="arc-tapa-linea"><span data-pieza="1">{nombre1}</span></span>
                  {nombre2 && (
                    <span className="arc-tapa-linea arc-tapa-linea--sangra">
                      <span data-pieza="1"><span className="arc-acento">&amp;</span>{nombre2}</span>
                    </span>
                  )}
                </>
              )}
          ...
          <div data-cl="3" className="arc-tapa-pie">
            <span className="arc-regla" aria-hidden="true" />
            <p className="arc-tapa-mensaje">
              {saludaAlInvitado
            ...
            <button type="button" onClick={abrir} className="arc-tapa-btn">
...
function Sello({ texto, amp = false }: { texto: string; amp?: boolean }) {
  ...
  return (
    <div className="arc-sello-circular" aria-hidden="true">
      ...
      {amp && <span className="arc-sello-amp">&amp;</span>}
    </div>
  );
}
...
function CuentaArcade({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="arc-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={i * 100} data-dist={i % 2 === 0 ? -80 : 80} className={`arc-cuenta-caja arc-cuenta-caja--${i + 1}`}>
          <span className="arc-cuenta-num"><span key={c.v}>{c.v}</span></span>
...
const CSS_ARC = `
  ...
  .arc-raiz a { color: inherit; text-decoration: none; }
  ...
  .arc-trama { position: absolute; inset: 0; pointer-events: none; z-index: 0; opacity: .16; color: currentColor;
    background-image: radial-gradient(currentColor 1.1px, transparent 1.2px); background-size: 9px 9px; }
  .arc-trama--media { opacity: .14; bottom: 45%; background-size: 12px 12px; }
  .arc-trama--tapa { -webkit-mask-image: linear-gradient(180deg, transparent 30%, #000 100%);
    mask-image: linear-gradient(180deg, transparent 30%, #000 100%); }

  /* ── El pliego ─────────────────────────────────────────────────────── */
  .arc-section { position: relative; z-index: 1; min-height: calc(var(--vh, 1vh) * 100); box-sizing: border-box;
    display: flex; flex-direction: column; justify-content: space-between; gap: 26px;
    padding: 64px max(22px, calc((100% - 1100px) / 2)) 80px; background: var(--pp-bg); color: var(--pp-ink); }
  .arc-section[data-tone="dark"] { background: var(--pp-ink); color: var(--pp-bg); }

  /* El folio: el renglón de arriba y el de abajo de cada pliego. */
  .arc-folio { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-start; gap: 16px;
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .22em;
    color: color-mix(in srgb, currentColor 62%, transparent); }
  .arc-folio--pie { align-items: center; margin-top: auto; }
  .arc-folio-etq { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .22em;
  ...
  .arc-spread { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 24px; }
  .arc-pagina { display: flex; flex-direction: column; gap: 16px; min-width: 0; }
  @media (min-width: 900px) {
    .arc-spread { flex-direction: row; align-items: flex-start; gap: 40px; }
    .arc-spread > * { flex: 1 1 0; min-width: 0; }
  }

  /* ── Tipos ─────────────────────────────────────────────────────────── */
  .arc-h2, .arc-panel-titulo, .arc-frase {
    position: relative; z-index: 1; margin: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive;
    font-weight: 400; line-height: .94; letter-spacing: -.035em; }
  .arc-h2 { font-size: clamp(40px, 12vw, 96px); }
  .arc-h2--album { font-size: clamp(34px, 9vw, 64px); }
  .arc-panel-titulo { font-size: clamp(48px, 15vw, 130px); }
  .arc-frase { font-size: clamp(30px, 8vw, 68px); line-height: 1.04; text-wrap: pretty; }
  .arc-acento { font-style: italic; color: var(--pp-acc); }
  .arc-acento--tinta { color: var(--pp-ink); }
  .arc-parrafo { margin: 0; font-size: 15px; line-height: 1.5; max-width: 34ch;
    color: color-mix(in srgb, currentColor 72%, transparent); }
  .arc-link { display: inline-flex; align-items: center; min-height: 28px; border-bottom: 2px solid var(--pp-acc); padding-bottom: 2px; }
  .arc-regla { display: block; height: 2px; background: currentColor; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .arc-std { justify-content: center; }
  .arc-fecha { display: flex; flex-direction: column; font-family: var(--arc-serif), 'Press Start 2P', cursive;
    line-height: .82; letter-spacing: -.04em; }
  .arc-fecha-linea { font-size: clamp(64px, 22vw, 180px); text-transform: lowercase; }
  .arc-fecha-linea--acc { font-style: italic; color: var(--pp-acc); text-align: right; }
  .arc-fecha-pie { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 10px;
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 13px; letter-spacing: .12em; }
  /* La foto va enmarcada como una foto de tapa, con el año encima. */
  .arc-foto { position: relative; width: 100%; aspect-ratio: 4 / 5; border: 3px solid currentColor; box-sizing: border-box;
  ...
  .arc-foto-revelado { position: absolute; inset: 0; z-index: 1; pointer-events: none;
    background-image: radial-gradient(var(--pp-ink) calc(var(--arc-punto, 7.2) * 1px), transparent calc(var(--arc-punto, 7.2) * 1px + .6px));
    background-size: 10px 10px; }
  .arc-foto-anio { position: absolute; right: 12px; top: 8px; z-index: 2; font-family: var(--arc-serif), 'Press Start 2P', cursive;
    font-style: italic; font-size: 34px; line-height: 1; color: var(--pp-acc); }
  .arc-foto-pie { position: absolute; left: 14px; bottom: 12px; z-index: 2; font-family: var(--arc-sans), 'Rubik', sans-serif;
  ...
  .arc-countdown { justify-content: space-between; }
  .arc-marquesina { position: relative; z-index: 1; overflow: hidden; border-top: 2px solid currentColor; border-bottom: 2px solid currentColor;
    padding: 8px 0; font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 12px; letter-spacing: .2em; text-transform: uppercase; }
  .arc-marquesina-tira { display: flex; width: max-content; animation: ebnCorre 26s linear infinite; }
  .arc-marquesina--contraria .arc-marquesina-tira { animation-direction: reverse; }
  ...
  .arc-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; }
  .arc-cuenta-caja { display: flex; flex-direction: column; gap: 6px; padding: 18px 14px 20px; overflow: hidden; }
  .arc-cuenta-caja:nth-child(1) { border-right: 2px solid currentColor; border-bottom: 2px solid currentColor; }
  .arc-cuenta-caja:nth-child(2) { border-bottom: 2px solid currentColor; }
  .arc-cuenta-caja:nth-child(3) { border-right: 2px solid currentColor; }
  .arc-cuenta-num, .arc-cuenta-dias, .arc-cifra { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-weight: 400;
    font-size: clamp(64px, 20vw, 150px); line-height: .82; letter-spacing: -.04em; font-variant-numeric: tabular-nums; }
  .arc-cuenta-caja:nth-child(4) .arc-cuenta-num { font-style: italic; color: var(--pp-acc); }
  .arc-cuenta-etq { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .24em;
    text-transform: uppercase; color: var(--pp-acc); }
  .arc-cuenta-aviso { display: flex; flex-direction: column; gap: 8px; }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .arc-frase-seccion { background: var(--pp-acc) !important; color: var(--pp-bg); }
  .arc-frase-seccion .arc-acento { color: var(--pp-bg); font-style: italic; }
  .arc-sello { align-self: flex-start; border: 2px solid currentColor; padding: 10px 16px; transform: rotate(-3deg);
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 12px; letter-spacing: .2em; text-transform: uppercase; }

  /* ── Paneles ───────────────────────────────────────────────────────── */
  .arc-pan { position: relative; z-index: 1; height: calc(100vh + var(--st-pasos, 2) * 90vh); }
  .arc-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; background: var(--pp-bg); }
  ...
  .arc-panel[data-tone="dark"] { background: var(--pp-ink); color: var(--pp-bg); }
  .arc-panel--acento { background: var(--pp-acc) !important; color: var(--pp-bg); }
  .arc-panel--acento .arc-acento { color: var(--pp-ink); }
  ...
  .arc-lineas { display: flex; flex-direction: column; border-top: 2px solid currentColor; }
  .arc-linea { display: flex; justify-content: space-between; gap: 16px; padding: 12px 0; border-bottom: 1px solid color-mix(in srgb, currentColor 30%, transparent); }
  .arc-linea > span:first-child { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 12px; letter-spacing: .14em;
    text-transform: uppercase; color: color-mix(in srgb, currentColor 66%, transparent); flex: 0 0 auto; }
  .arc-linea > span:last-child { text-align: right; font-size: 15px; }
  .arc-cta { margin-top: 14px; min-height: 48px; display: flex; align-items: center; justify-content: space-between;
    border: 2px solid currentColor; padding: 0 16px; font-family: var(--arc-sans), 'Rubik', sans-serif;
    font-size: 12px; letter-spacing: .18em; text-transform: uppercase; }
  .arc-cta-flecha { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-style: italic; font-size: 22px; }
  .arc-mapa { height: 190px; border: 2px solid currentColor; overflow: hidden; margin-top: 14px; }
  .arc-puntos { position: absolute; left: 0; right: 40px; bottom: 30px; display: flex; gap: 8px; justify-content: center; z-index: 2; }
  .arc-punto { width: 28px; height: 3px; transition: background 300ms ease; display: inline-block; }

  /* ── 05 Check-in: el cupón ─────────────────────────────────────────── */
  .arc-checkin { background: var(--pp-bg2); }
  .arc-cupon { position: relative; background: #FFFFFF; color: var(--pp-ink); border: 3px solid var(--pp-ink);
    padding: 26px 18px 18px; display: flex; flex-direction: column; gap: 14px; }
  .arc-cupon-corte { position: absolute; left: -3px; right: -3px; top: 52px; border-top: 2px dashed var(--pp-ink); }
  .arc-cupon .arc-talon-top { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .2em; }
  .arc-cupon input, .arc-cupon .arc-input { border: 2px solid var(--pp-ink); border-radius: 0; background: transparent; }
  .arc-cupon .arc-contador button { border: 2px solid var(--pp-ink); }
  .arc-sello, .arc-cupon .arc-sello { color: inherit; }

  /* ── 06 Álbum: hoja de contactos ───────────────────────────────────── */
  .arc-panel--album { background: color-mix(in srgb, var(--pp-bg) 92%, var(--pp-ink)); }
  .arc-contactos { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr);
    grid-auto-rows: 1fr; gap: 10px; }
  @media (min-width: 900px) { .arc-contactos { grid-template-columns: repeat(6, 1fr); } }
  .arc-contacto { position: relative; overflow: hidden; border: 1px solid color-mix(in srgb, currentColor 30%, transparent); cursor: pointer; }
  .arc-contacto-img { width: 100%; height: 100%; object-fit: cover; display: block; filter: grayscale(1) contrast(1.1); }
  .arc-contacto-tinta { position: absolute; inset: 0; background: var(--pp-acc); mix-blend-mode: multiply; opacity: .18; }
  .arc-contacto-n { position: absolute; left: 6px; bottom: 4px; font-family: var(--arc-sans), 'Rubik', sans-serif;
    font-size: 10px; letter-spacing: .14em; color: #FFFFFF; mix-blend-mode: difference; }

  /* ── 07 Música ─────────────────────────────────────────────────────── */
  .arc-eq { display: flex; align-items: flex-end; gap: 6px; height: 40px; }
  .arc-eq span { width: 6px; height: 100%; background: currentColor; transform-origin: bottom; animation: ebnEq 1.1s ease-in-out infinite; }
  @keyframes ebnEq { 0%, 100% { transform: scaleY(.25); } 50% { transform: scaleY(1); } }
  .arc-lista { display: flex; flex-direction: column; border-top: 2px solid currentColor; }
  .arc-lista-fila { display: flex; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid color-mix(in srgb, currentColor 30%, transparent); }
  .arc-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .arc-lista-tema { font-size: 15px; }
  .arc-lista-quien { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .12em;
    color: color-mix(in srgb, currentColor 62%, transparent); }

  /* ── 08 Regalos: fichas blancas ────────────────────────────────────── */
  .arc-tarjeta { position: relative; z-index: 1; background: #FFFFFF; color: var(--pp-ink); border: 3px solid var(--pp-ink);
    padding: 18px; display: flex; flex-direction: column; gap: 12px; transform: none !important; box-shadow: none; }
  .arc-tarjeta + .arc-tarjeta { margin-top: 12px; }
  .arc-tarjeta-kicker { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .2em; text-transform: uppercase; }
  .arc-tarjeta-titulo { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 28px; line-height: 1; }
  .arc-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }
  .arc-tarjeta .arc-fila { border-bottom: 1px solid color-mix(in srgb, var(--pp-ink) 22%, transparent); }
  .arc-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 10px 0; }
  .arc-fila--ultima { border-bottom: none; }
  .arc-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .arc-fila-etq { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .arc-fila-dato { font-size: 15px; overflow-wrap: anywhere; }
  .arc-fila-valor { text-align: right; }
  .arc-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 2px solid var(--pp-ink); background: transparent;
    color: var(--pp-ink); font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .14em;
    text-transform: uppercase; cursor: pointer; }
  .arc-btn-copiar--hecho { background: var(--pp-ink); color: #FFFFFF; }

  /* ── 09 Trivia: el pliego del acento ───────────────────────────────── */
  .arc-quiz { background: var(--pp-acc) !important; color: var(--pp-bg); }
  .arc-quiz .arc-acento { color: var(--pp-ink); }
  .arc-opciones { display: flex; flex-direction: column; gap: 10px; }
  .arc-opcion { min-height: 52px; text-align: left; padding: 0 16px; border: 2px solid currentColor; background: transparent;
    color: inherit; font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 200ms ease, color 200ms ease; }
  .arc-opcion--bien { background: var(--pp-bg); color: var(--pp-ink); }
  .arc-opcion--mal { opacity: .55; }

  /* ── 10 Tu pase ────────────────────────────────────────────────────── */
  .arc-pase { background: var(--pp-ink); color: var(--pp-bg); }
  .arc-pagina--qr { align-items: flex-start; }
  .arc-pagina--qr .qr-ingreso, .arc-pagina--qr section { background: transparent !important; border: none !important; padding: 0 !important; }
  .arc-pase-cabeza { display: flex; align-items: flex-end; justify-content: space-between; gap: 14px; }
  .arc-pase-numero { display: flex; flex-direction: column; }
  .arc-pase-numero > span:last-child { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: clamp(44px, 12vw, 86px); line-height: .9; }
  .arc-info-extra { margin-top: 12px; }
  .arc-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .arc-info-extra #ia-trigger-btn { background: transparent !important; color: inherit !important; border: 2px solid currentColor !important;
    border-radius: 0 !important; font-family: var(--arc-sans), 'Rubik', sans-serif !important; letter-spacing: .18em !important; }
  /* Los íconos de los componentes compartidos no entran: acá el dibujo es la
     tipografía. */
  .arc-raiz .ia-icon-box, .arc-raiz svg.lucide { display: none !important; }
  .arc-replay { cursor: pointer; }
  .arc-credito { display: flex; justify-content: center; opacity: .6; }
  .arc-error { margin: 0; font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 12px; }

  /* ── El sello circular ─────────────────────────────────────────────── */
  .arc-sello-circular { position: relative; width: clamp(72px, 18vw, 96px); aspect-ratio: 1; flex: 0 0 auto; color: var(--pp-acc); }
  .arc-sello-circular svg { position: absolute; inset: 0; animation: ebnGira 26s linear infinite; }
  .arc-sello-circular text { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 9.2px; letter-spacing: 1.4px; fill: currentColor; }
  .arc-sello-amp { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center;
    font-family: var(--arc-serif), 'Press Start 2P', cursive; font-style: italic; font-size: 30px; color: var(--pp-acc); }
  @keyframes ebnGira { to { transform: rotate(360deg); } }

  /* ── La tapa ───────────────────────────────────────────────────────── */
  .arc-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden; background: var(--pp-bg); color: var(--pp-ink); }
  .arc-portada-hoja { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
    padding: calc(18px + env(safe-area-inset-top)) max(22px, calc((100% - 1100px) / 2)) calc(22px + env(safe-area-inset-bottom)); }
  .arc-tapa-centro { position: relative; z-index: 1; display: flex; flex-direction: column; gap: clamp(8px, 2vh, 20px); }
  .arc-tapa-fila { display: flex; align-items: center; justify-content: space-between; gap: 16px; }
  .arc-tapa-fecha { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .22em;
    text-transform: uppercase; color: var(--pp-acc); }
  .arc-tapa-nombres { margin: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive; font-weight: 400;
    font-size: min(clamp(56px, 20vw, 180px), 15vh); line-height: .84; letter-spacing: -.035em; display: flex; flex-direction: column; }
  .arc-tapa-linea { overflow: hidden; display: block; }
  .arc-tapa-linea > span { display: block; }
  .arc-tapa-linea--sangra { padding-left: 14%; }
  .arc-tapa-pase { text-align: right; }
  .arc-tapa-pie { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 14px; }
  .arc-tapa-mensaje { margin: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: clamp(20px, 5.4vw, 26px);
    line-height: 1.2; max-width: 34ch; }
  .arc-tapa-btn { min-height: 52px; border: 2px solid var(--pp-ink); background: var(--pp-ink); color: var(--pp-bg);
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-weight: 600; font-size: 13px; letter-spacing: .2em;
    text-transform: uppercase; padding: 0 22px; cursor: pointer; transition: background 200ms ease, color 200ms ease; }
  @media (hover: hover) { .arc-tapa-btn:hover { background: var(--pp-acc); border-color: var(--pp-acc); color: var(--pp-bg); } }
  ...
  .arc-riel-top, .arc-riel-etiqueta { writing-mode: vertical-rl; font-family: var(--arc-sans), 'Rubik', sans-serif;
    font-size: 10px; letter-spacing: .28em; transition: color 500ms ease; }
  .arc-riel-top { color: var(--pp-ink2); }
  .arc-riel-etiqueta { color: var(--pp-acc); }
  .arc-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: color-mix(in srgb, var(--pp-ink) 20%, transparent); position: relative; }
  ...
  .arc-pista { position: absolute; left: 0; right: 34px; bottom: calc(18px + env(safe-area-inset-bottom)); z-index: 6; text-align: center;
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2);
  ...
  .arc-lupa { position: fixed; inset: 0; z-index: 200; background: color-mix(in srgb, var(--pp-ink) 94%, transparent);
    display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .arc-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 2px solid var(--pp-bg);
    background: transparent; color: var(--pp-bg); font-size: 18px; line-height: 1; cursor: pointer; }
  .arc-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; border: 3px solid var(--pp-bg); }

  /* ── Formularios (check-in y canciones) ────────────────────────────── */
  .arc-campo { display: flex; flex-direction: column; gap: 6px; }
  .arc-etiqueta { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .2em; text-transform: uppercase;
    color: color-mix(in srgb, currentColor 66%, transparent); }
  .arc-input { min-height: 48px; border: 2px solid currentColor; background: transparent; color: inherit;
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 16px; padding: 0 12px; border-radius: 0; }
  .arc-input:focus { outline: none; border-color: var(--pp-acc); }
  .arc-contador { display: flex; align-items: center; gap: 12px; }
  .arc-contador button { width: 48px; height: 48px; border: 2px solid currentColor; background: transparent; color: inherit;
    font-size: 20px; line-height: 1; cursor: pointer; }
  .arc-contador button:disabled { opacity: .35; cursor: default; }
  .arc-contador > span { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 36px; min-width: 40px; text-align: center; line-height: 1; }
  .arc-btn-solido { min-height: 48px; padding: 0 22px; border: 2px solid currentColor; background: currentColor; color: var(--pp-bg);
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 12px; letter-spacing: .18em; text-transform: uppercase; cursor: pointer; }
  .arc-btn-solido--tinta { background: var(--pp-acc); border-color: var(--pp-acc); color: var(--pp-bg); }
  .arc-btn-fantasma { min-height: 48px; padding: 0 22px; border: 2px solid currentColor; background: transparent; color: inherit;
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 12px; letter-spacing: .18em; text-transform: uppercase; cursor: pointer; }
  .arc-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 2px solid currentColor; padding-top: 12px; }
  .arc-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .arc-precio-total { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 28px; line-height: 1; }
  .arc-precio-detalle { font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 11px; letter-spacing: .1em; }
  .arc-talon-top { display: flex; justify-content: space-between; gap: 10px; font-family: var(--arc-sans), 'Rubik', sans-serif;
    font-size: 11px; letter-spacing: .2em; text-transform: uppercase; }
  .arc-talon-estado { transition: color 400ms ease; }
  .arc-filas { display: flex; flex-direction: column; }
  .arc-petalos { display: none; }

  /* Arcade: píxel. Press Start 2P es mucho más ancha que una serif, así que
     los titulares bajan de tamaño y suben de interlínea; la sombra es doble
     y dura, y la trama son scanlines. */
  .arc-tapa-nombres { font-size: min(clamp(20px, 6.5vw, 40px), 8vh); line-height: 1.35; letter-spacing: 0; }
  .arc-h2, .arc-frase, .arc-panel-titulo { font-size: clamp(18px, 5vw, 32px); line-height: 1.45; letter-spacing: 0; }
  .arc-fecha-linea { font-size: clamp(30px, 11vw, 78px); line-height: 1.1; }
  .arc-cuenta-num, .arc-cuenta-dias, .arc-cifra { font-size: clamp(26px, 8vw, 56px); line-height: 1.2; }
  .arc-tapa-nombres, .arc-h2, .arc-panel-titulo { text-shadow: 3px 3px 0 var(--pp-acc), 6px 6px 0 var(--pp-acc2); }
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    .arc-scroller [data-xin] { opacity: 1 !important; transform: none !important; }
    /* Sin movimiento no hay revelado: la foto se ve, sin la trama encima. */
    .arc-foto { --arc-punto: 0; }
  }
`;
