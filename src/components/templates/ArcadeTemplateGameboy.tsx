...
// ArcadeTemplate.tsx — no editar a mano: los cambios se pierden al regenerar.

/**
 * ARCADE · Colección Iconic — sub-colección "Tipográfica Editorial"
 * Variante: Game Boy.
 *
 ...
 * esta familia está en scripts/familias/tipografica/arc.json.
 *
 * La maquinita: Press Start 2P para los titulares (píxel puro), Rubik para
 * el texto -- que en píxel sería ilegible -- y la paleta de un salón de
 * juegos. Sombras duras, sin una sola curva.
 *
...
export function ArcadeTemplateGameboy({ invitation, guest, isPersonalized = false }: ArcadeTemplateProps) {
  const tx = useTextos();
  ...
  useEffect(() => {
    ...
    reveals.forEach((el) => {
      ...
      el.style.opacity = "0";
      el.style.setProperty("--arc-y", `${dist}px`);
    ...
    return (
      <PostEventoStorytelling
        ...
        paleta={{
          ...
          tinta: PALETA.ink,
          acento: PALETA.acc,
          clase: `${arcSerif.variable} ${arcSans.variable} `,
          fuente: "var(--arc-sans), 'Rubik', sans-serif",
        }}
      />
    );
  }
  ...
  return (
    <div
      ref={raizRef}
      className={`${arcSerif.variable} ${arcSans.variable} arc-raiz`}
      style={varsDePaleta}
    >
      <style>{CSS_ARC}</style>
      <style>{COVER_RESPONSIVE_STYLE}</style>

      <div ref={scrollerRef} className="arc-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="arc-section arc-std">
          <div className="arc-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="arc-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="arc-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="arc-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="arc-std-fila">
              <span className="arc-cinta">{mesLargo}</span>
              <span className="arc-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="arc-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="arc-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="arc-btn-papel arc-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="arc-foto-seccion">
            <div className="arc-foto-marco">
              <div ref={ventanaRef} className="arc-ventana">
                {fotoMobile && (
                  <div className="acp-mobile-only arc-foto-capa">
                    <AnimatedCoverPhoto photoSrc={fotoMobile} tint={false} effect="enfoque" scrimColorRgb="243,235,221" />
                  </div>
                )}
                {fotoDesktop && (
                  <div className="acp-desktop-only arc-foto-capa">
                    <AnimatedCoverPhoto photoSrc={fotoDesktop} tint={false} effect="enfoque" scrimColorRgb="243,235,221" />
                  </div>
                )}
              </div>
              <div className="arc-foto-firma">{titulo}</div>
            </div>
          </section>
        )}

        {/* ── 02 Falta poco ──────────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="arc-section arc-countdown">
          <div className="arc-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="arc-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="arc-h2">
            {tx("invitacion.cuentaRegresiva.estamosContando")}<br /><em>{tx("invitacion.cuentaRegresiva.losDias")}</em>
          </h2>
          <CuentaArcade targetDate={fechaHora} />
        </section>

        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="arc-section arc-frase-seccion">
            <div className="arc-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="arc-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            <h2 ref={fraseRef} className="arc-frase">
              {palabras.map((p, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= desdeAcento ? "arc-acento" : undefined}>{p}</span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="200" className="arc-firma">
              <span className="arc-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="arc-pan-fijo">
            <div data-strip="1" className="arc-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="arc-panel arc-panel--abajo" style={{ background: PALETA.bg }}>
                ...
                <div className="arc-tarjeta arc-tarjeta--izq">
                  <span className="arc-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="arc-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="arc-filas">
                    {direccion && (
                      <div className="arc-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="arc-fila-valor">{direccion}</span></div>
                    )}
                    <div className="arc-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="arc-fila arc-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="arc-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="arc-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="arc-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="arc-panel arc-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="arc-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="arc-tarjeta arc-tarjeta--der">
                    <h2 className="arc-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="arc-filas">
                      {ceremoniaDireccion && (
                        <div className="arc-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="arc-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="arc-fila arc-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="arc-panel arc-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="arc-escena" aria-hidden="true">
                  ...
                  <h2 className="arc-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="arc-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                    <div className="arc-mapa">
                      <iframe
                        src={embedMapUrl}
                        width="100%"
                        height="100%"
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="arc-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="arc-panel arc-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="arc-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="arc-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="arc-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="arc-crono-fila">
                        <span className="arc-crono-hora">{item.time || ""}</span>
                        <span className="arc-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* ── 05 Check-in ────────────────────────────────────────────── */}
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="arc-section arc-checkin">
            <div className="arc-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="arc-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="arc-h2">
            ...
            <div className="arc-pan-fijo">
              <div data-strip="1" className="arc-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="arc-panel arc-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="arc-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="arc-h2 arc-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="arc-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="arc-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="arc-polaroid-img" />
                          {i % 2 === 0 && <span className="arc-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="arc-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="arc-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="arc-section arc-musica">
            <div className="arc-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="arc-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="arc-h2">
              {tituloEnDosLineas(tx("invitacion.sabor.preguntaCancionFaltar"), "arc-acento")}
            </h2>
            <CancionesArcade
              invitationId={String(invitation.id ?? "")}
              guestToken={guest?.uniqueToken}
        ...
        {/* ── 08 Regalos ─────────────────────────────────────────────── */}
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="arc-section arc-regalos">
            <span data-xin="1" data-dist="-20" className="arc-kicker arc-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="arc-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="arc-parrafo arc-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {/* ── 09 Trivia ──────────────────────────────────────────────── */}
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="arc-section arc-quiz">
            <div className="arc-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="arc-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="arc-h2 arc-h2--claro">{triviaTitulo}</h2>
            <TriviaArcade
        ...
        {/* ── 10 Tu pase ─────────────────────────────────────────────── */}
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="arc-section arc-pase">
          <div className="arc-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="arc-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="arc-ticket">
            <div className="arc-ticket-izq">
              <span className="arc-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="arc-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="arc-ticket-dato">
                {[
              ...
              <span className="arc-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="arc-ticket-muesca arc-ticket-muesca--arriba" aria-hidden="true" />
              <span className="arc-ticket-muesca arc-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="arc-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="arc-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="arc-pase-pie">
            <span className="arc-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="arc-pase-creditos">
              <LogoFooterCredit bgColor="transparent" />
              <span className="arc-replay" role="button" tabIndex={0} onClick={volverAVerla} onKeyDown={(e) => { if (e.key === "Enter") volverAVerla(); }}>
      ...
      {/* ── Riel de progreso ───────────────────────────────────────────── */}
      <div ref={rielRef} className="arc-riel">
        <span ref={rielTopRef} className="arc-riel-top">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
        <div ref={rielLineaRef} className="arc-riel-linea">
          <span ref={rielBarraRef} className="arc-riel-barra" />
        </div>
        <span ref={rielEtiquetaRef} className="arc-riel-etiqueta">{tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
      </div>

      {/* ── La portada ─────────────────────────────────────────────────── */}
      <div ref={portadaRef} data-tone={TONO} className="arc-portada">
        ...
        <div ref={escenaPortadaRef} className="arc-portada-escena">
          <div data-cl="1" className="arc-trama" aria-hidden="true" />
        </div>
        <div className="arc-portada-contenido">
          <div className="arc-portada-arriba">
            <div ref={cartelRef} className="arc-cartel">
              <h1 className="arc-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="arc-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="arc-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="arc-portada-abajo">
            <div className="arc-portada-datos">
              <span className="arc-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="arc-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="arc-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="arc-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="arc-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="arc-portada-pase">
            ...
            <button type="button" onClick={abrir} className="arc-portada-btn">
      ...
      <div ref={pistaRef} className="arc-pista">{tx("invitacion.portada.desliza").toUpperCase()} ↓</div>

      {fotoAmpliada && (
        <div className="arc-lupa" onClick={() => setFotoAmpliada(null)} onContextMenu={(e) => e.preventDefault()}>
          <button
            type="button"
            className="arc-lupa-cerrar"
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={fotoAmpliada} alt={tx("invitacion.album.fotoAmpliada")} className="arc-lupa-img" draggable={false} onClick={(e) => e.stopPropagation()} />
        </div>
      )}
...
function Puntos({ cantidad }: { cantidad: number }) {
  ...
  return (
    <div className="arc-puntos" aria-hidden="true">
      {Array.from({ length: cantidad }).map((_, i) => (
        <span key={i} data-dot={i} className="arc-punto" style={{ background: i === 0 ? PALETA.acc : "rgba(43,42,51,.18)" }} />
      ))}
    </div>
  );
}
 ...
 */
function CuentaArcade({ targetDate }: { targetDate: Date }) {
  const tx = useTextos();
  const { time, isEventDay, isPast, hasEnded } = useCountdown(targetDate);

  if (isEventDay || hasEnded || isPast) {
    return (
      <div className="arc-tarjeta arc-tarjeta--hoy" data-xin="1" data-delay="80" data-dist="30">
        <span className="arc-tarjeta-kicker">{tx("invitacion.cuentaRegresiva.kicker").toUpperCase()}</span>
        <span className="arc-tarjeta-titulo">
  ...
  return (
    <div className="arc-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`arc-cuenta-caja arc-cuenta-caja--${i + 1}`}>
          <span className={`arc-cuenta-num${i === 3 ? " arc-cuenta-num--acc" : ""}`}>{c.v}</span>
          <span className="arc-cuenta-etq">{c.l.toUpperCase()}</span>
        </div>
      ))}
    </div>
  );
...
function FilaCopiable({ etiqueta, valor }: { etiqueta: string; valor: string }) {
  ...
  return (
    <div className="arc-fila arc-fila--copiable">
      <div className="arc-fila-texto">
        <span className="arc-fila-etq">{etiqueta.toUpperCase()}</span>
        <span className="arc-fila-dato">{valor}</span>
      </div>
      <button type="button" onClick={copiar} className={`arc-btn-copiar${copiado ? " arc-btn-copiar--hecho" : ""}`}>
...
function TarjetaBancaria({
  ...
  return (
    <div
      ...
      data-delay={retraso}
      data-dist="30"
      className={`arc-tarjeta arc-tarjeta--banco${dobleZ ? " arc-doblez" : ""}${inclinada ? " arc-tarjeta--der" : " arc-tarjeta--izq"}`}
    >
      <span className="arc-tarjeta-kicker">{titulo.toUpperCase()}</span>
      {mensaje && <p className="arc-tarjeta-mensaje">{mensaje}</p>}
      <div className="arc-filas">
        <FilaCopiable etiqueta="Alias" valor={alias} />
        <FilaCopiable etiqueta="CBU" valor={cbu} />
        {(banco || titular) && (
          <div className="arc-fila arc-fila--ultima">
            <span>{banco || tx("invitacion.regalos.banco")}</span>
            <span className="arc-fila-valor">{titular}</span>
          </div>
        )}
      </div>
    </div>
...
function CheckinArcade({
  ...
  if (estado === "DECLINED") {
    return (
      <div data-xin="1" data-delay="140" data-dist="30" className="arc-tarjeta arc-tarjeta--izq">
        <p className="arc-tarjeta-mensaje">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
        <button type="button" className="arc-btn-fantasma" onClick={() => setEstado("PENDING")}>
  ...
  return (
    <>
      <div ref={tarjetaRef} data-xin="1" data-delay="140" data-dist="30" className="arc-tarjeta arc-tarjeta--talon">
        <div className="arc-talon-top">
          <span>{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}{guestName ? ` · ${guestName.toUpperCase()}` : ""}</span>
          <span ref={estadoRef} className="arc-talon-estado">
        ...
        {!confirmadoYa ? (
          <>
            {lugares > 1 && (
              <div className="arc-campo">
                <label className="arc-etiqueta">{tx("invitacion.rsvp.adultos").toUpperCase()}</label>
                <div className="arc-contador">
            ...
            {maxAdolescentes > 0 && (
              <div className="arc-campo">
                <label className="arc-etiqueta">{tx("invitacion.rsvp.adolescentes").toUpperCase()}</label>
                <div className="arc-contador">
            ...
            {maxNinos > 0 && (
              <div className="arc-campo">
                <label className="arc-etiqueta">{tx("invitacion.rsvp.ninos").toUpperCase()}</label>
                <div className="arc-contador">
            ...
            <div className="arc-campo">
              <label className="arc-etiqueta">{tx("invitacion.rsvp.restriccionAlimentaria").toUpperCase()}</label>
              <input
                value={dieta}
          ...
          <div className="arc-filas">
            {lugares > 1 && adultos > 0 && <div className="arc-fila"><span>{tx("invitacion.rsvp.adultos")}</span><span>{adultos}</span></div>}
            {adolescentes > 0 && <div className="arc-fila"><span>{tx("invitacion.rsvp.adolescentes")}</span><span>{adolescentes}</span></div>}
            {ninos > 0 && <div className="arc-fila"><span>{tx("invitacion.rsvp.ninos")}</span><span>{ninos}</span></div>}
            <div className="arc-fila arc-fila--ultima">
              <span>{tx("invitacion.rsvp.restriccionAlimentaria")}</span>
              <span className="arc-fila-valor">{restricciones || dieta || "—"}</span>
            </div>
          </div>
        )}

        {hayPago && monto != null && !exento && (
          <div className="arc-precio">
            <span>{estadoDePago === "PAID" ? tx("invitacion.pago.abonado") : tx("invitacion.pago.valor")}</span>
            <div className="arc-precio-valor">
              <span className="arc-precio-total">{formatearMoneda(totalAPagar)}</span>
              {usarTotalDelServidor && vistaDePago
                ? vistaDePago.lines.map((l, i) => <span key={i} className="arc-precio-detalle">{l}</span>)
                : (
                  <>
                    {adultos > 0 && <span className="arc-precio-detalle">{adultos} × {formatearMoneda(precioAdulto)}</span>}
                    {adolescentes > 0 && <span className="arc-precio-detalle">{adolescentes} × {formatearMoneda(precioAdo)}</span>}
                    {ninos > 0 && <span className="arc-precio-detalle">{ninos} × {formatearMoneda(precioNin)}</span>}
                  </>
                )}
            </div>
          </div>
        ...
        <div ref={selloRef} className="arc-sello" aria-hidden="true">{tx("invitacion.pase.confirmado").toUpperCase()}</div>
        <div ref={petalosRef} className="arc-petalos" aria-hidden="true" />
      </div>
      ...
      {!confirmadoYa ? (
        <>
          <button type="button" data-xin="1" data-delay="220" className="arc-btn-solido" disabled={enviando} onClick={() => enviar("CONFIRMA")}>
            {enviando ? tx("invitacion.rsvp.guardando").toUpperCase() : tx("invitacion.rsvp.confirmarAsistencia").toUpperCase()}
          </button>
          <button type="button" data-xin="1" data-delay="280" className="arc-btn-fantasma" disabled={enviando} onClick={() => enviar("NO_ASISTE")}>
...
function CancionesArcade({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
  ...
  return (
    <>
      <form onSubmit={enviar} data-xin="1" data-delay="140" data-dist="30" className="arc-tarjeta arc-tarjeta--der">
        <div className="arc-campo">
          <label className="arc-etiqueta">{tx("invitacion.musica.tema").toUpperCase()}</label>
          <input value={tema} onChange={(e) => setTema(e.target.value)} placeholder={tx("invitacion.musica.nombreDeLaCancion")} maxLength={100} className="arc-input arc-input--serif" />
        </div>
        <div className="arc-campo">
          <label className="arc-etiqueta">{tx("invitacion.musica.artista").toUpperCase()}</label>
          <input value={artista} onChange={(e) => setArtista(e.target.value)} placeholder={tx("invitacion.musica.artista")} maxLength={80} className="arc-input arc-input--serif" />
        </div>
        {error && <p className="arc-error">{error}</p>}
        <button type="submit" disabled={enviando} className="arc-btn-solido arc-btn-solido--tinta">
          {enviando ? tx("invitacion.musica.enviando").toUpperCase() : tx("invitacion.musica.sugeriUnaCancion").toUpperCase()}
        </button>
      </form>

      {canciones.length > 0 && (
        <div data-xin="1" data-delay="220" className="arc-lista">
          {canciones.slice(0, 8).map((c) => (
            <div key={c.id} className="arc-lista-fila">
              <div className="arc-lista-texto">
                <span className="arc-lista-tema">{c.title}</span>
                <span className="arc-lista-quien">{c.artist} · {tx("invitacion.musica.sumadoPor")} {c.guestName || tx("invitacion.evento.invitado")}</span>
              </div>
            </div>
          ))}
        </div>
 ...
 */
function TriviaArcade({ preguntas, invitationId, guestToken, guestName }: { preguntas: QuizPregunta[]; invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
  ...
  if (terminado) {
    return (
      <div data-xin="1" data-delay="140" data-dist="30" className="arc-tarjeta arc-tarjeta--izq">
        <span className="arc-tarjeta-kicker">{tx("invitacion.quiz.kicker").toUpperCase()}</span>
        <span className="arc-tarjeta-titulo">
        ...
        {!guardando && stats && stats.count > 0 && (
          <p className="arc-tarjeta-mensaje">{tx("invitacion.quiz.promedio", { n: stats.count, avg: stats.avg })}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div data-xin="1" data-delay="140" data-dist="30" className="arc-tarjeta arc-tarjeta--izq">
      <span className="arc-tarjeta-kicker">{tx("invitacion.quiz.preguntaDeTotal", { n: indice + 1, total: preguntas.length }).toUpperCase()}</span>
      <span className="arc-tarjeta-pregunta">{q.pregunta}</span>
      <div className="arc-opciones">
        {q.opciones.map((op, oi) => {
          let clase = "";
          if (yaEligio && correcta !== undefined) {
            if (oi === correcta) clase = " arc-opcion--bien";
            else if (elegidas[indice] === oi) clase = " arc-opcion--mal";
          }
          return (
            <button key={oi} type="button" disabled={yaEligio} onClick={() => elegir(oi)} className={`arc-opcion${clase}`}>
              {op}
            </button>
          );
        })}
...
// leen la Bienvenida y el Post-evento compartidos (esperan `arc-section` y
// `arc-kicker`).
const CSS_ARC = `
  .arc-raiz { position: fixed; inset: 0; width: 100%; height: calc(var(--vh, 1vh) * 100); overflow: hidden;
    background: var(--pp-bg); color: var(--pp-ink); font-family: var(--arc-sans), 'Rubik', sans-serif; }
  .arc-raiz a { color: var(--pp-acc); text-decoration: none; }
  .arc-raiz button { font: inherit; }
  .arc-defs { position: absolute; width: 0; height: 0; overflow: hidden; }

  .arc-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0;
    transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .arc-scroller::-webkit-scrollbar { width: 0; height: 0; }
  ...
  /* Lo que entra al scrollear se mueve en --arc-y y gira en --arc-giro: son
  ...
  .arc-scroller [data-xin] { transform: translate3d(0, var(--arc-y, 0px), 0) rotate(var(--arc-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .arc-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .arc-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .arc-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .arc-kicker--tinta { color: var(--pp-ink); }
  .arc-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .arc-h2 em, .arc-acento { font-style: italic; color: var(--pp-acc); }
  .arc-h2--claro { color: var(--pp-night-ink); }
  .arc-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .arc-acento-plano { color: var(--pp-acc); }
  .arc-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .arc-parrafo--tinta { color: var(--pp-ink); }
  .arc-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .arc-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .arc-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .arc-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .arc-std-dia { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-weight: 500;
  ...
  .arc-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .arc-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--arc-serif), 'Press Start 2P', cursive; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .arc-std-anio { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .arc-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .arc-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .arc-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .arc-foto-capa { position: absolute; inset: 0; }
  .arc-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--arc-serif), 'Press Start 2P', cursive; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .arc-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .arc-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .arc-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .arc-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .arc-cuenta-caja--1 { --arc-giro: -1.6deg; transform: rotate(var(--arc-giro)); }
  .arc-cuenta-caja--2 { --arc-giro: 1.2deg; transform: rotate(var(--arc-giro)); margin-top: 10px; }
  .arc-cuenta-caja--3 { --arc-giro: 1.8deg; transform: rotate(var(--arc-giro)); }
  .arc-cuenta-caja--4 { --arc-giro: -1.1deg; transform: rotate(var(--arc-giro)); margin-top: 10px; }
  .arc-cuenta-num { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .arc-cuenta-num--acc { color: var(--pp-acc); }
  .arc-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .arc-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .arc-frase { position: relative; z-index: 1; margin: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .arc-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .arc-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .arc-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .arc-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .arc-tira { position: absolute; top: 0; left: 0; height: 100%; display: flex; will-change: transform; }
  .arc-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden;
    display: flex; flex-direction: column; gap: 18px; padding: 80px max(24px, calc((100vw - 560px) / 2)) 96px; }
  .arc-panel--abajo { justify-content: flex-end; }
  .arc-panel--centro { justify-content: center; }
  .arc-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  /* Apilado: los mismos paneles, uno abajo del otro, recorridos bajando. */
  .arc-pan[data-scroll="vertical"] { height: auto; }
  .arc-pan[data-scroll="vertical"] .arc-pan-fijo { position: static; height: auto; overflow: visible; }
  .arc-pan[data-scroll="vertical"] .arc-tira { position: static; display: block; width: 100%; transform: none !important; }
  .arc-pan[data-scroll="vertical"] .arc-panel { height: auto; min-height: calc(var(--vh, 1vh) * 100); }
  .arc-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .arc-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .arc-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .arc-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .arc-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .arc-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .arc-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .arc-tarjeta--izq { --arc-giro: -1deg; transform: rotate(var(--arc-giro)); }
  .arc-tarjeta--der { --arc-giro: 1.1deg; transform: rotate(var(--arc-giro)); }
  .arc-tarjeta--talon { --arc-giro: -.8deg; transform: rotate(var(--arc-giro)); overflow: visible; }
  .arc-tarjeta--hoy { --arc-giro: -1deg; transform: rotate(var(--arc-giro)); align-items: flex-start; }
  .arc-tarjeta--banco { margin-top: 4px; }
  .arc-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .arc-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .arc-tarjeta-titulo { margin: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .arc-tarjeta-pregunta { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 26px; line-height: 1.15; }
  .arc-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .arc-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .arc-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .arc-fila > span:first-child { color: var(--pp-ink2); }
  .arc-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .arc-fila-valor { text-align: right; }
  .arc-fila--copiable { align-items: center; }
  .arc-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .arc-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .arc-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .arc-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .arc-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .arc-checkin { background: var(--pp-bg); }
  .arc-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .arc-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .arc-campo { display: flex; flex-direction: column; gap: 6px; }
  .arc-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .arc-contador { display: flex; align-items: center; gap: 12px; }
  .arc-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .arc-contador button:disabled { opacity: .35; cursor: default; }
  .arc-contador > span { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .arc-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 16px; padding: 6px 2px; }
  .arc-input--serif { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 20px; }
  .arc-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .arc-input::placeholder { color: #9A9388; }
  .arc-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .arc-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .arc-precio-total { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .arc-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .arc-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .arc-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .arc-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .arc-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .arc-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .arc-btn-solido:disabled { opacity: .6; cursor: default; }
  .arc-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .arc-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --arc-giro: -1deg; transform: rotate(var(--arc-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .arc-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .arc-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .arc-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .arc-crono-hora { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: 26px; min-width: 64px; line-height: 1; }
  .arc-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .arc-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .arc-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .arc-polaroids[data-cantidad="1"] .arc-polaroid { grid-column: span 6; }
  .arc-polaroids[data-cantidad="2"] .arc-polaroid { grid-column: span 3; }
  .arc-polaroids[data-cantidad="4"] .arc-polaroid { grid-column: span 3; }
  .arc-polaroids[data-cantidad="5"] .arc-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .arc-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .arc-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .arc-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .arc-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .arc-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .arc-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .arc-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .arc-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .arc-lista-tema { font-size: 15px; }
  .arc-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .arc-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .arc-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .arc-opciones { display: flex; flex-direction: column; gap: 8px; }
  .arc-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .arc-opcion:disabled { cursor: default; }
  .arc-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .arc-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .arc-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .arc-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --arc-giro: -.6deg; transform: rotate(var(--arc-giro));
    border-radius: 12px; overflow: hidden; }
  .arc-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .arc-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .arc-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .arc-ticket-nombre { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .arc-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .arc-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .arc-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .arc-ticket-muesca--arriba { top: -12px; }
  .arc-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .arc-ticket-der .qr-ingreso, .arc-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .arc-info-extra { position: relative; z-index: 1; }
  .arc-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .arc-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .arc-raiz .ia-icon-box { display: none !important; }
  .arc-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .arc-pase-firma { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .arc-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .arc-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }

  /* ── Riel ──────────────────────────────────────────────────────────── */
  .arc-riel { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column;
  ...
  .arc-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .arc-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  .arc-riel-barra { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: var(--pp-acc); transition: height 260ms linear; display: block; }
  .arc-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .arc-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .arc-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .arc-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .arc-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .arc-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .arc-cartel-nombres { margin: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive; font-weight: 500;
  ...
  .arc-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .arc-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .arc-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .arc-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .arc-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .arc-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .arc-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .arc-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--arc-sans), 'Rubik', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .arc-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }

  .arc-pista { position: absolute; left: 0; right: 34px; bottom: calc(18px + env(safe-area-inset-bottom)); z-index: 6; text-align: center;
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .arc-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .arc-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .arc-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .arc-section, .arc-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .arc-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .arc-raiz { font-family: var(--arc-sans), 'Rubik', sans-serif; }
  ...
  .arc-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .arc-kicker, .arc-panel-top, .arc-panel-pie, .arc-meta, .arc-riel-top, .arc-riel-etiqueta,
  .arc-ticket-kicker, .arc-cuenta-etq, .arc-tarjeta-kicker, .arc-fila-etq, .arc-portada-pase {
    font-family: var(--arc-sans), 'Rubik', sans-serif; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .arc-h2, .arc-frase, .arc-tarjeta-titulo, .arc-std-dia, .arc-cartel-nombres, .arc-ticket-nombre, .arc-cifra, .arc-cuenta-num {
    font-family: var(--arc-serif), 'Press Start 2P', cursive; letter-spacing: -.03em; }
  .arc-h2 em, .arc-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .arc-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .arc-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .arc-std-fila { justify-content: space-between; }
  .arc-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .arc-std-anio { font-family: var(--arc-serif), 'Press Start 2P', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .arc-std .arc-kicker, .arc-std .arc-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .arc-foto-marco { border: 3px solid var(--pp-ink); }
  .arc-ventana { clip-path: none !important; box-shadow: none; }
  .arc-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--arc-serif), 'Press Start 2P', cursive; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .arc-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .arc-tarjeta--izq, .arc-tarjeta--der, .arc-tarjeta--talon, .arc-tarjeta--hoy { transform: none; }
  .arc-doblez::before { display: none; }
  .arc-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .arc-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .arc-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .arc-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .arc-cuenta-caja:first-child { border-left: none; }
  .arc-cuenta-caja::before { display: none; }
  .arc-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .arc-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .arc-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .arc-panel[data-tone="dark"] .arc-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .arc-panel-top, .arc-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .arc-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .arc-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .arc-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .arc-btn-solido, .arc-btn-papel, .arc-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .arc-btn-papel { background: transparent; border: 1px solid currentColor; }
  .arc-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .arc-portada { background: var(--pp-bg); }
  .arc-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .arc-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .arc-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .arc-cartel-chinche { display: none; }
  .arc-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .arc-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .arc-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Arcade: píxel. El titular baja de tamaño porque Press Start 2P es
     mucho más ancha que una serif, y lleva sombra dura de dos colores. */
  .arc-cartel-nombres { font-size: min(clamp(22px, 7vw, 44px), 8vh); line-height: 1.3; letter-spacing: 0; }
  .arc-h2, .arc-frase { font-size: clamp(20px, 5.5vw, 34px); line-height: 1.4; letter-spacing: 0; }
  .arc-std-dia, .arc-cinta, .arc-std-anio { font-size: clamp(38px, 13vw, 90px); }
  .arc-cartel-nombres, .arc-h2 { text-shadow: 3px 3px 0 var(--pp-acc), 6px 6px 0 var(--pp-acc2); }
  .arc-tarjeta, .arc-ticket, .arc-opcion { border-radius: 0; }
  .arc-trama { opacity: .12; background-image: linear-gradient(currentColor 1px, transparent 1px); background-size: 100% 4px; }

  @media (prefers-reduced-motion: reduce) {
    .arc-raiz * { animation: none !important; }
    .arc-scroller [data-xin] { opacity: 1 !important; --arc-y: 0px; }
  }
`;
