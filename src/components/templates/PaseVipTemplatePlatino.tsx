...
/**
 * PaseVipTemplatePlatino.tsx
 *
 * Variante de color "Platino" de PaseVipTemplate.tsx
...
export function PaseVipTemplatePlatino({ invitation, guest, isPersonalized = false }: PaseVipTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#DCEAF7";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#8FB4D9";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(143,180,217,.35), 0 18px 50px -30px #8FB4D9";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#3E6E96" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(143,180,217,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(143,180,217,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#3E6E96" : "#8FB4D9";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A0D12", tinta: "#F4F1EA", acento: "#8FB4D9", clase: `${pvpBodoni.variable} ${pvpMono.variable}`, fuente: "var(--pvp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="pvp-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="pvp-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #131B24 0%, #0A0E14 55%, #0A0D12 100%)" }}>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="pvp-section pvp-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #142230 0%, #0A0E14 55%, #0A0D12 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="pvp-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #122236 0%, #0A0E14 52%, #0A0D12 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="pvp-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="pvp-pan-sticky">
            <div data-strip="1" className="pvp-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="pvp-panel pvp-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="pvp-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#3E6E96" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#3E6E96" />
                  </svg>
              ...
              <div data-tone="dark" className="pvp-panel pvp-panel--center" style={{ background: "#0A0E14", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="pvp-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #131B24 0%, #0A0E14 60%, #0A0D12 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="pvp-section" style={{ background: "#0A0E14" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="pvp-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="pvp-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#DCEAF7" : "#8FB4D9" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="pvp-section" style={{ background: "#0A0E14" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="pvp-section" style={{ background: "#0A0E14" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pase VIP" className="pvp-section pvp-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #131B24 0%, #0A0E14 55%, #0A0D12 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#8FB4D9" guest={guest} />
      )}
...
function PvpCoverHalf({
  ...
  return (
    <div className="pvp-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#8FB4D9" tintColor2="#0A0D12" effect="enfoque" scrimColorRgb="10,13,18" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#8FB4D9" tintColor2="#0A0D12" effect="enfoque" scrimColorRgb="10,13,18" />
        </div>
      )}
...
const PVP_CSS = `
  ...
  a { color: #8FB4D9; text-decoration: none; }
  ...
  .pvp-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0A0D12; }
  ...
  @media (min-width: 768px) {
    .pvp-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(143,180,217,.3); }
  ...
  .pvp-date-month { font-family: var(--pvp-bodoni), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #8FB4D9; padding-left: 12%; }

  .pvp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .pvp-divider-line { width: 52px; height: 1px; background: #8FB4D9; display: inline-block; }
  ...
  .pvp-cal-link:hover { color: #8FB4D9; }
  ...
  .pvp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2F8FE8, #5B6FA8, #8FB4D9, #DCEAF7, #2F8FE8); filter: saturate(.75); }
  .pvp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0A0E15; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .pvp-medallion-label { font-family: var(--pvp-bodoni), serif; font-size: 26px; line-height: 1; color: #DCEAF7; }
  .pvp-medallion-label-sm { font-family: var(--pvp-bodoni), serif; font-size: 16px; color: #DCEAF7; }
  .pvp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .pvp-medallion-sub--accent { color: #8FB4D9; }
  .pvp-medallion-arc { position: absolute; inset: -14%; }
  .pvp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #8FB4D9; opacity: 0.4; font-family: var(--pvp-mono), monospace; }

  .pvp-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(143,180,217,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .pvp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #DCEAF7, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .pvp-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .pvp-cd-box { border: 1px solid #1C2C3A; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .pvp-cd-num { font-family: var(--pvp-bodoni), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #EAF3FB; display: inline-block; }
  .pvp-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #8FB4D9; }
  .pvp-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A0D12 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .pvp-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(176,196,224,0.5), rgba(91,111,168,0.32), rgba(143,180,217,0.46), rgba(176,196,224,0.5)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .pvp-accent-italic { font-style: italic; color: #8FB4D9; }
  .pvp-accent-serif { font-style: italic; color: #3E6E96; font-family: var(--pvp-bodoni), serif; }
  .pvp-accent-serif-2 { color: #3E6E96; }
  ...
  .pvp-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #3E6E96; margin-top: auto; }
  ...
  .pvp-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #3E6E96; }
  ...
  .pvp-crono-time { font-family: var(--pvp-mono), monospace; color: #3E6E96; min-width: 42px; }
  ...
  .pvp-stub { position: relative; overflow: hidden; border: 1px solid #1C2C3A; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .pvp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #DCEAF7, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .pvp-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(143,180,217,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .pvp-rsvp-payment-total { color: #DCEAF7; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .pvp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #8FB4D9; background: transparent; color: #8FB4D9; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .pvp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(143,180,217,0.3); color: #F4F1EA; font-family: var(--pvp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .pvp-rsvp-input::placeholder { color: #6E6A5D; }
  .pvp-rsvp-input:focus { outline: none; border-bottom-color: #8FB4D9; }
  .pvp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--pvp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #DCEAF7, #8FB4D9); border: 1px solid #8FB4D9; color: #0A0E14; cursor: pointer; }
  .pvp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .pvp-rsvp-btn--ghost { background: transparent; color: #8FB4D9; }
  .pvp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #8FB4D9; margin: 0; }
  ...
  .pvp-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(143,180,217,0.18); }
  ...
  .pvp-bank-copy { flex-shrink: 0; font-family: var(--pvp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #8FB4D9; background: transparent; color: #8FB4D9; cursor: pointer; }
  .pvp-bank-copy:hover { background: rgba(143,180,217,0.12); }
  ...
  .pvp-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(143,180,217,0.3); padding-bottom: 12px; }
  ...
  .pvp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(143,180,217,0.3); color: #F4F1EA; font-family: var(--pvp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .pvp-song-input::placeholder { color: #6E6A5D; }
  .pvp-song-input:focus { outline: none; border-bottom-color: #8FB4D9; }
  .pvp-song-sep { color: #8A8577; flex-shrink: 0; }
  .pvp-song-submit { flex-shrink: 0; background: none; border: none; color: #8FB4D9; font-family: var(--pvp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .pvp-song-submit:hover { color: #DCEAF7; }
  .pvp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .pvp-song-error { font-size: 10px; color: #8FB4D9; margin-top: 6px; }
  ...
  .pvp-quiz-opt { font-family: var(--pvp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(143,180,217,0.4); background: transparent; color: #8FB4D9; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .pvp-quiz-opt:disabled { cursor: default; }
  .pvp-quiz-opt--picked { background: #8FB4D9; border-color: #8FB4D9; color: #0A0E14; }
  .pvp-quiz-opt--correct { background: #8FB4D9; border-color: #8FB4D9; color: #0A0E14; }
  .pvp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .pvp-quiz-result { padding-top: 18px; border-top: 1px solid rgba(143,180,217,0.2); }
  .pvp-quiz-result-score { margin: 0 0 6px; font-family: var(--pvp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #DCEAF7; }
  .pvp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .pvp-final-card { border: 1px solid #8FB4D9; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .pvp-final-names { font-family: var(--pvp-bodoni), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #EAF3FB; }
  ...
  .pvp-replay { cursor: pointer; color: #8FB4D9; }
  .pvp-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .pvp-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(143,180,217,0.14); }
  .pvp-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .pvp-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(143,180,217,0.16); position: relative; }
  .pvp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#DCEAF7, #8FB4D9); transition: height 260ms linear; display: block; }
  .pvp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8FB4D9; transition: color 500ms ease; }
  ...
  .pvp-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #111C2A 0%, #0A0E14 46%, #0A0D12 100%); }
  .pvp-cover-half--bottom .pvp-cover-inner { top: auto; bottom: 0; }
  .pvp-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(47,143,232,0.55), rgba(91,111,168,0.4), rgba(143,180,217,0.7), rgba(47,143,232,0.55)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .pvp-cover-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(220,234,247,0.14) 0 1px, transparent 1px 7px), repeating-linear-gradient(90deg, rgba(220,234,247,0.1) 0 1px, transparent 1px 7px); opacity: .3; -webkit-mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); }
  ...
  .pvp-cover-names { margin: 0; font-family: var(--pvp-bodoni), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EAF3FB; }
  .pvp-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#8FB4D9, transparent); display: block; }
  ...
  .pvp-cover-cta { border: 1px solid #8FB4D9; background: linear-gradient(100deg, rgba(143,180,217,0.08), rgba(220,234,247,0.2), rgba(143,180,217,0.08)); color: #EAF3FB; font-family: var(--pvp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .pvp-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .pvp-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(220,234,247,0.34), rgba(234,243,251,0.5), rgba(220,234,247,0.34)); color: #0A0D12; }
  }
  ...
  .pvp-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(10,13,18,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .pvp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #8FB4D9; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
