...
/**
 * PaseVipTemplateVioleta.tsx
 *
 * Variante de color "Violeta" de PaseVipTemplate.tsx
...
export function PaseVipTemplateVioleta({ invitation, guest, isPersonalized = false }: PaseVipTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E6BFC4";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#9B7FC4";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(155,127,196,.35), 0 18px 50px -30px #9B7FC4";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#5E3E8C" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(155,127,196,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(155,127,196,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#5E3E8C" : "#9B7FC4";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0C0810", tinta: "#F4F1EA", acento: "#9B7FC4", clase: `${pvpBodoni.variable} ${pvpMono.variable}`, fuente: "var(--pvp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="pvp-scroller">
            ...
            <div className="pvp-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="12,8,16" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="12,8,16" />
                </div>
              )}
            </div>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="pvp-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #241432 0%, #0E0A14 52%, #0C0810 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="pvp-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="pvp-pan-sticky">
            <div data-strip="1" className="pvp-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="pvp-panel pvp-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="pvp-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#5E3E8C" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#5E3E8C" />
                  </svg>
              ...
              <div data-tone="dark" className="pvp-panel pvp-panel--center" style={{ background: "#0E0A14", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="pvp-section" style={{ background: "#0E0A14" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="pvp-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="pvp-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E6BFC4" : "#9B7FC4" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="pvp-section" style={{ background: "#0E0A14" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="pvp-section" style={{ background: "#0E0A14" }}>
...
function PvpCoverHalf({
  ...
  return (
    <div className="pvp-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#9B7FC4" tintColor2="#0C0810" effect="enfoque" scrimColorRgb="12,8,16" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#9B7FC4" tintColor2="#0C0810" effect="enfoque" scrimColorRgb="12,8,16" />
        </div>
      )}
...
const PVP_CSS = `
  ...
  a { color: #9B7FC4; text-decoration: none; }
  a:hover { color: #E6BFC4; }
  ...
  .pvp-date-month { font-family: var(--pvp-bodoni), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #9B7FC4; padding-left: 12%; }

  .pvp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .pvp-divider-line { width: 52px; height: 1px; background: #9B7FC4; display: inline-block; }
  ...
  .pvp-cal-link:hover { color: #9B7FC4; }
  ...
  .pvp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #C96B8A, #6A5A9E, #9B7FC4, #E6BFC4, #C96B8A); filter: saturate(.75); }
  .pvp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0F0A16; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .pvp-medallion-label { font-family: var(--pvp-bodoni), serif; font-size: 26px; line-height: 1; color: #E6BFC4; }
  .pvp-medallion-label-sm { font-family: var(--pvp-bodoni), serif; font-size: 16px; color: #E6BFC4; }
  .pvp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .pvp-medallion-sub--accent { color: #9B7FC4; }
  .pvp-medallion-arc { position: absolute; inset: -14%; }
  .pvp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #9B7FC4; opacity: 0.4; font-family: var(--pvp-mono), monospace; }

  .pvp-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(155,127,196,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .pvp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E6BFC4, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .pvp-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .pvp-cd-box { border: 1px solid #33203F; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .pvp-cd-num { font-family: var(--pvp-bodoni), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F3E6EC; display: inline-block; }
  .pvp-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #9B7FC4; }
  .pvp-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0C0810 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .pvp-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(200,150,180,0.5), rgba(106,90,158,0.32), rgba(155,127,196,0.46), rgba(200,150,180,0.5)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .pvp-accent-italic { font-style: italic; color: #9B7FC4; }
  .pvp-accent-serif { font-style: italic; color: #5E3E8C; font-family: var(--pvp-bodoni), serif; }
  .pvp-accent-serif-2 { color: #5E3E8C; }
  ...
  .pvp-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #5E3E8C; margin-top: auto; }
  ...
  .pvp-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #5E3E8C; }
  ...
  .pvp-crono-time { font-family: var(--pvp-mono), monospace; color: #5E3E8C; min-width: 42px; }
  ...
  .pvp-stub { position: relative; overflow: hidden; border: 1px solid #33203F; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .pvp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E6BFC4, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .pvp-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(155,127,196,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .pvp-rsvp-payment-total { color: #E6BFC4; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .pvp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #9B7FC4; background: transparent; color: #9B7FC4; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .pvp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(155,127,196,0.3); color: #F4F1EA; font-family: var(--pvp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .pvp-rsvp-input::placeholder { color: #6E6A5D; }
  .pvp-rsvp-input:focus { outline: none; border-bottom-color: #9B7FC4; }
  .pvp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--pvp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #E6BFC4, #9B7FC4); border: 1px solid #9B7FC4; color: #0E0A14; cursor: pointer; }
  .pvp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .pvp-rsvp-btn--ghost { background: transparent; color: #9B7FC4; }
  .pvp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #9B7FC4; margin: 0; }
  ...
  .pvp-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(155,127,196,0.18); }
  ...
  .pvp-bank-copy { flex-shrink: 0; font-family: var(--pvp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #9B7FC4; background: transparent; color: #9B7FC4; cursor: pointer; }
  .pvp-bank-copy:hover { background: rgba(155,127,196,0.12); }
  ...
  .pvp-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(155,127,196,0.3); padding-bottom: 12px; }
  ...
  .pvp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(155,127,196,0.3); color: #F4F1EA; font-family: var(--pvp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .pvp-song-input::placeholder { color: #6E6A5D; }
  .pvp-song-input:focus { outline: none; border-bottom-color: #9B7FC4; }
  .pvp-song-sep { color: #8A8577; flex-shrink: 0; }
  .pvp-song-submit { flex-shrink: 0; background: none; border: none; color: #9B7FC4; font-family: var(--pvp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .pvp-song-submit:hover { color: #E6BFC4; }
  .pvp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .pvp-song-error { font-size: 10px; color: #9B7FC4; margin-top: 6px; }
  ...
  .pvp-quiz-opt { font-family: var(--pvp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(155,127,196,0.4); background: transparent; color: #9B7FC4; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .pvp-quiz-opt:disabled { cursor: default; }
  .pvp-quiz-opt--picked { background: #9B7FC4; border-color: #9B7FC4; color: #0E0A14; }
  .pvp-quiz-opt--correct { background: #9B7FC4; border-color: #9B7FC4; color: #0E0A14; }
  .pvp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .pvp-quiz-result { padding-top: 18px; border-top: 1px solid rgba(155,127,196,0.2); }
  .pvp-quiz-result-score { margin: 0 0 6px; font-family: var(--pvp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E6BFC4; }
  .pvp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .pvp-final-card { border: 1px solid #9B7FC4; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .pvp-final-names { font-family: var(--pvp-bodoni), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F3E6EC; }
  ...
  .pvp-replay { cursor: pointer; color: #9B7FC4; }
  .pvp-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .pvp-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(155,127,196,0.14); }
  .pvp-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .pvp-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(155,127,196,0.16); position: relative; }
  .pvp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E6BFC4, #9B7FC4); transition: height 260ms linear; display: block; }
  .pvp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #9B7FC4; transition: color 500ms ease; }
  ...
  .pvp-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #201530 0%, #0E0A14 46%, #0C0810 100%); }
  .pvp-cover-half--bottom .pvp-cover-inner { top: auto; bottom: 0; }
  .pvp-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(201,107,138,0.55), rgba(106,90,158,0.4), rgba(155,127,196,0.7), rgba(201,107,138,0.55)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .pvp-cover-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(230,191,196,0.14) 0 1px, transparent 1px 7px), repeating-linear-gradient(90deg, rgba(230,191,196,0.1) 0 1px, transparent 1px 7px); opacity: .3; -webkit-mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); }
  ...
  .pvp-cover-names { margin: 0; font-family: var(--pvp-bodoni), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F3E6EC; }
  .pvp-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#9B7FC4, transparent); display: block; }
  ...
  .pvp-cover-cta { border: 1px solid #9B7FC4; background: linear-gradient(100deg, rgba(155,127,196,0.08), rgba(230,191,196,0.2), rgba(155,127,196,0.08)); color: #F3E6EC; font-family: var(--pvp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .pvp-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .pvp-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(230,191,196,0.34), rgba(243,230,236,0.5), rgba(230,191,196,0.34)); color: #0C0810; }
  }
  ...
  .pvp-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(12,8,16,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .pvp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #9B7FC4; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
