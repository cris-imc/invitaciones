...
export function GuestPassVipTemplate({ invitation, guest, isPersonalized = false }: GuestPassVipTemplateProps) {
  ...
  // horizontal mientras se baja) o apilados, que se recorren bajando como el
  // resto de la invitación. Lo elige el anfitrión en el wizard.
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#08080B", tinta: "#F4F1EA", acento: "#C8A45C", clase: `${gpBodoni.variable} ${gpMono.variable}`, fuente: "var(--gp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="gpv-scroller">
        ...
        {hasFrase && (
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="gpv-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #08080B 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="gpv-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="gpv-pan-sticky">
            <div data-strip="1" className="gpv-strip">
              ...
              <div data-tone="dark" className="gpv-panel gpv-panel--center" style={{ background: "#0B0B10", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="gpv-section" style={{ background: "#0B0B10" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="gpv-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="gpv-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E8D6A8" : "#C8A45C" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="gpv-section" style={{ background: "#0B0B10" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="gpv-section gpv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #0B0B10 55%, #08080B 100%)" }}>
...
const GP_CSS = `
  ...
  .gpv-date-month { font-family: var(--gp-bodoni), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C8A45C; padding-left: 12%; }

  .gpv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .gpv-divider-line { width: 52px; height: 1px; background: #C8A45C; display: inline-block; }
  ...
  .gpv-cal-link:hover { color: #C8A45C; }
  ...
  .gpv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #C8A45C, #E8D6A8, #6E5AA8); filter: saturate(.75); }
  .gpv-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .gpv-medallion-label { font-family: var(--gp-bodoni), serif; font-size: 26px; line-height: 1; color: #E8D6A8; }
  .gpv-medallion-label-sm { font-family: var(--gp-bodoni), serif; font-size: 16px; color: #E8D6A8; }
  .gpv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .gpv-medallion-sub--accent { color: #C8A45C; }
  ...
  .gpv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C8A45C; opacity: 0.4; font-family: var(--gp-mono), monospace; }

  .gpv-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .gpv-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E8D6A8, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .gpv-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .gpv-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .gpv-cd-num { font-family: var(--gp-bodoni), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6EFDD; display: inline-block; }
  .gpv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C8A45C; }
  .gpv-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #08080B 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .gpv-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }

  .gpv-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(200,164,92,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .gpv-accent-italic { font-style: italic; color: #C8A45C; }
  .gpv-accent-serif { font-style: italic; color: #8C6F35; font-family: var(--gp-bodoni), serif; }
  .gpv-accent-serif-2 { color: #8C6F35; }
  ...
  /* Recorrido apilado: la seccion deja de fijarse y el riel deja de ser un
     ...
     del otro y se recorren bajando. El transform que aplica el JS se anula
     por si quedo alguno de una vuelta anterior. */
  ...
  .gpv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #8C6F35; margin-top: auto; }
  ...
  .gpv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #8C6F35; }
  ...
  .gpv-crono-time { font-family: var(--gp-mono), monospace; color: #8C6F35; min-width: 42px; }
  ...
  .gpv-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .gpv-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E8D6A8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .gpv-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .gpv-rsvp-payment-total { color: #E8D6A8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .gpv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C8A45C; background: transparent; color: #C8A45C; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .gpv-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .gpv-rsvp-input::placeholder { color: #6E6A5D; }
  .gpv-rsvp-input:focus { outline: none; border-bottom-color: #C8A45C; }
  .gpv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--gp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C8A45C, #A6813F); border: 1px solid #C8A45C; color: #0B0B10; cursor: pointer; }
  .gpv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .gpv-rsvp-btn--ghost { background: transparent; color: #C8A45C; }
  .gpv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C8A45C; margin: 0; }
  ...
  .gpv-bank-copy { flex-shrink: 0; font-family: var(--gp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C8A45C; background: transparent; color: #C8A45C; cursor: pointer; }
  .gpv-bank-copy:hover { background: rgba(200,164,92,0.12); }
  ...
  .gpv-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  ...
  .gpv-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--gp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .gpv-song-input::placeholder { color: #6E6A5D; }
  .gpv-song-input:focus { outline: none; border-bottom-color: #C8A45C; }
  .gpv-song-sep { color: #8A8577; flex-shrink: 0; }
  .gpv-song-submit { flex-shrink: 0; background: none; border: none; color: #C8A45C; font-family: var(--gp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  ...
  .gpv-quiz-opt { font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #C8A45C; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .gpv-quiz-opt:disabled { cursor: default; }
  .gpv-quiz-opt--picked { background: #C8A45C; border-color: #C8A45C; color: #0B0B10; }
  .gpv-quiz-opt--correct { background: #C8A45C; border-color: #C8A45C; color: #0B0B10; }
  .gpv-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .gpv-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .gpv-quiz-result-score { margin: 0 0 6px; font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E8D6A8; }
  .gpv-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .gpv-final-card { border: 1px solid #C8A45C; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .gpv-final-names { font-family: var(--gp-bodoni), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E8D6A8; }
  ...
  .gpv-replay { cursor: pointer; color: #C8A45C; }
  .gpv-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .gpv-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .gpv-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .gpv-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .gpv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E8D6A8, #C8A45C); transition: height 260ms linear; display: block; }
  .gpv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C8A45C; transition: color 500ms ease; }
  ...
  .gpv-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1A1726 0%, #0B0B10 46%, #08080B 100%); }
  .gpv-cover-half--bottom .gpv-cover-inner { top: auto; bottom: 0; }
  .gpv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.7), rgba(78,127,134,0.45), rgba(200,164,92,0.65), rgba(110,90,168,0.7)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .gpv-cover-names { margin: 0; font-family: var(--gp-bodoni), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E8D6A8; }
  .gpv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C8A45C, transparent); display: block; }
  ...
  .gpv-cover-cta { border: 1px solid #C8A45C; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--gp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .gpv-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .gpv-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0B0B0F; }
  }
