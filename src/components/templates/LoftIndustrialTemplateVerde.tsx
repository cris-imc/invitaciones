/**
 * LoftIndustrialTemplateVerde.tsx
 ...
 * Estudio Oscuro"): fondo casi negro (#10140F) + acento dorado/latón
 * (#7FA65C, variable por color), tipografía Space Grotesk (display) + Sora
...
interface LoftIndustrialTemplateVerdeProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#7FA65C]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#7FA65C] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#F0EFEC] break-all">{value}</span>
      </div>
      <button
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#7FA65C]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#7FA65C] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#F0EFEC] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), sans-serif", fontSize: "2rem", color: "#F0EFEC" }}>
          ¡Juego Completado!
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#F0EFEC" }}>
          RESPONDISTE {score} DE {preguntas.length} CORRECTAMENTE ({percent}%)
        </p>

        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#9C9992" }}>Guardando tus resultados...</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.04)", padding: "8px 16px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#7FA65C] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#F0EFEC" }}>
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#F0EFEC] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), sans-serif', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function LoftIndustrialTemplateVerde({ invitation, guest, isPersonalized = false }: LoftIndustrialTemplateVerdeProps) {
  ...
  // Portada animada -- effect="geometric" y sin tinte, mismo criterio que la
  // base de LoftIndustrial. scrimColorRgb = rgb del propio bg oscuro de esta
  // variante (cada variante tiene su propio bg, no uno compartido).
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#10140F] via-[#161616] to-[#0A0A0A] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
        ...
        <main className="relative z-10 max-w-5xl mx-auto w-full px-4 md:px-6 py-12 lg:py-20">
          <div className="rounded-none bg-black/40 border border-white/10 shadow-2xl backdrop-blur-3xl text-center max-w-4xl mx-auto relative overflow-hidden flex flex-col">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-[1px] bg-gradient-to-r from-transparent via-[#7FA65C]/50 to-transparent" />

            <div className="p-10 md:p-16 space-y-8">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-sans font-light text-white tracking-wide drop-shadow-md">
                Un aniversario <AnimatedSynonyms words={["inolvidable", "único", "memorable", "especial"]} className="text-[#7FA65C]/90 font-sans" />
              </h1>

              <div className="flex justify-center items-center gap-4 py-2 opacity-60">
                <div className="h-[1px] w-12 bg-white/20" />
                <div className="w-1.5 h-1.5 rounded-full bg-[#7FA65C]/50" />
                <div className="h-[1px] w-12 bg-white/20" />
              </div>
              ...
              <div className="pt-6">
                <div className="inline-flex items-center gap-3 px-5 py-2.5 bg-white/5 border border-white/10 text-slate-300 text-xs tracking-widest uppercase backdrop-blur-md" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <span className="w-1.5 h-1.5 rounded-full bg-[#7FA65C]/80 animate-pulse" />
  ...
  return (
    ...
    >
      <style>{`
        ...
        #countdown.dark {
          background-color: #10140F !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #7FA65C !important;
          color: #000000 !important;
          font-weight: 700 !important;
          border: 1px solid #7FA65C !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark {
          background-color: #1A2016 !important;
      ...
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#10140F', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#F0EFEC] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          {portadaFondoAnimado && (
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="loftindustrial-seal opacity-0" style={{
              width: 44, height: 44, border: '1px solid #7FA65C',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#7FA65C',
            }}>
              {monogram}
            </div>
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-sm font-medium text-[#9C9992] tracking-wide uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8 }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #7FA65C', color: '#7FA65C',
                background: 'rgba(224,184,75,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#7FA65C'; e.currentTarget.style.color = '#10140F'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(224,184,75,0.08)'; e.currentTarget.style.color = '#7FA65C'; }}
            >
              ABRIR INVITACIÓN
            </button>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#7FA65C]/40 shadow-md ${isTicketMaximized ? 'bg-[#10140F]/95 backdrop-blur-md w-[90%] max-w-sm px-5 py-2.5 rounded-full' : 'bg-[#000000]/95 backdrop-blur-md px-5 py-2 rounded-full'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#7FA65C] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#F0EFEC] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), sans-serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#7FA65C]/20 pl-3">
                <span className="text-[#F0EFEC] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#9C9992] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? 'Lugar' : 'Lugares'}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#7FA65C]" />
              <span className="text-[#F0EFEC] text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#7FA65C', margin: '6px 0 14px', animation: 'loftindustrial-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#10140F]">
          <div className="px-8 pt-16 pb-12 text-left bg-[#10140F] z-10 relative">
            ...
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#7FA65C] mb-6" style={{ fontFamily: "'Space Mono', var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#F0EFEC] leading-[1.0] mb-3 uppercase" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), sans-serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), sans-serif', fontStyle: 'normal', color: '#7FA65C' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#7FA65C', margin: '0 0 20px', animation: 'loftindustrial-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#9C9992] tracking-wide" style={{ fontFamily: "'Space Mono', var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#7FA65C] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #10140F 100%)' }} />
            <div
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              ...
              <IconRivets style={{ position: "absolute", top: -6, left: 8, width: 34, height: 10, color: "#7FA65C" }} />
              <IconBolt style={{ position: "absolute", bottom: -10, right: -8, width: 22, height: 22, color: "#7FA65C" }} />
            </div>
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#10140F]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #7FA65C, transparent)' }} />
          <IconValve className="loftindustrial-scroll-doodle opacity-0" style={{ width: 22, height: 22, color: '#7FA65C' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #7FA65C, transparent)' }} />
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #7FA65C14, transparent 70%), #1A2016" }}>
            <div className="max-w-2xl mx-auto text-center">
              <TypewriterText
                text={`"${String(invitation.frasePersonalizadaTexto)}"`}
                className="text-[#F0EFEC] text-2xl md:text-3xl leading-relaxed tracking-wide"
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#10140F] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#7FA65C]" style={{ fontFamily: "'Space Mono', var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-black/40 border-l-[2px] border-l-[#7FA65C] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9C9992] block mb-3" style={{ fontFamily: "'Space Mono', var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#F0EFEC] mb-3" style={{ fontFamily: 'var(--font-cormorant), sans-serif' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#9C9992]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#9C9992]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#7FA65C] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/40 border-l-[2px] border-l-[#7FA65C] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9C9992] block mb-3" style={{ fontFamily: "'Space Mono', var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#7FA65C] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#7FA65C]" style={{ fontFamily: "'Space Mono', var(--font-body-custom, var(--font-inter))" }}>
                  <IconBeam className="loftindustrial-scroll-doodle opacity-0" style={{ width: 18, height: 12 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#7FA65C]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#9C9992] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "'Space Mono', var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#F0EFEC] font-light" style={{ fontFamily: 'var(--font-cormorant), sans-serif' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#1A2016] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-10 flex items-center gap-2">
                <IconRivets className="loftindustrial-scroll-doodle opacity-0" style={{ width: 20, height: 6, color: '#7FA65C' }} />
                ÁLBUM
              </p>
            </div>
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#1A2016] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#7FA65C]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #7FA65C18, transparent 70%), #1A2016" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#10140F' }} aria-hidden="true">
            <IconMusic className="loftindustrial-scroll-doodle opacity-0" style={{ width: 22, height: 22, color: '#7FA65C' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#10140F' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #7FA65C, transparent)' }} />
          <IconBolt className="loftindustrial-scroll-doodle opacity-0" style={{ width: 16, height: 16, color: '#7FA65C' }} />
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #7FA65C, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#10140F" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#7FA65C" surfaceColor="#1A2016" />}
      </div>
  );
}
