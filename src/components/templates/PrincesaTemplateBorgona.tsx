...
export function PrincesaTemplateBorgona({ invitation, guest, isPersonalized = false }: PrincesaTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#EAF6FB";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#8FC4D9";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(143,196,217,.35), 0 18px 50px -30px #8FC4D9";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#3E7A8C" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(143,196,217,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(143,196,217,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#3E7A8C" : "#8FC4D9";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#23090F", tinta: "#F4F1EA", acento: "#8FC4D9", clase: `${prcCormorant.variable} ${prcMono.variable}`, fuente: "var(--prc-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="prc-scroller">
            ...
            <div className="prc-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="35,9,15" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="35,9,15" />
                </div>
              )}
            </div>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="prc-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #200A10 0%, #0F0508 52%, #23090F 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="prc-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="prc-pan-sticky">
            <div data-strip="1" className="prc-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="prc-panel prc-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="prc-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#3E7A8C" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#3E7A8C" />
                  </svg>
              ...
              <div data-tone="dark" className="prc-panel prc-panel--center" style={{ background: "#3A0F1C", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="prc-section" style={{ background: "#3A0F1C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="prc-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="prc-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#EAF6FB" : "#8FC4D9" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="prc-section" style={{ background: "#3A0F1C" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#8FC4D9" guest={guest} />
      )}
    ...
    <>
      ...
      <div className="prc-medallion-core">
        <svg viewBox="0 0 60 34" className={compact ? "prc-tiara-svg-sm" : "prc-tiara-svg"}>
          <path d="M4 30 L4 16 L16 24 L30 6 L44 24 L56 16 L56 30 Z" fill="none" stroke="#EAF6FB" strokeWidth={2.2} />
        </svg>
...
const PRC_CSS = `
  ...
  a { color: #8FC4D9; text-decoration: none; }
  a:hover { color: #EAF6FB; }
  ...
  .prc-date-month { font-family: var(--prc-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #8FC4D9; padding-left: 12%; }

  .prc-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .prc-divider-line { width: 52px; height: 1px; background: #8FC4D9; display: inline-block; }
  ...
  .prc-cal-link:hover { color: #8FC4D9; }
  ...
  .prc-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #3E7A8C, #1E3A45, #8FC4D9, #EAF6FB, #3E7A8C); filter: saturate(.75); }
  ...
  .prc-medallion-sub--accent { color: #8FC4D9; }
  .prc-medallion-arc { position: absolute; inset: -14%; }
  .prc-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #8FC4D9; opacity: 0.4; font-family: var(--prc-mono), monospace; }

  .prc-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(143,196,217,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .prc-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #EAF6FB, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .prc-cd-num { font-family: var(--prc-cormorant), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #E3F1F6; display: inline-block; }
  .prc-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #8FC4D9; }
  .prc-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #23090F 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .prc-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(143,196,217,0.3), rgba(163,214,230,0.24), rgba(143,196,217,0.34), rgba(143,196,217,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .prc-accent-italic { font-style: italic; color: #8FC4D9; }
  .prc-accent-serif { font-style: italic; color: #3E7A8C; font-family: var(--prc-cormorant), serif; }
  .prc-accent-serif-2 { color: #3E7A8C; }
  ...
  .prc-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #3E7A8C; margin-top: auto; }
  ...
  .prc-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #3E7A8C; }
  ...
  .prc-crono-time { font-family: var(--prc-mono), monospace; color: #3E7A8C; min-width: 42px; }
  ...
  .prc-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #EAF6FB, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .prc-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(143,196,217,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .prc-rsvp-payment-total { color: #EAF6FB; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .prc-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #8FC4D9; background: transparent; color: #8FC4D9; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .prc-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(143,196,217,0.3); color: #F4F1EA; font-family: var(--prc-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .prc-rsvp-input::placeholder { color: #6E6A5D; }
  .prc-rsvp-input:focus { outline: none; border-bottom-color: #8FC4D9; }
  .prc-rsvp-btn { width: 100%; padding: 16px; font-family: var(--prc-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #8FC4D9, #3E7A8C); border: 1px solid #8FC4D9; color: #0B0B10; cursor: pointer; }
  .prc-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .prc-rsvp-btn--ghost { background: transparent; color: #8FC4D9; }
  .prc-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #8FC4D9; margin: 0; }
  ...
  .prc-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(143,196,217,0.18); }
  ...
  .prc-bank-copy { flex-shrink: 0; font-family: var(--prc-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #8FC4D9; background: transparent; color: #8FC4D9; cursor: pointer; }
  .prc-bank-copy:hover { background: rgba(143,196,217,0.12); }
  ...
  .prc-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(143,196,217,0.3); padding-bottom: 12px; }
  ...
  .prc-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(143,196,217,0.3); color: #F4F1EA; font-family: var(--prc-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .prc-song-input::placeholder { color: #6E6A5D; }
  .prc-song-input:focus { outline: none; border-bottom-color: #8FC4D9; }
  .prc-song-sep { color: #8A8577; flex-shrink: 0; }
  .prc-song-submit { flex-shrink: 0; background: none; border: none; color: #8FC4D9; font-family: var(--prc-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .prc-song-submit:hover { color: #EAF6FB; }
  .prc-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .prc-song-error { font-size: 10px; color: #8FC4D9; margin-top: 6px; }
  ...
  .prc-quiz-opt { font-family: var(--prc-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(143,196,217,0.4); background: transparent; color: #8FC4D9; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .prc-quiz-opt:disabled { cursor: default; }
  .prc-quiz-opt--picked { background: #8FC4D9; border-color: #8FC4D9; color: #0B0B10; }
  .prc-quiz-opt--correct { background: #8FC4D9; border-color: #8FC4D9; color: #0B0B10; }
  .prc-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .prc-quiz-result { padding-top: 18px; border-top: 1px solid rgba(143,196,217,0.2); }
  .prc-quiz-result-score { margin: 0 0 6px; font-family: var(--prc-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #EAF6FB; }
  .prc-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .prc-final-card { border: 1px solid #8FC4D9; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .prc-final-names { font-family: var(--prc-cormorant), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #EAF6FB; }
  ...
  .prc-replay { cursor: pointer; color: #8FC4D9; }
  .prc-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .prc-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(143,196,217,0.14); }
  .prc-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .prc-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(143,196,217,0.16); position: relative; }
  .prc-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#EAF6FB, #8FC4D9); transition: height 260ms linear; display: block; }
  .prc-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8FC4D9; transition: color 500ms ease; }
  ...
  .prc-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #401520 0%, #3A0F1C 46%, #23090F 100%); }
  .prc-cover-half--bottom .prc-cover-inner { top: auto; bottom: 0; }
  .prc-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(143,196,217,0.5), rgba(163,214,230,0.4), rgba(143,196,217,0.55), rgba(143,196,217,0.5)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .prc-cover-names { margin: 0; font-family: var(--prc-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EAF6FB; }
  .prc-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#8FC4D9, transparent); display: block; }
  ...
  .prc-cover-cta { border: 1px solid #8FC4D9; background: linear-gradient(100deg, rgba(143,196,217,0.08), rgba(234,246,251,0.2), rgba(143,196,217,0.08)); color: #E3F1F6; font-family: var(--prc-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .prc-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .prc-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(163,214,230,0.34), rgba(227,241,246,0.5), rgba(163,214,230,0.34)); color: #0B0B0F; }
  }
  ...
  .prc-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #8FC4D9; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
