...
/**
 * AtelierDePapelTemplateVerdeSalvia.tsx
 *
 * Portado 1:1 desde el diseño "Atelier de Papel" aprobado en Claude Design.
 * Variante de color "Verde Salvia" (colección Storytelling para boda: mismo
 * fondo casi negro, ahora con velo verde salvia, acento verde salvia
 * #5F7A52 en vez del bronce original, Cormorant Garamond + IBM Plex Mono, ticket
...
export function AtelierDePapelTemplateVerdeSalvia({ invitation, guest, isPersonalized = false }: AtelierDePapelTemplateProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#5F7A52";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(95,122,82,.35), 0 18px 50px -30px #5F7A52";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(95,122,82,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(95,122,82,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#2B2620" : "#5F7A52";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#11150F", tinta: "#F4F1EA", acento: "#5F7A52", clase: `${adpSerif.variable} ${adpMono.variable}`, fuente: "var(--adp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="adp-scroller">
            ...
            <div className="adp-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="17,21,15" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="17,21,15" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="adp-section adp-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #20251D 0%, #11150F 55%, #11150F 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#5F7A52" guest={guest} />
      )}
...
function CoverHalf({
  ...
  return (
    <div className="adp-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="232,244,225" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="232,244,225" />
        </div>
      )}
...
const ADP_CSS = `
  ...
  a { color: #5F7A52; text-decoration: none; }
  a:hover { color: #E8F4E1; }
  ...
  @media (min-width: 768px) {
    .adp-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(95,122,82,.3); }
  ...
  .adp-date-month { font-family: var(--adp-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #5F7A52; padding-left: 12%; }

  .adp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .adp-divider-line { width: 52px; height: 1px; background: #5F7A52; display: inline-block; }
  ...
  .adp-cal-link:hover { color: #5F7A52; }
  ...
  .adp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2B2620, #5F7A52, #5F7A52, #E8F4E1, #2B2620); filter: saturate(.75); }
  .adp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C1012; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .adp-medallion-label { font-family: var(--adp-serif), serif; font-size: 26px; line-height: 1; color: #E8F4E1; }
  .adp-medallion-label-sm { font-family: var(--adp-serif), serif; font-size: 16px; color: #E8F4E1; }
  .adp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .adp-medallion-sub--accent { color: #5F7A52; }
  ...
  .adp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #5F7A52; opacity: 0.4; font-family: var(--adp-mono), monospace; }

  .adp-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(95,122,82,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .adp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E8F4E1, transparent); animation: adpScan 6s linear infinite; pointer-events: none; }

  .adp-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .adp-cd-box { border: 1px solid #1E2B18; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .adp-cd-num { font-family: var(--adp-serif), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #E8F4E1; display: inline-block; }
  ...
  .adp-accent-italic { font-style: italic; color: #5F7A52; }
  .adp-phrase-accent { font-style: italic; color: #A8C799; }
  .adp-accent-serif { font-style: italic; color: #2B2620; font-family: var(--adp-serif), serif; }
  .adp-accent-serif-loc { font-style: italic; color: #5F7A52; font-family: var(--adp-serif), serif; }
  ...
  .adp-stub { position: relative; overflow: hidden; border: 1px solid #1E2B18; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .adp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E8F4E1, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .adp-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(95,122,82,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .adp-rsvp-payment-total { color: #E8F4E1; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .adp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #5F7A52; background: transparent; color: #5F7A52; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .adp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(95,122,82,0.3); color: #F4F1EA; font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .adp-rsvp-input::placeholder { color: #6E6A5D; }
  .adp-rsvp-input:focus { outline: none; border-bottom-color: #5F7A52; }
  .adp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--adp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #5F7A52, #475D3C); border: 1px solid #5F7A52; color: #11150F; cursor: pointer; }
  .adp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .adp-rsvp-btn--ghost { background: transparent; color: #5F7A52; }
  .adp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #5F7A52; margin: 0; }
  ...
  .adp-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(95,122,82,0.18); }
  ...
  .adp-bank-copy { flex-shrink: 0; font-family: var(--adp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #5F7A52; background: transparent; color: #5F7A52; cursor: pointer; }
  .adp-bank-copy:hover { background: rgba(95,122,82,0.12); }
  ...
  .adp-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(95,122,82,0.3); padding-bottom: 12px; }
  ...
  .adp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(95,122,82,0.3); color: #F4F1EA; font-family: var(--adp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .adp-song-input::placeholder { color: #6E6A5D; }
  .adp-song-input:focus { outline: none; border-bottom-color: #5F7A52; }
  .adp-song-sep { color: #8A8577; flex-shrink: 0; }
  .adp-song-submit { flex-shrink: 0; background: none; border: none; color: #5F7A52; font-family: var(--adp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .adp-song-submit:hover { color: #E8F4E1; }
  .adp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .adp-song-error { font-size: 10px; color: #5F7A52; margin-top: 6px; }
  ...
  .adp-quiz-opt { font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(95,122,82,0.4); background: transparent; color: #5F7A52; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .adp-quiz-opt:disabled { cursor: default; }
  .adp-quiz-opt--picked { background: #5F7A52; border-color: #5F7A52; color: #11150F; }
  .adp-quiz-opt--correct { background: #5F7A52; border-color: #5F7A52; color: #11150F; }
  .adp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .adp-quiz-result { padding-top: 18px; border-top: 1px solid rgba(95,122,82,0.2); }
  .adp-quiz-result-score { margin: 0 0 6px; font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E8F4E1; }
  .adp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .adp-final-card { border: 1px solid #5F7A52; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .adp-final-names { font-family: var(--adp-serif), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E8F4E1; }
  ...
  .adp-replay { cursor: pointer; color: #5F7A52; }
  .adp-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .adp-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(95,122,82,0.14); }
  .adp-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .adp-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(95,122,82,0.16); position: relative; }
  .adp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E8F4E1, #5F7A52); transition: height 260ms linear; display: block; }
  .adp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #5F7A52; transition: color 500ms ease; }
  ...
  .adp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #5F7A52; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
