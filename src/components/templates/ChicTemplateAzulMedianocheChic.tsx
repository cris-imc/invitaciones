 ...
 * (#10141C) + tinta oscura (#EAEEF5) + dorado fijo (#7FA8D9) + acento oliva
...
interface ChicTemplateAzulMedianocheChicProps {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#7FA8D9]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        ...
        <span className="text-xs sm:text-sm font-mono text-[#EAEEF5] break-all">{value}</span>
      </div>
      <button 
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#7FA8D9]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#7FA8D9] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#EAEEF5] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#EAEEF5" }}>
          {tx("invitacion.quiz.juegoCompletado")}
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#EAEEF5" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#8590A0" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                ...
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#EAEEF5" }}>
                  {tx("invitacion.quiz.promedioGlobal", { n: stats.count })} <strong style={{ color: "#EAEEF5" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#EAEEF5] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function ChicTemplateAzulMedianocheChic({ invitation, guest, isPersonalized = false }: ChicTemplateAzulMedianocheChicProps) {
  const tx = useTextos();
  ...
  const coverScrimRgb = "36,30,18"; // rgb(#EAEEF5)
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#10141C] via-[#181E2A] to-[#FFFFFF] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center border-t border-white/10 font-sans">
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #EAEEF5)" />
        </footer>
      </div>
    );
  }
    ...
    >
      <style>{`
        ...
        .desktop-stage .d-foot {
          background-color: #10141C !important; /* Matches light sections */
          color: #EAEEF5 !important;
      ...
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#10141C', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#EAEEF5] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            {/* Guest Name */}
            <h2 className={`text-4xl sm:text-5xl font-light tracking-wide leading-relaxed${portadaFondoAnimado ? " chic-cover-text" : ""}`} style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif', color: portadaFondoAnimado ? undefined : '#EAEEF5' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className={`text-sm font-medium tracking-wide uppercase${portadaFondoAnimado ? " chic-cover-text-muted" : ""}`} style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8, color: portadaFondoAnimado ? undefined : '#8590A0' }}>
            ...
            <button
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = '#7FA8D9'; e.currentTarget.style.color = '#10141C'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(201,168,118,0.08)'; e.currentTarget.style.color = '#7FA8D9'; }}
            >
          ...
          <style jsx>{`
            ...
            .chic-cover-text { color: #EAEEF5; }
            .chic-cover-text-muted { color: rgba(251,243,234,0.75); }
            @media (min-width: 768px) {
              .chic-cover-text { color: #EAEEF5; }
              .chic-cover-text-muted { color: #8590A0; }
            }
          `}</style>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#7FA8D9]/40 shadow-md ${isTicketMaximized ? 'bg-[#10141C]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#EAEEF5]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                ...
                <span className="text-[#EAEEF5] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              ...
              <div className="flex flex-col items-end border-l border-[#7FA8D9]/20 pl-3">
                <span className="text-[#EAEEF5] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#8590A0] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#7FA8D9]" />
              <span className="text-[#10141C]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#7FA8D9', margin: '6px 0 14px', animation: 'chic-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#10141C]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#10141C] z-10 relative">
            ...
            <h1 className="text-[4rem] font-light text-[#EAEEF5] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
            ...
            <div style={{ width: 40, height: 2, background: '#7FA8D9', margin: '0 0 20px', animation: 'chic-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#8590A0] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #10141C 100%)' }} />
            <div
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              ...
              <IconRings style={{ position: "absolute", top: -10, left: -8, width: 30, height: 20, color: "#7FA8D9" }} />
          ...
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #6B7A4F14, transparent 70%), var(--t-surface)" }}>
            <div className="max-w-2xl mx-auto text-center">
              ...
              <TypewriterText 
                ...
                className="text-[#EAEEF5] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#10141C] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#7FA8D9] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8590A0] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#EAEEF5] mb-3" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#8590A0]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {tx("invitacion.ubicacion.horaConSufijo", { hora: String(invitation.ceremoniaHora) })}</p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#8590A0]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#7FA8D9]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#8590A0] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#EAEEF5] font-light" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#181E2A] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <div className="flex justify-center mb-4">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #6B7A4F18, transparent 70%), #181E2A" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#10141C' }} aria-hidden="true">
        ...
        <LogoFooterCredit bgColor="#10141C" textColor="var(--chic-ink, #EAEEF5)" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#7FA8D9" surfaceColor="#FFFFFF" />}
      </div>
  );
}
