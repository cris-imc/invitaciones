...
export function PrincesaTemplateBosqueEncantado({ invitation, guest, isPersonalized = false }: PrincesaTemplateProps) {
  const tx = useTextos();
  ...
  const coverKickerText = showGuestNameInCover ? tx("invitacion.sabor.cuentoDeHadasPara").toUpperCase() : tx("invitacion.sabor.cuentoDeHadasQuinceDe").toUpperCase();
  ...
  const portadaMensaje = String(
    invitation.portadaMensaje || tx("invitacion.sabor.mensajeVosSosLaProtagonista")
  );
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#FBEAF2";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#D98CB0";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(217,140,176,.35), 0 18px 50px -30px #D98CB0";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#8C4A6E" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(217,140,176,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(217,140,176,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#8C4A6E" : "#D98CB0";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0E1F16", tinta: "#F4F1EA", acento: "#D98CB0", clase: `${prcCormorant.variable} ${prcMono.variable}`, fuente: "var(--prc-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="prc-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="prc-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #0A160F 0%, #16311F 55%, #0E1F16 100%)" }}>
          <span data-xin="1" data-dist="-60" className="prc-kicker">{"01 — " + tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          ...
          <div data-drift="-70" className="prc-medallion prc-medallion--corner">
            <TiaraMedallion sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="prcArc1" arcText={tx("invitacion.sabor.arcoCuentoDeHadas").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        {(photoMobile || photoDesktop) && (
          <section
            ...
            className={`prc-hero-photo-section${!photoMobile ? " prc-hero-photo-section--no-mobile" : ""}${!photoDesktop ? " prc-hero-photo-section--no-desktop" : ""}`}
          >
            <div className="prc-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="14,31,22" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="14,31,22" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="prc-section prc-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #244A2E 0%, #0F2417 55%, #0E1F16 100%)" }}>
          <div className="prc-scan-grid" />
          <div className="prc-scanline" />
          <span data-xin="1" data-dist="-60" className="prc-kicker" style={{ position: "relative" }}>{knPre(2)} — LA MAGIA EMPIEZA EN</span>
          <div className="prc-cd-grid">
            <PrcCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <PrcCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <PrcCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <PrcCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
          <div className="prc-perf-strip" />
        </section>

        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="prc-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #13251A 0%, #070E0A 52%, #0E1F16 100%)" }}>
          <div data-drift="-130" className="prc-glow-blob" />
          <span data-xin="1" data-dist="-60" className="prc-kicker" style={{ position: "relative" }}>{knPre(3)} {"— " + tx("invitacion.frase.unMensajeParaVos").toUpperCase()}</span>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="prc-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
            ...
            <div data-strip="1" className="prc-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="prc-panel prc-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <div className="prc-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="prc-side-hint">{scrollVertical ? "↓" : "→"}</span></div>
                </div>
              )}
              ...
              <div id="details" data-tone="light" className="prc-panel prc-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                <div className="prc-facts">
                  ...
                  {dressCode && (
                    <div className="prc-facts-row prc-facts-row--last">
                      <span>{tx("invitacion.pase.codigo").toUpperCase()}</span><span className="prc-accent-serif-2">{dressCode.toUpperCase()}</span>
                    </div>
                  )}
                </div>
                {cronograma.length > 0 && (
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="prc-panel prc-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="prc-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#8C4A6E" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#8C4A6E" />
                  </svg>
                  <div className="prc-panel-block">
                    <span className="prc-mini-label">{ceremoniaHabilitada ? "03" : "02"} / {LUGAR_PANEL_COUNT}</span>
                    <span className="prc-panel-title-sm">{tx("invitacion.ubicacion.comoLlegar")}</span>
              ...
              <div data-tone="dark" className="prc-panel prc-panel--center" style={{ background: "#16311F", color: "#F4F1EA" }}>
                <div className="prc-medallion prc-medallion--lg">
                  <TiaraMedallion sub={tx("invitacion.pase.numeroTiara", { n: passNumber }).toUpperCase()} arcId="prcArc2" arcText={tx("invitacion.pase.arcoAccesoVipTiara", { n: passNumber }).toUpperCase()} spin="reverse" />
                </div>
                <span className="prc-mini-label">{LUGAR_PANEL_COUNT} / {LUGAR_PANEL_COUNT} {"— " + tx("invitacion.ubicacion.tuUbicacion").toUpperCase()}</span>
                {/* mesa-en-ubicacion */}
                {guest?.mesas && guest.mesas.length > 0 && (
                  <span
                    className="prc-mini-label"
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="prc-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #0A160F 0%, #16311F 60%, #0E1F16 100%)" }}>
          <span data-xin="1" data-dist="-60" className="prc-kicker">{kn(4)} — CHECK-IN</span>
          <h2 data-xin="1" data-delay="80" data-dist="130" className="prc-h2">
            {tx("invitacion.rsvp.confirmaLinea1")}<br /><span className="prc-accent-italic">tu asistencia</span>
          </h2>
          ...
          {rsvpEnabled ? (
            ...
            <p className="prc-lead">{tx("invitacion.rsvp.cerrada")}</p>
          )}
        </section>

        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="prc-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="prc-pan-sticky">
            <div data-strip="1" className="prc-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="prc-panel prc-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  {pageIndex === 0 && <h2 className="prc-panel-title-md">{tx("invitacion.album.titulo")} <span className="prc-accent-serif">{tx("invitacion.album.deFotos")}</span></h2>}
                  <div className="prc-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      ...
                      <span className="prc-photo-placeholder">{tx("invitacion.album.sinFotos")}</span>
                    )}
                  </div>
                  <div className="prc-seguir prc-seguir--split">
                    <span>{tx("invitacion.album.fotosSubidas", { n: allPhotos.length }).toUpperCase()}</span>
                    <span className="prc-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <PrcDots count={photoPages.length} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="prc-section" style={{ background: "#16311F" }}>
            <span data-xin="1" data-dist="-60" className="prc-kicker">{kn(6)} {"— " + tx("invitacion.musica.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="prc-h2">{tituloEnDosLineas(tx("invitacion.sabor.preguntaValsAbreBaile"), "prc-accent-italic")}</h2>
            <div data-xin="1" data-delay="160" data-dist="-80" className="prc-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="prc-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FBEAF2" : "#D98CB0" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="prc-section" style={{ background: "#16311F" }}>
            <span data-xin="1" data-dist="-60" className="prc-kicker">{sugerenciaMusicaHabilitada ? kn(7) : kn(6)} {"— " + tx("invitacion.regalos.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="prc-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><span className="prc-accent-italic">{tx("invitacion.regalos.siQueresLinea2")}</span>
            </h2>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="prc-section" style={{ background: "#16311F" }}>
            <span data-xin="1" data-dist="-60" className="prc-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection].filter(Boolean).length + 6)} {"— " + tx("invitacion.quiz.kicker").toUpperCase()}</span>
        ...
        <section data-tone="dark" data-screen-label="Tu tiara" className="prc-section prc-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #0A160F 0%, #16311F 55%, #0E1F16 100%)" }}>
          <span data-xin="1" data-dist="-60" className="prc-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection, quizEnabled].filter(Boolean).length + 6)} — {tx("invitacion.kicker.guarda")} TU TIARA</span>
          <div data-xin="1" data-delay="100" data-dist="130" className="prc-final-card">
            <div className="prc-medallion prc-medallion--final">
              <TiaraMedallion sub={confirmed ? "CONFIRMADO" : tx("invitacion.pase.pendiente").toUpperCase()} arcId="prcArc3" arcText={textoArco(namesTitle, fechaCorta)} spin="reverse" />
            </div>
            <span className="prc-mini-label prc-accent-serif-2">{tx("invitacion.pase.numeroTiaraAdmite", { n: passNumber, cantidad: guestAdults + guestTeens + guestChildren || 1 }).toUpperCase()}</span>
          ...
          <div className="prc-final-footer">
            ...
            <span className="prc-replay" onClick={reset}>{tx("invitacion.portada.verAperturaOtraVez").toUpperCase() + " ↺"}</span>
          </div>
      ...
      <div ref={railRef} className="prc-rail">
        <span ref={railTopRef} className="prc-rail-top">{tx("invitacion.pase.numeroTiara", { n: passNumber }).toUpperCase()}</span>
      ...
      <div ref={coverRef} className="prc-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="prc-cover-half prc-cover-half--top">
          ...
          >
            <div className="prc-cover-cta">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</div>
          </PrcCoverHalf>
        </div>
        <div ref={bottomRef} className="prc-cover-half prc-cover-half--bottom">
          <PrcCoverHalf
            ...
            photoDesktop={photoDesktop}
          >
            <button onClick={open} className="prc-cover-cta prc-cover-cta--btn">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</button>
          </PrcCoverHalf>
        </div>
      </div>

      <div ref={hintRef} className="prc-hint">{tx("invitacion.portada.desliza").toUpperCase() + " ↓"}</div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#D98CB0" guest={guest} />
      )}
    ...
    <>
      ...
      <div className="prc-medallion-core">
        <svg viewBox="0 0 60 34" className={compact ? "prc-tiara-svg-sm" : "prc-tiara-svg"}>
          <path d="M4 30 L4 16 L16 24 L30 6 L44 24 L56 16 L56 30 Z" fill="none" stroke="#FBEAF2" strokeWidth={2.2} />
        </svg>
  ...
  if (status === "DECLINED") {
    return (
      <div className="prc-rsvp-declined">
        <p className="prc-rsvp-declined-text">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
  ...
  return (
    <>
      <div className="prc-rsvp-rows">
        ...
        {status === "CONFIRMED" && (
          <>
            {totalGuests > 1 && adultCount > 0 && <div className="prc-rsvp-row"><span>{tx("invitacion.rsvp.adultos").toUpperCase()}</span><span>{String(adultCount).padStart(2, "0")}</span></div>}
            {teenCount > 0 && <div className="prc-rsvp-row"><span>{tx("invitacion.rsvp.adolescentes").toUpperCase()}</span><span>{String(teenCount).padStart(2, "0")}</span></div>}
            {childCount > 0 && <div className="prc-rsvp-row"><span>{tx("invitacion.rsvp.ninos").toUpperCase()}</span><span>{String(childCount).padStart(2, "0")}</span></div>}
          </>
        )}
          ...
          <div className="prc-rsvp-row prc-rsvp-row--payment">
            ...
            <div className="prc-rsvp-payment-value">
              ...
              {paymentStatus === "PARTIAL" && (
                <div className="prc-rsvp-payment-detail"><span>{tx("invitacion.pago.pagoParcialRegistrado")}</span></div>
              )}
      ...
      <div ref={stubRef} className="prc-stub">
        <div className="prc-stub-top">
          <span>{tx("invitacion.pase.numeroTiara", { n: passNumber }).toUpperCase()}</span>
    ...
    <div className="prc-song">
      <form onSubmit={handleSubmit} className="prc-song-row">
        <div className="prc-song-inputs">
          <input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder={tx("invitacion.musica.artista").toUpperCase()} maxLength={80} className="prc-song-input" />
          <span className="prc-song-sep">—</span>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tx("invitacion.musica.tema").toUpperCase()} maxLength={100} className="prc-song-input" />
        </div>
        ...
        <div className="prc-song-list">
          {songs.slice(0, 12).map((s, i) => (
            <div key={s.id} className="prc-song-item">
              ...
              <span className="prc-song-item-by">{tx("invitacion.musica.sumadoPor")} {s.guestName || tx("invitacion.evento.invitado")}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div className="prc-cover-inner">
      ...
      <div className="prc-cover-content">
        <div className="prc-cover-top-row">
          <span>{tx("invitacion.pase.numeroTiara", { n: passNumber }).toUpperCase()}</span><span className="prc-accent-serif-2">ALL ACCESS</span>
        </div>
        ...
        <div className="prc-cover-bottom">
          <div ref={perfRef} className="prc-perf-strip prc-perf-strip--reveal prc-perf-strip--cover" />
          ...
          <div className="prc-barcode-wrap">
            <div className="prc-barcode" style={{ width: "62%", height: "clamp(15px, 3vh, 26px)", opacity: 0.6 }} />
            <span className="prc-mini-label prc-mini-label--cover" style={{ color: "#56534A" }}>{tx("invitacion.pase.noTransferible").toUpperCase()}</span>
          </div>
        </div>
      </div>
    </div>
...
const PRC_CSS = `
  ...
  a { color: #D98CB0; text-decoration: none; }
  a:hover { color: #FBEAF2; }
  ...
  .prc-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0E1F16; }
  ...
  @media (max-width: 767px) {
    .prc-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .prc-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(217,140,176,.3); }
    .prc-hero-photo-kicker { bottom: 40px; }
    .prc-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }
  ...
  .prc-date-month { font-family: var(--prc-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #D98CB0; padding-left: 12%; }

  .prc-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .prc-divider-line { width: 52px; height: 1px; background: #D98CB0; display: inline-block; }
  ...
  .prc-cal-link:hover { color: #D98CB0; }
  ...
  .prc-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #8C4A6E, #4A2A3C, #D98CB0, #FBEAF2, #8C4A6E); filter: saturate(.75); }
  ...
  .prc-medallion-sub--accent { color: #D98CB0; }
  .prc-medallion-arc { position: absolute; inset: -14%; }
  .prc-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #D98CB0; opacity: 0.4; font-family: var(--prc-mono), monospace; }

  .prc-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(217,140,176,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .prc-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #FBEAF2, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .prc-cd-num { font-family: var(--prc-cormorant), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6E1EC; display: inline-block; }
  .prc-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #D98CB0; }
  .prc-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0E1F16 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .prc-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .prc-perf-strip--cover { opacity: .9; }

  .prc-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(217,140,176,0.3), rgba(230,163,197,0.24), rgba(217,140,176,0.34), rgba(217,140,176,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .prc-accent-italic { font-style: italic; color: #D98CB0; }
  .prc-accent-serif { font-style: italic; color: #8C4A6E; font-family: var(--prc-cormorant), serif; }
  .prc-accent-serif-2 { color: #8C4A6E; }
  .prc-h2 { margin: 0; font-family: var(--prc-cormorant), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .prc-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .prc-pan[data-scroll="vertical"] { height: auto; }
  .prc-pan[data-scroll="vertical"] .prc-pan-sticky { position: static; height: auto; overflow: visible; }
  .prc-pan[data-scroll="vertical"] .prc-strip { position: static; display: block; width: 100%; transform: none !important; }
  .prc-pan[data-scroll="vertical"] .prc-panel { height: calc(var(--vh, 1vh) * 100); }
  ...
  .prc-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  ...
  .prc-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #8C4A6E; margin-top: auto; }
  ...
  .prc-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #8C4A6E; }
  ...
  .prc-crono-time { font-family: var(--prc-mono), monospace; color: #8C4A6E; min-width: 42px; }
  ...
  .prc-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #FBEAF2, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .prc-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(217,140,176,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .prc-rsvp-payment-total { color: #FBEAF2; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .prc-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #D98CB0; background: transparent; color: #D98CB0; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .prc-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(217,140,176,0.3); color: #F4F1EA; font-family: var(--prc-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .prc-rsvp-input::placeholder { color: #6E6A5D; }
  .prc-rsvp-input:focus { outline: none; border-bottom-color: #D98CB0; }
  .prc-rsvp-btn { width: 100%; padding: 16px; font-family: var(--prc-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #D98CB0, #8C4A6E); border: 1px solid #D98CB0; color: #0B0B10; cursor: pointer; }
  .prc-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .prc-rsvp-btn--ghost { background: transparent; color: #D98CB0; }
  .prc-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #D98CB0; margin: 0; }
  ...
  .prc-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(217,140,176,0.18); }
  ...
  .prc-bank-copy { flex-shrink: 0; font-family: var(--prc-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #D98CB0; background: transparent; color: #D98CB0; cursor: pointer; }
  .prc-bank-copy:hover { background: rgba(217,140,176,0.12); }
  ...
  .prc-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(217,140,176,0.3); padding-bottom: 12px; }
  ...
  .prc-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(217,140,176,0.3); color: #F4F1EA; font-family: var(--prc-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .prc-song-input::placeholder { color: #6E6A5D; }
  .prc-song-input:focus { outline: none; border-bottom-color: #D98CB0; }
  .prc-song-sep { color: #8A8577; flex-shrink: 0; }
  .prc-song-submit { flex-shrink: 0; background: none; border: none; color: #D98CB0; font-family: var(--prc-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .prc-song-submit:hover { color: #FBEAF2; }
  .prc-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .prc-song-error { font-size: 10px; color: #D98CB0; margin-top: 6px; }
  ...
  .prc-quiz-opt { font-family: var(--prc-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(217,140,176,0.4); background: transparent; color: #D98CB0; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .prc-quiz-opt:disabled { cursor: default; }
  .prc-quiz-opt--picked { background: #D98CB0; border-color: #D98CB0; color: #0B0B10; }
  .prc-quiz-opt--correct { background: #D98CB0; border-color: #D98CB0; color: #0B0B10; }
  .prc-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .prc-quiz-result { padding-top: 18px; border-top: 1px solid rgba(217,140,176,0.2); }
  .prc-quiz-result-score { margin: 0 0 6px; font-family: var(--prc-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #FBEAF2; }
  .prc-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .prc-final-card { border: 1px solid #D98CB0; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .prc-final-names { font-family: var(--prc-cormorant), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #FBEAF2; }
  ...
  .prc-replay { cursor: pointer; color: #D98CB0; }
  .prc-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .prc-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(217,140,176,0.14); }
  .prc-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .prc-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(217,140,176,0.16); position: relative; }
  .prc-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FBEAF2, #D98CB0); transition: height 260ms linear; display: block; }
  .prc-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #D98CB0; transition: color 500ms ease; }
  ...
  .prc-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1C3B26 0%, #16311F 46%, #0E1F16 100%); }
  .prc-cover-half--bottom .prc-cover-inner { top: auto; bottom: 0; }
  .prc-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(217,140,176,0.5), rgba(230,163,197,0.4), rgba(217,140,176,0.55), rgba(217,140,176,0.5)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .prc-cover-names { margin: 0; font-family: var(--prc-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FBEAF2; }
  .prc-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#D98CB0, transparent); display: block; }
  ...
  .prc-cover-cta { border: 1px solid #D98CB0; background: linear-gradient(100deg, rgba(217,140,176,0.08), rgba(251,234,242,0.2), rgba(217,140,176,0.08)); color: #F6E1EC; font-family: var(--prc-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .prc-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .prc-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(230,163,197,0.34), rgba(246,225,236,0.5), rgba(230,163,197,0.34)); color: #0B0B0F; }
  }
  ...
  .prc-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #D98CB0; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
