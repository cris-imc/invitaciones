/**
 * CineTemplate.tsx
 * Derivado de ModernoTemplate.tsx (misma arquitectura que Chic/Neon): misma
 * estructura, props, secciones y componentes reutilizados (Countdown,
 * AlbumCarousel, RSVPWizardV2, SongSuggestion, etc). Cambia la capa visual a
 * la estética "Cine" (documental/film, mockup/nuevo Plantillas Casamiento ->
 * Elena & Tomás): fondo casi negro cálido (#2A1D10) + acento sepia/latón
 * (#CE9A4A, variable por color), tipografía serif clásica itálica (EB
 * Garamond) + texto geométrico (Jost) + cifras del countdown en monoespaciada
 * (Space Mono, "estilo timestamp"), doodles de trazo fino de cine
...
// decorativo (claqueta, rollo de película, estrella).
...
const IconMusic = ({ className, style }: { className?: string; style?: React.CSSProperties } = {}) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.3} className={className} style={style} aria-hidden="true"><circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="2.2"/><path d="M12 3.5v1.6M12 18.9v1.6M3.5 12h1.6M18.9 12h1.6" opacity={0.6}/></svg>;
...
// Claqueta de cine doodle -- decorativa, usada en portada/hero y como marca
// de esquina del marco de la foto. Trazo fino, con las rayas típicas de la
// tapa y el cuerpo con "ACCIÓN" implícito por la forma, sin texto.
const IconClapper = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 32 26" fill="none" stroke="currentColor" strokeWidth={1.1} className={className} style={style} aria-hidden="true">
    <path d="M3 10.5 4.6 3.8 27.5 9.2 25.9 15.9Z" />
    <path d="M6 5 8.8 10.2M13 6.6l2.8 5.2M20 8.2l2.8 5.2" strokeWidth={0.9} />
    <rect x="3" y="10.5" width="26" height="12.5" rx="0.6" />
    <path d="M3 16.7h26" strokeWidth={0.8} opacity={0.6} />
  </svg>
);
// Rollo de película doodle -- separador de secciones y kickers. Dos carretes
// unidos por la cinta, con los agujeros del eje marcados.
const IconFilmReel = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 30 18" fill="none" stroke="currentColor" strokeWidth={1.05} className={className} style={style} aria-hidden="true">
    <circle cx="7.5" cy="9" r="6.7" />
    <circle cx="22.5" cy="9" r="6.7" />
    <circle cx="7.5" cy="9" r="1.4" />
    <circle cx="22.5" cy="9" r="1.4" />
    <path d="M7.5 4.6v0M4.2 11.2v0M10.8 11.2v0" strokeWidth={1.6} strokeLinecap="round" opacity={0.85} />
    <path d="M22.5 4.6v0M19.2 11.2v0M25.8 11.2v0" strokeWidth={1.6} strokeLinecap="round" opacity={0.85} />
    <path d="M13.6 9h2.8" strokeWidth={0.9} />
  </svg>
);
// Estrella de trazo fino tipo "5 estrellas de la crítica" -- acompaña
// títulos, separadores y el marco de la foto de portada.
const IconStarDoodle = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 22 22" fill="none" stroke="currentColor" strokeWidth={1.15} className={className} style={style} aria-hidden="true">
    <path d="M11 1.6 13.7 8l6.9.5-5.3 4.5 1.7 6.7L11 15.9l-6 3.8 1.7-6.7-5.3-4.5L8.3 8Z" />
  </svg>
);

// Tipografía exacta del mockup "Cine" (EB Garamond itálica + Jost texto +
// Space Mono para las cifras del countdown, "estilo timestamp"), escopeada
// solo a este componente vía CSS var override en el wrapper raíz — no toca
// layout.tsx ni las demás plantillas que comparten
...
interface CineTemplateAmbarProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#CE9A4A]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#CE9A4A] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#F5E6C8] break-all">{value}</span>
      </div>
      <button 
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#CE9A4A]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#CE9A4A] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#F5E6C8] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#F5E6C8" }}>
          {tx("invitacion.quiz.juegoCompletado")}
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#EDDCB0" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#B5A484" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#CE9A4A] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#EDDCB0" }}>
                  {tx("invitacion.quiz.promedioGlobal", { n: stats.count })} <strong style={{ color: "#F5E6C8" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#EDDCB0] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function CineTemplateAmbar({ invitation, guest, isPersonalized = false }: CineTemplateAmbarProps) {
  const tx = useTextos();
  ...
  // Entrada animada de los doodles de la portada (claqueta, rollo de
  // película, estrella) con anime.js. Corre una sola vez, cuando la portada
  ...
  useEffect(() => {
    ...
    animate(root.querySelectorAll(".cine-doodle"), {
      scale: [0, 1],
      rotate: [-15, 0],
      opacity: [0, 1],
      duration: 900,
    ...
    animate(root.querySelectorAll(".cine-seal"), {
      scale: [0.6, 1],
      opacity: [0, 1],
      duration: 700,
      delay: 150,
  ...
  useEffect(() => {
    ...
    const els = document.querySelectorAll(".cine-scroll-doodle");
  ...
  // Grano de película + parpadeo de proyector sobre la foto de portada,
  // ligado al progreso de scroll (ScrollObserver de anime.js): un velo de
  // grano estático que titila de intensidad + una franja de luz cálida (tipo
  // fuga de luz de proyector) que recorre la foto en diagonal.
  ...
  useEffect(() => {
    ...
    const observer = onScroll({
      ...
      onUpdate: (self) => {
        ...
        flare.style.opacity = String(intensity * 0.4);
        flare.style.transform = `translateX(${(p * 220) - 60}%)`;
        if (grain) {
          // Parpadeo tipo proyector: la opacidad del grano fluctúa en vez de
          // quedar fija, simulando el flicker de un rollo de película.
          grain.style.opacity = String(0.12 + Math.sin(p * 38) * 0.05 + intensity * 0.06);
        }
      },
    });
  ...
  const portadaTintColor1 = "#CE9A4A"; // ámbar, acento propio de Cine
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#2A1D10] via-[#3A240F] to-[#2A1D10] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
  ...
  return (
    <div
      className={`${cineGaramond.variable} ${cineJost.variable} ${cineMono.variable}`}
      style={{
        "--font-cormorant": "var(--cine-garamond)",
        "--font-inter": "var(--cine-jost)",
        "--font-sans": "var(--cine-jost)",
        "--t-acc": "#CE9A4A",
        "--t-acc2": "#CE9A4A",
        "--c-accent": "#CE9A4A",
        "--t-bg": "#2A1D10",
    ...
    >
      <style>{`
        ...
        .desktop-stage .tpl .cine-light-card h4 {
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(192,138,62, 0.2) !important;
        }
        /* Cuenta regresiva "estilo timestamp" -- cifras en monoespaciada,
           coherente con el motivo documental/film del mockup. "b" es el tag
           que usan las 4 variantes de estilo del Countdown compartido para
           el número, así que este selector cubre clasico/minimalista/
           capsulas/flip sin acoplarse a un data-style puntual. */
        #countdown b {
          font-family: var(--cine-mono), monospace !important;
          font-style: normal !important;
        }
        /* Perforaciones tipo filmina en los bordes de la foto de portada */
        .cine-sprockets {
          background-image: repeating-linear-gradient(90deg, var(--t-acc) 0 5px, transparent 5px 13px);
          opacity: 0.75;
        }
        ...
        #rsvp.section.dark input {
          background-color: #34240F !important;
          color: #F5E6C8 !important;
          border-radius: 6px !important;
          border: 1px solid rgba(192,138,62, 0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #CE9A4A !important;
          color: #2A1D10 !important;
          font-weight: 600 !important;
          border: none !important;
          text-transform: uppercase !important;
        ...
        #songs.d-sec.dark {
          background-color: #251A0D !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #CE9A4A !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #2A1D10 !important; /* Matches light sections */
          color: #F5E6C8 !important;
      ...
      {/* PORTADA / WELCOME OVERLAY (mesh sepia animado, glow pulsante) */}
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#2A1D10', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#EDDCB0] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          {portadaFondoAnimado && (
          ...
          {/* Doodles decorativos (claqueta, rollo de película, estrella) --
              ...
              coverRootRef), inertes hasta que corre el JS (opacity-0 inicial). */}
          <IconClapper className="cine-doodle opacity-0 absolute" style={{ width: 38, height: 30, top: '9%', left: '10%', color: 'rgba(192,138,62,0.5)' }} />
          <IconStarDoodle className="cine-doodle opacity-0 absolute" style={{ width: 18, height: 18, top: '15%', right: '14%', color: 'rgba(192,138,62,0.5)' }} />
          <IconFilmReel className="cine-doodle opacity-0 absolute" style={{ width: 30, height: 18, bottom: '20%', left: '12%', color: 'rgba(192,138,62,0.4)' }} />
          <IconStarDoodle className="cine-doodle opacity-0 absolute" style={{ width: 12, height: 12, bottom: '26%', right: '18%', color: 'rgba(192,138,62,0.35)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            {/* Guest Name */}
            <h2 className="text-4xl sm:text-5xl font-light tracking-wide text-[#EDDCB0] leading-relaxed" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className=" text-sm font-medium text-[#B5A484] tracking-wide uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8 }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #CE9A4A', color: '#CE9A4A',
                background: 'rgba(192,138,62,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#CE9A4A'; e.currentTarget.style.color = '#2A1D10'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(192,138,62,0.08)'; e.currentTarget.style.color = '#CE9A4A'; }}
            >
          ...
          <style jsx>{`
            @keyframes cine-meshDrift { 0%, 100% { background-position: 0% 0%, 100% 100%; } 50% { background-position: 30% 20%, 70% 80%; } }
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#CE9A4A]/40 shadow-md ${isTicketMaximized ? 'bg-[#2A1D10]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#F5E6C8]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#CE9A4A] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                <span className="text-[#F5E6C8] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#CE9A4A] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#CE9A4A]/20 pl-3">
                <span className="text-[#F5E6C8] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#B5A484] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#CE9A4A]" />
              <span className="text-[#3A2814]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <div className="desktop-stage" data-theme={theme} style={{
        ...
        // docs/GUIA_TECNICA_PLANTILLAS.md, es un gap real que ya afectó a
        // otras plantillas nuevas si se omite acá.
        "--t-acc": "#CE9A4A",
        "--t-acc2": "#CE9A4A",
        "--c-accent": "#CE9A4A",
        "--t-bg": "#2A1D10",
      ...
      <aside className="d-left hide-mobile" style={{ position: "relative", overflow: "hidden" }}>
        ...
        />
        {/* Perforaciones tipo filmina + marco fino, calcado del mockup Elena
            & Tomás (Cine) -- estáticos en escritorio (el efecto de grano
            ligado al scroll vive en la foto de portada de mobile). */}
        <div className="cine-sprockets absolute left-0 right-0 top-0 h-[7px] pointer-events-none z-20" aria-hidden="true" />
        <div className="cine-sprockets absolute left-0 right-0 bottom-0 h-[7px] pointer-events-none z-20" aria-hidden="true" />
        <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
          <div className="absolute inset-0" style={{ border: "1px solid rgba(192,138,62,0.55)" }} />
          <IconClapper style={{ position: "absolute", bottom: -10, right: -8, width: 26, height: 20, color: "#CE9A4A" }} />
        </div>
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#CE9A4A', margin: '6px 0 14px', animation: 'cine-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#2A1D10]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#2A1D10] z-10 relative">
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#CE9A4A] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#EDDCB0] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#CE9A4A' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#CE9A4A', margin: '0 0 20px', animation: 'cine-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#B5A484] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#CE9A4A] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          
          {/* Image Container -- filtro sepia + marco completo + perforaciones
              tipo filmina + grano/luz de proyector ligados al scroll (ver
              useEffect de heroPhotoRef). */}
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #2A1D10 100%)' }} />
            <div
            ...
            />
            {/* Fuga de luz cálida tipo proyector, ligada al progreso de
                scroll. */}
            <div ref={heroFlareRef} className="pointer-events-none z-20" style={{ position: "absolute", inset: 0, opacity: 0, background: "linear-gradient(75deg, transparent 30%, rgba(255,224,168,0.5) 48%, rgba(192,138,62,0.25) 55%, transparent 72%)" }} />
            {/* Marco completo + perforaciones tipo filmina + claqueta
                "saliendo" de la esquina, calcado del mockup. */}
            <div className="cine-sprockets absolute left-0 right-0 top-0 h-[7px] pointer-events-none z-20" aria-hidden="true" />
            <div className="cine-sprockets absolute left-0 right-0 bottom-0 h-[7px] pointer-events-none z-20" aria-hidden="true" />
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              <div className="absolute inset-0" style={{ border: "1px solid rgba(192,138,62,0.6)" }} />
              <IconClapper style={{ position: "absolute", top: -10, left: -8, width: 28, height: 22, color: "#CE9A4A" }} />
              <IconStarDoodle style={{ position: "absolute", bottom: -8, right: -6, width: 16, height: 16, color: "#CE9A4A" }} />
            </div>
          </div>
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #6B463014, transparent 70%), #3A2814" }}>
            <div className="max-w-2xl mx-auto text-center">
              ...
              <TypewriterText 
                ...
                className="text-[#EDDCB0] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        {/* Divisor doodle tipo "filmina": rollo de película flanqueado por
            perforaciones, entre el hero y la cuenta regresiva. */}
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#2A1D10]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #CE9A4A, transparent)' }} />
          <IconFilmReel className="cine-scroll-doodle opacity-0" style={{ width: 26, height: 16, color: '#CE9A4A' }} />
          <IconStarDoodle className="cine-scroll-doodle opacity-0" style={{ width: 13, height: 13, color: '#CE9A4A' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #CE9A4A, transparent)' }} />
        </div>

        <SectionWrapper id="details" delay={150} className="w-full bg-[#2A1D10] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#CE9A4A]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#CE9A4A] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#B5A484] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#F5E6C8] mb-3" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#B5A484]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {tx("invitacion.ubicacion.horaConSufijo", { hora: String(invitation.ceremoniaHora) })}</p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#B5A484]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#CE9A4A] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#CE9A4A] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#B5A484] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#CE9A4A] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#CE9A4A]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#CE9A4A]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#B5A484] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--cine-mono), monospace" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#F5E6C8] font-light" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#3A2814] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#251A0D] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <div className="flex justify-center mb-4">
                  <DrawLucideIcon icon={Landmark} size={46} color="var(--t-acc)" strokeWidth={1.5} />
                </div>
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#CE9A4A]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #6B463018, transparent 70%), #251A0D" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {/* Separador doodle de footer: rollo de película + perforaciones,
            calcado del motivo "filmina" del resto de la plantilla. */}
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#2A1D10' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #CE9A4A, transparent)' }} />
          <IconFilmReel className="cine-scroll-doodle opacity-0" style={{ width: 20, height: 12, color: '#CE9A4A' }} />
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #CE9A4A, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#2A1D10" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#CE9A4A" surfaceColor="#34240F" />}
      </div>
  );
}
