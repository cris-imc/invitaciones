...
export function PostalTemplate({ invitation, guest, isPersonalized = false }: PostalTemplateProps) {
  const tx = useTextos();
    ...
    return (
      <PostEventoStorytelling
        ...
        paleta={{
          ...
          tinta: PALETA.ink,
          acento: PALETA.acc,
          clase: `${posSerif.variable} ${posSans.variable} ${posMono.variable}`,
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="pos-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="pos-section pos-std">
          <div className="pos-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="pos-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="pos-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="pos-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="pos-std-fila">
              <span className="pos-cinta">{mesLargo}</span>
              <span className="pos-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="pos-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="pos-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="pos-btn-papel pos-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="pos-foto-seccion">
            <div className="pos-foto-marco">
              <div ref={ventanaRef} className="pos-ventana">
                {fotoMobile && (
              ...
              <div className="pos-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="pos-section pos-countdown">
          <div className="pos-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="pos-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="pos-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="pos-section pos-frase-seccion">
            <div className="pos-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pos-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="pos-firma">
              <span className="pos-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="pos-pan-fijo">
            <div data-strip="1" className="pos-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="pos-panel pos-panel--abajo" style={{ background: PALETA.bg }}>
                ...
                <div className="pos-tarjeta pos-tarjeta--izq">
                  <span className="pos-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="pos-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="pos-filas">
                    {direccion && (
                      <div className="pos-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="pos-fila-valor">{direccion}</span></div>
                    )}
                    <div className="pos-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="pos-fila pos-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="pos-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="pos-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="pos-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="pos-panel pos-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="pos-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="pos-tarjeta pos-tarjeta--der">
                    <h2 className="pos-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="pos-filas">
                      {ceremoniaDireccion && (
                        <div className="pos-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="pos-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="pos-fila pos-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="pos-panel pos-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="pos-escena" aria-hidden="true">
                  ...
                  <h2 className="pos-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="pos-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="pos-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="pos-panel pos-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="pos-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="pos-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="pos-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="pos-crono-fila">
                        <span className="pos-crono-hora">{item.time || ""}</span>
                        <span className="pos-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="pos-section pos-checkin">
            <div className="pos-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pos-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="pos-h2">
            ...
            <div className="pos-pan-fijo">
              <div data-strip="1" className="pos-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="pos-panel pos-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="pos-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="pos-h2 pos-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="pos-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="pos-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="pos-polaroid-img" />
                          {i % 2 === 0 && <span className="pos-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="pos-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="pos-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="pos-section pos-musica">
            <div className="pos-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pos-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="pos-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="pos-section pos-regalos">
            <span data-xin="1" data-dist="-20" className="pos-kicker pos-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="pos-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="pos-parrafo pos-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="pos-section pos-quiz">
            <div className="pos-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="pos-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="pos-h2 pos-h2--claro">{triviaTitulo}</h2>
            <TriviaPostal
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="pos-section pos-pase">
          <div className="pos-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="pos-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="pos-ticket">
            <div className="pos-ticket-izq">
              <span className="pos-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="pos-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="pos-ticket-dato">
                {[
              ...
              <span className="pos-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="pos-ticket-muesca pos-ticket-muesca--arriba" aria-hidden="true" />
              <span className="pos-ticket-muesca pos-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="pos-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="pos-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="pos-pase-pie">
            <span className="pos-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="pos-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="pos-portada">
        ...
        <div ref={escenaPortadaRef} className="pos-portada-escena">
          <div data-cl="1" className="pos-trama" aria-hidden="true" />
        </div>
        <div className="pos-portada-contenido">
          <div className="pos-portada-arriba">
            <div ref={cartelRef} className="pos-cartel">
              <h1 className="pos-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="pos-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="pos-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="pos-portada-abajo">
            <div className="pos-portada-datos">
              <span className="pos-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="pos-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="pos-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="pos-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="pos-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="pos-portada-pase">
            ...
            <button type="button" onClick={abrir} className="pos-portada-btn">
...
function CuentaPostal({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="pos-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`pos-cuenta-caja pos-cuenta-caja--${i + 1}`}>
          <span className={`pos-cuenta-num${i === 3 ? " pos-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_POS = `
  ...
  .pos-raiz a { color: var(--pp-acc); text-decoration: none; }
  .pos-raiz button { font: inherit; }
  .pos-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --pos-y y gira en --pos-giro: son
  ...
  .pos-scroller [data-xin] { transform: translate3d(0, var(--pos-y, 0px), 0) rotate(var(--pos-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .pos-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .pos-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .pos-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .pos-kicker--tinta { color: var(--pp-ink); }
  .pos-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--pos-serif), 'Alfa Slab One', serif;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .pos-h2 em, .pos-acento { font-style: italic; color: var(--pp-acc); }
  .pos-h2--claro { color: var(--pp-night-ink); }
  .pos-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .pos-acento-plano { color: var(--pp-acc); }
  .pos-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .pos-parrafo--tinta { color: var(--pp-ink); }
  .pos-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .pos-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .pos-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .pos-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .pos-std-dia { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-weight: 500;
  ...
  .pos-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .pos-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--pos-serif), 'Alfa Slab One', serif; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .pos-std-anio { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .pos-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .pos-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .pos-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .pos-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--pos-serif), 'Alfa Slab One', serif; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .pos-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .pos-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .pos-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .pos-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .pos-cuenta-caja--1 { --pos-giro: -1.6deg; transform: rotate(var(--pos-giro)); }
  .pos-cuenta-caja--2 { --pos-giro: 1.2deg; transform: rotate(var(--pos-giro)); margin-top: 10px; }
  .pos-cuenta-caja--3 { --pos-giro: 1.8deg; transform: rotate(var(--pos-giro)); }
  .pos-cuenta-caja--4 { --pos-giro: -1.1deg; transform: rotate(var(--pos-giro)); margin-top: 10px; }
  .pos-cuenta-num { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .pos-cuenta-num--acc { color: var(--pp-acc); }
  .pos-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .pos-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .pos-frase { position: relative; z-index: 1; margin: 0; font-family: var(--pos-serif), 'Alfa Slab One', serif;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .pos-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .pos-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .pos-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .pos-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .pos-panel--abajo { justify-content: flex-end; }
  .pos-panel--centro { justify-content: center; }
  .pos-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .pos-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .pos-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .pos-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .pos-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .pos-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .pos-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .pos-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .pos-tarjeta--izq { --pos-giro: -1deg; transform: rotate(var(--pos-giro)); }
  .pos-tarjeta--der { --pos-giro: 1.1deg; transform: rotate(var(--pos-giro)); }
  .pos-tarjeta--talon { --pos-giro: -.8deg; transform: rotate(var(--pos-giro)); overflow: visible; }
  .pos-tarjeta--hoy { --pos-giro: -1deg; transform: rotate(var(--pos-giro)); align-items: flex-start; }
  .pos-tarjeta--banco { margin-top: 4px; }
  .pos-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .pos-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .pos-tarjeta-titulo { margin: 0; font-family: var(--pos-serif), 'Alfa Slab One', serif; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .pos-tarjeta-pregunta { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: 26px; line-height: 1.15; }
  .pos-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .pos-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .pos-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .pos-fila > span:first-child { color: var(--pp-ink2); }
  .pos-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .pos-fila-valor { text-align: right; }
  .pos-fila--copiable { align-items: center; }
  .pos-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .pos-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .pos-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .pos-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .pos-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .pos-checkin { background: var(--pp-bg); }
  .pos-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .pos-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .pos-campo { display: flex; flex-direction: column; gap: 6px; }
  .pos-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .pos-contador { display: flex; align-items: center; gap: 12px; }
  .pos-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .pos-contador button:disabled { opacity: .35; cursor: default; }
  .pos-contador > span { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .pos-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--pos-sans), 'Work Sans', sans-serif; font-size: 16px; padding: 6px 2px; }
  .pos-input--serif { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: 20px; }
  .pos-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .pos-input::placeholder { color: #9A9388; }
  .pos-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .pos-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .pos-precio-total { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .pos-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .pos-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .pos-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .pos-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .pos-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--pos-sans), 'Work Sans', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .pos-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .pos-btn-solido:disabled { opacity: .6; cursor: default; }
  .pos-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .pos-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --pos-giro: -1deg; transform: rotate(var(--pos-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .pos-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .pos-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .pos-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .pos-crono-hora { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: 26px; min-width: 64px; line-height: 1; }
  .pos-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .pos-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .pos-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .pos-polaroids[data-cantidad="1"] .pos-polaroid { grid-column: span 6; }
  .pos-polaroids[data-cantidad="2"] .pos-polaroid { grid-column: span 3; }
  .pos-polaroids[data-cantidad="4"] .pos-polaroid { grid-column: span 3; }
  .pos-polaroids[data-cantidad="5"] .pos-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .pos-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .pos-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .pos-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .pos-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .pos-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .pos-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .pos-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .pos-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .pos-lista-tema { font-size: 15px; }
  .pos-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .pos-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .pos-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .pos-opciones { display: flex; flex-direction: column; gap: 8px; }
  .pos-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--pos-sans), 'Work Sans', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .pos-opcion:disabled { cursor: default; }
  .pos-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .pos-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .pos-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .pos-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --pos-giro: -.6deg; transform: rotate(var(--pos-giro));
    border-radius: 12px; overflow: hidden; }
  .pos-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .pos-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .pos-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .pos-ticket-nombre { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .pos-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .pos-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .pos-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .pos-ticket-muesca--arriba { top: -12px; }
  .pos-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .pos-ticket-der .qr-ingreso, .pos-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .pos-info-extra { position: relative; z-index: 1; }
  .pos-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .pos-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .pos-raiz .ia-icon-box { display: none !important; }
  .pos-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .pos-pase-firma { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .pos-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .pos-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }
  ...
  .pos-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .pos-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  ...
  .pos-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .pos-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .pos-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .pos-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .pos-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .pos-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .pos-cartel-nombres { margin: 0; font-family: var(--pos-serif), 'Alfa Slab One', serif; font-weight: 500;
  ...
  .pos-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .pos-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .pos-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .pos-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .pos-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .pos-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .pos-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .pos-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--pos-sans), 'Work Sans', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .pos-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .pos-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .pos-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .pos-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .pos-section, .pos-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .pos-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .pos-raiz { font-family: var(--pos-sans), 'Work Sans', sans-serif; }
  ...
  .pos-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .pos-kicker, .pos-panel-top, .pos-panel-pie, .pos-meta, .pos-riel-top, .pos-riel-etiqueta,
  .pos-ticket-kicker, .pos-cuenta-etq, .pos-tarjeta-kicker, .pos-fila-etq, .pos-portada-pase {
    font-family: var(--pos-mono), 'JetBrains Mono', monospace; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .pos-h2, .pos-frase, .pos-tarjeta-titulo, .pos-std-dia, .pos-cartel-nombres, .pos-ticket-nombre, .pos-cifra, .pos-cuenta-num {
    font-family: var(--pos-serif), 'Alfa Slab One', serif; letter-spacing: -.03em; }
  .pos-h2 em, .pos-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .pos-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .pos-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .pos-std-fila { justify-content: space-between; }
  .pos-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .pos-std-anio { font-family: var(--pos-serif), 'Alfa Slab One', serif; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .pos-std .pos-kicker, .pos-std .pos-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .pos-foto-marco { border: 3px solid var(--pp-ink); }
  .pos-ventana { clip-path: none !important; box-shadow: none; }
  .pos-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--pos-serif), 'Alfa Slab One', serif; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .pos-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .pos-tarjeta--izq, .pos-tarjeta--der, .pos-tarjeta--talon, .pos-tarjeta--hoy { transform: none; }
  .pos-doblez::before { display: none; }
  .pos-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .pos-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .pos-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .pos-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .pos-cuenta-caja:first-child { border-left: none; }
  .pos-cuenta-caja::before { display: none; }
  .pos-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .pos-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .pos-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .pos-panel[data-tone="dark"] .pos-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .pos-panel-top, .pos-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .pos-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .pos-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .pos-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .pos-btn-solido, .pos-btn-papel, .pos-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .pos-btn-papel { background: transparent; border: 1px solid currentColor; }
  .pos-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .pos-portada { background: var(--pp-bg); }
  .pos-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .pos-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .pos-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .pos-cartel-chinche { display: none; }
  .pos-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .pos-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .pos-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Postal: el titular va con el contorno de una postal impresa. */
  .pos-cartel-nombres, .pos-h2 { letter-spacing: -.01em; }
  .pos-tarjeta { border-style: dashed; }

  @media (prefers-reduced-motion: reduce) {
    .pos-raiz * { animation: none !important; }
    .pos-scroller [data-xin] { opacity: 1 !important; --pos-y: 0px; }
  }
`;
