...
export function CeramicaEditorialTemplate({ invitation, guest, isPersonalized = false }: CeramicaEditorialTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#B98B5E";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(185,139,94,.35), 0 18px 50px -30px #B98B5E";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#6E4A34" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(185,139,94,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(185,139,94,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#6E4A34" : "#B98B5E";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#19120D", tinta: "#F4F1EA", acento: "#B98B5E", clase: `${cmeSerif.variable} ${cmeMono.variable}`, fuente: "var(--cme-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="cme-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="cme-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #211712 55%, #19120D 100%)" }}>
            ...
            <div className="cme-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="25,18,13" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="25,18,13" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="cme-section cme-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #3A2A1E 0%, #26190F 55%, #19120D 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cme-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #19120D 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="cme-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="cme-pan-sticky">
            <div data-strip="1" className="cme-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cme-panel cme-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cme-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#6E4A34" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#6E4A34" />
                  </svg>
              ...
              <div data-tone="dark" className="cme-panel cme-panel--center" style={{ background: "#211712", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="cme-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #211712 60%, #19120D 100%)" }}>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="cme-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="cme-pan-sticky">
            <div data-strip="1" className="cme-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="cme-panel cme-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="cme-seguir cme-seguir--split">
                    ...
                    <span className="cme-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <CmeDots count={photoPages.length} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cme-section" style={{ background: "#211712" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="cme-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cme-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#EDEAE0" : "#B98B5E" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="cme-section" style={{ background: "#211712" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="cme-section" style={{ background: "#211712" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pieza" className="cme-section cme-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #211712 55%, #19120D 100%)" }}>
      ...
      <div ref={coverRef} className="cme-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
  ...
  return (
    <div className="cme-cover-inner">
      {/* Con foto cargada, la foto reemplaza el degradé de fondo -- el
          resto de la ornamentación (glow, glasé, medallón, nombres, CTA) se
          mantiene arriba sin cambios. Misma lógica que Flat: SOLO en mobile;
          en desktop el degradé original de la plantilla se ve igual que
          siempre, con o sin foto cargada. */}
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto
...
const CME_CSS = `
  ...
  a { color: #B98B5E; text-decoration: none; }
  ...
  .cme-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #19120D; }
  ...
  @media (min-width: 768px) {
    .cme-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(185,139,94,.3); }
  ...
  .cme-date-month { font-family: var(--cme-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B98B5E; padding-left: 12%; }

  .cme-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cme-divider-line { width: 52px; height: 1px; background: #B98B5E; display: inline-block; }
  ...
  .cme-cal-link:hover { color: #B98B5E; }
  ...
  .cme-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E4A34, #3A2A20, #B98B5E, #EDEAE0, #6E4A34); filter: saturate(.75); }
  .cme-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .cme-medallion-sub--accent { color: #B98B5E; }
  ...
  .cme-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B98B5E; opacity: 0.4; font-family: var(--cme-mono), monospace; }

  .cme-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(185,139,94,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  ...
  .cme-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  ...
  .cme-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B98B5E; }
  .cme-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #19120D 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .cme-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,74,52,0.3), rgba(237,234,224,0.16), rgba(185,139,94,0.32), rgba(110,74,52,0.3)); filter: blur(80px); opacity: .4; animation: cmeFoil 30s linear infinite; }
  ...
  .cme-accent-italic { font-style: italic; color: #B98B5E; }
  .cme-accent-serif { font-style: italic; color: #6E4A34; font-family: var(--cme-serif), serif; }
  .cme-accent-serif-2 { color: #6E4A34; }
  .cme-h2 { margin: 0; font-family: var(--cme-serif), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .cme-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .cme-pan[data-scroll="vertical"] { height: auto; }
  .cme-pan[data-scroll="vertical"] .cme-pan-sticky { position: static; height: auto; overflow: visible; }
  .cme-pan[data-scroll="vertical"] .cme-strip { position: static; display: block; width: 100%; transform: none !important; }
  .cme-pan[data-scroll="vertical"] .cme-panel { height: calc(var(--vh, 1vh) * 100); }
  ...
  .cme-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #6E4A34; margin-top: auto; }
  ...
  .cme-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #6E4A34; }
  ...
  .cme-crono-time { font-family: var(--cme-mono), monospace; color: #6E4A34; min-width: 42px; }
  ...
  .cme-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cme-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(185,139,94,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cme-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B98B5E; background: transparent; color: #B98B5E; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cme-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(185,139,94,0.3); color: #F4F1EA; font-family: var(--cme-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cme-rsvp-input::placeholder { color: #6E6A5D; }
  .cme-rsvp-input:focus { outline: none; border-bottom-color: #B98B5E; }
  .cme-rsvp-btn { width: 100%; padding: 17px 0; font-family: var(--cme-mono), monospace; font-size: 11.5px; letter-spacing: 0.26em; text-align: center; text-transform: uppercase; border: 1px solid #B98B5E; background: linear-gradient(100deg, rgba(185,139,94,0.08), rgba(237,234,224,0.2), rgba(185,139,94,0.08)); color: #F6EFDD; cursor: pointer; }
  ...
  .cme-rsvp-btn--ghost { background: transparent; color: #B98B5E; }
  .cme-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(185,139,94,0.1); color: #B98B5E; }
  .cme-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B98B5E; margin: 0; }
  ...
  .cme-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(185,139,94,0.18); }
  ...
  .cme-bank-copy { flex-shrink: 0; font-family: var(--cme-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B98B5E; background: transparent; color: #B98B5E; cursor: pointer; }
  .cme-bank-copy:hover { background: rgba(185,139,94,0.12); }
  ...
  .cme-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(185,139,94,0.3); padding-bottom: 12px; }
  ...
  .cme-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(185,139,94,0.3); color: #F4F1EA; font-family: var(--cme-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cme-song-input::placeholder { color: #6E6A5D; }
  .cme-song-input:focus { outline: none; border-bottom-color: #B98B5E; }
  .cme-song-sep { color: #8A8577; flex-shrink: 0; }
  .cme-song-submit { flex-shrink: 0; background: none; border: none; color: #B98B5E; font-family: var(--cme-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cme-song-submit:hover { color: #EDEAE0; }
  .cme-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cme-song-error { font-size: 10px; color: #B98B5E; margin-top: 6px; }
  ...
  .cme-quiz-opt { font-family: var(--cme-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(185,139,94,0.4); background: transparent; color: #B98B5E; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cme-quiz-opt:disabled { cursor: default; }
  .cme-quiz-opt--picked { background: #B98B5E; border-color: #B98B5E; color: #0B0B10; }
  .cme-quiz-opt--correct { background: #B98B5E; border-color: #B98B5E; color: #0B0B10; }
  .cme-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cme-quiz-result { padding-top: 18px; border-top: 1px solid rgba(185,139,94,0.2); }
  ...
  .cme-final-card { border: 1px solid #B98B5E; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .cme-replay { cursor: pointer; color: #B98B5E; }
  .cme-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cme-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(185,139,94,0.14); }
  .cme-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cme-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(185,139,94,0.16); position: relative; }
  .cme-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#EDEAE0, #B98B5E); transition: height 260ms linear; display: block; }
  .cme-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B98B5E; transition: color 500ms ease; }
  ...
  .cme-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #2E2018 0%, #211712 46%, #19120D 100%); }
  .cme-cover-half--bottom .cme-cover-inner { top: auto; bottom: 0; }
  .cme-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,74,52,0.5), rgba(237,234,224,0.24), rgba(185,139,94,0.5), rgba(110,74,52,0.5)); filter: blur(64px); opacity: .62; animation: cmeFoil 26s linear infinite; }
  ...
  .cme-cover-names { margin: 0; font-family: var(--cme-serif), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EDEAE0; }
  .cme-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B98B5E, transparent); display: block; }
  ...
  .cme-cover-cta { border: 1px solid #B98B5E; background: linear-gradient(100deg, rgba(185,139,94,0.08), rgba(237,234,224,0.2), rgba(185,139,94,0.08)); color: #F6EFDD; font-family: var(--cme-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .cme-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B98B5E; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
