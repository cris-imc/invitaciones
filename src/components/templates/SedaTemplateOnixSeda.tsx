 ...
 *   --t-bg #10141A · --t-surface #182029 · --seda-ink #EAEFF5
 *   --t-muted #8590A0 · --t-acc #7FA8D9 (rosa polvo) · --t-acc2 #C9A15C (oro champagne)
 *
...
interface SedaTemplateOnixSedaProps {
...
export function SedaTemplateOnixSeda({ invitation, guest, isPersonalized = false }: SedaTemplateOnixSedaProps) {
  ...
  // Portada animada -- Seda es CSS-vars, tema claro (--t-bg #10141A, ink
  // #EAEFF5) -- mismo caso que Perlada/Petalos: texto no puede forzarse a
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #FFFFFF 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #EAEFF5)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #EAEFF5)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#7FA8D9" surfaceColor="#182029" inactiveColor="#EAEFF5" solid />}
      </div>
  );
}
