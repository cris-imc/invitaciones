 ...
 *   --t-bg #241417 · --t-surface #331B20 · --seda-ink #F3E4DC
 *   --t-muted #C9A8A0 · --t-acc #8C4A52 (rosa polvo) · --t-acc2 #D9BFA0 (oro champagne)
 *
...
interface SedaTemplateNocturnaProps {
...
export function SedaTemplateNocturna({ invitation, guest, isPersonalized = false }: SedaTemplateNocturnaProps) {
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #FFFFFF 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #F3E4DC)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <h2 className="text-4xl sm:text-5xl font-light tracking-wide leading-relaxed" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif', color: 'var(--seda-ink)' }}>
              {guestNameDisplay}
            </h2>
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #F3E4DC)" />
        </div>
      </div>
