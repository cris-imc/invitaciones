...
// TrazoDePapelTemplate.tsx — no editar a mano: los cambios se pierden al regenerar.

/**
 * TRAZO DE PAPEL · Colección Iconic — sub-colección "Capas de papel"
 * Variante: Tinta china
 *
 ...
 * scripts/familias/capas/trazo-de-papel.json.
 *
 * El casamiento dibujado a mano: en vez de capas de papel recortado, trazos
 * de pincel sueltos -- la pareja abrazada, el ramo, las copas, el auto con
 * latas -- y lettering escrito a mano en las secciones. Caveat Brush para
 * los títulos y Karla para el resto.
 * 
 * Es la única familia de la sub-colección SIN foto principal: el mockup no
 * trae esa sección porque el dibujo es el protagonista. Si el anfitrión
 * carga un recorte igual, la sección aparece sin decoración propia.
 *
 * Las escenas son las del mockup, byte por byte (ver
 * escenas/TdpEscenas.ts y scripts/extraer-escenas-capas.js): son
...
import { Caveat_Brush, Karla } from "next/font/google";
...
import {
  DEFS_TDP, SAVETHEDATE_TDP, COUNTDOWN_TDP, FRASE_TDP, CUANDO_TDP,
  CHECKIN_TDP, ALBUM_TDP, MUSICA_TDP, REGALOS_TDP, QUIZ_TDP, PASE_TDP, COVER_TDP,
} from "@/components/templates/escenas/TdpEscenas";

const tdpSerif = Caveat_Brush({
  subsets: ["latin"],
  weight: ["400"],
  display: "swap",
  variable: "--tdp-serif",
...
interface TrazoDePapelTemplateProps {
...
const ESCENA_DE_PANEL: Record<string, string> = {
  recepcion: CUANDO_TDP[0] ?? "",
  ceremonia: CUANDO_TDP[1] ?? "",
  llegar: CUANDO_TDP[2] ?? "",
  cronograma: CUANDO_TDP[3] ?? "",
};
...
export function TrazoDePapelTemplateTinta({ invitation, guest, isPersonalized = false }: TrazoDePapelTemplateProps) {
  const tx = useTextos();
  ...
  useEffect(() => {
    ...
    reveals.forEach((el) => {
      ...
      el.style.opacity = "0";
      el.style.setProperty("--tdp-y", `${dist}px`);
      ...
      if (sc) {
        ...
        for (let i = pendientes.length - 1; i >= 0; i--) {
          ...
          if (r.top < vh * 0.92 && r.bottom > 0 && r.left < window.innerWidth && r.right > 0) {
            el.style.opacity = "1";
            el.style.setProperty("--tdp-y", "0px");
    ...
    return (
      <PostEventoStorytelling
        ...
        paleta={{
          ...
          tinta: PALETA.ink,
          acento: PALETA.acc,
          clase: `${tdpSerif.variable} ${tdpSans.variable}`,
          fuente: "var(--tdp-sans), 'Karla', sans-serif",
        }}
      />
    );
  }
  ...
  return (
    <div
      ref={raizRef}
      className={`${tdpSerif.variable} ${tdpSans.variable} tdp-raiz`}
      style={varsDePaleta}
    >
      <style>{CSS_TDP}</style>
      ...
      <Escena html={DEFS_TDP} className="tdp-defs" />

      <div ref={scrollerRef} className="tdp-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="tdp-section tdp-std">
          <Escena html={SAVETHEDATE_TDP} className="tdp-escena" />
          <span data-xin="1" data-dist="-20" className="tdp-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="tdp-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="tdp-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="tdp-std-fila">
              <span className="tdp-cinta">{mesLargo}</span>
              <span className="tdp-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="tdp-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="tdp-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="tdp-btn-papel tdp-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="tdp-foto-seccion">
            <div className="tdp-foto-marco">
              <div ref={ventanaRef} className="tdp-ventana">
                {fotoMobile && (
                  <div className="acp-mobile-only tdp-foto-capa">
                    <AnimatedCoverPhoto photoSrc={fotoMobile} tint={false} effect="enfoque" scrimColorRgb="243,235,221" />
                  </div>
                )}
                {fotoDesktop && (
                  <div className="acp-desktop-only tdp-foto-capa">
                    <AnimatedCoverPhoto photoSrc={fotoDesktop} tint={false} effect="enfoque" scrimColorRgb="243,235,221" />
                  </div>
                )}
              </div>
              <div className="tdp-foto-firma">{titulo}</div>
            </div>
          </section>
        )}

        {/* ── 02 Falta poco ──────────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="tdp-section tdp-countdown">
          <Escena html={COUNTDOWN_TDP} className="tdp-escena" />
          <span data-xin="1" data-dist="-20" className="tdp-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="tdp-h2">
            {tx("invitacion.cuentaRegresiva.estamosContando")}<br /><em>{tx("invitacion.cuentaRegresiva.losDias")}</em>
          </h2>
          <CuentaTrazoPapel targetDate={fechaHora} />
        </section>

        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="tdp-section tdp-frase-seccion">
            <Escena html={FRASE_TDP} className="tdp-escena" />
            <span data-xin="1" data-dist="-20" className="tdp-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            <h2 ref={fraseRef} className="tdp-frase">
              {palabras.map((p, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= desdeAcento ? "tdp-acento" : undefined}>{p}</span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="200" className="tdp-firma">
              <span className="tdp-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="tdp-pan-fijo">
            <div data-strip="1" className="tdp-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="tdp-panel tdp-panel--abajo" style={{ background: PALETA.bg }}>
                <Escena html={ESCENA_DE_PANEL.recepcion} className="tdp-escena" />
                <div className="tdp-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="tdp-tarjeta tdp-tarjeta--izq">
                  <span className="tdp-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="tdp-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="tdp-filas">
                    {direccion && (
                      <div className="tdp-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="tdp-fila-valor">{direccion}</span></div>
                    )}
                    <div className="tdp-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="tdp-fila tdp-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="tdp-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="tdp-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="tdp-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="tdp-panel tdp-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <Escena html={ESCENA_DE_PANEL.ceremonia} className="tdp-escena" />
                  <div className="tdp-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="tdp-tarjeta tdp-tarjeta--der">
                    <h2 className="tdp-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="tdp-filas">
                      {ceremoniaDireccion && (
                        <div className="tdp-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="tdp-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="tdp-fila tdp-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="tdp-panel tdp-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="tdp-escena" aria-hidden="true">
                  ...
                  <Escena html={ESCENA_DE_PANEL.llegar} className="tdp-escena" />
                  <div className="tdp-panel-top">
                    <span>{tx("invitacion.ubicacion.comoLlegar").toUpperCase()}</span><span>{deLugar("llegar")}</span>
                  </div>
                  <h2 className="tdp-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="tdp-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                    <div className="tdp-mapa">
                      <iframe
                        src={embedMapUrl}
                        width="100%"
                        height="100%"
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="tdp-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="tdp-panel tdp-panel--centro" style={{ background: PALETA.bg2 }}>
                  <Escena html={ESCENA_DE_PANEL.cronograma} className="tdp-escena" />
                  <div className="tdp-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="tdp-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="tdp-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="tdp-crono-fila">
                        <span className="tdp-crono-hora">{item.time || ""}</span>
                        <span className="tdp-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* ── 05 Check-in ────────────────────────────────────────────── */}
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="tdp-section tdp-checkin">
            <Escena html={CHECKIN_TDP} className="tdp-escena" />
            <span data-xin="1" data-dist="-20" className="tdp-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="tdp-h2">
            ...
            <div className="tdp-pan-fijo">
              <div data-strip="1" className="tdp-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="tdp-panel tdp-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <Escena html={ALBUM_TDP[iHoja % ALBUM_TDP.length]} className="tdp-escena" />
                    <div className="tdp-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="tdp-h2 tdp-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="tdp-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="tdp-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="tdp-polaroid-img" />
                          {i % 2 === 0 && <span className="tdp-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="tdp-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="tdp-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="tdp-section tdp-musica">
            <Escena html={MUSICA_TDP} className="tdp-escena" />
            <span data-xin="1" data-dist="-20" className="tdp-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="tdp-h2">
              {tituloEnDosLineas(tx("invitacion.sabor.preguntaCancionFaltar"), "tdp-acento")}
            </h2>
            <CancionesTrazoPapel
              invitationId={String(invitation.id ?? "")}
              guestToken={guest?.uniqueToken}
        ...
        {/* ── 08 Regalos ─────────────────────────────────────────────── */}
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="tdp-section tdp-regalos">
            <Escena html={REGALOS_TDP} className="tdp-escena" />
            <span data-xin="1" data-dist="-20" className="tdp-kicker tdp-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="tdp-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="tdp-parrafo tdp-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {/* ── 09 Trivia ──────────────────────────────────────────────── */}
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="tdp-section tdp-quiz">
            <Escena html={QUIZ_TDP} className="tdp-escena" />
            <span data-xin="1" data-dist="-20" className="tdp-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="tdp-h2 tdp-h2--claro">{triviaTitulo}</h2>
            <TriviaTrazoPapel
        ...
        {/* ── 10 Tu pase ─────────────────────────────────────────────── */}
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="tdp-section tdp-pase">
          <Escena html={PASE_TDP} className="tdp-escena" />
          <span data-xin="1" data-dist="-20" className="tdp-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="tdp-ticket">
            <div className="tdp-ticket-izq">
              <span className="tdp-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="tdp-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="tdp-ticket-dato">
                {[
              ...
              <span className="tdp-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="tdp-ticket-muesca tdp-ticket-muesca--arriba" aria-hidden="true" />
              <span className="tdp-ticket-muesca tdp-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="tdp-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="tdp-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="tdp-pase-pie">
            <span className="tdp-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="tdp-pase-creditos">
              <LogoFooterCredit bgColor="transparent" />
              <span className="tdp-replay" role="button" tabIndex={0} onClick={volverAVerla} onKeyDown={(e) => { if (e.key === "Enter") volverAVerla(); }}>
      ...
      {/* ── Riel de progreso ───────────────────────────────────────────── */}
      <div ref={rielRef} className="tdp-riel">
        <span ref={rielTopRef} className="tdp-riel-top">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
        <div ref={rielLineaRef} className="tdp-riel-linea">
          <span ref={rielBarraRef} className="tdp-riel-barra" />
        </div>
        <span ref={rielEtiquetaRef} className="tdp-riel-etiqueta">{tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
      </div>

      {/* ── La portada ─────────────────────────────────────────────────── */}
      <div ref={portadaRef} data-tone={TONO} className="tdp-portada">
        <div ref={escenaPortadaRef} className="tdp-portada-escena" dangerouslySetInnerHTML={{ __html: COVER_TDP }} />
        <div className="tdp-portada-contenido">
          <div className="tdp-portada-arriba">
            <div ref={cartelRef} className="tdp-cartel">
              <h1 className="tdp-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="tdp-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="tdp-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="tdp-portada-abajo">
            <div className="tdp-portada-datos">
              <span className="tdp-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="tdp-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="tdp-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="tdp-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="tdp-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="tdp-portada-pase">
            ...
            <button type="button" onClick={abrir} className="tdp-portada-btn">
      ...
      <div ref={pistaRef} className="tdp-pista">{tx("invitacion.portada.desliza").toUpperCase()} ↓</div>

      {fotoAmpliada && (
        <div className="tdp-lupa" onClick={() => setFotoAmpliada(null)} onContextMenu={(e) => e.preventDefault()}>
          <button
            type="button"
            className="tdp-lupa-cerrar"
          ...
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={fotoAmpliada} alt={tx("invitacion.album.fotoAmpliada")} className="tdp-lupa-img" draggable={false} onClick={(e) => e.stopPropagation()} />
        </div>
      )}
...
function Puntos({ cantidad }: { cantidad: number }) {
  ...
  return (
    <div className="tdp-puntos" aria-hidden="true">
      {Array.from({ length: cantidad }).map((_, i) => (
        <span key={i} data-dot={i} className="tdp-punto" style={{ background: i === 0 ? PALETA.acc : "rgba(43,42,51,.18)" }} />
      ))}
    </div>
  );
}
 ...
 */
function CuentaTrazoPapel({ targetDate }: { targetDate: Date }) {
  const tx = useTextos();
  const { time, isEventDay, isPast, hasEnded } = useCountdown(targetDate);

  if (isEventDay || hasEnded || isPast) {
    return (
      <div className="tdp-tarjeta tdp-tarjeta--hoy" data-xin="1" data-delay="80" data-dist="30">
        <span className="tdp-tarjeta-kicker">{tx("invitacion.cuentaRegresiva.kicker").toUpperCase()}</span>
        <span className="tdp-tarjeta-titulo">
  ...
  return (
    <div className="tdp-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`tdp-cuenta-caja tdp-cuenta-caja--${i + 1}`}>
          <span className={`tdp-cuenta-num${i === 3 ? " tdp-cuenta-num--acc" : ""}`}>{c.v}</span>
          <span className="tdp-cuenta-etq">{c.l.toUpperCase()}</span>
        </div>
      ))}
    </div>
  );
...
function FilaCopiable({ etiqueta, valor }: { etiqueta: string; valor: string }) {
  ...
  return (
    <div className="tdp-fila tdp-fila--copiable">
      <div className="tdp-fila-texto">
        <span className="tdp-fila-etq">{etiqueta.toUpperCase()}</span>
        <span className="tdp-fila-dato">{valor}</span>
      </div>
      <button type="button" onClick={copiar} className={`tdp-btn-copiar${copiado ? " tdp-btn-copiar--hecho" : ""}`}>
...
function TarjetaBancaria({
  ...
  return (
    <div
      ...
      data-delay={retraso}
      data-dist="30"
      className={`tdp-tarjeta tdp-tarjeta--banco${dobleZ ? " tdp-doblez" : ""}${inclinada ? " tdp-tarjeta--der" : " tdp-tarjeta--izq"}`}
    >
      <span className="tdp-tarjeta-kicker">{titulo.toUpperCase()}</span>
      {mensaje && <p className="tdp-tarjeta-mensaje">{mensaje}</p>}
      <div className="tdp-filas">
        <FilaCopiable etiqueta="Alias" valor={alias} />
        <FilaCopiable etiqueta="CBU" valor={cbu} />
        {(banco || titular) && (
          <div className="tdp-fila tdp-fila--ultima">
            <span>{banco || tx("invitacion.regalos.banco")}</span>
            <span className="tdp-fila-valor">{titular}</span>
          </div>
        )}
      </div>
    </div>
...
function CheckinTrazoPapel({
  ...
  if (estado === "DECLINED") {
    return (
      <div data-xin="1" data-delay="140" data-dist="30" className="tdp-tarjeta tdp-tarjeta--izq">
        <p className="tdp-tarjeta-mensaje">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
        <button type="button" className="tdp-btn-fantasma" onClick={() => setEstado("PENDING")}>
  ...
  return (
    <>
      <div ref={tarjetaRef} data-xin="1" data-delay="140" data-dist="30" className="tdp-tarjeta tdp-tarjeta--talon">
        <div className="tdp-talon-top">
          <span>{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}{guestName ? ` · ${guestName.toUpperCase()}` : ""}</span>
          <span ref={estadoRef} className="tdp-talon-estado">
        ...
        {!confirmadoYa ? (
          <>
            {lugares > 1 && (
              <div className="tdp-campo">
                <label className="tdp-etiqueta">{tx("invitacion.rsvp.adultos").toUpperCase()}</label>
                <div className="tdp-contador">
            ...
            {maxAdolescentes > 0 && (
              <div className="tdp-campo">
                <label className="tdp-etiqueta">{tx("invitacion.rsvp.adolescentes").toUpperCase()}</label>
                <div className="tdp-contador">
            ...
            {maxNinos > 0 && (
              <div className="tdp-campo">
                <label className="tdp-etiqueta">{tx("invitacion.rsvp.ninos").toUpperCase()}</label>
                <div className="tdp-contador">
            ...
            <div className="tdp-campo">
              <label className="tdp-etiqueta">{tx("invitacion.rsvp.restriccionAlimentaria").toUpperCase()}</label>
              <input
                value={dieta}
          ...
          <div className="tdp-filas">
            {lugares > 1 && adultos > 0 && <div className="tdp-fila"><span>{tx("invitacion.rsvp.adultos")}</span><span>{adultos}</span></div>}
            {adolescentes > 0 && <div className="tdp-fila"><span>{tx("invitacion.rsvp.adolescentes")}</span><span>{adolescentes}</span></div>}
            {ninos > 0 && <div className="tdp-fila"><span>{tx("invitacion.rsvp.ninos")}</span><span>{ninos}</span></div>}
            <div className="tdp-fila tdp-fila--ultima">
              <span>{tx("invitacion.rsvp.restriccionAlimentaria")}</span>
              <span className="tdp-fila-valor">{restricciones || dieta || "—"}</span>
            </div>
          </div>
        )}

        {hayPago && monto != null && !exento && (
          <div className="tdp-precio">
            <span>{estadoDePago === "PAID" ? tx("invitacion.pago.abonado") : tx("invitacion.pago.valor")}</span>
            <div className="tdp-precio-valor">
              <span className="tdp-precio-total">{formatearMoneda(totalAPagar)}</span>
              {usarTotalDelServidor && vistaDePago
                ? vistaDePago.lines.map((l, i) => <span key={i} className="tdp-precio-detalle">{l}</span>)
                : (
                  <>
                    {adultos > 0 && <span className="tdp-precio-detalle">{adultos} × {formatearMoneda(precioAdulto)}</span>}
                    {adolescentes > 0 && <span className="tdp-precio-detalle">{adolescentes} × {formatearMoneda(precioAdo)}</span>}
                    {ninos > 0 && <span className="tdp-precio-detalle">{ninos} × {formatearMoneda(precioNin)}</span>}
                  </>
                )}
            </div>
          </div>
        ...
        <div ref={selloRef} className="tdp-sello" aria-hidden="true">{tx("invitacion.pase.confirmado").toUpperCase()}</div>
        <div ref={petalosRef} className="tdp-petalos" aria-hidden="true" />
      </div>

      {error && <p className="tdp-error">{error}</p>}

      {!confirmadoYa ? (
        <>
          <button type="button" data-xin="1" data-delay="220" className="tdp-btn-solido" disabled={enviando} onClick={() => enviar("CONFIRMA")}>
            {enviando ? tx("invitacion.rsvp.guardando").toUpperCase() : tx("invitacion.rsvp.confirmarAsistencia").toUpperCase()}
          </button>
          <button type="button" data-xin="1" data-delay="280" className="tdp-btn-fantasma" disabled={enviando} onClick={() => enviar("NO_ASISTE")}>
...
function CancionesTrazoPapel({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
  ...
  return (
    <>
      <form onSubmit={enviar} data-xin="1" data-delay="140" data-dist="30" className="tdp-tarjeta tdp-tarjeta--der">
        <div className="tdp-campo">
          <label className="tdp-etiqueta">{tx("invitacion.musica.tema").toUpperCase()}</label>
          <input value={tema} onChange={(e) => setTema(e.target.value)} placeholder={tx("invitacion.musica.nombreDeLaCancion")} maxLength={100} className="tdp-input tdp-input--serif" />
        </div>
        <div className="tdp-campo">
          <label className="tdp-etiqueta">{tx("invitacion.musica.artista").toUpperCase()}</label>
          <input value={artista} onChange={(e) => setArtista(e.target.value)} placeholder={tx("invitacion.musica.artista")} maxLength={80} className="tdp-input tdp-input--serif" />
        </div>
        {error && <p className="tdp-error">{error}</p>}
        <button type="submit" disabled={enviando} className="tdp-btn-solido tdp-btn-solido--tinta">
          {enviando ? tx("invitacion.musica.enviando").toUpperCase() : tx("invitacion.musica.sugeriUnaCancion").toUpperCase()}
        </button>
      </form>

      {canciones.length > 0 && (
        <div data-xin="1" data-delay="220" className="tdp-lista">
          {canciones.slice(0, 8).map((c) => (
            <div key={c.id} className="tdp-lista-fila">
              <div className="tdp-lista-texto">
                <span className="tdp-lista-tema">{c.title}</span>
                <span className="tdp-lista-quien">{c.artist} · {tx("invitacion.musica.sumadoPor")} {c.guestName || tx("invitacion.evento.invitado")}</span>
              </div>
            </div>
          ))}
        </div>
 ...
 */
function TriviaTrazoPapel({ preguntas, invitationId, guestToken, guestName }: { preguntas: QuizPregunta[]; invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
  ...
  if (terminado) {
    return (
      <div data-xin="1" data-delay="140" data-dist="30" className="tdp-tarjeta tdp-tarjeta--izq">
        <span className="tdp-tarjeta-kicker">{tx("invitacion.quiz.kicker").toUpperCase()}</span>
        <span className="tdp-tarjeta-titulo">
        ...
        {!guardando && stats && stats.count > 0 && (
          <p className="tdp-tarjeta-mensaje">{tx("invitacion.quiz.promedio", { n: stats.count, avg: stats.avg })}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div data-xin="1" data-delay="140" data-dist="30" className="tdp-tarjeta tdp-tarjeta--izq">
      <span className="tdp-tarjeta-kicker">{tx("invitacion.quiz.preguntaDeTotal", { n: indice + 1, total: preguntas.length }).toUpperCase()}</span>
      <span className="tdp-tarjeta-pregunta">{q.pregunta}</span>
      <div className="tdp-opciones">
        {q.opciones.map((op, oi) => {
          let clase = "";
          if (yaEligio && correcta !== undefined) {
            if (oi === correcta) clase = " tdp-opcion--bien";
            else if (elegidas[indice] === oi) clase = " tdp-opcion--mal";
          }
          return (
            <button key={oi} type="button" disabled={yaEligio} onClick={() => elegir(oi)} className={`tdp-opcion${clase}`}>
              {op}
            </button>
          );
        })}
...
// leen la Bienvenida y el Post-evento compartidos (esperan `tdp-section` y
// `tdp-kicker`).
const CSS_TDP = `
  .tdp-raiz { position: fixed; inset: 0; width: 100%; height: calc(var(--vh, 1vh) * 100); overflow: hidden;
    background: var(--pp-bg); color: var(--pp-ink); font-family: var(--tdp-sans), 'Karla', sans-serif; }
  .tdp-raiz a { color: var(--pp-acc); text-decoration: none; }
  .tdp-raiz button { font: inherit; }
  .tdp-defs { position: absolute; width: 0; height: 0; overflow: hidden; }

  .tdp-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0;
    transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .tdp-scroller::-webkit-scrollbar { width: 0; height: 0; }
  ...
  /* Lo que entra al scrollear se mueve en --tdp-y y gira en --tdp-giro: son
  ...
  .tdp-scroller [data-xin] { transform: translate3d(0, var(--tdp-y, 0px), 0) rotate(var(--tdp-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .tdp-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .tdp-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .tdp-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .tdp-kicker--tinta { color: var(--pp-ink); }
  .tdp-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--tdp-serif), 'Caveat Brush', cursive;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .tdp-h2 em, .tdp-acento { font-style: italic; color: var(--pp-acc); }
  .tdp-h2--claro { color: var(--pp-night-ink); }
  .tdp-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .tdp-acento-plano { color: var(--pp-acc); }
  .tdp-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .tdp-parrafo--tinta { color: var(--pp-ink); }
  .tdp-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .tdp-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .tdp-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .tdp-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .tdp-std-dia { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-weight: 500;
  ...
  .tdp-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .tdp-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .tdp-std-anio { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .tdp-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .tdp-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .tdp-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .tdp-foto-capa { position: absolute; inset: 0; }
  .tdp-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .tdp-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .tdp-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .tdp-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .tdp-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .tdp-cuenta-caja--1 { --tdp-giro: -1.6deg; transform: rotate(var(--tdp-giro)); }
  .tdp-cuenta-caja--2 { --tdp-giro: 1.2deg; transform: rotate(var(--tdp-giro)); margin-top: 10px; }
  .tdp-cuenta-caja--3 { --tdp-giro: 1.8deg; transform: rotate(var(--tdp-giro)); }
  .tdp-cuenta-caja--4 { --tdp-giro: -1.1deg; transform: rotate(var(--tdp-giro)); margin-top: 10px; }
  .tdp-cuenta-num { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .tdp-cuenta-num--acc { color: var(--pp-acc); }
  .tdp-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .tdp-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .tdp-frase { position: relative; z-index: 1; margin: 0; font-family: var(--tdp-serif), 'Caveat Brush', cursive;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .tdp-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .tdp-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .tdp-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .tdp-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  .tdp-tira { position: absolute; top: 0; left: 0; height: 100%; display: flex; will-change: transform; }
  .tdp-panel { flex: 0 0 100vw; min-width: 0; height: 100%; box-sizing: border-box; position: relative; overflow: hidden;
    display: flex; flex-direction: column; gap: 18px; padding: 80px max(24px, calc((100vw - 560px) / 2)) 96px; }
  .tdp-panel--abajo { justify-content: flex-end; }
  .tdp-panel--centro { justify-content: center; }
  .tdp-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  /* Apilado: los mismos paneles, uno abajo del otro, recorridos bajando. */
  .tdp-pan[data-scroll="vertical"] { height: auto; }
  .tdp-pan[data-scroll="vertical"] .tdp-pan-fijo { position: static; height: auto; overflow: visible; }
  .tdp-pan[data-scroll="vertical"] .tdp-tira { position: static; display: block; width: 100%; transform: none !important; }
  .tdp-pan[data-scroll="vertical"] .tdp-panel { height: auto; min-height: calc(var(--vh, 1vh) * 100); }
  .tdp-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .tdp-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .tdp-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .tdp-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .tdp-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .tdp-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .tdp-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .tdp-tarjeta--izq { --tdp-giro: -1deg; transform: rotate(var(--tdp-giro)); }
  .tdp-tarjeta--der { --tdp-giro: 1.1deg; transform: rotate(var(--tdp-giro)); }
  .tdp-tarjeta--talon { --tdp-giro: -.8deg; transform: rotate(var(--tdp-giro)); overflow: visible; }
  .tdp-tarjeta--hoy { --tdp-giro: -1deg; transform: rotate(var(--tdp-giro)); align-items: flex-start; }
  .tdp-tarjeta--banco { margin-top: 4px; }
  .tdp-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .tdp-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .tdp-tarjeta-titulo { margin: 0; font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .tdp-tarjeta-pregunta { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: 26px; line-height: 1.15; }
  .tdp-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .tdp-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .tdp-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .tdp-fila > span:first-child { color: var(--pp-ink2); }
  .tdp-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .tdp-fila-valor { text-align: right; }
  .tdp-fila--copiable { align-items: center; }
  .tdp-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .tdp-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .tdp-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .tdp-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .tdp-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .tdp-checkin { background: var(--pp-bg); }
  .tdp-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .tdp-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .tdp-campo { display: flex; flex-direction: column; gap: 6px; }
  .tdp-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .tdp-contador { display: flex; align-items: center; gap: 12px; }
  .tdp-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .tdp-contador button:disabled { opacity: .35; cursor: default; }
  .tdp-contador > span { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .tdp-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--tdp-sans), 'Karla', sans-serif; font-size: 16px; padding: 6px 2px; }
  .tdp-input--serif { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: 20px; }
  .tdp-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .tdp-input::placeholder { color: #9A9388; }
  .tdp-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .tdp-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .tdp-precio-total { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .tdp-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .tdp-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .tdp-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .tdp-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .tdp-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--tdp-sans), 'Karla', sans-serif; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .tdp-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .tdp-btn-solido:disabled { opacity: .6; cursor: default; }
  .tdp-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .tdp-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --tdp-giro: -1deg; transform: rotate(var(--tdp-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .tdp-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .tdp-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .tdp-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .tdp-crono-hora { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: 26px; min-width: 64px; line-height: 1; }
  .tdp-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .tdp-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .tdp-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .tdp-polaroids[data-cantidad="1"] .tdp-polaroid { grid-column: span 6; }
  .tdp-polaroids[data-cantidad="2"] .tdp-polaroid { grid-column: span 3; }
  .tdp-polaroids[data-cantidad="4"] .tdp-polaroid { grid-column: span 3; }
  .tdp-polaroids[data-cantidad="5"] .tdp-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .tdp-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .tdp-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .tdp-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .tdp-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .tdp-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .tdp-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .tdp-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .tdp-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .tdp-lista-tema { font-size: 15px; }
  .tdp-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .tdp-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .tdp-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .tdp-opciones { display: flex; flex-direction: column; gap: 8px; }
  .tdp-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--tdp-sans), 'Karla', sans-serif; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .tdp-opcion:disabled { cursor: default; }
  .tdp-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .tdp-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .tdp-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .tdp-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --tdp-giro: -.6deg; transform: rotate(var(--tdp-giro));
    border-radius: 12px; overflow: hidden; }
  .tdp-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .tdp-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .tdp-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .tdp-ticket-nombre { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .tdp-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .tdp-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .tdp-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .tdp-ticket-muesca--arriba { top: -12px; }
  .tdp-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .tdp-ticket-der .qr-ingreso, .tdp-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .tdp-info-extra { position: relative; z-index: 1; }
  .tdp-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .tdp-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .tdp-raiz .ia-icon-box { display: none !important; }
  .tdp-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .tdp-pase-firma { font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .tdp-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .tdp-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }

  /* ── Riel ──────────────────────────────────────────────────────────── */
  .tdp-riel { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column;
  ...
  .tdp-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .tdp-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  .tdp-riel-barra { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: var(--pp-acc); transition: height 260ms linear; display: block; }
  .tdp-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .tdp-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .tdp-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .tdp-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .tdp-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .tdp-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .tdp-cartel-nombres { margin: 0; font-family: var(--tdp-serif), 'Caveat Brush', cursive; font-weight: 500;
  ...
  .tdp-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .tdp-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .tdp-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .tdp-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .tdp-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .tdp-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .tdp-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .tdp-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--tdp-sans), 'Karla', sans-serif; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .tdp-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }

  .tdp-pista { position: absolute; left: 0; right: 34px; bottom: calc(18px + env(safe-area-inset-bottom)); z-index: 6; text-align: center;
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .tdp-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .tdp-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .tdp-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .tdp-section, .tdp-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .tdp-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }

  @media (prefers-reduced-motion: reduce) {
    .tdp-raiz * { animation: none !important; }
    .tdp-scroller [data-xin] { opacity: 1 !important; --tdp-y: 0px; }
  }
`;
