/**
 * LuzLunaTemplateMedianocheAzul.tsx
 ...
 *   --t-bg #0F1420 · --t-surface #182036 · --luzluna-ink #E4E9F5
 *   --t-muted #8FA0C4 · --t-acc #7EA3D9 (lavanda) · --t-acc2 #4A5FA0 (violeta profundo)
 *
...
interface LuzLunaTemplateMedianocheAzulProps {
...
export function LuzLunaTemplateMedianocheAzul({ invitation, guest, isPersonalized = false }: LuzLunaTemplateMedianocheAzulProps) {
  ...
  // Portada animada -- mismo criterio que la base de LuzLuna: tinte +
  // effect="shimmer", valores literales propios de esta variante (no var(),
  // AnimatedCoverPhoto no lo acepta), scrimColorRgb = rgb del propio --t-bg.
  const portadaImagenFondoDesktopRaw = String(invitation.portadaImagenFondoDesktop ?? "") || undefined;
  const portadaFondoAnimado = Boolean(portadaImagenFondoDesktopRaw);
  const portadaTintColor1 = "#7EA3D9";
  const portadaTintColor2 = "#4A5FA0";
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #0A0816 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #E4E9F5)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #E4E9F5)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#7EA3D9" surfaceColor="#182036" inactiveColor="#E4E9F5" solid />}
      </div>
  );
}
