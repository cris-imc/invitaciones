...
interface AniversarioTemplateQuizQuestion {
  pregunta: string;
  opciones: string[];
...
interface AniversarioTemplateProps {
...
export function AniversarioTemplate({ invitation, guest, isPersonalized = false }: AniversarioTemplateProps) {
  const tx = useTextos();
  ...
  const triviaPreguntas: AniversarioTemplateQuizQuestion[] = safeJson<AniversarioTemplateQuizQuestion[]>(String(invitation.triviaPreguntas ?? ""), []);
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F6E6DC";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C48A6E";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #C48A6E";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#7A4E36" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#7A4E36" : "#C48A6E";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#130C09", tinta: "#F4F1EA", acento: "#C48A6E", clase: `${anvDisplay.variable} ${anvMono.variable}`, fuente: "var(--anv-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="anv-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="anv-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-drift="-70" className="anv-medallion anv-medallion--corner">
            <AniversarioTemplateMedallionCmp label="20" sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="anvArc1" arcText={tx("invitacion.sabor.arcoAniversario").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="anv-section anv-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2E1E14 0%, #0D0B10 55%, #130C09 100%)" }}>
          ...
          <div className="anv-cd-grid">
            <AniversarioTemplateCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <AniversarioTemplateCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <AniversarioTemplateCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <AniversarioTemplateCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="anv-pan">
          <div className="anv-pan-sticky">
            <div data-strip="1" className="anv-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="anv-panel anv-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="anv-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#7A4E36" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#7A4E36" />
                  </svg>
              ...
              <div data-tone="dark" className="anv-panel anv-panel--center" style={{ background: "#180F0C", color: "#F4F1EA" }}>
                <div className="anv-medallion anv-medallion--lg">
                  <AniversarioTemplateMedallionCmp label={dressCode ? dressCode.toUpperCase() : tx("invitacion.pase.acceso").toUpperCase()} sub={tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()} arcId="anvArc2" arcText={tx("invitacion.pase.arcoAccesoVipPase", { n: passNumber }).toUpperCase()} spin="reverse" title={tx("invitacion.pase.reservado")} />
                </div>
            ...
            <AniversarioTemplateDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="anv-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="anv-pan-sticky">
            ...
            <AniversarioTemplateDots count={photoPages.length} />
          </div>
        </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="anv-section" style={{ background: "#180F0C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="anv-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="anv-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F6E6DC" : "#C48A6E" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="anv-song-wrap">
              <AniversarioTemplateSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="anv-bank-wrap">
              {pagoTarjetaHabilitado && (
                <BankDetailsCard
                  ...
                  InfoRow={AniversarioTemplateInfoRow}
                  CopyField={AniversarioTemplateCopyField}
                />
              )}
              {regaloHabilitado && (
                <BankDetailsCard
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="anv-section anv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="anv-final-card">
            <div className="anv-medallion anv-medallion--final">
              <AniversarioTemplateMedallionCmp label="20" sub={confirmed ? "CONFIRMADO" : tx("invitacion.pase.pendiente").toUpperCase()} arcId="anvArc3" arcText={textoArco(namesTitle, fechaCorta)} spin="reverse" />
            </div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C48A6E" guest={guest} />
      )}
...
function AniversarioTemplateCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
...
function AniversarioTemplateDots({ count = 3 }: { count?: number }) {
...
function AniversarioTemplateMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
...
function AniversarioTemplateCopyField({ label, value }: { label: string; value: string }) {
  const tx = useTextos();
...
function AniversarioTemplateRsvpCard({
  invitationId,
  guestToken,
  guestName,
  maxAdults,
    ...
    <>
      ...
      <div ref={stubRef} className="anv-stub">
        ...
        <div ref={sealRef} className="anv-seal">
          <AniversarioTemplateMedallionCmp label="20" sub="" arcId="" arcText="" spin="none" compact />
        </div>
...
function AniversarioTemplateSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
...
function AniversarioTemplateQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: AniversarioTemplateQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  const tx = useTextos();
...
function AniversarioTemplateInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="anv-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="anv-bank-row-label">{label}</span>
...
function AniversarioTemplateCoverHalf({
  namesRef,
  kickerRef,
  perfRef,
  kickerText,
...
const GP_CSS = `
  ...
  a { color: #C48A6E; text-decoration: none; }
  a:hover { color: #F6E6DC; }
  ...
  .anv-date-month { font-family: var(--anv-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C48A6E; padding-left: 12%; }

  .anv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .anv-divider-line { width: 52px; height: 1px; background: #C48A6E; display: inline-block; }
  ...
  .anv-cal-link:hover { color: #C48A6E; }
  ...
  .anv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #C48A6E, #F6E6DC, #6E5AA8); filter: saturate(.75); }
  ...
  .anv-medallion-label { font-family: var(--anv-display), serif; font-size: 26px; line-height: 1; color: #F6E6DC; }
  .anv-medallion-label-sm { font-family: var(--anv-display), serif; font-size: 16px; color: #F6E6DC; }
  .anv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .anv-medallion-sub--accent { color: #C48A6E; }
  ...
  .anv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C48A6E; opacity: 0.4; font-family: var(--anv-mono), monospace; }
  ...
  .anv-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F6E6DC, transparent); animation: anvScan 6s linear infinite; pointer-events: none; }
  ...
  .anv-cd-num { font-family: var(--anv-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6E6DC; display: inline-block; }
  .anv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C48A6E; }
  ...
  .anv-accent-italic { font-style: italic; color: #C48A6E; }
  .anv-accent-serif { font-style: italic; color: #7A4E36; font-family: var(--anv-display), serif; }
  .anv-accent-serif-2 { color: #7A4E36; }
  ...
  .anv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #7A4E36; margin-top: auto; }
  ...
  .anv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #7A4E36; }
  ...
  .anv-crono-time { font-family: var(--anv-mono), monospace; color: #7A4E36; min-width: 42px; }
  ...
  .anv-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F6E6DC, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .anv-rsvp-payment-total { color: #F6E6DC; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .anv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C48A6E; background: transparent; color: #C48A6E; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .anv-rsvp-input:focus { outline: none; border-bottom-color: #C48A6E; }
  .anv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--anv-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C48A6E, #7A4E36); border: 1px solid #C48A6E; color: #180F0C; cursor: pointer; }
  .anv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .anv-rsvp-btn--ghost { background: transparent; color: #C48A6E; }
  .anv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C48A6E; margin: 0; }
  ...
  .anv-bank-copy { flex-shrink: 0; font-family: var(--anv-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C48A6E; background: transparent; color: #C48A6E; cursor: pointer; }
  ...
  .anv-song-input:focus { outline: none; border-bottom-color: #C48A6E; }
  .anv-song-sep { color: #8A8577; flex-shrink: 0; }
  .anv-song-submit { flex-shrink: 0; background: none; border: none; color: #C48A6E; font-family: var(--anv-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .anv-song-submit:hover { color: #F6E6DC; }
  .anv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .anv-song-error { font-size: 10px; color: #C48A6E; margin-top: 6px; }
  ...
  .anv-quiz-opt { font-family: var(--anv-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #C48A6E; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .anv-quiz-opt:disabled { cursor: default; }
  .anv-quiz-opt--picked { background: #C48A6E; border-color: #C48A6E; color: #180F0C; }
  .anv-quiz-opt--correct { background: #C48A6E; border-color: #C48A6E; color: #180F0C; }
  ...
  .anv-quiz-result-score { margin: 0 0 6px; font-family: var(--anv-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F6E6DC; }
  .anv-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .anv-final-card { border: 1px solid #C48A6E; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .anv-final-names { font-family: var(--anv-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F6E6DC; }
  ...
  .anv-replay { cursor: pointer; color: #C48A6E; }
  ...
  .anv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F6E6DC, #C48A6E); transition: height 260ms linear; display: block; }
  .anv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C48A6E; transition: color 500ms ease; }
  ...
  .anv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(90,58,68,0.4), rgba(196,138,110,0.32), rgba(196,138,110,0.55), rgba(90,58,68,0.4)); filter: blur(64px); opacity: .62; animation: anvFoil 26s linear infinite; }
  .anv-cover-texture { position: absolute; inset: 0; background-image: radial-gradient(circle at 70% 30%, rgba(246,230,220,.14), transparent 45%); }
  ...
  .anv-cover-names { margin: 0; font-family: var(--anv-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F6E6DC; }
  .anv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C48A6E, transparent); display: block; }
  ...
  .anv-cover-cta { border: 1px solid #C48A6E; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--anv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .anv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C48A6E; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
