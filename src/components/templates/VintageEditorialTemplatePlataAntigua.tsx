...
export function VintageEditorialTemplatePlataAntigua({ invitation, guest, isPersonalized = false }: VintageEditorialTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E8EAED";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#AAB1BB";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(170,177,187,.35), 0 18px 50px -30px #AAB1BB";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#5B6570" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(170,177,187,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(170,177,187,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#5B6570" : "#AAB1BB";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#101113", tinta: "#F4F1EA", acento: "#AAB1BB", clase: `${vtePlayfair.variable} ${vteMono.variable}`, fuente: "var(--vte-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="vte-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="vte-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #161A1F 0%, #181A1E 55%, #101113 100%)" }}>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="vte-section vte-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #23262D 0%, #131417 55%, #101113 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="vte-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #191B20 0%, #0E0F11 52%, #101113 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="vte-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="vte-pan-sticky">
            <div data-strip="1" className="vte-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="vte-panel vte-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="vte-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#5B6570" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#5B6570" />
                  </svg>
              ...
              <div data-tone="dark" className="vte-panel vte-panel--center" style={{ background: "#181A1E", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="vte-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #161A1F 0%, #181A1E 60%, #101113 100%)" }}>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="vte-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="vte-pan-sticky">
            <div data-strip="1" className="vte-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="vte-panel vte-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="vte-seguir vte-seguir--split">
                    ...
                    <span className="vte-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="vte-section" style={{ background: "#181A1E" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="vte-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="vte-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E8EAED" : "#AAB1BB" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="vte-section" style={{ background: "#181A1E" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="vte-section" style={{ background: "#181A1E" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.sabor.tuEdicion")} className="vte-section vte-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #161A1F 0%, #181A1E 55%, #101113 100%)" }}>
      ...
      <div ref={coverRef} className="vte-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#AAB1BB" guest={guest} />
      )}
...
const VTE_CSS = `
  ...
  a { color: #AAB1BB; text-decoration: none; }
  a:hover { color: #E8EAED; }
  ...
  .vte-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #101113; }
  ...
  @media (min-width: 768px) {
    .vte-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(170,177,187,.3); }
  ...
  .vte-date-month { font-family: var(--vte-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #AAB1BB; padding-left: 12%; }

  .vte-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .vte-divider-line { width: 52px; height: 1px; background: #AAB1BB; display: inline-block; }
  ...
  .vte-cal-link:hover { color: #AAB1BB; }
  ...
  .vte-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #5B6570, #2C2F36, #AAB1BB, #E8EAED, #5B6570); filter: saturate(.75); }
  .vte-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0E0E10; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .vte-medallion-label { font-family: var(--vte-playfair), serif; font-size: 26px; line-height: 1; color: #E8EAED; }
  .vte-medallion-label-sm { font-family: var(--vte-playfair), serif; font-size: 16px; color: #E8EAED; }
  .vte-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .vte-medallion-sub--accent { color: #AAB1BB; }
  ...
  .vte-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #AAB1BB; opacity: 0.4; font-family: var(--vte-mono), monospace; }

  .vte-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(170,177,187,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .vte-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E8EAED, transparent); animation: vteScan 6s linear infinite; pointer-events: none; }

  .vte-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .vte-cd-box { border: 1px solid #262930; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .vte-cd-num { font-family: var(--vte-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F3F4F6; display: inline-block; }
  .vte-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #AAB1BB; }
  .vte-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #101113 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .vte-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(91,101,112,0.28), rgba(44,47,54,0.3), rgba(170,177,187,0.32), rgba(91,101,112,0.28)); filter: blur(80px); opacity: .4; animation: vteFoil 30s linear infinite; }
  ...
  .vte-accent-italic { font-style: italic; color: #AAB1BB; }
  .vte-accent-serif { font-style: italic; color: #5B6570; font-family: var(--vte-playfair), serif; }
  .vte-accent-serif-2 { color: #5B6570; }
  .vte-h2 { margin: 0; font-family: var(--vte-playfair), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .vte-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .vte-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #5B6570; margin-top: auto; }
  ...
  .vte-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #5B6570; }
  ...
  .vte-crono-time { font-family: var(--vte-mono), monospace; color: #5B6570; min-width: 42px; }
  ...
  .vte-stub { position: relative; overflow: hidden; border: 1px solid #262930; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .vte-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E8EAED, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .vte-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(170,177,187,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .vte-rsvp-payment-total { color: #E8EAED; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .vte-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #AAB1BB; background: transparent; color: #AAB1BB; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .vte-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(170,177,187,0.3); color: #F4F1EA; font-family: var(--vte-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .vte-rsvp-input::placeholder { color: #6E6A5D; }
  .vte-rsvp-input:focus { outline: none; border-bottom-color: #AAB1BB; }
  .vte-rsvp-btn { width: 100%; padding: 16px; font-family: var(--vte-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #AAB1BB, #767E88); border: 1px solid #AAB1BB; color: #0B0B10; cursor: pointer; }
  .vte-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .vte-rsvp-btn--ghost { background: transparent; color: #AAB1BB; }
  .vte-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #AAB1BB; margin: 0; }
  ...
  .vte-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(170,177,187,0.18); }
  ...
  .vte-bank-copy { flex-shrink: 0; font-family: var(--vte-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #AAB1BB; background: transparent; color: #AAB1BB; cursor: pointer; }
  .vte-bank-copy:hover { background: rgba(170,177,187,0.12); }
  ...
  .vte-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(170,177,187,0.3); padding-bottom: 12px; }
  ...
  .vte-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(170,177,187,0.3); color: #F4F1EA; font-family: var(--vte-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .vte-song-input::placeholder { color: #6E6A5D; }
  .vte-song-input:focus { outline: none; border-bottom-color: #AAB1BB; }
  .vte-song-sep { color: #8A8577; flex-shrink: 0; }
  .vte-song-submit { flex-shrink: 0; background: none; border: none; color: #AAB1BB; font-family: var(--vte-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .vte-song-submit:hover { color: #E8EAED; }
  .vte-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .vte-song-error { font-size: 10px; color: #AAB1BB; margin-top: 6px; }
  ...
  .vte-quiz-opt { font-family: var(--vte-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(170,177,187,0.4); background: transparent; color: #AAB1BB; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .vte-quiz-opt:disabled { cursor: default; }
  .vte-quiz-opt--picked { background: #AAB1BB; border-color: #AAB1BB; color: #0B0B10; }
  .vte-quiz-opt--correct { background: #AAB1BB; border-color: #AAB1BB; color: #0B0B10; }
  .vte-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .vte-quiz-result { padding-top: 18px; border-top: 1px solid rgba(170,177,187,0.2); }
  .vte-quiz-result-score { margin: 0 0 6px; font-family: var(--vte-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E8EAED; }
  .vte-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .vte-final-card { border: 1px solid #AAB1BB; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .vte-final-names { font-family: var(--vte-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E8EAED; }
  ...
  .vte-replay { cursor: pointer; color: #AAB1BB; }
  .vte-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .vte-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(170,177,187,0.14); }
  .vte-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .vte-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(170,177,187,0.16); position: relative; }
  .vte-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E8EAED, #AAB1BB); transition: height 260ms linear; display: block; }
  .vte-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #AAB1BB; transition: color 500ms ease; }
  ...
  .vte-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #202329 0%, #181A1E 46%, #101113 100%); }
  .vte-cover-half--bottom .vte-cover-inner { top: auto; bottom: 0; }
  .vte-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(91,101,112,0.4), rgba(44,47,54,0.5), rgba(170,177,187,0.5), rgba(91,101,112,0.4)); filter: blur(64px); opacity: .62; animation: vteFoil 26s linear infinite; }
  .vte-cover-sunburst { position: absolute; inset: 0; background-image: radial-gradient(rgba(232,234,237,.07) .8px, transparent .8px); background-size: 3px 3px; }
  ...
  .vte-cover-names { margin: 0; font-family: var(--vte-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E8EAED; }
  .vte-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#AAB1BB, transparent); display: block; }
  ...
  .vte-cover-cta { border: 1px solid #AAB1BB; background: linear-gradient(100deg, rgba(170,177,187,0.08), rgba(232,234,237,0.2), rgba(170,177,187,0.08)); color: #F3F4F6; font-family: var(--vte-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .vte-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .vte-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,234,237,0.34), rgba(243,244,246,0.5), rgba(232,234,237,0.34)); color: #0B0B0F; }
  }
  ...
  .vte-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #AAB1BB; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
