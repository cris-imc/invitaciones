...
/**
 * PapeleriaDeHotelDeLujoTemplateGrisCarbonOroBlanco.tsx
 *
...
export function PapeleriaDeHotelDeLujoTemplateGrisCarbonOroBlanco({ invitation, guest, isPersonalized = false }: PapeleriaDeHotelDeLujoTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#EDE8D8";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C9C2AA";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(201,194,170,.35), 0 18px 50px -30px #C9C2AA";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#6B6550" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(201,194,170,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(201,194,170,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#6B6550" : "#C9C2AA";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#101113", tinta: "#F4F1EA", acento: "#C9C2AA", clase: `${phlPlayfair.variable} ${phlMono.variable}`, fuente: "var(--phl-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="phl-scroller">
        <section data-tone="dark" data-screen-label="Save the Date" className="phl-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #232529 0%, #1B1D20 55%, #101113 100%)" }}>
            ...
            <div className="phl-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="16,17,19" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="16,17,19" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="phl-section phl-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #1A1C1F 0%, #141517 55%, #101113 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="phl-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #26272B 0%, #0C0B11 52%, #101113 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="phl-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="phl-pan-sticky">
            <div data-strip="1" className="phl-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="phl-panel phl-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="phl-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#6B6550" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#6B6550" />
                  </svg>
              ...
              <div data-tone="dark" className="phl-panel phl-panel--center" style={{ background: "#1B1D20", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="phl-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #232529 0%, #1B1D20 60%, #101113 100%)" }}>
            ...
            <div data-strip="1" className="phl-strip">
              ...
              <div data-tone="light" className="phl-panel phl-panel--gap" style={{ background: "#EDE8DE", color: "#14141B" }}>
                ...
                <div className="phl-album-embed">
                  {livePhotos.length > 0 ? (
                    <LiveAlbumStrip photos={livePhotos} tone="light" accentColor="#6B6550" />
                  ) : (
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="phl-section" style={{ background: "#1B1D20" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="phl-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="phl-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#EDE8D8" : "#C9C2AA" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="phl-section" style={{ background: "#1B1D20" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="phl-section" style={{ background: "#1B1D20" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu reserva" className="phl-section phl-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #232529 0%, #1B1D20 55%, #101113 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C9C2AA" guest={guest} />
      )}
...
const PHL_CSS = `
  ...
  a { color: #C9C2AA; text-decoration: none; }
  a:hover { color: #EDE8D8; }
  ...
  .phl-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #101113; }
  ...
  @media (min-width: 768px) {
    .phl-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(201,194,170,.3); }
  ...
  .phl-date-month { font-family: var(--phl-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C9C2AA; padding-left: 12%; }

  .phl-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .phl-divider-line { width: 52px; height: 1px; background: #C9C2AA; display: inline-block; }
  ...
  .phl-cal-link:hover { color: #C9C2AA; }
  ...
  .phl-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2E3236, #454A50, #C9C2AA, #EDE8D8, #2E3236); filter: saturate(.75); }
  ...
  .phl-medallion-label { font-family: var(--phl-playfair), serif; font-size: 26px; line-height: 1; color: #EDE8D8; }
  .phl-medallion-label-sm { font-family: var(--phl-playfair), serif; font-size: 16px; color: #EDE8D8; }
  .phl-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .phl-medallion-sub--accent { color: #C9C2AA; }
  ...
  .phl-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C9C2AA; opacity: 0.4; font-family: var(--phl-mono), monospace; }

  .phl-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(201,194,170,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .phl-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #EDE8D8, transparent); animation: phlScan 6s linear infinite; pointer-events: none; }

  .phl-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .phl-cd-box { border: 1px solid #26241D; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .phl-cd-num { font-family: var(--phl-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F5F1E6; display: inline-block; }
  .phl-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C9C2AA; }
  .phl-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #101113 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .phl-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(46,50,54,0.5), rgba(69,74,80,0.3), rgba(201,194,170,0.4), rgba(46,50,54,0.5)); filter: blur(80px); opacity: .4; animation: phlFoil 30s linear infinite; }
  ...
  .phl-accent-italic { font-style: italic; color: #C9C2AA; }
  .phl-accent-serif { font-style: italic; color: #6B6550; font-family: var(--phl-playfair), serif; }
  .phl-accent-serif-2 { color: #6B6550; }
  ...
  .phl-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #6B6550; margin-top: auto; }
  ...
  .phl-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #6B6550; }
  ...
  .phl-crono-time { font-family: var(--phl-mono), monospace; color: #6B6550; min-width: 42px; }
  ...
  .phl-stub { position: relative; overflow: hidden; border: 1px solid #26241D; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .phl-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #EDE8D8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .phl-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(201,194,170,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .phl-rsvp-payment-total { color: #EDE8D8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .phl-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C9C2AA; background: transparent; color: #C9C2AA; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .phl-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(201,194,170,0.3); color: #F4F1EA; font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .phl-rsvp-input::placeholder { color: #6E6A5D; }
  .phl-rsvp-input:focus { outline: none; border-bottom-color: #C9C2AA; }
  .phl-rsvp-btn { width: 100%; padding: 16px; font-family: var(--phl-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C9C2AA, #A39B7E); border: 1px solid #C9C2AA; color: #0B0B10; cursor: pointer; }
  .phl-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .phl-rsvp-btn--ghost { background: transparent; color: #C9C2AA; }
  .phl-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #C9C2AA; margin: 0; }
  ...
  .phl-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(201,194,170,0.18); }
  ...
  .phl-bank-copy { flex-shrink: 0; font-family: var(--phl-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C9C2AA; background: transparent; color: #C9C2AA; cursor: pointer; }
  .phl-bank-copy:hover { background: rgba(201,194,170,0.12); }
  ...
  .phl-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(201,194,170,0.3); padding-bottom: 12px; }
  ...
  .phl-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(201,194,170,0.3); color: #F4F1EA; font-family: var(--phl-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .phl-song-input::placeholder { color: #6E6A5D; }
  .phl-song-input:focus { outline: none; border-bottom-color: #C9C2AA; }
  .phl-song-sep { color: #8A8577; flex-shrink: 0; }
  .phl-song-submit { flex-shrink: 0; background: none; border: none; color: #C9C2AA; font-family: var(--phl-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .phl-song-submit:hover { color: #EDE8D8; }
  .phl-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .phl-song-error { font-size: 10px; color: #C9C2AA; margin-top: 6px; }
  ...
  .phl-quiz-opt { font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(201,194,170,0.4); background: transparent; color: #C9C2AA; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .phl-quiz-opt:disabled { cursor: default; }
  .phl-quiz-opt--picked { background: #C9C2AA; border-color: #C9C2AA; color: #0B0B10; }
  .phl-quiz-opt--correct { background: #C9C2AA; border-color: #C9C2AA; color: #0B0B10; }
  .phl-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .phl-quiz-result { padding-top: 18px; border-top: 1px solid rgba(201,194,170,0.2); }
  .phl-quiz-result-score { margin: 0 0 6px; font-family: var(--phl-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #EDE8D8; }
  .phl-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .phl-final-card { border: 1px solid #C9C2AA; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .phl-final-names { font-family: var(--phl-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #EDE8D8; }
  ...
  .phl-replay { cursor: pointer; color: #C9C2AA; }
  .phl-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .phl-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(201,194,170,0.14); }
  .phl-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .phl-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(201,194,170,0.16); position: relative; }
  .phl-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#EDE8D8, #C9C2AA); transition: height 260ms linear; display: block; }
  .phl-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C9C2AA; transition: color 500ms ease; }
  ...
  .phl-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #1D1F22 0%, #1B1D20 46%, #101113 100%); }
  .phl-cover-half--bottom .phl-cover-inner { top: auto; bottom: 0; }
  .phl-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(46,50,54,0.7), rgba(69,74,80,0.4), rgba(201,194,170,0.6), rgba(46,50,54,0.7)); filter: blur(64px); opacity: .62; animation: phlFoil 26s linear infinite; }
  .phl-cover-sunburst { position: absolute; inset: 0; background-image: repeating-conic-gradient(from 0deg at 50% 50%, rgba(201,194,170,.05) 0deg 1deg, transparent 1deg 3deg); }
  ...
  .phl-cover-names { margin: 0; font-family: var(--phl-playfair), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EDE8D8; }
  .phl-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C9C2AA, transparent); display: block; }
  ...
  .phl-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(16,17,19,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .phl-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C9C2AA; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
