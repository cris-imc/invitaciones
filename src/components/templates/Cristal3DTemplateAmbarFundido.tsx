/**
 * Cristal3DTemplateAmbarFundido.tsx
 ...
 * etc). Cambia la capa visual a la estética "Cristal 3D" del pack Tech 15
 * años (mockup/nuevo/Plantillas 15 Años Tech.dc.html, tarjeta Elena/Nina):
 * fondo azul-negro casi opaco (#160F08) + acentos celeste (#FFB347) y
 * lavanda (#FF6B9D), tarjetas con tilt 3D ambiente continuo y capas con
 * sombras desfasadas (sensación de profundidad/vidrio apilado), tipografía
 * refinada (Outfit para títulos + Manrope para cuerpo + Space Mono para
 * kickers), doodles de trazo fino propios (faceta/diamante, esquirla de
 * cristal) y un barrido de refracción prismática sobre la foto de portada.
...
// decorativo (diamante con caras, esquirla de vidrio).
...
// Faceta de diamante doodle -- contorno de gema tallada con líneas de corte
// internas (caras del cristal). Decorativa, usada en portada/hero.
const IconFacet = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 32 34" fill="none" stroke="currentColor" strokeWidth={1.1} className={className} style={style} aria-hidden="true">
    <path d="M6 12 16 2l10 10-4.5 20h-11Z" />
    <path d="M6 12h20M9.5 12 16 2M22.5 12 16 2M11.5 32 16 12l4.5 20" strokeWidth={0.8} opacity={0.7} />
  </svg>
);
// Esquirla de cristal doodle -- rombo alargado de trazo fino con una faceta
// interna, como una astilla desprendida. Acompaña títulos y CTAs.
const IconShard = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 16 26" fill="none" stroke="currentColor" strokeWidth={1.2} className={className} style={style} aria-hidden="true">
    <path d="M8 1 14 11 8 25 2 11Z" />
    <path d="M4.5 11h7" strokeWidth={0.8} opacity={0.7} />
  </svg>
);

// Marco de capas de vidrio apiladas alrededor de la foto de portada/hero:
// tres bordes finos con inset creciente y opacidad decreciente (sensación
// de profundidad) -- perímetro completo, no solo esquinas -- con un doodle
// de esquirla en la esquina.
const PhotoCornerFrame = () => (
  <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
    <div className="absolute inset-0" style={{ border: "1px solid rgba(143,211,255,0.6)" }} />
    <div className="absolute" style={{ inset: 5, border: "1px solid rgba(185,166,255,0.35)" }} />
    <div className="absolute" style={{ inset: 10, border: "1px solid rgba(240,246,255,0.18)" }} />
    <IconShard style={{ position: "absolute", bottom: -10, right: -6, width: 14, height: 22, color: "#FF6B9D" }} />
  </div>
);

// Tipografía exacta del mockup "Cristal 3D" (Outfit para títulos display +
// Manrope para todo el cuerpo de texto + Space Mono para cifras/kickers),
...
interface Cristal3DTemplateAmbarFundidoProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#FFB347]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#FFB347] uppercase tracking-wider mb-0.5">{label}</span>
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        ...
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#FFF6E8" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#B8A68F" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#FFB347] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#FFF6E8" }}>
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#FFF6E8] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function Cristal3DTemplateAmbarFundido({ invitation, guest, isPersonalized = false }: Cristal3DTemplateAmbarFundidoProps) {
  const tx = useTextos();
  ...
  // Entrada animada de los doodles de la portada (facetas de diamante,
  // esquirlas de cristal) y pulso del sello con anime.js.
  ...
  useEffect(() => {
    ...
    animate(root.querySelectorAll(".crys-doodle"), {
      scale: [0, 1],
      rotate: [-25, 0],
      opacity: [0, 1],
      duration: 900,
    ...
    animate(root.querySelectorAll(".crys-seal"), {
      scale: [0.6, 1],
      opacity: [0, 1],
      duration: 700,
      delay: 150,
  ...
  useEffect(() => {
    ...
    const els = document.querySelectorAll(".crys-scroll-doodle");
  ...
  // Barrido de refracción prismática sobre la foto de portada, ligado al
  // progreso de scroll (no dispara una vez y listo -- va "scrubando" con el
  // scroll real, hacia adelante y atrás) usando ScrollObserver de anime.js.
  ...
  // Portada animada -- Cristal3D es tema oscuro con paleta cian/violeta
  // (facetas de vidrio) -> tinte con sus dos acentos propios + effect=
  // "shimmer" (glint/destello, coherente con la estética de cristal, más
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#160F08] via-[#241A0C] to-[#160F08] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
  ...
  return (
    <div
      className={`${crysDisplay.variable} ${crysBody.variable} ${crysMono.variable}`}
      style={{
    ...
    >
      <style>{`
        .desktop-stage .tpl .font-mono, .tpl .font-mono {
          font-family: var(--crys-mono), monospace !important;
        }
        /* Texto con degradé animado (blanco→celeste→lavanda→blanco), como
           luz atravesando facetas de cristal. */
        .crys-gradient-text {
          background: linear-gradient(120deg, #FFF6E8, #FFB347, #FF6B9D, #FFF6E8);
          background-size: 300% 100%;
          -webkit-background-clip: text;
          background-clip: text;
          color: transparent;
          animation: crys-shimmer 7s ease-in-out infinite;
        }
        @keyframes crys-shimmer {
        ...
        /* A diferencia de Moderno/Neon, Cristal 3D es vidrio con capas
        ...
        /* Tilt 3D ambiente continuo -- oscilación sutil e infinita de
           perspectiva/rotación, aplicada a tarjetas de vidrio (Ceremonia,
           Fiesta, countdown). Cada tarjeta puede desfasar el delay para que
           no se muevan todas en sincro (sensación más orgánica). */
        .crys-tilt-card {
          transform-style: preserve-3d;
          animation: crys-ambientTilt 7s ease-in-out infinite;
          will-change: transform;
        }
        @keyframes crys-ambientTilt {
          0%, 100% { transform: perspective(900px) rotateX(0deg) rotateY(0deg) translateZ(0); }
          25% { transform: perspective(900px) rotateX(1.4deg) rotateY(-1.6deg) translateZ(4px); }
          50% { transform: perspective(900px) rotateX(-1deg) rotateY(1.2deg) translateZ(0); }
          75% { transform: perspective(900px) rotateX(0.8deg) rotateY(1.8deg) translateZ(4px); }
        }

        /* Override Countdown Hardcoded Colors */
        #countdown.dark {
          background-color: #160F08 !important;
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          box-shadow: 0 6px 18px rgba(10,14,22,0.4), inset 0 1px 0 rgba(255,255,255,0.06) !important;
          transform-style: preserve-3d;
          animation: crys-ambientTilt 7s ease-in-out infinite;
        }

        /* RSVP Custom Aesthetics for Cristal3DTemplateAmbarFundido */
        #rsvp.section.dark {
          background-color: #160F08 !important;
          color: #FFFFFF !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #FFB347 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #160F08 !important; /* Matches light sections */
          color: #FFFFFF !important;
      ...
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#160F08', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#FFF6E8] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          {portadaFondoAnimado && (
          ...
          <div className={portadaFondoAnimado ? "acp-desktop-only" : undefined}>
            <div style={{
              ...
              background: 'radial-gradient(50% 40% at 15% 15%, rgba(143,211,255,0.13), transparent), radial-gradient(45% 40% at 85% 80%, rgba(185,166,255,0.2), transparent)',
              backgroundSize: '160% 160%',
              animation: 'crys-meshDrift 14s ease-in-out infinite',
            }} />
            <div style={{
              position: 'absolute', width: 220, height: 220, borderRadius: '50%',
              background: 'radial-gradient(circle, rgba(143,211,255,0.13), transparent 70%)',
          ...
          <IconFacet className="crys-doodle opacity-0 absolute" style={{ width: 34, height: 34, top: '10%', left: '12%', color: 'rgba(143,211,255,0.5)' }} />
          <IconFacet className="crys-doodle opacity-0 absolute" style={{ width: 26, height: 26, top: '14%', right: '14%', color: 'rgba(185,166,255,0.4)' }} />
          <IconFacet className="crys-doodle opacity-0 absolute" style={{ width: 30, height: 30, bottom: '16%', left: '18%', color: 'rgba(143,211,255,0.35)' }} />
          <IconShard className="crys-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '30%', right: '20%', color: '#FF6B9D' }} />
          <IconShard className="crys-doodle opacity-0 absolute" style={{ width: 16, height: 16, bottom: '22%', right: '28%', color: '#FFB347' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            {/* Guest Name */}
            <h2 className="text-4xl sm:text-5xl font-light tracking-wide text-[#FFF6E8] leading-relaxed" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className=" text-sm font-medium text-[#B8A68F] tracking-wide uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8 }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}

            {/* Botón de apertura -- vidrio esmerilado con borde celeste fino,
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #FFB347', color: '#FFB347',
                background: 'rgba(143,211,255,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#FFB347'; e.currentTarget.style.color = '#160F08'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(143,211,255,0.08)'; e.currentTarget.style.color = '#FFB347'; }}
            >
          ...
          <style jsx>{`
            @keyframes crys-meshDrift { 0%, 100% { background-position: 0% 0%, 100% 100%; } 50% { background-position: 30% 20%, 70% 80%; } }
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#FFB347]/40 shadow-md ${isTicketMaximized ? 'bg-[#160F08]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#FFFFFF]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#FFB347] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#FFB347] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#FFB347]/20 pl-3">
                <span className="text-[#FFFFFF] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#B8A68F] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#FFB347]" />
              <span className="text-[#241A0C]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <div className="desktop-stage" data-theme={theme} style={{
        ...
        // de fallback en vez del celeste/lavanda de Cristal 3D).
        "--t-acc": "#FFB347",
        "--t-acc2": "#FF6B9D",
        "--c-accent": "#FFB347",
        "--t-bg": "#160F08",
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#FFB347', margin: '6px 0 14px', animation: 'crys-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#160F08]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#160F08] z-10 relative">
            <IconFacet className="crys-scroll-doodle opacity-0 absolute" style={{ width: 24, height: 24, top: 10, right: 24, color: '#FF6B9D' }} />
            <IconShard className="crys-scroll-doodle opacity-0 absolute" style={{ width: 16, height: 16, top: 60, right: 56, color: '#FFB347' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#FFB347] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light leading-[1.0] mb-3 crys-gradient-text" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
            ...
            <div style={{ width: 40, height: 2, background: '#FFB347', margin: '0 0 20px', animation: 'crys-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#B8A68F] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#FFB347] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          
          {/* Image Container -- marco de capas de vidrio apiladas + barrido
              de refracción prismática ligado al scroll. */}
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #160F08 100%)' }} />
            <div
            ...
            />
            <PhotoCornerFrame />
            {/* Barrido de refracción prismática (celeste→lavanda→blanco) que
                cruza la foto según el progreso de scroll (anime.js
                onScroll, ver useEffect de heroPhotoRef) -- sensación de luz
                atravesando un cristal tallado. */}
            <div
              ref={heroShineRef}
        ...
        {/* Divisor doodle entre hero y cuenta regresiva (dos facetas de
            diamante flanqueando una línea con una esquirla al centro). */}
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #FF6B9D14, transparent 70%), #241A0C" }}>
            <div className="max-w-2xl mx-auto text-center">
              ...
              <TypewriterText 
                ...
                className="text-[#FFF6E8] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#160F08] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#FFB347]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="crys-tilt-card bg-white/[0.04] border border-[#FFB347]/25 rounded-2xl backdrop-blur-sm p-6 sm:p-8 mb-6" style={{ boxShadow: '0 10px 30px rgba(10,14,22,0.45), 0 2px 0 rgba(143,211,255,0.06)' }}>
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#B8A68F] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  ...
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#B8A68F]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {tx("invitacion.ubicacion.horaConSufijo", { hora: String(invitation.ceremoniaHora) })}</p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#B8A68F]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#FFB347] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="crys-tilt-card bg-white/[0.04] border border-[#FFB347]/25 rounded-2xl backdrop-blur-sm p-6 sm:p-8 mb-10" style={{ boxShadow: '0 10px 30px rgba(10,14,22,0.45), 0 2px 0 rgba(143,211,255,0.06)', animationDelay: '1.4s' }}>
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#B8A68F] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#FFB347] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#FFB347]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#FFB347]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#B8A68F] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#241A0C] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#241A0C] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <div className="flex justify-center mb-4">
                  <DrawLucideIcon icon={Landmark} size={46} color="var(--t-acc)" strokeWidth={1.5} />
                </div>
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#FFB347]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #FF6B9D18, transparent 70%), #241A0C" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {/* Separador doodle de footer -- tres facetas pequeñas en fila,
            eco del diamante de la portada. */}
        <div className="w-full flex justify-center gap-3 py-6" style={{ background: '#160F08' }} aria-hidden="true">
          <IconFacet className="crys-scroll-doodle opacity-0" style={{ width: 14, height: 15, color: 'rgba(143,211,255,0.5)' }} />
          <IconFacet className="crys-scroll-doodle opacity-0" style={{ width: 18, height: 19, color: 'rgba(185,166,255,0.5)' }} />
          <IconFacet className="crys-scroll-doodle opacity-0" style={{ width: 14, height: 15, color: 'rgba(143,211,255,0.5)' }} />
        </div>

        <LogoFooterCredit bgColor="#160F08" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#FFB347" surfaceColor="#121A28" />}
      </div>
  );
}
