 ...
 *
 * Portado 1:1 desde el diseño "Cine Abstracto XV" (Colección Storytelling,
 * mis quince años: fondo negro-carmesí, acento rojo cine + dorado marquesina,
...
export function CineAbstractoXvTemplate({ invitation, guest, isPersonalized = false }: CineAbstractoXvTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#F5C400";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(245,196,0,.35), 0 18px 50px -30px #F5C400";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#E8123A" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#E8123A" : "#F5C400";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A0306", tinta: "#F4F1EA", acento: "#F5C400", clase: `${cxvArchivoBlack.variable} ${cxvMono.variable}`, fuente: "var(--cxv-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="cxv-scroller">
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cxv-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #0A0306 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cxv-section" style={{ background: "#0D0508" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="cxv-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cxv-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F5F1E8" : "#F5C400" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="cxv-section" style={{ background: "#0D0508" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#F5C400" guest={guest} />
      )}
...
const CXV_CSS = `
  ...
  .cxv-date-month { font-family: var(--cxv-archivo), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #F5C400; padding-left: 12%; }

  .cxv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cxv-divider-line { width: 52px; height: 1px; background: #F5C400; display: inline-block; }
  ...
  .cxv-cal-link:hover { color: #F5C400; }
  ...
  .cxv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #E8123A, #4A0E1E, #F5C400, #F5F1E8, #E8123A); filter: saturate(.75); }
  .cxv-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .cxv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #F5C400; }
  .cxv-medallion-arc { position: absolute; inset: -14%; }
  .cxv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #F5C400; font-family: var(--cxv-mono), monospace; }

  .cxv-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  ...
  .cxv-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .cxv-cd-num { font-family: var(--cxv-archivo), sans-serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6EFDD; display: inline-block; }
  .cxv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #F5C400; }
  .cxv-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0A0306 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .cxv-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(232,18,58,0.3), rgba(245,196,0,0.2), rgba(245,196,0,0.32), rgba(232,18,58,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .cxv-accent-italic { font-style: italic; color: #E8123A; }
  .cxv-accent-serif { font-style: italic; color: #E8123A; font-family: var(--cxv-archivo), sans-serif; }
  .cxv-accent-serif-2 { color: #E8123A; }
  ...
  .cxv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #E8123A; margin-top: auto; }
  ...
  .cxv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #E8123A; }
  ...
  .cxv-crono-time { font-family: var(--cxv-mono), monospace; color: #E8123A; min-width: 42px; }
  ...
  .cxv-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cxv-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cxv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #F5C400; background: transparent; color: #F5C400; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cxv-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--cxv-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cxv-rsvp-input::placeholder { color: #6E6A5D; }
  .cxv-rsvp-input:focus { outline: none; border-bottom-color: #F5C400; }
  .cxv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cxv-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #F5C400, #E8123A); border: 1px solid #F5C400; color: #0B0B10; cursor: pointer; }
  .cxv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cxv-rsvp-btn--ghost { background: transparent; color: #F5C400; }
  .cxv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #E8123A; margin: 0; }
  ...
  .cxv-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  ...
  .cxv-bank-copy { flex-shrink: 0; font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #F5C400; background: transparent; color: #F5C400; cursor: pointer; }
  .cxv-bank-copy:hover { background: rgba(245,196,0,0.12); }
  ...
  .cxv-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  ...
  .cxv-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--cxv-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cxv-song-input::placeholder { color: #6E6A5D; }
  .cxv-song-input:focus { outline: none; border-bottom-color: #F5C400; }
  .cxv-song-sep { color: #8A8577; flex-shrink: 0; }
  .cxv-song-submit { flex-shrink: 0; background: none; border: none; color: #F5C400; font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cxv-song-submit:hover { color: #F5F1E8; }
  .cxv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cxv-song-error { font-size: 10px; color: #E8123A; margin-top: 6px; }
  ...
  .cxv-quiz-opt { font-family: var(--cxv-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(245,196,0,0.4); background: transparent; color: #F5C400; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cxv-quiz-opt:disabled { cursor: default; }
  .cxv-quiz-opt--picked { background: #F5C400; border-color: #F5C400; color: #0B0B10; }
  .cxv-quiz-opt--correct { background: #F5C400; border-color: #F5C400; color: #0B0B10; }
  .cxv-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cxv-quiz-result { padding-top: 18px; border-top: 1px solid rgba(245,196,0,0.2); }
  ...
  .cxv-final-card { border: 1px solid #F5C400; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .cxv-replay { cursor: pointer; color: #F5C400; }
  .cxv-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cxv-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .cxv-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cxv-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .cxv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F5F1E8, #F5C400); transition: height 260ms linear; display: block; }
  .cxv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #F5C400; transition: color 500ms ease; }
  ...
  .cxv-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #150508 0%, #0D0508 46%, #0A0306 100%); }
  .cxv-cover-half--bottom .cxv-cover-inner { top: auto; bottom: 0; }
  .cxv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(232,18,58,0.4), rgba(245,196,0,0.3), rgba(245,196,0,0.5), rgba(232,18,58,0.4)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .cxv-bulb { width: 5px; height: 5px; border-radius: 50%; background: #F5C400; animation: cxvBulbFlicker 1.4s ease-in-out infinite; display: inline-block; }
  ...
  .cxv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#F5C400, transparent); display: block; }
  ...
  .cxv-cover-cta { border: 1px solid #F5C400; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--cxv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cxv-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cxv-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0B0B0F; }
  }
  ...
  .cxv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #F5C400; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
