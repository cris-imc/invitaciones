  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0A0D0A", tinta: "#F4F1EA", acento: "#3F8A55", clase: `${encPlayfair.variable} ${encMono.variable}`, fuente: "var(--enc-mono), monospace" }}
      />
    );
  }
            ...
            <div className="enc-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="10,13,10" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="10,13,10" />
                </div>
              )}
            </div>
...
const ENC_CSS = `
  ...
  .enc-cal-link:hover { color: #3F8A55; }
