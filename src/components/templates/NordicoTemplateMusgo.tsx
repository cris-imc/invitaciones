 ...
 * Bruno): fondo blanco (#EDEFE9) + tinta casi negra (#1A1F1B) + acento
 * grafito cálido (#5B6E52, variable por color), tipografía grotesca
...
interface NordicoTemplateMusgoProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#5B6E52]/25 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#5B6E52] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#1A1F1B] break-all">{value}</span>
      </div>
      <button 
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#5B6E52]/25 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#5B6E52] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#1A1F1B] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#EDEFE9" }}>
          {tx("invitacion.quiz.juegoCompletado")}
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#1A1F1B" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#78826F" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#5B6E52] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#1A1F1B" }}>
                  {tx("invitacion.quiz.promedioGlobal", { n: stats.count })} <strong style={{ color: "#EDEFE9" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#1A1F1B] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function NordicoTemplateMusgo({ invitation, guest, isPersonalized = false }: NordicoTemplateMusgoProps) {
  const tx = useTextos();
  ...
  // #1A1F1B, sin acento de color fuerte) -> effect="geometric" sin tinte
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#EDEFE9] via-[#E4E8DE] to-[#EDEFE9] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
  ...
  return (
    <div
      ...
      style={{
        ...
        // como `dark ? var(--chic-ink, #EDEFE9) : "inherit"` -- el nombre de
        ...
        // que su fallback (#EDEFE9/#EDE9F4) los deja exactamente como estaban.
        "--chic-ink": "#1A1F1B",
    ...
    >
      <style>{`
        ...
        #songs.d-sec.dark {
          background-color: #E0E4D8 !important;
        ...
        #banco .copy-btn.copied {
          background-color: #5B6E52 !important;
          color: #EDEFE9 !important;
        }
      ...
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#EDEFE9', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#1A1F1B] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          {portadaFondoAnimado && (
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem', position: 'relative' }}>
            ...
            {/* Guest Name */}
            <h2 className={`text-3xl sm:text-4xl font-bold tracking-tight leading-tight${portadaFondoAnimado ? " nordico-cover-text" : ""}`} style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), sans-serif', color: portadaFondoAnimado ? undefined : '#1A1F1B', textTransform: isCinzelTitle ? 'uppercase' : 'capitalize' }}>
              {guestNameDisplay}
            </h2>

            <div className={`nordico-lineExpand${portadaFondoAnimado ? " nordico-cover-line" : ""}`} style={{ width: 40, height: 2, background: portadaFondoAnimado ? undefined : '#1A1F1B', margin: '0.25rem 0' }} />

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className={`text-sm font-medium tracking-wide uppercase${portadaFondoAnimado ? " nordico-cover-text-muted" : ""}`} style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8, color: portadaFondoAnimado ? undefined : '#78826F' }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #1A1F1B', color: '#EDEFE9',
                background: '#1A1F1B', borderRadius: 0,
                marginTop: '0.75rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#EDEFE9'; e.currentTarget.style.color = '#1A1F1B'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = '#1A1F1B'; e.currentTarget.style.color = '#EDEFE9'; }}
            >
          ...
          <style jsx>{`
            ...
            .nordico-cover-text { color: #EDEFE9; }
            .nordico-cover-text-muted { color: rgba(255,255,255,0.75); }
            .nordico-cover-line { background: #EDEFE9; }
            @media (min-width: 768px) {
              .nordico-cover-text { color: #1A1F1B; }
              .nordico-cover-text-muted { color: #78826F; }
              .nordico-cover-line { background: #1A1F1B; }
            }
          `}</style>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#5B6E52]/40 shadow-md ${isTicketMaximized ? 'bg-[#EDEFE9]/95 backdrop-blur-md w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#1A1F1B]/95 backdrop-blur-md px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#5B6E52] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                <span className="text-[#1A1F1B] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#5B6E52] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#5B6E52]/20 pl-3">
                <span className="text-[#1A1F1B] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#78826F] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#A9915C]" />
              <span className="text-[#EDEFE9]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <aside className="d-left hide-mobile" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
          ...
          <IconRegisterMark style={{ position: "absolute", top: -11, left: -11, width: 20, height: 20, color: "#EDEFE9" }} />
          <IconRegisterMark style={{ position: "absolute", bottom: -11, right: -11, width: 20, height: 20, color: "#EDEFE9" }} />
        </div>
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#5B6E52', margin: '6px 0 14px', animation: 'nordico-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#EDEFE9]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#EDEFE9] z-10 relative">
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#5B6E52] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#1A1F1B] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#5B6E52' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#5B6E52', margin: '0 0 20px', animation: 'nordico-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#78826F] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#5B6E52] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #EDEFE9 100%)' }} />
            <div
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              ...
              <IconRegisterMark style={{ position: "absolute", top: -11, left: -11, width: 20, height: 20, color: "#EDEFE9" }} />
              <IconRegisterMark style={{ position: "absolute", bottom: -11, right: -11, width: 20, height: 20, color: "#EDEFE9" }} />
              <IconSquareDoodle style={{ position: "absolute", top: -8, right: -8, width: 14, height: 14, color: "#EDEFE9" }} />
            </div>
          </div>
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #8A857814, transparent 70%), #E4E8DE" }}>
            <div className="max-w-2xl mx-auto text-center">
              ...
              <TypewriterText 
                ...
                className="text-[#1A1F1B] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#EDEFE9]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'rgba(17,17,17,0.18)' }} />
          <IconLineDoodle className="nordico-scroll-doodle opacity-0" style={{ width: 28, height: 6, color: '#5B6E52' }} />
          <div style={{ width: 40, height: 1, background: 'rgba(17,17,17,0.18)' }} />
        </div>

        <SectionWrapper id="details" delay={150} className="w-full bg-[#EDEFE9] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#5B6E52]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="border border-[#1A1F1B]/12 border-l-2 border-l-[#5B6E52] p-6 sm:p-8 mb-6">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#78826F] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-semibold text-[#1A1F1B] mb-3" style={{ fontFamily: 'var(--font-cormorant), sans-serif' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#78826F]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {tx("invitacion.ubicacion.horaConSufijo", { hora: String(invitation.ceremoniaHora) })}</p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#78826F]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#5B6E52] hover:text-[#1A1F1B] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="border border-[#1A1F1B]/12 border-l-2 border-l-[#5B6E52] p-6 sm:p-8 mb-10">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#78826F] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#5B6E52] hover:text-[#1A1F1B] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#5B6E52]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#1A1F1B]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#78826F] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#1A1F1B] font-semibold" style={{ fontFamily: 'var(--font-cormorant), sans-serif' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#E4E8DE] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#E0E4D8] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <div className="flex justify-center mb-4">
                  <DrawLucideIcon icon={Landmark} size={46} color="var(--t-acc)" strokeWidth={1.5} />
                </div>
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#5B6E52]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #8A857818, transparent 70%), #E0E4D8" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#EDEFE9' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'rgba(17,17,17,0.15)' }} />
          <IconRegisterMark className="nordico-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#5B6E52' }} />
          <div style={{ width: 28, height: 1, background: 'rgba(17,17,17,0.15)' }} />
        </div>

        <LogoFooterCredit bgColor="#EDEFE9" textColor="var(--chic-ink, #1A1F1B)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#A9915C" surfaceColor="#1A1F1B" />}
      </div>
  );
}
