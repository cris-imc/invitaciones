 ...
 * (tinta #1B3A5C), acento azul océano (#6B6FA6, variable por color) + acento
...
interface BonVoyageTemplateLavandaProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#6B6FA6]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#6B6FA6] uppercase tracking-wider mb-0.5">{label}</span>
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        ...
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#1B3A5C" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>

        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#5C87A6" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#6B6FA6] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#1B3A5C" }}>
                  {tx("invitacion.quiz.promedioGlobal", { n: stats.count })} <strong style={{ color: "#1B3A5C" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
...
export function BonVoyageTemplateLavanda({ invitation, guest, isPersonalized = false }: BonVoyageTemplateLavandaProps) {
  const tx = useTextos();
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(107, 111, 166, 0.2) !important;
        }

        /* RSVP Custom Aesthetics for BonVoyageTemplateLavanda */
        #rsvp.section.dark {
          background-color: #F4F9FB !important;
          color: #1B3A5C !important;
        ...
        #rsvp.section.dark input {
          ...
          border: 1px solid rgba(107, 111, 166, 0.2) !important;
        ...
        #banco .copy-btn {
          background-color: #6B6FA6 !important;
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div className={portadaFondoAnimado ? "acp-desktop-only" : undefined}>
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(107,111,166,0.14), transparent), radial-gradient(45% 40% at 85% 80%, rgba(201,169,110,0.2), transparent)',
            backgroundSize: '160% 160%',
            animation: 'bonvoyage-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(107,111,166,0.14), transparent 70%)',
          ...
          {/* Doodles decorativos (avioneta, estrella de mar, valija) --
              ...
              inicial). */}
          <IconAirplane className="bonvoyage-doodle opacity-0 absolute" style={{ width: 56, height: 40, top: '9%', left: '8%', color: 'rgba(107,111,166,0.5)' }} />
          ...
          <IconWaves className="bonvoyage-doodle opacity-0 absolute" style={{ width: 46, height: 16, bottom: '17%', left: '12%', color: 'rgba(107,111,166,0.4)' }} />
          <IconStarfish className="bonvoyage-doodle opacity-0 absolute" style={{ width: 15, height: 15, bottom: '25%', right: '18%', color: 'rgba(107,111,166,0.4)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #6B6FA6', color: '#6B6FA6',
                background: 'rgba(107,111,166,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#6B6FA6'; e.currentTarget.style.color = '#F4F9FB'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(107,111,166,0.08)'; e.currentTarget.style.color = '#6B6FA6'; }}
            >
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#6B6FA6]/40 shadow-md ${isTicketMaximized ? 'bg-[#F4F9FB]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#1B3A5C]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#6B6FA6] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#6B6FA6] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#6B6FA6]/20 pl-3">
                <span className="text-[#1B3A5C] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#5C87A6] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#6B6FA6]" />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#F4F9FB]">
          ...
          <div className="px-8 pt-16 pb-12 text-left bg-[#F4F9FB] z-10 relative">
            ...
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#6B6FA6] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#6B6FA6] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              <div className="absolute inset-0" style={{ border: "1px solid rgba(107,111,166,0.6)" }} />
              <IconCompass style={{ position: "absolute", top: -14, left: -12, width: 32, height: 32, color: "#6B6FA6" }} />
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#F4F9FB] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#6B6FA6]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}
            </p>

            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#6B6FA6] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#6B6FA6] hover:text-[#1B3A5C] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#6B6FA6] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#5C87A6] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#6B6FA6] hover:text-[#1B3A5C] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#6B6FA6]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#6B6FA6]/10 last:border-b-0">
                      {item.time && (
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#F6EFE0] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                ...
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#6B6FA6]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #6B6FA618, transparent 70%), #F6EFE0" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#F4F9FB' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #6B6FA6, transparent)' }} />
          <IconWaves className="bonvoyage-scroll-doodle opacity-0" style={{ width: 22, height: 8, color: '#6B6FA6' }} />
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #6B6FA6, transparent)' }} />
        </div>
      ...
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#6B6FA6" surfaceColor="#FFFFFF" />}
      </div>
  );
}
