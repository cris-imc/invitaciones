...
/**
 * BolaDeDiscotecaTemplateEsmeralda.tsx
 *
...
// palabras.
const ACCENT_CYCLE = ["#00E58A", "#7B2FFF", "#C7CDD9"];
...
export function BolaDeDiscotecaTemplateEsmeralda({ invitation, guest, isPersonalized = false }: BolaDeDiscotecaTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#00E58A";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #00E58A";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#7B2FFF" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#7B2FFF" : "#00E58A";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#070F0C", tinta: "#F4F1EA", acento: "#00E58A", clase: `${bddArchivo.variable} ${bddMono.variable}`, fuente: "var(--bdd-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="bdd-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="bdd-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #15241D 0%, #0A1712 55%, #070F0C 100%)" }}>
            ...
            <div className="bdd-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="7,15,12" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="7,15,12" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="bdd-section bdd-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #101F17 0%, #0C1B15 55%, #070F0C 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="bdd-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #182A20 0%, #0D1A14 52%, #070F0C 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="bdd-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="bdd-pan-sticky">
            <div data-strip="1" className="bdd-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="bdd-panel bdd-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="bdd-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#7B2FFF" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#7B2FFF" />
                  </svg>
              ...
              <div data-tone="dark" className="bdd-panel bdd-panel--center" style={{ background: "#0A1712", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="bdd-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #15241D 0%, #0A1712 60%, #070F0C 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="bdd-section" style={{ background: "#0A1712" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="bdd-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="bdd-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FFFFFF" : "#00E58A" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="bdd-section" style={{ background: "#0A1712" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="bdd-section" style={{ background: "#0A1712" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="bdd-section bdd-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #15241D 0%, #0A1712 55%, #070F0C 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#00E58A" guest={guest} />
      )}
...
function BddCoverHalf({
  ...
  return (
    <div className="bdd-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#00E58A" tintColor2="#08080E" effect="enfoque" scrimColorRgb="7,15,12" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#00E58A" tintColor2="#08080E" effect="enfoque" scrimColorRgb="7,15,12" />
        </div>
      )}
...
const BDD_CSS = `
  ...
  a { color: #00E58A; text-decoration: none; }
  ...
  @media (min-width: 768px) { .bdd-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(0,229,138,.3); } }
  ...
  .bdd-date-month { font-family: var(--bdd-archivo), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #00E58A; padding-left: 12%; }

  .bdd-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .bdd-divider-line { width: 52px; height: 1px; background: #00E58A; display: inline-block; }
  ...
  .bdd-cal-link:hover { color: #00E58A; }
  ...
  .bdd-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #7B2FFF, #00E58A, #00E58A, #FFFFFF, #7B2FFF); filter: saturate(.75); }
  .bdd-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0D1913; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .bdd-medallion-label--accent { color: #00E58A; }
  .bdd-medallion-arc { position: absolute; inset: -14%; }
  .bdd-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #00E58A; font-family: var(--bdd-mono), monospace; }
  ...
  .bdd-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #00E58A; }
  .bdd-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #070F0C 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .bdd-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(123,47,255,0.28), rgba(0,229,138,0.24), rgba(199,205,217,0.3), rgba(123,47,255,0.28)); filter: blur(80px); opacity: .4; animation: bddFoil 30s linear infinite; }
  ...
  .bdd-accent-italic { font-style: italic; color: #00E58A; }
  .bdd-accent-pink-italic { font-style: italic; color: #7B2FFF; }
  .bdd-accent-2 { color: #7B2FFF; }
  ...
  .bdd-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #7B2FFF; margin-top: auto; }
  ...
  .bdd-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #7B2FFF; }
  ...
  .bdd-crono-time { font-family: var(--bdd-mono), monospace; color: #7B2FFF; min-width: 42px; }
  ...
  .bdd-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(0,229,138,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #6E6A5D; }
  ...
  .bdd-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #00E58A; background: transparent; color: #00E58A; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .bdd-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(0,229,138,0.3); color: #F4F1EA; font-family: var(--bdd-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .bdd-rsvp-input::placeholder { color: #6E6A5D; }
  .bdd-rsvp-input:focus { outline: none; border-bottom-color: #00E58A; }
  .bdd-rsvp-btn { width: 100%; padding: 16px; font-family: var(--bdd-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; border: 1px solid #00E58A; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 300ms ease, color 300ms ease; }
  .bdd-rsvp-btn:hover:not(:disabled) { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0A150F; }
  .bdd-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .bdd-rsvp-btn--ghost { background: transparent; color: #00E58A; }
  .bdd-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(0,229,138,0.1); color: #00E58A; }
  .bdd-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #7B2FFF; margin: 0; }
  ...
  .bdd-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(0,229,138,0.18); }
  ...
  .bdd-bank-copy { flex-shrink: 0; font-family: var(--bdd-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #00E58A; background: transparent; color: #00E58A; cursor: pointer; }
  .bdd-bank-copy:hover { background: rgba(0,229,138,0.12); }
  ...
  .bdd-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(0,229,138,0.3); padding-bottom: 12px; }
  ...
  .bdd-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(0,229,138,0.3); color: #F4F1EA; font-family: var(--bdd-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .bdd-song-input::placeholder { color: #6E6A5D; }
  .bdd-song-input:focus { outline: none; border-bottom-color: #00E58A; }
  .bdd-song-sep { color: #8A8577; flex-shrink: 0; }
  .bdd-song-submit { flex-shrink: 0; background: none; border: none; color: #00E58A; font-family: var(--bdd-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .bdd-song-submit:hover { color: #FFFFFF; }
  .bdd-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .bdd-song-error { font-size: 10px; color: #7B2FFF; margin-top: 6px; }
  ...
  .bdd-quiz-opt { font-family: var(--bdd-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(0,229,138,0.4); background: transparent; color: #00E58A; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .bdd-quiz-opt:disabled { cursor: default; }
  .bdd-quiz-opt--picked { background: #00E58A; border-color: #00E58A; color: #0A150F; }
  .bdd-quiz-opt--correct { background: #00E58A; border-color: #00E58A; color: #0A150F; }
  .bdd-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .bdd-quiz-result { padding-top: 18px; border-top: 1px solid rgba(0,229,138,0.2); }
  ...
  .bdd-final-card { border: 1px solid #00E58A; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .bdd-replay { cursor: pointer; color: #00E58A; }
  ...
  .bdd-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FFFFFF, #00E58A); transition: height 260ms linear; display: block; }
  .bdd-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #00E58A; transition: color 500ms ease; }
  ...
  .bdd-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #122018 0%, #0A1712 46%, #070F0C 100%); }
  .bdd-cover-half--bottom .bdd-cover-inner { top: auto; bottom: 0; }
  .bdd-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(123,47,255,0.4), rgba(0,229,138,0.36), rgba(199,205,217,0.4), rgba(123,47,255,0.4)); filter: blur(64px); opacity: .62; animation: bddFoil 26s linear infinite; }
  ...
  .bdd-discolight { position: absolute; inset: -20%; animation: bddDiscoSweep 8s ease-in-out infinite; background:
    ...
    radial-gradient(3% 2% at 70% 15%, rgba(123,47,255,.7), transparent 60%),
    radial-gradient(3% 2% at 40% 60%, rgba(0,229,138,.7), transparent 60%),
    radial-gradient(3% 2% at 85% 70%, rgba(199,205,217,.6), transparent 60%),
  ...
  .bdd-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#00E58A, transparent); display: block; }
  ...
  .bdd-cover-cta { border: 1px solid #00E58A; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--bdd-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .bdd-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .bdd-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0A150F; }
  }
  ...
  .bdd-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #00E58A; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
