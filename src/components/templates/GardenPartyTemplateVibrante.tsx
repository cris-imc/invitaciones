/**
 * GardenPartyTemplateVibrante.tsx
 ...
 * tema "Cumple de Valeria"): fondo crema cálido (#FFF4F0) + tinta marrón oscuro
 * (#411D14) + acento terracota (#FF5A36, variable por color) + acento salvia
 * (#2FA88A), tipografía Fraunces itálica (display) + Poppins (texto), doodles
...
interface GardenPartyTemplateVibranteProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#FF5A36]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#FF5A36] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#411D14] break-all">{value}</span>
      </div>
      <button
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#FF5A36]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#FF5A36] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#411D14] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#411D14" }}>
          ¡Juego Completado!
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#411D14" }}>
          RESPONDISTE {score} DE {preguntas.length} CORRECTAMENTE ({percent}%)
        </p>

        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#9c5a45" }}>Guardando tus resultados...</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(217,119,87,0.08)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(217,119,87,0.18)", textAlign: "left", maxWidth: "90%" }}>
                ...
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#411D14" }}>
                  El promedio global de aciertos del resto de los invitados ({stats.count}) es del <strong style={{ color: "#411D14" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#411D14] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function GardenPartyTemplateVibrante({ invitation, guest, isPersonalized = false }: GardenPartyTemplateVibranteProps) {
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#FFF4F0] via-[#FFE1D6] to-[#FFFFFF] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center border-t border-white/10 font-sans">
          <LogoFooterCredit bgColor="transparent" textColor="var(--gardenparty-ink, #411D14)" />
        </footer>
      </div>
    );
  }
    ...
    >
      <style>{`
        ...
        #banco .copy-btn.copied {
          background-color: #411D14 !important;
      ...
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#FFF4F0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#411D14] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          {portadaFondoAnimado && (
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="gardenparty-seal opacity-0" style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #FF5A36',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#FF5A36',
            }}>
              {monogram}
            </div>

            <h2 className={`text-4xl sm:text-5xl font-light tracking-wide leading-relaxed${portadaFondoAnimado ? " gardenparty-cover-text" : ""}`} style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif', fontStyle: 'italic', color: portadaFondoAnimado ? undefined : '#411D14' }}>
              {guestNameDisplay}
            </h2>

            {Boolean(activeDressCode) && (
              <p className={`text-sm font-medium tracking-wide uppercase${portadaFondoAnimado ? " gardenparty-cover-text-muted" : ""}`} style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8, color: portadaFondoAnimado ? undefined : '#9c5a45' }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #FF5A36', color: '#FF5A36',
                background: 'rgba(217,119,87,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#FF5A36'; e.currentTarget.style.color = '#FFFFFF'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(217,119,87,0.08)'; e.currentTarget.style.color = '#FF5A36'; }}
            >
              ABRIR INVITACIÓN
            </button>
          ...
          <style jsx>{`
            ...
            .gardenparty-cover-text { color: #FFF4F0; }
            .gardenparty-cover-text-muted { color: rgba(65,29,20,0.75); }
            @media (min-width: 768px) {
              .gardenparty-cover-text { color: #411D14; }
              .gardenparty-cover-text-muted { color: #9c5a45; }
            }
          `}</style>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#FF5A36]/40 shadow-md rounded-full ${isTicketMaximized ? 'bg-[#FFF4F0]/95 backdrop-blur-md w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#411D14]/95 backdrop-blur-md px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#FF5A36] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#411D14] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#FF5A36]/20 pl-3">
                <span className="text-[#411D14] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#9c5a45] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? 'Lugar' : 'Lugares'}</span>
              </div>
            </div>
          ) : (
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#FF5A36', margin: '6px 0 14px', animation: 'gardenparty-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#FFF4F0]">
          <div className="px-8 pt-16 pb-12 text-left bg-[#FFF4F0] z-10 relative">
            ...
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#FF5A36] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#411D14] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif', fontStyle: 'italic' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#FF5A36' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#FF5A36', margin: '0 0 20px', animation: 'gardenparty-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#9c5a45] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#FF5A36] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #FFF4F0 100%)' }} />
            <div
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              ...
              <IconFlower style={{ position: "absolute", top: -12, left: -10, width: 26, height: 26, color: "#FF5A36" }} />
              <IconLeaf style={{ position: "absolute", bottom: -8, right: -8, width: 22, height: 22, color: "#2FA88A" }} />
            </div>
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#FFF4F0]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #FF5A36, transparent)' }} />
          <IconGarland className="gardenparty-scroll-doodle opacity-0" style={{ width: 30, height: 13, color: '#FF5A36' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #FF5A36, transparent)' }} />
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #2FA88A14, transparent 70%), var(--t-surface)" }}>
            <div className="max-w-2xl mx-auto text-center">
              <TypewriterText
                text={`"${String(invitation.frasePersonalizadaTexto)}"`}
                className="text-[#411D14] text-2xl md:text-3xl leading-relaxed tracking-wide"
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#FFF4F0] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#FF5A36]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              ...
              <div className="bg-white border-l-[3px] border-l-[#FF5A36] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#411D14] mb-3" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#9c5a45]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#9c5a45]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#FF5A36] hover:text-[#411D14] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-white border-l-[3px] border-l-[#FF5A36] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#9c5a45] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#FF5A36] hover:text-[#411D14] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#FF5A36]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <IconGarland className="gardenparty-scroll-doodle opacity-0" style={{ width: 22, height: 9 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#FF5A36]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#9c5a45] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#411D14] font-light" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
          ...
          <SectionWrapper id="album" delay={200} className="w-full py-20 overflow-hidden" style={{ background: "var(--t-surface)" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-10 flex items-center gap-2">
                <IconFlower className="gardenparty-scroll-doodle opacity-0" style={{ width: 16, height: 16, color: '#FF5A36' }} />
                ÁLBUM
              </p>
            </div>
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#FFE1D6] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#FF5A36]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #2FA88A18, transparent 70%), #FFE1D6" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#FFF4F0' }} aria-hidden="true">
            <IconMusic className="gardenparty-scroll-doodle opacity-0" style={{ width: 26, height: 26, color: '#FF5A36' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#FFF4F0' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #FF5A36, transparent)' }} />
          <IconButterfly className="gardenparty-scroll-doodle opacity-0" style={{ width: 20, height: 17, color: '#2FA88A' }} />
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #FF5A36, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#FFF4F0" textColor="var(--gardenparty-ink, #411D14)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#FF5A36" surfaceColor="#FFFFFF" />}
      </div>
  );
}
