/**
 * EditorialTemplateGrafito.tsx
 * Variante "Grafito" de Editorial -- a diferencia del resto de las
 * variantes de esta familia (Azul, Gris, Malva, Terracota, Verde), que
 * solo cambian el acento y heredan el mismo fondo beige cálido (#EDEBE5)
 * y tinta casi negra cálida (#17140F) del default, acá se recolorea TODA
 * la base a una paleta gris fría: fondo #E4E4E6, tinta #1B1C1E, acento
 * grafito #4B4F58, secundario #9C9CA0, bordes #D6D6D9 -- pensada para que
 * al cambiar de variante en el live preview del wizard se note una base
 * genuinamente distinta, no solo un cambio de acento. Contraste tinta/fondo
 * ~15:1, acento/fondo ~6.5:1 (verificado antes de aplicar). Misma
 * estructura/secciones/componentes que el resto de Editorial, ver
 * EditorialTemplate.tsx para el detalle.
 */
"use client";
...
const IconInfo  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2} aria-hidden="true"><circle cx="12" cy="12" r="9.5"/><path d="M12 15.5v-4M12 8.2h.01"/></svg>;
const IconCheck = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2} aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg>;
const IconMusic = ({ className, style }: { className?: string; style?: React.CSSProperties } = {}) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2} className={className} style={style} aria-hidden="true"><circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="2.2"/></svg>;
const IconMap   = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2} aria-hidden="true"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0Z"/><circle cx="12" cy="10" r="3"/></svg>;
const IconGift  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2} aria-hidden="true"><rect x="3" y="8" width="18" height="4" rx="1"/><path d="M12 8v13"/><path d="M19 12v7a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2v-7"/><path d="M7.5 8a2.5 2.5 0 0 1 0-5A4.8 8 0 0 1 12 8a4.8 8 0 0 1 4.5-5 2.5 2.5 0 0 1 0 5"/></svg>;
const IconQuiz  = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2} aria-hidden="true"><circle cx="12" cy="12" r="9.5"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg>;
// Ramita de laurel doodle -- decorativa, usada en portada/hero y como
// divisor de secciones. Dos hileras de hojas alargadas en espejo sobre un
// tallo central, trazo fino sin relleno (motivo editorial clásico).
const IconRings = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 30 16" fill="none" stroke="currentColor" strokeWidth={1} className={className} style={style} aria-hidden="true">
    <path d="M1 8h28" />
    <path d="M6 8c0-2.6 1.8-4 4-4.4M6 8c0 2.6 1.8 4 4 4.4" />
    <path d="M12 8c0-2.6 1.8-4 4-4.4M12 8c0 2.6 1.8 4 4 4.4" />
    <path d="M18 8c0-2.6 1.8-4 4-4.4M18 8c0 2.6 1.8 4 4 4.4" />
    <circle cx="29" cy="8" r="1" />
  </svg>
);
// Estrella de cinco puntas de trazo simple -- acento de portada y footer,
// deliberadamente austera (sin relleno, sin brillo extra) para mantener el
// minimalismo editorial.
const IconHeartDoodle = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1} className={className} style={style} aria-hidden="true">
    <path d="M12 2.5 14.6 9.4 22 10 16.2 14.6 18 22 12 17.8 6 22 7.8 14.6 2 10 9.4 9.4Z" />
  </svg>
);
// Rombo/marca de diamante doodle -- separador de secciones, dos rombos
// concéntricos (línea externa fina + interna más chica) tipo sello editorial.
const IconRibbon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth={1} className={className} style={style} aria-hidden="true">
    <path d="M10 1.5 18.5 10 10 18.5 1.5 10Z" />
    <path d="M10 6 14 10 10 14 6 10Z" strokeWidth={0.8} />
  </svg>
);
// Marca "cuándo y dónde" -- reloj de arena de trazo fino sin relleno, motivo
// austero y atemporal en vez de un ícono figurativo de lugar puntual.
const IconChurch = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1} className={className} style={style} aria-hidden="true">
    <path d="M6 3h12M6 21h12" />
    <path d="M7 3c0 4.5 3 6 5 7.5C10 12 7 13.5 7 18v3M17 3c0 4.5-3 6-5 7.5 2 1.5 5 3 5 7.5v3" strokeWidth={0.9} />
    <path d="M7 3h10M7 21h10" strokeWidth={1.3} />
  </svg>
);

// Tipografía del sistema "Editorial" (Bodoni Moda display + Archivo texto),
...
interface EditorialTemplateGrafitoProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#4B4F58]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#4B4F58] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#1B1C1E] break-all">{value}</span>
      </div>
      <button 
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#4B4F58]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#4B4F58] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#1B1C1E] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#1B1C1E" }}>
          {tx("invitacion.quiz.juegoCompletado")}
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#1B1C1E" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#78787C" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#4B4F58] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#1B1C1E" }}>
                  {tx("invitacion.quiz.promedioGlobal", { n: stats.count })} <strong style={{ color: "#1B1C1E" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#1B1C1E] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function EditorialTemplateGrafito({ invitation, guest, isPersonalized = false }: EditorialTemplateGrafitoProps) {
  const tx = useTextos();
  ...
  // Entrada animada de los doodles de la portada (laurel, estrella, rombo)
  // con anime.js -- ver docs/GUIA_TECNICA_PLANTILLAS.md. Corre una sola vez,
  ...
  useEffect(() => {
    ...
    animate(root.querySelectorAll(".editorial-doodle"), {
      scale: [0, 1],
      rotate: [-15, 0],
      opacity: [0, 1],
      duration: 900,
    ...
    animate(root.querySelectorAll(".editorial-seal"), {
      scale: [0.6, 1],
      opacity: [0, 1],
      duration: 700,
      delay: 150,
  ...
  useEffect(() => {
    ...
    const els = document.querySelectorAll(".editorial-scroll-doodle");
  ...
  // "Gloss editorial": un barrido diagonal sutil de luz, como el reflejo de
  // una página de revista satinada, ligado al progreso de scroll (en vez de
  // un lens-flare figurativo). Una sola banda de brillo que cruza la foto
  // de punta a punta -- coherente con el minimalismo del sistema Editorial
  // (nada de múltiples reflejos fantasma).
  ...
  useEffect(() => {
    ...
    const observer = onScroll({
      ...
      onUpdate: (self) => {
        ...
        const x = p * 160 - 30; // % a lo largo de la diagonal, de punta a punta
        sheen.style.opacity = String(intensity * 0.5);
        sheen.style.transform = `translateX(${x}%) rotate(18deg)`;
      },
    });
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#E4E4E6] via-[#D6D6D9] to-[#FFFFFF] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center border-t border-white/10 font-sans">
          <LogoFooterCredit bgColor="transparent" textColor="var(--editorial-ink, #1B1C1E)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    <div
      className={`${editorialBodoni.variable} ${editorialArchivo.variable}`}
      style={{
        "--font-cormorant": "var(--editorial-bodoni)",
        "--font-inter": "var(--editorial-archivo)",
        "--font-sans": "var(--editorial-archivo)",
        "--t-acc": "#4B4F58",
        "--t-acc2": "#4B4F58",
        "--c-accent": "#4B4F58",
        "--t-bg": "#E4E4E6",
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(163, 18, 59, 0.2) !important;
        }

        /* RSVP Custom Aesthetics for EditorialTemplateGrafito */
        #rsvp.section.dark {
          background-color: #E4E4E6 !important;
          color: #1B1C1E !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #4B4F58 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #E4E4E6 !important; /* Matches light sections */
          color: #1B1C1E !important;
      ...
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#E4E4E6', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#1B1C1E] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          {portadaFondoAnimado && (
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            {/* Guest Name */}
            <h2 className={`text-4xl sm:text-5xl font-light tracking-wide leading-relaxed${portadaFondoAnimado ? " editorial-cover-text" : ""}`} style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif', color: portadaFondoAnimado ? undefined : '#1B1C1E' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className={`text-sm font-medium tracking-wide uppercase${portadaFondoAnimado ? " editorial-cover-text-muted" : ""}`} style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8, color: portadaFondoAnimado ? undefined : '#78787C' }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #4B4F58', color: '#4B4F58',
                background: 'rgba(163,18,59,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#4B4F58'; e.currentTarget.style.color = '#E4E4E6'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(163,18,59,0.08)'; e.currentTarget.style.color = '#4B4F58'; }}
            >
          ...
          <style jsx>{`
            ...
            .editorial-cover-text { color: #E4E4E6; }
            .editorial-cover-text-muted { color: rgba(237,235,229,0.75); }
            @media (min-width: 768px) {
              .editorial-cover-text { color: #1B1C1E; }
              .editorial-cover-text-muted { color: #78787C; }
            }
          `}</style>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#4B4F58]/40 shadow-md ${isTicketMaximized ? 'bg-[#E4E4E6]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#1B1C1E]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#4B4F58] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                <span className="text-[#1B1C1E] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#4B4F58] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#4B4F58]/20 pl-3">
                <span className="text-[#1B1C1E] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#78787C] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#4B4F58]" />
              <span className="text-[#FFFFFF]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#4B4F58', margin: '6px 0 14px', animation: 'editorial-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#E4E4E6]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#E4E4E6] z-10 relative">
            <IconHeartDoodle className="editorial-scroll-doodle opacity-0 absolute" style={{ width: 18, height: 18, top: 12, right: 28, color: 'rgba(184,169,140,0.4)' }} />
            <IconRibbon className="editorial-scroll-doodle opacity-0 absolute" style={{ width: 26, height: 15, top: 64, right: 52, color: 'rgba(163,18,59,0.4)' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#4B4F58] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#1B1C1E] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#4B4F58' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#4B4F58', margin: '0 0 20px', animation: 'editorial-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#78787C] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#4B4F58] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          
          {/* Image Container -- marco doble línea completo + gloss editorial
              ligado al scroll (ver useEffect de heroPhotoRef). */}
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #E4E4E6 100%)' }} />
            <div
            ...
            />
            {/* Marco doble línea completo + estrella/rombo "saliendo" de dos
                esquinas opuestas, motivo editorial de sello/ex-libris. */}
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              <div className="absolute inset-0" style={{ border: "1px solid rgba(163,18,59,0.7)" }} />
              <div className="absolute inset-[3px]" style={{ border: "1px solid rgba(163,18,59,0.35)" }} />
              <IconHeartDoodle style={{ position: "absolute", top: -9, right: -9, width: 18, height: 18, color: "#4B4F58" }} />
              <IconRibbon style={{ position: "absolute", bottom: -8, left: -8, width: 16, height: 16, color: "#9C9CA0" }} />
            </div>
            {/* Gloss editorial: una única banda diagonal de brillo que cruza
                la foto de punta a punta al hacer scroll -- como el reflejo
                de una página satinada, sin reflejos fantasma adicionales. */}
            <div
              ref={heroSheenRef}
              className="pointer-events-none z-20"
              style={{
                position: "absolute", top: "-30%", left: "-10%", width: "45%", height: "160%",
                background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.55) 45%, rgba(255,255,255,0.7) 50%, rgba(255,255,255,0.55) 55%, transparent)",
                opacity: 0, transform: "translateX(-30%) rotate(18deg)",
              }}
            />
          </div>
        </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #9C9CA014, transparent 70%), var(--t-surface)" }}>
            <div className="max-w-2xl mx-auto text-center">
              ...
              <TypewriterText 
                ...
                className="text-[#1B1C1E] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#E4E4E6] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#4B4F58]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#4B4F58] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#78787C] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#1B1C1E] mb-3" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#78787C]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {tx("invitacion.ubicacion.horaConSufijo", { hora: String(invitation.ceremoniaHora) })}</p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#78787C]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#4B4F58] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#4B4F58] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#78787C] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#4B4F58] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#4B4F58]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#4B4F58]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#78787C] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#1B1C1E] font-light" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#D6D6D9] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <div className="flex justify-center mb-4">
                  <DrawLucideIcon icon={Landmark} size={46} color="var(--t-acc)" strokeWidth={1.5} />
                </div>
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#4B4F58]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #9C9CA018, transparent 70%), #D6D6D9" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#E4E4E6' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #4B4F58, transparent)' }} />
          <svg className="editorial-scroll-doodle opacity-0" viewBox="0 0 16 12" width="16" height="12" fill="none" stroke="#9C9CA0" strokeWidth="1.1" aria-hidden="true">
            <path d="M8 11V2M8 5 4 3M8 5l4-2M8 8 4.5 6.3M8 8l3.5-1.7" />
          </svg>
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #4B4F58, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#E4E4E6" textColor="var(--editorial-ink, #1B1C1E)" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#4B4F58" surfaceColor="#FFFFFF" />}
      </div>
  );
}
