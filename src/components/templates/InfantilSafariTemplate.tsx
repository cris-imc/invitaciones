...
interface InfantilSafariTemplateQuizQuestion {
  pregunta: string;
  opciones: string[];
...
interface InfantilSafariTemplateProps {
...
export function InfantilSafariTemplate({ invitation, guest, isPersonalized = false }: InfantilSafariTemplateProps) {
  const tx = useTextos();
  ...
  const triviaPreguntas: InfantilSafariTemplateQuizQuestion[] = safeJson<InfantilSafariTemplateQuizQuestion[]>(String(invitation.triviaPreguntas ?? ""), []);
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#FDF2DE";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#D98A3C";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #D98A3C";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#6E4A24" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#6E4A24" : "#D98A3C";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#160F07", tinta: "#F4F1EA", acento: "#D98A3C", clase: `${ifsDisplay.variable} ${ifsMono.variable}`, fuente: "var(--ifs-mono), monospace" }}
      />
    );
  }
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="ifs-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="ifs-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #1C1409 55%, #160F07 100%)" }}>
          ...
          <div data-drift="-70" className="ifs-medallion ifs-medallion--corner">
            <InfantilSafariTemplateMedallionCmp label="SF" sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="ifsArc1" arcText={tx("invitacion.sabor.arcoSafariParty").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="ifs-section ifs-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #3A2A12 0%, #0D0B10 55%, #160F07 100%)" }}>
          ...
          <div className="ifs-cd-grid">
            <InfantilSafariTemplateCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <InfantilSafariTemplateCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <InfantilSafariTemplateCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <InfantilSafariTemplateCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="ifs-pan">
          <div className="ifs-pan-sticky">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="ifs-panel ifs-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="ifs-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#6E4A24" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#6E4A24" />
                  </svg>
            ...
            <InfantilSafariTemplateDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="ifs-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #1C1409 60%, #160F07 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="ifs-rsvp">
              <InfantilSafariTemplateRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="ifs-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="ifs-pan-sticky">
            ...
            <InfantilSafariTemplateDots count={photoPages.length} />
          </div>
        </div>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="ifs-section" style={{ background: "#1C1409" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="ifs-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="ifs-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FDF2DE" : "#D98A3C" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="ifs-song-wrap">
              <InfantilSafariTemplateSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="ifs-bank-wrap">
              {pagoTarjetaHabilitado && (
                <BankDetailsCard
                  ...
                  InfoRow={InfantilSafariTemplateInfoRow}
                  CopyField={InfantilSafariTemplateCopyField}
                />
              )}
              {regaloHabilitado && (
                <BankDetailsCard
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="ifs-section ifs-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #1C1409 55%, #160F07 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="ifs-final-card">
            <div className="ifs-medallion ifs-medallion--final">
              <InfantilSafariTemplateMedallionCmp label="SF" sub={confirmed ? "CONFIRMADO" : tx("invitacion.pase.pendiente").toUpperCase()} arcId="ifsArc3" arcText={textoArco(namesTitle, fechaCorta)} spin="reverse" />
            </div>
      ...
      <div ref={coverRef} className="ifs-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="ifs-cover-half ifs-cover-half--top">
          <InfantilSafariTemplateCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#D98A3C" guest={guest} />
      )}
...
function InfantilSafariTemplateCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
...
function InfantilSafariTemplateDots({ count = 3 }: { count?: number }) {
...
function InfantilSafariTemplateMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
...
function InfantilSafariTemplateCopyField({ label, value }: { label: string; value: string }) {
  const tx = useTextos();
...
function InfantilSafariTemplateRsvpCard({
  invitationId,
  guestToken,
  guestName,
  maxAdults,
    ...
    <>
      ...
      <div ref={stubRef} className="ifs-stub">
        ...
        <div ref={sealRef} className="ifs-seal">
          <InfantilSafariTemplateMedallionCmp label="SF" sub="" arcId="" arcText="" spin="none" compact />
        </div>
...
function InfantilSafariTemplateSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  const tx = useTextos();
...
function InfantilSafariTemplateQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: InfantilSafariTemplateQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  const tx = useTextos();
...
function InfantilSafariTemplateInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="ifs-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="ifs-bank-row-label">{label}</span>
...
function InfantilSafariTemplateCoverHalf({
  namesRef,
  kickerRef,
  perfRef,
  kickerText,
...
const GP_CSS = `
  ...
  a { color: #D98A3C; text-decoration: none; }
  a:hover { color: #FDF2DE; }
  ...
  .ifs-date-month { font-family: var(--ifs-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #D98A3C; padding-left: 12%; }

  .ifs-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .ifs-divider-line { width: 52px; height: 1px; background: #D98A3C; display: inline-block; }
  ...
  .ifs-cal-link:hover { color: #D98A3C; }
  ...
  .ifs-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #D98A3C, #FDF2DE, #6E5AA8); filter: saturate(.75); }
  ...
  .ifs-medallion-label { font-family: var(--ifs-display), serif; font-size: 26px; line-height: 1; color: #FDF2DE; }
  .ifs-medallion-label-sm { font-family: var(--ifs-display), serif; font-size: 16px; color: #FDF2DE; }
  .ifs-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .ifs-medallion-sub--accent { color: #D98A3C; }
  ...
  .ifs-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #D98A3C; opacity: 0.4; font-family: var(--ifs-mono), monospace; }
  ...
  .ifs-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #FDF2DE, transparent); animation: ifsScan 6s linear infinite; pointer-events: none; }
  ...
  .ifs-cd-num { font-family: var(--ifs-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #FDF2DE; display: inline-block; }
  .ifs-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #D98A3C; }
  ...
  .ifs-accent-italic { font-style: italic; color: #D98A3C; }
  .ifs-accent-serif { font-style: italic; color: #6E4A24; font-family: var(--ifs-display), serif; }
  .ifs-accent-serif-2 { color: #6E4A24; }
  ...
  .ifs-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #6E4A24; margin-top: auto; }
  ...
  .ifs-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #6E4A24; }
  ...
  .ifs-crono-time { font-family: var(--ifs-mono), monospace; color: #6E4A24; min-width: 42px; }
  ...
  .ifs-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #FDF2DE, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .ifs-rsvp-payment-total { color: #FDF2DE; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .ifs-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #D98A3C; background: transparent; color: #D98A3C; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .ifs-rsvp-input:focus { outline: none; border-bottom-color: #D98A3C; }
  .ifs-rsvp-btn { width: 100%; padding: 16px; font-family: var(--ifs-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #D98A3C, #6E4A24); border: 1px solid #D98A3C; color: #1C1409; cursor: pointer; }
  .ifs-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .ifs-rsvp-btn--ghost { background: transparent; color: #D98A3C; }
  .ifs-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #D98A3C; margin: 0; }
  ...
  .ifs-bank-copy { flex-shrink: 0; font-family: var(--ifs-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #D98A3C; background: transparent; color: #D98A3C; cursor: pointer; }
  ...
  .ifs-song-input:focus { outline: none; border-bottom-color: #D98A3C; }
  .ifs-song-sep { color: #8A8577; flex-shrink: 0; }
  .ifs-song-submit { flex-shrink: 0; background: none; border: none; color: #D98A3C; font-family: var(--ifs-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .ifs-song-submit:hover { color: #FDF2DE; }
  .ifs-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .ifs-song-error { font-size: 10px; color: #D98A3C; margin-top: 6px; }
  ...
  .ifs-quiz-opt { font-family: var(--ifs-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #D98A3C; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .ifs-quiz-opt:disabled { cursor: default; }
  .ifs-quiz-opt--picked { background: #D98A3C; border-color: #D98A3C; color: #1C1409; }
  .ifs-quiz-opt--correct { background: #D98A3C; border-color: #D98A3C; color: #1C1409; }
  ...
  .ifs-quiz-result-score { margin: 0 0 6px; font-family: var(--ifs-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #FDF2DE; }
  .ifs-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .ifs-final-card { border: 1px solid #D98A3C; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .ifs-final-names { font-family: var(--ifs-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #FDF2DE; }
  ...
  .ifs-replay { cursor: pointer; color: #D98A3C; }
  ...
  .ifs-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FDF2DE, #D98A3C); transition: height 260ms linear; display: block; }
  .ifs-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #D98A3C; transition: color 500ms ease; }
  ...
  .ifs-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(62,110,74,0.45), rgba(217,138,60,0.32), rgba(217,138,60,0.55), rgba(62,110,74,0.45)); filter: blur(64px); opacity: .62; animation: ifsFoil 26s linear infinite; }
  .ifs-cover-texture { position: absolute; inset: 0; background-image: repeating-linear-gradient(115deg, rgba(253,242,222,.06) 0 2px, transparent 2px 30px); }
  ...
  .ifs-cover-names { margin: 0; font-family: var(--ifs-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FDF2DE; }
  .ifs-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#D98A3C, transparent); display: block; }
  ...
  .ifs-cover-cta { border: 1px solid #D98A3C; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--ifs-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .ifs-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #D98A3C; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
