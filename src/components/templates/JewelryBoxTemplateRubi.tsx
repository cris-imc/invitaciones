...
/**
 * JewelryBoxTemplateRubi.tsx (variante de color "Rubi")
 *
 * Portado 1:1 desde el diseño "Jewelry Box" (Colección Storytelling, mis
 * quince años, variante "Rubi": fondo borgoña rubí oscuro, acento rojo rubí + dorado,
...
export function JewelryBoxTemplateRubi({ invitation, guest, isPersonalized = false }: JewelryBoxTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#FBEAEE";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#E0748F";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(196,140,70,.35), 0 18px 50px -30px #E0748F";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#D9A24E" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(196,140,70,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(196,140,70,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#D9A24E" : "#E0748F";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#240D14", tinta: "#F4F1EA", acento: "#E0748F", clase: `${jwbCormorant.variable} ${jwbMono.variable}`, fuente: "var(--jwb-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="jwb-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="jwb-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #1A0A0F 0%, #3D1420 55%, #240D14 100%)" }}>
            ...
            <div className="jwb-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="36,13,20" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="36,13,20" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="jwb-section jwb-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #5C1F30 0%, #331222 55%, #240D14 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="jwb-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #200E17 0%, #13080B 52%, #240D14 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="jwb-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="jwb-pan-sticky">
            <div data-strip="1" className="jwb-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="jwb-panel jwb-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="jwb-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#D9A24E" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#D9A24E" />
                  </svg>
              ...
              <div data-tone="dark" className="jwb-panel jwb-panel--center" style={{ background: "#3D1420", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="jwb-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #1A0A0F 0%, #3D1420 60%, #240D14 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="jwb-section" style={{ background: "#3D1420" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="jwb-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="jwb-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FBEAEE" : "#E0748F" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="jwb-section" style={{ background: "#3D1420" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="jwb-section" style={{ background: "#3D1420" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pieza" className="jwb-section jwb-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #1A0A0F 0%, #3D1420 55%, #240D14 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#E0748F" guest={guest} />
      )}
...
function JwbCoverHalf({
  ...
  return (
    <div className="jwb-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#E0748F" tintColor2="#240D14" effect="enfoque" scrimColorRgb="36,13,20" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#E0748F" tintColor2="#240D14" effect="enfoque" scrimColorRgb="36,13,20" />
        </div>
      )}
...
const JWB_CSS = `
  ...
  a { color: #E0748F; text-decoration: none; }
  a:hover { color: #FBEAEE; }
  ...
  .jwb-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #240D14; }
  ...
  @media (min-width: 768px) {
    .jwb-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(224,116,143,.3); }
  ...
  .jwb-date-month { font-family: var(--jwb-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #E0748F; padding-left: 12%; }

  .jwb-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .jwb-divider-line { width: 52px; height: 1px; background: #E0748F; display: inline-block; }
  ...
  .jwb-cal-link:hover { color: #E0748F; }
  ...
  .jwb-gem { position: absolute; inset: -6px; border-radius: 50%; box-shadow: 0 0 24px 4px rgba(224,116,143,.7); animation: gemFlash 4s ease-in-out infinite; pointer-events: none; }
  .jwb-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #7A2E48, #4A1A29, #E0748F, #FBEAEE, #7A2E48); filter: saturate(.75); }
  ...
  .jwb-medallion-label { font-family: var(--jwb-cormorant), serif; font-size: 26px; line-height: 1; color: #FBEAEE; }
  .jwb-medallion-label-sm { font-family: var(--jwb-cormorant), serif; font-size: 16px; color: #FBEAEE; }
  .jwb-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .jwb-medallion-sub--accent { color: #E0748F; }
  .jwb-medallion-arc { position: absolute; inset: -14%; }
  .jwb-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #E0748F; opacity: 0.4; font-family: var(--jwb-mono), monospace; }

  .jwb-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(196,140,70,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .jwb-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #FBEAEE, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .jwb-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .jwb-cd-box { border: 1px solid #331620; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .jwb-cd-num { font-family: var(--jwb-cormorant), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F2E2BE; display: inline-block; }
  .jwb-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #E0748F; }
  .jwb-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #240D14 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .jwb-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(122,46,72,0.4), rgba(217,162,78,0.24), rgba(224,116,143,0.36), rgba(122,46,72,0.4)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .jwb-accent-italic { font-style: italic; color: #E0748F; }
  .jwb-accent-serif { font-style: italic; color: #D9A24E; font-family: var(--jwb-cormorant), serif; }
  .jwb-accent-serif-2 { color: #D9A24E; }
  ...
  .jwb-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #D9A24E; margin-top: auto; }
  ...
  .jwb-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #D9A24E; }
  ...
  .jwb-crono-time { font-family: var(--jwb-mono), monospace; color: #D9A24E; min-width: 42px; }
  ...
  .jwb-stub { position: relative; overflow: hidden; border: 1px solid #331620; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .jwb-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #FBEAEE, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .jwb-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(224,116,143,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .jwb-rsvp-payment-total { color: #FBEAEE; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .jwb-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #E0748F; background: transparent; color: #E0748F; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .jwb-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(224,116,143,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .jwb-rsvp-input::placeholder { color: #6E6A5D; }
  .jwb-rsvp-input:focus { outline: none; border-bottom-color: #E0748F; }
  .jwb-rsvp-btn { width: 100%; padding: 16px; font-family: var(--jwb-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #E0748F, #D9A24E); border: 1px solid #E0748F; color: #0B0B10; cursor: pointer; }
  .jwb-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .jwb-rsvp-btn--ghost { background: transparent; color: #E0748F; }
  .jwb-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #E0748F; margin: 0; }
  ...
  .jwb-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(224,116,143,0.18); }
  ...
  .jwb-bank-copy { flex-shrink: 0; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #E0748F; background: transparent; color: #E0748F; cursor: pointer; }
  .jwb-bank-copy:hover { background: rgba(224,116,143,0.12); }
  ...
  .jwb-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(224,116,143,0.3); padding-bottom: 12px; }
  ...
  .jwb-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(224,116,143,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .jwb-song-input::placeholder { color: #6E6A5D; }
  .jwb-song-input:focus { outline: none; border-bottom-color: #E0748F; }
  .jwb-song-sep { color: #8A8577; flex-shrink: 0; }
  .jwb-song-submit { flex-shrink: 0; background: none; border: none; color: #E0748F; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .jwb-song-submit:hover { color: #FBEAEE; }
  .jwb-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .jwb-song-error { font-size: 10px; color: #E0748F; margin-top: 6px; }
  ...
  .jwb-quiz-opt { font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(224,116,143,0.4); background: transparent; color: #E0748F; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .jwb-quiz-opt:disabled { cursor: default; }
  .jwb-quiz-opt--picked { background: #E0748F; border-color: #E0748F; color: #0B0B10; }
  .jwb-quiz-opt--correct { background: #E0748F; border-color: #E0748F; color: #0B0B10; }
  .jwb-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .jwb-quiz-result { padding-top: 18px; border-top: 1px solid rgba(224,116,143,0.2); }
  .jwb-quiz-result-score { margin: 0 0 6px; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #FBEAEE; }
  .jwb-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .jwb-final-card { border: 1px solid #E0748F; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .jwb-final-names { font-family: var(--jwb-cormorant), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #FBEAEE; }
  ...
  .jwb-replay { cursor: pointer; color: #E0748F; }
  .jwb-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .jwb-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(196,140,70,0.14); }
  .jwb-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .jwb-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(196,140,70,0.16); position: relative; }
  .jwb-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FBEAEE, #E0748F); transition: height 260ms linear; display: block; }
  .jwb-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #E0748F; transition: color 500ms ease; }
  ...
  .jwb-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #4A1A29 0%, #3D1420 46%, #240D14 100%); }
  .jwb-cover-half--bottom .jwb-cover-inner { top: auto; bottom: 0; }
  .jwb-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(122,46,72,0.6), rgba(217,162,78,0.35), rgba(224,116,143,0.55), rgba(122,46,72,0.6)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .jwb-cover-names { margin: 0; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FBEAEE; }
  .jwb-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#E0748F, transparent); display: block; }
  ...
  .jwb-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #E0748F; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
