 ...
 * Cambia solo la capa visual: paleta grafito (#000000) + dorado champagne (#C9A876)
...
interface ModernoTemplateNegroProps {
...
export function ModernoTemplateNegro({ invitation, guest, isPersonalized = false }: ModernoTemplateNegroProps) {
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-screen w-full bg-gradient-to-b bg-[#000000] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
  ...
  return (
    <>
      <style>{`
        ...
        #rsvp.section.dark {
          background-color: #000000 !important; /* Dark brown/black */
        ...
        #songs.d-sec.dark {
          background-color: #000000 !important; /* Light theme */
        ...
        #songs.d-sec.dark input {
          ...
          background-color: #0A0A0A !important; /* Pastel */
        ...
        /* Portada / Mesh animations */
        @keyframes moderno-meshDrift {
          0% { background-position: 0% 0%; }
          50% { background-position: 100% 100%; }
          100% { background-position: 0% 0%; }
        }
        @keyframes moderno-glowPulse {
        ...
        .moderno-portada-bg {
          ...
          background-image:
            radial-gradient(ellipse at 75% 20%, rgba(201,168,118,0.18) 0%, transparent 45%),
            radial-gradient(ellipse at 20% 80%, rgba(62,122,106,0.15) 0%, transparent 45%);
        ...
        .moderno-glow-orb {
          ...
          animation: moderno-glowPulse 5s ease-in-out infinite;
          pointer-events: none;
          z-index: 0;
        }
        /* Countdown Overrides */
      ...
      {/* PORTADA / WELCOME OVERLAY — Moderno: mesh dorado/esmeralda animado, sello, glow orb */}
      {!isCoverOpen && (
        <div
          style={{
            ...
            justifyContent: 'flex-start', paddingTop: '18vh',
            overflow: 'hidden',
          }}
          className="text-[#EDE9F4] transition-all duration-1000 animate-in fade-in"
        >
          {/* Capa 1: mesh gradient dorado + esmeralda animado */}
          <div style={{
          ...
          {/* Capa 2: glow orb pulsante detrás del nombre */}
          <div style={{
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            {/* Sello circular con monograma */}
            <div style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #C9A876',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 18, color: '#C9A876',
            }}>
              {monogram}
            </div>

            {/* Guest Name — Fraunces itálica (via --font-fraunces / --font-cormorant) */}
            <h2
              className="text-4xl sm:text-5xl font-light tracking-wide text-[#EDE9F4] leading-relaxed"
              style={{ fontFamily: 'var(--font-fraunces), var(--font-cormorant), serif', fontStyle: 'italic' }}
            >
              {guestNameDisplay}
            </h2>

            {/* Dress Code — pequeño, dorado, muy espaciado */}
            ...
            {/* Botón con borde dorado y vidrio esmerilado */}
            <button
              type="button"
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = '#C9A876'; e.currentTarget.style.color = '#000000'; }}
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#C9A876]/40 shadow-md ${isTicketMaximized ? 'bg-[#000000]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#FFFFFF]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            ...
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              ...
              <span className="text-[#0A0A0A] font-sans text-[10px] font-semibold tracking-wider uppercase">Pase</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#000000] relative">
          ...
          {/* Text Container */}
          <div className="px-8 pt-8 pb-12 text-left z-10 relative">
            <div className="w-12 h-12 rounded-full border border-[#C9A876]/40 flex items-center justify-center mb-6">
              <span className="text-[#C9A876] text-xl leading-none" style={{ marginTop: '-2px' }}>✧</span>
            </div>
            <p className="font-sans text-[10px] font-semibold uppercase tracking-[0.25em] text-[#C9A876] mb-4">
              {eyebrow}
            </p>
            <h1 className="text-[3.5rem] font-light text-[#FFFFFF] leading-[1.05] mb-6 flex items-center flex-wrap gap-x-3 gap-y-1" style={{ fontFamily: 'var(--font-cormorant), serif' }}>
              {em ? (
                <>
                  <span>{title.slice(0, title.indexOf(em)).trim()}</span>
                  <em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#C9A876' }}>&amp;</em> 
                  <span>{em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span>{title}</span>
              )}
            </h1>
            <div className="w-12 h-[2px] bg-[#C9A876] mb-6 opacity-80" />
            <p className="font-sans text-xs font-semibold text-[#9B92AF] tracking-[0.1em] uppercase">
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` · ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className="font-sans text-[10px] font-semibold text-[#C9A876] tracking-widest uppercase mt-4">
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full bg-[#0A0A0A] py-24 px-6 md:px-12 flex items-center justify-center">
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#000000] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            {/* TARJETA 1: CEREMONIA / CIVIL (Si est� cargada) */}
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-[#151219] border-l-2 border-l-[#C9A876] border-t border-r border-b border-white/5 rounded-md p-6 mb-8 shadow-lg transition-all hover:shadow-xl">
                <div>
                  <span className="font-sans text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C9A876] block mb-3">
                    {String(invitation.ceremoniaTitulo || "Ceremonia / Civil")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-medium text-[#FFFFFF] mb-4" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="flex items-center gap-2 text-[#9B92AF] font-medium text-sm sm:text-base mb-3">
                      <Clock className="w-4 h-4 text-[#C9A876]" /> {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="flex items-center gap-2 text-[#9B92AF] font-medium text-sm sm:text-base mb-4">
                      <MapPin className="w-4 h-4 text-[#C9A876]" /> {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 font-sans text-xs font-semibold tracking-wider text-[#FFFFFF] uppercase underline underline-offset-4 decoration-[#C9A876] hover:text-[#C9A876] transition-colors">
                      Ver mapa ceremonia ?
                    </a>
                  )}
                </div>
            ...
            {/* TARJETA 2: FIESTA / SAL�N (Siempre visible si se ingres� lugar o direcci�n) */}
            {(lugarNombre || direccion) && (
              <div className="bg-[#151219] border-l-2 border-l-[#C9A876] border-t border-r border-b border-white/5 rounded-md p-6 mb-12 shadow-lg transition-all hover:shadow-xl">
                <span className="font-sans text-[10px] font-semibold uppercase tracking-[0.2em] text-[#C9A876] block mb-3">
                  Fiesta / Sal�n
                </span>
                {lugarNombre && (
                ...
                {hora && (
                  <p className="flex items-center gap-2 text-[#9B92AF] font-medium text-sm sm:text-base mb-3">
                    <Clock className="w-4 h-4 text-[#C9A876]" /> {hora} hs
                  </p>
                )}
                {direccion && (
                  <p className="flex items-center gap-2 text-[#9B92AF] font-medium text-sm sm:text-base mb-4">
                    <MapPin className="w-4 h-4 text-[#C9A876]" /> {direccion}
                  </p>
                )}
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 font-sans text-xs font-semibold tracking-wider text-[#FFFFFF] uppercase underline underline-offset-4 decoration-[#C9A876] hover:text-[#C9A876] transition-colors">
                    Ver mapa fiesta ?
                  </a>
                )}
              </div>
              ...
              <div className="mt-16">
                ...
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex items-center py-5 border-b border-[#C9A876]/30 last:border-b-0">
                      {item.time && (
                        <span className="font-sans text-[1.1rem] sm:text-xl text-[#C9A876] font-bold tracking-[0.2em] w-28 sm:w-32 flex-shrink-0">
                          {item.time}
                        </span>
                      )}
                      <span className="font-sans text-[1.1rem] sm:text-xl text-[#FFFFFF] font-bold">
                        {item.title}
                      </span>
                    </div>
                  ))}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? true) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#0A0A0A] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#000000] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                ...
                {bothAccounts ? (
                    ...
                    {showBankDetails && (
                      <div className="grid grid-cols-1 gap-6 text-left w-full mt-4 items-stretch">
                        ...
                        {pagoTarjetaHabilitado && (
                          <div className="px-4 py-5 sm:p-5 bg-[#0A0A0A] space-y-2">
                            <div className="flex items-center gap-2 font-semibold text-[#FFFFFF] text-sm border-b border-[#C9A876]/20 pb-2">
                              <CreditCard className="w-5 h-5 text-[#C9A876]" strokeWidth={1.5} />
                              <span>{String((invitation as any).pagoTarjetaTitulo || "Pago de Tarjetas / Pases")}</span>
                            </div>
                            {Boolean((invitation as any).pagoTarjetaMensaje) && (
                              <p className="text-xs text-[#9B92AF] italic py-1">
                        ...
                        {regaloHabilitado && (
                          <div className="px-4 py-5 sm:p-5 bg-[#0A0A0A] space-y-2">
                            <div className="flex items-center gap-2 font-semibold text-[#FFFFFF] text-sm border-b border-[#C9A876]/20 pb-2">
                              <Gift className="w-5 h-5 text-[#C9A876]" strokeWidth={1.5} />
                  ...
                  <>
                    {(() => {
                      ...
                      return (
                        <>
                          <h2 className="mb-8 text-2xl text-[#FFFFFF]" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{activeBank.titulo}</h2>
                          <p style={{ opacity: 0.8, marginBottom: "20px" }} className="text-sm text-[#9B92AF]">
                          ...
                          {showBankDetails && (
                            <div className="w-full text-left mt-4">
                              <div className="px-4 py-5 sm:p-5 bg-[#0A0A0A] space-y-1">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full bg-[#0A0A0A] py-20 px-6 md:px-12">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-8">
