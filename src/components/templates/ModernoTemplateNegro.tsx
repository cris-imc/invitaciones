...
export function ModernoTemplateNegro({ invitation, guest, isPersonalized = false }: ModernoTemplateNegroProps) {
  ...
  return (
    ...
    >
      ...
      {!isCoverOpen && (
        <div 
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#000000', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden' }}
      ...
      <div className="d-right tpl">
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #C9A87614, transparent 70%), #0A0A0A" }}>
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #C9A87618, transparent 70%), #0A0A0A" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
