...
/**
 * AtelierDePapelTemplate.tsx
 *
 * Portado 1:1 desde el diseño "Atelier de Papel" aprobado en Claude Design
 * (colección Storytelling para boda: fondo casi negro sepia/kraft, acento
 * bronce/papel #7A6852, Cormorant Garamond + IBM Plex Mono, ticket
...
export function AtelierDePapelTemplate({ invitation, guest, isPersonalized = false }: AtelierDePapelTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F4EEE3";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#7A6852";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(122,104,82,.35), 0 18px 50px -30px #7A6852";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(122,104,82,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(122,104,82,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#2B2620" : "#7A6852";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#14120E", tinta: "#F4F1EA", acento: "#7A6852", clase: `${adpSerif.variable} ${adpMono.variable}`, fuente: "var(--adp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="adp-scroller">
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="adp-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #14120E 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="adp-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="adp-pan-sticky">
            <div data-strip="1" className="adp-strip">
              ...
              <div data-tone="dark" className="adp-panel adp-panel--center" style={{ background: "#181611", color: "#F4F1EA" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="adp-section" style={{ background: "#181611" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="adp-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="adp-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F4EEE3" : "#7A6852" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="adp-section" style={{ background: "#181611" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.sabor.tuLamina")} className="adp-section adp-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #181611 55%, #14120E 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#7A6852" guest={guest} />
      )}
...
const ADP_CSS = `
  ...
  a { color: #7A6852; text-decoration: none; }
  a:hover { color: #F4EEE3; }
  ...
  .adp-date-month { font-family: var(--adp-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #7A6852; padding-left: 12%; }

  .adp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .adp-divider-line { width: 52px; height: 1px; background: #7A6852; display: inline-block; }
  ...
  .adp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #2B2620, #7A6852, #7A6852, #F4EEE3, #2B2620); filter: saturate(.75); }
  .adp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .adp-medallion-label { font-family: var(--adp-serif), serif; font-size: 26px; line-height: 1; color: #F4EEE3; }
  .adp-medallion-label-sm { font-family: var(--adp-serif), serif; font-size: 16px; color: #F4EEE3; }
  .adp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .adp-medallion-sub--accent { color: #7A6852; }
  ...
  .adp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #7A6852; opacity: 0.4; font-family: var(--adp-mono), monospace; }

  .adp-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(122,104,82,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .adp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F4EEE3, transparent); animation: adpScan 6s linear infinite; pointer-events: none; }

  .adp-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .adp-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .adp-cd-num { font-family: var(--adp-serif), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6EFDD; display: inline-block; }
  .adp-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #7A6852; }
  .adp-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #14120E 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .adp-perf-strip--light { background: radial-gradient(circle at 6px 50%, #F4EEE3 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; }

  .adp-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(122,104,82,0.3), rgba(199,178,153,0.2), rgba(122,104,82,0.32), rgba(122,104,82,0.3)); filter: blur(80px); opacity: .4; animation: adpFoil 30s linear infinite; }
  ...
  .adp-accent-italic { font-style: italic; color: #7A6852; }
  .adp-phrase-accent { font-style: italic; color: #C7B299; }
  .adp-accent-serif { font-style: italic; color: #2B2620; font-family: var(--adp-serif), serif; }
  .adp-accent-serif-loc { font-style: italic; color: #7A6852; font-family: var(--adp-serif), serif; }
  ...
  .adp-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .adp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F4EEE3, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .adp-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(122,104,82,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .adp-rsvp-payment-total { color: #F4EEE3; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .adp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #7A6852; background: transparent; color: #7A6852; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .adp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(122,104,82,0.3); color: #F4F1EA; font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .adp-rsvp-input::placeholder { color: #6E6A5D; }
  .adp-rsvp-input:focus { outline: none; border-bottom-color: #7A6852; }
  .adp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--adp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #7A6852, #5C4C3C); border: 1px solid #7A6852; color: #14120E; cursor: pointer; }
  .adp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .adp-rsvp-btn--ghost { background: transparent; color: #7A6852; }
  .adp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #7A6852; margin: 0; }
  ...
  .adp-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(122,104,82,0.18); }
  ...
  .adp-bank-copy { flex-shrink: 0; font-family: var(--adp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #7A6852; background: transparent; color: #7A6852; cursor: pointer; }
  .adp-bank-copy:hover { background: rgba(122,104,82,0.12); }
  ...
  .adp-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(122,104,82,0.3); padding-bottom: 12px; }
  ...
  .adp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(122,104,82,0.3); color: #F4F1EA; font-family: var(--adp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .adp-song-input::placeholder { color: #6E6A5D; }
  .adp-song-input:focus { outline: none; border-bottom-color: #7A6852; }
  .adp-song-sep { color: #8A8577; flex-shrink: 0; }
  .adp-song-submit { flex-shrink: 0; background: none; border: none; color: #7A6852; font-family: var(--adp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .adp-song-submit:hover { color: #F4EEE3; }
  .adp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .adp-song-error { font-size: 10px; color: #7A6852; margin-top: 6px; }
  ...
  .adp-quiz-opt { font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(122,104,82,0.4); background: transparent; color: #7A6852; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .adp-quiz-opt:disabled { cursor: default; }
  .adp-quiz-opt--picked { background: #7A6852; border-color: #7A6852; color: #14120E; }
  .adp-quiz-opt--correct { background: #7A6852; border-color: #7A6852; color: #14120E; }
  .adp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .adp-quiz-result { padding-top: 18px; border-top: 1px solid rgba(122,104,82,0.2); }
  .adp-quiz-result-score { margin: 0 0 6px; font-family: var(--adp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F4EEE3; }
  .adp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .adp-final-card { border: 1px solid #7A6852; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .adp-final-names { font-family: var(--adp-serif), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F4EEE3; }
  ...
  .adp-replay { cursor: pointer; color: #7A6852; }
  .adp-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .adp-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(122,104,82,0.14); }
  .adp-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .adp-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(122,104,82,0.16); position: relative; }
  .adp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F4EEE3, #7A6852); transition: height 260ms linear; display: block; }
  .adp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #7A6852; transition: color 500ms ease; }
  ...
  .adp-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: #F4EEE3; }
  .adp-cover-half--bottom .adp-cover-inner { top: auto; bottom: 0; }
  /* Textura de papel rayado -- motivo decorativo propio de esta tapa,
     nunca el sunburst radial de Guest Pass VIP. */
  .adp-cover-glow { position: absolute; inset: 0; background-image: repeating-linear-gradient(0deg, rgba(122,104,82,.16) 0 1px, transparent 1px 5px), repeating-linear-gradient(90deg, rgba(122,104,82,.12) 0 1.5px, transparent 1.5px 46px); }
  .adp-cover-paper { position: absolute; width: 66%; height: 50%; left: 17%; top: 22%; background: #E3D5C0; border-radius: 8px; box-shadow: 0 20px 40px rgba(43,38,32,.18); transform: rotate(-3deg); }
  .adp-cover-paper--2 { width: 62%; height: 46%; left: 19%; top: 24%; background: #C7B299; box-shadow: 0 14px 28px rgba(43,38,32,.14); transform: rotate(2deg); }
  ...
  .adp-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #7A6852; }
  .adp-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .adp-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #7A6852; }
  ...
  .adp-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#7A6852, transparent); display: block; }
  ...
  .adp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #7A6852; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
