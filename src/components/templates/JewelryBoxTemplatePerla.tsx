...
/**
 * JewelryBoxTemplatePerla.tsx (variante de color "Perla")
 *
 * Portado 1:1 desde el diseño "Jewelry Box" (Colección Storytelling, mis
 * quince años, variante "Perla": fondo gris perlado oscuro, acento lavanda perlado + plata,
 * Cormorant Garamond + IBM Plex Mono, medallón de texto "15" -- sin ícono --
 * en vez de tiara/corona, tarjeta-cofre perforada, scroll horizontal
...
interface JewelryBoxTemplateProps {
...
export function JewelryBoxTemplatePerla({ invitation, guest, isPersonalized = false }: JewelryBoxTemplateProps) {
  const tx = useTextos();
  ...
  // plano, segunda mitad en nude itálico, partiendo por la mitad de la
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F1EFF4";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#BDB4CC";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(168,162,180,.35), 0 18px 50px -30px #BDB4CC";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#D6D2DC" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(168,162,180,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(168,162,180,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#D6D2DC" : "#BDB4CC";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#201E26", tinta: "#F4F1EA", acento: "#BDB4CC", clase: `${jwbCormorant.variable} ${jwbMono.variable}`, fuente: "var(--jwb-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${jwbCormorant.variable} ${jwbMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#201E26",
        fontFamily: "var(--jwb-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{JWB_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="jwb-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="jwb-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17161C 0%, #33303C 55%, #201E26 100%)" }}>
          ...
          <div className="jwb-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="jwb-date-num jwb-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="jwb-divider">
            ...
            <div className="jwb-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="32,30,38" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="32,30,38" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="jwb-section jwb-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #4C4858 0%, #2A2732 55%, #201E26 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="jwb-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1D1B22 0%, #101015 52%, #201E26 100%)" }}>
          ...
          <h2 ref={phraseRef} className="jwb-phrase" style={{ fontSize: fraseFontSize }}>
            {fraseWords.map((w, i) => (
              ...
              <span key={i}>
                <span data-w="1" className={i >= fraseAccentStart ? "jwb-accent-italic" : undefined}>
                  {w}
                </span>{" "}
              </span>
            ))}
          </h2>
          <div data-xin="1" data-delay="120" data-dist="90" className="jwb-divider" style={{ position: "relative" }}>
            <span className="jwb-divider-line jwb-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
          </div>
        </section>
        )}
          ...
          <div className="jwb-pan-sticky">
            <div data-strip="1" className="jwb-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="jwb-panel jwb-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <h2 className="jwb-panel-title">
                    ...
                    {ceremoniaDireccion && <><br /><span className="jwb-accent-serif">{ceremoniaDireccion}</span></>}
                  </h2>
              ...
              <div id="details" data-tone="light" className="jwb-panel jwb-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="jwb-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="jwb-crono-row">
                        <span className="jwb-crono-time">{item.time || ""}</span>
                        <span className="jwb-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="jwb-panel jwb-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="jwb-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#D6D2DC" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#D6D2DC" />
                  </svg>
              ...
              <div data-tone="dark" className="jwb-panel jwb-panel--center" style={{ background: "#33303C", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="jwb-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17161C 0%, #33303C 60%, #201E26 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="jwb-rsvp">
              <JwbRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="jwb-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="jwb-pan-sticky">
            <div data-strip="1" className="jwb-strip">
                ...
                <div key={pageIndex} data-tone="light" className="jwb-panel jwb-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="jwb-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`jwb-mosaic-cell${i === 0 ? " jwb-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="jwb-seguir jwb-seguir--split">
                    ...
                    <span className="jwb-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="jwb-section" style={{ background: "#33303C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="jwb-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="jwb-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F1EFF4" : "#BDB4CC" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="jwb-song-wrap">
              <JwbSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="jwb-section" style={{ background: "#33303C" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="jwb-section" style={{ background: "#33303C" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pieza" className="jwb-section jwb-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17161C 0%, #33303C 55%, #201E26 100%)" }}>
      ...
      <div ref={railRef} className="jwb-rail">
        ...
        <span ref={railLabelRef} className="jwb-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="jwb-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="jwb-cover-half jwb-cover-half--top">
          <JwbCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="jwb-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="jwb-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
...
function JwbCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="jwb-cd-box">
      <span ref={refEl} className="jwb-cd-num">—</span>
      <span className="jwb-cd-label">{label}</span>
    </div>
  );
}

function JwbDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="jwb-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="jwb-dot" />
      ))}
    </div>
  );
}

// Medallón con el número "15" en Cormorant Garamond en el centro -- en vez de
// un ícono SVG (tiara/corona/VIP) como en el resto de la colección. Mismo
// patrón que el `label`/`sub` de texto de Medallion en GuestPassVipTemplate.tsx,
// simplificado (sin prop `title`, porque acá el label es siempre "15").
function JwbMedallion({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      <div className="jwb-medallion-ring" style={{ animation: spin === "none" ? "none" : `gpRing ${ringDuration}s linear infinite` }} />
      <div className="jwb-medallion-core">
        <span className={compact ? "jwb-medallion-label-sm" : "jwb-medallion-label"}>{label}</span>
        {sub && <span className="jwb-medallion-sub jwb-medallion-sub--accent">{sub}</span>}
      </div>
      {arcId && (
        <svg viewBox="0 0 100 100" className="jwb-medallion-arc" style={{ animation: spin === "reverse" ? "gpRingRev 32s linear infinite" : "gpRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="jwb-medallion-arc-text">
...
function JwbCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="jwb-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="jwb-bank-row-label">{label}</span>
    ...
    <>
      <div className="jwb-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          <div className="jwb-rsvp-row jwb-rsvp-row--payment">
      ...
      {error && <p className="jwb-rsvp-error">{error}</p>}
...
function JwbSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="jwb-song">
      ...
      {error && <p className="jwb-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="jwb-song-list">
...
function JwbQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: JwbQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="jwb-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="jwb-quiz-q">
            <span className="jwb-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="jwb-quiz-q-text">{q.pregunta}</p>
            <div className="jwb-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " jwb-quiz-opt--correct";
                  else if (chosen) stateClass = " jwb-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " jwb-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`jwb-quiz-opt${stateClass}`}
...
function JwbInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="jwb-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="jwb-bank-row-label">{label}</span>
...
function JwbCoverHalf({
  ...
  return (
    <div className="jwb-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#BDB4CC" tintColor2="#201E26" effect="enfoque" scrimColorRgb="32,30,38" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#BDB4CC" tintColor2="#201E26" effect="enfoque" scrimColorRgb="32,30,38" />
        </div>
      )}
      ...
      <div className="jwb-cover-content">
        ...
        <div className="jwb-cover-center">
          ...
          <h1 ref={namesRef} className="jwb-cover-names">{namesTitle}</h1>
          <span className="jwb-cover-rule" />
          <span className="jwb-cover-date">{fechaCorta}</span>
        </div>
        <div className="jwb-cover-bottom">
          <div ref={perfRef} className="jwb-perf-strip jwb-perf-strip--reveal jwb-perf-strip--cover" />
          <div className="jwb-cover-facts">
...
// ---------------------------------------------------------------------
const JWB_CSS = `
  .jwb-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .jwb-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #BDB4CC; text-decoration: none; }
  a:hover { color: #F1EFF4; }
  ...
  @keyframes gemFlash { 0%,100% { opacity: 0; } 8% { opacity: 1; } 16% { opacity: 0; } }
  @media (prefers-reduced-motion: reduce) { .jwb-scroller * { animation: none !important; } }
  ...
  .jwb-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #201E26; }
  ...
  @media (max-width: 767px) {
    .jwb-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .jwb-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(189,180,204,.3); }
    .jwb-hero-photo-kicker { bottom: 40px; }
    .jwb-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .jwb-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .jwb-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .jwb-date-num { font-family: var(--jwb-cormorant), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .jwb-date-num--right { text-align: right; line-height: 0.86; }
  .jwb-date-month { font-family: var(--jwb-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #BDB4CC; padding-left: 12%; }

  .jwb-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .jwb-divider-line { width: 52px; height: 1px; background: #BDB4CC; display: inline-block; }
  ...
  .jwb-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .jwb-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .jwb-gem { position: absolute; inset: -6px; border-radius: 50%; box-shadow: 0 0 24px 4px rgba(189,180,204,.7); animation: gemFlash 4s ease-in-out infinite; pointer-events: none; }
  .jwb-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #5C566C, #3A3644, #BDB4CC, #F1EFF4, #5C566C); filter: saturate(.75); }
  .jwb-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .jwb-medallion-label { font-family: var(--jwb-cormorant), serif; font-size: 26px; line-height: 1; color: #F1EFF4; }
  .jwb-medallion-label-sm { font-family: var(--jwb-cormorant), serif; font-size: 16px; color: #F1EFF4; }
  .jwb-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .jwb-medallion-sub--accent { color: #BDB4CC; }
  .jwb-medallion-arc { position: absolute; inset: -14%; }
  .jwb-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #BDB4CC; opacity: 0.4; font-family: var(--jwb-mono), monospace; }

  .jwb-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(168,162,180,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .jwb-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F1EFF4, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .jwb-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .jwb-cd-box { border: 1px solid #2C2A34; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .jwb-cd-num { font-family: var(--jwb-cormorant), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #E2DEE8; display: inline-block; }
  .jwb-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #BDB4CC; }
  .jwb-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #201E26 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .jwb-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .jwb-perf-strip--cover { opacity: .9; }

  .jwb-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(92,86,108,0.4), rgba(214,210,220,0.24), rgba(189,180,204,0.36), rgba(92,86,108,0.4)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  .jwb-phrase { margin: 0; position: relative; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: clamp(50px, 15vw, 96px); line-height: 0.92; letter-spacing: -0.03em; }
  .jwb-accent-italic { font-style: italic; color: #BDB4CC; }
  .jwb-accent-serif { font-style: italic; color: #D6D2DC; font-family: var(--jwb-cormorant), serif; }
  .jwb-accent-serif-2 { color: #D6D2DC; }
  .jwb-h2 { margin: 0; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .jwb-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .jwb-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .jwb-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .jwb-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .jwb-panel-title { margin: 0; position: relative; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .jwb-panel-title-md { margin: 0; position: relative; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  .jwb-panel-title-sm { margin: 0; font-family: var(--jwb-cormorant), serif; font-size: clamp(34px, 10vw, 52px); line-height: 1; }
  .jwb-panel-block { position: relative; display: flex; flex-direction: column; gap: 12px; }
  .jwb-mini-label { font-size: 9px; letter-spacing: 0.26em; color: #7C7768; }
  .jwb-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  .jwb-facts { position: relative; display: flex; flex-direction: column; gap: 12px; font-size: 11px; letter-spacing: 0.14em; color: #4A473F; }
  .jwb-facts-row { display: flex; justify-content: space-between; border-bottom: 1px solid #D6D1C4; padding-bottom: 10px; }
  .jwb-facts-row--last { border-bottom: none; padding-bottom: 0; }
  .jwb-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #D6D2DC; margin-top: auto; }
  .jwb-seguir--split { justify-content: space-between; }
  .jwb-side-hint { display: inline-block; animation: gpSide 2.2s ease-in-out infinite; }
  .jwb-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .jwb-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #D6D2DC; }

  .jwb-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .jwb-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .jwb-crono-time { font-family: var(--jwb-mono), monospace; color: #D6D2DC; min-width: 42px; }
  .jwb-crono-title { flex: 1; }

  .jwb-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .jwb-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .jwb-stub { position: relative; overflow: hidden; border: 1px solid #2C2A34; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .jwb-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .jwb-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .jwb-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .jwb-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F1EFF4, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .jwb-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--jwb-mono), monospace; }
  .jwb-rsvp-rows { display: flex; flex-direction: column; }
  .jwb-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(189,180,204,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .jwb-rsvp-row > span:first-child { flex-shrink: 0; }
  .jwb-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .jwb-rsvp-row--payment { align-items: flex-start; }
  .jwb-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .jwb-rsvp-payment-total { color: #F1EFF4; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .jwb-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .jwb-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .jwb-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #BDB4CC; background: transparent; color: #BDB4CC; font-size: 14px; line-height: 1; cursor: pointer; }
  .jwb-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .jwb-rsvp-stepper span { font-family: var(--jwb-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .jwb-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(189,180,204,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .jwb-rsvp-input::placeholder { color: #6E6A5D; }
  .jwb-rsvp-input:focus { outline: none; border-bottom-color: #BDB4CC; }
  .jwb-rsvp-btn { width: 100%; padding: 16px; font-family: var(--jwb-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #BDB4CC, #D6D2DC); border: 1px solid #BDB4CC; color: #0B0B10; cursor: pointer; }
  .jwb-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .jwb-rsvp-btn--ghost { background: transparent; color: #BDB4CC; }
  .jwb-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #BDB4CC; margin: 0; }
  .jwb-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .jwb-rsvp-declined-text { margin: 0; font-family: var(--jwb-cormorant), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .jwb-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .jwb-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .jwb-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  .jwb-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }

  .jwb-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .jwb-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .jwb-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .jwb-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }

  .jwb-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .jwb-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(189,180,204,0.18); }
  .jwb-bank-row:last-child { border-bottom: none; }
  .jwb-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .jwb-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .jwb-bank-copy { flex-shrink: 0; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #BDB4CC; background: transparent; color: #BDB4CC; cursor: pointer; }
  .jwb-bank-copy:hover { background: rgba(189,180,204,0.12); }

  .jwb-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .jwb-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: gpEq 1.1s ease-in-out infinite; display: inline-block; }
  .jwb-song-wrap { font-family: var(--jwb-mono), monospace; }

  .jwb-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .jwb-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(189,180,204,0.3); padding-bottom: 12px; }
  .jwb-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .jwb-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(189,180,204,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .jwb-song-input::placeholder { color: #6E6A5D; }
  .jwb-song-input:focus { outline: none; border-bottom-color: #BDB4CC; }
  .jwb-song-sep { color: #8A8577; flex-shrink: 0; }
  .jwb-song-submit { flex-shrink: 0; background: none; border: none; color: #BDB4CC; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .jwb-song-submit:hover { color: #F1EFF4; }
  .jwb-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .jwb-song-error { font-size: 10px; color: #BDB4CC; margin-top: 6px; }
  .jwb-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .jwb-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--jwb-mono), monospace; }
  .jwb-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .jwb-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .jwb-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .jwb-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .jwb-quiz-q-num { font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .jwb-quiz-q-text { margin: 0; font-family: var(--jwb-cormorant), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .jwb-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .jwb-quiz-opt { font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(189,180,204,0.4); background: transparent; color: #BDB4CC; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .jwb-quiz-opt:disabled { cursor: default; }
  .jwb-quiz-opt--picked { background: #BDB4CC; border-color: #BDB4CC; color: #0B0B10; }
  .jwb-quiz-opt--correct { background: #BDB4CC; border-color: #BDB4CC; color: #0B0B10; }
  .jwb-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .jwb-quiz-result { padding-top: 18px; border-top: 1px solid rgba(189,180,204,0.2); }
  .jwb-quiz-result-score { margin: 0 0 6px; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F1EFF4; }
  .jwb-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .jwb-final-card { border: 1px solid #BDB4CC; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .jwb-final-names { font-family: var(--jwb-cormorant), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F1EFF4; }
  .jwb-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .jwb-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .jwb-replay { cursor: pointer; color: #BDB4CC; }
  .jwb-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .jwb-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(168,162,180,0.14); }
  .jwb-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .jwb-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(168,162,180,0.16); position: relative; }
  .jwb-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F1EFF4, #BDB4CC); transition: height 260ms linear; display: block; }
  .jwb-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #BDB4CC; transition: color 500ms ease; }

  .jwb-cover { position: absolute; inset: 0; z-index: 5; }
  .jwb-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .jwb-cover-half--top { top: 0; }
  .jwb-cover-half--bottom { bottom: 0; }
  .jwb-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #3A3644 0%, #33303C 46%, #201E26 100%); }
  .jwb-cover-half--bottom .jwb-cover-inner { top: auto; bottom: 0; }
  .jwb-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(92,86,108,0.6), rgba(214,210,220,0.35), rgba(189,180,204,0.55), rgba(92,86,108,0.6)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  /* Destellos (ver mockup original) -- mismo recurso decorativo que el resto
     de la colección, en el tono nude/dorado de esta plantilla. */
  @keyframes jwbSparkle { 0%, 100% { opacity: 0; transform: scale(.6); } 50% { opacity: 1; transform: scale(1); } }
  .jwb-sparkle { position: absolute; border-radius: 50%; background: #fff; box-shadow: 0 0 8px #fff; animation: jwbSparkle 2.6s ease-in-out infinite; }
  .jwb-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .jwb-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .jwb-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .jwb-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .jwb-cover-names { margin: 0; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F1EFF4; }
  .jwb-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#BDB4CC, transparent); display: block; }
  .jwb-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .jwb-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .jwb-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  .jwb-cover-cta { border: 1px solid #BDB4CC; background: linear-gradient(100deg, rgba(168,162,180,0.08), rgba(222,216,230,0.2), rgba(168,162,180,0.08)); color: #E2DEE8; font-family: var(--jwb-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .jwb-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .jwb-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(222,216,230,0.34), rgba(226,222,232,0.5), rgba(222,216,230,0.34)); color: #0B0B0F; }
  }
  ...
  .jwb-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .jwb-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #BDB4CC; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .jwb-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
