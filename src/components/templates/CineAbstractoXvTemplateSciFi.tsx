 ...
 *
 * Variante de color "Sci-Fi" de "Cine Abstracto XV" (Colección
...
export function CineAbstractoXvTemplateSciFi({ invitation, guest, isPersonalized = false }: CineAbstractoXvTemplateProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#18D2E0";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(24,210,224,.35), 0 18px 50px -30px #18D2E0";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#E01AA0" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(90,170,180,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(90,170,180,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#E01AA0" : "#18D2E0";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#06030F", tinta: "#F4F1EA", acento: "#18D2E0", clase: `${cxvArchivoBlack.variable} ${cxvMono.variable}`, fuente: "var(--cxv-mono), monospace" }}
      />
    );
  }
            ...
            <div className="cxv-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="6,3,15" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="6,3,15" />
                </div>
              )}
            </div>
 ...
 data-tone="dark" data-screen-label="Countdown" className="cxv-section cxv-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A0A2A 0%, #0C0616 55%, #06030F 100%)" }}>
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cxv-panel cxv-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cxv-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#E01AA0" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#E01AA0" />
                  </svg>
        ...
        <section data-tone="dark" data-screen-label="Tu entrada" className="cxv-section cxv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #150A2A 0%, #0B0620 55%, #06030F 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#18D2E0" guest={guest} />
      )}
...
const CXV_CSS = `
  ...
  a { color: #18D2E0; text-decoration: none; }
  ...
  .cxv-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #06030F; }
  ...
  @media (min-width: 768px) {
    .cxv-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(224,26,160,.3); }
  ...
  .cxv-date-month { font-family: var(--cxv-archivo), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #18D2E0; padding-left: 12%; }

  .cxv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cxv-divider-line { width: 52px; height: 1px; background: #18D2E0; display: inline-block; }
  ...
  .cxv-cal-link:hover { color: #18D2E0; }
  ...
  .cxv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #E01AA0, #3A0A32, #18D2E0, #F5F1E8, #E01AA0); filter: saturate(.75); }
  .cxv-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0A0616; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .cxv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #18D2E0; }
  .cxv-medallion-arc { position: absolute; inset: -14%; }
  .cxv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #18D2E0; font-family: var(--cxv-mono), monospace; }

  .cxv-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(90,170,180,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  ...
  .cxv-cd-box { border: 1px solid #2A1638; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .cxv-cd-num { font-family: var(--cxv-archivo), sans-serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #DFFBFF; display: inline-block; }
  .cxv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #18D2E0; }
  .cxv-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #06030F 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .cxv-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(224,26,160,0.3), rgba(24,210,224,0.2), rgba(24,210,224,0.32), rgba(224,26,160,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .cxv-accent-italic { font-style: italic; color: #E01AA0; }
  .cxv-accent-serif { font-style: italic; color: #E01AA0; font-family: var(--cxv-archivo), sans-serif; }
  .cxv-accent-serif-2 { color: #E01AA0; }
  ...
  .cxv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #E01AA0; margin-top: auto; }
  ...
  .cxv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #E01AA0; }
  ...
  .cxv-crono-time { font-family: var(--cxv-mono), monospace; color: #E01AA0; min-width: 42px; }
  ...
  .cxv-stub { position: relative; overflow: hidden; border: 1px solid #2A1638; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cxv-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(90,170,180,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cxv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #18D2E0; background: transparent; color: #18D2E0; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cxv-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(90,170,180,0.3); color: #F4F1EA; font-family: var(--cxv-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cxv-rsvp-input::placeholder { color: #6E6A5D; }
  .cxv-rsvp-input:focus { outline: none; border-bottom-color: #18D2E0; }
  .cxv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cxv-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #18D2E0, #E01AA0); border: 1px solid #18D2E0; color: #0B0B10; cursor: pointer; }
  .cxv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cxv-rsvp-btn--ghost { background: transparent; color: #18D2E0; }
  .cxv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #E01AA0; margin: 0; }
  ...
  .cxv-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(90,170,180,0.18); }
  ...
  .cxv-bank-copy { flex-shrink: 0; font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #18D2E0; background: transparent; color: #18D2E0; cursor: pointer; }
  .cxv-bank-copy:hover { background: rgba(24,210,224,0.12); }
  ...
  .cxv-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(90,170,180,0.3); padding-bottom: 12px; }
  ...
  .cxv-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(90,170,180,0.3); color: #F4F1EA; font-family: var(--cxv-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cxv-song-input::placeholder { color: #6E6A5D; }
  .cxv-song-input:focus { outline: none; border-bottom-color: #18D2E0; }
  .cxv-song-sep { color: #8A8577; flex-shrink: 0; }
  .cxv-song-submit { flex-shrink: 0; background: none; border: none; color: #18D2E0; font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cxv-song-submit:hover { color: #F5F1E8; }
  .cxv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cxv-song-error { font-size: 10px; color: #E01AA0; margin-top: 6px; }
  ...
  .cxv-quiz-opt { font-family: var(--cxv-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(24,210,224,0.4); background: transparent; color: #18D2E0; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cxv-quiz-opt:disabled { cursor: default; }
  .cxv-quiz-opt--picked { background: #18D2E0; border-color: #18D2E0; color: #0B0B10; }
  .cxv-quiz-opt--correct { background: #18D2E0; border-color: #18D2E0; color: #0B0B10; }
  .cxv-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cxv-quiz-result { padding-top: 18px; border-top: 1px solid rgba(24,210,224,0.2); }
  ...
  .cxv-final-card { border: 1px solid #18D2E0; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .cxv-replay { cursor: pointer; color: #18D2E0; }
  .cxv-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cxv-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(90,170,180,0.14); }
  .cxv-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cxv-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(90,170,180,0.16); position: relative; }
  .cxv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F5F1E8, #18D2E0); transition: height 260ms linear; display: block; }
  .cxv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #18D2E0; transition: color 500ms ease; }
  ...
  .cxv-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #100722 0%, #0B0620 46%, #06030F 100%); }
  .cxv-cover-half--bottom .cxv-cover-inner { top: auto; bottom: 0; }
  .cxv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(224,26,160,0.4), rgba(24,210,224,0.3), rgba(24,210,224,0.5), rgba(224,26,160,0.4)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .cxv-bulb { width: 5px; height: 5px; border-radius: 50%; background: #18D2E0; animation: cxvBulbFlicker 1.4s ease-in-out infinite; display: inline-block; }
  ...
  .cxv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#18D2E0, transparent); display: block; }
  ...
  .cxv-cover-cta { border: 1px solid #18D2E0; background: linear-gradient(100deg, rgba(90,170,180,0.08), rgba(190,240,250,0.2), rgba(90,170,180,0.08)); color: #DFFBFF; font-family: var(--cxv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cxv-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cxv-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(190,240,250,0.34), rgba(246,239,221,0.5), rgba(190,240,250,0.34)); color: #0B0B0F; }
  }
  ...
  .cxv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #18D2E0; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
