...
/**
 * CineAbstractoTemplateBlancoNegroPlata.tsx
 *
 * Variante de color "Blanco y Negro con Plata" de la familia Cine Abstracto (Casamiento).
...
export function CineAbstractoTemplateBlancoNegroPlata({ invitation, guest, isPersonalized = false }: CineAbstractoTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F3F4F5";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#B9C0C6";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(185,192,198,.35), 0 18px 50px -30px #B9C0C6";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#5B6066" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(185,192,198,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(185,192,198,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#5B6066" : "#B9C0C6";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#08080B", tinta: "#F4F1EA", acento: "#B9C0C6", clase: `${cabSerif.variable} ${cabMono.variable}`, fuente: "var(--cab-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="cab-scroller">
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cab-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #181819 0%, #0C0B11 52%, #08080B 100%)" }}>
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cab-panel cab-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cab-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#5B6066" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#5B6066" />
                  </svg>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cab-section" style={{ background: "#0B0B10" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="cab-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cab-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F3F4F5" : "#B9C0C6" }} />
              ))}
            </div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#B9C0C6" guest={guest} />
      )}
...
const CAB_CSS = `
  ...
  a { color: #B9C0C6; text-decoration: none; }
  a:hover { color: #F3F4F5; }
  ...
  .cab-date-month { font-family: var(--cab-serif), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B9C0C6; padding-left: 12%; }

  .cab-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cab-divider-line { width: 52px; height: 1px; background: #B9C0C6; display: inline-block; }
  ...
  .cab-cal-link:hover { color: #B9C0C6; }
  ...
  .cab-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: repeating-conic-gradient(#0C0C11 0deg 7deg, #B9C0C6 7deg 10deg, #0C0C11 10deg 16deg, #F3F4F5 16deg 19deg); }
  ...
  .cab-medallion-label { font-family: var(--cab-serif), serif; font-size: 26px; line-height: 1; color: #F3F4F5; }
  .cab-medallion-label-sm { font-family: var(--cab-serif), serif; font-size: 16px; color: #F3F4F5; }
  .cab-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .cab-medallion-sub--accent { color: #B9C0C6; }
  ...
  .cab-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B9C0C6; opacity: 0.4; font-family: var(--cab-mono), monospace; }

  .cab-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(185,192,198,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .cab-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F3F4F5, transparent); animation: cabScan 6s linear infinite; pointer-events: none; }

  .cab-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .cab-cd-box { border: 1px solid #2E3034; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  ...
  .cab-accent-italic { font-style: italic; color: #B9C0C6; }
  .cab-accent-serif { font-style: italic; color: #5B6066; font-family: var(--cab-serif), serif; }
  .cab-accent-serif-2 { color: #5B6066; }
  ...
  .cab-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #5B6066; margin-top: auto; }
  ...
  .cab-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #5B6066; }
  ...
  .cab-crono-time { font-family: var(--cab-mono), monospace; color: #5B6066; min-width: 42px; }
  ...
  .cab-stub { position: relative; overflow: hidden; border: 1px solid #2E3034; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cab-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F3F4F5, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .cab-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(185,192,198,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cab-rsvp-payment-total { color: #F3F4F5; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .cab-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B9C0C6; background: transparent; color: #B9C0C6; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cab-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(185,192,198,0.3); color: #F4F1EA; font-family: var(--cab-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cab-rsvp-input::placeholder { color: #6E6A5D; }
  .cab-rsvp-input:focus { outline: none; border-bottom-color: #B9C0C6; }
  .cab-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cab-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B9C0C6, #5B6066); border: 1px solid #B9C0C6; color: #0B0B10; cursor: pointer; }
  .cab-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cab-rsvp-btn--ghost { background: transparent; color: #B9C0C6; }
  .cab-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B9C0C6; margin: 0; }
  ...
  .cab-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(185,192,198,0.18); }
  ...
  .cab-bank-copy { flex-shrink: 0; font-family: var(--cab-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B9C0C6; background: transparent; color: #B9C0C6; cursor: pointer; }
  .cab-bank-copy:hover { background: rgba(185,192,198,0.12); }
  ...
  .cab-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(185,192,198,0.3); padding-bottom: 12px; }
  ...
  .cab-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(185,192,198,0.3); color: #F4F1EA; font-family: var(--cab-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cab-song-input::placeholder { color: #6E6A5D; }
  .cab-song-input:focus { outline: none; border-bottom-color: #B9C0C6; }
  .cab-song-sep { color: #8A8577; flex-shrink: 0; }
  .cab-song-submit { flex-shrink: 0; background: none; border: none; color: #B9C0C6; font-family: var(--cab-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cab-song-submit:hover { color: #F3F4F5; }
  .cab-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cab-song-error { font-size: 10px; color: #B9C0C6; margin-top: 6px; }
  ...
  .cab-quiz-opt { font-family: var(--cab-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(185,192,198,0.4); background: transparent; color: #B9C0C6; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cab-quiz-opt:disabled { cursor: default; }
  .cab-quiz-opt--picked { background: #B9C0C6; border-color: #B9C0C6; color: #0B0B10; }
  .cab-quiz-opt--correct { background: #B9C0C6; border-color: #B9C0C6; color: #0B0B10; }
  .cab-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cab-quiz-result { padding-top: 18px; border-top: 1px solid rgba(185,192,198,0.2); }
  .cab-quiz-result-score { margin: 0 0 6px; font-family: var(--cab-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F3F4F5; }
  .cab-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .cab-final-card { border: 1px solid #B9C0C6; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .cab-replay { cursor: pointer; color: #B9C0C6; }
  .cab-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cab-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(185,192,198,0.14); }
  .cab-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cab-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(185,192,198,0.16); position: relative; }
  .cab-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F3F4F5, #B9C0C6); transition: height 260ms linear; display: block; }
  .cab-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B9C0C6; transition: color 500ms ease; }
  ...
  .cab-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #18181B 0%, #0B0B10 46%, #08080B 100%); }
  .cab-cover-half--bottom .cab-cover-inner { top: auto; bottom: 0; }
  .cab-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(150,155,160,0.7), rgba(40,42,45,0.5), rgba(185,192,198,0.65), rgba(150,155,160,0.7)); filter: blur(64px); opacity: .62; animation: cabFoil 26s linear infinite; }
  ...
  .cab-cover-grain { position: absolute; inset: 0; background-image: radial-gradient(rgba(243,244,245,.09) .6px, transparent .6px); background-size: 3px 3px; mix-blend-mode: screen; animation: cabGrainFlicker 2.4s steps(6) infinite; }
  ...
  .cab-cover-sprockets { position: absolute; left: 0; right: 0; height: 7px; background-image: repeating-linear-gradient(90deg, rgba(243,244,245,.65) 0 7px, transparent 7px 18px); }
  ...
  .cab-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B9C0C6, transparent); display: block; }
  ...
  .cab-cover-cta { border: 1px solid #B9C0C6; background: linear-gradient(100deg, rgba(185,192,198,0.08), rgba(243,244,245,0.2), rgba(185,192,198,0.08)); color: #F4EEE6; font-family: var(--cab-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cab-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cab-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(243,244,245,0.34), rgba(243,244,245,0.55), rgba(243,244,245,0.34)); color: #0B0B0F; }
  }
  ...
  .cab-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B9C0C6; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
