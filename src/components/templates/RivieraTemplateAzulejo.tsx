 ...
 * Plantillas Casamiento -> Marina & Diego): fondo lino cálido (#FAF1E4) +
 * tinta café (#3E2E20) + acento terracota (#3E7C8C, variable por color),
...
interface RivieraTemplateAzulejoProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#3E7C8C]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#3E7C8C] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-xs sm:text-sm font-mono text-[#3E2E20] break-all">{value}</span>
      </div>
      <button 
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#3E7C8C]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#3E7C8C] uppercase tracking-wider mb-0.5">{label}</span>
        <span className="text-sm font-medium text-[#3E2E20] break-words">{value}</span>
      </div>
    </div>
  );
}
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        <h3 style={{ fontFamily: "var(--font-cormorant), serif", fontSize: "2rem", fontStyle: "italic", color: "#3E2E20" }}>
          ¡Juego Completado!
        </h3>
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#3E2E20" }}>
          RESPONDISTE {score} DE {preguntas.length} CORRECTAMENTE ({percent}%)
        </p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#8A7862" }}>Guardando tus resultados...</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(62,46,32,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(62,46,32,0.12)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#3E7C8C] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#3E2E20" }}>
                  El promedio global de aciertos del resto de los invitados ({stats.count}) es del <strong style={{ color: "#3E2E20" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#3E2E20] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function RivieraTemplateAzulejo({ invitation, guest, isPersonalized = false }: RivieraTemplateAzulejoProps) {
    ...
    >
      <style>{`
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(62,124,140, 0.35) !important;
        ...
        /* RSVP Custom Aesthetics for RivieraTemplateAzulejo */
        #rsvp.section.dark {
          background-color: #FAF1E4 !important;
          color: #3E2E20 !important;
        ...
        #rsvp.section.dark input {
          ...
          border: 1px solid rgba(62,124,140, 0.2) !important;
        ...
        #rsvp.section.dark .t-btn {
          ...
          background-color: #3E7C8C !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #3E7C8C !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #FAF1E4 !important; /* Matches light sections */
          color: #3E2E20 !important;
        ...
        #banco .copy-btn.copied {
          background-color: #3E2E20 !important;
          color: #FAF1E4 !important;
        }
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div style={{
            ...
            background: 'radial-gradient(50% 40% at 15% 15%, rgba(62,124,140,0.14), transparent), radial-gradient(45% 40% at 85% 80%, rgba(232,165,121,0.24), transparent)',
            backgroundSize: '160% 160%',
            animation: 'riviera-meshDrift 14s ease-in-out infinite',
          }} />
          <div style={{
            position: 'absolute', width: 220, height: 220, borderRadius: '50%',
            background: 'radial-gradient(circle, rgba(62,124,140,0.14), transparent 70%)',
          ...
          <IconArch className="riviera-doodle opacity-0 absolute" style={{ width: 30, height: 32, top: '9%', left: '11%', color: 'rgba(62,124,140,0.5)' }} />
          <IconSun className="riviera-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '15%', right: '13%', color: 'rgba(62,124,140,0.5)' }} />
          ...
          <IconSun className="riviera-doodle opacity-0 absolute" style={{ width: 14, height: 14, bottom: '26%', right: '17%', color: 'rgba(62,124,140,0.35)' }} />
          </div>
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <div className="riviera-seal opacity-0" style={{
              width: 44, height: 44, borderRadius: '50%', border: '1px solid #3E7C8C',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: '#3E7C8C',
            }}>
              {monogram}
            </div>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #3E7C8C', color: '#3E7C8C',
                background: 'rgba(62,124,140,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#3E7C8C'; e.currentTarget.style.color = '#FAF1E4'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(62,124,140,0.08)'; e.currentTarget.style.color = '#3E7C8C'; }}
            >
              ABRIR INVITACIÓN
            </button>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#3E7C8C]/40 shadow-md ${isTicketMaximized ? 'bg-[#FAF1E4]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#3E2E20]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#3E7C8C] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>Pase Especial</span>
                <span className="text-[#3E2E20] font-bold text-sm leading-none" style={{ fontFamily: 'var(--font-cormorant), serif' }}>{guest.name}</span>
              </div>
              <div className="flex flex-col items-end border-l border-[#3E7C8C]/20 pl-3">
                <span className="text-[#3E2E20] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#8A7862] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? 'Lugar' : 'Lugares'}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#3E7C8C]" />
      ...
      <aside className="d-left hide-mobile" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
          ...
          <IconSun style={{ position: "absolute", top: -12, right: -6, width: 24, height: 24, color: "#FAF1E4" }} />
        </div>
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#3E7C8C', margin: '6px 0 14px', animation: 'riviera-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#FAF1E4]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#FAF1E4] z-10 relative">
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#3E7C8C] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#3E2E20] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#3E7C8C' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#3E7C8C', margin: '0 0 20px', animation: 'riviera-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#8A7862] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#3E7C8C] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                Dress code: {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden" style={{ borderRadius: "0 0 50% 50% / 0 0 9% 9%" }}>
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #FAF1E4 100%)' }} />
            <div
            ...
            <div ref={heroGhostRef} className="pointer-events-none z-20" style={{ position: "absolute", width: 34, height: 34, borderRadius: "50%", background: "radial-gradient(circle, rgba(62,124,140,0.5), transparent 70%)", opacity: 0, transform: "translate(-50%,-50%)" }} />
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              ...
              <IconSun style={{ position: "absolute", top: -10, right: -8, width: 26, height: 26, color: "#FAF1E4" }} />
              <IconOlive style={{ position: "absolute", bottom: 6, left: -6, width: 22, height: 10, color: "#FAF1E4" }} />
            </div>
          </div>
        </div>
          ...
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #7A8F6B14, transparent 70%), #F5E9D8" }}>
            <div className="max-w-2xl mx-auto text-center">
              <TypewriterText 
                ...
                className="text-[#3E2E20] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8 bg-[#FAF1E4]" aria-hidden="true">
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #3E7C8C, transparent)' }} />
          <IconSun className="riviera-scroll-doodle opacity-0" style={{ width: 22, height: 22, color: '#3E7C8C' }} />
          <IconOlive className="riviera-scroll-doodle opacity-0" style={{ width: 20, height: 10, color: '#7A8F6B' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, #3E7C8C, transparent)' }} />
        </div>

        <SectionWrapper id="details" delay={150} className="w-full bg-[#FAF1E4] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#3E7C8C]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              <IconArch className="riviera-scroll-doodle opacity-0" style={{ width: 13, height: 14 }} />
              CUÁNDO Y DÓNDE
            </p>
            ...
            {(Boolean(invitation.ceremoniaHabilitada) || Boolean(invitation.ceremoniaNombre) || Boolean(invitation.ceremoniaDireccion)) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#3E7C8C] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8A7862] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    {String(invitation.ceremoniaTitulo || "CEREMONIA")}
                  </span>
                  {Boolean(invitation.ceremoniaNombre) && (
                    <h4 className="text-2xl sm:text-3xl font-light text-[#3E2E20] mb-3" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                      {String(invitation.ceremoniaNombre)}
                    </h4>
                  )}
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#8A7862]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#8A7862]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#3E7C8C] hover:text-[#3E2E20] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      Ver mapa ceremonia ↗
                    </a>
                  )}
                </div>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#3E7C8C] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#8A7862] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  FIESTA / SALÓN
                </span>
                {lugarNombre && (
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#3E7C8C] hover:text-[#3E2E20] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                    Ver mapa fiesta ↗
                  </a>
                )}
              </div>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#3E7C8C]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  <IconOlive className="riviera-scroll-doodle opacity-0" style={{ width: 18, height: 9 }} />
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#3E7C8C]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#8A7862] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
                      <span className="text-[1.2rem] sm:text-[1.3rem] text-[#3E2E20] font-light" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic' }}>
                        {item.title}
                      </span>
                    </div>
                  ))}
          ...
          <SectionWrapper id="album" delay={200} className="w-full bg-[#F5E9D8] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-10 flex items-center gap-2">
                <IconSun className="riviera-scroll-doodle opacity-0" style={{ width: 15, height: 15, color: '#3E7C8C' }} />
                ÁLBUM
              </p>
            </div>
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#F0E4D0] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#3E7C8C]">
                  <IconOlive className="riviera-scroll-doodle opacity-0" style={{ width: 20, height: 10 }} />
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #7A8F6B18, transparent 70%), #F0E4D0" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-8 flex items-center gap-2">
                <IconSun className="riviera-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: '#3E7C8C' }} />
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: '#F0E4D0' }} aria-hidden="true">
            <IconMusic className="riviera-scroll-doodle opacity-0" style={{ width: 26, height: 26, color: '#3E7C8C' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#FAF1E4' }} aria-hidden="true">
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #3E7C8C, transparent)' }} />
          <IconOlive className="riviera-scroll-doodle opacity-0" style={{ width: 20, height: 10, color: '#7A8F6B' }} />
          <div style={{ width: 28, height: 1, background: 'linear-gradient(90deg, transparent, #3E7C8C, transparent)' }} />
        </div>

        <LogoFooterCredit bgColor="#FAF1E4" textColor="var(--chic-ink, #3E2E20)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#3E7C8C" surfaceColor="#FFFFFF" />}
      </div>
  );
}
