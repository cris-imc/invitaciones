/**
 * LuzLunaTemplateNocheEstrellada.tsx
 ...
 *   --t-bg #12121F · --t-surface #1B1B30 · --luzluna-ink #E9E7FB
 *   --t-muted #A6A2D6 · --t-acc #9FB3E8 (lavanda) · --t-acc2 #5C6BB0 (violeta profundo)
 *
...
interface LuzLunaTemplateNocheEstrelladaProps {
...
export function LuzLunaTemplateNocheEstrellada({ invitation, guest, isPersonalized = false }: LuzLunaTemplateNocheEstrelladaProps) {
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #0A0816 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #E9E7FB)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #E9E7FB)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#9FB3E8" surfaceColor="#1B1B30" inactiveColor="#E9E7FB" solid />}
      </div>
  );
}
