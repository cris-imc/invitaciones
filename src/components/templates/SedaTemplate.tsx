 ...
 *   --t-bg #FBF3EE · --t-surface #F5E6DC · --seda-ink #3B2A28
 *   --t-muted #8a6f68 · --t-acc #C9A0A6 (rosa polvo) · --t-acc2 #D9BFA0 (oro champagne)
 *
...
export function SedaTemplate({ invitation, guest, isPersonalized = false }: SedaTemplateProps) {
  ...
  // Portada animada -- Seda es CSS-vars, tema claro (--t-bg #FBF3EE, ink
  // #3B2A28) -- mismo caso que Perlada/Petalos: texto no puede forzarse a
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #FFFFFF 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #3B2A28)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #3B2A28)" />
        </div>
      </div>
