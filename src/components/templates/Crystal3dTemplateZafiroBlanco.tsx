...
export function Crystal3dTemplateZafiroBlanco({ invitation, guest, isPersonalized = false }: Crystal3dTemplateProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#6E93E8";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(110,147,232,.35), 0 18px 50px -30px #6E93E8";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#D7DEEC" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(110,147,232,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(110,147,232,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#D7DEEC" : "#6E93E8";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#05070F", tinta: "#F4F1EA", acento: "#D7DEEC", clase: `${c3dPoppins.variable} ${c3dMono.variable}`, fuente: "var(--c3d-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="c3d-scroller">
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="c3d-section c3d-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #16264A 0%, #0B1526 55%, #05070F 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="c3d-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="c3d-pan-sticky">
            <div data-strip="1" className="c3d-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="c3d-panel c3d-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="c3d-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#D7DEEC" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#D7DEEC" />
                  </svg>
              ...
              <div data-tone="dark" className="c3d-panel c3d-panel--center" style={{ background: "#0A1220", color: "#F4F1EA" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#6E93E8" guest={guest} />
      )}
    ...
    <>
      ...
      <div ref={stubRef} className="c3d-stub">
        ...
        <div ref={sealRef} className="c3d-seal">
          <C3dMedallion ringGradient="conic-gradient(from 0deg, #D7DEEC, #6E93E8, #F2F6FF, #D7DEEC)" sub="" arcId="" arcText="" compact />
        </div>
...
function C3dCoverHalf({
  ...
  return (
    <div className="c3d-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#6E93E8" tintColor2="#080B10" effect="enfoque" scrimColorRgb="5,7,15" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#6E93E8" tintColor2="#080B10" effect="enfoque" scrimColorRgb="5,7,15" />
        </div>
      )}
...
const C3D_CSS = `
  ...
  a { color: #D7DEEC; text-decoration: none; }
  a:hover { color: #F2F6FF; }
  ...
  @media (min-width: 768px) { .c3d-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(110,147,232,.3); } }
  ...
  .c3d-date-month { font-family: var(--c3d-poppins), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #6E93E8; padding-left: 12%; }

  .c3d-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .c3d-divider-line { width: 52px; height: 1px; background: #6E93E8; display: inline-block; }
  ...
  .c3d-cal-link:hover { color: #6E93E8; }
  ...
  .c3d-medallion-num { font-family: var(--c3d-poppins), sans-serif; font-size: 30px; line-height: 1; color: #F2F6FF; }
  .c3d-medallion-num-sm { font-family: var(--c3d-poppins), sans-serif; font-size: 16px; line-height: 1; color: #F2F6FF; }
  .c3d-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .c3d-medallion-sub--cyan { color: #6E93E8; }
  .c3d-medallion-sub--lavender { color: #D7DEEC; }
  .c3d-medallion-arc { position: absolute; inset: -14%; }
  .c3d-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #6E93E8; font-family: var(--c3d-mono), monospace; }

  .c3d-facet-gem { position: absolute; inset: 0; clip-path: polygon(50% 0%, 95% 30%, 80% 90%, 20% 90%, 5% 30%); background: linear-gradient(135deg, #6E93E8, #D7DEEC 55%, #F2F6FF); filter: saturate(.9); animation: facetSpin 6s ease-in-out infinite; }
  ...
  .c3d-facet-num { font-family: var(--c3d-poppins), sans-serif; font-size: 26px; line-height: 1; color: #F2F6FF; }
  ...
  .c3d-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F2F6FF, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .c3d-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #6E93E8; }
  .c3d-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #05070F 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .c3d-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(215,222,236,0.32), rgba(110,147,232,0.26), rgba(110,147,232,0.36), rgba(215,222,236,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .c3d-accent-italic-cyan { font-style: italic; color: #6E93E8; }
  .c3d-accent-italic-lavender { font-style: italic; color: #D7DEEC; }
  .c3d-accent-cyan { color: #6E93E8; }
  .c3d-accent-lavender { color: #D7DEEC; }
  ...
  .c3d-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #D7DEEC; margin-top: auto; }
  ...
  .c3d-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #D7DEEC; }
  ...
  .c3d-crono-time { font-family: var(--c3d-mono), monospace; color: #5C6E96; min-width: 42px; }
  ...
  .c3d-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F2F6FF, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .c3d-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(110,147,232,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .c3d-rsvp-payment-total { color: #F2F6FF; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .c3d-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #6E93E8; background: transparent; color: #6E93E8; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .c3d-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(110,147,232,0.3); color: #F4F1EA; font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .c3d-rsvp-input::placeholder { color: #6E6A5D; }
  .c3d-rsvp-input:focus { outline: none; border-bottom-color: #6E93E8; }
  .c3d-rsvp-btn { width: 100%; padding: 16px; font-family: var(--c3d-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #6E93E8, #3A5FBF); border: 1px solid #6E93E8; color: #0B0B10; cursor: pointer; }
  .c3d-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .c3d-rsvp-btn--ghost { background: transparent; color: #6E93E8; }
  .c3d-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #6E93E8; margin: 0; }
  ...
  .c3d-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(110,147,232,0.18); }
  ...
  .c3d-bank-copy { flex-shrink: 0; font-family: var(--c3d-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #6E93E8; background: transparent; color: #6E93E8; cursor: pointer; }
  .c3d-bank-copy:hover { background: rgba(110,147,232,0.12); }
  ...
  .c3d-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(110,147,232,0.3); padding-bottom: 12px; }
  ...
  .c3d-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(110,147,232,0.3); color: #F4F1EA; font-family: var(--c3d-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .c3d-song-input::placeholder { color: #6E6A5D; }
  .c3d-song-input:focus { outline: none; border-bottom-color: #6E93E8; }
  .c3d-song-sep { color: #8A8577; flex-shrink: 0; }
  .c3d-song-submit { flex-shrink: 0; background: none; border: none; color: #6E93E8; font-family: var(--c3d-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .c3d-song-submit:hover { color: #F2F6FF; }
  .c3d-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .c3d-song-error { font-size: 10px; color: #6E93E8; margin-top: 6px; }
  ...
  .c3d-quiz-opt { font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(110,147,232,0.4); background: transparent; color: #6E93E8; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .c3d-quiz-opt:disabled { cursor: default; }
  .c3d-quiz-opt--picked { background: #6E93E8; border-color: #6E93E8; color: #0B0B10; }
  .c3d-quiz-opt--correct { background: #6E93E8; border-color: #6E93E8; color: #0B0B10; }
  .c3d-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .c3d-quiz-result { padding-top: 18px; border-top: 1px solid rgba(110,147,232,0.2); }
  .c3d-quiz-result-score { margin: 0 0 6px; font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F2F6FF; }
  .c3d-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .c3d-final-card { border: 1px solid #6E93E8; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .c3d-final-names { font-family: var(--c3d-poppins), sans-serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F2F6FF; }
  ...
  .c3d-replay { cursor: pointer; color: #6E93E8; }
  .c3d-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .c3d-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(110,147,232,0.14); }
  .c3d-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .c3d-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(110,147,232,0.16); position: relative; }
  .c3d-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F2F6FF, #6E93E8); transition: height 260ms linear; display: block; }
  .c3d-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #6E93E8; transition: color 500ms ease; }
  ...
  .c3d-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #13203D 0%, #0A1220 46%, #05070F 100%); }
  ...
  .c3d-cover-facets { position: absolute; inset: 0; background-image: repeating-linear-gradient(64deg, rgba(242,246,255,.08) 0 1px, transparent 1px 34px), repeating-linear-gradient(-64deg, rgba(215,222,236,.08) 0 1px, transparent 1px 34px); }
  ...
  .c3d-cover-names { margin: 0; font-family: var(--c3d-poppins), sans-serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F2F6FF; }
  .c3d-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#6E93E8, transparent); display: block; }
  ...
  .c3d-cover-cta { border: 1px solid #6E93E8; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--c3d-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .c3d-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #6E93E8; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
