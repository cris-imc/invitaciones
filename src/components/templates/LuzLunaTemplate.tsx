 ...
 *   --t-bg #171425 · --t-surface #211C36 · --luzluna-ink #EDE7FB
 *   --t-muted #A79BD6 · --t-acc #B9A6D9 (lavanda) · --t-acc2 #7C6BB0 (violeta profundo)
 *
...
interface LuzLunaTemplateProps {
...
export function LuzLunaTemplate({ invitation, guest, isPersonalized = false }: LuzLunaTemplateProps) {
  ...
  // Portada animada -- LuzLuna usa CSS custom properties (--t-bg/--t-acc/
  // --t-acc2), pero AnimatedCoverPhoto necesita valores literales (no
  // acepta var() anidado). Se usan los mismos hex que ya están hardcodeados
  // más abajo en el style inline de esta plantilla (línea ~675: --t-acc
  // #B9A6D9, --t-acc2 #7C6BB0, --t-bg #171425) -- no son "inventados", son
  // los valores reales que ya usa la plantilla. Tema oscuro -> tinte +
  // effect="shimmer" (lunar, delicado).
  const portadaImagenFondoDesktopRaw = String(invitation.portadaImagenFondoDesktop ?? "") || undefined;
  const portadaFondoAnimado = Boolean(portadaImagenFondoDesktopRaw);
  const portadaTintColor1 = "#B9A6D9";
  const portadaTintColor2 = "#7C6BB0";
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #0A0816 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #EDE7FB)" />
        </footer>
      </div>
    );
  }
    ...
    >
      ...
      <div className="d-right tpl">
        ...
        <div className="w-full flex items-center justify-center gap-3 py-8" style={{ background: 'var(--t-bg)' }} aria-hidden="true">
          ...
          <IconMoon className="luzluna-scroll-doodle opacity-0" style={{ width: 22, height: 22, color: 'var(--t-acc)' }} />
          <div style={{ width: 40, height: 1, background: 'linear-gradient(90deg, transparent, var(--t-acc), transparent)' }} />
        </div>
        ...
        <SectionWrapper id="details" delay={150} className="w-full py-20 px-6 md:px-12" style={{ background: 'var(--t-bg)' }}>
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))", color: 'var(--t-acc)' }}>
              <IconArch className="luzluna-scroll-doodle opacity-0" style={{ width: 15, height: 15 }} />
              CUÁNDO Y DÓNDE
            </p>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16">
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))", color: 'var(--t-acc)' }}>
                  <IconConstellation className="luzluna-scroll-doodle opacity-0" style={{ width: 20, height: 9 }} />
                  CRONOGRAMA
                </p>
          ...
          <SectionWrapper id="album" delay={200} className="w-full py-20 overflow-hidden" style={{ background: 'var(--t-surface)' }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-10 flex items-center gap-2">
                <IconMoon className="luzluna-scroll-doodle opacity-0" style={{ width: 18, height: 18, color: 'var(--t-acc2)' }} />
                ÁLBUM
              </p>
            </div>
          ...
          <SectionWrapper id="banco" delay={200} className="w-full py-20 px-6 md:px-12 overflow-hidden" style={{ background: 'var(--t-surface)' }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <p className="t-kicker mb-10 flex items-center gap-2" style={{ color: 'var(--t-acc)' }}>
                  <IconConstellation className="luzluna-scroll-doodle opacity-0" style={{ width: 22, height: 10 }} />
          ...
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, color-mix(in srgb, var(--t-acc2) 14%, transparent), transparent 70%), var(--t-bg)" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <p className="t-kicker mb-8 flex items-center gap-2">
                <IconStarSpark className="luzluna-scroll-doodle opacity-0" style={{ width: 14, height: 14, color: 'var(--t-acc2)' }} />
        ...
        {songsEnabled && (
          <div className="w-full flex justify-center pt-16" style={{ background: 'var(--t-bg)' }} aria-hidden="true">
            <IconMusic className="luzluna-scroll-doodle opacity-0" style={{ width: 24, height: 24, color: 'var(--t-acc)' }} />
          </div>
        )}
        {songsEnabled && (
          <SongSuggestion
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #EDE7FB)" />
        </div>
      </div>
