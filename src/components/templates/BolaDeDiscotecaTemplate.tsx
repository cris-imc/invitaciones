...
// palabras.
const ACCENT_CYCLE = ["#35E0FF", "#FF2E9A", "#F5C400"];
...
export function BolaDeDiscotecaTemplate({ invitation, guest, isPersonalized = false }: BolaDeDiscotecaTemplateProps) {
  const tx = useTextos();
  ...
  const coverKickerText = showGuestNameInCover ? tx("invitacion.sabor.pistaSePrendePara").toUpperCase() : tx("invitacion.sabor.pistaSePrendeQuinceDe").toUpperCase();
  ...
  const portadaMensaje = String(
    invitation.portadaMensaje || tx("invitacion.sabor.mensajePistaNoSeApaga")
  );
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#35E0FF";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #35E0FF";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#FF2E9A" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#FF2E9A" : "#35E0FF";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#08080E", tinta: "#F4F1EA", acento: "#35E0FF", clase: `${bddArchivo.variable} ${bddMono.variable}`, fuente: "var(--bdd-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="bdd-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="bdd-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #0A0A12 55%, #08080E 100%)" }}>
          <span data-xin="1" data-dist="-60" className="bdd-kicker">{"01 — " + tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
        ...
        {(photoMobile || photoDesktop) && (
          <section
            ...
            className={`bdd-hero-photo-section${!photoMobile ? " bdd-hero-photo-section--no-mobile" : ""}${!photoDesktop ? " bdd-hero-photo-section--no-desktop" : ""}`}
          >
            <div className="bdd-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="8,8,14" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="8,8,14" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="bdd-section bdd-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #161020 0%, #0C0A14 55%, #08080E 100%)" }}>
          <div className="bdd-scan-grid" />
          <div className="bdd-scanline" />
          <span data-xin="1" data-dist="-60" className="bdd-kicker" style={{ position: "relative" }}>{knPre(2)} — LA PISTA SE ENCIENDE EN</span>
          <div className="bdd-cd-grid">
            <BddCdBox refEl={dRef} delay={40} dist={-90} label={tx("invitacion.cuentaRegresiva.dias").toUpperCase()} />
            <BddCdBox refEl={hRef} delay={120} dist={110} label={tx("invitacion.cuentaRegresiva.horas").toUpperCase()} />
            <BddCdBox refEl={mRef} delay={200} dist={-140} label={tx("invitacion.cuentaRegresiva.minutosAbrev").toUpperCase()} />
            <BddCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} />
          </div>
          <div className="bdd-perf-strip" />
        </section>

        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="bdd-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #08080E 100%)" }}>
          <div data-drift="-130" className="bdd-glow-blob" />
          <span data-xin="1" data-dist="-60" className="bdd-kicker" style={{ position: "relative" }}>{knPre(3)} {"— " + tx("invitacion.cuentaRegresiva.cuandoLlegueACero").toUpperCase()}</span>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="bdd-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
            ...
            <div data-strip="1" className="bdd-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="bdd-panel bdd-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <div className="bdd-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="bdd-side-hint">{scrollVertical ? "↓" : "→"}</span></div>
                </div>
              )}
              ...
              <div id="details" data-tone="light" className="bdd-panel bdd-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                <div className="bdd-facts">
                  ...
                  {dressCode && (
                    <div className="bdd-facts-row bdd-facts-row--last">
                      <span>{tx("invitacion.pase.codigo").toUpperCase()}</span><span className="bdd-accent-2">{dressCode.toUpperCase()}</span>
                    </div>
                  )}
                </div>
                {cronograma.length > 0 && (
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="bdd-panel bdd-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="bdd-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#FF2E9A" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#FF2E9A" />
                  </svg>
                  <div className="bdd-panel-block">
                    <span className="bdd-mini-label">{ceremoniaHabilitada ? "03" : "02"} / {LUGAR_PANEL_COUNT}</span>
                    <span className="bdd-panel-title-sm">{tx("invitacion.ubicacion.comoLlegar")}</span>
              ...
              <div data-tone="dark" className="bdd-panel bdd-panel--center" style={{ background: "#0A0A12", color: "#F4F1EA" }}>
                <div className="bdd-medallion bdd-medallion--lg">
                  <BddMedallion topLabel={tx("invitacion.pase.sector").toUpperCase()} mainText={tx("invitacion.pase.pista")} subLabel={tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()} arcId="bddArc2" arcText={tx("invitacion.pase.arcoAccesoVipPase", { n: passNumber }).toUpperCase()} spin="reverse" />
                </div>
                <span className="bdd-mini-label">{LUGAR_PANEL_COUNT} / {LUGAR_PANEL_COUNT} {"— " + tx("invitacion.ubicacion.tuUbicacion").toUpperCase()}</span>
                {/* mesa-en-ubicacion */}
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="bdd-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #0A0A12 60%, #08080E 100%)" }}>
          <span data-xin="1" data-dist="-60" className="bdd-kicker">{kn(4)} — CHECK-IN</span>
          <h2 data-xin="1" data-delay="80" data-dist="130" className="bdd-h2">
            {tx("invitacion.rsvp.confirmaLinea1")}<br /><span className="bdd-accent-italic">{tx("invitacion.rsvp.confirmaLinea2")}</span>
          </h2>
          ...
          {rsvpEnabled ? (
            ...
            <p className="bdd-lead">{tx("invitacion.rsvp.cerrada")}</p>
          )}
        </section>

        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="bdd-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="bdd-pan-sticky">
            <div data-strip="1" className="bdd-strip">
                ...
                <div key={pageIndex} data-tone="light" className="bdd-panel bdd-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="bdd-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      ...
                      <span className="bdd-photo-placeholder">{tx("invitacion.album.sinFotos")}</span>
                    )}
                  </div>
                  <div className="bdd-seguir bdd-seguir--split">
                    <span>{tx("invitacion.album.fotosSubidas", { n: allPhotos.length }).toUpperCase()}</span>
                    <span className="bdd-accent-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="bdd-section" style={{ background: "#0A0A12" }}>
            <span data-xin="1" data-dist="-60" className="bdd-kicker">{kn(6)} {"— " + tx("invitacion.musica.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="bdd-h2">{tituloEnDosLineas(tx("invitacion.sabor.preguntaTemaBailar"), "bdd-accent-pink-italic")}</h2>
            <div data-xin="1" data-delay="160" data-dist="-80" className="bdd-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="bdd-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FFFFFF" : "#35E0FF" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="bdd-section" style={{ background: "#0A0A12" }}>
            <span data-xin="1" data-dist="-60" className="bdd-kicker">{sugerenciaMusicaHabilitada ? kn(7) : kn(6)} {"— " + tx("invitacion.regalos.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="bdd-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><span className="bdd-accent-italic">{tx("invitacion.regalos.siQueresLinea2")}</span>
            </h2>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="bdd-section" style={{ background: "#0A0A12" }}>
            <span data-xin="1" data-dist="-60" className="bdd-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection].filter(Boolean).length + 6)} {"— " + tx("invitacion.quiz.kicker").toUpperCase()}</span>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="bdd-section bdd-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #0A0A12 55%, #08080E 100%)" }}>
          ...
          <div className="bdd-final-footer">
            ...
            <span className="bdd-replay" onClick={reset}>{tx("invitacion.portada.verAperturaOtraVez").toUpperCase() + " ↺"}</span>
          </div>
      ...
      <div ref={railRef} className="bdd-rail">
        <span ref={railTopRef} className="bdd-rail-top">{tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()}</span>
      ...
      <div ref={coverRef} className="bdd-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="bdd-cover-half bdd-cover-half--top">
          ...
          >
            <div className="bdd-cover-cta">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</div>
          </BddCoverHalf>
        </div>
        <div ref={bottomRef} className="bdd-cover-half bdd-cover-half--bottom">
          <BddCoverHalf
            ...
            photoDesktop={photoDesktop}
          >
            <button onClick={open} className="bdd-cover-cta bdd-cover-cta--btn">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</button>
          </BddCoverHalf>
        </div>
      </div>

      <div ref={hintRef} className="bdd-hint">{tx("invitacion.portada.desliza").toUpperCase() + " ↓"}</div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#35E0FF" guest={guest} />
      )}
  ...
  if (status === "DECLINED") {
    return (
      <div className="bdd-rsvp-declined">
        <p className="bdd-rsvp-declined-text">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
  ...
  return (
    <>
      <div className="bdd-rsvp-rows">
        ...
        {status === "CONFIRMED" && (
          <>
            {totalGuests > 1 && adultCount > 0 && <div className="bdd-rsvp-row"><span>{tx("invitacion.rsvp.adultos").toUpperCase()}</span><span>{String(adultCount).padStart(2, "0")}</span></div>}
            {teenCount > 0 && <div className="bdd-rsvp-row"><span>{tx("invitacion.rsvp.adolescentes").toUpperCase()}</span><span>{String(teenCount).padStart(2, "0")}</span></div>}
            {childCount > 0 && <div className="bdd-rsvp-row"><span>{tx("invitacion.rsvp.ninos").toUpperCase()}</span><span>{String(childCount).padStart(2, "0")}</span></div>}
          </>
        )}
          ...
          <div className="bdd-rsvp-row bdd-rsvp-row--payment">
            ...
            <div className="bdd-rsvp-payment-value">
              ...
              {paymentStatus === "PARTIAL" && (
                <div className="bdd-rsvp-payment-detail"><span>{tx("invitacion.pago.pagoParcialRegistrado")}</span></div>
              )}
    ...
    <div className="bdd-song">
      <form onSubmit={handleSubmit} className="bdd-song-row">
        <div className="bdd-song-inputs">
          <input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder={tx("invitacion.musica.artista").toUpperCase()} maxLength={80} className="bdd-song-input" />
          <span className="bdd-song-sep">—</span>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tx("invitacion.musica.tema").toUpperCase()} maxLength={100} className="bdd-song-input" />
        </div>
        ...
        <div className="bdd-song-list">
          {songs.slice(0, 12).map((s, i) => (
            <div key={s.id} className="bdd-song-item">
              ...
              <span className="bdd-song-item-by">{tx("invitacion.musica.sumadoPor")} {s.guestName || tx("invitacion.evento.invitado")}</span>
            </div>
          ))}
        </div>
      )}
...
function BddCoverHalf({
  ...
  return (
    <div className="bdd-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#35E0FF" tintColor2="#08080E" effect="enfoque" scrimColorRgb="8,8,14" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#35E0FF" tintColor2="#08080E" effect="enfoque" scrimColorRgb="8,8,14" />
        </div>
      )}
      ...
      <div className="bdd-cover-content">
        <div className="bdd-cover-top-row">
          <span>{tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()}</span><span className="bdd-accent-2">ALL ACCESS</span>
        </div>
        ...
        <div className="bdd-cover-bottom">
          <div ref={perfRef} className="bdd-perf-strip bdd-perf-strip--reveal bdd-perf-strip--cover" />
          ...
          <div className="bdd-barcode-wrap">
            <div className="bdd-barcode" style={{ width: "62%", height: "clamp(15px, 3vh, 26px)", opacity: 0.6 }} />
            <span className="bdd-mini-label bdd-mini-label--cover" style={{ color: "#56534A" }}>{tx("invitacion.pase.noTransferible").toUpperCase()}</span>
          </div>
        </div>
      </div>
    </div>
...
const BDD_CSS = `
  ...
  a { color: #35E0FF; text-decoration: none; }
  ...
  .bdd-hero-photo-section { position: relative; min-height: calc(var(--vh, 1vh) * 100); overflow: hidden; background: #08080E; }
  .bdd-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  @media (min-width: 768px) { .bdd-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(53,224,255,.3); } }
  ...
  .bdd-date-month { font-family: var(--bdd-archivo), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #35E0FF; padding-left: 12%; }

  .bdd-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .bdd-divider-line { width: 52px; height: 1px; background: #35E0FF; display: inline-block; }
  ...
  .bdd-cal-link:hover { color: #35E0FF; }
  ...
  .bdd-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #FF2E9A, #35E0FF, #35E0FF, #FFFFFF, #FF2E9A); filter: saturate(.75); }
  .bdd-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  ...
  .bdd-medallion-label--accent { color: #35E0FF; }
  .bdd-medallion-arc { position: absolute; inset: -14%; }
  .bdd-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #35E0FF; font-family: var(--bdd-mono), monospace; }
  ...
  .bdd-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #35E0FF; }
  .bdd-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #08080E 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .bdd-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }
  .bdd-perf-strip--cover { opacity: .9; }

  .bdd-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(255,46,154,0.28), rgba(53,224,255,0.24), rgba(245,196,0,0.3), rgba(255,46,154,0.28)); filter: blur(80px); opacity: .4; animation: bddFoil 30s linear infinite; }
  ...
  .bdd-accent-italic { font-style: italic; color: #35E0FF; }
  .bdd-accent-pink-italic { font-style: italic; color: #FF2E9A; }
  .bdd-accent-2 { color: #FF2E9A; }
  .bdd-h2 { margin: 0; font-family: var(--bdd-archivo), sans-serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .bdd-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .bdd-mini-label--cover { font-size: 8.5px; letter-spacing: 0.3em; }
  ...
  .bdd-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #FF2E9A; margin-top: auto; }
  ...
  .bdd-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #FF2E9A; }
  ...
  .bdd-crono-time { font-family: var(--bdd-mono), monospace; color: #FF2E9A; min-width: 42px; }
  ...
  .bdd-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(53,224,255,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #6E6A5D; }
  ...
  .bdd-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #35E0FF; background: transparent; color: #35E0FF; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .bdd-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(53,224,255,0.3); color: #F4F1EA; font-family: var(--bdd-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .bdd-rsvp-input::placeholder { color: #6E6A5D; }
  .bdd-rsvp-input:focus { outline: none; border-bottom-color: #35E0FF; }
  .bdd-rsvp-btn { width: 100%; padding: 16px; font-family: var(--bdd-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; border: 1px solid #35E0FF; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; cursor: pointer; transition: background 300ms ease, color 300ms ease; }
  .bdd-rsvp-btn:hover:not(:disabled) { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0B0B0F; }
  .bdd-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .bdd-rsvp-btn--ghost { background: transparent; color: #35E0FF; }
  .bdd-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(53,224,255,0.1); color: #35E0FF; }
  .bdd-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #FF2E9A; margin: 0; }
  ...
  .bdd-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(53,224,255,0.18); }
  ...
  .bdd-bank-copy { flex-shrink: 0; font-family: var(--bdd-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #35E0FF; background: transparent; color: #35E0FF; cursor: pointer; }
  .bdd-bank-copy:hover { background: rgba(53,224,255,0.12); }
  ...
  .bdd-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(53,224,255,0.3); padding-bottom: 12px; }
  ...
  .bdd-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(53,224,255,0.3); color: #F4F1EA; font-family: var(--bdd-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .bdd-song-input::placeholder { color: #6E6A5D; }
  .bdd-song-input:focus { outline: none; border-bottom-color: #35E0FF; }
  .bdd-song-sep { color: #8A8577; flex-shrink: 0; }
  .bdd-song-submit { flex-shrink: 0; background: none; border: none; color: #35E0FF; font-family: var(--bdd-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .bdd-song-submit:hover { color: #FFFFFF; }
  .bdd-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .bdd-song-error { font-size: 10px; color: #FF2E9A; margin-top: 6px; }
  ...
  .bdd-quiz-opt { font-family: var(--bdd-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(53,224,255,0.4); background: transparent; color: #35E0FF; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .bdd-quiz-opt:disabled { cursor: default; }
  .bdd-quiz-opt--picked { background: #35E0FF; border-color: #35E0FF; color: #0B0B10; }
  .bdd-quiz-opt--correct { background: #35E0FF; border-color: #35E0FF; color: #0B0B10; }
  .bdd-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .bdd-quiz-result { padding-top: 18px; border-top: 1px solid rgba(53,224,255,0.2); }
  ...
  .bdd-final-card { border: 1px solid #35E0FF; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .bdd-replay { cursor: pointer; color: #35E0FF; }
  ...
  .bdd-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FFFFFF, #35E0FF); transition: height 260ms linear; display: block; }
  .bdd-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #35E0FF; transition: color 500ms ease; }
  ...
  .bdd-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #12121A 0%, #0A0A12 46%, #08080E 100%); }
  .bdd-cover-half--bottom .bdd-cover-inner { top: auto; bottom: 0; }
  .bdd-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(255,46,154,0.4), rgba(53,224,255,0.36), rgba(245,196,0,0.4), rgba(255,46,154,0.4)); filter: blur(64px); opacity: .62; animation: bddFoil 26s linear infinite; }
  ...
  .bdd-discolight { position: absolute; inset: -20%; animation: bddDiscoSweep 8s ease-in-out infinite; background:
    ...
    radial-gradient(3% 2% at 70% 15%, rgba(255,46,154,.7), transparent 60%),
    radial-gradient(3% 2% at 40% 60%, rgba(53,224,255,.7), transparent 60%),
    radial-gradient(3% 2% at 85% 70%, rgba(245,196,0,.6), transparent 60%),
  ...
  .bdd-cover-names { margin: 0; font-family: var(--bdd-archivo), sans-serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FFFFFF; }
  .bdd-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#35E0FF, transparent); display: block; }
  ...
  .bdd-cover-cta { border: 1px solid #35E0FF; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--bdd-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .bdd-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .bdd-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0B0B0F; }
  }
  ...
  .bdd-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #35E0FF; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
