 ...
 * (#150A1C) + acentos neón cian (#FF6EC7) y magenta (#4BE8FF), tipografía
...
const PhotoCornerFrame = () => (
  <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
    ...
    <IconSpark style={{ position: "absolute", bottom: -8, right: -8, width: 16, height: 16, color: "#4BE8FF" }} />
  </div>
);
...
interface NeonTemplateEclipseProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#FF6EC7]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#FF6EC7] uppercase tracking-wider mb-0.5">{label}</span>
  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        ...
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#F3EEF7" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#A08FB0" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,255,255,0.1)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#FF6EC7] shrink-0" />
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#F3EEF7" }}>
  ...
  return (
    <div className="quiz-box flex flex-col items-center text-center">
      <div className="quiz-q w-full max-w-lg" key={currentIdx}>
        <p className="text-[#F3EEF7] text-2xl md:text-3xl leading-relaxed tracking-wide" style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', margin: 0, fontWeight: 500, marginBottom: "3.5rem" }}>
...
export function NeonTemplateEclipse({ invitation, guest, isPersonalized = false }: NeonTemplateEclipseProps) {
  const tx = useTextos();
  ...
  // Portada animada -- Neon es "alocada/extravagante" (cian/magenta) ->
  ...
  const portadaTintColor1 = "#FF6EC7";
  const portadaTintColor2 = "#4BE8FF";
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#150A1C] via-[#1E1024] to-[#150A1C] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
  ...
  return (
    ...
    >
      <style>{`
        ...
        #countdown.dark {
          background-color: #150A1C !important;
        ...
        #countdown[data-style="clasico"].dark > div > div > div {
          ...
          border-color: rgba(57,255,208,0.2) !important;
        }

        /* RSVP Custom Aesthetics for NeonTemplate */
        #rsvp.section.dark {
          background-color: #150A1C !important;
          color: #FFFFFF !important;
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #FF6EC7 !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        .desktop-stage .d-foot {
          background-color: #150A1C !important; /* Matches light sections */
          color: #FFFFFF !important;
      ...
      {!isCoverOpen && (
        <div
          ref={coverRootRef}
          style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', zIndex: 99999, backgroundColor: '#150A1C', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '25vh', overflow: 'hidden', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) }}
          className={`text-[#F3EEF7] ${isClosingCover ? "acp-cover-exit" : "transition-all duration-1000 animate-in fade-in"}`}
        >
          {portadaFondoAnimado && (
          ...
          <IconDiscoBall className="neon-doodle opacity-0 absolute" style={{ width: 34, height: 34, top: '10%', left: '12%', color: 'rgba(57,255,208,0.5)' }} />
          ...
          <IconDiscoBall className="neon-doodle opacity-0 absolute" style={{ width: 30, height: 30, bottom: '16%', left: '18%', color: 'rgba(57,255,208,0.35)' }} />
          <IconSpark className="neon-doodle opacity-0 absolute" style={{ width: 22, height: 22, top: '30%', right: '20%', color: '#4BE8FF' }} />
          <IconSpark className="neon-doodle opacity-0 absolute" style={{ width: 16, height: 16, bottom: '22%', right: '28%', color: '#FF6EC7' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            {/* Guest Name */}
            <h2 className="text-4xl sm:text-5xl font-light tracking-wide text-[#F3EEF7] leading-relaxed" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {guestNameDisplay}
            </h2>

            {/* Dress Code */}
            {Boolean(activeDressCode) && (
              <p className=" text-sm font-medium text-[#A08FB0] tracking-wide uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter)), sans-serif", letterSpacing: "0.2em", opacity: 0.8 }}>
            ...
            <button
              ...
              style={{
                fontFamily: 'var(--font-body-custom, var(--font-inter)), sans-serif', border: '1px solid #FF6EC7', color: '#FF6EC7',
                background: 'rgba(57,255,208,0.08)', backdropFilter: 'blur(6px)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#FF6EC7'; e.currentTarget.style.color = '#150A1C'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(57,255,208,0.08)'; e.currentTarget.style.color = '#FF6EC7'; }}
            >
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#FF6EC7]/40 shadow-md ${isTicketMaximized ? 'bg-[#150A1C]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#FFFFFF]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#FF6EC7] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#FF6EC7] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#FF6EC7]/20 pl-3">
                <span className="text-[#FFFFFF] font-bold text-sm leading-none">{guest.expectedCount}</span>
                <span className="text-[#A08FB0] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#FF6EC7]" />
              <span className="text-[#1C1024]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <aside className="d-left hide-mobile">
        ...
        <div className="d-left-top drop-shadow-md">
          ...
          <div style={{ width: 40, height: 2, background: '#FF6EC7', margin: '6px 0 14px', animation: 'neon-lineExpand 1.2s ease-out' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#150A1C]">
          {/* Text Container */}
          <div className="px-8 pt-16 pb-12 text-left bg-[#150A1C] z-10 relative">
            <IconDiscoBall className="neon-scroll-doodle opacity-0 absolute" style={{ width: 24, height: 24, top: 10, right: 24, color: '#4BE8FF' }} />
            <IconSpark className="neon-scroll-doodle opacity-0 absolute" style={{ width: 16, height: 16, top: 60, right: 56, color: '#FF6EC7' }} />
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#FF6EC7] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#F3EEF7] leading-[1.0] mb-3" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#FF6EC7' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            </h1>
            <div style={{ width: 40, height: 2, background: '#FF6EC7', margin: '0 0 20px', animation: 'neon-lineExpand 1.2s ease-out' }} />
            <p className=" text-sm font-medium text-[#A08FB0] tracking-wide" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {fechaStr}{lugarNombre ? ` · ${lugarNombre}` : ""}{ciudad ? ` — ${ciudad}` : ""}
            </p>
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#FF6EC7] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            <div className="absolute inset-x-0 bottom-0 h-1/2 pointer-events-none z-10" style={{ background: 'linear-gradient(to bottom, transparent 0%, #150A1C 100%)' }} />
            <div
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center" style={{ background: "linear-gradient(160deg, #4BE8FF14, transparent 70%), #1C1024" }}>
            <div className="max-w-2xl mx-auto text-center">
              ...
              <TypewriterText 
                ...
                className="text-[#F3EEF7] text-2xl md:text-3xl leading-relaxed tracking-wide" 
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#150A1C] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#FF6EC7]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#FF6EC7] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#A08FB0] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  ...
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="text-[#A08FB0]  text-sm sm:text-base mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {tx("invitacion.ubicacion.horaConSufijo", { hora: String(invitation.ceremoniaHora) })}</p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="text-[#A08FB0]  text-sm sm:text-base mb-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                      {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#FF6EC7] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-black/20 border-l-[2px] border-l-[#FF6EC7] p-6 sm:p-8 mb-10 shadow-sm">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#A08FB0] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-semibold tracking-wider text-[#FF6EC7] hover:text-white transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-semibold tracking-[0.2em] uppercase text-[#FF6EC7]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b border-[#FF6EC7]/10 last:border-b-0">
                      {item.time && (
                        <span className=" text-sm sm:text-base text-[#A08FB0] font-medium w-24 flex-shrink-0 mb-1 sm:mb-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#1C1024] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#1E1024] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                <div className="flex justify-center mb-4">
                  <DrawLucideIcon icon={Landmark} size={46} color="var(--t-acc)" strokeWidth={1.5} />
                </div>
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#FF6EC7]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full py-20 px-6 md:px-12" style={{ background: "linear-gradient(160deg, #4BE8FF18, transparent 70%), #111114" }}>
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        <div className="w-full flex justify-center py-6" style={{ background: '#150A1C' }} aria-hidden="true">
          <svg className="neon-scroll-doodle opacity-0" viewBox="0 0 90 12" width="90" height="12" stroke="rgba(57,255,208,0.6)" strokeWidth="1.4">
            <path d="M2 10 L10 2 M14 10 L22 2 M26 10 L34 2 M38 10 L46 2 M50 10 L58 2 M62 10 L70 2 M74 10 L82 2" />
          </svg>
        </div>

        <LogoFooterCredit bgColor="#150A1C" />
        </div>
      </div>
      
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#FF6EC7" surfaceColor="#1E1024" />}
      </div>
  );
}
