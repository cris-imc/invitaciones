...
export function Y2kTemplateFrutilla({ invitation, guest, isPersonalized = false }: Y2kTemplateProps) {
  const tx = useTextos();
    ...
    return (
      <PostEventoStorytelling
        ...
        paleta={{
          ...
          tinta: PALETA.ink,
          acento: PALETA.acc,
          clase: `${y2kSerif.variable} ${y2kSans.variable} ${y2kMono.variable}`,
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} className="y2k-scroller">
        ...
        {/* ── 01 Guardá la fecha ─────────────────────────────────────── */}
        <section data-tone={TONO} data-screen-label={tx("invitacion.saveTheDate.guardaLaFecha")} className="y2k-section y2k-std">
          <div className="y2k-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="y2k-kicker">{nSaveTheDate} — {tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          <div className="y2k-std-fecha">
            <span data-xin="1" data-delay="60" data-dist="40" className="y2k-std-dia">{diaNum}</span>
            <div data-xin="1" data-delay="160" data-dist="30" className="y2k-std-fila">
              <span className="y2k-cinta">{mesLargo}</span>
              <span className="y2k-std-anio">{anio}</span>
            </div>
          </div>
          <div data-xin="1" data-delay="240" data-dist="20" className="y2k-meta">
            <span>{diaSemana} · {hora} H</span>
            <span className="y2k-meta-punto" aria-hidden="true" />
          ...
          <AddToCalendarLink
            ...
            className="y2k-btn-papel y2k-btn-papel--reveal"
            showIcon={false}
          >
        ...
        {hayFoto && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.album.nuestraFoto")} className="y2k-foto-seccion">
            <div className="y2k-foto-marco">
              <div ref={ventanaRef} className="y2k-ventana">
                {fotoMobile && (
              ...
              <div className="y2k-foto-firma">{titulo}</div>
            </div>
          </section>
        )}
        ...
        <section data-tone={TONO} data-screen-label={tx("invitacion.cuentaRegresiva.kicker")} className="y2k-section y2k-countdown">
          <div className="y2k-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="y2k-kicker">{nCountdown} — {tx("invitacion.cuentaRegresiva.faltan").toUpperCase()}</span>
          <h2 data-xin="1" data-delay="60" className="y2k-h2">
        ...
        {/* ── 03 Unas palabras ───────────────────────────────────────── */}
        {hayFrase && (
          <section data-tone={TONO} data-screen-label={tx("invitacion.frase.etiqueta")} className="y2k-section y2k-frase-seccion">
            <div className="y2k-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="y2k-kicker">{nFrase} — {tx("invitacion.frase.unasPalabras").toUpperCase()}</span>
            ...
            <div data-xin="1" data-delay="200" className="y2k-firma">
              <span className="y2k-firma-linea" aria-hidden="true" />{titulo.toUpperCase()}
            </div>
          </section>
        )}
        ...
        >
          <div className="y2k-pan-fijo">
            <div data-strip="1" className="y2k-tira">
              {/* Recepción */}
              <div data-tone={TONO} className="y2k-panel y2k-panel--abajo" style={{ background: PALETA.bg }}>
                <div className="y2k-panel-top">
                  <span>{nCuando} — {tx("invitacion.ubicacion.cuandoYDonde").toUpperCase()}</span><span>{deLugar("recepcion")}</span>
                </div>
                <div className="y2k-tarjeta y2k-tarjeta--izq">
                  <span className="y2k-tarjeta-kicker">{tx("invitacion.ubicacion.fiestaSalon").toUpperCase()}</span>
                  <h2 className="y2k-tarjeta-titulo">{lugarNombre || tx("invitacion.ubicacion.elLugar")}</h2>
                  <div className="y2k-filas">
                    {direccion && (
                      <div className="y2k-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="y2k-fila-valor">{direccion}</span></div>
                    )}
                    <div className="y2k-fila"><span>{tx("invitacion.ubicacion.horario")}</span><span>{hora} h</span></div>
                    {dressCode && (
                      <div className="y2k-fila y2k-fila--ultima"><span>{tx("invitacion.ubicacion.dressCode")}</span><span className="y2k-acento-plano">{dressCode}</span></div>
                    )}
                  </div>
                </div>
                {!scrollVertical && panelesLugar.length > 1 && (
                  <div className="y2k-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="y2k-flecha">→</span></div>
                )}
              </div>

              {/* Ceremonia */}
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone={TONO} className="y2k-panel y2k-panel--abajo" style={{ background: PALETA.bg2 }}>
                  <div className="y2k-panel-top">
                    <span>{ceremoniaTitulo.toUpperCase()}</span><span>{deLugar("ceremonia")}</span>
                  </div>
                  <div className="y2k-tarjeta y2k-tarjeta--der">
                    <h2 className="y2k-tarjeta-titulo">{ceremoniaNombre || ceremoniaTitulo}</h2>
                    <div className="y2k-filas">
                      {ceremoniaDireccion && (
                        <div className="y2k-fila"><span>{tx("invitacion.ubicacion.direccion")}</span><span className="y2k-fila-valor">{ceremoniaDireccion}</span></div>
                      )}
                      {ceremoniaHora && (
                        <div className="y2k-fila y2k-fila--ultima"><span>{tx("invitacion.ubicacion.horario")}</span><span>{ceremoniaHora} h</span></div>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cómo llegar */}
              {hayComoLlegar && (
                <div id="location" data-tone={TONO} className="y2k-panel y2k-panel--abajo" style={{ background: PALETA.bg }}>
                  <svg viewBox="0 0 430 700" preserveAspectRatio="xMidYMid slice" className="y2k-escena" aria-hidden="true">
                  ...
                  <h2 className="y2k-h2">{tx("invitacion.ubicacion.comoLlegar")}</h2>
                  {(direccion || ciudad) && <p className="y2k-parrafo">{[direccion, ciudad].filter(Boolean).join(" · ")}</p>}
                  {embedMapUrl && (
                  ...
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="y2k-btn-solido">
              ...
              {/* Cronograma */}
              {cronograma.length > 0 && (
                <div id="schedule" data-tone={TONO} className="y2k-panel y2k-panel--centro" style={{ background: PALETA.bg2 }}>
                  <div className="y2k-panel-top">
                    <span>{tx("invitacion.ubicacion.cronograma").toUpperCase()}</span><span>{deLugar("cronograma")}</span>
                  </div>
                  <h2 className="y2k-h2">{tx("invitacion.ubicacion.laNochePasoAPaso")}</h2>
                  <div className="y2k-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="y2k-crono-fila">
                        <span className="y2k-crono-hora">{item.time || ""}</span>
                        <span className="y2k-crono-titulo">{item.title}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {rsvpHabilitado && (
          <section id="rsvp" data-tone={TONO} data-screen-label={tx("invitacion.rsvp.confirmar")} className="y2k-section y2k-checkin">
            <div className="y2k-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="y2k-kicker">{nCheckin} — CHECK-IN</span>
            <h2 data-xin="1" data-delay="60" className="y2k-h2">
            ...
            <div className="y2k-pan-fijo">
              <div data-strip="1" className="y2k-tira">
                {hojasDeFotos.map((hoja, iHoja) => (
                  <div key={iHoja} data-tone={TONO} className="y2k-panel y2k-panel--album" style={{ background: iHoja % 2 === 0 ? "#F6F1E7" : "#F1EBDF" }}>
                    <div className="y2k-panel-top">
                    ...
                    {iHoja === 0 && (
                      <h2 className="y2k-h2 y2k-h2--album">{tx("invitacion.album.titulo")} <em>{tx("invitacion.album.deFotos")}</em></h2>
                    )}
                    <div className="y2k-polaroids" data-cantidad={hoja.length}>
                      {hoja.map((url, i) => (
                        <div
                          key={i}
                          className="y2k-polaroid"
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <img src={url} alt="" loading="lazy" className="y2k-polaroid-img" />
                          {i % 2 === 0 && <span className="y2k-chinche" aria-hidden="true" />}
                        </div>
                      ))}
                    </div>
                    <div className="y2k-panel-pie">
                      <span>{tx("invitacion.album.fotosSubidas", { n: todasLasFotos.length }).toUpperCase()}</span>
                      {!scrollVertical && hojasDeFotos.length > 1 && <span className="y2k-acento-plano">{tx("invitacion.portada.segui").toUpperCase()} →</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="songs" data-tone={TONO} data-screen-label={tx("invitacion.musica.titulo")} className="y2k-section y2k-musica">
            <div className="y2k-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="y2k-kicker">{nMusica} — {tx("invitacion.musica.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="y2k-h2">
        ...
        {hayRegalos && (
          <section id="banco" data-tone={TONO} data-screen-label={tx("invitacion.regalos.titulo")} className="y2k-section y2k-regalos">
            <span data-xin="1" data-dist="-20" className="y2k-kicker y2k-kicker--tinta">{nRegalos} — {tx("invitacion.regalos.titulo").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="y2k-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><em>{tx("invitacion.regalos.siQueresLinea2")}</em>
            </h2>
            {Boolean(invitation.regaloMensaje) && (
              <p data-xin="1" data-delay="120" className="y2k-parrafo y2k-parrafo--tinta">{String(invitation.regaloMensaje)}</p>
            )}
            {regaloHabilitado && (
              <TarjetaBancaria
        ...
        {quizHabilitado && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="y2k-section y2k-quiz">
            <div className="y2k-trama" aria-hidden="true" />
            <span data-xin="1" data-dist="-20" className="y2k-kicker">{nQuiz} — {tx("invitacion.quiz.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="60" className="y2k-h2 y2k-h2--claro">{triviaTitulo}</h2>
            <TriviaY2k
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="y2k-section y2k-pase">
          <div className="y2k-trama" aria-hidden="true" />
          <span data-xin="1" data-dist="-20" className="y2k-kicker">{nPase} — {tx("invitacion.pase.tuPase").toUpperCase()}</span>

          <div data-xin="1" data-delay="80" data-dist="30" className="y2k-ticket">
            <div className="y2k-ticket-izq">
              <span className="y2k-ticket-kicker">{tx("invitacion.pase.numeroPase", { n: pase }).toUpperCase()}</span>
              <span className="y2k-ticket-nombre">{saludaAlInvitado ? nombreInvitado : titulo}</span>
              <span className="y2k-ticket-dato">
                {[
              ...
              <span className="y2k-ticket-fecha">{fechaPuntos} — {hora} H</span>
              <span className="y2k-ticket-muesca y2k-ticket-muesca--arriba" aria-hidden="true" />
              <span className="y2k-ticket-muesca y2k-ticket-muesca--abajo" aria-hidden="true" />
            </div>
            <div className="y2k-ticket-der">
          ...
          <div data-xin="1" data-delay="160" className="y2k-info-extra">
            <InfoAdicionalSection invitation={invitation} />
          </div>

          <div className="y2k-pase-pie">
            <span className="y2k-pase-firma">{tx("invitacion.frase.conAmor")}, {titulo}</span>
            <div className="y2k-pase-creditos">
      ...
      <div ref={portadaRef} data-tone={TONO} className="y2k-portada">
        ...
        <div ref={escenaPortadaRef} className="y2k-portada-escena">
          <div data-cl="1" className="y2k-trama" aria-hidden="true" />
        </div>
        <div className="y2k-portada-contenido">
          <div className="y2k-portada-arriba">
            <div ref={cartelRef} className="y2k-cartel">
              <h1 className="y2k-cartel-nombres">
                {saludaAlInvitado ? (
                  <span>{nombreInvitado}</span>
                ) : (
                  <>
                    <span>{nombre1}</span>
                    {nombre2 && <span className="y2k-cartel-amp">&amp;</span>}
                    {nombre2 && <span>{nombre2}</span>}
                  </>
                )}
              </h1>
              <span className="y2k-cartel-chinche" aria-hidden="true" />
            </div>
          </div>
          <div className="y2k-portada-abajo">
            <div className="y2k-portada-datos">
              <span className="y2k-portada-saludo">
                {saludaAlInvitado
                  ? <>{tx("invitacion.bienvenida.hola", { nombre: nombreInvitado }).toUpperCase()} · <span className="y2k-acento-plano">{titulo.toUpperCase()}</span></>
                  : <span className="y2k-acento-plano">{tx(invitation.tipo === "CASAMIENTO" ? "invitacion.evento.nosCasamos" : invitation.tipo === "QUINCE_ANOS" ? "invitacion.evento.misQuinceAnos" : "invitacion.evento.teInvitamos").toUpperCase()}</span>}
              </span>
              <div className="y2k-portada-fila">
                <span>{fechaPuntos}</span>
                <span className="y2k-meta-punto" aria-hidden="true" />
                <span>{(lugarNombre || ciudad || direccion).toUpperCase()}</span>
              </div>
              {isPersonalized && guest && (
                <div className="y2k-portada-pase">
            ...
            <button type="button" onClick={abrir} className="y2k-portada-btn">
...
function CuentaY2k({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="y2k-cuenta">
      {celdas.map((c, i) => (
        <div key={c.l} data-xin="1" data-delay={80 + i * 80} data-dist="30" className={`y2k-cuenta-caja y2k-cuenta-caja--${i + 1}`}>
          <span className={`y2k-cuenta-num${i === 3 ? " y2k-cuenta-num--acc" : ""}`}>{c.v}</span>
...
const CSS_Y2K = `
  ...
  .y2k-raiz a { color: var(--pp-acc); text-decoration: none; }
  .y2k-raiz button { font: inherit; }
  .y2k-defs { position: absolute; width: 0; height: 0; overflow: hidden; }
  ...
  /* Lo que entra al scrollear se mueve en --y2k-y y gira en --y2k-giro: son
  ...
  .y2k-scroller [data-xin] { transform: translate3d(0, var(--y2k-y, 0px), 0) rotate(var(--y2k-giro, 0deg)); }

  /* Las escenas del mockup: quedan detrás y no atajan ningún toque. */
  .y2k-escena { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; z-index: 0; }

  /* ── Secciones ─────────────────────────────────────────────────────── */
  .y2k-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; display: flex;
    flex-direction: column; justify-content: center; gap: 22px;
    padding: 90px max(24px, calc((100% - 560px) / 2)) 110px; }
  .y2k-kicker { position: relative; z-index: 1; font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .y2k-kicker--tinta { color: var(--pp-ink); }
  .y2k-h2 { position: relative; z-index: 1; margin: 0; font-family: var(--y2k-serif), 'Baloo 2', cursive;
    font-weight: 400; font-size: clamp(40px, 11vw, 64px); line-height: .96; color: inherit; }
  .y2k-h2 em, .y2k-acento { font-style: italic; color: var(--pp-acc); }
  .y2k-h2--claro { color: var(--pp-night-ink); }
  .y2k-h2--album { font-size: clamp(38px, 10vw, 56px); }
  .y2k-acento-plano { color: var(--pp-acc); }
  .y2k-parrafo { position: relative; z-index: 1; margin: 0; font-size: 15px; line-height: 1.55; color: var(--pp-ink2); max-width: 420px; }
  .y2k-parrafo--tinta { color: var(--pp-ink); }
  .y2k-meta { position: relative; z-index: 1; display: flex; flex-wrap: wrap; align-items: center; gap: 10px 18px;
    font-size: 14px; letter-spacing: .12em; color: var(--pp-ink2); }
  .y2k-meta-punto { width: 4px; height: 4px; border-radius: 50%; background: var(--pp-acc); display: inline-block; }

  /* ── 01 Guardá la fecha ────────────────────────────────────────────── */
  .y2k-std { justify-content: flex-end; gap: 22px; padding: 46vh max(24px, calc((100% - 560px) / 2)) 90px;
    background: linear-gradient(180deg, var(--pp-sky1) 0%, var(--pp-sky2) 48%, var(--pp-bg) 62%); }
  .y2k-std-fecha { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 6px; }
  .y2k-std-dia { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-weight: 500;
  ...
  .y2k-std-fila { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
  .y2k-cinta { display: inline-block; padding: 8px 26px; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--y2k-serif), 'Baloo 2', cursive; font-style: italic; font-size: clamp(30px, 8vw, 44px);
  ...
  .y2k-std-anio { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: clamp(30px, 8vw, 44px); line-height: 1; color: var(--pp-ink); }

  /* ── Nuestra foto: la ventana troquelada ───────────────────────────── */
  .y2k-foto-seccion { position: relative; overflow: hidden; padding: 20px max(24px, calc((100% - 900px) / 2)) 70px; background: var(--pp-bg); }
  .y2k-foto-marco { position: relative; width: 100%; max-width: 900px; margin: 0 auto; aspect-ratio: 4 / 5; max-height: 78vh; }
  .y2k-ventana { position: absolute; inset: 0; overflow: hidden;
  ...
  .y2k-foto-firma { position: absolute; left: 6%; bottom: -26px; padding: 12px 18px; background: ${CARTA}; color: ${CARTA_TINTA};
    font-family: var(--y2k-serif), 'Baloo 2', cursive; font-style: italic; font-size: 20px;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(-2deg); }

  /* ── 02 Falta poco ─────────────────────────────────────────────────── */
  .y2k-countdown { background: linear-gradient(180deg, var(--pp-bg), var(--pp-bg2)); }
  .y2k-cuenta { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 16px 14px; max-width: 460px; }
  .y2k-cuenta-caja { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 20px 16px 16px;
  ...
  .y2k-cuenta-caja::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 20px 20px 0;
    border-color: var(--pp-bg) var(--pp-bg) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .y2k-cuenta-caja--1 { --y2k-giro: -1.6deg; transform: rotate(var(--y2k-giro)); }
  .y2k-cuenta-caja--2 { --y2k-giro: 1.2deg; transform: rotate(var(--y2k-giro)); margin-top: 10px; }
  .y2k-cuenta-caja--3 { --y2k-giro: 1.8deg; transform: rotate(var(--y2k-giro)); }
  .y2k-cuenta-caja--4 { --y2k-giro: -1.1deg; transform: rotate(var(--y2k-giro)); margin-top: 10px; }
  .y2k-cuenta-num { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: clamp(56px, 15vw, 84px); line-height: .85; font-variant-numeric: tabular-nums; }
  .y2k-cuenta-num--acc { color: var(--pp-acc); }
  .y2k-cuenta-etq { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }

  /* ── 03 Unas palabras ──────────────────────────────────────────────── */
  .y2k-frase-seccion { gap: 36px; padding: 100px max(24px, calc((100% - 620px) / 2)); background: var(--pp-bg2); }
  .y2k-frase { position: relative; z-index: 1; margin: 0; font-family: var(--y2k-serif), 'Baloo 2', cursive;
    font-weight: 400; font-size: clamp(40px, 11.5vw, 76px); line-height: 1; letter-spacing: -.01em; text-wrap: pretty; }
  .y2k-firma { position: relative; z-index: 1; display: flex; align-items: center; gap: 14px; font-size: 14px; letter-spacing: .14em; color: var(--pp-ink2); }
  .y2k-firma-linea { width: 44px; height: 1.5px; background: var(--pp-acc); display: inline-block; }

  /* ── Paneles del recorrido ─────────────────────────────────────────── */
  .y2k-pan { height: calc(100vh + var(--st-pasos, 2) * 90vh); position: relative; }
  .y2k-pan-fijo { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .y2k-panel--abajo { justify-content: flex-end; }
  .y2k-panel--centro { justify-content: center; }
  .y2k-panel--album { gap: 14px; padding: clamp(52px, 8vh, 80px) max(24px, calc((100vw - 900px) / 2)) clamp(60px, 10vh, 96px); color: ${CARTA_TINTA}; }
  ...
  .y2k-panel-top { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 11px; letter-spacing: .3em; color: var(--pp-acc); }
  .y2k-panel-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; gap: 12px;
    font-size: 12px; letter-spacing: .22em; color: var(--pp-ink2); margin-top: auto; }
  .y2k-seguir { position: relative; z-index: 1; display: flex; align-items: center; gap: 10px; font-size: 12px;
    letter-spacing: .24em; color: var(--pp-ink2); }
  .y2k-flecha { display: inline-block; animation: ppSide 2.2s ease-in-out infinite; }
  .y2k-puntos { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .y2k-punto { width: 22px; height: 3px; transition: background 400ms ease; display: inline-block; }

  /* ── Tarjetas de papel ─────────────────────────────────────────────── */
  .y2k-tarjeta { position: relative; z-index: 1; background: ${CARTA}; color: ${CARTA_TINTA}; padding: 22px 20px;
    display: flex; flex-direction: column; gap: 12px; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .y2k-tarjeta--izq { --y2k-giro: -1deg; transform: rotate(var(--y2k-giro)); }
  .y2k-tarjeta--der { --y2k-giro: 1.1deg; transform: rotate(var(--y2k-giro)); }
  .y2k-tarjeta--talon { --y2k-giro: -.8deg; transform: rotate(var(--y2k-giro)); overflow: visible; }
  .y2k-tarjeta--hoy { --y2k-giro: -1deg; transform: rotate(var(--y2k-giro)); align-items: flex-start; }
  .y2k-tarjeta--banco { margin-top: 4px; }
  .y2k-doblez::before { content: ""; position: absolute; right: 0; top: 0; border-style: solid; border-width: 0 22px 22px 0;
    border-color: var(--pp-bg2) var(--pp-bg2) ${CARTA_LINEA} ${CARTA_LINEA}; }
  .y2k-tarjeta-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .y2k-tarjeta-titulo { margin: 0; font-family: var(--y2k-serif), 'Baloo 2', cursive; font-weight: 500;
    font-size: clamp(30px, 8vw, 48px); line-height: .98; }
  .y2k-tarjeta-pregunta { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: 26px; line-height: 1.15; }
  .y2k-tarjeta-mensaje { margin: 0; font-size: 14px; line-height: 1.5; color: var(--pp-ink2); }

  .y2k-filas { display: flex; flex-direction: column; gap: 10px; font-size: 14px; line-height: 1.4; }
  .y2k-fila { display: flex; justify-content: space-between; gap: 12px; border-bottom: 1px solid ${CARTA_LINEA}; padding-bottom: 8px; }
  .y2k-fila > span:first-child { color: var(--pp-ink2); }
  .y2k-fila--ultima { border-bottom: none; padding-bottom: 0; }
  .y2k-fila-valor { text-align: right; }
  .y2k-fila--copiable { align-items: center; }
  .y2k-fila-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .y2k-fila-etq { font-size: 12px; letter-spacing: .18em; color: var(--pp-ink2); }
  .y2k-fila-dato { font-size: 15px; font-weight: 500; overflow-wrap: anywhere; }
  .y2k-btn-copiar { flex-shrink: 0; min-height: 44px; padding: 0 14px; border: 1.5px solid ${CARTA_TINTA}; background: transparent;
    color: ${CARTA_TINTA}; font-size: 13px; letter-spacing: .12em; cursor: pointer; }
  .y2k-btn-copiar--hecho { background: ${CARTA_TINTA}; color: ${CARTA}; }

  /* ── Check-in ──────────────────────────────────────────────────────── */
  .y2k-checkin { background: var(--pp-bg); }
  .y2k-talon-top { display: flex; justify-content: space-between; align-items: center; gap: 10px; font-size: 12px; letter-spacing: .24em; color: var(--pp-ink2); }
  .y2k-talon-estado { transition: color 400ms ease; flex-shrink: 0; }
  .y2k-campo { display: flex; flex-direction: column; gap: 6px; }
  .y2k-etiqueta { font-size: 12px; letter-spacing: .2em; color: var(--pp-ink2); }
  .y2k-contador { display: flex; align-items: center; gap: 12px; }
  .y2k-contador button { width: 48px; height: 48px; border: 1.5px solid ${CARTA_TINTA}; background: transparent; color: ${CARTA_TINTA}; font-size: 22px; line-height: 1; cursor: pointer; }
  .y2k-contador button:disabled { opacity: .35; cursor: default; }
  .y2k-contador > span { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: 40px; min-width: 40px; text-align: center; line-height: 1; }
  .y2k-input { min-height: 44px; border: none; border-bottom: 1.5px solid #CFC4AE; background: transparent; color: ${CARTA_TINTA};
    font-family: var(--y2k-sans), 'Comfortaa', cursive; font-size: 16px; padding: 6px 2px; }
  .y2k-input--serif { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: 20px; }
  .y2k-input:focus { outline: none; border-bottom-color: var(--pp-acc); }
  .y2k-input::placeholder { color: #9A9388; }
  .y2k-precio { display: flex; justify-content: space-between; gap: 12px; border-top: 1px dashed #CFC4AE; padding-top: 12px; font-size: 14px; color: var(--pp-ink2); }
  .y2k-precio-valor { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; }
  .y2k-precio-total { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: 26px; line-height: 1; color: ${CARTA_TINTA}; }
  .y2k-precio-detalle { font-size: 12px; color: var(--pp-ink2); }
  .y2k-sello { position: absolute; right: 14px; top: 44px; width: 92px; height: 92px; border-radius: 50%;
  ...
  .y2k-petalos { position: absolute; inset: 0; pointer-events: none; overflow: visible; }
  .y2k-error { position: relative; z-index: 1; margin: 0; font-size: 13px; color: var(--pp-acc); }

  /* ── Botones ───────────────────────────────────────────────────────── */
  .y2k-btn-solido { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center; gap: 10px;
    min-height: 52px; padding: 0 22px; border: none; background: var(--pp-acc); color: ${CARTA};
    font-family: var(--y2k-sans), 'Comfortaa', cursive; font-size: 14px; letter-spacing: .2em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }
  .y2k-btn-solido--tinta { background: var(--pp-ink); color: var(--pp-btn-fg); }
  .y2k-btn-solido:disabled { opacity: .6; cursor: default; }
  .y2k-btn-fantasma { position: relative; z-index: 1; display: inline-flex; align-items: center; justify-content: center;
  ...
  .y2k-btn-papel { position: relative; z-index: 1; display: inline-flex; align-items: center; gap: 10px; min-height: 48px;
    ...
    --y2k-giro: -1deg; transform: rotate(var(--y2k-giro)); text-decoration: none; }

  /* ── Mapa ──────────────────────────────────────────────────────────── */
  .y2k-mapa { position: relative; z-index: 1; height: 180px; border: 1.5px solid ${CARTA_LINEA}; background: ${CARTA}; overflow: hidden;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); }

  /* ── Cronograma ────────────────────────────────────────────────────── */
  .y2k-crono { position: relative; z-index: 1; display: flex; flex-direction: column; border-left: 2px solid var(--pp-acc); margin-left: 6px; }
  .y2k-crono-fila { display: flex; gap: 18px; padding: 10px 0 10px 18px; align-items: baseline; }
  .y2k-crono-hora { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: 26px; min-width: 64px; line-height: 1; }
  .y2k-crono-titulo { font-size: 15px; }

  /* ── Álbum ─────────────────────────────────────────────────────────── */
  .y2k-polaroids { position: relative; z-index: 1; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(6, 1fr);
    gap: 14px; max-width: 900px; width: 100%; margin: 0 auto; align-content: start; }
  .y2k-polaroid { position: relative; background: #FDFBF6; padding: 8px 8px 30px; min-height: 0; display: flex;
    box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); cursor: pointer; grid-column: span 2; }
  .y2k-polaroids[data-cantidad="1"] .y2k-polaroid { grid-column: span 6; }
  .y2k-polaroids[data-cantidad="2"] .y2k-polaroid { grid-column: span 3; }
  .y2k-polaroids[data-cantidad="4"] .y2k-polaroid { grid-column: span 3; }
  .y2k-polaroids[data-cantidad="5"] .y2k-polaroid:nth-child(-n+2) { grid-column: span 3; }
  .y2k-polaroid:nth-child(odd) { transform: rotate(-2deg); }
  .y2k-polaroid:nth-child(even) { transform: rotate(1.5deg); margin-top: 14px; }
  .y2k-polaroid-img { width: 100%; height: 100%; min-height: 120px; object-fit: cover; display: block;
    background: repeating-linear-gradient(135deg, #DCD7CB 0 8px, #E9E5DC 8px 16px); }
  .y2k-chinche { position: absolute; left: 50%; top: -10px; width: 18px; height: 18px; margin-left: -9px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 4px rgba(0,0,0,.32); }

  /* ── Música ────────────────────────────────────────────────────────── */
  .y2k-musica { background: linear-gradient(180deg, var(--pp-bg2), var(--pp-sky2)); }
  .y2k-lista { position: relative; z-index: 1; display: flex; flex-direction: column; gap: 10px; max-height: 200px; overflow-y: auto; }
  .y2k-lista-fila { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid rgba(43,42,51,.14); }
  .y2k-lista-texto { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .y2k-lista-tema { font-size: 15px; }
  .y2k-lista-quien { font-size: 13px; color: var(--pp-ink2); }

  /* ── Regalos ───────────────────────────────────────────────────────── */
  .y2k-regalos { background: linear-gradient(180deg, var(--pp-sky2), var(--pp-hill1) 60%, var(--pp-night)); }

  /* ── Trivia ────────────────────────────────────────────────────────── */
  .y2k-quiz { background: linear-gradient(180deg, var(--pp-hill1), var(--pp-night) 70%); color: var(--pp-night-ink); }
  .y2k-opciones { display: flex; flex-direction: column; gap: 8px; }
  .y2k-opcion { min-height: 48px; text-align: left; padding: 0 16px; border: 1.5px solid #CFC4AE; background: transparent;
    color: ${CARTA_TINTA}; font-family: var(--y2k-sans), 'Comfortaa', cursive; font-size: 15px; cursor: pointer;
    transition: background 300ms, border-color 300ms, color 300ms; }
  .y2k-opcion:disabled { cursor: default; }
  .y2k-opcion--bien { background: var(--pp-acc2); border-color: var(--pp-acc2); color: ${CARTA}; }
  .y2k-opcion--mal { border-color: var(--pp-acc); color: var(--pp-acc); }

  /* ── Tu pase ───────────────────────────────────────────────────────── */
  .y2k-pase { justify-content: flex-end; gap: 20px; background: var(--pp-night); color: var(--pp-night-ink);
    padding: 44vh max(24px, calc((100% - 560px) / 2)) calc(40px + env(safe-area-inset-bottom)); }
  .y2k-ticket { position: relative; z-index: 1; display: grid; grid-template-columns: 1fr auto; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.2), 0 8px 14px rgba(0,0,0,.14); --y2k-giro: -.6deg; transform: rotate(var(--y2k-giro));
    border-radius: 12px; overflow: hidden; }
  .y2k-ticket-izq { padding: 22px 20px; display: flex; flex-direction: column; gap: 10px; border-right: 2px dashed #CFC4AE; position: relative; }
  .y2k-ticket-der { padding: 16px 14px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .y2k-ticket-kicker { font-size: 12px; letter-spacing: .26em; color: var(--pp-ink2); }
  .y2k-ticket-nombre { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: clamp(30px, 8vw, 40px); line-height: .95; }
  .y2k-ticket-dato { font-size: 14px; color: var(--pp-ink2); }
  .y2k-ticket-fecha { font-size: 14px; letter-spacing: .14em; color: var(--pp-acc); }
  .y2k-ticket-muesca { position: absolute; right: -12px; width: 24px; height: 24px; border-radius: 50%; background: var(--pp-night); }
  .y2k-ticket-muesca--arriba { top: -12px; }
  .y2k-ticket-muesca--abajo { bottom: -12px; }
  /* El QR compartido viene con su propio marco claro: acá va sobre el talón. */
  .y2k-ticket-der .qr-ingreso, .y2k-ticket-der section { background: transparent !important; border: none !important; padding: 0 !important; margin: 0 !important; }
  .y2k-info-extra { position: relative; z-index: 1; }
  .y2k-info-extra #info-adicional { background: transparent !important; padding: 0 !important; }
  .y2k-info-extra #ia-trigger-btn { background: transparent !important; color: var(--pp-night-ink) !important;
  ...
  .y2k-raiz .ia-icon-box { display: none !important; }
  .y2k-pase-pie { position: relative; z-index: 1; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; padding-top: 10px; }
  .y2k-pase-firma { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-style: italic; font-size: 24px; color: var(--pp-night-ink); }
  .y2k-pase-creditos { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .y2k-replay { cursor: pointer; font-size: 12px; letter-spacing: .18em; color: var(--pp-acc); }
  ...
  .y2k-riel-top { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-ink2); transition: color 500ms ease; }
  .y2k-riel-linea { flex: 1; width: 1px; margin: 16px 0; background: rgba(43,42,51,.14); position: relative; }
  ...
  .y2k-riel-etiqueta { writing-mode: vertical-rl; font-size: 10px; letter-spacing: .3em; color: var(--pp-acc); transition: color 500ms ease; }

  /* ── Portada ───────────────────────────────────────────────────────── */
  .y2k-portada { position: absolute; inset: 0; z-index: 5; overflow: hidden;
    background: linear-gradient(180deg, var(--pp-sky1), var(--pp-sky2)); }
  .y2k-portada-escena { position: absolute; inset: 0; pointer-events: none; }
  .y2k-portada-contenido { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between;
  ...
  .y2k-portada-arriba { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.6vh, 14px); width: 100%; }
  .y2k-cartel { position: relative; background: ${CARTA}; color: ${CARTA_TINTA}; padding: clamp(16px, 3vh, 26px) clamp(18px, 5vw, 30px);
  ...
  .y2k-cartel-nombres { margin: 0; font-family: var(--y2k-serif), 'Baloo 2', cursive; font-weight: 500;
  ...
  .y2k-cartel-amp { font-style: italic; font-weight: 400; font-size: .6em; color: var(--pp-acc); }
  .y2k-cartel-chinche { position: absolute; left: 50%; top: -10px; width: 20px; height: 20px; margin-left: -10px; border-radius: 50%;
    background: radial-gradient(circle at 34% 30%, #fff 0 14%, var(--pp-acc) 16% 68%, rgba(0,0,0,.45) 70%);
    box-shadow: 0 3px 5px rgba(0,0,0,.34); }
  .y2k-portada-abajo { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2vh, 16px); width: 100%; pointer-events: auto; }
  .y2k-portada-datos { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 14px 20px; background: ${CARTA};
    color: ${CARTA_TINTA}; box-shadow: 0 2px 0 rgba(0,0,0,.12), 0 8px 14px rgba(0,0,0,.08); transform: rotate(.8deg);
    max-width: 100%; box-sizing: border-box; }
  .y2k-portada-saludo { font-size: 11px; letter-spacing: .28em; color: var(--pp-ink2); }
  .y2k-portada-fila { display: flex; flex-wrap: wrap; justify-content: center; align-items: center; gap: 6px 14px; font-size: 14px; letter-spacing: .14em; }
  .y2k-portada-pase { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px 10px; font-size: 11px; letter-spacing: .18em; color: var(--pp-ink2); }
  .y2k-portada-btn { width: 100%; max-width: 360px; min-height: 52px; border: none; background: var(--pp-btn-bg); color: var(--pp-btn-fg);
    font-family: var(--y2k-sans), 'Comfortaa', cursive; font-size: 14px; letter-spacing: .22em; cursor: pointer;
    box-shadow: 0 2px 0 rgba(0,0,0,.14), 0 8px 14px rgba(0,0,0,.1); transition: background 300ms ease; }
  @media (hover: hover) { .y2k-portada-btn:hover { background: var(--pp-acc); color: ${CARTA}; } }
  ...
  /* ── Lupa ──────────────────────────────────────────────────────────── */
  .y2k-lupa { position: fixed; inset: 0; z-index: 200; background: rgba(43,42,51,.94); display: flex; align-items: center;
    justify-content: center; padding: 24px; cursor: zoom-out; }
  .y2k-lupa-cerrar { position: absolute; top: 20px; right: 20px; width: 40px; height: 40px; border: 1px solid ${CARTA};
    background: transparent; color: ${CARTA}; font-size: 18px; line-height: 1; cursor: pointer; }
  .y2k-lupa-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; background: ${CARTA}; padding: 10px 10px 30px; }
  ...
  @media (min-width: 1024px) {
    .y2k-section, .y2k-panel { padding-left: max(24px, calc((100% - 620px) / 2)); padding-right: max(24px, calc((100% - 620px) / 2)); }
    .y2k-panel--album { padding-left: max(24px, calc((100vw - 900px) / 2)); padding-right: max(24px, calc((100vw - 900px) / 2)); }
  }
  ...
  .y2k-raiz { font-family: var(--y2k-sans), 'Comfortaa', cursive; }
  ...
  .y2k-trama { position: absolute; inset: 0; pointer-events: none; opacity: .14; z-index: 0;
  ...
  .y2k-kicker, .y2k-panel-top, .y2k-panel-pie, .y2k-meta, .y2k-riel-top, .y2k-riel-etiqueta,
  .y2k-ticket-kicker, .y2k-cuenta-etq, .y2k-tarjeta-kicker, .y2k-fila-etq, .y2k-portada-pase {
    font-family: var(--y2k-mono), 'VT323', monospace; }

  /* El titular: Instrument Serif enorme, con la segunda línea en itálica y en
     el acento. Es la firma de la sub-colección. */
  .y2k-h2, .y2k-frase, .y2k-tarjeta-titulo, .y2k-std-dia, .y2k-cartel-nombres, .y2k-ticket-nombre, .y2k-cifra, .y2k-cuenta-num {
    font-family: var(--y2k-serif), 'Baloo 2', cursive; letter-spacing: -.03em; }
  .y2k-h2 em, .y2k-acento { font-style: italic; color: var(--pp-acc); }

  /* Save the Date: el día ocupa la hoja entera. */
  .y2k-std { background: var(--pp-ink); color: var(--pp-bg); padding: 64px max(22px, calc((100% - 560px) / 2)) 80px; justify-content: center; }
  .y2k-std-dia { font-size: clamp(64px, 22vw, 180px); line-height: .82; text-shadow: none; color: inherit; }
  .y2k-std-fila { justify-content: space-between; }
  .y2k-cinta { background: none; color: var(--pp-acc); font-style: italic; clip-path: none; box-shadow: none;
    animation: none; padding: 0; font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .y2k-std-anio { font-family: var(--y2k-serif), 'Baloo 2', cursive; font-size: clamp(64px, 22vw, 180px); line-height: .82; }
  .y2k-std .y2k-kicker, .y2k-std .y2k-meta { color: rgba(245,241,234,.62); }

  /* La foto va enmarcada, como una foto de tapa. */
  .y2k-foto-marco { border: 3px solid var(--pp-ink); }
  .y2k-ventana { clip-path: none !important; box-shadow: none; }
  .y2k-foto-firma { background: var(--pp-acc); color: var(--pp-bg); transform: none; box-shadow: none;
    font-family: var(--y2k-serif), 'Baloo 2', cursive; font-style: italic; }

  /* Las tarjetas son bloques de color, no papelitos inclinados. */
  .y2k-tarjeta { background: var(--pp-bg2); color: var(--pp-ink); box-shadow: none; border: 1px solid color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .y2k-tarjeta--izq, .y2k-tarjeta--der, .y2k-tarjeta--talon, .y2k-tarjeta--hoy { transform: none; }
  .y2k-doblez::before { display: none; }
  .y2k-fila { border-bottom-color: color-mix(in srgb, var(--pp-ink) 14%, transparent); }
  .y2k-chinche { display: none; }

  /* La cuenta regresiva: cuatro cifras al hilo, separadas por filetes. */
  .y2k-cuenta { grid-template-columns: repeat(4, 1fr); gap: 0; max-width: 560px; }
  .y2k-cuenta-caja { background: none; box-shadow: none; transform: none !important; margin: 0 !important;
    border-left: 1px solid color-mix(in srgb, var(--pp-ink) 18%, transparent); padding: 10px 12px; }
  .y2k-cuenta-caja:first-child { border-left: none; }
  .y2k-cuenta-caja::before { display: none; }
  .y2k-cuenta-num { font-size: clamp(38px, 12vw, 72px); line-height: .9; }
  ...
  .y2k-panel { padding: 64px max(22px, calc((100vw - 560px) / 2)) 80px; }
  .y2k-panel[data-tone="dark"] { background: var(--pp-ink) !important; color: var(--pp-bg); }
  .y2k-panel[data-tone="dark"] .y2k-tarjeta { background: transparent; border-color: rgba(245,241,234,.22); color: inherit; }
  .y2k-panel-top, .y2k-panel-pie { letter-spacing: .22em; font-size: 11px; }

  /* El pase es un cupón troquelado, no un ticket con muescas redondas. */
  .y2k-ticket { background: var(--pp-bg); color: var(--pp-ink); border: 1px solid var(--pp-ink); border-radius: 0; transform: none; box-shadow: none; }
  .y2k-ticket-izq { border-right: 1px dashed color-mix(in srgb, var(--pp-ink) 40%, transparent); }
  .y2k-ticket-muesca { display: none; }

  /* Botones: rectángulos con el acento, sin sombra ni inclinación. */
  .y2k-btn-solido, .y2k-btn-papel, .y2k-portada-btn { border-radius: 0; box-shadow: none; transform: none; }
  .y2k-btn-papel { background: transparent; border: 1px solid currentColor; }
  .y2k-opcion { border-color: color-mix(in srgb, var(--pp-ink) 24%, transparent); border-radius: 0; }

  /* La portada: sin capas de papel. Un titular enorme sobre la trama, el
     folio arriba y el botón abajo. */
  .y2k-portada { background: var(--pp-bg); }
  .y2k-cartel { background: none; box-shadow: none; transform: none; padding: 0; }
  .y2k-cartel-nombres { font-size: min(clamp(52px, 17vw, 120px), 13vh); line-height: .86; display: block; }
  .y2k-cartel-amp { display: block; font-style: italic; color: var(--pp-acc); font-size: .7em; }
  .y2k-cartel-chinche { display: none; }
  .y2k-portada-datos { background: none; box-shadow: none; transform: none; padding: 0; gap: 12px; }
  .y2k-portada-btn { background: var(--pp-ink); color: var(--pp-bg); }
  @media (hover: hover) { .y2k-portada-btn:hover { background: var(--pp-acc); color: var(--pp-bg); } }

  /* Y2K: burbujas. Nada es cuadrado y el titular tiene brillo de cromo. */
  .y2k-tarjeta, .y2k-ticket, .y2k-btn-solido, .y2k-portada-btn, .y2k-opcion, .y2k-cuenta-caja { border-radius: 22px; }
  .y2k-cartel-nombres, .y2k-h2 { letter-spacing: -.01em; text-shadow: 0 2px 0 #fff, 0 6px 0 color-mix(in srgb, var(--pp-acc) 40%, transparent); }

  @media (prefers-reduced-motion: reduce) {
    .y2k-raiz * { animation: none !important; }
    .y2k-scroller [data-xin] { opacity: 1 !important; --y2k-y: 0px; }
  }
`;
