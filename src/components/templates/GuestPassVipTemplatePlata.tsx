...
export function GuestPassVipTemplatePlata({ invitation, guest, isPersonalized = false }: GuestPassVipTemplateProps) {
  const tx = useTextos();
  ...
  const coverKickerText = showGuestNameInCover ? tx("invitacion.sabor.accesoBoda").toUpperCase() : tx("invitacion.sabor.accesoBodaDe").toUpperCase();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E4EBF1";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#B6C4CF";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(182,196,207,.35), 0 18px 50px -30px #B6C4CF";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#5C6C78" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(182,196,207,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(182,196,207,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#5C6C78" : "#B6C4CF";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#08090B", tinta: "#F4F1EA", acento: "#B6C4CF", clase: `${gpBodoni.variable} ${gpMono.variable}`, fuente: "var(--gp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="gpv-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="gpv-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #171B1F 0%, #0A0C0E 55%, #08090B 100%)" }}>
          <span data-xin="1" data-dist="-60" className="gpv-kicker">{"01 — " + tx("invitacion.saveTheDate.guardaLaFecha").toUpperCase()}</span>
          ...
          <div data-drift="-70" className="gpv-medallion gpv-medallion--corner">
            <Medallion label="VIP" sub={tx("invitacion.pase.acceso").toUpperCase()} arcId="gpArc1" arcText={tx("invitacion.sabor.arcoGuestPass").toUpperCase()} spin="normal" />
          </div>
        </section>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="gpv-section gpv-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #20262B 0%, #0C0E10 55%, #08090B 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="gpv-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1A1E22 0%, #0B0D0F 52%, #08090B 100%)" }}>
          <div data-drift="-130" className="gpv-glow-blob" />
          <span data-xin="1" data-dist="-60" className="gpv-kicker" style={{ position: "relative" }}>{knPre(3)} {"— " + tx("invitacion.frase.unMensajeParaVos").toUpperCase()}</span>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="gpv-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
            ...
            <div data-strip="1" className="gpv-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="gpv-panel gpv-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <div className="gpv-seguir">{tx("invitacion.portada.seguiBajando").toUpperCase()} <span className="gpv-side-hint">{scrollVertical ? "↓" : "→"}</span></div>
                </div>
              )}
              ...
              <div id="details" data-tone="light" className="gpv-panel gpv-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                <div className="gpv-facts">
                  ...
                  {dressCode && (
                    <div className="gpv-facts-row gpv-facts-row--last">
                      <span>{tx("invitacion.pase.codigo").toUpperCase()}</span><span className="gpv-accent-serif-2">{dressCode.toUpperCase()}</span>
                    </div>
                  )}
                </div>
                {cronograma.length > 0 && (
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="gpv-panel gpv-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="gpv-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#5C6C78" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#5C6C78" />
                  </svg>
                  <div className="gpv-panel-block">
                    <span className="gpv-mini-label">{ceremoniaHabilitada ? "03" : "02"} / {LUGAR_PANEL_COUNT}</span>
                    <span className="gpv-panel-title-sm">{tx("invitacion.ubicacion.comoLlegar")}</span>
              ...
              <div data-tone="dark" className="gpv-panel gpv-panel--center" style={{ background: "#0A0C0E", color: "#F4F1EA" }}>
                <div className="gpv-medallion gpv-medallion--lg">
                  <Medallion label={dressCode ? dressCode.toUpperCase() : tx("invitacion.pase.acceso").toUpperCase()} sub={tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()} arcId="gpArc2" arcText={tx("invitacion.pase.arcoAccesoVipPase", { n: passNumber }).toUpperCase()} spin="reverse" title={tx("invitacion.pase.reservado")} />
                </div>
                <span className="gpv-mini-label">{LUGAR_PANEL_COUNT} / {LUGAR_PANEL_COUNT} {"— " + tx("invitacion.ubicacion.tuUbicacion").toUpperCase()}</span>
                {/* mesa-en-ubicacion */}
                {guest?.mesas && guest.mesas.length > 0 && (
                  <span
                    className="gpv-mini-label"
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="gpv-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #171B1F 0%, #0A0C0E 60%, #08090B 100%)" }}>
          ...
          <h2 data-xin="1" data-delay="80" data-dist="130" className="gpv-h2">
            {tx("invitacion.rsvp.confirmaLinea1")}<br /><span className="gpv-accent-italic">{tx("invitacion.rsvp.confirmaLinea2")}</span>
          </h2>
          ...
          {rsvpEnabled ? (
            ...
            <p className="gpv-lead">{tx("invitacion.rsvp.cerrada")}</p>
          )}
        </section>

        <div id="album" data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="gpv-pan">
          <div className="gpv-pan-sticky">
            <div data-strip="1" className="gpv-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="gpv-panel gpv-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  {pageIndex === 0 && <h2 className="gpv-panel-title-md">{tx("invitacion.album.titulo")} <span className="gpv-accent-serif">{tx("invitacion.album.deFotos")}</span></h2>}
                  <div className="gpv-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      ...
                      <span className="gpv-photo-placeholder">{tx("invitacion.album.sinFotos")}</span>
                    )}
                  </div>
                  <div className="gpv-seguir gpv-seguir--split">
                    <span>{tx("invitacion.album.fotosSubidas", { n: allPhotos.length }).toUpperCase()}</span>
                    <span className="gpv-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + " →"}</span>
                  </div>
                </div>
              ))}

              <div data-tone="light" className="gpv-panel gpv-panel--gap" style={{ background: "#EDE8DE", color: "#14141B" }}>
                <span className="gpv-panel-top" style={{ display: "block" }}>{tx("invitacion.album.hoja", { n: String(photoPages.length + 1).padStart(2, "0"), etiqueta: "— " + tx("invitacion.enVivo.titulo").toUpperCase() }).toUpperCase()}</span>
                <h2 className="gpv-panel-title">{tx("invitacion.album.tituloLinea1")}<br /><span className="gpv-accent-serif">{tx("invitacion.album.tituloLinea2")}</span></h2>
                <div className="gpv-album-embed">
                  {livePhotos.length > 0 ? (
                    <LiveAlbumStrip photos={livePhotos} tone="light" accentColor="#5C6C78" />
                  ) : (
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="gpv-section" style={{ background: "#0A0C0E" }}>
            <span data-xin="1" data-dist="-60" className="gpv-kicker">{kn(6)} {"— " + tx("invitacion.musica.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="gpv-h2">{tituloEnDosLineas(tx("invitacion.sabor.preguntaTemaBailar"), "gpv-accent-italic")}</h2>
            <div data-xin="1" data-delay="160" data-dist="-80" className="gpv-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="gpv-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E4EBF1" : "#B6C4CF" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="gpv-section" style={{ background: "#0A0C0E" }}>
            <span data-xin="1" data-dist="-60" className="gpv-kicker">{sugerenciaMusicaHabilitada ? kn(7) : kn(6)} {"— " + tx("invitacion.regalos.kicker").toUpperCase()}</span>
            <h2 data-xin="1" data-delay="80" data-dist="140" className="gpv-h2">
              {tx("invitacion.regalos.siQueresLinea1")}<br /><span className="gpv-accent-italic">{tx("invitacion.regalos.siQueresLinea2")}</span>
            </h2>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="gpv-section" style={{ background: "#0A0C0E" }}>
            <span data-xin="1" data-dist="-60" className="gpv-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection].filter(Boolean).length + 6)} {"— " + tx("invitacion.quiz.kicker").toUpperCase()}</span>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="gpv-section gpv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #171B1F 0%, #0A0C0E 55%, #08090B 100%)" }}>
          <span data-xin="1" data-dist="-60" className="gpv-kicker">{knAcc([sugerenciaMusicaHabilitada, showBankSection, quizEnabled].filter(Boolean).length + 6)} {"— " + tx("invitacion.pase.guardaTuPase").toUpperCase()}</span>
          <div data-xin="1" data-delay="100" data-dist="130" className="gpv-final-card">
            <div className="gpv-medallion gpv-medallion--final">
              <Medallion label="VIP" sub={confirmed ? "CONFIRMADO" : tx("invitacion.pase.pendiente").toUpperCase()} arcId="gpArc3" arcText={textoArco(namesTitle, fechaCorta)} spin="reverse" />
            </div>
            <span className="gpv-mini-label gpv-accent-serif-2">{tx("invitacion.pase.numeroPaseAdmite", { n: passNumber, cantidad: guestAdults + guestTeens + guestChildren || 1 }).toUpperCase()}</span>
          ...
          <div className="gpv-final-footer">
            ...
            <span className="gpv-replay" onClick={reset}>{tx("invitacion.portada.verAperturaOtraVez").toUpperCase() + " ↺"}</span>
          </div>
      ...
      <div ref={railRef} className="gpv-rail">
        <span ref={railTopRef} className="gpv-rail-top">{tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()}</span>
      ...
      <div ref={coverRef} className="gpv-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="gpv-cover-half gpv-cover-half--top">
          ...
          >
            <div className="gpv-cover-cta">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</div>
          </CoverHalf>
        </div>
        <div ref={bottomRef} className="gpv-cover-half gpv-cover-half--bottom">
          ...
          photoMobile={photoMobile}
            photoDesktop={photoDesktop}
          >
            <button onClick={open} className="gpv-cover-cta gpv-cover-cta--btn">{tx("invitacion.portada.abrirInvitacion").toUpperCase()}</button>
          </CoverHalf>
        </div>
      </div>

      <div ref={hintRef} className="gpv-hint">{tx("invitacion.portada.desliza").toUpperCase() + " ↓"}</div>
  ...
  return (
    <>
      ...
      <div className="gpv-medallion-core">
        {title && <span className="gpv-medallion-sub">{tx("invitacion.pase.sector").toUpperCase()}</span>}
  ...
  if (status === "DECLINED") {
    return (
      <div className="gpv-rsvp-declined">
        <p className="gpv-rsvp-declined-text">{tx("invitacion.rsvp.graciasPorAvisarAcceso")}</p>
  ...
  return (
    <>
      <div className="gpv-rsvp-rows">
        ...
        {status === "CONFIRMED" && (
          <>
            {totalGuests > 1 && adultCount > 0 && <div className="gpv-rsvp-row"><span>{tx("invitacion.rsvp.adultos").toUpperCase()}</span><span>{String(adultCount).padStart(2, "0")}</span></div>}
            {teenCount > 0 && <div className="gpv-rsvp-row"><span>{tx("invitacion.rsvp.adolescentes").toUpperCase()}</span><span>{String(teenCount).padStart(2, "0")}</span></div>}
            {childCount > 0 && <div className="gpv-rsvp-row"><span>{tx("invitacion.rsvp.ninos").toUpperCase()}</span><span>{String(childCount).padStart(2, "0")}</span></div>}
          </>
        )}
          ...
          <div className="gpv-rsvp-row gpv-rsvp-row--payment">
            ...
            <div className="gpv-rsvp-payment-value">
              ...
              {paymentStatus === "PARTIAL" && (
                <div className="gpv-rsvp-payment-detail"><span>{tx("invitacion.pago.pagoParcialRegistrado")}</span></div>
              )}
    ...
    <div className="gpv-song">
      <form onSubmit={handleSubmit} className="gpv-song-row">
        <div className="gpv-song-inputs">
          <input value={artist} onChange={(e) => setArtist(e.target.value)} placeholder={tx("invitacion.musica.artista").toUpperCase()} maxLength={80} className="gpv-song-input" />
          <span className="gpv-song-sep">—</span>
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tx("invitacion.musica.tema").toUpperCase()} maxLength={100} className="gpv-song-input" />
        </div>
        ...
        <div className="gpv-song-list">
          {songs.slice(0, 12).map((s, i) => (
            <div key={s.id} className="gpv-song-item">
              ...
              <span className="gpv-song-item-by">{tx("invitacion.musica.sumadoPor")} {s.guestName || tx("invitacion.evento.invitado")}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <div className="gpv-cover-inner">
      ...
      <div className="gpv-cover-content">
        <div className="gpv-cover-top-row">
          <span>{tx("invitacion.pase.numeroPase", { n: passNumber }).toUpperCase()}</span><span className="gpv-accent-serif-2">ALL ACCESS</span>
        </div>
        ...
        <div className="gpv-cover-bottom">
          ...
          <div className="gpv-barcode-wrap">
            ...
            <span className="gpv-mini-label" style={{ color: "#56534A" }}>{tx("invitacion.pase.noTransferible").toUpperCase()}</span>
          </div>
        </div>
      </div>
    </div>
...
const GP_CSS = `
  ...
  .gpv-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #08090B; }
  ...
  @media (min-width: 768px) {
    .gpv-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(182,196,207,.3); }
  ...
  .gpv-date-month { font-family: var(--gp-bodoni), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B6C4CF; padding-left: 12%; }

  .gpv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .gpv-divider-line { width: 52px; height: 1px; background: #B6C4CF; display: inline-block; }
  ...
  .gpv-cal-link:hover { color: #B6C4CF; }
  ...
  .gpv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #B6C4CF, #E4EBF1, #6E5AA8); filter: saturate(.75); }
  .gpv-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0B0D0F; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .gpv-medallion-label { font-family: var(--gp-bodoni), serif; font-size: 26px; line-height: 1; color: #E4EBF1; }
  .gpv-medallion-label-sm { font-family: var(--gp-bodoni), serif; font-size: 16px; color: #E4EBF1; }
  .gpv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .gpv-medallion-sub--accent { color: #B6C4CF; }
  ...
  .gpv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B6C4CF; opacity: 0.4; font-family: var(--gp-mono), monospace; }

  .gpv-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(182,196,207,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .gpv-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E4EBF1, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .gpv-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .gpv-cd-box { border: 1px solid #20262B; background: rgba(16,18,21,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .gpv-cd-num { font-family: var(--gp-bodoni), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F5F8FA; display: inline-block; }
  .gpv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B6C4CF; }
  .gpv-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #08090B 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  .gpv-perf-strip--reveal { clip-path: inset(0 100% 0 0); transition: clip-path 900ms cubic-bezier(.16,1,.3,1) 500ms; }

  .gpv-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.5), rgba(78,127,134,0.32), rgba(182,196,207,0.46), rgba(110,90,168,0.5)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .gpv-accent-italic { font-style: italic; color: #B6C4CF; }
  .gpv-accent-serif { font-style: italic; color: #5C6C78; font-family: var(--gp-bodoni), serif; }
  .gpv-accent-serif-2 { color: #5C6C78; }
  ...
  .gpv-pan[data-scroll="vertical"] { height: auto; }
  .gpv-pan[data-scroll="vertical"] .gpv-pan-sticky { position: static; height: auto; overflow: visible; }
  .gpv-pan[data-scroll="vertical"] .gpv-strip { position: static; display: block; width: 100%; transform: none !important; }
  .gpv-pan[data-scroll="vertical"] .gpv-panel { height: calc(var(--vh, 1vh) * 100); }
  ...
  .gpv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #5C6C78; margin-top: auto; }
  ...
  .gpv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #5C6C78; }
  ...
  .gpv-crono-time { font-family: var(--gp-mono), monospace; color: #5C6C78; min-width: 42px; }
  ...
  .gpv-stub { position: relative; overflow: hidden; border: 1px solid #20262B; background: rgba(16,18,21,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .gpv-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E4EBF1, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .gpv-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(182,196,207,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .gpv-rsvp-payment-total { color: #E4EBF1; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .gpv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B6C4CF; background: transparent; color: #B6C4CF; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .gpv-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(182,196,207,0.3); color: #F4F1EA; font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .gpv-rsvp-input::placeholder { color: #6E6A5D; }
  .gpv-rsvp-input:focus { outline: none; border-bottom-color: #B6C4CF; }
  .gpv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--gp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B6C4CF, #8296A3); border: 1px solid #B6C4CF; color: #0A0C0E; cursor: pointer; }
  .gpv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .gpv-rsvp-btn--ghost { background: transparent; color: #B6C4CF; }
  .gpv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B6C4CF; margin: 0; }
  ...
  .gpv-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(182,196,207,0.18); }
  ...
  .gpv-bank-copy { flex-shrink: 0; font-family: var(--gp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B6C4CF; background: transparent; color: #B6C4CF; cursor: pointer; }
  .gpv-bank-copy:hover { background: rgba(182,196,207,0.12); }
  ...
  .gpv-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(182,196,207,0.3); padding-bottom: 12px; }
  ...
  .gpv-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(182,196,207,0.3); color: #F4F1EA; font-family: var(--gp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .gpv-song-input::placeholder { color: #6E6A5D; }
  .gpv-song-input:focus { outline: none; border-bottom-color: #B6C4CF; }
  .gpv-song-sep { color: #8A8577; flex-shrink: 0; }
  .gpv-song-submit { flex-shrink: 0; background: none; border: none; color: #B6C4CF; font-family: var(--gp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .gpv-song-submit:hover { color: #E4EBF1; }
  .gpv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .gpv-song-error { font-size: 10px; color: #B6C4CF; margin-top: 6px; }
  ...
  .gpv-quiz-opt { font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(182,196,207,0.4); background: transparent; color: #B6C4CF; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .gpv-quiz-opt:disabled { cursor: default; }
  .gpv-quiz-opt--picked { background: #B6C4CF; border-color: #B6C4CF; color: #0A0C0E; }
  .gpv-quiz-opt--correct { background: #B6C4CF; border-color: #B6C4CF; color: #0A0C0E; }
  .gpv-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .gpv-quiz-result { padding-top: 18px; border-top: 1px solid rgba(182,196,207,0.2); }
  .gpv-quiz-result-score { margin: 0 0 6px; font-family: var(--gp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E4EBF1; }
  .gpv-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .gpv-final-card { border: 1px solid #B6C4CF; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .gpv-final-names { font-family: var(--gp-bodoni), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E4EBF1; }
  ...
  .gpv-replay { cursor: pointer; color: #B6C4CF; }
  .gpv-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .gpv-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(182,196,207,0.14); }
  .gpv-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .gpv-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(182,196,207,0.16); position: relative; }
  .gpv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E4EBF1, #B6C4CF); transition: height 260ms linear; display: block; }
  .gpv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B6C4CF; transition: color 500ms ease; }
  ...
  .gpv-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #181C20 0%, #0A0C0E 46%, #08090B 100%); }
  .gpv-cover-half--bottom .gpv-cover-inner { top: auto; bottom: 0; }
  .gpv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(110,90,168,0.7), rgba(78,127,134,0.45), rgba(182,196,207,0.65), rgba(110,90,168,0.7)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  .gpv-cover-sunburst { position: absolute; inset: 0; background: repeating-conic-gradient(from 0deg at 50% 44%, rgba(228,235,241,0.75) 0deg 0.4deg, transparent 0.4deg 3.2deg); opacity: .3; -webkit-mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); mask-image: radial-gradient(closest-side at 50% 44%, #000 14%, rgba(0,0,0,0.5) 46%, transparent 82%); }
  ...
  .gpv-cover-names { margin: 0; font-family: var(--gp-bodoni), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E4EBF1; }
  .gpv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B6C4CF, transparent); display: block; }
  ...
  .gpv-cover-cta { border: 1px solid #B6C4CF; background: linear-gradient(100deg, rgba(182,196,207,0.08), rgba(228,235,241,0.2), rgba(182,196,207,0.08)); color: #F5F8FA; font-family: var(--gp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .gpv-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .gpv-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(228,235,241,0.34), rgba(245,248,250,0.5), rgba(228,235,241,0.34)); color: #0A0C0E; }
  }
  ...
  .gpv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B6C4CF; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
