...
/**
 * CoronaEscarlataTemplateImperial.tsx
 *
...
export function CoronaEscarlataTemplateImperial({ invitation, guest, isPersonalized = false }: CoronaEscarlataTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F8E8ED";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#E3A9BB";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(227,169,187,.35), 0 18px 50px -30px #E3A9BB";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#7A3F8C" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(227,169,187,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(227,169,187,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#7A3F8C" : "#E3A9BB";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#150720", tinta: "#F4F1EA", acento: "#E3A9BB", clase: `${cnePlayfair.variable} ${cneMono.variable}`, fuente: "var(--cne-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="cne-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="cne-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #1C1229 0%, #2A0E3A 55%, #150720 100%)" }}>
            ...
            <div className="cne-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="21,7,32" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="21,7,32" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="cne-section cne-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #3E1652 0%, #2A1830 55%, #150720 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cne-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1F1329 0%, #120A18 52%, #150720 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="cne-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="cne-pan-sticky">
            <div data-strip="1" className="cne-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cne-panel cne-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cne-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#7A3F8C" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#7A3F8C" />
                  </svg>
              ...
              <div data-tone="dark" className="cne-panel cne-panel--center" style={{ background: "#2A0E3A", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="cne-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #1C1229 0%, #2A0E3A 60%, #150720 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cne-section" style={{ background: "#2A0E3A" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="cne-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cne-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F8E8ED" : "#E3A9BB" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="cne-section" style={{ background: "#2A0E3A" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="cne-section" style={{ background: "#2A0E3A" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu corona" className="cne-section cne-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #1C1229 0%, #2A0E3A 55%, #150720 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#E3A9BB" guest={guest} />
      )}
    ...
    <>
      ...
      <div className="cne-medallion-core">
        <svg viewBox="0 0 60 34" className={compact ? "cne-tiara-svg-sm" : "cne-tiara-svg"}>
          <path d="M4 30 L4 16 L16 24 L30 6 L44 24 L56 16 L56 30 Z" fill="none" stroke="#F8E8ED" strokeWidth={2.2} />
        </svg>
...
function CneCoverHalf({
  ...
  return (
    <div className="cne-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#E3A9BB" tintColor2="#150720" effect="enfoque" scrimColorRgb="21,7,32" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#E3A9BB" tintColor2="#150720" effect="enfoque" scrimColorRgb="21,7,32" />
        </div>
      )}
...
const CNE_CSS = `
  ...
  a { color: #E3A9BB; text-decoration: none; }
  a:hover { color: #F8E8ED; }
  ...
  .cne-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #150720; }
  ...
  @media (min-width: 768px) {
    .cne-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(227,169,187,.3); }
  ...
  .cne-date-month { font-family: var(--cne-playfair), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #E3A9BB; padding-left: 12%; }

  .cne-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cne-divider-line { width: 52px; height: 1px; background: #E3A9BB; display: inline-block; }
  ...
  .cne-cal-link:hover { color: #E3A9BB; }
  ...
  .cne-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #7A3F8C, #5A2E63, #E3A9BB, #F8E8ED, #7A3F8C); filter: saturate(.75); }
  ...
  .cne-medallion-sub--accent { color: #E3A9BB; }
  .cne-medallion-arc { position: absolute; inset: -14%; }
  .cne-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #E3A9BB; opacity: 0.4; font-family: var(--cne-mono), monospace; }

  .cne-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(227,169,187,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .cne-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F8E8ED, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .cne-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .cne-cd-box { border: 1px solid #2E1F2E; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .cne-cd-num { font-family: var(--cne-playfair), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F8E8ED; display: inline-block; }
  .cne-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #E3A9BB; }
  .cne-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #150720 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .cne-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(227,169,187,0.3), rgba(122,63,140,0.24), rgba(227,169,187,0.34), rgba(227,169,187,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .cne-accent-italic { font-style: italic; color: #E3A9BB; }
  .cne-accent-serif { font-style: italic; color: #7A3F8C; font-family: var(--cne-playfair), serif; }
  .cne-accent-serif-2 { color: #7A3F8C; }
  ...
  .cne-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #7A3F8C; margin-top: auto; }
  ...
  .cne-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #7A3F8C; }
  ...
  .cne-crono-time { font-family: var(--cne-mono), monospace; color: #7A3F8C; min-width: 42px; }
  ...
  .cne-stub { position: relative; overflow: hidden; border: 1px solid #2E1F2E; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .cne-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F8E8ED, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .cne-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(227,169,187,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .cne-rsvp-payment-total { color: #F8E8ED; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .cne-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #E3A9BB; background: transparent; color: #E3A9BB; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .cne-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(227,169,187,0.3); color: #F4F1EA; font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cne-rsvp-input::placeholder { color: #6E6A5D; }
  .cne-rsvp-input:focus { outline: none; border-bottom-color: #E3A9BB; }
  .cne-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cne-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #E3A9BB, #7A3F8C); border: 1px solid #E3A9BB; color: #0B0B10; cursor: pointer; }
  .cne-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cne-rsvp-btn--ghost { background: transparent; color: #E3A9BB; }
  .cne-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #E3A9BB; margin: 0; }
  ...
  .cne-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(227,169,187,0.18); }
  ...
  .cne-bank-copy { flex-shrink: 0; font-family: var(--cne-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #E3A9BB; background: transparent; color: #E3A9BB; cursor: pointer; }
  .cne-bank-copy:hover { background: rgba(227,169,187,0.12); }
  ...
  .cne-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(227,169,187,0.3); padding-bottom: 12px; }
  ...
  .cne-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(227,169,187,0.3); color: #F4F1EA; font-family: var(--cne-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cne-song-input::placeholder { color: #6E6A5D; }
  .cne-song-input:focus { outline: none; border-bottom-color: #E3A9BB; }
  .cne-song-sep { color: #8A8577; flex-shrink: 0; }
  .cne-song-submit { flex-shrink: 0; background: none; border: none; color: #E3A9BB; font-family: var(--cne-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cne-song-submit:hover { color: #F8E8ED; }
  .cne-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cne-song-error { font-size: 10px; color: #E3A9BB; margin-top: 6px; }
  ...
  .cne-quiz-opt { font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(227,169,187,0.4); background: transparent; color: #E3A9BB; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cne-quiz-opt:disabled { cursor: default; }
  .cne-quiz-opt--picked { background: #E3A9BB; border-color: #E3A9BB; color: #0B0B10; }
  .cne-quiz-opt--correct { background: #E3A9BB; border-color: #E3A9BB; color: #0B0B10; }
  .cne-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cne-quiz-result { padding-top: 18px; border-top: 1px solid rgba(227,169,187,0.2); }
  .cne-quiz-result-score { margin: 0 0 6px; font-family: var(--cne-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F8E8ED; }
  .cne-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .cne-final-card { border: 1px solid #E3A9BB; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .cne-final-names { font-family: var(--cne-playfair), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F8E8ED; }
  ...
  .cne-replay { cursor: pointer; color: #E3A9BB; }
  .cne-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cne-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(227,169,187,0.14); }
  .cne-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cne-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(227,169,187,0.16); position: relative; }
  .cne-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F8E8ED, #E3A9BB); transition: height 260ms linear; display: block; }
  .cne-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #E3A9BB; transition: color 500ms ease; }
  ...
  .cne-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #431A5C 0%, #2A0E3A 46%, #150720 100%); }
  .cne-cover-half--bottom .cne-cover-inner { top: auto; bottom: 0; }
  .cne-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(122,63,140,0.6), rgba(74,14,20,0.5), rgba(227,169,187,0.55), rgba(122,63,140,0.6)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .cne-cover-lattice { position: absolute; inset: 0; background-image: repeating-linear-gradient(45deg, rgba(227,169,187,.14) 0 1px, transparent 1px 24px), repeating-linear-gradient(-45deg, rgba(227,169,187,.14) 0 1px, transparent 1px 24px); }
  ...
  .cne-cover-cta { border: 1px solid #E3A9BB; background: linear-gradient(100deg, rgba(227,169,187,0.08), rgba(248,232,237,0.2), rgba(227,169,187,0.08)); color: #F8E8ED; font-family: var(--cne-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cne-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cne-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(122,63,140,0.34), rgba(248,232,237,0.5), rgba(122,63,140,0.34)); color: #0B0B0F; }
  }
  ...
  .cne-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #E3A9BB; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
