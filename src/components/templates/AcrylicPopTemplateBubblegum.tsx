...
/**
 * AcrylicPopTemplateBubblegum.tsx
 *
 * Portado 1:1 desde el diseño "Acrylic Pop" (Colección Storytelling, mis
 * quince años: fondo tinta casi negra, acento neón rosa chicle/azul eléctrico/verde menta estilo
...
// Ciclo de acento rosa chicle/azul eléctrico/verde menta (variante "Chicle Eléctrico") del mockup para las palabras destacadas de
// la frase (mismo orden en que aparecen: prende=rosa chicle, una=azul eléctrico, sola=verde menta,
// vez.=fucsia otra vez).
const PHRASE_ACCENTS = ["#FF4FC3", "#2E6BFF", "#3DFFB8"];
...
interface AcrylicPopTemplateBubblegumProps {
...
export function AcrylicPopTemplateBubblegum({ invitation, guest, isPersonalized = false }: AcrylicPopTemplateBubblegumProps) {
  const tx = useTextos();
  ...
  // plano, segunda mitad itálica ciclando rosa chicle/azul eléctrico/verde menta (mismo orden que
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#3DFFB8";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#FF4FC3";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #FF4FC3";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#2E6BFF" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#2E6BFF" : "#FF4FC3";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0B0E1C", tinta: "#F4F1EA", acento: "#FF4FC3", clase: `${acpPoppins.variable} ${acpMono.variable}`, fuente: "var(--acp-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="acp-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="acp-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #131229 0%, #0F1020 55%, #0B0E1C 100%)" }}>
            ...
            <div className="acp-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="11,14,28" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="11,14,28" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="acp-section acp-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #161331 0%, #0D0E22 55%, #0B0E1C 100%)" }}>
          ...
          <div className="acp-cd-grid">
            ...
            <AcpCdBox refEl={sRef} delay={280} dist={170} label={tx("invitacion.cuentaRegresiva.segundosAbrev").toUpperCase()} numColor="#3DFFB8" />
          </div>
          <div className="acp-perf-strip" />
        </section>

        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="acp-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #181535 0%, #0C0D1E 52%, #0B0E1C 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="acp-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="acp-pan-sticky">
            <div data-strip="1" className="acp-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="acp-panel acp-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="acp-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#2E6BFF" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#2E6BFF" />
                  </svg>
              ...
              <div data-tone="dark" className="acp-panel acp-panel--center" style={{ background: "#0F1020", color: "#F4F1EA" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="acp-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #131229 0%, #0F1020 60%, #0B0E1C 100%)" }}>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="acp-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="acp-pan-sticky">
            <div data-strip="1" className="acp-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="acp-panel acp-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="acp-seguir acp-seguir--split">
                    ...
                    <span className="acp-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <AcpDots count={photoPages.length} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="acp-section" style={{ background: "#0F1020" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="acp-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="acp-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#3DFFB8" : "#FF4FC3" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="acp-section" style={{ background: "#0F1020" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="acp-section" style={{ background: "#0F1020" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="acp-section acp-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #131229 0%, #0F1020 55%, #0B0E1C 100%)" }}>
      ...
      <div ref={coverRef} className="acp-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#FF4FC3" guest={guest} />
      )}
...
const ACP_CSS = `
  ...
  a { color: #FF4FC3; text-decoration: none; }
  a:hover { color: #3DFFB8; }
  ...
  .acp-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0B0E1C; }
  ...
  @media (min-width: 768px) {
    .acp-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(255,79,195,.3); }
  ...
  .acp-date-month { font-family: var(--acp-poppins), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #FF4FC3; padding-left: 12%; }

  .acp-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .acp-divider-line { width: 52px; height: 1px; background: #FF4FC3; display: inline-block; }
  ...
  .acp-cal-link:hover { color: #FF4FC3; }
  ...
  .acp-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #FF4FC3, #2E6BFF, #FF4FC3, #3DFFB8, #FF4FC3); filter: saturate(.75); }
  .acp-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0A0D1A; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .acp-medallion-num { font-family: var(--acp-poppins), sans-serif; font-size: 26px; line-height: 1; color: #3DFFB8; }
  .acp-medallion-num-sm { font-family: var(--acp-poppins), sans-serif; font-size: 16px; line-height: 1; color: #3DFFB8; }
  .acp-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .acp-medallion-arc { position: absolute; inset: -14%; }
  .acp-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #FF4FC3; font-family: var(--acp-mono), monospace; }
  ...
  .acp-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #3DFFB8, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .acp-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #FF4FC3; }
  .acp-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0B0E1C 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .acp-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(255,79,195,0.32), rgba(46,107,255,0.26), rgba(61,255,184,0.34), rgba(255,79,195,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .acp-accent-serif { font-style: italic; color: #2E6BFF; font-family: var(--acp-poppins), sans-serif; }
  .acp-accent-serif-2 { color: #2E6BFF; }
  .acp-h2 { margin: 0; font-family: var(--acp-poppins), sans-serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .acp-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .acp-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #2E6BFF; margin-top: auto; }
  ...
  .acp-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #2E6BFF; }
  ...
  .acp-crono-time { font-family: var(--acp-mono), monospace; color: #2E6BFF; min-width: 42px; }
  ...
  .acp-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #3DFFB8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .acp-rsvp-payment-total { color: #3DFFB8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .acp-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #FF4FC3; background: transparent; color: #FF4FC3; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .acp-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(255,79,195,0.3); color: #F4F1EA; font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .acp-rsvp-input::placeholder { color: #6E6A5D; }
  .acp-rsvp-input:focus { outline: none; border-bottom-color: #FF4FC3; }
  .acp-rsvp-btn { width: 100%; padding: 16px; font-family: var(--acp-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); border: 1px solid #FF4FC3; color: #F6EFDD; cursor: pointer; }
  .acp-rsvp-btn:hover:not(:disabled) { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0A0C18; }
  .acp-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .acp-rsvp-btn--ghost { background: transparent; color: #FF4FC3; }
  .acp-rsvp-btn--ghost:hover:not(:disabled) { background: rgba(255,79,195,0.1); color: #FF4FC3; }
  .acp-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #FF4FC3; margin: 0; }
  ...
  .acp-bank-copy { flex-shrink: 0; font-family: var(--acp-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #FF4FC3; background: transparent; color: #FF4FC3; cursor: pointer; }
  .acp-bank-copy:hover { background: rgba(255,79,195,0.12); }
  ...
  .acp-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(255,79,195,0.3); color: #F4F1EA; font-family: var(--acp-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .acp-song-input::placeholder { color: #6E6A5D; }
  .acp-song-input:focus { outline: none; border-bottom-color: #FF4FC3; }
  .acp-song-sep { color: #8A8577; flex-shrink: 0; }
  .acp-song-submit { flex-shrink: 0; background: none; border: none; color: #FF4FC3; font-family: var(--acp-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .acp-song-submit:hover { color: #3DFFB8; }
  .acp-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .acp-song-error { font-size: 10px; color: #FF4FC3; margin-top: 6px; }
  ...
  .acp-quiz-opt { font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(255,79,195,0.4); background: transparent; color: #FF4FC3; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .acp-quiz-opt:disabled { cursor: default; }
  .acp-quiz-opt--picked { background: #FF4FC3; border-color: #FF4FC3; color: #0A0C18; }
  .acp-quiz-opt--correct { background: #3DFFB8; border-color: #3DFFB8; color: #0A0C18; }
  .acp-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .acp-quiz-result { padding-top: 18px; border-top: 1px solid #2A2A33; }
  .acp-quiz-result-score { margin: 0 0 6px; font-family: var(--acp-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #3DFFB8; }
  .acp-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .acp-final-card { border: 1px solid #FF4FC3; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  ...
  .acp-replay { cursor: pointer; color: #FF4FC3; }
  ...
  .acp-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#3DFFB8, #FF4FC3); transition: height 260ms linear; display: block; }
  .acp-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #FF4FC3; transition: color 500ms ease; }
  ...
  .acp-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #141026 0%, #0F1020 46%, #0B0E1C 100%); }
  .acp-cover-half--bottom .acp-cover-inner { top: auto; bottom: 0; }
  .acp-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(255,79,195,0.5), rgba(46,107,255,0.4), rgba(61,255,184,0.5), rgba(255,79,195,0.5)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  /* Manchas de luz suaves (ver mockup original) -- propias de Acrylic Pop:
     ...
     plantillas de la colección. */
  .acp-cover-spots { position: absolute; inset: 0; background-image: radial-gradient(circle at 20% 20%, rgba(255,79,195,.25), transparent 40%), radial-gradient(circle at 80% 70%, rgba(46,107,255,.22), transparent 40%), radial-gradient(circle at 50% 90%, rgba(61,255,184,.18), transparent 35%); }
  ...
  .acp-cover-names { margin: 0; font-family: var(--acp-poppins), sans-serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FFFFFF; }
  .acp-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#FF4FC3, transparent); display: block; }
  ...
  .acp-cover-cta { border: 1px solid #FF4FC3; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--acp-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .acp-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .acp-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0A0C18; }
  }
  ...
  .acp-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #FF4FC3; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
