...
/**
 * InauguracionTemplateGrafito.tsx
 *
...
interface InauguracionTemplateGrafitoProps {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        ...
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #B9B9BC";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#1C1C1E" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#1C1C1E" : "#B9B9BC";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#080808", tinta: "#F4F1EA", acento: "#B9B9BC", clase: `${ingDisplay.variable} ${ingMono.variable}`, fuente: "var(--ing-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="ing-panel ing-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="ing-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#1C1C1E" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#1C1C1E" />
                  </svg>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#B9B9BC" guest={guest} />
      )}
...
const GP_CSS = `
  ...
  .ing-date-month { font-family: var(--ing-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #B9B9BC; padding-left: 12%; }

  .ing-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .ing-divider-line { width: 52px; height: 1px; background: #B9B9BC; display: inline-block; }
  ...
  .ing-cal-link:hover { color: #B9B9BC; }
  ...
  .ing-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #B9B9BC, #F0F0F0, #6E5AA8); filter: saturate(.75); }
  ...
  .ing-medallion-label { font-family: var(--ing-display), serif; font-size: 26px; line-height: 1; color: #F0F0F0; }
  .ing-medallion-label-sm { font-family: var(--ing-display), serif; font-size: 16px; color: #F0F0F0; }
  .ing-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .ing-medallion-sub--accent { color: #B9B9BC; }
  ...
  .ing-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #B9B9BC; opacity: 0.4; font-family: var(--ing-mono), monospace; }
  ...
  .ing-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F0F0F0, transparent); animation: bbsScan 6s linear infinite; pointer-events: none; }
  ...
  .ing-cd-num { font-family: var(--ing-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F0F0F0; display: inline-block; }
  .ing-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #B9B9BC; }
  ...
  .ing-accent-italic { font-style: italic; color: #B9B9BC; }
  .ing-accent-serif { font-style: italic; color: #1C1C1E; font-family: var(--ing-display), serif; }
  .ing-accent-serif-2 { color: #1C1C1E; }
  ...
  .ing-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #1C1C1E; margin-top: auto; }
  ...
  .ing-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #1C1C1E; }
  ...
  .ing-crono-time { font-family: var(--ing-mono), monospace; color: #1C1C1E; min-width: 42px; }
  ...
  .ing-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F0F0F0, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .ing-rsvp-payment-total { color: #F0F0F0; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .ing-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #B9B9BC; background: transparent; color: #B9B9BC; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .ing-rsvp-input:focus { outline: none; border-bottom-color: #B9B9BC; }
  .ing-rsvp-btn { width: 100%; padding: 16px; font-family: var(--ing-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #B9B9BC, #1C1C1E); border: 1px solid #B9B9BC; color: #1C1C1E; cursor: pointer; }
  .ing-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .ing-rsvp-btn--ghost { background: transparent; color: #B9B9BC; }
  .ing-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #B9B9BC; margin: 0; }
  ...
  .ing-bank-copy { flex-shrink: 0; font-family: var(--ing-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #B9B9BC; background: transparent; color: #B9B9BC; cursor: pointer; }
  ...
  .ing-song-input:focus { outline: none; border-bottom-color: #B9B9BC; }
  .ing-song-sep { color: #8A8577; flex-shrink: 0; }
  .ing-song-submit { flex-shrink: 0; background: none; border: none; color: #B9B9BC; font-family: var(--ing-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .ing-song-submit:hover { color: #F0F0F0; }
  .ing-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .ing-song-error { font-size: 10px; color: #B9B9BC; margin-top: 6px; }
  ...
  .ing-quiz-opt { font-family: var(--ing-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #B9B9BC; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .ing-quiz-opt:disabled { cursor: default; }
  .ing-quiz-opt--picked { background: #B9B9BC; border-color: #B9B9BC; color: #1C1C1E; }
  .ing-quiz-opt--correct { background: #B9B9BC; border-color: #B9B9BC; color: #1C1C1E; }
  ...
  .ing-quiz-result-score { margin: 0 0 6px; font-family: var(--ing-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F0F0F0; }
  .ing-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .ing-final-card { border: 1px solid #B9B9BC; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .ing-final-names { font-family: var(--ing-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F0F0F0; }
  ...
  .ing-replay { cursor: pointer; color: #B9B9BC; }
  ...
  .ing-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F0F0F0, #B9B9BC); transition: height 260ms linear; display: block; }
  .ing-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #B9B9BC; transition: color 500ms ease; }
  ...
  .ing-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #141414 0%, #1C1C1E 46%, #080808 100%); }
  .ing-cover-half--bottom .ing-cover-inner { top: auto; bottom: 0; }
  .ing-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(185,185,188,0.35), rgba(28,28,30,0.5), rgba(185,185,188,0.5), rgba(185,185,188,0.35)); filter: blur(64px); opacity: .62; animation: bbsFoil 26s linear infinite; }
  .ing-cover-texture { position: absolute; inset: 0; background-image: linear-gradient(90deg, rgba(240,240,240,.08) 1px, transparent 1px); background-size: 44px 100%; }
  ...
  .ing-cover-names { margin: 0; font-family: var(--ing-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F0F0F0; }
  .ing-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#B9B9BC, transparent); display: block; }
  ...
  .ing-cover-cta { border: 1px solid #B9B9BC; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--ing-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .ing-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .ing-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #1C1C1E; }
  }
  ...
  .ing-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #B9B9BC; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
