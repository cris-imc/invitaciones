  ...
  if (finished) {
    ...
    return (
      <div className="quiz-box text-center flex flex-col items-center">
        ...
        <p style={{ marginTop: "12px", opacity: 0.8, fontFamily: "var(--font-sans)", textTransform: "uppercase", letterSpacing: "0.1em", fontSize: "0.8rem", color: "#2A3B36" }}>{tx("invitacion.quiz.respondisteCorrectamentePorcentaje", { aciertos: score, total: preguntas.length, pct: percent }).toUpperCase()}</p>
        
        {isSaving ? (
          <p style={{ marginTop: "16px", fontSize: "14px", opacity: 0.7, color: "#7A8F82" }}>{tx("invitacion.quiz.guardandoResultados")}</p>
        ) : (
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(62,46,32,0.05)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(62,46,32,0.12)", textAlign: "left", maxWidth: "90%" }}>
                ...
                <p style={{ fontSize: "11.5px", margin: 0, opacity: 0.85, lineHeight: 1.4, color: "#2A3B36" }}>
                  {tx("invitacion.quiz.promedioGlobal", { n: stats.count })} <strong style={{ color: "#2A3B36" }}>{stats.avg}%</strong>.
                </p>
              </div>
            </div>
          )
...
export function RivieraTemplateCal({ invitation, guest, isPersonalized = false }: RivieraTemplateCalProps) {
  ...
  if (eventStatus === "POST_EVENT") {
    return (
      <div className="min-h-dvh w-full bg-gradient-to-b from-[#F0F4F0] via-[#E6ECE6] to-[#F0F4F0] text-white relative overflow-x-hidden flex flex-col justify-between" data-theme={theme}>
          ...
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#D9899C] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#D9899C] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#D9899C]/20 pl-3">
                ...
                <span className="text-[#7A8F82] text-[8px] uppercase tracking-wider leading-none mt-1">{guest.expectedCount === 1 ? tx("invitacion.pase.lugar") : tx("invitacion.pase.lugares")}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#D9899C]" />
              <span className="text-[#E9F0EA]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
