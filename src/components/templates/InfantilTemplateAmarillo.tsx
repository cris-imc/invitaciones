 ...
 * (#FFF7F2) + tinta violeta oscuro (#2A2140) + acento coral (#FFB627,
 * variable por color) + acento lavanda (#9B7FE8) + acento menta (#3FBF9F),
...
interface InfantilTemplateAmarilloProps {
...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#FFB627]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#FFB627] uppercase tracking-wider mb-0.5">{label}</span>
          ...
          stats && stats.count > 0 && (
            <div style={{ marginTop: "28px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(255,92,138,0.08)", padding: "8px 16px", borderRadius: "99px", border: "1px solid rgba(255,92,138,0.18)", textAlign: "left", maxWidth: "90%" }}>
                <Users className="w-5 h-5 text-[#FFB627] shrink-0" />
...
export function InfantilTemplateAmarillo({ invitation, guest, isPersonalized = false }: InfantilTemplateAmarilloProps) {
  const tx = useTextos();
    ...
    >
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <div className={portadaFondoAnimado ? "acp-desktop-only" : undefined}>
            <div style={{ position: 'absolute', width: 200, height: 200, borderRadius: '50%', background: '#FFB627', opacity: 0.16, top: '6%', left: '-8%', filter: 'blur(14px)', animation: 'infantil-blobFloat 7s ease-in-out infinite', pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', width: 170, height: 170, borderRadius: '50%', background: '#9B7FE8', opacity: 0.18, bottom: '8%', right: '-6%', filter: 'blur(14px)', animation: 'infantil-blobFloat2 8s ease-in-out infinite', pointerEvents: 'none' }} />
          ...
          <div style={{ textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem', position: 'relative' }}>
            ...
            <button
              ...
              style={{
                ...
                background: '#FFB627', boxShadow: '0 10px 24px rgba(255,92,138,0.35)',
                marginTop: '1rem',
              }}
              onMouseEnter={(e) => { e.currentTarget.style.background = '#E8447A'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = '#FFB627'; }}
            >
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#FFB627]/40 shadow-md rounded-full ${isTicketMaximized ? 'bg-[#FFF7F2]/95 backdrop-blur-md w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#241B38]/95 backdrop-blur-md px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-bold uppercase tracking-[0.15em] text-[#FFB627] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-bold uppercase tracking-[0.15em] text-[#FFB627] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#FFB627]/20 pl-3">
          ...
          <h1 className="text-5xl font-extrabold text-white leading-tight mb-2 drop-shadow-md" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), sans-serif' }}>
            {em ? (
              <>
                ...
                <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), sans-serif', fontStyle: 'normal', color: '#FFB627' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
              </>
            ) : (
              <span className="block">{title}</span>
            )}
          </h1>
          <div style={{ width: 40, height: 4, background: '#FFB627', borderRadius: 4, margin: '6px 0 14px' }} />
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#FFF7F2]">
          <div className="px-8 pt-16 pb-12 text-left bg-[#FFF7F2] z-10 relative">
            ...
            <p className=" text-xs font-bold uppercase tracking-[0.15em] text-[#FFB627] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            ...
            <div style={{ width: 40, height: 4, background: '#FFB627', borderRadius: 4, margin: '0 0 20px' }} />
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-bold text-[#FFB627] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
          ...
          <div ref={heroPhotoRef} className="flex-1 w-full relative overflow-hidden">
            ...
            <div className="absolute inset-3 pointer-events-none z-20" aria-hidden="true">
              ...
              <IconStar style={{ position: "absolute", top: -12, left: -8, width: 24, height: 24, color: "#FFB627" }} />
              <IconBalloon style={{ position: "absolute", bottom: -14, right: -6, width: 20, height: 26, color: "#9B7FE8" }} />
            </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full py-24 px-6 md:px-12 flex items-center justify-center relative" style={{ background: "#9B7FE81A" }}>
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#FFF7F2] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            <p className="t-kicker mb-8 flex items-center gap-2 text-[11px] font-bold tracking-[0.15em] uppercase text-[#FFB627]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              ...
              <div className="bg-white border-l-[4px] border-l-[#3FBF9F] p-6 sm:p-8 mb-6 shadow-sm">
                <div>
                  ...
                  {Boolean(invitation.ceremoniaMapUrl) && (
                    <a href={String(invitation.ceremoniaMapUrl)} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-bold tracking-wider text-[#FFB627] hover:text-[#2A2140] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-white border-l-[4px] border-l-[#3FBF9F] p-6 sm:p-8 mb-10 shadow-sm">
                ...
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-1  text-xs font-bold tracking-wider text-[#FFB627] hover:text-[#2A2140] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
            ...
            {cronograma.length > 0 && (
              <div className="mt-16" id="schedule">
                ...
                <p className="t-kicker mb-6 flex items-center gap-2 text-[11px] font-bold tracking-[0.15em] uppercase text-[#FFB627]" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  CRONOGRAMA
                </p>
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center py-4 border-b-2 border-dashed border-[#9B7FE8]/25 last:border-b-0">
                      {item.time && (
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#FFF0E4] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                ...
                <p className="t-kicker mb-10 flex items-center gap-2 text-[#FFB627]">
        ...
        <div className="w-full flex items-center justify-center gap-2 py-6" style={{ background: '#FFF7F2' }} aria-hidden="true">
          <IconConfetti className="infantil-scroll-doodle opacity-0" style={{ width: 24, height: 12, color: '#9B7FE8' }} />
        </div>
      ...
      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#FFB627" surfaceColor="#FFFFFF" />}
      </div>
  );
}
