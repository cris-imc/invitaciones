...
/**
 * DespedidaSolteroTemplateGrafito.tsx
 *
...
interface DespedidaSolteroTemplateGrafitoProps {
...
export function DespedidaSolteroTemplateGrafito({ invitation, guest, isPersonalized = false }: DespedidaSolteroTemplateGrafitoProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#A8A8AE";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #A8A8AE";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#4A4A50" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#4A4A50" : "#A8A8AE";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#08090B", tinta: "#F4F1EA", acento: "#A8A8AE", clase: `${dpoDisplay.variable} ${dpoMono.variable}`, fuente: "var(--dpo-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="dpo-panel dpo-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="dpo-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#4A4A50" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#4A4A50" />
                  </svg>
          ...
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="dpo-section" style={{ background: "#0B0D10" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="dpo-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="dpo-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#EDEDEF" : "#A8A8AE" }} />
              ))}
            </div>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#A8A8AE" guest={guest} />
      )}
...
const GP_CSS = `
  ...
  a { color: #A8A8AE; text-decoration: none; }
  a:hover { color: #EDEDEF; }
  ...
  .dpo-date-month { font-family: var(--dpo-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #A8A8AE; padding-left: 12%; }

  .dpo-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .dpo-divider-line { width: 52px; height: 1px; background: #A8A8AE; display: inline-block; }
  ...
  .dpo-cal-link:hover { color: #A8A8AE; }
  ...
  .dpo-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #A8A8AE, #EDEDEF, #6E5AA8); filter: saturate(.75); }
  ...
  .dpo-medallion-label { font-family: var(--dpo-display), serif; font-size: 26px; line-height: 1; color: #EDEDEF; }
  .dpo-medallion-label-sm { font-family: var(--dpo-display), serif; font-size: 16px; color: #EDEDEF; }
  .dpo-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .dpo-medallion-sub--accent { color: #A8A8AE; }
  ...
  .dpo-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #A8A8AE; opacity: 0.4; font-family: var(--dpo-mono), monospace; }
  ...
  .dpo-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #EDEDEF, transparent); animation: dpoScan 6s linear infinite; pointer-events: none; }
  ...
  .dpo-cd-num { font-family: var(--dpo-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #EDEDEF; display: inline-block; }
  .dpo-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #A8A8AE; }
  ...
  .dpo-accent-italic { font-style: italic; color: #A8A8AE; }
  .dpo-accent-serif { font-style: italic; color: #4A4A50; font-family: var(--dpo-display), serif; }
  .dpo-accent-serif-2 { color: #4A4A50; }
  ...
  .dpo-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #4A4A50; margin-top: auto; }
  ...
  .dpo-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #4A4A50; }
  ...
  .dpo-crono-time { font-family: var(--dpo-mono), monospace; color: #4A4A50; min-width: 42px; }
  ...
  .dpo-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #EDEDEF, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .dpo-rsvp-payment-total { color: #EDEDEF; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .dpo-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #A8A8AE; background: transparent; color: #A8A8AE; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .dpo-rsvp-input:focus { outline: none; border-bottom-color: #A8A8AE; }
  .dpo-rsvp-btn { width: 100%; padding: 16px; font-family: var(--dpo-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #A8A8AE, #4A4A50); border: 1px solid #A8A8AE; color: #0B0D10; cursor: pointer; }
  .dpo-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .dpo-rsvp-btn--ghost { background: transparent; color: #A8A8AE; }
  .dpo-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #A8A8AE; margin: 0; }
  ...
  .dpo-bank-copy { flex-shrink: 0; font-family: var(--dpo-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #A8A8AE; background: transparent; color: #A8A8AE; cursor: pointer; }
  ...
  .dpo-song-input:focus { outline: none; border-bottom-color: #A8A8AE; }
  .dpo-song-sep { color: #8A8577; flex-shrink: 0; }
  .dpo-song-submit { flex-shrink: 0; background: none; border: none; color: #A8A8AE; font-family: var(--dpo-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .dpo-song-submit:hover { color: #EDEDEF; }
  .dpo-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .dpo-song-error { font-size: 10px; color: #A8A8AE; margin-top: 6px; }
  ...
  .dpo-quiz-opt { font-family: var(--dpo-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #A8A8AE; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .dpo-quiz-opt:disabled { cursor: default; }
  .dpo-quiz-opt--picked { background: #A8A8AE; border-color: #A8A8AE; color: #0B0D10; }
  .dpo-quiz-opt--correct { background: #A8A8AE; border-color: #A8A8AE; color: #0B0D10; }
  ...
  .dpo-quiz-result-score { margin: 0 0 6px; font-family: var(--dpo-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #EDEDEF; }
  .dpo-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .dpo-final-card { border: 1px solid #A8A8AE; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .dpo-final-names { font-family: var(--dpo-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #EDEDEF; }
  ...
  .dpo-replay { cursor: pointer; color: #A8A8AE; }
  ...
  .dpo-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#EDEDEF, #A8A8AE); transition: height 260ms linear; display: block; }
  .dpo-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #A8A8AE; transition: color 500ms ease; }
  ...
  .dpo-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(36,36,36,0.5), rgba(168,168,174,0.32), rgba(168,168,174,0.55), rgba(36,36,36,0.5)); filter: blur(64px); opacity: .62; animation: dpoFoil 26s linear infinite; }
  .dpo-cover-texture { position: absolute; inset: 0; background-image: repeating-linear-gradient(0deg, rgba(237,237,239,.05) 0 1px, transparent 1px 22px), repeating-linear-gradient(90deg, rgba(237,237,239,.05) 0 1px, transparent 1px 22px); }
  ...
  .dpo-cover-names { margin: 0; font-family: var(--dpo-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #EDEDEF; }
  .dpo-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#A8A8AE, transparent); display: block; }
  ...
  .dpo-cover-cta { border: 1px solid #A8A8AE; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--dpo-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .dpo-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #A8A8AE; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
