...
export function Crystal3dTemplateCuarzoRosa({ invitation, guest, isPersonalized = false }: Crystal3dTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#FBEAE2";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#E8A9C4";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(232,169,196,.35), 0 18px 50px -30px #E8A9C4";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#D9B36A" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(232,169,196,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(232,169,196,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#D9B36A" : "#E8A9C4";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#120A10", tinta: "#F4F1EA", acento: "#D9B36A", clase: `${c3dPoppins.variable} ${c3dMono.variable}`, fuente: "var(--c3d-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="c3d-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="c3d-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #241621 0%, #170F13 55%, #120A10 100%)" }}>
            ...
            <div className="c3d-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="18,10,16" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="18,10,16" />
                </div>
              )}
            </div>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="c3d-section c3d-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A1520 0%, #170D12 55%, #120A10 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="c3d-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #2B1420 0%, #180E12 52%, #120A10 100%)" }}>
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.cuandoYDonde")} className="c3d-pan" style={!scrollVertical && ceremoniaHabilitada ? { height: "340vh" } : undefined}>
          <div className="c3d-pan-sticky">
            <div data-strip="1" className="c3d-strip">
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="c3d-panel c3d-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="c3d-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#D9B36A" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#D9B36A" />
                  </svg>
              ...
              <div data-tone="dark" className="c3d-panel c3d-panel--center" style={{ background: "#170F13", color: "#F4F1EA" }}>
                <div className="c3d-medallion c3d-medallion--lg">
                  <C3dMedallion
                    ringGradient="conic-gradient(from 90deg, #E8A9C4, #FBEAE2, #D9B36A, #E8A9C4, #E8A9C4)"
                    topLabel="ACCESO VIP"
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="c3d-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #241621 0%, #170F13 60%, #120A10 100%)" }}>
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="c3d-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="c3d-pan-sticky">
            <div data-strip="1" className="c3d-strip">
              {photoPages.map((page, pageIndex) => (
                <div key={pageIndex} data-tone="light" className="c3d-panel c3d-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="c3d-seguir c3d-seguir--split">
                    ...
                    <span className="c3d-accent-lavender">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="c3d-section" style={{ background: "#170F13" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="c3d-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="c3d-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#FBEAE2" : "#E8A9C4" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="c3d-section" style={{ background: "#170F13" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="c3d-section" style={{ background: "#170F13" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="c3d-section c3d-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #241621 0%, #170F13 55%, #120A10 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="c3d-final-card">
            <div className="c3d-medallion c3d-medallion--final">
              <C3dMedallion
                ringGradient="conic-gradient(from 140deg, #D9B36A, #E8A9C4, #E8A9C4, #FBEAE2, #D9B36A)"
      ...
      <div ref={coverRef} className="c3d-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#E8A9C4" guest={guest} />
      )}
    ...
    <>
      ...
      <div ref={stubRef} className="c3d-stub">
        ...
        <div ref={sealRef} className="c3d-seal">
          <C3dMedallion ringGradient="conic-gradient(from 0deg, #D9B36A, #E8A9C4, #FBEAE2, #D9B36A)" sub="" arcId="" arcText="" compact />
        </div>
...
function C3dCoverHalf({
  ...
  return (
    <div className="c3d-cover-inner">
      {photoMobile && (
        <div className="acp-mobile-only">
          <AnimatedCoverPhoto photoSrc={photoMobile} tint tintColor1="#E8A9C4" tintColor2="#080B10" effect="enfoque" scrimColorRgb="18,10,16" />
        </div>
      )}
      {photoDesktop && (
        <div className="acp-desktop-only">
          <AnimatedCoverPhoto photoSrc={photoDesktop} tint tintColor1="#E8A9C4" tintColor2="#080B10" effect="enfoque" scrimColorRgb="18,10,16" />
        </div>
      )}
...
const C3D_CSS = `
  ...
  a { color: #D9B36A; text-decoration: none; }
  a:hover { color: #FBEAE2; }
  ...
  @media (min-width: 768px) { .c3d-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(232,169,196,.3); } }
  ...
  .c3d-date-month { font-family: var(--c3d-poppins), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #E8A9C4; padding-left: 12%; }

  .c3d-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .c3d-divider-line { width: 52px; height: 1px; background: #E8A9C4; display: inline-block; }
  ...
  .c3d-cal-link:hover { color: #E8A9C4; }
  ...
  .c3d-medallion-num { font-family: var(--c3d-poppins), sans-serif; font-size: 30px; line-height: 1; color: #FBEAE2; }
  .c3d-medallion-num-sm { font-family: var(--c3d-poppins), sans-serif; font-size: 16px; line-height: 1; color: #FBEAE2; }
  .c3d-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .c3d-medallion-sub--cyan { color: #E8A9C4; }
  .c3d-medallion-sub--lavender { color: #D9B36A; }
  .c3d-medallion-arc { position: absolute; inset: -14%; }
  .c3d-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #E8A9C4; font-family: var(--c3d-mono), monospace; }

  .c3d-facet-gem { position: absolute; inset: 0; clip-path: polygon(50% 0%, 95% 30%, 80% 90%, 20% 90%, 5% 30%); background: linear-gradient(135deg, #E8A9C4, #D9B36A 55%, #FBEAE2); filter: saturate(.9); animation: facetSpin 6s ease-in-out infinite; }
  ...
  .c3d-facet-num { font-family: var(--c3d-poppins), sans-serif; font-size: 26px; line-height: 1; color: #FBEAE2; }
  ...
  .c3d-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #FBEAE2, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }
  ...
  .c3d-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #E8A9C4; }
  .c3d-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #120A10 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .c3d-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(217,179,106,0.32), rgba(232,169,196,0.26), rgba(232,169,196,0.36), rgba(217,179,106,0.32)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .c3d-accent-italic-cyan { font-style: italic; color: #E8A9C4; }
  .c3d-accent-italic-lavender { font-style: italic; color: #D9B36A; }
  .c3d-accent-cyan { color: #E8A9C4; }
  .c3d-accent-lavender { color: #D9B36A; }
  .c3d-h2 { margin: 0; font-family: var(--c3d-poppins), sans-serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .c3d-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  ...
  .c3d-pan[data-scroll="vertical"] { height: auto; }
  .c3d-pan[data-scroll="vertical"] .c3d-pan-sticky { position: static; height: auto; overflow: visible; }
  .c3d-pan[data-scroll="vertical"] .c3d-strip { position: static; display: block; width: 100%; transform: none !important; }
  .c3d-pan[data-scroll="vertical"] .c3d-panel { height: calc(var(--vh, 1vh) * 100); }
  ...
  .c3d-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #D9B36A; margin-top: auto; }
  ...
  .c3d-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #D9B36A; }
  ...
  .c3d-crono-time { font-family: var(--c3d-mono), monospace; color: #9C7A3E; min-width: 42px; }
  ...
  .c3d-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #FBEAE2, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .c3d-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(232,169,196,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .c3d-rsvp-payment-total { color: #FBEAE2; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .c3d-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #E8A9C4; background: transparent; color: #E8A9C4; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .c3d-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(232,169,196,0.3); color: #F4F1EA; font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .c3d-rsvp-input::placeholder { color: #6E6A5D; }
  .c3d-rsvp-input:focus { outline: none; border-bottom-color: #E8A9C4; }
  .c3d-rsvp-btn { width: 100%; padding: 16px; font-family: var(--c3d-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #E8A9C4, #A85D78); border: 1px solid #E8A9C4; color: #0B0B10; cursor: pointer; }
  .c3d-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .c3d-rsvp-btn--ghost { background: transparent; color: #E8A9C4; }
  .c3d-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #E8A9C4; margin: 0; }
  ...
  .c3d-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(232,169,196,0.18); }
  ...
  .c3d-bank-copy { flex-shrink: 0; font-family: var(--c3d-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #E8A9C4; background: transparent; color: #E8A9C4; cursor: pointer; }
  .c3d-bank-copy:hover { background: rgba(232,169,196,0.12); }
  ...
  .c3d-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(232,169,196,0.3); padding-bottom: 12px; }
  ...
  .c3d-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(232,169,196,0.3); color: #F4F1EA; font-family: var(--c3d-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .c3d-song-input::placeholder { color: #6E6A5D; }
  .c3d-song-input:focus { outline: none; border-bottom-color: #E8A9C4; }
  .c3d-song-sep { color: #8A8577; flex-shrink: 0; }
  .c3d-song-submit { flex-shrink: 0; background: none; border: none; color: #E8A9C4; font-family: var(--c3d-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .c3d-song-submit:hover { color: #FBEAE2; }
  .c3d-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .c3d-song-error { font-size: 10px; color: #E8A9C4; margin-top: 6px; }
  ...
  .c3d-quiz-opt { font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(232,169,196,0.4); background: transparent; color: #E8A9C4; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .c3d-quiz-opt:disabled { cursor: default; }
  .c3d-quiz-opt--picked { background: #E8A9C4; border-color: #E8A9C4; color: #0B0B10; }
  .c3d-quiz-opt--correct { background: #E8A9C4; border-color: #E8A9C4; color: #0B0B10; }
  .c3d-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .c3d-quiz-result { padding-top: 18px; border-top: 1px solid rgba(232,169,196,0.2); }
  .c3d-quiz-result-score { margin: 0 0 6px; font-family: var(--c3d-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #FBEAE2; }
  .c3d-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .c3d-final-card { border: 1px solid #E8A9C4; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .c3d-final-names { font-family: var(--c3d-poppins), sans-serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #FBEAE2; }
  ...
  .c3d-replay { cursor: pointer; color: #E8A9C4; }
  .c3d-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .c3d-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(232,169,196,0.14); }
  .c3d-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .c3d-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(232,169,196,0.16); position: relative; }
  .c3d-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#FBEAE2, #E8A9C4); transition: height 260ms linear; display: block; }
  .c3d-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #E8A9C4; transition: color 500ms ease; }
  ...
  .c3d-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #241623 0%, #170F13 46%, #120A10 100%); }
  ...
  .c3d-cover-facets { position: absolute; inset: 0; background-image: repeating-linear-gradient(64deg, rgba(251,234,226,.08) 0 1px, transparent 1px 34px), repeating-linear-gradient(-64deg, rgba(217,179,106,.08) 0 1px, transparent 1px 34px); }
  ...
  .c3d-cover-names { margin: 0; font-family: var(--c3d-poppins), sans-serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #FBEAE2; }
  .c3d-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#E8A9C4, transparent); display: block; }
  ...
  .c3d-cover-cta { border: 1px solid #E8A9C4; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--c3d-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .c3d-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #E8A9C4; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
