...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#52718A]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#384D5E] uppercase tracking-wider mb-0.5">{label}</span>
...
export function ElegantTemplateBlue({ invitation, guest, isPersonalized = false }: ConviteTemplateProps) {
  const tx = useTextos();
  ...
  return (
    <>
      <style>{`
        ...
        /* RSVP Custom Aesthetics for ElegantTemplateBlue */
        ...
        #rsvp.section.dark {
          background-color: #384D5E !important; /* Dark brown/black */
        ...
        #songs.d-sec.dark .t-kicker {
          font-family: var(--font-body-custom, var(--font-inter)), sans-serif !important;
          color: #52718A !important; /* Gold/Orange */
          font-size: 11px !important;
          font-weight: 600 !important;
          text-transform: uppercase !important;
          letter-spacing: 0.2em !important;
        ...
        #songs.d-sec.dark input {
          ...
          background-color: #E3E8EC !important; /* Pastel */
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#52718A]/40 shadow-md ${isTicketMaximized ? 'bg-[#F9F7F1]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#1A2B33]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#52718A] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#52718A] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
              <div className="flex flex-col items-end border-l border-[#52718A]/20 pl-3">
            ...
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <Ticket className="w-4 h-4 text-[#52718A]" />
              <span className="text-[#E3E8EC]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <div className="desktop-stage" data-theme={theme} style={{ '--t-acc': '#52718A', '--t-acc2': '#52718A', '--c-accent': '#52718A' , '--t-bg': '#F9F7F1', ...getTypographyCssVars(invitation.fontTitle as string, invitation.fontBody as string) } as React.CSSProperties}>
      ...
      <div className="d-right tpl">
        <div className="hide-desktop w-full flex flex-col min-h-[100dvh] bg-[#F9F7F1]">
          ...
          <div className="px-8 pt-16 pb-12 text-left bg-[#F9F7F1] z-10 relative">
            <p className=" text-xs font-semibold uppercase tracking-[0.2em] text-[#52718A] mb-6" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              {eyebrow}
            </p>
            <h1 className="text-[4rem] font-light text-[#2C2C2C] leading-[1.0] mb-8" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#52718A' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
            ...
            {Boolean(activeDressCode) && (
              <p className=" text-xs font-semibold text-[#52718A] tracking-widest uppercase mt-4" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                {tx("invitacion.ubicacion.dressCode")} {activeDressCode}
              </p>
            )}
          </div>
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full bg-[#E3E8EC] py-24 px-6 md:px-12 flex items-center justify-center">
        ...
        <SectionWrapper id="details" delay={150} className="w-full bg-[#F9F7F1] py-20 px-6 md:px-12">
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
            ...
            {Boolean(invitation.ceremoniaHabilitada) && (
              <div className="bg-[#FAF8F5] border border-[#52718A]/20 rounded-xl p-5 sm:p-7 mb-6 shadow-sm transition-all hover:shadow-md">
                <div>
                  <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#384D5E] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                  ...
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="flex items-center gap-2 text-[#4A5568] font-medium text-sm sm:text-base mb-3">
                      <Clock className="w-4 h-4 text-[#384D5E]" /> {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-[#FAF8F5] border border-[#52718A]/20 rounded-xl p-5 sm:p-7 mb-10 shadow-sm transition-all hover:shadow-md">
                <span className=" text-[10px] font-semibold uppercase tracking-[0.2em] text-[#384D5E] block mb-3" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                ...
                {hora && (
                  <p className="flex items-center gap-2 text-[#4A5568] font-medium text-sm sm:text-base mb-3">
                    <Clock className="w-4 h-4 text-[#384D5E]" /> {hora} hs
                  </p>
                )}
                {direccion && (
                  <p className="flex items-center gap-2 text-[#4A5568] font-medium text-sm sm:text-base mb-4">
                    <MapPin className="w-4 h-4 text-[#384D5E]" /> {direccion}
                  </p>
                )}
                {mapUrl && (
                  <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-2  text-xs font-semibold tracking-wider text-[#1A2B33] uppercase underline underline-offset-4 decoration-[#52718A] hover:text-[#52718A] transition-colors" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
              ...
              <div className="mt-16" id="schedule">
                ...
                <div className="flex flex-col w-full">
                  {cronograma.map((item, i) => (
                    <div key={i} className="flex items-center py-5 border-b border-[#52718A]/30 last:border-b-0">
                      {item.time && (
                        <span className=" text-lg sm:text-xl text-[#52718A]/90 font-light tracking-widest w-28 sm:w-32 flex-shrink-0" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>
                          {item.time}
                        </span>
                      )}
        ...
        {(invitation.galeriaPrincipalHabilitada ?? false) && allPhotos.length > 0 && (
          <SectionWrapper id="album" delay={200} className="w-full bg-[#E3E8EC] py-20 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full bg-[#E3E8EC] py-20 px-6 md:px-12">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
