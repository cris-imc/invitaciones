...
/**
 * CineAbstractoXvTemplate.tsx
 *
 * Variante de color "Western" de "Cine Abstracto XV" (Colección
 * Storytelling, mis quince años: mismo layout/copy/tipografía/estructura que
 * CineAbstractoXvTemplate.tsx -- solo cambian el tinte de fondo oscuro y los
 * acentos protagonistas (antes rojo cine + dorado marquesina).
 * Archivo Black + IBM Plex Mono, medallón tipo "entrada de función" con el
 * número 15 en vez de ícono, foquitos de marquesina parpadeando en la tapa,
 ...
 * PrincesaTemplate.tsx/GuestPassVipTemplate.tsx (ver esos archivos para el
 * detalle del motor de motion) -- todo el motion es CSS + SVG + un loop de
 * scroll propio, cero PNGs.
 *
...
interface CineAbstractoXvTemplateProps {
...
export function CineAbstractoXvTemplateWestern({ invitation, guest, isPersonalized = false }: CineAbstractoXvTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#C9852E";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(201,133,46,.35), 0 18px 50px -30px #C9852E";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#8C2333" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railRef.current) railRef.current.style.borderLeftColor = light ? "rgba(20,20,27,0.14)" : "rgba(168,120,72,0.14)";
          if (railLineRef.current) railLineRef.current.style.background = light ? "rgba(20,20,27,0.14)" : "rgba(168,120,72,0.16)";
          if (railTopRef.current) railTopRef.current.style.color = light ? "#7C7768" : "#8A8577";
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#8C2333" : "#C9852E";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#0D0704", tinta: "#F4F1EA", acento: "#C9852E", clase: `${cxvArchivoBlack.variable} ${cxvMono.variable}`, fuente: "var(--cxv-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${cxvArchivoBlack.variable} ${cxvMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#0D0704",
        fontFamily: "var(--cxv-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{CXV_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="cxv-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="cxv-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #1F150C 0%, #150E08 55%, #0D0704 100%)" }}>
          ...
          <div className="cxv-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="cxv-date-num cxv-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="cxv-divider">
            <span className="cxv-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="cxv-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="cxv-cal-link"
          />
            ...
            <div className="cxv-hero-photo-frame">
              {photoMobile && (
                <div className="acp-mobile-only">
                  <AnimatedCoverPhoto photoSrc={photoMobile} tint={false} effect="enfoque" scrimColorRgb="13,7,4" />
                </div>
              )}
              {photoDesktop && (
                <div className="acp-desktop-only">
                  <AnimatedCoverPhoto photoSrc={photoDesktop} tint={false} effect="enfoque" scrimColorRgb="13,7,4" />
                </div>
              )}
            </div>
 ...
 data-tone="dark" data-screen-label="Countdown" className="cxv-section cxv-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #2A140A 0%, #170F09 55%, #0D0704 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="cxv-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #241708 0%, #150F0A 52%, #0D0704 100%)" }}>
          ...
          <h2 ref={phraseRef} className="cxv-phrase" style={{ fontSize: fraseFontSize }}>
            {fraseWords.map((w, i) => (
              ...
              <span key={i}>
                <span data-w="1" className={i >= fraseAccentStart ? "cxv-accent-italic" : undefined}>
                  {w}
                </span>{" "}
              </span>
            ))}
          </h2>
          <div data-xin="1" data-delay="120" data-dist="90" className="cxv-divider" style={{ position: "relative" }}>
            <span className="cxv-divider-line cxv-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
          </div>
        </section>
        )}
          ...
          <div className="cxv-pan-sticky">
            <div data-strip="1" className="cxv-strip">
              {ceremoniaHabilitada && (
                <div id="ceremonia" data-tone="light" className="cxv-panel cxv-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                  ...
                  <h2 className="cxv-panel-title">
                    ...
                    {ceremoniaDireccion && <><br /><span className="cxv-accent-serif">{ceremoniaDireccion}</span></>}
                  </h2>
              ...
              <div id="details" data-tone="light" className="cxv-panel cxv-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="cxv-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="cxv-crono-row">
                        <span className="cxv-crono-time">{item.time || ""}</span>
                        <span className="cxv-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="cxv-panel cxv-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="cxv-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#8C2333" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#8C2333" />
                  </svg>
              ...
              <div data-tone="dark" className="cxv-panel cxv-panel--center" style={{ background: "#150E08", color: "#F4F1EA" }}>
            ...
            <CxvDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>

        <section data-tone="dark" data-screen-label="Check-in" className="cxv-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #1F150C 0%, #150E08 60%, #0D0704 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="cxv-rsvp">
              <CxvRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="cxv-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="cxv-pan-sticky">
            <div data-strip="1" className="cxv-strip">
                ...
                <div key={pageIndex} data-tone="light" className="cxv-panel cxv-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="cxv-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`cxv-mosaic-cell${i === 0 ? " cxv-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="cxv-seguir cxv-seguir--split">
                    ...
                    <span className="cxv-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="cxv-section" style={{ background: "#150E08" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="cxv-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="cxv-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F5F1E8" : "#C9852E" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="cxv-song-wrap">
              <CxvSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="cxv-section" style={{ background: "#150E08" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="cxv-section" style={{ background: "#150E08" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu entrada" className="cxv-section cxv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #1F150C 0%, #150E08 55%, #0D0704 100%)" }}>
          ...
          <div className="cxv-footer-credit">
      ...
      <div ref={railRef} className="cxv-rail">
        ...
        <span ref={railLabelRef} className="cxv-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="cxv-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="cxv-cover-half cxv-cover-half--top">
          <CxvCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="cxv-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="cxv-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#C9852E" guest={guest} />
      )}
...
function CxvCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="cxv-cd-box">
      <span ref={refEl} className="cxv-cd-num">—</span>
      <span className="cxv-cd-label">{label}</span>
    </div>
  );
}

function CxvDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="cxv-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="cxv-dot" />
      ))}
    </div>
  );
}

// Medallón tipo "entrada de función" con el número 15 en texto plano en el
// centro (así lo muestra el mockup -- ver [data-bulb]/anillo giratorio del
// diseño original), en vez de un ícono.
function CxvMedallion({
  sub,
  arcId,
  arcText,
  spin,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="cxv-medallion-arc" style={{ animation: spin === "reverse" ? "gpRingRev 32s linear infinite" : "gpRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="cxv-medallion-arc-text">
...
function CxvCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="cxv-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="cxv-bank-row-label">{label}</span>
    ...
    <>
      <div className="cxv-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          <div className="cxv-rsvp-row cxv-rsvp-row--payment">
      ...
      <div ref={stubRef} className="cxv-stub">
        ...
        <div ref={sealRef} className="cxv-seal">
          <CxvMedallion sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="cxv-beam" />
        <div className="cxv-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="cxv-rsvp-error">{error}</p>}
...
function CxvSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="cxv-song">
      ...
      {error && <p className="cxv-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="cxv-song-list">
...
function CxvQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: CxvQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="cxv-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="cxv-quiz-q">
            <span className="cxv-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="cxv-quiz-q-text">{q.pregunta}</p>
            <div className="cxv-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " cxv-quiz-opt--correct";
                  else if (chosen) stateClass = " cxv-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " cxv-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`cxv-quiz-opt${stateClass}`}
...
function CxvInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="cxv-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="cxv-bank-row-label">{label}</span>
  ...
  return (
    <div className="cxv-cover-inner">
      ...
      {/* Textura de tapa propia de esta plantilla: foquitos de marquesina de
          ...
          del abanico ("sunburst") de Guest Pass VIP, de los destellos de
          Princesa y de la trama de Corona Escarlata. */}
      <div className="cxv-bulb-row">
        {[0, 0.2, 0.4, 0.6, 0.8].map((d, i) => (
          <span key={i} className="cxv-bulb" style={{ animationDelay: `${d}s` }} />
        ))}
      </div>
      <div className="cxv-cover-content">
        ...
        <div className="cxv-cover-center">
          ...
          <h1 ref={namesRef} className="cxv-cover-names">{namesTitle}</h1>
          <span className="cxv-cover-rule" />
...
// ---------------------------------------------------------------------
const CXV_CSS = `
  .cxv-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .cxv-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #C9852E; text-decoration: none; }
  ...
  @keyframes cxvBulbFlicker { 0%,100% { opacity: .35; } 50% { opacity: 1; } }
  ...
  .cxv-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #0D0704; }
  ...
  @media (max-width: 767px) {
    .cxv-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .cxv-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(140,35,51,.3); }
    .cxv-hero-photo-kicker { bottom: 40px; }
    .cxv-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }
  .cxv-section--between { justify-content: space-between; }

  .cxv-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .cxv-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .cxv-date-num { font-family: var(--cxv-archivo), sans-serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .cxv-date-num--right { text-align: right; line-height: 0.86; }
  .cxv-date-month { font-family: var(--cxv-archivo), sans-serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #C9852E; padding-left: 12%; }

  .cxv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .cxv-divider-line { width: 52px; height: 1px; background: #C9852E; display: inline-block; }
  .cxv-divider-line--long { width: 64px; }

  .cxv-lead { margin: 0; font-family: var(--cxv-archivo), sans-serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .cxv-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .cxv-cal-link:hover { color: #C9852E; }

  .cxv-medallion { position: relative; }
  .cxv-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .cxv-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .cxv-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .cxv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #8C2333, #3A0F16, #C9852E, #F5F1E8, #8C2333); filter: saturate(.75); }
  .cxv-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #120C08; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .cxv-medallion-num { font-family: var(--cxv-archivo), sans-serif; font-size: 26px; line-height: 1; color: #F5F1E8; }
  .cxv-medallion-num-sm { font-family: var(--cxv-archivo), sans-serif; font-size: 16px; line-height: 1; color: #F5F1E8; }
  .cxv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #C9852E; }
  .cxv-medallion-arc { position: absolute; inset: -14%; }
  .cxv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #C9852E; font-family: var(--cxv-mono), monospace; }

  .cxv-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(168,120,72,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .cxv-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F5F1E8, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .cxv-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .cxv-cd-box { border: 1px solid #3A2A18; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .cxv-cd-num { font-family: var(--cxv-archivo), sans-serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F2E2C4; display: inline-block; }
  .cxv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #C9852E; }
  .cxv-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #0D0704 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .cxv-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(140,35,51,0.3), rgba(201,133,46,0.2), rgba(201,133,46,0.32), rgba(140,35,51,0.3)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .cxv-accent-italic { font-style: italic; color: #8C2333; }
  .cxv-accent-serif { font-style: italic; color: #8C2333; font-family: var(--cxv-archivo), sans-serif; }
  .cxv-accent-serif-2 { color: #8C2333; }
  .cxv-h2 { margin: 0; font-family: var(--cxv-archivo), sans-serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .cxv-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .cxv-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .cxv-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .cxv-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .cxv-panel-title { margin: 0; position: relative; font-family: var(--cxv-archivo), sans-serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .cxv-panel-title-md { margin: 0; position: relative; font-family: var(--cxv-archivo), sans-serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  ...
  .cxv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #8C2333; margin-top: auto; }
  .cxv-seguir--split { justify-content: space-between; }
  .cxv-side-hint { display: inline-block; animation: gpSide 2.2s ease-in-out infinite; }
  .cxv-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .cxv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #8C2333; }

  .cxv-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .cxv-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .cxv-crono-time { font-family: var(--cxv-mono), monospace; color: #8C2333; min-width: 42px; }
  .cxv-crono-title { flex: 1; }

  .cxv-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .cxv-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .cxv-stub { position: relative; overflow: hidden; border: 1px solid #3A2A18; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .cxv-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .cxv-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .cxv-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .cxv-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F5F1E8, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .cxv-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--cxv-mono), monospace; }
  .cxv-rsvp-rows { display: flex; flex-direction: column; }
  .cxv-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(168,120,72,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .cxv-rsvp-row > span:first-child { flex-shrink: 0; }
  .cxv-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .cxv-rsvp-row--payment { align-items: flex-start; }
  .cxv-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .cxv-rsvp-payment-total { color: #F5F1E8; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .cxv-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .cxv-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .cxv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #C9852E; background: transparent; color: #C9852E; font-size: 14px; line-height: 1; cursor: pointer; }
  .cxv-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .cxv-rsvp-stepper span { font-family: var(--cxv-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .cxv-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(168,120,72,0.3); color: #F4F1EA; font-family: var(--cxv-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .cxv-rsvp-input::placeholder { color: #6E6A5D; }
  .cxv-rsvp-input:focus { outline: none; border-bottom-color: #C9852E; }
  .cxv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--cxv-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #C9852E, #8C2333); border: 1px solid #C9852E; color: #0B0B10; cursor: pointer; }
  .cxv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .cxv-rsvp-btn--ghost { background: transparent; color: #C9852E; }
  .cxv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #8C2333; margin: 0; }
  .cxv-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .cxv-rsvp-declined-text { margin: 0; font-family: var(--cxv-archivo), sans-serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .cxv-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .cxv-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .cxv-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  .cxv-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }

  .cxv-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .cxv-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .cxv-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .cxv-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }

  .cxv-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .cxv-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(168,120,72,0.18); }
  .cxv-bank-row:last-child { border-bottom: none; }
  .cxv-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .cxv-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .cxv-bank-copy { flex-shrink: 0; font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #C9852E; background: transparent; color: #C9852E; cursor: pointer; }
  .cxv-bank-copy:hover { background: rgba(201,133,46,0.12); }

  .cxv-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .cxv-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: gpEq 1.1s ease-in-out infinite; display: inline-block; }
  .cxv-song-wrap { font-family: var(--cxv-mono), monospace; }

  .cxv-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .cxv-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(168,120,72,0.3); padding-bottom: 12px; }
  .cxv-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .cxv-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(168,120,72,0.3); color: #F4F1EA; font-family: var(--cxv-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .cxv-song-input::placeholder { color: #6E6A5D; }
  .cxv-song-input:focus { outline: none; border-bottom-color: #C9852E; }
  .cxv-song-sep { color: #8A8577; flex-shrink: 0; }
  .cxv-song-submit { flex-shrink: 0; background: none; border: none; color: #C9852E; font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .cxv-song-submit:hover { color: #F5F1E8; }
  .cxv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .cxv-song-error { font-size: 10px; color: #8C2333; margin-top: 6px; }
  .cxv-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .cxv-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--cxv-mono), monospace; }
  .cxv-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .cxv-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .cxv-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .cxv-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .cxv-quiz-q-num { font-family: var(--cxv-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .cxv-quiz-q-text { margin: 0; font-family: var(--cxv-archivo), sans-serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .cxv-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .cxv-quiz-opt { font-family: var(--cxv-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(201,133,46,0.4); background: transparent; color: #C9852E; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .cxv-quiz-opt:disabled { cursor: default; }
  .cxv-quiz-opt--picked { background: #C9852E; border-color: #C9852E; color: #0B0B10; }
  .cxv-quiz-opt--correct { background: #C9852E; border-color: #C9852E; color: #0B0B10; }
  .cxv-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .cxv-quiz-result { padding-top: 18px; border-top: 1px solid rgba(201,133,46,0.2); }
  .cxv-quiz-result-score { margin: 0 0 6px; font-family: var(--cxv-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F5F1E8; }
  .cxv-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .cxv-final-card { border: 1px solid #C9852E; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .cxv-final-names { font-family: var(--cxv-archivo), sans-serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F5F1E8; }
  .cxv-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .cxv-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .cxv-replay { cursor: pointer; color: #C9852E; }
  .cxv-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .cxv-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(168,120,72,0.14); }
  .cxv-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .cxv-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(168,120,72,0.16); position: relative; }
  .cxv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F5F1E8, #C9852E); transition: height 260ms linear; display: block; }
  .cxv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #C9852E; transition: color 500ms ease; }

  .cxv-cover { position: absolute; inset: 0; z-index: 5; }
  .cxv-cover-half { position: absolute; left: 0; right: 0; height: 50%; overflow: hidden; transition: transform 1100ms cubic-bezier(.7,0,.2,1); }
  .cxv-cover-half--top { top: 0; }
  .cxv-cover-half--bottom { bottom: 0; }
  .cxv-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #180D06 0%, #150E08 46%, #0D0704 100%); }
  .cxv-cover-half--bottom .cxv-cover-inner { top: auto; bottom: 0; }
  .cxv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(140,35,51,0.4), rgba(201,133,46,0.3), rgba(201,133,46,0.5), rgba(140,35,51,0.4)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  /* Foquitos de marquesina (ver mockup original) -- textura de tapa propia,
     distinta del abanico ("sunburst") de Guest Pass VIP, de los destellos de
     Princesa y de la trama de Corona Escarlata. */
  .cxv-bulb-row { position: absolute; top: 10%; left: 0; right: 0; display: flex; justify-content: space-around; padding: 0 8%; }
  .cxv-bulb { width: 5px; height: 5px; border-radius: 50%; background: #C9852E; animation: cxvBulbFlicker 1.4s ease-in-out infinite; display: inline-block; }
  .cxv-cover-content { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: space-between; padding: 24px max(24px, calc((100% - 440px) / 2)) 26px; }
  .cxv-cover-top-row { display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.24em; color: #8A8577; }
  .cxv-cover-center { display: flex; flex-direction: column; align-items: center; gap: clamp(10px, 2.2vh, 22px); text-align: center; }
  .cxv-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .cxv-cover-names { margin: 0; font-family: var(--cxv-archivo), sans-serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F5F1E8; }
  .cxv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#C9852E, transparent); display: block; }
  .cxv-cover-date { font-size: 11.5px; letter-spacing: 0.3em; color: #A8A292; white-space: nowrap; }
  .cxv-cover-bottom { display: flex; flex-direction: column; gap: clamp(12px, 2.4vh, 22px); }
  .cxv-cover-facts { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.2em; color: #6E6A5D; }
  .cxv-cover-cta { border: 1px solid #C9852E; background: linear-gradient(100deg, rgba(168,120,72,0.08), rgba(224,196,150,0.2), rgba(168,120,72,0.08)); color: #F2E2C4; font-family: var(--cxv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .cxv-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .cxv-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(224,196,150,0.34), rgba(246,239,221,0.5), rgba(224,196,150,0.34)); color: #0B0B0F; }
  }
  ...
  .cxv-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .cxv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #C9852E; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .cxv-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
