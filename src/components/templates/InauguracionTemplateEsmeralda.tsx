...
/**
 * InauguracionTemplateEsmeralda.tsx
 *
 * Portado 1:1 desde el mockup "Inauguracion Gran Apertura" (mockup/Eventos),
 * siguiendo el mismo motor que Guest Pass VIP/Baby Shower (pase de acceso
 * estilo ticket, Archivo Black + IBM Plex Mono, medallón circular con texto en arco,
...
interface InauguracionTemplateEsmeraldaProps {
...
export function InauguracionTemplateEsmeralda({ invitation, guest, isPersonalized = false }: InauguracionTemplateEsmeraldaProps) {
  const tx = useTextos();
  ...
  const namesTitle = festejado || "Nuestro Local";
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#E7F5EF";
      }
      if (stubRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#0F2B20" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#0F2B20" : "#4FA983";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#080808", tinta: "#F4F1EA", acento: "#4FA983", clase: `${ingDisplay.variable} ${ingMono.variable}`, fuente: "var(--ing-mono), monospace" }}
      />
    );
  }
  ...
  return (
    <div
      ref={rootRef}
      className={`${ingDisplay.variable} ${ingMono.variable}`}
      style={{
        position: "fixed",
        inset: 0,
        width: "100%",
        ...
        overflow: "hidden",
        background: "#080808",
        fontFamily: "var(--ing-mono), monospace",
        color: "#F4F1EA",
      }}
    >
      <style>{GP_CSS}</style>
      ...
      <div ref={scrollerRef} data-scroller="1" className="ing-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="ing-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #0F2B20 55%, #080808 100%)" }}>
          ...
          <div className="ing-date-stack">
            ...
            <span data-xin="1" data-delay="280" data-dist="220" className="ing-date-num ing-date-num--right">
              {String(fechaEvento.getFullYear()).slice(-2)}
            </span>
          </div>
          <div data-xin="1" data-delay="380" data-dist="-80" className="ing-divider">
            <span className="ing-divider-line" /><span>{weekday} · {hora} H</span>
          </div>
          <p data-xin="1" data-delay="460" className="ing-lead">{portadaMensaje}</p>
          <AddToCalendarLink
            eventName={namesTitle}
            targetDate={eventDateTime}
            location={lugarNombre || direccion}
            description={portadaMensaje}
            className="ing-cal-link"
          />
        ...
        {(photoMobile || photoDesktop) && (
          ...
          >
            ...
            <span data-xin="1" data-dist="-60" className="ing-kicker ing-hero-photo-kicker">02 — EL PRIMER VISTAZO</span>
          </section>
        )}

        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="ing-section ing-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #1A1414 0%, #0D0B10 55%, #080808 100%)" }}>
          <div className="ing-scan-grid" />
          ...
          <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="ing-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #080808 100%)" }}>
            ...
            <h2 ref={phraseRef} className="ing-phrase" style={{ fontSize: fraseFontSize }}>
              {fraseWords.map((w, i) => (
                ...
                <span key={i}>
                  <span data-w="1" className={i >= fraseAccentStart ? "ing-accent-italic" : undefined}>
                    {w}
                  </span>{" "}
                </span>
              ))}
            </h2>
            <div data-xin="1" data-delay="120" data-dist="90" className="ing-divider" style={{ position: "relative" }}>
              <span className="ing-divider-line ing-divider-line--long" /><span>{fechaCorta} — {hora} H</span>
            </div>
          </section>
        )}
          ...
          <div className="ing-pan-sticky">
            <div data-strip="1" className="ing-strip">
              <div id="details" data-tone="light" className="ing-panel ing-panel--between" style={{ background: "#EFEBE1", color: "#14141B" }}>
                ...
                {cronograma.length > 0 && (
                  <div className="ing-crono">
                    {cronograma.map((item, i) => (
                      <div key={i} className="ing-crono-row">
                        <span className="ing-crono-time">{item.time || ""}</span>
                        <span className="ing-crono-title">{item.title}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {hayComoLlegar && (
                <div data-tone="light" className="ing-panel ing-panel--end" style={{ background: "#E4DFD3", color: "#14141B" }}>
                  <svg viewBox="0 0 300 500" preserveAspectRatio="none" className="ing-route-svg">
                    <path ref={routeRef} d="M18 468 C 130 400, 54 262, 152 220 S 254 140, 282 40" fill="none" stroke="#0F2B20" strokeWidth={1.6} />
                    <circle cx={282} cy={40} r={5} fill="#0F2B20" />
                  </svg>
              ...
              <div data-tone="dark" className="ing-panel ing-panel--center" style={{ background: "#0F2B20", color: "#F4F1EA" }}>
            ...
            <IngDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>

        <section data-tone="dark" data-screen-label="Check-in" className="ing-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #0F2B20 60%, #080808 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="ing-rsvp">
              <IngRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="ing-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="ing-pan-sticky">
            <div data-strip="1" className="ing-strip">
                ...
                <div key={pageIndex} data-tone="light" className="ing-panel ing-panel--gap" style={{ background: ALBUM_TONES[pageIndex % ALBUM_TONES.length], color: "#14141B" }}>
                  ...
                  <div className="ing-mosaic">
                    {page.length > 0 ? page.map((url, i) => (
                      <div
                        key={i}
                        className={`ing-mosaic-cell${i === 0 ? " ing-mosaic-cell--featured" : ""}`}
                        role="button"
                        tabIndex={0}
                  ...
                  <div className="ing-seguir ing-seguir--split">
                    ...
                    <span className="ing-accent-serif-2">{tx("invitacion.portada.segui").toUpperCase() + (!scrollVertical && photoPages.length > 1 ? " →" : " ↓")}</span>
                  </div>
                </div>
              ))}
            </div>
            <IngDots count={photoPages.length} />
          </div>
        </div>

        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="ing-section" style={{ background: "#0F2B20" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="ing-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="ing-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#E7F5EF" : "#4FA983" }} />
              ))}
            </div>
            <div data-xin="1" data-delay="240" data-dist="110" className="ing-song-wrap">
              <IngSongSuggestion
                invitationId={String(invitation.id ?? "")}
                guestToken={guest?.uniqueToken}
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="ing-section" style={{ background: "#0F2B20" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="ing-section" style={{ background: "#0F2B20" }}>
        ...
        <section data-tone="dark" data-screen-label={tx("invitacion.pase.tuPase")} className="ing-section ing-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #0F2B20 55%, #080808 100%)" }}>
      ...
      <div ref={railRef} className="ing-rail">
        ...
        <span ref={railLabelRef} className="ing-rail-label">SAVE THE DATE</span>
      </div>

      <div ref={coverRef} className="ing-cover" style={{ "--esc": escalaTitulo(largoTitulo(showGuestNameInCover ? coverGuestName : namesTitle)) } as React.CSSProperties}>
        <div ref={topRef} className="ing-cover-half ing-cover-half--top">
          <IngCoverHalf
            namesRef={namesRef}
            kickerRef={kickerRef}
            perfRef={perfRef}
      ...
      {expandedPhoto && (
        <div
          className="ing-lightbox"
          onClick={() => setExpandedPhoto(null)}
          onContextMenu={(e) => e.preventDefault()}
        >
          <button
            type="button"
            className="ing-lightbox-close"
            onClick={(e) => {
              e.stopPropagation();
...
function IngCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
    <div data-xin="1" data-delay={delay} data-dist={dist} className="ing-cd-box">
      <span ref={refEl} className="ing-cd-num">—</span>
      <span className="ing-cd-label">{label}</span>
    </div>
  );
}

function IngDots({ count = 3 }: { count?: number }) {
  // Un punto solo no pagina nada.
  if (count <= 1) return null;

  return (
    <div data-dots="1" className="ing-dots">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} data-dot={i} className="ing-dot" />
      ))}
    </div>
  );
}

function IngMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
  ...
  return (
    <>
      ...
      {arcId && (
        <svg viewBox="0 0 100 100" className="ing-medallion-arc" style={{ animation: spin === "reverse" ? "bbsRingRev 32s linear infinite" : "bbsRingRev 34s linear infinite" }}>
          <defs>
            <path id={arcId} d="M50 50 m -38 0 a 38 38 0 1 1 76 0 a 38 38 0 1 1 -76 0" fill="none" />
          </defs>
          <text className="ing-medallion-arc-text">
...
function IngCopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="ing-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="ing-bank-row-label">{label}</span>
    ...
    <>
      <div className="ing-rsvp-rows">
        ...
        {hasPayment && paymentAmount != null && !isExempt && (
          ...
          <div className="ing-rsvp-row ing-rsvp-row--payment">
      ...
      <div ref={stubRef} className="ing-stub">
        ...
        <div ref={sealRef} className="ing-seal">
          <IngMedallionCmp label="BB" sub="" arcId="" arcText="" spin="none" compact />
        </div>
        <div ref={beamRef} className="ing-beam" />
        <div className="ing-barcode" style={{ width: "70%", opacity: confirmed ? 0.7 : 0.2 }} />
      </div>

      {error && <p className="ing-rsvp-error">{error}</p>}
...
function IngSongSuggestion({ invitationId, guestToken, guestName }: { invitationId: string; guestToken?: string; guestName: string }) {
  ...
  return (
    <div className="ing-song">
      ...
      {error && <p className="ing-song-error">{error}</p>}
      {songs.length > 0 && (
        <div className="ing-song-list">
...
function IngQuiz({ preguntas, invitationId, guestToken, guestName }: { preguntas: IngQuizQuestion[]; invitationId: string; guestToken?: string; guestName?: string }) {
  ...
  return (
    <div className="ing-quiz">
      {preguntas.map((q, qi) => {
        const correctIdx = q.respuestaCorrecta ?? q.correcta;
        return (
          <div key={qi} className="ing-quiz-q">
            <span className="ing-quiz-q-num">{String(qi + 1).padStart(2, "0")}</span>
            <p className="ing-quiz-q-text">{q.pregunta}</p>
            <div className="ing-quiz-opts">
              {q.opciones.map((opt, oi) => {
                const chosen = picks[qi] === oi;
                const revealed = picks[qi] !== undefined && correctIdx !== undefined;
                let stateClass = "";
                if (revealed) {
                  if (oi === correctIdx) stateClass = " ing-quiz-opt--correct";
                  else if (chosen) stateClass = " ing-quiz-opt--wrong";
                } else if (chosen) {
                  stateClass = " ing-quiz-opt--picked";
                }
                return (
                  <button
                    key={oi}
                    type="button"
                    className={`ing-quiz-opt${stateClass}`}
...
function IngInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="ing-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="ing-bank-row-label">{label}</span>
    ...
    <div className="ing-cover-inner">
      ...
      <div className="ing-cover-content">
        ...
        <div className="ing-cover-center">
          ...
          <h1 ref={namesRef} className="ing-cover-names">{namesTitle}</h1>
          <span className="ing-cover-rule" />
...
// ---------------------------------------------------------------------
const GP_CSS = `
  .ing-scroller { position: absolute; inset: 0; overflow-y: auto; overflow-x: hidden; opacity: 0; transition: opacity 900ms ease 260ms; scrollbar-width: none; }
  .ing-scroller::-webkit-scrollbar { width: 0; height: 0; }

  a { color: #4FA983; text-decoration: none; }
  a:hover { color: #E7F5EF; }
  ...
  @media (prefers-reduced-motion: reduce) { .ing-scroller * { animation: none !important; } }

  .ing-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; display: flex; flex-direction: column; justify-content: center; gap: 30px; padding: 96px max(30px, calc((100% - 560px) / 2)) 110px max(24px, calc((100% - 560px) / 2)); overflow: hidden; }
  .ing-section--between { justify-content: space-between; }
  ...
  .ing-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #080808; }
  .ing-hero-photo-frame { position: absolute; inset: 0; overflow: hidden; }
  .ing-hero-photo-kicker { position: absolute; left: 0; right: 0; bottom: 0; z-index: 2; padding: 0 max(24px, calc((100% - 560px) / 2)) 48px; }
  ...
  @media (max-width: 767px) {
    .ing-hero-photo-section--no-mobile { min-height: 0; height: 0; }
  }
  @media (min-width: 768px) {
    .ing-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(200,164,92,.3); }
    .ing-hero-photo-kicker { bottom: 40px; }
    .ing-hero-photo-section--no-desktop { min-height: 0; height: 0; }
  }

  .ing-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }

  .ing-date-stack { display: flex; flex-direction: column; gap: 2px; }
  .ing-date-num { font-family: var(--ing-display), serif; font-size: clamp(66px, 23vw, 140px); line-height: 0.82; letter-spacing: -0.04em; }
  .ing-date-num--right { text-align: right; line-height: 0.86; }
  .ing-date-month { font-family: var(--ing-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #4FA983; padding-left: 12%; }

  .ing-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .ing-divider-line { width: 52px; height: 1px; background: #4FA983; display: inline-block; }
  .ing-divider-line--long { width: 64px; }

  .ing-lead { margin: 0; font-family: var(--ing-display), serif; font-style: italic; font-size: 20px; line-height: 1.4; color: #A8A292; max-width: 330px; }
  .ing-cal-link { display: inline-flex; align-items: center; gap: 7px; margin-top: 4px; font-family: var(--ing-mono), monospace; font-size: 10px; letter-spacing: 0.24em; text-transform: uppercase; color: #8A8577; text-decoration: none; transition: color 200ms ease; }
  .ing-cal-link:hover { color: #4FA983; }

  .ing-medallion { position: relative; }
  .ing-medallion--corner { position: absolute; right: max(32px, calc((100% - 560px) / 2)); top: 12%; width: clamp(86px, 22vw, 116px); aspect-ratio: 1; }
  .ing-medallion--lg { width: clamp(130px, 34vw, 178px); aspect-ratio: 1; margin: 0 auto; }
  .ing-medallion--final { width: clamp(104px, 26vw, 132px); aspect-ratio: 1; margin: -12px auto 0; }
  .ing-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #4FA983, #E7F5EF, #6E5AA8); filter: saturate(.75); }
  .ing-medallion-core { position: absolute; inset: 3px; border-radius: 50%; background: #0C0C11; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; text-align: center; padding: 6px; }
  .ing-medallion-label { font-family: var(--ing-display), serif; font-size: 26px; line-height: 1; color: #E7F5EF; }
  .ing-medallion-label-sm { font-family: var(--ing-display), serif; font-size: 16px; color: #E7F5EF; }
  .ing-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .ing-medallion-sub--accent { color: #4FA983; }
  .ing-medallion-arc { position: absolute; inset: -14%; }
  ...
  .ing-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #4FA983; opacity: 0.4; font-family: var(--ing-mono), monospace; }

  .ing-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .ing-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #E7F5EF, transparent); animation: bbsScan 6s linear infinite; pointer-events: none; }

  .ing-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .ing-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .ing-cd-num { font-family: var(--ing-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #E7F5EF; display: inline-block; }
  .ing-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #4FA983; }
  ...
  .ing-accent-italic { font-style: italic; color: #4FA983; }
  .ing-accent-serif { font-style: italic; color: #0F2B20; font-family: var(--ing-display), serif; }
  .ing-accent-serif-2 { color: #0F2B20; }
  .ing-h2 { margin: 0; font-family: var(--ing-display), serif; font-weight: 400; font-size: clamp(40px, 12vw, 68px); line-height: 0.96; }

  .ing-pan { height: calc(100vh + var(--st-pasos, 2) * 80vh); position: relative; }
  .ing-pan-sticky { position: sticky; top: 0; height: calc(var(--vh, 1vh) * 100); overflow: hidden; }
  ...
  .ing-hair-bg { position: absolute; inset: 0; background: repeating-linear-gradient(90deg, rgba(20,20,27,0.05) 0 1px, transparent 1px 26px); pointer-events: none; }
  .ing-panel-top { position: relative; display: flex; justify-content: space-between; font-size: 9.5px; letter-spacing: 0.32em; color: #7C7768; }
  .ing-panel-title { margin: 0; position: relative; font-family: var(--ing-display), serif; font-weight: 400; font-size: clamp(46px, 13.5vw, 76px); line-height: 0.92; }
  .ing-panel-title-md { margin: 0; position: relative; font-family: var(--ing-display), serif; font-weight: 400; font-size: clamp(42px, 12.5vw, 64px); line-height: 0.94; }
  ...
  .ing-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #0F2B20; margin-top: auto; }
  .ing-seguir--split { justify-content: space-between; }
  .ing-side-hint { display: inline-block; animation: bbsSide 2.2s ease-in-out infinite; }
  .ing-route-svg { position: absolute; inset: 0; width: 100%; height: 100%; }
  .ing-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #0F2B20; }

  .ing-crono { position: relative; display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
  .ing-crono-row { display: flex; align-items: baseline; gap: 14px; font-size: 11px; letter-spacing: 0.1em; color: #4A473F; border-bottom: 1px solid #D6D1C4; padding-bottom: 8px; }
  .ing-crono-time { font-family: var(--ing-mono), monospace; color: #0F2B20; min-width: 42px; }
  .ing-crono-title { flex: 1; }

  .ing-dots { position: absolute; left: 0; right: 34px; bottom: 26px; display: flex; gap: 6px; justify-content: center; z-index: 2; }
  .ing-dot { width: 22px; height: 2px; background: rgba(20,20,27,0.18); transition: background 400ms ease; display: inline-block; }

  .ing-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  .ing-stub-top { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.24em; color: #8A8577; }
  .ing-stub-status { color: #6E6A5D; transition: color 400ms ease; }
  .ing-seal { width: 62px; aspect-ratio: 1; position: relative; opacity: .25; transform: scale(.86); transition: opacity 700ms ease, transform 700ms cubic-bezier(.16,1,.3,1); align-self: center; }
  .ing-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #E7F5EF, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }

  .ing-rsvp { display: flex; flex-direction: column; gap: 22px; width: 100%; max-width: 420px; font-family: var(--ing-mono), monospace; }
  .ing-rsvp-rows { display: flex; flex-direction: column; }
  .ing-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(200,164,92,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  .ing-rsvp-row > span:first-child { flex-shrink: 0; }
  .ing-rsvp-row > span:last-child { color: #F4F1EA; letter-spacing: 0.02em; text-align: right; }
  .ing-rsvp-row--payment { align-items: flex-start; }
  .ing-rsvp-payment-value { display: flex; flex-direction: column; align-items: flex-end; gap: 6px; }
  .ing-rsvp-payment-total { color: #E7F5EF; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  .ing-rsvp-payment-detail { display: flex; flex-direction: column; align-items: flex-end; gap: 2px; font-size: 9px; letter-spacing: 0.05em; color: #8A8577; }
  .ing-rsvp-stepper { display: flex; align-items: center; gap: 12px; }
  .ing-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #4FA983; background: transparent; color: #4FA983; font-size: 14px; line-height: 1; cursor: pointer; }
  .ing-rsvp-stepper button:disabled { opacity: 0.3; cursor: default; }
  .ing-rsvp-stepper span { font-family: var(--ing-mono), monospace; color: #F4F1EA; min-width: 20px; text-align: center; }
  .ing-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--ing-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .ing-rsvp-input::placeholder { color: #6E6A5D; }
  .ing-rsvp-input:focus { outline: none; border-bottom-color: #4FA983; }
  .ing-rsvp-btn { width: 100%; padding: 16px; font-family: var(--ing-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #4FA983, #0F2B20); border: 1px solid #4FA983; color: #0F2B20; cursor: pointer; }
  .ing-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .ing-rsvp-btn--ghost { background: transparent; color: #4FA983; }
  .ing-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #4FA983; margin: 0; }
  .ing-rsvp-declined { display: flex; flex-direction: column; gap: 18px; align-items: center; text-align: center; max-width: 380px; }
  .ing-rsvp-declined-text { margin: 0; font-family: var(--ing-display), serif; font-style: italic; font-size: 18px; line-height: 1.4; color: #A8A292; }

  .ing-photo-grid { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; gap: 12px; }
  .ing-photo-cell { height: 100%; min-height: 0; aspect-ratio: 4/5; max-width: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); display: flex; align-items: flex-end; padding: 10px; box-sizing: border-box; overflow: hidden; position: relative; }
  .ing-photo-cell--offset { margin-top: 18px; }
  .ing-photo-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  .ing-photo-placeholder { position: relative; font-size: 8.5px; letter-spacing: 0.16em; color: #6E6A5D; }
  .ing-upload-wrap { flex: 1; min-height: 0; overflow-y: auto; }
  .ing-album-embed { flex: 1; min-height: 0; overflow-y: auto; }
  .ing-live-placeholder { flex: 1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 24px; border: 1px dashed rgba(20,20,27,0.2); }
  ...
  .ing-mosaic { position: relative; flex: 1; min-height: 0; display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: auto; grid-auto-flow: dense; align-content: start; gap: 10px; }
  .ing-mosaic-cell--featured { grid-column: span 2; grid-row: span 2; cursor: pointer; }
  .ing-mosaic-cell { position: relative; height: 0; padding-top: 100%; background: repeating-linear-gradient(135deg, #DCD7CB 0 6px, #E9E5DC 6px 12px); overflow: hidden; cursor: pointer; }
  .ing-mosaic-img { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; }
  ...
  .ing-bank-wrap { position: relative; display: flex; flex-direction: column; gap: 14px; width: 100%; max-width: 420px; margin: 0; }
  .ing-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(200,164,92,0.18); }
  .ing-bank-row:last-child { border-bottom: none; }
  .ing-bank-row-label { display: block; font-size: 9px; letter-spacing: 0.2em; color: #8A8577; margin-bottom: 3px; }
  .ing-bank-row-value { font-size: 12px; color: #F4F1EA; word-break: break-all; }
  .ing-bank-copy { flex-shrink: 0; font-family: var(--ing-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #4FA983; background: transparent; color: #4FA983; cursor: pointer; }
  .ing-bank-copy:hover { background: rgba(200,164,92,0.12); }

  .ing-eq { display: flex; align-items: flex-end; gap: 5px; height: 52px; }
  .ing-eq-bar { width: 4px; height: 100%; transform-origin: bottom; animation: bbsEq 1.1s ease-in-out infinite; display: inline-block; }
  .ing-song-wrap { font-family: var(--ing-mono), monospace; }

  .ing-song { position: relative; width: 100%; max-width: 420px; margin: 0 auto; }
  .ing-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(200,164,92,0.3); padding-bottom: 12px; }
  .ing-song-inputs { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; text-transform: uppercase; }
  .ing-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(200,164,92,0.3); color: #F4F1EA; font-family: var(--ing-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .ing-song-input::placeholder { color: #6E6A5D; }
  .ing-song-input:focus { outline: none; border-bottom-color: #4FA983; }
  .ing-song-sep { color: #8A8577; flex-shrink: 0; }
  .ing-song-submit { flex-shrink: 0; background: none; border: none; color: #4FA983; font-family: var(--ing-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .ing-song-submit:hover { color: #E7F5EF; }
  .ing-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .ing-song-error { font-size: 10px; color: #4FA983; margin-top: 6px; }
  .ing-song-list { display: flex; flex-direction: column; margin-top: 14px; max-height: 180px; overflow-y: auto; }
  .ing-song-item { display: flex; flex-direction: column; gap: 3px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-family: var(--ing-mono), monospace; }
  .ing-song-item-title { font-size: 11.5px; letter-spacing: 0.04em; color: #F4F1EA; }
  .ing-song-item-by { font-size: 9.5px; letter-spacing: 0.1em; color: #6E6A5D; }

  .ing-quiz { display: flex; flex-direction: column; gap: 28px; width: 100%; max-width: 460px; }
  .ing-quiz-q { display: flex; flex-direction: column; gap: 14px; }
  .ing-quiz-q-num { font-family: var(--ing-mono), monospace; font-size: 10px; letter-spacing: 0.24em; color: #8A8577; }
  .ing-quiz-q-text { margin: 0; font-family: var(--ing-display), serif; font-size: clamp(18px, 4vw, 22px); line-height: 1.3; color: #F4F1EA; }
  .ing-quiz-opts { display: flex; flex-wrap: wrap; gap: 10px; }
  .ing-quiz-opt { font-family: var(--ing-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #4FA983; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .ing-quiz-opt:disabled { cursor: default; }
  .ing-quiz-opt--picked { background: #4FA983; border-color: #4FA983; color: #0F2B20; }
  .ing-quiz-opt--correct { background: #4FA983; border-color: #4FA983; color: #0F2B20; }
  .ing-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .ing-quiz-result { padding-top: 18px; border-top: 1px solid rgba(200,164,92,0.2); }
  .ing-quiz-result-score { margin: 0 0 6px; font-family: var(--ing-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #E7F5EF; }
  .ing-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .ing-final-card { border: 1px solid #4FA983; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .ing-final-names { font-family: var(--ing-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #E7F5EF; }
  .ing-barcode { height: 26px; background: repeating-linear-gradient(90deg, #6E6A5D 0 2px, transparent 2px 4px, #6E6A5D 4px 5px, transparent 5px 9px, #6E6A5D 9px 11px, transparent 11px 14px); }
  .ing-final-footer { display: flex; justify-content: space-between; align-items: center; font-size: 9px; letter-spacing: 0.24em; color: #56534A; }
  .ing-replay { cursor: pointer; color: #4FA983; }
  .ing-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .ing-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .ing-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .ing-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .ing-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#E7F5EF, #4FA983); transition: height 260ms linear; display: block; }
  .ing-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #4FA983; transition: color 500ms ease; }

  .ing-cover { position: absolute; inset: 0; z-index: 5; }
  ...
  .ing-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #141414 0%, #0F2B20 46%, #080808 100%); }
  .ing-cover-half--bottom .ing-cover-inner { top: auto; bottom: 0; }
  .ing-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(79,169,131,0.35), rgba(15,43,32,0.5), rgba(79,169,131,0.5), rgba(79,169,131,0.35)); filter: blur(64px); opacity: .62; animation: bbsFoil 26s linear infinite; }
  .ing-cover-texture { position: absolute; inset: 0; background-image: linear-gradient(90deg, rgba(231,245,239,.08) 1px, transparent 1px); background-size: 44px 100%; }
  ...
  .ing-cover-kicker { font-size: 9.5px; letter-spacing: 0.34em; color: #8A8577; }
  .ing-cover-names { margin: 0; font-family: var(--ing-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #E7F5EF; }
  .ing-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#4FA983, transparent); display: block; }
  ...
  .ing-cover-cta { border: 1px solid #4FA983; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--ing-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .ing-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .ing-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0F2B20; }
  }
  .ing-barcode-wrap { display: flex; flex-direction: column; align-items: center; gap: 10px; }

  .ing-hint { position: absolute; left: 0; right: 34px; bottom: 18px; z-index: 6; text-align: center; font-size: 9px; letter-spacing: 0.28em; color: #8A8577; opacity: 0; transition: opacity 600ms ease; pointer-events: none; animation: bbsHint 2.4s ease-in-out infinite; }

  .ing-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(8,8,11,0.96); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }
  .ing-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #4FA983; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .ing-lightbox-img { max-width: 100%; max-height: 88vh; object-fit: contain; cursor: default; }
`;
