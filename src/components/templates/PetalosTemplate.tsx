 ...
 *   --t-bg #FFF1EF · --t-surface #FFDEDA · --petalos-ink #3D1418
 *   --t-muted #8f4a4f · --t-acc #E23B4E (rojo vibrante) · --t-acc2 #B5142A (rojo profundo)
 *
...
interface PetalosTemplateProps {
...
export function PetalosTemplate({ invitation, guest, isPersonalized = false }: PetalosTemplateProps) {
  ...
  // Portada animada -- Petalos es CSS-vars, tema claro (--t-bg #FFF1EF, ink
  // #3D1418) -- mismo caso que Perlada/LuzLuna: texto no puede forzarse a
  // claro por JS, se resuelve con clases CSS + media query
  // (.petalos-cover-text/-muted), sin tinte. scrimColorRgb = rgb del ink
  // oscuro propio (#3D1418).
    ...
    return (
      <div className="min-h-dvh w-full text-white relative overflow-x-hidden flex flex-col justify-between" style={{ background: "linear-gradient(180deg, var(--t-bg) 0%, var(--t-surface) 55%, #FFFFFF 100%)" }} data-theme={theme}>
        ...
        <footer className="relative z-10 pt-4 pb-2 text-center font-sans" style={{ borderTop: "1px solid color-mix(in srgb, var(--t-acc) 10%, transparent)" }}>
          <LogoFooterCredit bgColor="transparent" textColor="var(--chic-ink, #3D1418)" />
        </footer>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      {!isCoverOpen && (
        ...
        >
          ...
          <style jsx>{`
            ...
            .petalos-cover-text { color: #FFF1EF; }
            .petalos-cover-text-muted { color: rgba(255,241,239,0.75); }
      ...
      <div className="d-right tpl">
        ...
        <LogoFooterCredit bgColor="var(--t-bg)" textColor="var(--chic-ink, #3D1418)" />
        </div>
      </div>

      {isCoverOpen && <BottomNavPill sections={navSections} variant="moderno" accentColor="#E23B4E" surfaceColor="#FFDEDA" inactiveColor="#3D1418" solid />}
      </div>
  );
}
