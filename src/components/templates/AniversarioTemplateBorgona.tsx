...
/**
 * AniversarioTemplateBorgona.tsx
 *
...
interface AniversarioTemplateBorgonaQuizQuestion {
  pregunta: string;
  opciones: string[];
...
interface AniversarioTemplateBorgonaProps {
...
export function AniversarioTemplateBorgona({ invitation, guest, isPersonalized = false }: AniversarioTemplateBorgonaProps) {
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      ...
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#8C4A54";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #8C4A54";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#522A32" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#522A32" : "#8C4A54";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#130C09", tinta: "#F4F1EA", acento: "#8C4A54", clase: `${anvDisplay.variable} ${anvMono.variable}`, fuente: "var(--anv-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="anv-scroller">
        ...
        <div data-pan="1" data-scroll={scrollVertical ? "vertical" : "lateral"} data-screen-label={tx("invitacion.ubicacion.elLugar")} className="anv-pan">
          <div className="anv-pan-sticky">
            ...
            <AniversarioTemplateBorgonaDots count={LUGAR_PANEL_COUNT} />
          </div>
        </div>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="anv-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #180F0C 60%, #130C09 100%)" }}>
          ...
          {rsvpEnabled ? (
            <div data-xin="1" data-delay="170" data-dist="-90" className="anv-rsvp">
              <AniversarioTemplateBorgonaRsvpCard
        ...
        <div id="album" data-pan="1" data-scroll={scrollVertical || photoPages.length <= 1 ? "vertical" : "lateral"} data-screen-label={tx("invitacion.album.titulo")} className="anv-pan" style={{ "--st-pasos": Math.max(0, photoPages.length - 1) } as React.CSSProperties}>
          <div className="anv-pan-sticky">
            ...
            <AniversarioTemplateBorgonaDots count={photoPages.length} />
          </div>
        </div>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="anv-bank-wrap">
              {pagoTarjetaHabilitado && (
                <BankDetailsCard
                  ...
                  InfoRow={AniversarioTemplateBorgonaInfoRow}
                  CopyField={AniversarioTemplateBorgonaCopyField}
                />
              )}
              {regaloHabilitado && (
                <BankDetailsCard
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="anv-section" style={{ background: "#180F0C" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80">
              <AniversarioTemplateBorgonaQuiz
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#8C4A54" guest={guest} />
      )}
...
function AniversarioTemplateBorgonaCdBox({ refEl, delay, dist, label }: { refEl: React.RefObject<HTMLSpanElement | null>; delay: number; dist: number; label: string }) {
  return (
...
function AniversarioTemplateBorgonaDots({ count = 3 }: { count?: number }) {
...
function AniversarioTemplateBorgonaMedallionCmp({
  label,
  sub,
  arcId,
  arcText,
...
function AniversarioTemplateBorgonaRsvpCard({
  invitationId,
  guestToken,
  guestName,
  maxAdults,
    ...
    <>
      ...
      <div ref={stubRef} className="anv-stub">
        ...
        <div ref={sealRef} className="anv-seal">
          <AniversarioTemplateBorgonaMedallionCmp label="20" sub="" arcId="" arcText="" spin="none" compact />
        </div>
...
function AniversarioTemplateBorgonaInfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="anv-bank-row">
      <div style={{ minWidth: 0, flex: 1 }}>
        <span className="anv-bank-row-label">{label}</span>
...
function AniversarioTemplateBorgonaCoverHalf({
  namesRef,
  kickerRef,
  perfRef,
  kickerText,
...
const GP_CSS = `
  ...
  a { color: #8C4A54; text-decoration: none; }
  a:hover { color: #F3DCE0; }
  ...
  .anv-date-month { font-family: var(--anv-display), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #8C4A54; padding-left: 12%; }

  .anv-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .anv-divider-line { width: 52px; height: 1px; background: #8C4A54; display: inline-block; }
  ...
  .anv-cal-link:hover { color: #8C4A54; }
  ...
  .anv-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #6E5AA8, #4E7F86, #8C4A54, #F3DCE0, #6E5AA8); filter: saturate(.75); }
  ...
  .anv-medallion-label { font-family: var(--anv-display), serif; font-size: 26px; line-height: 1; color: #F3DCE0; }
  .anv-medallion-label-sm { font-family: var(--anv-display), serif; font-size: 16px; color: #F3DCE0; }
  .anv-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .anv-medallion-sub--accent { color: #8C4A54; }
  ...
  .anv-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #8C4A54; opacity: 0.4; font-family: var(--anv-mono), monospace; }
  ...
  .anv-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F3DCE0, transparent); animation: anvScan 6s linear infinite; pointer-events: none; }
  ...
  .anv-cd-num { font-family: var(--anv-display), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F3DCE0; display: inline-block; }
  .anv-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #8C4A54; }
  ...
  .anv-accent-italic { font-style: italic; color: #8C4A54; }
  .anv-accent-serif { font-style: italic; color: #522A32; font-family: var(--anv-display), serif; }
  .anv-accent-serif-2 { color: #522A32; }
  ...
  .anv-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #522A32; margin-top: auto; }
  ...
  .anv-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #522A32; }
  ...
  .anv-crono-time { font-family: var(--anv-mono), monospace; color: #522A32; min-width: 42px; }
  ...
  .anv-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F3DCE0, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .anv-rsvp-payment-total { color: #F3DCE0; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .anv-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #8C4A54; background: transparent; color: #8C4A54; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .anv-rsvp-input:focus { outline: none; border-bottom-color: #8C4A54; }
  .anv-rsvp-btn { width: 100%; padding: 16px; font-family: var(--anv-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #8C4A54, #522A32); border: 1px solid #8C4A54; color: #180F0C; cursor: pointer; }
  .anv-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .anv-rsvp-btn--ghost { background: transparent; color: #8C4A54; }
  .anv-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #8C4A54; margin: 0; }
  ...
  .anv-bank-copy { flex-shrink: 0; font-family: var(--anv-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #8C4A54; background: transparent; color: #8C4A54; cursor: pointer; }
  ...
  .anv-song-input:focus { outline: none; border-bottom-color: #8C4A54; }
  .anv-song-sep { color: #8A8577; flex-shrink: 0; }
  .anv-song-submit { flex-shrink: 0; background: none; border: none; color: #8C4A54; font-family: var(--anv-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .anv-song-submit:hover { color: #F3DCE0; }
  .anv-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .anv-song-error { font-size: 10px; color: #8C4A54; margin-top: 6px; }
  ...
  .anv-quiz-opt { font-family: var(--anv-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(200,164,92,0.4); background: transparent; color: #8C4A54; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .anv-quiz-opt:disabled { cursor: default; }
  .anv-quiz-opt--picked { background: #8C4A54; border-color: #8C4A54; color: #180F0C; }
  .anv-quiz-opt--correct { background: #8C4A54; border-color: #8C4A54; color: #180F0C; }
  ...
  .anv-quiz-result-score { margin: 0 0 6px; font-family: var(--anv-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F3DCE0; }
  .anv-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .anv-final-card { border: 1px solid #8C4A54; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .anv-final-names { font-family: var(--anv-display), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F3DCE0; }
  ...
  .anv-replay { cursor: pointer; color: #8C4A54; }
  ...
  .anv-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F3DCE0, #8C4A54); transition: height 260ms linear; display: block; }
  .anv-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8C4A54; transition: color 500ms ease; }
  ...
  .anv-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(90,58,68,0.4), rgba(140,74,84,0.32), rgba(140,74,84,0.55), rgba(90,58,68,0.4)); filter: blur(64px); opacity: .62; animation: anvFoil 26s linear infinite; }
  .anv-cover-texture { position: absolute; inset: 0; background-image: radial-gradient(circle at 70% 30%, rgba(243,220,224,.14), transparent 45%); }
  ...
  .anv-cover-names { margin: 0; font-family: var(--anv-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F3DCE0; }
  .anv-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#8C4A54, transparent); display: block; }
  ...
  .anv-cover-cta { border: 1px solid #8C4A54; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--anv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  ...
  .anv-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #8C4A54; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
