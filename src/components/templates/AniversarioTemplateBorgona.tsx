        ...
        <section data-tone="dark" data-screen-label="Tu pase" className="anv-section anv-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #180F0C 55%, #130C09 100%)" }}>
          ...
          <div data-xin="1" data-delay="100" data-dist="130" className="anv-final-card">
            <div className="anv-medallion anv-medallion--final">
              <AniversarioTemplateBorgonaMedallionCmp label="20" sub={confirmed ? "CONFIRMADO" : "PENDIENTE"} arcId="anvArc3" arcText={`${namesTitle.toUpperCase()} · ${fechaCorta} · `} spin="reverse" />
            </div>
...
const GP_CSS = `
  ...
  .anv-cover-names { margin: 0; font-family: var(--anv-display), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), 12.5vh); line-height: 0.86; letter-spacing: -0.02em; color: #F3DCE0; }
  ...
  .anv-cover-cta { border: 1px solid #8C4A54; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F3DCE0; font-family: var(--anv-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
