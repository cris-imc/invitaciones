...
/**
 * JewelryBoxTemplate.tsx
 *
 * Portado 1:1 desde el diseño "Jewelry Box" (Colección Storytelling, mis
 * quince años: fondo berenjena oscuro, acento nude/rosa viejo + dorado,
...
export function JewelryBoxTemplate({ invitation, guest, isPersonalized = false }: JewelryBoxTemplateProps) {
  const tx = useTextos();
  ...
  const handleConfirmed = useCallback((data: { attending: boolean; count: number }) => {
    ...
    window.setTimeout(() => {
      if (statusRef.current) {
        ...
        statusRef.current.style.color = "#F7EDE6";
      }
      if (stubRef.current) {
        stubRef.current.style.borderColor = "#E8B4A0";
        stubRef.current.style.boxShadow = "0 0 0 1px rgba(200,164,92,.35), 0 18px 50px -30px #E8B4A0";
      }
      if (sealRef.current) {
    ...
    const frame = () => {
      ...
      if (sc) {
        ...
        pans.forEach((pan, pi) => {
          ...
          dots.forEach((dot, i) => {
            dot.style.background = i === active ? "#D9B063" : "rgba(20,20,27,0.18)";
          });
        });
        ...
        if (cur) {
          ...
          if (railLabelRef.current) railLabelRef.current.style.color = light ? "#D9B063" : "#E8B4A0";
        }
  ...
  if (estadoDelEvento === "POST_EVENT" || estadoDelEvento === "EXPIRED") {
    return (
      <PostEventoStorytelling
        ...
        fotos={livePhotos}
        paleta={{ fondo: "#1E0D26", tinta: "#F4F1EA", acento: "#E8B4A0", clase: `${jwbCormorant.variable} ${jwbMono.variable}`, fuente: "var(--jwb-mono), monospace" }}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div ref={scrollerRef} data-scroller="1" className="jwb-scroller">
        ...
        <section data-tone="dark" data-screen-label="Save the Date" className="jwb-section" style={{ background: "radial-gradient(120% 80% at 50% 0%, #17141F 0%, #24102E 55%, #1E0D26 100%)" }}>
        ...
        <section id="countdown" data-tone="dark" data-screen-label="Countdown" className="jwb-section jwb-section--between" style={{ background: "radial-gradient(100% 60% at 50% 100%, #3A1C48 0%, #22102A 55%, #1E0D26 100%)" }}>
        ...
        {hasFrase && (
        <section id="quote" data-tone="dark" data-screen-label={tx("invitacion.frase.etiqueta")} className="jwb-section" style={{ background: "radial-gradient(130% 90% at 86% 16%, #1C1727 0%, #0C0B11 52%, #1E0D26 100%)" }}>
        ...
        <section data-tone="dark" data-screen-label="Check-in" className="jwb-section" style={{ background: "radial-gradient(110% 70% at 50% 100%, #17141F 0%, #24102E 60%, #1E0D26 100%)" }}>
        ...
        {sugerenciaMusicaHabilitada && (
          <section id="music" data-tone="dark" data-screen-label={tx("invitacion.musica.titulo")} className="jwb-section" style={{ background: "#24102E" }}>
            ...
            <div data-xin="1" data-delay="160" data-dist="-80" className="jwb-eq">
              {[0, 0.18, 0.36, 0.54, 0.72].map((delay, i) => (
                <span key={i} className="jwb-eq-bar" style={{ animationDelay: `${delay}s`, background: i === 2 ? "#F7EDE6" : "#E8B4A0" }} />
              ))}
            </div>
        ...
        {showBankSection && (
          <section id="banco" data-tone="dark" data-screen-label={tx("invitacion.regalos.titulo")} className="jwb-section" style={{ background: "#24102E" }}>
        ...
        {quizEnabled && (
          <section id="quiz" data-tone="dark" data-screen-label="Quiz" className="jwb-section" style={{ background: "#24102E" }}>
        ...
        <section data-tone="dark" data-screen-label="Tu pieza" className="jwb-section jwb-section--between" style={{ padding: "96px max(30px, calc((100% - 560px) / 2)) 48px max(24px, calc((100% - 560px) / 2))", background: "radial-gradient(120% 70% at 50% 100%, #17141F 0%, #24102E 55%, #1E0D26 100%)" }}>
      ...
      {mounted && isPersonalized && guest && isCoverOpen && (
        <BurbujaPase acento="#E8B4A0" guest={guest} />
      )}
...
const JWB_CSS = `
  ...
  a { color: #E8B4A0; text-decoration: none; }
  a:hover { color: #F7EDE6; }
  ...
  .jwb-hero-photo-section { min-height: calc(var(--vh, 1vh) * 100); position: relative; overflow: hidden; background: #1E0D26; }
  ...
  @media (min-width: 768px) {
    .jwb-hero-photo-frame { inset: 64px max(24px, calc((100% - 900px) / 2)); border: 1px solid rgba(232,180,160,.3); }
  ...
  .jwb-date-month { font-family: var(--jwb-cormorant), serif; font-style: italic; font-size: clamp(50px, 18vw, 104px); line-height: 0.9; color: #E8B4A0; padding-left: 12%; }

  .jwb-divider { display: flex; align-items: center; gap: 16px; font-size: 10px; letter-spacing: 0.26em; color: #8A8577; }
  .jwb-divider-line { width: 52px; height: 1px; background: #E8B4A0; display: inline-block; }
  ...
  .jwb-cal-link:hover { color: #E8B4A0; }
  ...
  .jwb-gem { position: absolute; inset: -6px; border-radius: 50%; box-shadow: 0 0 24px 4px rgba(232,180,160,.7); animation: gemFlash 4s ease-in-out infinite; pointer-events: none; }
  .jwb-medallion-ring { position: absolute; inset: 0; border-radius: 50%; background: conic-gradient(from 0deg, #4A2456, #2A1436, #E8B4A0, #F7EDE6, #4A2456); filter: saturate(.75); }
  ...
  .jwb-medallion-label { font-family: var(--jwb-cormorant), serif; font-size: 26px; line-height: 1; color: #F7EDE6; }
  .jwb-medallion-label-sm { font-family: var(--jwb-cormorant), serif; font-size: 16px; color: #F7EDE6; }
  .jwb-medallion-sub { font-size: 6.5px; letter-spacing: 0.24em; color: #8A8577; }
  .jwb-medallion-sub--accent { color: #E8B4A0; }
  .jwb-medallion-arc { position: absolute; inset: -14%; }
  .jwb-medallion-arc-text { font-size: 7px; letter-spacing: 1.6px; fill: #E8B4A0; opacity: 0.4; font-family: var(--jwb-mono), monospace; }

  .jwb-scan-grid { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, rgba(200,164,92,0.06) 0 1px, transparent 1px 5px); pointer-events: none; }
  .jwb-scanline { position: absolute; left: 0; right: 0; top: 30%; height: 2px; background: linear-gradient(90deg, transparent, #F7EDE6, transparent); animation: gpScan 6s linear infinite; pointer-events: none; }

  .jwb-cd-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; position: relative; }
  .jwb-cd-box { border: 1px solid #2A2417; background: rgba(20,20,27,0.72); padding: 18px 15px; display: flex; flex-direction: column; gap: 6px; overflow: hidden; }
  .jwb-cd-num { font-family: var(--jwb-cormorant), serif; font-size: clamp(48px, 14vw, 80px); line-height: 0.82; color: #F6EFDD; display: inline-block; }
  .jwb-cd-label { font-size: 9px; letter-spacing: 0.3em; color: #E8B4A0; }
  .jwb-perf-strip { height: 12px; position: relative; background: radial-gradient(circle at 6px 50%, #1E0D26 3.4px, transparent 3.8px) 0 0/12px 12px repeat-x; opacity: .85; }
  ...
  .jwb-glow-blob { position: absolute; right: -26%; top: 4%; width: 82vw; max-width: 540px; aspect-ratio: 1; border-radius: 50%; background: conic-gradient(from 200deg, rgba(74,36,86,0.4), rgba(217,176,99,0.24), rgba(232,180,160,0.36), rgba(74,36,86,0.4)); filter: blur(80px); opacity: .4; animation: gpFoil 30s linear infinite; }
  ...
  .jwb-accent-italic { font-style: italic; color: #E8B4A0; }
  .jwb-accent-serif { font-style: italic; color: #D9B063; font-family: var(--jwb-cormorant), serif; }
  .jwb-accent-serif-2 { color: #D9B063; }
  ...
  .jwb-seguir { position: relative; display: flex; align-items: center; gap: 10px; font-size: 9px; letter-spacing: 0.26em; color: #D9B063; margin-top: auto; }
  ...
  .jwb-link-cta { font-size: 10px; letter-spacing: 0.24em; color: #D9B063; }
  ...
  .jwb-crono-time { font-family: var(--jwb-mono), monospace; color: #D9B063; min-width: 42px; }
  ...
  .jwb-stub { position: relative; overflow: hidden; border: 1px solid #2A2417; background: rgba(20,20,27,0.7); padding: 20px; display: flex; flex-direction: column; gap: 16px; transition: border-color 600ms ease, box-shadow 600ms ease; }
  ...
  .jwb-beam { position: absolute; left: 0; right: 0; top: 0; height: 2px; background: linear-gradient(90deg, transparent, #F7EDE6, transparent); opacity: 0; transition: transform 900ms cubic-bezier(.16,1,.3,1), opacity 300ms ease; }
  ...
  .jwb-rsvp-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid rgba(232,180,160,0.18); font-size: 10.5px; letter-spacing: 0.18em; color: #8A8577; }
  ...
  .jwb-rsvp-payment-total { color: #F7EDE6; font-size: 13px; letter-spacing: 0.02em; font-weight: 600; }
  ...
  .jwb-rsvp-stepper button { width: 26px; height: 26px; border-radius: 50%; border: 1px solid #E8B4A0; background: transparent; color: #E8B4A0; font-size: 14px; line-height: 1; cursor: pointer; }
  ...
  .jwb-rsvp-input { background: transparent; border: none; border-bottom: 1px solid rgba(232,180,160,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.02em; padding: 4px 2px; text-align: right; max-width: 55%; }
  .jwb-rsvp-input::placeholder { color: #6E6A5D; }
  .jwb-rsvp-input:focus { outline: none; border-bottom-color: #E8B4A0; }
  .jwb-rsvp-btn { width: 100%; padding: 16px; font-family: var(--jwb-mono), monospace; font-size: 11px; font-weight: 600; letter-spacing: 0.22em; text-transform: uppercase; background: linear-gradient(180deg, #E8B4A0, #D9B063); border: 1px solid #E8B4A0; color: #0B0B10; cursor: pointer; }
  .jwb-rsvp-btn:disabled { opacity: 0.6; cursor: default; }
  .jwb-rsvp-btn--ghost { background: transparent; color: #E8B4A0; }
  .jwb-rsvp-error { font-size: 10px; letter-spacing: 0.06em; color: #E8B4A0; margin: 0; }
  ...
  .jwb-bank-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 10px 0; border-bottom: 1px solid rgba(232,180,160,0.18); }
  ...
  .jwb-bank-copy { flex-shrink: 0; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid #E8B4A0; background: transparent; color: #E8B4A0; cursor: pointer; }
  .jwb-bank-copy:hover { background: rgba(232,180,160,0.12); }
  ...
  .jwb-song-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; border-bottom: 1px solid rgba(232,180,160,0.3); padding-bottom: 12px; }
  ...
  .jwb-song-input { background: transparent; border: none; border-bottom: 1px solid rgba(232,180,160,0.3); color: #F4F1EA; font-family: var(--jwb-mono), monospace; font-size: 12px; padding: 6px 2px; min-width: 0; flex: 1; }
  .jwb-song-input::placeholder { color: #6E6A5D; }
  .jwb-song-input:focus { outline: none; border-bottom-color: #E8B4A0; }
  .jwb-song-sep { color: #8A8577; flex-shrink: 0; }
  .jwb-song-submit { flex-shrink: 0; background: none; border: none; color: #E8B4A0; font-family: var(--jwb-mono), monospace; font-size: 10px; letter-spacing: 0.2em; cursor: pointer; }
  .jwb-song-submit:hover { color: #F7EDE6; }
  .jwb-song-submit:disabled { color: #6E6A5D; cursor: default; }
  .jwb-song-error { font-size: 10px; color: #E8B4A0; margin-top: 6px; }
  ...
  .jwb-quiz-opt { font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.06em; padding: 10px 16px; border: 1px solid rgba(232,180,160,0.4); background: transparent; color: #E8B4A0; cursor: pointer; transition: background 160ms ease, color 160ms ease, border-color 160ms ease; }
  .jwb-quiz-opt:disabled { cursor: default; }
  .jwb-quiz-opt--picked { background: #E8B4A0; border-color: #E8B4A0; color: #0B0B10; }
  .jwb-quiz-opt--correct { background: #E8B4A0; border-color: #E8B4A0; color: #0B0B10; }
  .jwb-quiz-opt--wrong { border-color: #8A6A6A; color: #C99; opacity: 0.6; }
  .jwb-quiz-result { padding-top: 18px; border-top: 1px solid rgba(232,180,160,0.2); }
  .jwb-quiz-result-score { margin: 0 0 6px; font-family: var(--jwb-mono), monospace; font-size: 11px; letter-spacing: 0.16em; color: #F7EDE6; }
  .jwb-quiz-result-stat { margin: 0; font-size: 11.5px; line-height: 1.5; color: #A8A292; }

  .jwb-final-card { border: 1px solid #E8B4A0; padding: 30px 24px; display: flex; flex-direction: column; gap: 22px; align-items: center; text-align: center; position: relative; }
  .jwb-final-names { font-family: var(--jwb-cormorant), serif; font-size: clamp(34px, 10vw, 46px); line-height: .9; color: #F7EDE6; }
  ...
  .jwb-replay { cursor: pointer; color: #E8B4A0; }
  .jwb-footer-credit { display: flex; justify-content: center; padding-top: 8px; opacity: 0.7; }

  .jwb-rail { position: absolute; right: 0; top: 0; bottom: 0; width: 34px; z-index: 4; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 20px 0; opacity: 0; transition: opacity 700ms ease; pointer-events: none; border-left: 1px solid rgba(200,164,92,0.14); }
  .jwb-rail-top { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #8A8577; transition: color 500ms ease; }
  .jwb-rail-line { flex: 1; width: 1px; margin: 16px 0; background: rgba(200,164,92,0.16); position: relative; }
  .jwb-rail-bar { position: absolute; left: -1px; top: 0; width: 3px; height: 0%; background: linear-gradient(#F7EDE6, #E8B4A0); transition: height 260ms linear; display: block; }
  .jwb-rail-label { writing-mode: vertical-rl; font-size: 8px; letter-spacing: 0.34em; color: #E8B4A0; transition: color 500ms ease; }
  ...
  .jwb-cover-inner { position: absolute; left: 0; right: 0; top: 0; height: 200%; overflow: hidden; background: radial-gradient(120% 70% at 50% 8%, #2A1436 0%, #24102E 46%, #1E0D26 100%); }
  .jwb-cover-half--bottom .jwb-cover-inner { top: auto; bottom: 0; }
  .jwb-cover-glow { position: absolute; left: 50%; top: 6%; width: 190%; aspect-ratio: 1; transform: translate(-50%, -14%); border-radius: 50%; background: conic-gradient(from 200deg, rgba(74,36,86,0.6), rgba(217,176,99,0.35), rgba(232,180,160,0.55), rgba(74,36,86,0.6)); filter: blur(64px); opacity: .62; animation: gpFoil 26s linear infinite; }
  ...
  .jwb-cover-names { margin: 0; font-family: var(--jwb-cormorant), serif; font-weight: 400; font-size: min(clamp(48px, 16vw, 96px), calc(12.5vh * var(--esc, 1))); line-height: 0.86; letter-spacing: -0.02em; color: #F7EDE6; }
  .jwb-cover-rule { width: 1px; height: clamp(16px, 4vh, 44px); background: linear-gradient(#E8B4A0, transparent); display: block; }
  ...
  .jwb-cover-cta { border: 1px solid #E8B4A0; background: linear-gradient(100deg, rgba(200,164,92,0.08), rgba(232,214,168,0.2), rgba(200,164,92,0.08)); color: #F6EFDD; font-family: var(--jwb-mono), monospace; font-size: 12px; letter-spacing: 0.26em; padding: clamp(13px, 2.1vh, 19px) 0; text-align: center; width: 100%; }
  .jwb-cover-cta--btn { cursor: pointer; border-radius: 0; }
  @media (hover: hover) {
    .jwb-cover-cta--btn:hover { background: linear-gradient(100deg, rgba(232,214,168,0.34), rgba(246,239,221,0.5), rgba(232,214,168,0.34)); color: #0B0B0F; }
  }
  ...
  .jwb-lightbox-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; border-radius: 50%; border: 1px solid #E8B4A0; background: rgba(0,0,0,0.4); color: #F4F1EA; font-size: 18px; line-height: 1; cursor: pointer; display: flex; align-items: center; justify-content: center; }
