...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-[#5C8A7A]/20 last:border-b-0">
      <div className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold text-[#3E6152] uppercase tracking-wider mb-0.5">{label}</span>
  ...
  return (
    <>
      <style>{`
        ...
        /* RSVP Custom Aesthetics for ElegantTemplateGreen */
        ...
        #rsvp.section.dark {
          background-color: #3E6152 !important; /* Dark brown/black */
        ...
        #songs.d-sec.dark input {
          ...
          background-color: #E5ECE9 !important; /* Pastel */
        ...
        #songs.d-sec.dark button[type="submit"] {
          background-color: #A8C1B7 !important;
          color: #18241D !important;
          border-radius: 0 !important;
          border: none !important;
          padding: 14px 24px !important;
      ...
      {mounted && isPersonalized && guest && isCoverOpen && createPortal(
        <div 
          ...
          className={`fixed top-3 left-1/2 -translate-x-1/2 z-[99999] transition-all duration-500 cursor-pointer overflow-hidden border border-[#5C8A7A]/40 shadow-md ${isTicketMaximized ? 'bg-[#F9F7F1]/95 backdrop-blur-md rounded-full w-[90%] max-w-sm px-5 py-2.5' : 'bg-[#1A2B33]/95 backdrop-blur-md rounded-full px-5 py-2'}`}
        >
          {isTicketMaximized ? (
            <div className="flex items-center justify-between w-full animate-in fade-in duration-300">
              <div className="flex flex-col text-left">
                <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#5C8A7A] leading-none mb-1" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.paseEspecial")}</span>
                ...
                {guest.mesas && guest.mesas.length > 0 && (
                  <span className=" text-[8px] font-semibold uppercase tracking-[0.2em] text-[#5C8A7A] leading-none mt-1.5" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{guest.mesas.join(" · ")}</span>
                )}
              </div>
            ...
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              ...
              <span className="text-[#E5ECE9]  text-[10px] font-semibold tracking-wider uppercase" style={{ fontFamily: "var(--font-body-custom, var(--font-inter))" }}>{tx("invitacion.pase.pase")}</span>
            </div>
          )}
        </div>,
        document.body
      ...
      <div className="d-right tpl">
          ...
          <div className="px-8 pt-16 pb-12 text-left bg-[#F9F7F1] z-10 relative">
            ...
            <h1 className="text-[4rem] font-light text-[#2C2C2C] leading-[1.0] mb-8" style={{ fontFamily: 'var(--font-title, var(--font-cormorant)), serif' }}>
              {em ? (
                <>
                  <span className="block">{title.slice(0, title.indexOf(em)).trim()}</span>
                  <span className="block"><em style={{ fontFamily: 'var(--font-cormorant), serif', fontStyle: 'italic', color: '#5C8A7A' }}>&amp;</em> {em.replace('& ', '').trim()}</span>
                </>
              ) : (
                <span className="block">{title}</span>
              )}
        ...
        {(Boolean(invitation.frasePersonalizadaHabilitada) && Boolean(invitation.frasePersonalizadaTexto)) ? (
          <SectionWrapper id="quote" delay={100} className="w-full bg-[#E5ECE9] py-24 px-6 md:px-12 flex items-center justify-center">
          ...
          <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                ...
                <div>
                  ...
                  {Boolean(invitation.ceremoniaHora) && (
                    <p className="flex items-center gap-2 text-[#4A5568] font-medium text-sm sm:text-base mb-3">
                      <Clock className="w-4 h-4 text-[#3E6152]" /> {String(invitation.ceremoniaHora)} hs
                    </p>
                  )}
                  {Boolean(invitation.ceremoniaDireccion) && (
                    <p className="flex items-center gap-2 text-[#4A5568] font-medium text-sm sm:text-base mb-4">
                      <MapPin className="w-4 h-4 text-[#3E6152]" /> {String(invitation.ceremoniaDireccion)}
                    </p>
                  )}
            ...
            {(lugarNombre || direccion) && (
              <div className="bg-[#FAF8F5] border border-[#5C8A7A]/20 rounded-xl p-5 sm:p-7 mb-10 shadow-sm transition-all hover:shadow-md">
                ...
                {hora && (
                  <p className="flex items-center gap-2 text-[#4A5568] font-medium text-sm sm:text-base mb-3">
                    <Clock className="w-4 h-4 text-[#3E6152]" /> {hora} hs
                  </p>
                )}
                {direccion && (
                  <p className="flex items-center gap-2 text-[#4A5568] font-medium text-sm sm:text-base mb-4">
                    <MapPin className="w-4 h-4 text-[#3E6152]" /> {direccion}
                  </p>
                )}
                {mapUrl && (
        ...
        {showGiftSection && (
          <SectionWrapper id="banco" delay={200} className="w-full bg-[#F9F7F1] py-20 px-6 md:px-12 overflow-hidden">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
                ...
                <p className="t-kicker mb-10 text-[#5C8A7A]">
        ...
        {triviaHabilitada && triviaPreguntas.length > 0 && (
          <SectionWrapper id="quiz" delay={300} className="w-full bg-[#E5ECE9] py-20 px-6 md:px-12">
            <div className="w-full max-w-[340px] sm:max-w-xl mx-auto text-left">
              <div className="flex justify-center mb-4">
